...
pub fn init_tracing_subscriber(otlp_endpoint: Option<String>) -> OtelGuard {
    ...
    let mut dirs = if cfg!(feature = "prod") {
        "pilotcodi=info,otel=debug,http_api_bindings=info,llama_cpp_server=info".into()
    } else {
        "pilotcodi=debug,otel=debug,http_api_bindings=debug,llama_cpp_server=debug".into()
    };
