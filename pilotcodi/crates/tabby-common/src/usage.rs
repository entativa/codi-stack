...
static USAGE_API_ENDPOINT: &str = "https://app.pilotcodiml.com/api/usage";
        ...
        if fs::metadata(usage_id_file()).is_err() {
            ...
            InfoMessage::print_messages(&[
                InfoMessage::new("TELEMETRY", HeaderFormat::BoldBlue, &[
                    ...
                    "information, read https://pilotcodi.pilotcodiml.com/docs/configuration#usage-collection",
                    "",
                    "We will not see or collect any code in your development process."
                ]),
                InfoMessage::new("Welcome to PilotCodi!", HeaderFormat::BoldWhite, &[
                    "If you have any questions or would like to engage with the PilotCodi team, please join us on Slack",
                    "(https://links.pilotcodiml.com/join-slack-terminal)."
                ])
            ]);
        }
...
lazy_static! {
    static ref TRACKER: Option<UsageTracker> = {
        if std::env::var("PILOTCODI_DISABLE_USAGE_COLLECTION").is_ok() {
            None
        } else {
