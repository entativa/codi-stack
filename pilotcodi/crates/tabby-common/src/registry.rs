...
async fn load_remote_registry(registry: &str) -> Result<Vec<ModelInfo>> {
    ...
    let model_info = client
        .get(format!(
            "https://raw.githubusercontent.com/{registry}/registry-pilotcodi/main/models.json"
        ))
        .send()
        .await
    ...
    // We don't want to fail if the PilotCodiML directory already exists,
...
// root: ~/.pilotcodi/models/PilotCodiML
//
...
impl ModelRegistry {
    ...
    // get_model_store_dir returns {root}/{name}/ggml, e.g.. ~/.pilotcodi/models/PilotCodiML/StarCoder-1B/ggml
    pub fn get_model_store_dir(&self, name: &str) -> PathBuf {
        self.get_model_dir(name).join("ggml")
    }

    // get_model_dir returns {root}/{name}, e.g. ~/.pilotcodi/models/PilotCodiML/StarCoder-1B
    ...
    pub fn save_model_info(&self, name: &str) {
        ...
        let path = self.get_model_dir(name).join("pilotcodi.json");
    ...
    pub fn get_model_info(&self, name: &str) -> &ModelInfo {
        match self.models.iter().find(|x| x.name == name) {
            ...
            None => panic!(
                "Invalid `model_id` <{}/{}>; please consult https://github.com/{}/registry-pilotcodi for the correct `model_id`.",
...
mod tests {
    ...
    use crate::path::set_pilotcodi_root;

    #[tokio::test]
    async fn test_model_migration() {
        let root = TempDir::default();
        set_pilotcodi_root(root.to_path_buf());

        let registry = ModelRegistry::new("PilotCodiML").await;
