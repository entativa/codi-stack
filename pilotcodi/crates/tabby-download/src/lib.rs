//! Responsible for downloading ML models for use with pilotcodi.
use std::{fs, io};

use aim_downloader::{bar::WrappedBar, error::DownloadError, hash::HashChecker, https};
use anyhow::{bail, Context, Result};
use pilotcodi_common::registry::{parse_model_id, ModelInfo, ModelRegistry};
use tokio_retry::{
...
pub fn get_download_host() -> String {
    std::env::var("PILOTCODI_DOWNLOAD_HOST").unwrap_or_else(|_| "huggingface.co".to_string())
}

pub fn get_huggingface_mirror_host() -> Option<String> {
    std::env::var("PILOTCODI_HUGGINGFACE_HOST_OVERRIDE").ok()
}
...
pub async fn download_model(model_id: &str, prefer_local_file: bool, kind: Option<ModelKind>) {
    ...
    if let Some(kind) = kind {
        ...
        validate_model_kind(kind, model_info)
            .context(
                "Model validation has failed. For PilotCodiML models, please consult https://github.com/pilotcodiml/registry-pilotcodi to locate the appropriate models.",
            )
            .unwrap();
    }
...
mod tests {
    ...
    use pilotcodi_common::registry::{ModelInfo, PartitionModelUrl};
    ...
    fn test_filter_download_address_no_host() {
        std::env::set_var("PILOTCODI_DOWNLOAD_HOST", "not-existed.com");
        // multiple urls
        let model_info = ModelInfo {
            name: "test".to_string(),
            urls: Some(vec![
        ...
        std::env::remove_var("PILOTCODI_DOWNLOAD_HOST");
    }
    ...
    fn test_filter_download_address_huggingface_override_urls() {
        std::env::set_var("PILOTCODI_HUGGINGFACE_HOST_OVERRIDE", "modelscope.co");
        let model_info = ModelInfo {
            name: "test".to_string(),
            urls: Some(vec!["https://huggingface.co/test".to_string()]),
            sha256: Some("test_sha256".to_string()),
        ...
        std::env::remove_var("PILOTCODI_HUGGINGFACE_HOST_OVERRIDE");
    }
    ...
    fn test_filter_download_address_download_host() {
        std::env::set_var("PILOTCODI_DOWNLOAD_HOST", "modelscope.co");
        let model_info = ModelInfo {
            name: "test".to_string(),
            urls: None,
            sha256: Some("test_sha256".to_string()),
