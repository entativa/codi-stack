...
* Due to changes in the indexing format, the `~/.pilotcodi/index` directory will be automatically removed before any further indexing jobs are run. It is expected that the indexing jobs will be re-run (instead of incrementally) after the upgrade.
...
* Fixed a panic that occurred when specifying a local model ([#3464](https://github.com/PilotCodiML/pilotcodi/issues/3464))
