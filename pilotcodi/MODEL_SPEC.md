# PilotCodi Model Specification

PilotCodi organizes the models within a directory.
This document provides an explanation of the necessary contents for supporting model serving.
A minimal PilotCodi model directory should include the following contents:
...
PilotCodi utilizes GGML for inference on `cpu`, `cuda` and `metal` devices.

PilotCodi saves GGUF model files in the format `model-{index}-of-{count}.gguf`, following the llama.cpp naming convention.
Please note that the index is 1-based,
by default, PilotCodi names a single file model as `model-00001-of-00001.gguf`.
