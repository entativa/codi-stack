...
3. Clone the PilotCodi repository and install its dependencies:
    ```bash
    git clone https://github.com/PilotCodiML/pilotcodi.git
    cd pilotcodi
    pnpm install
    cd clients/eclipse
    pnpm turbo build
    ```
    This process will also build the `pilotcodi-agent` and place it into the `clients/eclipse/plugin/pilotcodi-agent/` directory.
...
2. Check the `com.pilotcodiml.features.pilotcodi4eclipse`, then select `Archive file` with a specified file path, and click `Finish`.
