...
- Removed the deprecated `PilotCodiAgent` interface and related types.
...
- Introduced `pilotcodi/status` and `pilotcodi/config` methods, deprecating `pilotcodi/agent` methods.
...
- The pilotcodi-agent will only support running as a language server starting from version 1.7.0.
