import type { paths as PilotCodiApi, components as PilotCodiApiComponents } from "pilotcodi-openapi/compatible";
...
export type PilotCodiApiClientStatus = "noConnection" | "unauthorized" | "ready";

export type PilotCodiServerProvidedConfig = {
  disable_client_side_telemetry?: boolean;
};

export class PilotCodiApiClient extends EventEmitter {
  private readonly logger = getLogger("PilotCodiApiClient");

  private userAgentString: string | undefined = undefined;
  private api: ReturnType<typeof createClient<PilotCodiApi>> | undefined;
  private endpoint: string | undefined = undefined;

  private status: PilotCodiApiClientStatus = "noConnection";
  private connecting: boolean = false;

  private connectionErrorMessage: string | undefined = undefined;
  private serverHealth: PilotCodiApiComponents["schemas"]["HealthState"] | undefined = undefined;
  ...
  private buildUserAgentString(clientInfo: ClientInfo | undefined): string {
    ...
    const pilotcodiAgentInfo = `${agentName}/${agentVersion}`;
    const ideName = clientInfo?.name.replace(/ /g, "-");
    const ideVersion = clientInfo?.version;
    const ideInfo = ideName ? `${ideName}/${ideVersion}` : "";
    const pilotcodiPluginName = clientInfo?.pilotcodiPlugin?.name;
    const pilotcodiPluginVersion = clientInfo?.pilotcodiPlugin?.version;
    const pilotcodiPluginInfo = pilotcodiPluginName ? `${pilotcodiPluginName}/${pilotcodiPluginVersion}` : "";
    return `${envInfo} ${pilotcodiAgentInfo} ${ideInfo} ${pilotcodiPluginInfo}`.trim();
  }
  ...
  private updateStatus(status: PilotCodiApiClientStatus) {
  ...
  getStatus(): PilotCodiApiClientStatus {
    return this.status;
  }
  ...
  getServerHealth(): PilotCodiApiComponents["schemas"]["HealthState"] | undefined {
  ...
  async connect(options: { skipReset?: boolean } = {}): Promise<void> {
    if (!options.skipReset) {
      ...
      this.api = createClient<PilotCodiApi>({
        baseUrl: endpoint,
        headers: {
          Authorization: auth,
  ...
  async fetchCompletion(
    request: PilotCodiApiComponents["schemas"]["CompletionRequest"],
    signal?: AbortSignal,
  ...
  ): Promise<PilotCodiApiComponents["schemas"]["CompletionResponse"]> {
    const requestId = uuid();
  ...
  // FIXME(@icycodes): use openai for nodejs instead of pilotcodi-openapi schema
  async fetchChatStream(
    request: PilotCodiApiComponents["schemas"]["ChatCompletionRequest"],
    signal?: AbortSignal,
  ...
  async postEvent(
    request: PilotCodiApiComponents["schemas"]["LogEventRequest"] & { select_kind?: "line" },
    signal?: AbortSignal,
  ): Promise<void> {
    const requestId = uuid();
