...
import type {
  ...
  Config as PilotCodiLspConfig,
} from "../protocol";
import type { PilotCodiServerProvidedConfig } from "../http/pilotcodiApiClient";
...
function mergeConfig(
  ...
  serverProvided?: PilotCodiServerProvidedConfig,
): ConfigData {
...
export class Configurations extends EventEmitter implements Feature {
  ...
  private configFile: ConfigFile | undefined = undefined; // config from `~/.pilotcodi-client/agent/config.toml`
  private clientProvided: ClientProvidedConfig = {}; // config from lsp client
  private serverProvided: PilotCodiServerProvidedConfig = {}; // config fetched from server and saved in dataStore
  private mergedConfig: ConfigData = defaultConfigData; // merged config from (default, configFile, clientProvided, serverProvided)

  private configForLsp: PilotCodiLspConfig = { server: defaultConfigData["server"] }; // config for lsp client
  ...
  private pickStoredServerProvidedConfig(data: Partial<StoredData>): PilotCodiServerProvidedConfig {
  ...
  async initialize(
    ...
    if (clientCapabilities.pilotcodi?.configDidChangeListener) {
      this.on("configForLspUpdated", (config: PilotCodiLspConfig) => {
  ...
  async initialized(connection: Connection): Promise<void> {
    if (this.clientCapabilities?.pilotcodi?.configDidChangeListener) {
  ...
  getConfigForLsp(): PilotCodiLspConfig {
  ...
  async updateServerProvidedConfig(config: PilotCodiServerProvidedConfig, save: boolean = false) {
