...
export class AnonymousUsageLogger {
  ...
  async initialize(clientInfo: ClientInfo | undefined) {
    ...
    const endpoint = "https://app.pilotcodiml.com/api";
  ...
  private updateClientInfoProperties(clientInfo: ClientInfo | undefined) {
    const properties = {
      ...
      pilotcodi_plugin: clientInfo?.pilotcodiPlugin ?? {
        name: agentName,
  ...
  private getClientType(clientInfo: ClientInfo | undefined): string {
    ...
    if (clientInfo.pilotcodiPlugin?.name.includes("vscode")) {
      return "vscode";
    } else if (clientInfo.pilotcodiPlugin?.name.includes("intellij")) {
      return "intellij";
    } else if (clientInfo.pilotcodiPlugin?.name.includes("vim")) {
      return "vim";
    }
