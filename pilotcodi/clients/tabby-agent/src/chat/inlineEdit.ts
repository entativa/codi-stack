...
export class ChatEditProvider implements Feature {
  ...
  async fetchFileContent(uri: URI, range: Range | undefined, token: CancellationToken) {
    ...
    } else if (this.clientCapabilities?.pilotcodi?.workspaceFileSystem) {
  ...
  async provideEdit(params: ChatEditParams, token: CancellationToken): Promise<ChatEditToken | null> {
    ...
    const readableStream = await this.chat.pilotcodiApiClient.fetchChatStream(
      {
        messages,
        model: "",
        stream: true,
    ...
    const editId = "pilotcodi-" + cryptoRandomString({ length: 6, type: "alphanumeric" });
    this.currentEdit = {
      id: editId,
  ...
  async resolveEdit(params: ChatEditResolveParams): Promise<boolean> {
    ...
    for (; line < document.lineCount; line++) {
      ...
      const match = /^>>>>>>> (pilotcodi-[0-9|a-z|A-Z]{6}) (\[.*\])/g.exec(lineText);
      markers = match?.[2];
      if (markers) {
        break;
      }
