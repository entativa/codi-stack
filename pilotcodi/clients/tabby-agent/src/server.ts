...
import { PilotCodiApiClient } from "./http/pilotcodiApiClient";
...
export class Server {
  private readonly logger = getLogger("PilotCodiLSP");
  ...
  private readonly pilotcodiApiClient = new PilotCodiApiClient(this.configurations, this.anonymousUsageLogger);
  ...
  private readonly chatFeature = new ChatFeature(this.pilotcodiApiClient);
  ...
  private async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    await this.pilotcodiApiClient.initialize(clientInfo);
  ...
  private async shutdown() {
    ...
    await this.pilotcodiApiClient.shutdown();
