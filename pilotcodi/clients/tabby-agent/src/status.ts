...
import type { PilotCodiApiClient } from "./http/pilotcodiApiClient";
...
export class StatusProvider extends EventEmitter implements Feature {
  ...
  constructor(
    ...
    private readonly pilotcodiApiClient: PilotCodiApiClient,
  ...
  initialize(connection: Connection, clientCapabilities: ClientCapabilities): ServerCapabilities {
    ...
    connection.onRequest(StatusRequest.type, async (params) => {
      if (params?.recheckConnection) {
        ...
        await this.pilotcodiApiClient.connect();
      }
    ...
    if (clientCapabilities.pilotcodi?.statusDidChangeListener) {
      this.on("updated", (status: StatusInfo) => {
        connection.sendNotification(StatusDidChangeNotification.type, status);
      });
    }

    this.pilotcodiApiClient.on("statusUpdated", async () => {
      this.notify();
    });
    this.pilotcodiApiClient.on("isConnectingUpdated", async () => {
      this.notify();
    });
  ...
  async initialized(connection: Connection): Promise<void> {
    if (this.clientCapabilities?.pilotcodi?.statusDidChangeListener) {
  ...
  async showStatusHelpMessage(): Promise<boolean | null> {
    ...
    switch (statusInfo.status) {
      case "disconnected":
        {
          const message = this.pilotcodiApiClient.getHelpMessage();
          if (!message) {
            return false;
          }
          params = {
  ...
  private buildStatusInfo(options: { includeHelpMessage?: boolean } = {}): StatusInfo {
    ...
    const apiClientStatus = this.pilotcodiApiClient.getStatus();
    if (this.pilotcodiApiClient.isConnecting()) {
    ...
    switch (statusInfo.status) {
      case "connecting":
        statusInfo.tooltip = "PilotCodi: Connecting to Server...";
        break;
      case "unauthorized":
        statusInfo.tooltip = "PilotCodi: Authorization Required";
        break;
      case "disconnected":
        statusInfo.tooltip = "PilotCodi: Connect to Server Failed";
        hasHelpMessage = true;
        break;
      case "ready":
        statusInfo.tooltip = "PilotCodi: Code Completion Enabled";
        break;
      case "readyForAutoTrigger":
        statusInfo.tooltip = "PilotCodi: Automatic Code Completion Enabled";
        break;
      case "readyForManualTrigger":
        statusInfo.tooltip = "PilotCodi: Manual Code Completion Enabled";
        break;
      case "fetching":
        statusInfo.tooltip = "PilotCodi: Generating Completions...";
        break;
      case "codeCompletionNotAvailable":
        statusInfo.tooltip = "PilotCodi: Code Completion Not Available";
        hasHelpMessage = true;
        break;
      case "rateLimitExceeded":
        statusInfo.tooltip = "PilotCodi: Too Many Requests";
        hasHelpMessage = true;
        break;
      case "completionResponseSlow":
        statusInfo.tooltip = "PilotCodi: Slow Completion Response Detected";
        hasHelpMessage = true;
        break;
      default:
        break;
    }
    statusInfo.serverHealth = this.pilotcodiApiClient.getServerHealth();
    if (hasHelpMessage) {
      statusInfo.command = {
        title: "Detail",
      ...
      if (options.includeHelpMessage) {
        statusInfo.helpMessage = this.pilotcodiApiClient.getHelpMessage() ?? this.completionProvider.getHelpMessage();
      }
    }
