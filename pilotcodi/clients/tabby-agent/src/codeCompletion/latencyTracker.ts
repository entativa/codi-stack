import type { components as PilotCodiApiComponents } from "pilotcodi-openapi/compatible";
...
export function buildHelpMessageForLatencyIssue(
  ...
  data?:
    | {
        latencyStatistics: LatencyStatistics;
        endpoint?: string;
        serverHealth?: PilotCodiApiComponents["schemas"]["HealthState"];
      }
    | undefined,
  ...
  if (serverHealth?.device === "cpu" && serverHealth?.model?.match(/[0-9.]+B$/)) {
    helpMessageForRunningLargeModelOnCPU +=
      `Your PilotCodi server is running model <i>${serverHealth?.model}</i> on CPU. ` +
      ...
      "You can find a list of recommend models in the <a href='https://pilotcodi.pilotcodiml.com/docs/'>online documentation</a>.<br/>";
  }
  ...
  if (helpMessageForRunningLargeModelOnCPU.length == 0) {
    ...
    commonHelpMessage +=
      "Please consider trying smaller models. You can find a list of recommend models in the <a href='https://pilotcodi.pilotcodiml.com/docs/'>online documentation</a>.</li>";
  }
  if (endpoint) {
    ...
    if (!(host.startsWith("localhost") || host.startsWith("127.0.0.1") || host.startsWith("0.0.0.0"))) {
      ...
      commonHelpMessage += "<li>Server overload. Please contact your PilotCodi server administrator for assistance.</li>";
    }
  }
  let helpMessage = "";
  if (helpMessageForRunningLargeModelOnCPU.length > 0) {
