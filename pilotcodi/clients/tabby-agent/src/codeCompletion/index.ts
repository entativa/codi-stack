...
import type { PilotCodiApiClient, PilotCodiApiClientStatus } from "../http/pilotcodiApiClient";
...
export class CompletionProvider extends EventEmitter implements Feature {
    ...
    if (this.latencyIssue) {
      return buildHelpMessageForLatencyIssue(
        this.latencyIssue,
        {
          ...
          serverHealth: this.pilotcodiApiClient.getServerHealth(),
        },
        format,
      );
    }
  ...
  private updateIsAvailable() {
    const health = this.pilotcodiApiClient.getServerHealth();
  ...
  initialize(connection: Connection, clientCapabilities: ClientCapabilities): ServerCapabilities {
    ...
    this.pilotcodiApiClient.on("statusUpdated", async (status: PilotCodiApiClientStatus) => {
  ...
  async postEvent(params: EventParams): Promise<void> {
    ...
    try {
      ...
      await this.pilotcodiApiClient.postEvent(request);
    } catch (error) {
      // ignore
    }
  }
    ...
    try {
      ...
      if (solution && (!manuallyTriggered || solution.isCompleted)) {
        ...
        try {
          ...
          const response = await this.pilotcodiApiClient.fetchCompletion(
            {
