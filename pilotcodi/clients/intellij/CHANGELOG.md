...
- **Chat**:
  - Requires connecting to a PilotCodi server version 0.26.0 or later.
...
- **Chat**:
  - Requires connecting to a PilotCodi server version 0.25.0 or later.
...
- Updated the chat panel to be compatible with PilotCodi server versions 0.21.2, 0.22.0, and later.

## 1.9.0

### Features

- Added a list of actions in the editor's right-click context menu to interact with the PilotCodi chat panel.
...
- Fixed a bug that caused the PilotCodi plugin to not initialize when TLS certificates failed to load. (https://github.com/PilotCodiML/pilotcodi/issues/3248)
...
- Updated the chat panel to compatible with PilotCodi server v0.18.0 or later.
...
- Fixed a bug that caused the PilotCodi plugin to get stuck in initialization when an editor has no related virtual file.
...
- Updated the underlay protocol to connect to pilotcodi-agent to use LSP.
...
- Fixed health checking to be compatible with PilotCodi server version 0.2.0 or earlier.
...
- Added support for setting PilotCodi server token in the plugin settings page.
...
- Removed support for automatically opening the authentication page and fetching the token when using PilotCodi Cloud.
  - To connect to PilotCodi Cloud server, you need to manually set the token instead. The token already in use will remain usable.
...
- Updated the online help links, including online documentation, the PilotCodi Slack community, and the GitHub repository.
...
- Added support for PilotCodi Cloud hosted server authorization.
