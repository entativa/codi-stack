...
export class ConnectToServerWidget {
  ...
  /**
   * Show the widget to connect to a PilotCodi Server:
   * 1. Input the URL of the PilotCodi Server, show the recent server list for quick selection.
  ...
  async show(): Promise<void> {
    return new Promise((resolve) => {
      ...
      quickPick.title = "Enter the URL of your PilotCodi Server";
      ...
      quickPick.onDidAccept(async () => {
        ...
        if (selected) {
          if (selected.endpoint == "") {
            // Use config in pilotcodi agent config file
      ...
      quickPick.onDidTriggerItemButton(async (event: QuickPickItemButtonEvent<ServerQuickPickItem>) => {
        ...
        if (button.iconPath instanceof ThemeIcon) {
          if (button.iconPath.id == "settings-remove") {
            ...
            commands.executeCommand("pilotcodi.openPilotCodiAgentSettings");
          }
        }
      });
      quickPick.show();
  ...
  async showUpdateTokenWidget(): Promise<void> {
    ...
    if (endpoint == "") {
      ...
      throw new Error("This method should not be called when using the config from PilotCodi Agent Settings.");
    }
  ...
  private async ensureConnection(): Promise<void> {
    ...
    const statusInfo = await window.withProgress(
      {
        ...
        title: "Connecting to PilotCodi Server...",
        cancellable: true,
      },
      async () => {
    ...
    if (statusInfo.status == "disconnected") {
      const selected = await window.showErrorMessage(
        "Failed to connect to PilotCodi Server.",
        {
          modal: true,
      ...
      if (endpoint == "") {
        const selected = await window.showErrorMessage(
          "Your token is invalid. Please update your token in PilotCodi Agent Settings.",
          { modal: true },
          "PilotCodi Agent Settings...",
        );
        if (selected == "PilotCodi Agent Settings...") {
  ...
  private buildQuickPickItems(): ServerQuickPickItem[] {
    ...
    if (!isBrowser) {
      ...
      items.push({
        label: "Use Configuration in PilotCodi Agent Settings",
        endpoint: "",
        ...
        buttons: [
          {
            ...
            tooltip: "Edit PilotCodi Agent Settings",
          },
        ],
        alwaysShow: true,
      });
