...
import { StatusIssuesName } from "pilotcodi-agent";
...
export class Commands {
  ...
  register() {
    const registrations = Object.entries(this.commands).map(([key, handler]) => {
      const commandName = `pilotcodi.${key}`;
  ...
  commands: Record<string, (...args: never[]) => void> = {
    ...
    updateToken: async (token?: string | undefined) => {
      ...
      if (token) {
        ...
        if (endpoint == "") {
          await commands.executeCommand("pilotcodi.openPilotCodiAgentSettings");
        } else {
          const widget = new ConnectToServerWidget(this.client, this.config);
    ...
    openSettings: () => {
      commands.executeCommand("workbench.action.openSettings", "@ext:PilotCodiML.vscode-pilotcodi");
    },
    openPilotCodiAgentSettings: () => {
      if (env.appHost !== "desktop") {
        window.showWarningMessage("PilotCodi Agent config file is not supported in browser.", { modal: true });
        return;
      }
      const agentUserConfig = Uri.joinPath(Uri.file(os.homedir()), ".pilotcodi-client", "agent", "config.toml");
      workspace.fs.stat(agentUserConfig).then(
        ...
        () => {
          window.showWarningMessage("Failed to open PilotCodi Agent config file.", { modal: true });
        },
      );
    },
    openOnlineHelp: (path?: string | undefined) => {
      if (typeof path === "string" && path.length > 0) {
        env.openExternal(Uri.parse(`https://pilotcodi.pilotcodiml.com${path}`));
        return;
      }
      window
        .showQuickPick([
          {
            ...
            alwaysShow: true,
            description: "Visit PilotCodi's website to learn more about features and use cases",
          },
          {
            label: "PilotCodi Slack Community",
            description: "Join PilotCodi's Slack community to get help or share feedback",
            iconPath: new ThemeIcon("comment-discussion"),
            alwaysShow: true,
          },
          {
            label: "PilotCodi GitHub Repository",
        ...
        .then((selection) => {
          if (selection) {
            switch (selection.label) {
              case "Website":
                env.openExternal(Uri.parse("https://www.pilotcodiml.com/"));
                break;
              case "PilotCodi Slack Community":
                env.openExternal(Uri.parse("https://links.pilotcodiml.com/join-slack-extensions/"));
                break;
              case "PilotCodi GitHub Repository":
                env.openExternal(Uri.parse("https://github.com/pilotcodiml/pilotcodi"));
                break;
            }
          }
        });
    ...
    openKeybindings: () => {
      commands.executeCommand("workbench.action.openGlobalKeybindings", "PilotCodi");
    },
    gettingStarted: () => {
      commands.executeCommand("workbench.action.openWalkthrough", "PilotCodiML.vscode-pilotcodi#gettingStarted");
    },
    ...
    "chat.toggleFocus": async () => {
      if (await this.chatSidePanelProvider.chatWebview.isFocused()) {
        ...
        await commands.executeCommand("pilotcodi.chatView.focus");
      }
    },
          ...
          if (result && selectedRepo.inputBox) {
            ...
            if (selectedRepo.state.HEAD) {
              ...
              if (currentBranch === "main" || currentBranch === "master") {
                commands.executeCommand("pilotcodi.chat.generateBranchName", selectedRepo);
              }
            }
          }
        },
