...
import type {
  ...
  TerminalContext,
  ChatCommand,
} from "pilotcodi-chat-panel";
import * as semver from "semver";
import debounce from "debounce";
import { v4 as uuid } from "uuid";
import type { StatusInfo, Config } from "pilotcodi-agent";
...
export class ChatWebview extends EventEmitter {
          ...
          case "chatIframeLoaded": {
            this.createClientTimeout = setTimeout(() => {
              ...
              if (!endpoint) {
                ...
                const command = `command:pilotcodi.openExternal?${encodeURIComponent(`["${endpoint}/chat"]`)}`;
                this.loadErrorPage(
                  `Failed to load the chat panel. <br/>Please check your network to ensure access to <a href='${command}'>${endpoint}/chat</a>. <br/><br/><a href='command:pilotcodi.reconnectToServer'><b>Reload</b></a>`,
                );
              }
            }, 10000);
            return;
  ...
  private async createChatPanelApiClient(webview: Webview): Promise<ServerApiList> {
    return await createClient(webview, {
      refresh: async () => {
        commands.executeCommand("pilotcodi.reconnectToServer");
        return;
      },
      ...
      runShell: async (command: string) => {
        const terminal = window.createTerminal("PilotCodi");
        terminal.show();
  ...
  private getUriAvatarPilotCodi() {
    return this.webview?.asWebviewUri(Uri.joinPath(this.context.extensionUri, "assets", "pilotcodi.png")).toString() ?? "";
  }
  ...
  private loadChatPanel() {
    ...
    webview.html = mainHtml
      ...
      .replace(/{{URI_AVATAR_PILOTCODI}}/g, this.getUriAvatarPilotCodi());
  ...
  private checkStatusInfo(statusInfo: StatusInfo | undefined): string | undefined {
    if (!statusInfo || statusInfo.status === "connecting") {
      return 'Connecting to the PilotCodi server...<br/><span class="loader"></span>';
    }

    if (statusInfo.status === "unauthorized") {
      return "Your token is invalid.<br/><a href='command:pilotcodi.updateToken'><b>Update Token</b></a>";
    }

    if (statusInfo.status === "disconnected") {
      return "Failed to connect to the PilotCodi server.<br/><a href='command:pilotcodi.connectToServer'><b>Connect To Server</b></a>";
    }

    const health = statusInfo.serverHealth;
    if (!health) {
      return "Cannot get the health status of the PilotCodi server.";
    }
    ...
    if (health["version"]) {
      ...
      if (version && semver.lt(version, MIN_VERSION)) {
        return `PilotCodi Chat requires PilotCodi server version ${MIN_VERSION} or later. Your server is running version ${version}.`;
      }
    }
