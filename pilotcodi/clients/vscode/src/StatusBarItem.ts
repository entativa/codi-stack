...
export class StatusBarItem {
  ...
  constructor(
    ...
    this.item.command = {
      title: "Show PilotCodi Command Palette",
      command: "pilotcodi.commandPalette.trigger",
    };
  ...
  update() {
    ...
    switch (languageClientState) {
      ...
      case LanguageClientState.Starting: {
        ...
        this.setTooltip("PilotCodi: Initializing...");
        break;
      }
  ...
  private checkIfCurrentLanguageDisabled(): boolean {
    ...
    this.setTooltip(`PilotCodi: disabled for ${currentLanguageId} files`);
    return true;
  }
  ...
  private checkIfVSCodeInlineCompletionEnabled() {
    if (this.config.vscodeInlineSuggestEnabled) {
      ...
      this.setColorWarning();
      this.setIcon(iconDisabled);
      this.setTooltip("PilotCodi: inline completion is disabled.");
  ...
  private showInformationWhenInlineSuggestDisabled() {
    ...
    window
      .showWarningMessage(
        "PilotCodi's suggestion is not showing because inline suggestion is disabled. Please enable it first.",
        "Enable",
        "Settings",
      )
