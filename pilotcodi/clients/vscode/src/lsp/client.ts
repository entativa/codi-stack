...
export function createClient(context: ExtensionContext, logger: LogOutputChannel): Client {
  ...
  if (isBrowser) {
    const workerModulePath = Uri.joinPath(context.extensionUri, "dist/pilotcodi-agent/browser/index.mjs");
    const worker = new Worker(workerModulePath.toString());
    const languageClient = new BrowserLanguageClient("PilotCodi", "PilotCodi", clientOptions, worker);
    return new Client(context, languageClient);
  } else {
    const serverModulePath = context.asAbsolutePath("dist/pilotcodi-agent/node/index.js");
    ...
    const languageClient = new NodeLanguageClient("PilotCodi", serverOptions, clientOptions);
    return new Client(context, languageClient);
  }
}
