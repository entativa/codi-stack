...
export async function activate(context: ExtensionContext) {
  logger.info("Activating PilotCodi extension...");
  ...
  context.subscriptions.push(
    window.registerWebviewViewProvider("pilotcodi.chatView", chatViewProvider, {
  ...
  logger.info("PilotCodi extension activated.");

  // Start async initialization
  const startClient = async () => {
    await gitProvider.init();

    logger.info("Launching language server pilotcodi-agent...");
    await client.start();
    logger.info("Language server pilotcodi-agent launched.");
  };
  ...
  const tryReadAuthenticationToken = async (): Promise<{ token: string | undefined }> => {
    ...
    const response = await window.showInformationMessage(
      "Do you consent to sharing your PilotCodi token with another VSCode extension?",
      {
        modal: true,
        detail: `The extension requests your token to access the PilotCodi server at ${endpoint}. Sharing your token allows the extension to perform actions as if it were you. Only proceed if you trust the extension.`,
      },
      "Yes",
      "No",
    );
...
export async function deactivate() {
  logger.info("Deactivating PilotCodi extension...");
  await clientRef?.stop();
  logger.info("PilotCodi extension deactivated.");
}
