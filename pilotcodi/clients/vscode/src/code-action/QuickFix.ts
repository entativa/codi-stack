...
export class QuickFixCodeActionProvider implements CodeActionProviderInterface {
  ...
  provideCodeActions(
    ...
    const quickFixEditing = new CodeAction("Fix using PilotCodi", CodeActionKind.QuickFix);

    quickFixEditing.command = {
      command: "pilotcodi.chat.edit.start",
      title: "Fix using PilotCodi",
    ...
    const explainError = new CodeAction("Explain using PilotCodi", CodeActionKind.QuickFix);
    explainError.command = {
      command: "pilotcodi.chat.explainCodeBlock",
      title: "Explain using PilotCodi",
