...
- **Chat Panel**:
  ...
  - Requires PilotCodi Server `0.27.0` or later.
  ...
  - You can now select code in the editor and use the `Code Review` option from the context menu to review your code and add comments using PilotCodi Chat.
...
- **Git Integration**:
  - Improved the `Generate Commit Messages` command. After generating commit messages, if you are still on the `main` or `master` branch, PilotCodi will automatically prompt you to create a new branch with suggested branch names.
...
- Added support for other extensions to read PilotCodi server configurations after user grants permission.
...
- **Chat**:
  - You can now use `@` to add a selected file as context for the chat conversation when typing in the chat panel. (Requires connecting to a PilotCodi server version 0.24.0 or later.)
- Added an inline edit option in the editor right-click context menu.

### Fixes & Improvements

- Retained the chat conversation when moving the chat panel to a different view group. (Requires connecting to a PilotCodi server version 0.24.0 or later.)
...
- **Chat**:
  ...
  - **Note**: Requires updating the PilotCodi server to version 0.23.0 or later.

### Fixes & Improvements

- Updated the status bar item to show warning when the server returns an error due to too many requests.
- Improved the UI for the PilotCodi command palette and when updating the token.
...
- **Chat**:
  ...
  - **Note**: Requires updating the PilotCodi server to version 0.21.2, 0.22.0, or later.
- **Code Completion**:
...
- Streamlined the `PilotCodi: Connect to Server...` command process and added a server history list for quick selection.
...
- Introduced a "Smart Apply" button in the chat panel's generated code block. This feature suggests edits directly in the current editor, enabling users to apply generated code quickly without manual intervention. Note: Requires the latest version of the PilotCodi server.
- Added a dynamic indicator in the chat panel's input box to show the currently selected text, which will be used as context for the chat conversation. Note: Requires the latest version of the PilotCodi server.
...
- Fixed a bug causing the PilotCodi web extension to not initialize in browsers. (https://github.com/PilotCodiML/pilotcodi/issues/3302)
...
- Fixed a bug that caused the PilotCodi extension to not initialize when TLS certificates failed to load. (https://github.com/PilotCodiML/pilotcodi/issues/3248)

## 1.12.1

### Fixes & Improvements

- Fix DataStore initialization that prevented PilotCodi from starting on a fresh installation. (https://github.com/PilotCodiML/pilotcodi/issues/3234)

## 1.12.0

### Features

- Updated the chat panel to be compatible with PilotCodi server v0.18.0 or later.
- Added support to open the chat panel as a tab in editor groups.
- Introduced actions in the quick-fix menu to explain or fix errors using PilotCodi.

## 1.10.3

### Fixes & Improvements

- Fixed theme detection issue in the chat side panel. (https://github.com/PilotCodiML/pilotcodi/issues/3031)
...
- Added the option to utilize `Edit with PilotCodi` in the quick fix menu for quicker access.
...
- Introduced a new UI for the PilotCodi command palette, helping you quickly inspect status and navigate to commands. Access the command palette by clicking the `PilotCodi` label in the status bar.
...
- Renamed the command `PilotCodi: Edit...` to `PilotCodi: Start Inline Editing`. You can edit commands from the history now.
...
- Enhanced the chat side panel interaction with various improvements. The chat panel improvements require the latest version of the PilotCodi server to fully function.
- Updated and optimized the PilotCodi configurations in the VSCode Settings page.
- Resolved compatibility issue with chat panel on connections to an older version of the PilotCodi server.
...
- Merged output channels `PilotCodi` and `PilotCodi Agent` into one output channel `PilotCodi`.

### Fixes

- Corrected server-side config retrieval behavior for connections to PilotCodi servers with version < 0.9.
...
- Added support for loading configurations from PilotCodi server, including `Disabling Client-side Telemetry`.
- Added output channel for logging. The log level can be configured by VSCode command `Developer: Set Log Level...`.
- Removed the notification when disconnected from PilotCodi server, keep only status bar icon.
...
- Fixed health checking to be compatibility with PilotCodi server version 0.2.0 or earlier.

## 1.2.0

### Features:

- Added support for setting PilotCodi server token in VSCode.
...
- Removed support for automatically opening the authentication page and fetching the token when using PilotCodi Cloud.
  - For connecting to PilotCodi Cloud server, you need to manually set the token instead. The token already in use will still be usable.
...
- Added a command to display useful online help links, including online documentation, the PilotCodi Slack community, and the GitHub repository.
...
- Migrated PilotCodi cloud authorization tokens and anonymous usage tracking id from the old data directory to the new one.

## 0.4.0

### Features:

- Relocated the user data directory from `$HOME/.pilotcodi/agent` to `$HOME/.pilotcodi-client/agent` to avoid conflicts with PilotCodi server data. Note that the old data will not be migrated automatically. Please update the config file manually if you have made changes in the old path.
- Added a template config file for PilotCodi client agent located at `$HOME/.pilotcodi-client/agent/config.toml`.
...
- Resolved conflict with the auto completion widget. The PilotCodi inline completion will no longer be displayed when the auto completion widget is visible.
...
- Supported setting HTTP request headers in `$HOME/.pilotcodi/agent/config.toml`. The `Authorization` header can be used to set a token for self-hosted servers authorization.

## 0.2.1

### Fixes:

- Set keybindings for accepting inline completion default to VSCode-style, mark PilotCodi-Style as experimental.

## 0.1.2

### Features:

- Added PilotCodi extension walkthrough guides.
- Added PilotCodi-Style keybindings for accepting inline completion as an alternative to VSCode default keybindings.
- Supported reading user config from `$HOME/.pilotcodi/agent/config.toml`, instead of `Developer Options` in extension settings.
