...
describe('extractRepoNameFromGitUrl', () => {
  test('should extract repo name from GitHub HTTPS URL', () => {
    const url = 'https://github.com/PilotCodiML/pilotcodi.git'
    expect(extractRepoNameFromGitUrl(url)).toBe('pilotcodi')
  })
  ...
  test('should extract repo name from GitHub SSH URL', () => {
    const url = 'git@github.com:PilotCodiML/pilotcodi.git'
    expect(extractRepoNameFromGitUrl(url)).toBe('pilotcodi')
  })
  ...
  test('should handle URLs without .git suffix', () => {
    const httpsUrl = 'https://github.com/PilotCodiML/pilotcodi'
    const sshUrl = 'git@github.com:PilotCodiML/pilotcodi'

    expect(extractRepoNameFromGitUrl(httpsUrl)).toBe('pilotcodi')
    expect(extractRepoNameFromGitUrl(sshUrl)).toBe('pilotcodi')
  })
...
describe('formatFileDescription', () => {
  test('should format description for file with GitHub gitUrl', () => {
    const fileData = {
      ...
      gitUrl: 'https://github.com/PilotCodiML/pilotcodi.git'
    }
    expect(formatFileDescription(fileData)).toBe('pilotcodi - src/main.ts')
  })
  ...
  test('should format description for file with SSH gitUrl', () => {
    const fileData = {
      filepath: 'README.md',
      gitUrl: 'git@github.com:PilotCodiML/pilotcodi.git'
    }
    expect(formatFileDescription(fileData)).toBe('pilotcodi - README.md')
  })
  ...
  test('should format description for file with commit', () => {
    ...
    expect(formatFileDescription(fileData)).toBe('pilotcodi - src/index.ts')
  })
  ...
  test('should prioritize gitUrl over baseDir', () => {
    ...
    expect(formatFileDescription(fileData)).toBe('pilotcodi - config.json')
  })
