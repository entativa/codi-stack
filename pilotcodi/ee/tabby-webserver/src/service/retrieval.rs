...
pub async fn attachment_docs_from_db(
    auth: Arc<dyn AuthenticationService>,
    docs: Vec<pilotcodi_db::AttachmentDoc>,
) -> Vec<AttachmentDoc> {
    ...
    for doc in docs {
        let author = {
            let author_id = match &doc {
                pilotcodi_db::AttachmentDoc::Issue(issue) => issue.author_user_id.as_deref(),
                pilotcodi_db::AttachmentDoc::Pull(pull) => pull.author_user_id.as_deref(),
                pilotcodi_db::AttachmentDoc::Commit(commit) => commit.author_user_id.as_deref(),
                _ => None,
            };
...
pub fn attachment_doc_from_db(
    value: pilotcodi_db::AttachmentDoc,
    author: Option<UserValue>,
) -> AttachmentDoc {
    match value {
        pilotcodi_db::AttachmentDoc::Web(web) => AttachmentDoc::Web(AttachmentWebDoc {
            title: web.title,
            link: web.link,
            content: web.content,
        }),
        pilotcodi_db::AttachmentDoc::Issue(issue) => AttachmentDoc::Issue(AttachmentIssueDoc {
            title: issue.title,
            link: issue.link,
            author,
            body: issue.body,
        ...
        pilotcodi_db::AttachmentDoc::Pull(pull) => AttachmentDoc::Pull(AttachmentPullDoc {
            title: pull.title,
            link: pull.link,
            author,
            body: pull.body,
        ...
        pilotcodi_db::AttachmentDoc::Commit(commit) => AttachmentDoc::Commit(AttachmentCommitDoc {
            sha: commit.sha,
            message: commit.message,
            author,
            author_at: commit.author_at,
        }),
        pilotcodi_db::AttachmentDoc::Page(page) => AttachmentDoc::Page(AttachmentPageDoc {
            link: page.page_link,
            title: page.title,
            content: page.content,
        }),
        pilotcodi_db::AttachmentDoc::Ingested(ingested) => {
    ...
    pub fn make_context_info_helper() -> ContextInfoHelper {
        ContextInfoHelper::new(&ContextInfo {
            sources: vec![ContextSourceValue::Repository(Repository {
                ...
                name: "pilotcodi".to_owned(),
                kind: RepositoryKind::Github,
                dir: PathBuf::from("pilotcodi"),
