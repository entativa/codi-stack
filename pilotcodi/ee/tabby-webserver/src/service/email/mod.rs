...
fn make_smtp_builder(
    ...
    if let Ok(cert_string) = std::env::var("PILOTCODI_WEBSERVER_EMAIL_CERT") {
...
impl EmailServiceImpl {
    ...
    async fn send_email_in_background(
        ...
        let msg = MessageBuilder::new()
            ...
            .from(Mailbox::new(Some("PilotCodi Admin".to_owned()), address_from))
...
impl EmailService for EmailServiceImpl {
    ...
    async fn send_invitation(&self, email: String, code: String) -> Result<JoinHandle<()>> {
        ...
        self.send_email_in_background(
            email,
            "You've been invited to join a PilotCodi server!".into(),
            body,
        )
        .await
    }
    ...
    async fn send_signup(&self, email: String) -> Result<JoinHandle<()>> {
        ...
        self.send_email_in_background(email, "Welcome to PilotCodi!".into(), body)
            .await
    }
    ...
    async fn send_password_reset(&self, email: String, code: String) -> Result<JoinHandle<()>> {
        ...
        self.send_email_in_background(email, "Reset your PilotCodi account password".into(), body)
            .await
    }
