...
use pilotcodi_common::config::CodeRepository;
use pilotcodi_index::public::{CodeIndexer, StructuredDoc, StructuredDocIndexer, StructuredDocState};
...
async fn fetch_all_issues(
    integration: &Integration,
    repository: &ProvidedRepository,
) -> pilotcodi_schema::Result<BoxStream<'static, (StructuredDocState, StructuredDoc)>> {
...
async fn fetch_all_pull_states(
    integration: &Integration,
    repository: &ProvidedRepository,
) -> pilotcodi_schema::Result<BoxStream<'static, (pulls::Pull, StructuredDocState)>> {
