...
use pilotcodi_crawler::{crawl_pipeline, crawler_llms};
use pilotcodi_index::public::{
...
use pilotcodi_schema::CoreError;
...
impl WebCrawlerJob {
    ...
    pub async fn run_impl(self, embedding: Arc<dyn Embedding>) -> pilotcodi_schema::Result<()> {
    ...
    pub async fn run(self, embedding: Arc<dyn Embedding>) -> pilotcodi_schema::Result<()> {
...
mod tests {
    ...
    async fn run(count: Arc<AtomicUsize>, finished: Arc<AtomicBool>) -> pilotcodi_schema::Result<()> {
        let now = Instant::now();
