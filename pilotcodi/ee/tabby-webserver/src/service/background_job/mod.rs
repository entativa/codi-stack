...
fn calculate_current_shard(number_of_repo: usize, timestamp_seconds: i64) -> Option<usize> {
    // Only run on PILOTCODI_INDEX_REPO_IN_SHARD is not empty and number_of_repo > SHARDING_THRESHOLD
    // otherwise return None
    if !(env::var("PILOTCODI_INDEX_REPO_IN_SHARD").is_ok_and(|v| !v.is_empty())
