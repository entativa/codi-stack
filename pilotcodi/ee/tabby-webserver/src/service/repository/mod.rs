...
impl RepositoryService for RepositoryServiceImpl {
    ...
    async fn search_files(
        ...
        let matching = pilotcodi_git::search_files(&dir, rev, &pattern, top_n)
            .await
            .map(|x| {
                x.into_iter()
    ...
    async fn list_files(
        ...
        let dir = self.resolve_repository(policy, kind, id).await?.dir;
        let (files, truncated) =
            pilotcodi_git::list_files(&dir, rev, top_n)
                .await
                .map(|list_file| {
                    let files = list_file
                        .files
    ...
    async fn grep(
        ...
        query: &str,
        top_n: usize,
    ) -> Result<Vec<pilotcodi_schema::repository::GrepFile>> {
        ...
        let ret = pilotcodi_git::grep(&dir, rev, query)
            .await?
            .map(to_grep_file)
            .take(top_n)
            .collect::<Vec<_>>()
...
fn to_grep_file(file: pilotcodi_git::GrepFile) -> pilotcodi_schema::repository::GrepFile {
    pilotcodi_schema::repository::GrepFile {
        path: file.path.display().to_string(),
        lines: file.lines.into_iter().map(to_grep_line).collect(),
    }
}

fn to_grep_line(line: pilotcodi_git::GrepLine) -> pilotcodi_schema::repository::GrepLine {
    pilotcodi_schema::repository::GrepLine {
        line: match line.line {
            pilotcodi_git::GrepTextOrBase64::Text(text) => {
                pilotcodi_schema::repository::GrepTextOrBase64::Text(text)
            }
            pilotcodi_git::GrepTextOrBase64::Base64(bytes) => {
                pilotcodi_schema::repository::GrepTextOrBase64::Base64(bytes)
            }
        },
...
fn to_sub_match(m: pilotcodi_git::GrepSubMatch) -> pilotcodi_schema::repository::GrepSubMatch {
    pilotcodi_schema::repository::GrepSubMatch {
        bytes_start: m.bytes_start as i32,
        bytes_end: m.bytes_end as i32,
    }
}

fn list_refs(git_url: &str) -> Vec<pilotcodi_git::GitReference> {
    let dir = RepositoryConfig::resolve_dir(git_url);
    pilotcodi_git::list_refs(&dir).unwrap_or_default()
}
...
fn repository_config_to_repository(index: usize, config: &RepositoryConfig) -> Repository {
    ...
    Repository {
        ...
        dir: config.dir(),
        refs: pilotcodi_git::list_refs(&config.dir())
            .unwrap_or_default()
            .into_iter()
            .map(|r| GitReference {
                name: r.name,
