...
use juniper::ID;
use pilotcodi_db::{DbConn, InvitationDAO};
...
fn create_impl(
    ...
    if let Ok(value) = std::env::var("PILOTCODI_OWNER_IMPERSONATE_OVERRIDE") {
...
#[cfg(test)]
mod tests {
    use pilotcodi_schema::auth::LdapEncryptionKind;
    ...
    impl LicenseService for MockLicenseService {
        async fn read(&self) -> Result<LicenseInfo> {
            Ok(LicenseInfo {
                r#type: pilotcodi_schema::license::LicenseType::Team,
    ...
    async fn test_is_admin_initialized() {
        ...
        assert!(!service.is_admin_initialized().await.unwrap());
        pilotcodi_db::testutils::create_user(&service.db).await;
        assert!(service.is_admin_initialized().await.unwrap());
    }
    ...
    async fn test_get_or_create_oauth_user() {
        ...
        assert_eq!(mail.list_mail().await[0].subject, "Welcome to PilotCodi!");
    ...
    async fn test_personate_user_auth_token() {
        std::env::set_var("PILOTCODI_OWNER_IMPERSONATE_OVERRIDE", "abc@example.com:123456");
        let service = test_authentication_service().await;
        register_admin_user(&service).await;
        assert!(service
