...
use pilotcodi_db::{DbConn, UserDAO, UserGroupDAO};
use pilotcodi_inference::{ChatCompletionStream, CompletionStream, Embedding as EmbeddingService};
...
impl ServiceLocator for ArcServerContext {
    ...
    fn notification(&self) -> Arc<dyn pilotcodi_schema::notification::NotificationService> {
...
trait UserSecuredExt {
    fn new(db: DbConn, val: UserDAO) -> pilotcodi_schema::auth::UserSecured;
}

impl UserSecuredExt for pilotcodi_schema::auth::UserSecured {
    fn new(db: DbConn, val: UserDAO) -> pilotcodi_schema::auth::UserSecured {
        let is_owner = val.is_owner();
        let id = val.id.as_id();
        pilotcodi_schema::auth::UserSecured {
