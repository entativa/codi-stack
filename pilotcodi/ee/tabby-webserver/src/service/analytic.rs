...
impl AnalyticService for AnalyticServiceImpl {
    ...
    async fn disk_usage_stats(&self) -> Result<DiskUsageStats> {
        Ok(DiskUsageStats {
            events: dir_size(pilotcodi_common::path::events_dir()).await?,
            indexed_repositories: dir_size(pilotcodi_common::path::index_dir()).await?,
            database: dir_size(crate::path::pilotcodi_ee_root()).await?,
            models: dir_size(pilotcodi_common::path::models_dir()).await?,
        })
    }
}
...
mod tests {
    ...
    use pilotcodi_common::path::set_pilotcodi_root;
    ...
    async fn test_disk_usage() {
        ...
        set_pilotcodi_root(tmp_dir.to_path_buf());

        tokio::fs::create_dir_all(pilotcodi_common::path::models_dir())
            .await
            .unwrap();

        tokio::fs::write(
            pilotcodi_common::path::models_dir().join("testfile"),
