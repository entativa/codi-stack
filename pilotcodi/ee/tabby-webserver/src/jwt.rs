...
use pilotcodi_common::terminal::{HeaderFormat, InfoMessage};
use pilotcodi_schema::auth::JWTPayload;
use tracing::warn;
use uuid::Uuid;
...
fn jwt_token_secret() -> String {
    let jwt_secret = std::env::var("PILOTCODI_WEBSERVER_JWT_TOKEN_SECRET").unwrap_or_else(|_| {
        InfoMessage::new("JWT secret is not set", HeaderFormat::BoldYellow, &[
            "PilotCodi server will generate a one-time (non-persisted) JWT secret for the current process.",
            &format!("Please set the {} environment variable for production usage.", HeaderFormat::Blue.format("PILOTCODI_WEBSERVER_JWT_TOKEN_SECRET")),
        ]).print();
