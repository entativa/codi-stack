...
- `kind`: The kind (or provider) of the repository, one of `git`, `github`, or `gitlab`.
  - Found at https://github.com/PilotCodiML/pilotcodi/blob/main/ee/pilotcodi-schema/src/schema/repository/mod.rs#L32
- `id`: The object ID identifying the repository within its kind - short alphanumeric key (like `E16n1q`)
  - Encoded rowids generated from https://github.com/PilotCodiML/pilotcodi/blob/main/ee/pilotcodi-schema/src/dao.rs#L219
...
curl --request GET \
  --url http://localhost:9090/repositories/github/E16n1q/resolve/ee/pilotcodi-webserver/ \
```
...
For the request `/repositories/git/lNrAwW/resolve/package.json`, the content type is `application/json`, and the response is as follows (the content of package.json in the PilotCodi repository):

```json
{
  "private": true,
  "workspaces": [
    "clients/pilotcodi-agent",
    "clients/vscode",
    "clients/vim",
    "clients/intellij",
...
For request `/repositories/git/lNrAwW/resolve/ee/pilotcodi-webserver/src/lib.rs` (again in the PilotCodi repository), the content type is `text/x-rust`, and the response is:

```text
//! Defines behavior for the pilotcodi webserver which allows users to interact with enterprise features.
...
pub mod public {
    ...
    pub use super::{
        /* used by pilotcodi workers (consumer of /hub api) */
        hub::{
