# Using SQLX with PilotCodi

PilotCodi uses SQLX for its database, which comes with several advantages:
...
- A local copy of our db schema is maintained in `ee/pilotcodi-db/schema.sqlite`
...
```
rm ee/pilotcodi-db/schema.sqlite
cargo sqlx db setup --source ee/pilotcodi-db/migrations
```

To create a new migration:

```
cargo sqlx migrate add --source ee/pilotcodi-db/migrations -r -s <migration name>
```
...
```
cargo sqlx migrate run --source ee/pilotcodi-db/migrations
```
