...
async fn test_repository_migration_0029() {
    ...
    sqlx::query("INSERT INTO github_provided_repositories(github_repository_provider_id, vendor_id, name, git_url, active)
                VALUES (1, 'vendor_id', 'pilotcodi-gh', 'https://github.com/PilotCodiML/pilotcodi', true);")
            .execute(&db.pool)
            .await
            .unwrap();
    ...
    sqlx::query("INSERT INTO gitlab_provided_repositories(gitlab_repository_provider_id, vendor_id, name, git_url, active)
                VALUES (1, 'vendor_id', 'pilotcodi-gl', 'https://gitlab.com/PilotCodiML/pilotcodi', false);")
            .execute(&db.pool)
            .await
            .unwrap();
    ...
    assert_eq!(repos[0].name, "pilotcodi-gh");
    assert_eq!(repos[0].integration_id, 1);
    assert_eq!(repos[0].git_url, "https://github.com/PilotCodiML/pilotcodi");
    assert!(repos[0].active);

    assert_eq!(repos[1].name, "pilotcodi-gl");
    assert_eq!(repos[1].integration_id, 2);
    assert_eq!(repos[1].git_url, "https://gitlab.com/PilotCodiML/pilotcodi");
