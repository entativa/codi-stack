...
By default, PilotCodi stores all its data in the `$HOME/.pilotcodi` directory. However, you can override this behavior by using the `PILOTCODI_ROOT` environment variable. This directory contains all the necessary data for PilotCodi's operation, including the database, logs, and configuration files.

## Database Backup

PilotCodi uses SQLite for data storage, with the default database located at `$HOME/.pilotcodi/ee/db.sqlite`.
To conduct backup operations, you'll require the SQLite CLI.

1. Access the PilotCodi database using the SQLite CLI:
   ```bash
   sqlite3 $HOME/.pilotcodi/ee/db.sqlite
   ```
...
PilotCodi stores all event logs in the `~/.pilotcodi/events` directory. These events, stored in JSON format, are named after the date of their creation.
...
Starting with version v0.27.0, PilotCodi saves all background job logs in the `~/.pilotcodi/ee/jobs` directory.
To display the logs in the management UI, it is essential to back up this directory.

```
% ls ~/.pilotcodi/ee/jobs
