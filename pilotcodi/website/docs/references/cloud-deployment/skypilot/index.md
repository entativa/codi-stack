...
When leveraging SkyServe, all replica PilotCodi instances are seamlessly deployed within your own cloud accounts and VPCs.

## Configuration

At first, let's specified the resource requirements for the PilotCodi service in the YAML configuration for SkyServe.
...
PilotCodi exposes its health check at the `/metrics` endpoint, which also serves as a prometrics endpoint. Therefore, we can define the following readiness probe:
...
```yaml
run: |
  docker run --gpus all -p 8080:8080 -v ~/.pilotcodi:/data \
    pilotcodiml/pilotcodi \
    serve --model PilotCodiML/StarCoder-1B --device cuda
```

## Launch the service

We first execute `sky serve up pilotcodi.yaml -n pilotcodi`.

![start pilotcodi service](./start-service.png)

If everything goes well, you'll see messages below
![service ready](./service-ready.png)

This finishes launching SkyServe's control VM which runs a load balancer for this serve; the actual replica running the PilotCodi service is undergoing provisioning.
...
You can monitor the progress of starting the actual pilotcodi job by checking the replica log:

```bash
# Tailing the logs of replica 1 for the pilotcodi service
sky serve logs pilotcodi 1
```

Once the service is ready, you will see something like the following:

![pilotcodi ready](./pilotcodi-ready.png)

Now, you can utilize the load balancer URL (`http://44.203.34.65:30001` in this case) within PilotCodi editor extensions. Please refer to [`pilotcodi.yaml`](https://github.com/PilotCodiML/pilotcodi/blob/main/website/docs/installation/skypilot/pilotcodi.yaml) for the full configuration used in this tutorial.
