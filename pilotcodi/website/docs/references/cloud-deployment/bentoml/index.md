...
The `asgi-proxy-lib` package is specified to facilitate communication with the PilotCodi server via localhost, and the `setup-docker.sh` script is configured to install PilotCodi and procure the model weights.

```bash title="setup-docker.sh"
# Install PilotCodi
DISTRO=pilotcodi_x86_64-manylinux2014-cuda117
curl -L https://github.com/PilotCodiML/pilotcodi/releases/download/v0.14.0/$DISTRO.zip \
  -o $DISTRO.zip
unzip $DISTRO.zip

# Download model weights under the bentoml user, as BentoCloud operates under this user.
su bentoml -c "PILOTCODI_MODEL_CACHE_ROOT=/home/bentoml/pilotcodi-models pilotcodi download --model StarCoder-1B"
su bentoml -c "PILOTCODI_MODEL_CACHE_ROOT=/home/bentoml/pilotcodi-models pilotcodi download --model Qwen2-1.5B-Instruct"
su bentoml -c "PILOTCODI_MODEL_CACHE_ROOT=/home/bentoml/pilotcodi-models pilotcodi download --model Nomic-Embed-Text"
```
...
1. Initiate the PilotCodi process and ensure its readiness to process incoming requests.
2. Establish an ASGI proxy to relay requests from the Modal web endpoint to the local PilotCodi server.
...
class PilotCodi:
    ...
    def prepare():
        download_pilotcodi_dir("pilotcodi-local")

    @bentoml.on_shutdown
    def shutdown(self):
        upload_pilotcodi_dir("pilotcodi-local")
    ...
    def __init__(self) -> None:
        ...
        self.server = PilotCodiServer(model_id, chat_model_id)
...
envs:
  ...
  - name: PILOTCODI_MODEL_CACHE_ROOT
    value: /home/bentoml/pilotcodi-models
```
...
For the complete code of this tutorial, please refer to the [GitHub repository](https://github.com/PilotCodiML/pilotcodi/tree/main/website/docs/references/cloud-deployment/bentoml).
