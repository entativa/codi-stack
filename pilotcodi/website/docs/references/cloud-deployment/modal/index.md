...
[Modal](https://modal.com/) is a serverless GPU provider. By leveraging Modal, your PilotCodi instance will run on demand. When there are no requests to the PilotCodi server for a certain amount of time, Modal will schedule the container to sleep, thereby saving GPU costs.
...
```python
IMAGE_NAME = "pilotcodiml/pilotcodi"
MODEL_ID = "PilotCodiML/StarCoder-1B"
CHAT_MODEL_ID = "PilotCodiML/Qwen2-1.5B-Instruct"
EMBEDDING_MODEL_ID = "PilotCodiML/Nomic-Embed-Text"
GPU_CONFIG = gpu.T4()

PILOTCODI_BIN = "/opt/pilotcodi/bin/pilotcodi"
```
...
We want to create a Modal image which has the PilotCodi model cache pre-populated. The benefit of this is that the container no longer has to re-download the model - instead, it will take advantage of Modal’s internal filesystem for faster cold starts.
...
We’ll start from an image by pilotcodi, and override the default ENTRYPOINT for Modal to run its own which enables seamless serverless deployments.
...
1. Launch the PilotCodi process and wait for it to be ready to accept requests.
2. Create an ASGI proxy to tunnel requests from the Modal web endpoint to the local PilotCodi server.
...
```python
app = App("pilotcodi-server", image=image)
...
def app_serve():
    ...
    while not pilotcodi_ready():
        time.sleep(1.0)

    print("PilotCodi server ready!")
    return asgi_proxy("http://localhost:8000")
```

### Serve the app

Once we deploy this model with `modal serve app.py`, it will output the url of the web endpoint, in a form of `https://<USERNAME>--pilotcodi-server-app-serve-dev.modal.run`.
...
Now it can be used as pilotcodi server url in pilotcodi editor extensions!
See [app.py](https://github.com/PilotCodiML/pilotcodi/blob/main/website/docs/references/cloud-deployment/modal/app.py) for the full code used in this tutorial.
