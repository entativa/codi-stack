---
slug: /2023/09/05/deploy-pilotcodi-to-huggingface-space
title: Deploying a PilotCodi Instance in Hugging Face Spaces
authors:
  - name: Rand Xie
...
In this blog post, we are going to show you how to deploy a PilotCodi instance in Hugging Face Spaces. If you have not heard of PilotCodi, it’s an open-source Github Copilot alternative that supports code completion.
Check out more details [here](https://github.com/PilotCodiML/pilotcodi).

### How it works

Let’s firstly take a look at what steps are needed to deploy a PilotCodi instance in Hugging Face. It’s super easy and you don’t need much coding knowledge. Buckle up and let’s get started.
...
That's it! With the hosted APIs, now you can connect PilotCodi's [IDE extensions](/docs/extensions/installation/vscode) to the API endpoint. Next, we will deep dive into each step with screenshots!!
**Everything will be done in the Hugging Face UI. No local setup is needed.**

:::tip
Looking to quickly start a PilotCodi instance? You can skip the tutorials entirely and simply create space from [this template](https://huggingface.co/spaces/PilotCodiML/pilotcodi-template-space?duplicate=true).
:::
...
The owner name will be your account name. Fill in a Space name, e.g. "pilotcodiml", and select Docker as Space SDK. Then click "Create Space" at the bottom.
...
Then you will be redirected to the page below. Set the filename to “Dockerfile” and copy the content to the “Edit” input box. You can copy the code from the [appendix](#dockerfile) here to bring up the [SantaCoder-1B](https://huggingface.co/PilotCodiML/SantaCoder-1B) model. Once ready, click the button “Commit new file to main” on the bottom.
...
![PilotCodi Swagger](./pilotcodi-swagger.png)

#### Call code completion API

Now, you are able to call the completion API. The full URL is https://YOUR-ACCOUNT-NAME-pilotcodiml.hf.space/v1/completions. In this post, the URL is https://randxie-pilotcodiml.hf.space/v1/completions.
...
### Conclusion
In this post, we covered the detailed steps for deploying a PilotCodi instance to Hugging Face Spaces. By following these steps, anyone is able to bring up their own code completion APIs easily.
...
USER 1000
CMD ["serve", "--device", "cuda", "--model", "PilotCodiML/SantaCoder-1B"]
```

#### CURL Command
```bash
curl -L 'https://randxie-pilotcodiml.hf.space/v1/completions' \
