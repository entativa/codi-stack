...
To demonstrate the effectiveness of this approach, below is an example showcasing [PilotCodiML/StarCoder-1B](https://hf.co/PilotCodiML/StarCoder-1B) performing code completion within PilotCodi's own repository.
...
However, with the repository context (Specifically, if we include the entire file of `crates/pilotcodi/src/serve/completions.rs` into the prompt).


```rust title="Prepend to the completion request"
// === crates/pilotcodi/serve/completions.rs ===
// ......
// ......
```
...
In the [v0.3.0 release](https://github.com/PilotCodiML/pilotcodi/releases/tag/v0.3.0), we introduced Retrieval Augmented Code Completion, a nifty feature that taps into the repository context to enhance code suggestions. Here's a sneak peek of a snippet we pulled from the repository context:

```rust title="Snippet from the Repository Context: A Glimpse into the Magic"
// Path: crates/pilotcodi/src/serve/completions.rs
...
// }
//
// Path: crates/pilotcodi/src/serve/mod.rs
// Router::new()
...
PilotCodi, under the hood, leverages 🌳 Tree-sitter query to construct its index. Tree-sitter is capable of scanning source code written in various languages and extracting data about all the symbols defined in each file.
...
1. Installing [pilotcodi](/docs/quick-start/installation/docker).
