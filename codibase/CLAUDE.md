...
CodiBase uses Maven as its build system with a multi-module structure.
...
CodiBase is a comprehensive DevOps platform built with a sophisticated multi-module Maven architecture:
...
- Main entry point: `server-core/src/main/java/io/codibase/server/CodiBase.java`
- Module configuration: `server-core/src/main/java/io/codibase/server/CoreModule.java`
- Handles server lifecycle, clustering, and graceful shutdown

#### 2. Entity Management
Key domain entities and managers in `server-core/src/main/java/io/codibase/server/model/`:
...
- Git hooks for policy enforcement in `server-core/src/main/java/io/codibase/server/git/`
- Code browsing, diff visualization, and blame tracking
- SSH server for Git operations

#### 4. Web Layer (Wicket)
- Component-based UI in `server-core/src/main/java/io/codibase/server/web/`
- AJAX-heavy interface with WebSocket support
- Project browsing, issue boards, pull request review interface

#### 5. REST API (Jersey)
- RESTful services in `server-core/src/main/java/io/codibase/server/rest/`
...
- Entities are in `server-core/src/main/java/io/codibase/server/model/`
- Use corresponding managers for database operations
- Follow JPA/Hibernate patterns for persistence

### Adding REST Endpoints
- Create resources in `server-core/src/main/java/io/codibase/server/rest/resource/`
...
### Working with Git
- Use JGit APIs through CodiBase's Git service layer
- Follow patterns in `server-core/src/main/java/io/codibase/server/git/`
- Handle Git operations asynchronously when possible

### Adding Web Components
- Create Wicket components in `server-core/src/main/java/io/codibase/server/web/`
...
- CodiBase uses a custom plugin loading framework
