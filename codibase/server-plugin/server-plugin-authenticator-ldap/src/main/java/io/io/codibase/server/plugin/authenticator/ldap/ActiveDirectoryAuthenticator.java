...
public class ActiveDirectoryAuthenticator extends LdapAuthenticator {
    ...
    @Editable(order=100, name="LDAP URL", description = "" +
			...
			"<a href='https://docs.codibase.io/administration-guide/trust-self-signed-certificates' target='_blank'>configure CodiBase to trust the certificate</a>")
    @NotEmpty
	@Override
	...
	@Editable(order=300, description=""
			+ "Specify manager DN to authenticate CodiBase itself to Active Directory. The manager DN should be specified "
	...
	@Editable(order=1000, placeholder="Do not retrieve groups", description=""
			...
			+ "permissions to a Active Directory group, a CodiBase group with same name should be defined. "
			+ "Leave empty to manage group memberships at CodiBase side")
