...
import static io.codibase.server.plugin.report.problem.ProblemReport.CATEGORY;
import static io.codibase.server.plugin.report.problem.ProblemReport.getReportLockName;
	...
	protected void configure() {
		...
		contribute(ProjectMenuContribution.class, new ProjectMenuContribution() {
			...
			public List<SidebarMenuItem> getMenuItems(Project project) {
				...
				if (!CodiBase.getInstance(BuildMetricService.class).getAccessibleReportNames(project, ProblemMetric.class).isEmpty()) {
		...
		contribute(BuildStorageSyncer.class, ((projectId, buildNumber, activeServer) -> {
			CodiBase.getInstance(ProjectService.class).syncDirectory(projectId,
