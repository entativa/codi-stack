...
@Editable(name="OpenID", order=10000, description="Refer to this <a href='https://docs.codibase.io/tutorials/security/sso-with-okta' target='_blank'>tutorial</a> for an example setup")
public class OpenIdConnector extends SsoConnector {
	...
	@Editable(order=200, description="Specify configuration discovery url of your OpenID provider, " +
			...
			"Make sure to use HTTPS protocol as CodiBase relies on TLS encryption to ensure token " +
			"validity")
	@NotEmpty
	...
	@Editable(order=1000, description="OpenID client identification will be assigned by your OpenID "
			+ "provider when registering this CodiBase instance as client application")
	@NotEmpty
	...
	protected ProviderMetadata discoverProviderMetadata() {
		try {
			JsonNode json = CodiBase.getInstance(ObjectMapper.class).readTree(
