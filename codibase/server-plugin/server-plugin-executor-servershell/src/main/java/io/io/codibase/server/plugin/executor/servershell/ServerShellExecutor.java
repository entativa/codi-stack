...
import static io.codibase.agent.ExecutorUtils.newWarningLogger;
import static io.codibase.agent.ShellExecutorUtils.testCommands;
...
import static io.codibase.k8shelper.KubernetesHelper.replacePlaceholders;
...
import io.codibase.server.plugin.executor.servershell.ServerShellExecutor.TestData;
...
@Editable(order=ServerShellExecutor.ORDER, name="Server Shell Executor", description="" +
		"This executor runs build jobs with CodiBase server's shell facility.<br>" +
		"<b class='text-danger'>WARNING</b>: Jobs running with this executor has same " +
		"permission as CodiBase server process. Make sure it can only be used by trusted jobs")
					...
					var successful = entryFacade.execute(new LeafHandler() {
						...
						private boolean doExecute(LeafFacade facade, List<Integer> position) {
							if (facade instanceof CommandFacade) {
								...
								environments.put("CODIBASE_WORKSPACE", workspaceDir.getAbsolutePath());
	...
	private void checkApplicable() {
		if (CodiBase.getK8sService() != null) {
			throw new ExplicitException(""
					+ "CodiBase running inside kubernetes cluster does not support server shell executor. "
					+ "Please use kubernetes executor instead");
		} else if (Bootstrap.isInDocker()) {
			throw new ExplicitException("Server shell executor is only supported when CodiBase is installed "
