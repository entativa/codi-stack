package io.codibase.server.plugin.mail.gmail;
...
import io.codibase.server.mail.*;
import io.codibase.server.model.support.administration.mailservice.ImapImplicitSsl;
import io.codibase.server.model.support.administration.mailservice.MailConnector;
import io.codibase.server.model.support.administration.mailservice.SmtpExplicitSsl;
...
public class GmailConnector implements MailConnector {
	...
	@Editable(order=100, description="Client ID of this CodiBase instance registered in Google cloud")
	@NotEmpty
	...
	@Editable(order=200, description="Client secret of this CodiBase instance registered in Google cloud")
	@Password
	@NotEmpty
	...
	private void updateRefreshToken(String refreshToken) {
		this.refreshToken = refreshToken;
		CodiBase.getInstance(SettingService.class).saveMailConnector(this);
	}
	...
	private MailService getMailService() {
		return CodiBase.getInstance(MailService.class);
	}
