...
public class ImportResult {
	...
	public String toHtml(String leadingText) {
		...
		if (!unmappedIssueStatuses.isEmpty()) 
			feedback.append(getEntryFeedback("JIRA issue statuses not mapped to CodiBase custom field", unmappedIssueStatuses));
		if (!unmappedIssueTypes.isEmpty()) 
			feedback.append(getEntryFeedback("JIRA issue types not mapped to CodiBase custom field", unmappedIssueTypes));
		if (!unmappedIssuePriorities.isEmpty()) 
			feedback.append(getEntryFeedback("JIRA issue priorities not mapped to CodiBase custom field", unmappedIssuePriorities));
		if (!nonExistentLogins.isEmpty()) {
			feedback.append(getEntryFeedback("JIRA accounts not available in CodiBase (matching by full name)", nonExistentLogins));
		}
