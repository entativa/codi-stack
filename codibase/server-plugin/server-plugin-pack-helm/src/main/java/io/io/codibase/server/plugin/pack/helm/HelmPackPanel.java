...
public class HelmPackPanel extends GenericPanel<Pack> {
	...
	protected void onInitialize() {
		...
		add(new Label("addRepo", "$ helm repo add codibase --username <codibase_account_name> --password <codibase_password_or_access_token> " + registryUrl));
		add(new Label("installChart", "$ helm install " + getPack().getName() + " codibase/" + getPack().getName() + " --version " + getPack().getVersion()));
