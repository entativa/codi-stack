...
import static io.codibase.agent.DockerExecutorUtils.buildImage;
import static io.codibase.agent.DockerExecutorUtils.callWithDockerConfig;
import static io.codibase.agent.DockerExecutorUtils.changeOwner;
import static io.codibase.agent.DockerExecutorUtils.createNetwork;
import static io.codibase.agent.DockerExecutorUtils.deleteDir;
import static io.codibase.agent.DockerExecutorUtils.deleteNetwork;
import static io.codibase.agent.DockerExecutorUtils.getEntrypoint;
import static io.codibase.agent.DockerExecutorUtils.getOwner;
import static io.codibase.agent.DockerExecutorUtils.isUseProcessIsolation;
import static io.codibase.agent.DockerExecutorUtils.newDockerKiller;
import static io.codibase.agent.DockerExecutorUtils.pruneBuilderCache;
import static io.codibase.agent.DockerExecutorUtils.runImagetools;
import static io.codibase.agent.DockerExecutorUtils.startService;
import static io.codibase.agent.DockerExecutorUtils.useDockerSock;
import static io.codibase.agent.ExecutorUtils.newErrorLogger;
import static io.codibase.agent.ExecutorUtils.newInfoLogger;
import static io.codibase.k8shelper.KubernetesHelper.cloneRepository;
import static io.codibase.k8shelper.KubernetesHelper.installGitCert;
import static io.codibase.k8shelper.KubernetesHelper.stringifyStepPosition;
import static io.codibase.k8shelper.RegistryLoginFacade.merge;
...
import io.codibase.agent.DockerExecutorUtils;
...
import io.codibase.k8shelper.BuildImageFacade;
import io.codibase.k8shelper.CheckoutFacade;
...
import io.codibase.k8shelper.LeafFacade;
import io.codibase.k8shelper.LeafHandler;
...
import io.codibase.k8shelper.RunContainerFacade;
...
import io.codibase.k8shelper.SetupCacheFacade;
...
import io.codibase.server.job.JobRunnable;
import io.codibase.server.job.ResourceAllocator;
import io.codibase.server.job.ServerCacheHelper;
...
import io.codibase.server.model.support.administration.jobexecutor.RegistryLogin;
import io.codibase.server.plugin.executor.serverdocker.ServerDockerExecutor.TestData;
import io.codibase.server.terminal.CommandlineShell;
...
import io.codibase.server.web.util.Testable;

@Editable(order=ServerDockerExecutor.ORDER, name="Server Docker Executor", 
		description="This executor runs build jobs as docker containers on CodiBase server")
@ClassValidating
public class ServerDockerExecutor extends JobExecutor implements DockerAware, Testable<TestData>, Validatable {
	...
	private String dockerBuilder = "codibase";
	...
	@Editable(order=515, group="More Settings", name="Buildx Builder", description = "Specify dockerx builder used to " +
			"build docker image. CodiBase will create the builder automatically if it does not exist. Check " +
			"<a href='https://docs.codibase.io/tutorials/cicd/insecure-docker-registry' target='_blank'>this tutorial</a> " +
	...
	@Editable(order=520, group="Privilege Settings", description="Whether or not to mount docker sock into job container to "
			...
			+ "<b class='text-danger'>WARNING</b>: Malicious jobs can take control of whole CodiBase "
	...
	private void checkApplicable() {
		if (CodiBase.getK8sService() != null) {
			throw new ExplicitException(""
					+ "CodiBase running inside kubernetes cluster does not support server docker executor. "
		...
		ClusterTask<Boolean> runnable = () -> getJobService().runJob(jobContext, new JobRunnable() {
			...
			public boolean run(TaskLogger jobLogger) {
				...
				hostBuildHome = new File(Bootstrap.getTempDir(),
						"codibase-build-" + jobContext.getProjectId() + "-" + jobContext.getBuildNumber() + "-"	+ jobContext.getSubmitSequence());
				...
				try {
					...
					try {
						OsInfo osInfo = CodiBase.getInstance(OsInfo.class);
						...
						try {
							...
							if (SystemUtils.IS_OS_WINDOWS) {
								containerBuildHome = "C:\\codibase-build";
								containerWorkspace = "C:\\codibase-build\\workspace";
								containerTrustCerts = "C:\\codibase-build\\trust-certs.pem";
							} else {
								containerBuildHome = "/codibase-build";
								containerWorkspace = "/codibase-build/workspace";
								containerTrustCerts = "/codibase-build/trust-certs.pem";
							}
							...
							var successful = entryFacade.execute(new LeafHandler() {
								...
								private int runStepContainer(Commandline docker, String image, @Nullable String runAs, 
									...
									try {
										...
										docker.addArgs("-e", "CODIBASE_WORKSPACE=" + containerWorkspace);
							...
							// Fix https://code.codibase.io/codibase/server/~issues/597
							if (SystemUtils.IS_OS_WINDOWS) {
	...
	public void test(TestData testData, TaskLogger jobLogger) {
		...
		callWithDockerConfig(docker, getRegistryLogins(UUID.randomUUID().toString()), () -> {
			...
			try {
				...
				if (SystemUtils.IS_OS_WINDOWS) 
					containerWorkspacePath = "C:\\codibase-build\\workspace";
				else 
					containerWorkspacePath = "/codibase-build/workspace";
