package io.codibase.server.plugin.buildspec.golang;
...
import io.codibase.server.plugin.report.checkstyle.PublishCheckstyleReportStep;
	...
	public Collection<Job> suggestJobs(Project project, ObjectId commitId) {
		...
		if (project.getBlob(new BlobIdent(commitId.name(), "go.mod", FileMode.TYPE_FILE), false) != null) {
			...
			buildAndTest.getInterpreter().setCommands("" +
					"set -e\n" +
					"# Use double at to avoid being interpreted as CodiBase variable substitution\n" +
