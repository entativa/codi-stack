package io.codibase.server.web.component.gitsignature;

import io.codibase.server.CodiBase;
import io.codibase.server.git.signatureverification.SignatureVerificationService;
import io.codibase.server.git.signatureverification.VerificationResult;
...
public abstract class SignatureStatusPanel extends DropdownLink {
	...
	private IModel<VerificationResult> model = new LoadableDetachableModel<>() {
		...
		protected VerificationResult load() {
			return CodiBase.getInstance(SignatureVerificationService.class).verifySignature(getRevObject());
		}
