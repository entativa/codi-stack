...
public abstract class ParsedUrl implements Serializable {
	...
	private List<String> getServers() {
		return CodiBase.getInstance(ClusterService.class).getServerAddresses();
	}
		...
		case "~settings":
			if (SecurityUtils.canManageProject(project)) {
				if (segment2.equals("service-desk")) {
					return CodiBase.getInstance(SettingService.class).getServiceDeskSetting() != null
	...
	static Project getProject(Map<String, String> paramValues) {
		...
		return CodiBase.getInstance(ProjectService.class).findByPath(projectPath);
	}
	...
	static Build getBuild(Map<String, String> paramValues) {
		...
		return CodiBase.getInstance(BuildService.class).find(getProject(paramValues), buildNumber);
	}
	...
	static Issue getIssue(Map<String, String> paramValues) {
		...
		return CodiBase.getInstance(IssueService.class).find(getProject(paramValues), issueNumber);
	}
