		...
		add(new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				target.appendJavaScript(
						String.format("codibase.server.taskFeedback.processData('%s', %s, %s);", 
			...
			private String getData() {
				...
				try {
					return CodiBase.getInstance(ObjectMapper.class).writeValueAsString(data);
			...
			public void renderHead(Component component, IHeaderResponse response) {
				...
				String script = String.format("codibase.server.taskFeedback.processData('%s', %s, %s);", 
						getMarkupId(), getCallbackFunction(), getData());
