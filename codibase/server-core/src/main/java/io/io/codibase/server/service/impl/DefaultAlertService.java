...
import static io.codibase.server.model.Alert.PROP_SUBJECT;
...
import io.codibase.server.event.cluster.ConnectionLost;
	...
	public void on(ConnectionLost event) {
		if (clusterService.isLeaderServer()) {
			...
			alert("Server '" + server + "' can not be reached", 
					"Server '" + server + "' is part of CodiBase cluster, but can not be reached for some reason", 
					false);
		}
	}
