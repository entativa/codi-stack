...
import static io.codibase.server.util.DateUtils.toLocalDate;
...
import io.codibase.server.model.support.TimeGroups;
import io.codibase.server.ssh.SshKeyUtils;
import io.codibase.server.util.CryptoUtils;
import io.codibase.server.util.DateUtils;
import io.codibase.server.util.DirectoryVersionUtils;
import io.codibase.server.util.Pair;
import io.codibase.server.util.ParsedEmailAddress;
...
public class DataMigrator {
	...
	private void migrate15(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			try {
				String content = FileUtils.readFileToString(file, UTF_8);
				content = StringUtils.replace(content, "com.turbodev", "io.codibase");
				content = StringUtils.replace(content, "com/turbodev", "io/codibase");
				content = StringUtils.replace(content, "turbodev.com", "codibase.io");
				content = StringUtils.replace(content, "turbodev", "codibase");
				content = StringUtils.replace(content, "TurboDev", "CodiBase");
				FileUtils.writeFile(file, content, UTF_8);
			} catch (IOException e) {
				throw new RuntimeException(e);
	...
	private void migrate16(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("BranchWatches.xml")
				...
				content = StringUtils.replace(content, "io.codibase.server.security.authenticator.",
						"io.codibase.server.model.support.authenticator.");
				VersionedXmlDoc dom = VersionedXmlDoc.fromXML(content);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.Setting");
					Element settingElement = element.element("setting");
					if (settingElement != null) {
					...
					for (Element branchProtectionElement : element.element("branchProtections").elements()) {
						...
						for (Element verificationElement : verificationsElement.elements()) {
							...
							if (configurationId == null) {
								...
								Element configurationElement = configurationListElement.addElement("io.codibase.server.model.Configuration");
								...
								configurationElement.addElement("buildCleanupRule").addAttribute("class", "io.codibase.server.model.support.configuration.DoNotCleanup");
							}
							for (String request : openRequests) {
								Element requestBuildElement = requestBuildListElement.addElement("io.codibase.server.model.PullRequestBuild");
						...
						submitterClass = submitterClass.replace("io.codibase.server.model.support.submitter.",
								"io.codibase.server.model.support.usermatcher.");
			...
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (commitMessageTransformSettingElement != null) {
						...
						commitMessageTransformSettingElement.setName("io.codibase.server.model.support.CommitMessageTransform");
					...
					if (element.elementTextTrim("key").equals("ISSUE")) {
						...
						if (valueElement != null) {
							for (Element fieldElement : valueElement.element("fieldSpecs").elements()) {
								fieldElement.addElement("canBeChangedBy").addAttribute("class", "io.codibase.server.model.support.usermatcher.Anyone");
							}
						}
					}
				}
	...
	private void migrate21(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					if (element.elementTextTrim("key").equals("LICENSE")) {
						element.element("value").addAttribute("class", "io.codibase.commons.utils.license.LicenseDetail");
					}
				}
				dom.writeToFile(file, false);
			}
	...
	private void migrate22(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("IssueFieldUnarys.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.IssueFieldEntity");
				}
				dom.writeToFile(renamedFile, false);
			}
		}
	...
	private void migrate25(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Settings.xml")) {
				...
				content = content.replace("io.codibase.server.model.IssueFieldEntity",
						"io.codibase.server.model.IssueField");
			...
			if (file.getName().startsWith("Roles.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					element.addElement("editableIssueFields").addAttribute(
							"class", "io.codibase.server.model.support.role.AllIssueFields");
				}
				dom.writeToFile(file, false);
			}
		}
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (namedQueriesElement != null) {
						for (Element queryElement : namedQueriesElement.elements())
							queryElement.setName("io.codibase.server.model.support.build.NamedBuildQuery");
					}
					...
					for (Element secretElement : secretsElement.elements())
						secretElement.setName("io.codibase.server.model.support.build.JobSecret");
					for (Element buildPreservationElement : buildSettingElement.element("buildPreservations").elements())
						buildPreservationElement.setName("io.codibase.server.model.support.build.BuildPreservation");
	...
	private void migrate38(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().contains(".xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.pullrequest.NamedPullRequestQuery")) {
				...
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.issue.NamedIssueQuery")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						if (element.elementTextTrim("query").equals("all"))
							element.element("query").detach();
				...
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.build.NamedBuildQuery")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						if (element.elementTextTrim("query").equals("all"))
							element.element("query").detach();
				...
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.NamedProjectQuery")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						if (element.elementTextTrim("query").equals("all"))
							element.element("query").detach();
				...
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.build.BuildPreservation")) {
					if (node instanceof Element) {
						Element element = (Element) node;
	...
	private void migrate40(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().contains(".xml")) {
				try {
					String content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content, "io.codibase.server.issue.",
							"io.codibase.server.model.support.issue.");
					content = StringUtils.replace(content, "io.codibase.server.util.inputspec.",
							"io.codibase.server.model.support.inputspec.");
					FileUtils.writeFile(file, content, UTF_8);
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
		...
		if (!hasOwnerRole) {
			...
			Element ownerRoleElement = dom.getRootElement().addElement("io.codibase.server.model.Role");
			...
			ownerRoleElement.addElement("editableIssueFields").addAttribute("class",
					"io.codibase.server.model.support.role.AllIssueFields");
		...
		for (Map.Entry<String, String> entry : projectOwners.entrySet()) {
			Element userAuthorizationElement = dom.getRootElement().addElement("io.codibase.server.model.UserAuthorization");
			userAuthorizationElement.addAttribute("revision", "0.0");
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Settings.xml")) {
				try {
					...
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.administration.authenticator.ldap.",
							"io.codibase.server.plugin.authenticator.ldap.");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.issue.transitiontrigger.DiscardPullRequest",
							"io.codibase.server.model.support.issue.transitiontrigger.DiscardPullRequestTrigger");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.issue.transitiontrigger.MergePullRequest",
							"io.codibase.server.model.support.issue.transitiontrigger.MergePullRequestTrigger");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.issue.transitiontrigger.OpenPullRequest",
							"io.codibase.server.model.support.issue.transitiontrigger.OpenPullRequestTrigger");
					FileUtils.writeFile(file, content, UTF_8);
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (actionAuthorizationsElement.elements().isEmpty()) {
						actionAuthorizationsElement.addElement("io.codibase.server.model.support.build.actionauthorization.CreateTagAuthorization");
						actionAuthorizationsElement.addElement("io.codibase.server.model.support.build.actionauthorization.CloseMilestoneAuthorization");
					}
	...
	private void migrate53(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			try {
				String content = FileUtils.readFileToString(file, UTF_8);
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.fieldspec.",
						"io.codibase.server.model.support.issue.field.spec.");
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.fieldsupply.",
						"io.codibase.server.model.support.issue.field.supply.");
				content = StringUtils.replace(content,
						"org.server.plugin.report.checkstyle.",
						"io.codibase.server.plugin.report.checkstyle.");
				content = StringUtils.replace(content,
						"org.server.plugin.report.clover.",
						"io.codibase.server.plugin.report.clover.");
	...
	private void migrate57(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			try {
				String content = FileUtils.readFileToString(file, UTF_8);
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.inputspec.numberinput.",
						"io.codibase.server.model.support.inputspec.integerinput.");
				FileUtils.writeStringToFile(file, content, UTF_8);
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
					...
					if (element.elementTextTrim("key").equals("ISSUE")) {
						...
						if (valueElement != null) {
							for (Element fieldSpecElement : valueElement.element("fieldSpecs").elements()) {
								if (fieldSpecElement.getName().equals("io.codibase.server.model.support.issue.field.spec.NumberField"))
									fieldSpecElement.setName("io.codibase.server.model.support.issue.field.spec.IntegerField");
								else if (fieldSpecElement.getName().equals("io.codibase.server.model.support.issue.field.spec.TextField"))
					...
					if (element.elementTextTrim("key").equals("MAIL")) {
						...
						if (valueElement != null) {
							...
							if (senderAddressElement != null) {
								...
								valueElement.addElement("emailAddress").setText("codibase@" + hostName);
							}
						}
					}
				}
			...
			if (file.getName().startsWith("Settings.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					if (element.elementTextTrim("key").equals("JOB_EXECUTORS")) {
						...
						if (valueElement != null) {
							for (Element executorElement : valueElement.elements()) {
								if (executorElement.getName().equals("io.codibase.server.plugin.docker.DockerExecutor"))
									executorElement.setName("io.codibase.server.plugin.executor.docker.DockerExecutor");
							}
						}
					}
				}
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("PullRequests.xml")) {
				...
				if (file.getName().equals("Users.xml")) {
					Element element = dom.getRootElement().addElement("io.codibase.server.model.User");
				...
				if (oldSenderAuthorizationElements != null && !oldSenderAuthorizationElements.isEmpty()) {
					Element serviceDeskSettingElement = dom.getRootElement().addElement("io.codibase.server.model.Setting");
					...
					valueElement.addAttribute("class", "io.codibase.server.model.support.administration.ServiceDeskSetting");
					...
					for (Element oldSenderAuthorizationElement : oldSenderAuthorizationElements) {
						Element senderAuthorizationElement = senderAuthorizationsElement
								.addElement("io.codibase.server.model.support.administration.SenderAuthorization");
						Element projectDesignationElement = projectDesignationsElement
								.addElement("io.codibase.server.model.support.administration.ProjectDesignation");
						Element issueCreationSettingElement = issueCreationSettingsElement
								.addElement("io.codibase.server.model.support.administration.IssueCreationSetting");
	...
	private void migrate62(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Settings.xml")) {
						...
						if (valueElement != null) {
							for (Element executorElement : valueElement.elements()) {
								if (executorElement.getName().contains("DockerExecutor")) {
									executorElement.setName("io.codibase.server.plugin.executor.serverdocker.ServerDockerExecutor");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.IssueQueryPersonalization");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.PullRequestQueryPersonalization");
					element.element("userQueries").setName("queries");
					Element queryWatchesElement = element.element("queryWatches");
					for (Element queryWatchElement : queryWatchesElement.elements()) {
						Element queryNameElement = queryWatchElement.element("string");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.BuildQueryPersonalization");
					element.element("userQueries").setName("queries");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.CodeCommentQueryPersonalization");
					element.element("userQueries").setName("queries");
				}
				FileUtils.deleteFile(file);
				dom.writeToFile(new File(dataDir, file.getName().replace("Settings", "Personalizations")), false);
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.CommitQueryPersonalization");
					element.element("userQueries").setName("queries");
	...
	private void migrate67(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("JestTestMetric.xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements())
					element.setName("io.codibase.server.model.UnitTestMetric");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements())
					element.setName("io.codibase.server.model.CoverageMetric");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements())
					element.setName("io.codibase.server.model.ProblemMetric");
			...
			if (file.getName().startsWith("Issues.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (milestoneElement != null) {
						Element scheduleElement = listElement.addElement("io.codibase.server.model.IssueSchedule");
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (commentElement != null) {
						Element pullRequestCommentElement = pullRequestCommentsDom.getRootElement()
								.addElement("io.codibase.server.model.PullRequestComment");
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (commentElement != null) {
						Element issueCommentElement = issueCommentsDom.getRootElement()
								.addElement("io.codibase.server.model.IssueComment");
	...
	private void migrate82(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (userId.equals("-1")) {
						element.addElement("fullName").setText("CodiBase");
		...
		for (Map.Entry<String, String> entry : primaryEmails.entrySet()) {
			Element emailAddressElement = listElement.addElement("io.codibase.server.model.EmailAddress");
			...
			if (file.getName().startsWith("Settings.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (key.equals("MAIL")) {
						Element valueElement = element.element("value");
						if (valueElement != null) {
							valueElement.addAttribute("class", "io.codibase.server.model.support.administration.mailsetting.OtherMailSetting");
							Element receiveMailSettingElement = valueElement.element("receiveMailSetting");
							if (receiveMailSettingElement != null)
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					Element updateElement = listElement.addElement("io.codibase.server.model.ProjectUpdate");
			...
			if (file.getName().startsWith("Settings.xml")) {
					...
					if (key.equals("SYSTEM")) {
						...
						if (valueElement != null) {
							...
							var clazz = gitConfigElement.attributeValue("class").replace(
									"io.codibase.server.git.config.",
									"io.codibase.server.git.location.");
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					for (var branchProtectionElement : element.element("branchProtections").elements()) {
						branchProtectionElement.setName("io.codibase.server.model.support.code.BranchProtection");
						for (var fileProtectionElement : branchProtectionElement.element("fileProtections").elements())
							fileProtectionElement.setName("io.codibase.server.model.support.code.FileProtection");
					}
					for (var tagProtectionElement : element.element("tagProtections").elements()) {
						tagProtectionElement.setName("io.codibase.server.model.support.code.TagProtection");
					}
				}
				dom.writeToFile(file, false);
			} else if (file.getName().startsWith("Agents.xml")) {
	...
	private Set<String> getMentioned108(Map<String, String> userIds, String content) {
		...
		MarkdownService markdownService = CodiBase.getInstance(MarkdownService.class);
	...
	private void migrate108(File dataDir, Stack<Integer> versions) {
		...
		for (Pair<String, String> issueMention : issueMentions) {
			Element mentionElement = mentionsElement.addElement("io.codibase.server.model.IssueMention");
		...
		for (Pair<String, String> it : pullRequestMentions) {
			Element mentionElement = mentionsElement.addElement("io.codibase.server.model.PullRequestMention");
			mentionElement.addElement("id").setText(String.valueOf(id++));
			mentionElement.addAttribute("revision", "0.0");
		...
		for (Pair<String, String> it : codeCommentMentions) {
			Element mentionElement = mentionsElement.addElement("io.codibase.server.model.CodeCommentMention");
			mentionElement.addElement("id").setText(String.valueOf(id++));
			mentionElement.addAttribute("revision", "0.0");
	...
	private void migrate109(File dataDir, Stack<Integer> versions) {
			...
			if (file.getName().startsWith("ProjectUpdates.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					if (!updateIds.contains(element.elementTextTrim("id"))) {
						...
						element.setName("io.codibase.server.model.ProjectDynamics");
					}
				}
				FileUtils.deleteFile(file);
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().contains(".xml")) {
				...
				selectedNodes.addAll(dom.selectNodes("//io.codibase.server.model.support.pullrequest.NamedPullRequestQuery"));
				selectedNodes.addAll(dom.selectNodes("//io.codibase.server.model.support.issue.NamedIssueQuery"));
				selectedNodes.addAll(dom.selectNodes("//io.codibase.server.model.support.NamedCodeCommentQuery"));
				selectedNodes.addAll(dom.selectNodes("//io.codibase.server.model.support.NamedProjectQuery"));
			...
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					if (element.elementTextTrim("key").equals("ISSUE")) {
						...
						if (valueElement != null) {
							for (Element fieldSpecElement : valueElement.element("fieldSpecs").elements()) {
								if (fieldSpecElement.getName().equals("io.codibase.server.model.support.issue.field.spec.ChoiceField")) {
									...
									if (defaultValueProviderElement != null
										...
										defaultValuesElement.addElement("io.codibase.server.model.support.inputspec.choiceinput.defaultvalueprovider.DefaultValue").add(defaultValueElement);
									}
									...
									if (defaultMultiValueProviderElement != null
										...
										defaultValueElement.detach();
										defaultValuesElement.addElement("io.codibase.server.model.support.inputspec.choiceinput.defaultmultivalueprovider.DefaultMultiValue").add(defaultValueElement);
									}
								}
							}
						}
	...
	private void migrate113(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Settings.xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.issue.field.spec.ChoiceField")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.choicefield.ChoiceField");
					}
				}
				for (Node node : dom.selectNodes("//defaultValueProvider")) {
					if (node instanceof Element) {
						...
						if (clazz != null) {
							clazz = clazz.replace(
									"io.codibase.server.model.support.inputspec.choiceinput.defaultvalueprovider.",
									"io.codibase.server.model.support.issue.field.spec.choicefield.defaultvalueprovider.");
				...
				for (Node node : dom.selectNodes("//defaultMultiValueProvider")) {
					if (node instanceof Element) {
						...
						if (clazz != null) {
							clazz = clazz.replace(
									"io.codibase.server.model.support.inputspec.choiceinput.defaultmultivalueprovider.",
									"io.codibase.server.model.support.issue.field.spec.choicefield.defaultmultivalueprovider.");
							element.addAttribute("class", clazz);
						}
					}
				}
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.inputspec.choiceinput.defaultvalueprovider.DefaultValue")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.choicefield.defaultvalueprovider.DefaultValue");
					}
				}
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.inputspec.choiceinput.defaultmultivalueprovider.DefaultMultiValue")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.choicefield.defaultmultivalueprovider.DefaultMultiValue");
					}
				}
				dom.writeToFile(file, false);
			}
	...
	private void migrate114(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			try {
				...
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.inputspec.",
						"io.codibase.server.buildspecmodel.inputspec.");
				FileUtils.writeStringToFile(file, content, UTF_8);
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
			...
			if (file.getName().startsWith("Settings.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					if (element.elementTextTrim("key").equals("ISSUE")) {
						...
						if (valueElement != null) {
							for (Element fieldSpecElement : valueElement.element("fieldSpecs").elements()) {
								if (fieldSpecElement.getName().equals("io.codibase.server.model.support.issue.field.spec.UserChoiceField")) {
									Element defaultValueProviderElement = fieldSpecElement.element("defaultValueProvider");
									if (defaultValueProviderElement != null
											&& defaultValueProviderElement.attributeValue("class").contains("SpecifiedDefaultValue")) {
										Element defaultValueElement = defaultValueProviderElement.element("value");
										Element defaultValuesElement = defaultValueProviderElement.addElement("defaultValues");
										defaultValueElement.detach();
										defaultValuesElement.addElement("io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultvalueprovider.DefaultValue").add(defaultValueElement);
									}
									Element defaultMultiValueProviderElement = fieldSpecElement.element("defaultMultiValueProvider");
									if (defaultMultiValueProviderElement != null
											&& defaultMultiValueProviderElement.attributeValue("class").contains("SpecifiedDefaultMultiValue")) {
										...
										Element defaultValuesElement = defaultMultiValueProviderElement.addElement("defaultValues");
										defaultValueElement.detach();
										defaultValuesElement.addElement("io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultmultivalueprovider.DefaultMultiValue").add(defaultValueElement);
									}
								}
							}
						}
	...
	private void migrate116(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Settings.xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Node node : dom.selectNodes("//io.codibase.server.model.support.issue.field.spec.UserChoiceField")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.userchoicefield.UserChoiceField");
					}
				}
				for (Node node : dom.selectNodes("//defaultValueProvider")) {
					if (node instanceof Element) {
						...
						if (clazz != null) {
							clazz = clazz.replace(
									"io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultvalueprovider.",
									"io.codibase.server.model.support.issue.field.spec.userchoicefield.defaultvalueprovider.");
							element.addAttribute("class", clazz);
						}
					}
				}
				for (Node node : dom.selectNodes("//defaultMultiValueProvider")) {
					if (node instanceof Element) {
						...
						if (clazz != null) {
							clazz = clazz.replace(
									"io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultmultivalueprovider.",
									"io.codibase.server.model.support.issue.field.spec.userchoicefield.defaultmultivalueprovider.");
							element.addAttribute("class", clazz);
						}
					}
				}
				for (Node node : dom.selectNodes("//io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultvalueprovider.DefaultValue")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.userchoicefield.defaultvalueprovider.DefaultValue");
					}
				}
				for (Node node : dom.selectNodes("//io.codibase.server.buildspecmodel.inputspec.userchoiceinput.defaultmultivalueprovider.DefaultMultiValue")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.model.support.issue.field.spec.userchoicefield.defaultmultivalueprovider.DefaultMultiValue");
					}
				}
				dom.writeToFile(file, false);
			}
	...
	private void migrate117(File dataDir, Stack<Integer> versions) {
		...
		for (var file : dataDir.listFiles()) {
			if (file.getName().startsWith("ClusterCredentials.xml")) {
				...
				var dom = VersionedXmlDoc.fromFile(file);
				for (var element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.ProjectLastEventDate");
				...
				for (var element : dom.getRootElement().elements()) {
					...
					lastUsedDateElement = agentLastUsedDatesElement.addElement("io.codibase.server.model.AgentLastUsedDate");
					...
					if (!agentTokenIds.remove(tokenId)) {
						...
						var agentTokenElement = partialAgentTokensElement.addElement("io.codibase.server.model.AgentToken");
				...
				for (Element element : dom.getRootElement().elements()) {
					if (element.elementTextTrim("key").equals("ISSUE")) {
						...
						if (valueElement != null) {
							...
							var entryElement = entriesElement.addElement("io.codibase.server.model.support.issue.CommitMessageFixPatterns_-Entry");
							...
							entryElement = entriesElement.addElement("io.codibase.server.model.support.issue.CommitMessageFixPatterns_-Entry");
	...
	private void migrate123(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Issues.xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					var issueTouchElement = issueTouchesElement.addElement("io.codibase.server.model.IssueTouch");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					var pullRequestTouchElement = pullRequestTouchesElement.addElement("io.codibase.server.model.PullRequestTouch");
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					var codeCommentTouchElement = codeCommentTouchesElement.addElement("io.codibase.server.model.CodeCommentTouch");
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Users.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					var newAccessTokenElement = accessTokensElement.addElement("io.codibase.server.model.support.AccessToken");
							...
							for (Element executorElement : valueElement.elements()) {
								if (executorElement.getName().contains("KubernetesExecutor")
									...
									for (var registryLoginElement : executorElement.element("registryLogins").elements()) {
										registryLoginElement.setName("io.codibase.server.model.support.administration.jobexecutor.RegistryLogin");
									}
								}
							}
						}
			...
			if (file.getName().startsWith("CodeCommentTouchs.xml") || file.getName().startsWith("PullRequestTouchs.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (keyElement.getTextTrim().equals("NOTIFICATION_TEMPLATE_SETTING")) {
						...
						emailTemplatesElement.addAttribute("class", "io.codibase.server.model.support.administration.emailtemplates.EmailTemplates");
						...
						if (valueElement != null) {
							if (valueElement.attributeValue("class").contains("OtherMailSetting")) {
								...
								if (enableStartTLSElement.getTextTrim().equals("true")) {
									sslSettingElement.addAttribute("class", "io.codibase.server.model.support.administration.mailsetting.SmtpExplicitSsl");
									sslSettingElement.addElement("trustAll").setText("false");
								} else {
									sslSettingElement.addAttribute("class", "io.codibase.server.model.support.administration.mailsetting.SmtpWithoutSsl");
								}
								...
								if (pollSettingElement != null) {
									...
									if (enableSSLElement.getTextTrim().equals("true")) {
										sslSettingElement.addAttribute("class", "io.codibase.server.model.support.administration.mailsetting.ImapImplicitSsl");
										sslSettingElement.addElement("trustAll").setText("false");
									} else {
										sslSettingElement.addAttribute("class", "io.codibase.server.model.support.administration.mailsetting.ImapWithoutSsl");
									}
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				try {
					content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content,
							"io.codibase.server.util.channelnotification.",
							"io.codibase.server.model.support.channelnotification.");
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
				VersionedXmlDoc dom = VersionedXmlDoc.fromXML(content);
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (dataElement.attributeValue("class").startsWith("io.codibase.server.model.support.issue.changedata.IssueLink"))
						element.detach();
				}
				dom.writeToFile(file, false);
			} else if (file.getName().startsWith("Settings.xml")) {
				...
				try {
					content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.administration.mailsetting.Office365Setting",
							"io.codibase.server.plugin.mailservice.office365.Office365MailService");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.administration.mailsetting.GmailSetting",
							"io.codibase.server.plugin.mailservice.gmail.GmailMailService");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.administration.mailsetting.OtherMailSetting",
							"io.codibase.server.plugin.mailservice.smtpimap.SmtpImapMailService");
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.administration.mailsetting.",
							"io.codibase.server.model.support.administration.mailservice.");
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
				VersionedXmlDoc dom = VersionedXmlDoc.fromXML(content);
	...
	private void migrate150(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			try {
				...
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.field.supply.FieldSupply",
						"io.codibase.server.model.support.issue.field.instance.FieldInstance");
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.field.supply.Ignore",
						"io.codibase.server.model.support.issue.field.instance.IgnoreValue");
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.field.supply.ScriptingValue",
						"io.codibase.server.model.support.issue.field.instance.ScriptingValue");
				content = StringUtils.replace(content,
						"io.codibase.server.model.support.issue.field.supply.SpecifiedValue",
						"io.codibase.server.model.support.issue.field.instance.SpecifiedValue");
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Packs.xml")) {
				try {
					var content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content,
							"io.codibase.server.ee.pack.",
							"io.codibase.server.plugin.pack.");
					FileUtils.writeStringToFile(file, content, UTF_8.name());
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
			...
			if (file.getName().startsWith("Packs.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					for (var widgetElement : widgetsElement.elements()) {
						...
						var newWidgetElement = widgetsElement.addElement("io.codibase.server.model.support.widget.Widget");
			...
			if (file.getName().startsWith("Settings.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (key.equals("JOB_EXECUTORS")) {
						...
						Element valueElement = element.element("value");
						if (valueElement != null)
							valueElement.addElement("url").setText("https://codibase.io");
					} else if (key.equals("SECURITY")) {
						Element valueElement = element.element("value");
						if (valueElement != null)
						...
						if (valueElement != null) {
							for (Element executorElement : valueElement.elements()) {
								if (executorElement.getName().contains("ServerDockerExecutor")
									...
									executorElement.addElement("dockerBuilder").setText("codibase");
								}
			...
			if (file.getName().startsWith("Users.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					for (var accessTokenElement : accessTokensElement.elements()) {
						var newAccessTokenElement = listElement.addElement("io.codibase.server.model.AccessToken");
	...
	private void migrate167(File dataDir, Stack<Integer> versions) {
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements())
					element.setName("io.codibase.server.model.Iteration");
				dom.writeToFile(file, false);
			} else if (file.getName().startsWith("Settings.xml")) {
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					...
					if (key.equals("ISSUE")) {
						for (var fieldSpecElement : element.element("value").element("fieldSpecs").elements()) {
							if (fieldSpecElement.getName().contains("MilestoneChoiceField"))
								fieldSpecElement.setName("io.codibase.server.model.support.issue.field.spec.IterationChoiceField");
						}
					}
				}
				dom.writeToFile(file, false);
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Node node : dom.selectNodes("//io.codibase.server.ee.dashboard.widgets.MilestoneListWidget")) {
					if (node instanceof Element) {
						Element element = (Element) node;
						element.setName("io.codibase.server.ee.dashboard.widgets.IterationListWidget");
					}
				}
				for (Node node : dom.selectNodes("//io.codibase.server.ee.dashboard.widgets.BurnDownChartWidget")) {
					if (node instanceof Element) {
						Element element = (Element) node;
				...
				for (Node node : dom.selectNodes("//io.codibase.server.ee.dashboard.widgets.BuildListWidget")) {
					if (node instanceof Element)
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (dataElement.attributeValue("class").contains("IssueMilestoneAddData"))
						dataElement.addAttribute("class", "io.codibase.server.model.support.issue.changedata.IssueIterationAddData");
					else if (dataElement.attributeValue("class").contains("IssueMilestoneRemoveData"))
						dataElement.addAttribute("class", "io.codibase.server.model.support.issue.changedata.IssueIterationRemoveData");
					else if (dataElement.attributeValue("class").contains("IssueMilestoneChangeData"))
						dataElement.addAttribute("class", "io.codibase.server.model.support.issue.changedata.IssueIterationChangeData");
	...
	private void migrate171(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			try {
				var content = StringUtils.replace(
						FileUtils.readFileToString(file, UTF_8),
						"io.codibase.server.ee.dashboard.widgets.BuildListWidget",
						"io.codibase.server.ee.dashboard.widgets.build.BuildListWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.BurnDownChartWidget",
						"io.codibase.server.ee.dashboard.widgets.iteration.BurnDownChartWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.IssueListWidget",
						"io.codibase.server.ee.dashboard.widgets.issue.IssueListWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.IterationListWidget",
						"io.codibase.server.ee.dashboard.widgets.iteration.IterationListWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.MarkdownBlobWidget",
						"io.codibase.server.ee.dashboard.widgets.markdown.MarkdownBlobWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.MarkdownWidget",
						"io.codibase.server.ee.dashboard.widgets.markdown.MarkdownWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.ProjectListWidget",
						"io.codibase.server.ee.dashboard.widgets.project.ProjectListWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.PullRequestListWidget",
						"io.codibase.server.ee.dashboard.widgets.pullrequest.PullRequestListWidget");
				content = StringUtils.replace(
						content,
						"io.codibase.server.ee.dashboard.widgets.projectoverview.ProjectOverviewWidget",
						"io.codibase.server.ee.dashboard.widgets.project.ProjectOverviewWidget");
				FileUtils.writeStringToFile(file, content, UTF_8);
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
	...
	private Element migrate171_createHistoryElement(Element parentElement, IssueStateHistory history) {
		var historyElement = parentElement.addElement("io.codibase.server.model.IssueStateHistory");
					...
					if (key.equals("SYSTEM")) {
						...
						for (var transitionSpecElement : transitionSpecsElement.elements()) {
							...
							if (triggerClass.contains("PressButtonTrigger")) {
								...
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.ManualSpec");
							} else if (triggerClass.contains("BranchUpdateTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.BranchUpdatedSpec");
							} else if (triggerClass.contains("BuildSuccessfulTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.BuildSuccessfulSpec");
							} else if (triggerClass.contains("DiscardPullRequestTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.PullRequestDiscardedSpec");
							} else if (triggerClass.contains("MergePullRequestTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.PullRequestMergedSpec");
							} else if (triggerClass.contains("NoActivityTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.NoActivitySpec");
							} else if (triggerClass.contains("OpenPullRequestTrigger")) {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.PullRequestOpenedSpec");
							} else {
								transitionSpecElement.setName("io.codibase.server.model.support.issue.transitionspec.IssueStateTransitedSpec");
							}
						...
						var transitionSpecElement = transitionSpecsElement.addElement("io.codibase.server.model.support.issue.transitionspec.ManualSpec");
					...
					if (key.equals("SYSTEM")) {
						...
						if (valueElement != null) {
							...
							for (var projectDesignationElement: projectDesignationsElement.elements()) {
								projectDesignationElement.setName("io.codibase.server.model.support.administration.DefaultProjectSetting");
							}
						}
					} else if (key.equals("JOB_EXECUTORS")) {
						Element valueElement = element.element("value");
					...
					for (var widgetElement : element.element("widgets").elements()) {
						...
						for (var tabElement: widgetElement.element("tabs").elements()) {
							if (tabElement.getName().endsWith("CompositeWidget")) {
								...
								tabElement.setName("io.codibase.server.ee.dashboard.widgets.markdown.MarkdownWidget");
	...
	private void migrate189(File dataDir, Stack<Integer> versions) {
		...
		for (var entry : pullRequestIdToProjectId.entrySet()) {
			var pullRequestTouchElement = listElement.addElement("io.codibase.server.model.PullRequestTouch");
			pullRequestTouchElement.addAttribute("revision", "0.0.0");
		...
		for (var entry : codeCommentIdToProjectId.entrySet()) {
			var codeCommentTouchElement = listElement.addElement("io.codibase.server.model.CodeCommentTouch");
			codeCommentTouchElement.addAttribute("revision", "0.0.0");
						...
						if (element.elementTextTrim("key").equals("CONTRIBUTED_SETTINGS")) {
							...
							if (valueElement != null) {
								for (var entryElement: valueElement.elements()) {
									var storageSettingElement = entryElement.element("io.codibase.server.ee.storage.StorageSetting");
				...
				if (file.getName().startsWith("PackBlobReferences.xml")) {
					...
					for (Element element : dom.getRootElement().elements()) {
						...
						if (packBlobCopyId == null) {
							...
							var targetPackBlobElement = packBlobsDom.getRootElement().addElement("io.codibase.server.model.PackBlob");
						...
						if (valueElement != null) {
							for (Element connectorElement : valueElement.elements()) {
								...
								if (buttonImageUrlElement != null) {
									...
									if (buttonImageUrl.startsWith("/wicket/resource/io.codibase.server.plugin.sso.openid.openidconnector")) {
										buttonImageUrl = "/wicket/resource/io.codibase.server.plugin.sso.openid.OpenIdConnector/openid.png";
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Users.xml")) {
				...
				try {
					String content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content,
							"io.codibase.server.model.support.issue.field.spec.WorkingPeriodField",
							"io.codibase.server.model.support.issue.field.spec.IntegerField");
					content = StringUtils.replace(content,
							"io.codibase.server.buildspecmodel.inputspec.workingperiodinput",
							"io.codibase.server.buildspecmodel.inputspec.integerinput");
					FileUtils.writeFile(file, content, UTF_8);
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
	...
	private void migrate199(File dataDir, Stack<Integer> versions) {
		...
		if (defaultLoginGroupId != null) {			
			...
			for (String userId: userIdsNotInDefaultLoginGroup) {
				var membershipElement = listElement.addElement("io.codibase.server.model.Membership");
						...
						if (valueElement != null) {
							for (Element fieldSpecElement : valueElement.element("fieldSpecs").elements()) {
								...
								switch (fieldSpecElement.getName()) {
									case "io.codibase.server.model.support.issue.field.spec.choicefield.ChoiceField":
										fieldTypes.put(name, InputSpec.ENUMERATION);
										break;
									case "io.codibase.server.model.support.issue.field.spec.IntegerField":
										fieldTypes.put(name, InputSpec.INTEGER);
										break;
									case "io.codibase.server.model.support.issue.field.spec.FloatField":
										fieldTypes.put(name, InputSpec.FLOAT);
										break;
									case "io.codibase.server.model.support.issue.field.spec.BooleanField":
										fieldTypes.put(name, InputSpec.BOOLEAN);
										break;
									case "io.codibase.server.model.support.issue.field.spec.DateField":
										fieldTypes.put(name, InputSpec.DATE);
										break;
									case "io.codibase.server.model.support.issue.field.spec.DateTimeField":
										fieldTypes.put(name, InputSpec.DATE_TIME);
										break;
									case "io.codibase.server.model.support.issue.field.spec.SecretField":
										fieldTypes.put(name, InputSpec.SECRET);
										break;
									case "io.codibase.server.model.support.issue.field.spec.userchoicefield.UserChoiceField":
										fieldTypes.put(name, InputSpec.USER);
										break;
									case "io.codibase.server.model.support.issue.field.spec.GroupChoiceField":
										fieldTypes.put(name, InputSpec.GROUP);
										break;
									case "io.codibase.server.model.support.issue.field.spec.BuildChoiceField":
										fieldTypes.put(name, InputSpec.BUILD);
										break;
									case "io.codibase.server.model.support.issue.field.spec.PullRequestChoiceField":
										fieldTypes.put(name, InputSpec.PULL_REQUEST);
										break;
									case "io.codibase.server.model.support.issue.field.spec.CommitField":
										fieldTypes.put(name, InputSpec.COMMIT);
										break;
									case "io.codibase.server.model.support.issue.field.spec.IssueChoiceField":
										fieldTypes.put(name, InputSpec.ISSUE);
										break;
									case "io.codibase.server.model.support.issue.field.spec.IterationChoiceField":
										fieldTypes.put(name, InputSpec.ITERATION);
										break;
									case "io.codibase.server.model.support.issue.field.spec.TextField":
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (dataClass.contains("IssueBatchUpdateData") || dataClass.contains("IssueFieldChangeData")
						...
						for (Element entryElement : dataElement.element("oldFields").elements()) {
							var inputElement = entryElement.element("io.codibase.server.util.Input");
			...
			if (file.getName().startsWith("Projects.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (defaultRoleElement != null) {
						var baseAuthorizationElement = baseAuthorizationsListElement.addElement("io.codibase.server.model.BaseAuthorization");
			...
			if (file.getName().startsWith("Settings.xml")) {
				...
				for (Element element : dom.getRootElement().elements()) {
					...
					if (key.equals("SYSTEM_UUID")) {
						Element valueElement = element.element("value");
						if (valueElement != null) {
							valueElement.setText(CodiBase.getInstance(SettingService.class).encryptUUID(valueElement.getText().trim()));
						}
	...
	private void migrate207(File dataDir, Stack<Integer> versions) {
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Projects.xml")) {
				...
				VersionedXmlDoc dom = VersionedXmlDoc.fromFile(file);
				for (Element element : dom.getRootElement().elements()) {
					element.setName("io.codibase.server.model.ProjectLastActivityDate");
		...
		for (File file : dataDir.listFiles()) {
			if (file.getName().startsWith("Groups.xml")) {
				...
				try {
					var content = FileUtils.readFileToString(file, UTF_8);
					content = StringUtils.replace(content,
							"io.codibase.server.util.Input",
							"io.codibase.server.buildspecmodel.inputspec.Input");
					FileUtils.writeStringToFile(file, content, UTF_8.name());
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
					...
					if (key.equals("SSO_CONNECTORS")) {
						...
						if (valueElement != null) {
							for (var connectorElement : valueElement.elements()) {
								...
								var ssoProviderElement = ssoProvidersListElement.addElement("io.codibase.server.model.SsoProvider");
