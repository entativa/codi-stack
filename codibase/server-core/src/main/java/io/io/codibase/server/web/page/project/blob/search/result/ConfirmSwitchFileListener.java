...
class ConfirmSwitchFileListener implements IAjaxCallListener {
	...
	public CharSequence getPrecondition(Component component) {
		if (path != null) {
			return String.format("return codibase.server.searchResult.confirmSwitchFileByPath('%s', %b);", 
					StringEscapeUtils.escapeEcmaScript(path), hasMark);
		} else {
			return String.format("return codibase.server.searchResult.confirmSwitchFileByLink('%s');", 
