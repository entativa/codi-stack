...
public class CodeContribsPage extends CodeStatsPage {
	...
	protected void onInitialize() {
		...
		add(userCardBehavior = new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				replace(userCard);
				target.add(userCard);
				target.appendJavaScript("codibase.server.codeStats.contribs.onUserCardAvailable();");
			}
	...
	public void renderHead(IHeaderResponse response) {
		...
		try {
			...
			String script = String.format("codibase.server.codeStats.contribs.onDomReady(%s, '%s', %s, %b, %s);", 
