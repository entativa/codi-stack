		...
		Form<?> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				if (parentError != null) {
					...
					if (editor.isValid()) {
						CodiBase.getInstance(TransactionService.class).run(new Runnable() {

							@Override
							public void run() {
								var project = getProject();
								getProjectService().update(project);
								CodiBase.getInstance(BaseAuthorizationService.class).syncRoles(project, defaultRolesBean.getRoles());
								CodiBase.getInstance(ProjectLabelService.class).sync(project, labelsBean.getLabels());
								var auditData = editor.getPropertyValues();
								auditData.put("defaultRoles", defaultRolesBean.getRoleNames());
								auditData.put("labels", labelsBean.getLabels());
								auditData.put("parent", parentBean.getParentPath());
			...
			public void onClick(AjaxRequestTarget target) {
				new ConfirmModalPanel(target) {
					...
					protected void onConfirm(AjaxRequestTarget target) {
						Project project = getProject();
						CodiBase.getInstance(ProjectService.class).delete(project);
