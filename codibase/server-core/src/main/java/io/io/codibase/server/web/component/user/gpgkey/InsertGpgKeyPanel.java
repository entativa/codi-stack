    ...
    protected void onInitialize() {
        ...
        form.add(new AjaxButton("add") {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                ...
                GpgKeyService gpgKeyService = CodiBase.getInstance(GpgKeyService.class);
                ...
                if (gpgKey.getKeyIds().stream().anyMatch(it->gpgKeyService.findSigningKey(it)!=null)) { 
                	...
                	for (String emailAddressValue: GpgUtils.getEmailAddresses(gpgKey.getPublicKeys().get(0))) {
                    	EmailAddress emailAddress = CodiBase.getInstance(EmailAddressService.class).findByValue(emailAddressValue);
                	...
                	if (!hasErrors) {
                        ...
                        if (getPage() instanceof UserPage)
							CodiBase.getInstance(AuditService.class).audit(null, "added GPG key \"" + GpgUtils.getKeyIDString(gpgKey.getKeyId()) + "\" in account \"" + gpgKey.getOwner().getName() + "\"", null, null);
                        onSave(target);
                	}
                }
            }
