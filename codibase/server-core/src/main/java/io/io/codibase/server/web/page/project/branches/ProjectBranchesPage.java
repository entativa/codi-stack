...
import io.codibase.server.git.service.AheadBehind;
...
import io.codibase.server.search.entity.pullrequest.PullRequestQueryLexer;
...
import io.codibase.server.web.component.link.ArchiveMenuLink;
...
public class ProjectBranchesPage extends ProjectPage {
	...
	private IModel<Map<ProjectAndBranch, PullRequest>> effectiveRequestsModel = 
		...
		protected Map<ProjectAndBranch, PullRequest> load() {
			...
			return CodiBase.getInstance(PullRequestService.class).findEffectives(target, sources);
		}
					...
					protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
						...
						if (getProject().getObjectId(GitUtils.branch2ref(branchName), false) != null) {
							...
							if (!getProject().isCommitSignatureRequirementSatisfied(user, branchName, commit)) {
								...
								CodiBase.getInstance(GitService.class).createBranch(getProject(), branchName, helperBean.getRevision());
								modal.close();
								target.add(branchesTable);
				...
				actionsContainer.add(new ModalLink("delete") {
					...
					protected Component newContent(String id, ModalPanel modal) {
						...
						fragment.add(new AjaxLink<Void>("delete") {
							...
							@Override
							public void onClick(AjaxRequestTarget target) {
								CodiBase.getInstance(ProjectService.class).deleteBranch(getProject(), branch);
