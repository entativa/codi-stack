...
import static io.codibase.server.web.page.security.SsoProcessPage.MOUNT_PATH;
import static io.codibase.server.web.page.security.SsoProcessPage.STAGE_INITIATE;
...
import io.codibase.server.security.realm.PasswordAuthenticatingRealm;
import io.codibase.server.web.component.link.ViewStateAwarePageLink;
import io.codibase.server.web.component.user.twofactorauthentication.TwoFactorAuthenticationSetupPanel;
import io.codibase.server.web.page.simple.SimpleCssResourceReference;
...
public class LoginPage extends SimplePage {
	...
	protected void onInitialize() {
		...
		Form<?> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				try {
					...
					var user = (User) CodiBase.getInstance(PasswordAuthenticatingRealm.class).getAuthenticationInfo(token);
					if (user.isEnforce2FA()) {
						if (user.getTwoFactorAuthentication() != null) {
							subTitle = _T("Two-factor authentication is enabled. Please input passcode displayed on your TOTP authenticator. If you encounter problems, make sure time of CodiBase server and your device running TOTP authenticator is in sync");
		...
		var ssoProviderService = CodiBase.getInstance(SsoProviderService.class);
	...
	private void afterLogin(User user) {
		RememberMeManager rememberMeManager = CodiBase.getInstance(RememberMeManager.class);
		if (rememberMe) {
	...
	protected String getTitle() {
		return _T("Sign In To") + " " + CodiBase.getInstance(SettingService.class).getBrandingSetting().getName();
	}
