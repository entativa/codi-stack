package io.codibase.server.web.page.project.pullrequests.detail.changes;

import com.google.common.base.Preconditions;
import com.google.common.collect.Sets;
import io.codibase.commons.utils.PlanarRange;
import io.codibase.server.CodiBase;
import io.codibase.server.codequality.*;
...
import io.codibase.server.web.component.diff.revision.RevisionDiffReviewSupport;
...
public class PullRequestChangesPage extends PullRequestDetailPage implements RevisionAnnotationSupport, EditParamsAware {
	...
	private ReviewedDiffService getDiffReviewStatusManager() {
		return CodiBase.getInstance(ReviewedDiffService.class);
	}
			...
			protected Component newContent(String id, FloatingPanel dropdown) {
				...
				Fragment fragment = new Fragment(id, "commitsFrag", PullRequestChangesPage.this) {
					...
					public void renderHead(IHeaderResponse response) {
						...
						if ((state.oldCommitHash.equals(getPullRequest().getBaseCommitHash()) || oldIndex != -1) 
								&& newIndex != -1) {
							script = String.format("codibase.server.requestChanges.initCommitSelector(%s, '%s', %d, %d);", 
									callback, getPullRequest().getBaseCommitHash(), oldIndex+1, newIndex);
						} else {
							script = String.format("codibase.server.requestChanges.initCommitSelector(%s, '%s');", 
