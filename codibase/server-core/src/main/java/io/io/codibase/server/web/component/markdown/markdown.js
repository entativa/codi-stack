codibase.server.markdown = {
	...
	onDomReady: function(containerId, callback, atWhoLimit, attachmentUploadUrl, 
		...
		codibase.server.markdown.translations = translations;
		...
		if ($suggestion.attr("disabled") == "disabled") {
			$suggestion.attr("data-tippy-content", codibase.server.markdown.translations["commented-code-outdated"]);
		} else {
			if (codibase.server.util.isMac())		
				$suggestion.attr("data-tippy-content", codibase.server.markdown.translations["suggest-changes"] + " (cmd-g)");
			else
				$suggestion.attr("data-tippy-content", codibase.server.markdown.translations["suggest-changes"] + " (ctrl-g)");
			...
			$suggestion.click(function() {
				...
				$input.focus();
				var selected = $input.range();
				var prefix = codibase.server.markdown.getBlockPrefix($input);
				var suffix = codibase.server.markdown.getBlockSuffix($input);
				if (suffix == "")
					suffix = "\n";
				...
				codibase.server.markdown.fireInputEvent($input);
			});
		}
		...
		$editLink.click(function() {
			...
			Cookies.set(codibase.server.markdown.getCookiePrefix($container)+".split", false, {expires: Infinity});
		});
		$previewLink.click(function() {
			...
			codibase.server.perfectScrollbar.empty($rendered);
			$rendered.html("<div class='message'>" + codibase.server.markdown.translations["loading"] + "</div>");
		...
		$splitLink.click(function() {
			...
			callback("render", $input.val());
			Cookies.set(codibase.server.markdown.getCookiePrefix($container)+".split", true, {expires: Infinity});
		});
		
		codibase.server.markdown.setupActionMenu($container, $head.find(".normal-mode"));
		...
		$input.doneEvents("input inserted.atwho", function() {
			function render() {
				 ...
				 */
				if (codibase.server.ajaxRequests.count != 0) {  
		...
		$input.doneEvents("keydown", function(e) {
			if (e.keyCode>=33 && e.keyCode<=40 && $preview.is(":visible") 
				...
				codibase.server.markdown.syncPreviewScroll(containerId);
			}
		}, previewTimeout);
		...
		$body.resizable({
			...
			minHeight: minHeight,
			stop: function() {
				Cookies.set(codibase.server.markdown.getCookiePrefix($container)+".bodyHeight", 
		...
		$edit.resizable({
			...
			minHeight: minHeight,
			stop: function() {
				Cookies.set(codibase.server.markdown.getCookiePrefix($container)+".editHeight", 
		...
		$preview.resizable({
			...
			minHeight: minHeight,
			stop: function() {
				Cookies.set(codibase.server.markdown.getCookiePrefix($container)+".previewHeight", 
		...
		$input.on("keydown", function(e) {
			if ($(".atwho-view").filter(":visible").length == 0) {
				if (codibase.server.util.isMac() && e.metaKey || !codibase.server.util.isMac() && e.ctrlKey) {
					var key;
					...
					if (key == "g") {
						...
						var selector = codibase.server.markdown.shortcuts[key];
						if (selector) {
							e.preventDefault();
	    ...
	    if (attachmentUploadUrl) {
			...
			function uploadFile(file) {
				if (file.size> attachmentMaxSize) {
					var message = "!!" + codibase.server.markdown.translations["upload-should-be-less-than"] + "!!";
					codibase.server.markdown.updateUploadMessage($input, message.replace("{0}", Math.round(attachmentMaxSize/1024/1024)));
				} else {
					var xhr = new XMLHttpRequest();
					var val = $input.val();
					var i=1;
					var message = "[" + codibase.server.markdown.translations["uploading-file"] + "...]";
					while (val.indexOf(message) != -1) {
						message = "[" + codibase.server.markdown.translations["uploading-file"] + (++i) + "...]";
					}
					...
					xhr.onload = function() {
						...
						if (xhr.status == 200) { 
							...
							codibase.server.markdown.updateUploadMessage($input, 
									"!!" + response + "!!", xhr.replaceMessage);
						}
					};
					xhr.onerror = function() {
						codibase.server.markdown.updateUploadMessage($input, 
								"!!" + codibase.server.markdown.translations["unable-to-connect-to-server"] + "!!", xhr.replaceMessage);
					};
	...
	setupActionMenu: function($container, $actionMenu) {
		...
		$actionMenu.find(".do-code").click(function() {
			$input.focus();
			var langHint = codibase.server.markdown.translations["programming-language"];
			var selected = $input.range();
			var prefix = codibase.server.markdown.getBlockPrefix($input);
			var suffix = codibase.server.markdown.getBlockSuffix($input);
			if (suffix == "")
		...
		$actionMenu.find(".do-quote").click(function() {
			...
			if (selected.length != 0) {
				document.execCommand("insertText", false, 
					prefix + codibase.server.markdown.getQuoted(selected.text) + suffix);				
			} else {
				document.execCommand("insertText", false, 
					prefix + "> " + suffix);				
		...
		$actionMenu.find(".do-emoji").click(function() {
			if (!$emojis.hasClass("loaded") && !$emojis.hasClass("loading")) {
				...
				$emojis.html(codibase.server.markdown.translations["loading-emojis"]);
				callback("loadEmojis");
			}
			$emojis.toggle();
		...
		$actionMenu.find(".do-mention").click(function() {
			...
			if (!$edit.is(":visible")) 
				return;
			var prefix = codibase.server.markdown.getAtWhoPrefix($input);
			$input.focus();
		...
		$actionMenu.find(".do-reference").click(function() {
			...
			if (!$edit.is(":visible"))
				return;
			var text = codibase.server.markdown.getAtWhoPrefix($input);
		...
		var prefix = codibase.server.util.isMac()?"cmd-":"ctrl-";		
		for (var key in codibase.server.markdown.shortcuts) {
			var selector = codibase.server.markdown.shortcuts[key];
	...
	onLoad: function(containerId) {
		...
		if ($body.find(".ui-resizable-handle:visible").length != 0) {
			...
			if ($container.hasClass("normal-mode")) {
				var bodyHeight = Cookies.get(codibase.server.markdown.getCookiePrefix($container)+".bodyHeight");
				if (bodyHeight) 
					$body.height(parseInt(bodyHeight));
				else 
					$body.height(defaultHeight);
			} else {
				var editHeight = Cookies.get(codibase.server.markdown.getCookiePrefix($container)+".editHeight");
				if (editHeight) 
					$edit.height(parseInt(editHeight));
				else
					$edit.height(defaultHeight);
					
				var previewHeight = Cookies.get(codibase.server.markdown.getCookiePrefix($container)+".previewHeight");
				if (previewHeight) 
	...
	onRendered: function(containerId, html) {
		...
		codibase.server.markdown.initRendered(containerId);
	...
	initRendered: function(containerId, translations) {
		...
		$rendered.find("a.header-anchor").each(function() {
			...
			$headerAnchor.before("<a href='" + $headerAnchor.attr("href") 
				...
				+ codibase.server.icons + "#link'/></svg></a>");
		});
		
		$rendered.find("a").click(function() {
			codibase.server.viewState.getFromViewAndSetToHistory();
		});
		...
		$rendered.find("pre>code").each(function() {
			...
			if (suggestionFile) {
				codibase.server.diff.highlightSyntax($this.children(".text-diff"), suggestionFile);
			} else {
				var modeName = $this.data("language");
				if (modeName) {
				    var modeInfo = CodeMirror.findModeByName(modeName);
			        if (modeInfo) { 
						codibase.server.codemirror.highlightSyntax($this.text(), modeInfo, this);
			...
			if (!$this.parent().hasClass("suggestion")) {
				var icon = "<svg class='icon'><use xlink:href='" + codibase.server.icons + "#copy'/></svg>";
				var $copy = $("<a class='pressable' data-tippy-content='" + codibase.server.markdown.translations["copy-to-clipboard"] + "'>" + icon + "</a>");
				$actions.append($copy);
				var options = {
					text: function() {
						return $this.text();
			...
			if (suggestionCallback) {
				if ($this.data("suggestionoutdated")) {
					var icon = "<svg class='icon icon-sm'><use xlink:href='" + codibase.server.icons + "#warning'/></svg>";
					var $warning = $("<a class='ml-2 link-gray' data-tippy-content='" + codibase.server.markdown.translations["suggestion-outdated"] + "'>" + icon + "</a>");
					$actions.append($warning);
				} else if ($this.data("suggestionapplyinbatch")) {
					var icon = "<svg class='icon'><use xlink:href='" + codibase.server.icons + "#minus-square'/></svg>";
					var $removeFromBatch = $("<a class='pressable ml-2' data-tippy-content='" + codibase.server.markdown.translations["remove-from-batch"] + "'>" + icon + "</a>");
					...
					var icon = "<svg class='icon'><use xlink:href='" + codibase.server.icons + "#add-to-git'/></svg>";
					var $apply = $("<a class='pressable ml-2' data-tippy-content='" + codibase.server.markdown.translations["commit-suggestion"] + "'>" + icon + "</a>");
					...
					if ($this.data("suggestionbatchappliable")) {
						var icon = "<svg class='icon flip-x'><use xlink:href='" + codibase.server.icons + "#plus-square'/></svg>";
						var $addToBatch = $("<a class='pressable ml-2' data-tippy-content='" + codibase.server.markdown.translations["add-to-batch"] + "'>" + icon + "</a>");
	...
	onViewerDomReady: function(containerId, taskCallback, taskSourcePositionDataAttribute, referenceCallback, 
		...
		$container.find(".reference").hover(function() {
			...
			if (referenceType) {
				var $tooltip = $("<div id='reference-tooltip'>" + codibase.server.markdown.translations["loading"] + "</div>");
	...
	renderIssueTooltip: function(title, state, stateFontColor, stateBackgroundColor) {
		...
		if (title) {
			...
			$tooltip.empty().append("<i>" + codibase.server.markdown.translations["issue-not-exist-or-access-denied"] + "</i>");			
		}
	...
	renderPullRequestTooltip: function(title, status, statusCss) {
		...
		if (title) {
			...
			$tooltip.empty().append("<i>" + codibase.server.markdown.translations["pull-request-not-exist-or-access-denied"] + "</i>");			
		}
		$tooltip.align({placement: $tooltip.data("alignment"), target: {element: $tooltip.data("trigger")}});
	},
	renderBuildTooltip: function(title, iconHref, iconCss) {
		...
		if (title) {
			...
			$tooltip.empty().append("<i>" + codibase.server.markdown.translations["build-not-exist-or-access-denied"] + "</i>");			
		}
		$tooltip.align({placement: $tooltip.data("alignment"), target: {element: $tooltip.data("trigger")}});
	},
	...
	renderCommitTooltip: function(author, date, commitMessage) {
		...
		if (commitMessage) {
			...
			$tooltip.empty().append("<i>" + codibase.server.markdown.translations["commit-not-exist-or-access-denied"] + "</i>");			
		}
		$tooltip.align({placement: $tooltip.data("alignment"), target: {element: $tooltip.data("trigger")}});
	},
	...
	insertUrl: function(containerId, isImage, url, name, replaceMessage) {
    	...
    	var message;
    	var defaultDescription = codibase.server.markdown.translations["enter-description-here"];
    	if (name)
    	...
    	codibase.server.markdown.updateUploadMessage($input, message, replaceMessage);
	...
	onInputUrlDomReady: function(containerId) {
		...
		$url.on("input", function() {
			$text.attr("placeholder", codibase.server.util.describeUrl($url.val()));
		});
	},
	...
	onQuote: function(containerId, content) {
		var quoted = codibase.server.markdown.getQuoted(content).replace(/\r/g, "");
