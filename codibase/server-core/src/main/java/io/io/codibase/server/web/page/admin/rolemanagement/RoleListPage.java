	...
	protected void onInitialize() {
			...
			public void populateItem(Item<ICellPopulator<Role>> cellItem, String componentId, IModel<Role> rowModel) {
				...
				fragment.add(new AjaxLink<Void>("delete") {
					...
					public void onClick(AjaxRequestTarget target) {
						Role role = rowModel.getObject();
						CodiBase.getInstance(RoleService.class).delete(role);
						var oldAuditContent = VersionedXmlDoc.fromBean(role).toXML();
						CodiBase.getInstance(AuditService.class).audit(null, "deleted role \"" + role.getName() + "\"", oldAuditContent, null);
		...
		SortableDataProvider<Role, Void> dataProvider = new SortableDataProvider<Role, Void>() {
			...
			public Iterator<? extends Role> iterator(long first, long count) {
				return CodiBase.getInstance(RoleService.class).query(query, (int)first, (int)count).iterator();
			}

			@Override
			public long size() {
				return CodiBase.getInstance(RoleService.class).count(query);
			}
			...
			public IModel<Role> model(Role object) {
				...
				return new LoadableDetachableModel<Role>() {
					...
					@Override
					protected Role load() {
						return CodiBase.getInstance(RoleService.class).load(id);
					}
