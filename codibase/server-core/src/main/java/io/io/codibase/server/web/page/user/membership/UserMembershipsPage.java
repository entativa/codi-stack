package io.codibase.server.web.page.user.membership;
...
import io.codibase.server.web.component.groupchoice.AbstractGroupChoiceProvider;
import io.codibase.server.web.component.groupchoice.GroupChoiceResourceReference;
	...
	protected void onInitialize() {
		...
		add(new SelectToActChoice<Group>("addNew", new AbstractGroupChoiceProvider() {
			...
			public void query(String term, int page, Response<Group> response) {
				List<Group> notMembersOf = CodiBase.getInstance(GroupService.class).query();
		...
		dataProvider = new SortableDataProvider<>() {
			...
			public Iterator<? extends Membership> iterator(long first, long count) {
				...
				return CodiBase.getInstance(MembershipService.class).query(criteria, (int) first, (int) count).iterator();
			}

			@Override
			public long size() {
				return CodiBase.getInstance(MembershipService.class).count(getCriteria());
			}
			...
			public IModel<Membership> model(Membership object) {
				...
				return new LoadableDetachableModel<>() {
					...
					protected Membership load() {
						return CodiBase.getInstance(MembershipService.class).load(id);
					}
