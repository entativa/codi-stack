...
import static io.codibase.server.web.behavior.ChangeObserver.filterObservables;
...
import io.codibase.server.web.behavior.ZoneIdBehavior;
...
import io.codibase.server.web.websocket.WebSocketMessages;
...
public abstract class BasePage extends WebPage {
	...
	protected void onInitialize() {
		...
		add(popStateBehavior = new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				resizeWindow(target);
				target.appendJavaScript("codibase.server.viewState.getFromHistoryAndSetToView();");
			}
			...
			public void renderHead(IHeaderResponse response) {
				...
				try {
					response.render(OnDomReadyHeaderItem.forScript(
						String.format("codibase.server.onDomReady('%s', '%s', %s, %s, %s);",
								String.valueOf(CodiBase.getInstance().getBootDate().getTime()),
								SpriteImage.getVersionedHref(IconScope.class, null),
								popStateBehavior.getCallbackFunction(explicit("data")).toString(), 
								CodiBase.getInstance(ObjectMapper.class).writeValueAsString(getRemoveAutosaveKeys()),
								CodiBase.getInstance(ObjectMapper.class).writeValueAsString(translations))));
				} catch (JsonProcessingException e) {
					throw new RuntimeException(e);
				}
				response.render(OnLoadHeaderItem.forScript("codibase.server.onWindowLoad();"));
	...
	public void pushState(IPartialPageRequestHandler handler, String url, Serializable data) {
		...
		String script = String.format("codibase.server.history.pushState('%s', '%s', '%s');",
	...
	public void replaceState(IPartialPageRequestHandler handler, String url, Serializable data) {
		String encodedData = new String(Base64.encodeBase64(CryptoUtils.encrypt(SerializationUtils.serialize(data))));
		String script = String.format("codibase.server.history.replaceState('%s', '%s', '%s');",
				url, encodedData, JavaScriptEscape.escapeJavaScript(getPageTitle()));
		handler.prependJavaScript(script);
	}
	...
	private void checkReady() {
		if (!CodiBase.getInstance().isReady() && getClass() != ServerInitPage.class)
	...
	protected String getPageTitle() {
		return "CodiBase - Git Server with CI/CD, Kanban, and Packages";
	}
