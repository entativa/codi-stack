package io.codibase.server.web.page.project.stats.buildmetric;
...
import io.codibase.server.util.MetricIndicator;
...
import io.codibase.server.web.behavior.BuildMetricQueryBehavior;
...
import static io.codibase.server.web.util.StatsGroup.BY_DAY;
...
public abstract class BuildMetricStatsPage<T extends AbstractEntity> extends ProjectPage {
		...
		content.add(new ListView<>("charts", new LoadableDetachableModel<List<LineSeries>>() {
			...
			protected List<LineSeries> load() {
				...
				if (parsedQuery != null) {
					...
					Map<Integer, T> stats = CodiBase.getInstance(BuildMetricService.class)
	...
	protected BookmarkablePageLink<Void> navToProject(String componentId, Project project) {
		if (!CodiBase.getInstance(BuildMetricService.class).getAccessibleReportNames(project, metricClass).isEmpty())
