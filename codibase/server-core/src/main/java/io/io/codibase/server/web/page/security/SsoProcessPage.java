...
import io.codibase.server.model.support.administration.sso.SsoAuthenticated;
...
import io.codibase.server.web.component.tabbable.ActionTab;
...
public class SsoProcessPage extends SimplePage {
		...
		try {
			...
			providerModel = new LoadableDetachableModel<SsoProvider>() {
				...
				protected SsoProvider load() {
					return CodiBase.getInstance(SsoProviderService.class).find(providerName);
				}
			};
	...
	private SsoAccountService getSsoAccountService() {
		return CodiBase.getInstance(SsoAccountService.class);
	}
	...
	private SshKeyService getSshKeyService() {
		return CodiBase.getInstance(SshKeyService.class);
	}

	private PasswordAuthenticatingRealm getPasswordAuthenticatingRealm() {
		return CodiBase.getInstance(PasswordAuthenticatingRealm.class);
	}
}
