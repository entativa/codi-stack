codibase.server.codeSupport = {
	...
	onEditorLoad: function(inputId, modeName, varQueryCallback) {
		...
		codibase.server.codemirror.setModeByName(cm, modeName);
		...
		cm.on("change", function() {
			cm.save();
			codibase.server.form.markDirty($input.closest("form"));
			setTimeout(function() {
				codibase.server.codeSupport.adjustHeight($input);
				cm.refresh();
			}, 0);
		});
