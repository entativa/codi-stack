package io.codibase.server.web.behavior.dropdown;
...
public abstract class DropdownHoverBehavior extends AbstractPostAjaxBehavior {
		...
		if (dropdown == null) {
			dropdown = new FloatingPanel(target, new Alignment(new ComponentTarget(alignTarget!=null?alignTarget:getComponent()), placement)) {
				...
				protected void onClosed() {
					...
					if (target != null) {
						String script = String.format("codibase.server.dropdownHover.closed('%s', '%s');", 
			...
			String script = String.format("codibase.server.dropdownHover.opened('%s', '%s');", 
	...
	public void renderHead(Component component, IHeaderResponse response) {
		...
		String script = String.format("codibase.server.dropdownHover.onDomReady('%s', %s, %s);", 
