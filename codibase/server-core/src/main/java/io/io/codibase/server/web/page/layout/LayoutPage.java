...
import static io.codibase.server.model.Alert.PROP_DATE;
...
import io.codibase.server.SubscriptionService;
...
import io.codibase.server.web.component.brandlogo.BrandLogoPanel;
import io.codibase.server.web.component.commandpalette.CommandPalettePanel;
import io.codibase.server.web.component.datatable.DefaultDataTable;
...
import io.codibase.server.web.component.modal.message.MessageModal;
...
import io.codibase.server.web.page.admin.servicedesk.ServiceDeskSettingPage;
...
public abstract class LayoutPage extends BasePage {
	...
	protected void onInitialize() {
		...
		MainMenuCustomization customization = CodiBase.getInstance(MainMenuCustomization.class);
		...
		sidebar.add(new ListView<SidebarMenu>("menus", new LoadableDetachableModel<>() {
			...
			protected List<SidebarMenu> load() {
				...
				if (SecurityUtils.isAdministrator()) {
					...
					var sshPort = CodiBase.getInstance(ServerConfig.class).getSshPort();
				...
				if (SecurityUtils.isAdministrator()) {
					...
					for (AdministrationSettingContribution contribution : CodiBase.getExtensions(AdministrationSettingContribution.class)) {
					...
					var contributions = new ArrayList<>(CodiBase.getExtensions(AdministrationMenuContribution.class));
										...
										public void onClick(AjaxRequestTarget target) {
											new MessageModal(target) {
												...
												protected Component newMessageContent(String componentId) {
													return new Label(componentId, _T("This is an enterprise feature. <a href='https://codibase.io/pricing' target='_blank'>Try free</a> for 30 days")).setEscapeModelStrings(false);
												}
											};																
										}
									};
		...
		sidebar.add(new ExternalLink("productVersion", "https://codibase.io", "CodiBase " + version));
		...
		var checkUpdateUrl = "https://codibase.io/check-update/" + commitHash + "-"
				...
				add(new WebMarkupContainer("icon") {
					...
					protected void onComponentTag(ComponentTag tag) {
						...
						if (newVersionStatus != null) {
							...
							tag.put("onload", "codibase.server.layout.onNewVersionStatusIconLoaded();");
						} else {
							tag.put("src", checkUpdateUrl + "/icon.svg");
							var script = String.format("codibase.server.layout.onNewVersionStatusIconLoaded(%s);",
		...
		if (CodiBase.getInstance(ServerConfig.class).getSshPort() != 0 && loginUser != null && !loginUser.isDisabled()) {
	...
	private SubscriptionService getSubscriptionService() {
		return CodiBase.getInstance(SubscriptionService.class);
	}

	private AlertService getAlertService() {
		return CodiBase.getInstance(AlertService.class);
	}
	...
	private UpdateCheckService getUpdateCheckService() {
		return CodiBase.getInstance(UpdateCheckService.class);
	}
	...
	public void renderHead(IHeaderResponse response) {
		...
		response.render(OnDomReadyHeaderItem.forScript(String.format("codibase.server.layout.onDomReady(%s, '%s', '%s');",
				commandPaletteBehavior.getCallbackFunction(), 
				_T("cmd-k to show command palette"), 
				_T("ctrl-k to show command palette"))));
		response.render(OnLoadHeaderItem.forScript("codibase.server.layout.onLoad();"));
	}
