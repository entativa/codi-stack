...
import static io.codibase.server.web.component.iteration.burndown.BurndownIndicators.ESTIMATED_TIME;
import static io.codibase.server.web.component.iteration.burndown.BurndownIndicators.ISSUE_COUNT;
import static io.codibase.server.web.component.iteration.burndown.BurndownIndicators.REMAINING_TIME;
import static io.codibase.server.web.component.iteration.burndown.BurndownIndicators.getDefault;
...
import io.codibase.server.web.component.chart.line.Line;
import io.codibase.server.web.component.chart.line.LineChartPanel;
import io.codibase.server.web.component.chart.line.LineSeries;
...
public class IterationBurndownPanel extends GenericPanel<Iteration> {
	...
	protected void onInitialize() {
		...
		if (message != null) {
			...
			fragment.add(new LineChartPanel("chart", new LoadableDetachableModel<>() {
				...
				protected LineSeries load() {
					...
					for (IssueSchedule schedule : getIteration().getSchedules()) {
						...
						var issueInfoService = CodiBase.getInstance(IssueInfoService.class);
					...
					for (StateSpec spec : CodiBase.getInstance(SettingService.class).getIssueSetting().getStateSpecs()) {
					...
					if (getIndicator().equals(REMAINING_TIME) || getIndicator().equals(ESTIMATED_TIME)) {
						yAxisValueFormatter = String.format(""
								+ "function(value) {"
								+ "  if (value != undefined) "
								+ "    return codibase.server.util.formatWorkingPeriod(value, %b);"
								+ "  else "
								+ "    return '-';"
			...
			var aggregationLink = CodiBase.getInstance(SettingService.class).getIssueSetting()
