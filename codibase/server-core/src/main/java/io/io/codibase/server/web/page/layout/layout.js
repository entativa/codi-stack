codibase.server.layout = {
	onDomReady: function(commandPaletteCallback, macCommandPaletteTooltip, nonMacCommandPaletteTooltip) {
		...
		$sidebar.find(".menu-link:not('.menu-toggle')").click(function() {
			window.sessionStorage.setItem("codibase.sidebar.scrollTop", $sidebarBody.scrollTop());	
		});		
	...
	onLoad: function() {
		var $sidebarBody = $(".sidebar-body");
		var scrollTop = window.sessionStorage.getItem("codibase.sidebar.scrollTop");
		if (scrollTop) {
			window.sessionStorage.removeItem("codibase.sidebar.scrollTop");
