package io.codibase.server.web.component.issue.authorizations;

import static io.codibase.server.model.IssueAuthorization.PROP_ISSUE;
import static io.codibase.server.model.IssueAuthorization.PROP_USER;
	...
	protected void onInitialize() {
		...
		add(new SelectToActChoice<User>("addNew", new AbstractUserChoiceProvider() {
			...
			protected void onSelect(AjaxRequestTarget target, User selection) {
				...
				authorization.setUser(CodiBase.getInstance(UserService.class).load(selection.getId()));
				CodiBase.getInstance(IssueAuthorizationService.class).createOrUpdate(authorization);
				target.add(authorizationsTable);
			...
			public void populateItem(Item<ICellPopulator<IssueAuthorization>> cellItem, String componentId,
				...
				fragment.add(new AjaxLink<Void>("unauthorize") {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						CodiBase.getInstance(IssueAuthorizationService.class).delete(authorization);
		...
		dataProvider = new SortableDataProvider<IssueAuthorization, Void>() {
			...
			public Iterator<? extends IssueAuthorization> iterator(long first, long count) {
				...
				return CodiBase.getInstance(IssueAuthorizationService.class).query(criteria, (int)first,
						(int)count).iterator();
			}

			@Override
			public long size() {
				return CodiBase.getInstance(IssueAuthorizationService.class).count(getCriteria());
			}
			...
			public IModel<IssueAuthorization> model(IssueAuthorization object) {
				...
				return new LoadableDetachableModel<IssueAuthorization>() {
					...
					protected IssueAuthorization load() {
						return CodiBase.getInstance(IssueAuthorizationService.class).load(id);
					}
