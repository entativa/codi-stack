package io.codibase.server.web.page.project.blob;
...
import io.codibase.server.git.LfsPointer;
import io.codibase.server.git.exception.BlobEditException;
...
import io.codibase.server.job.JobAuthorizationContextAware;
...
import io.codibase.server.search.code.query.BlobQuery;
import io.codibase.server.search.code.query.TextQuery;
...
import io.codibase.server.util.FilenameUtils;
...
import io.codibase.server.web.component.commit.status.CommitStatusLink;
...
import io.codibase.server.web.component.revision.RevisionPicker;
import io.codibase.server.web.page.project.ProjectPage;
import io.codibase.server.web.page.project.blob.navigator.BlobNavigator;
...
import io.codibase.server.web.page.project.blob.render.commitoption.CommitOptionPanel;
import io.codibase.server.web.page.project.blob.render.folder.FolderViewPanel;
import io.codibase.server.web.page.project.blob.render.nocommits.NoCommitsPanel;
import io.codibase.server.web.page.project.blob.render.noname.NoNameEditPanel;
import io.codibase.server.web.page.project.blob.render.source.SourceEditPanel;
...
import io.codibase.server.web.page.project.blob.search.advanced.AdvancedSearchPanel;
import io.codibase.server.web.page.project.blob.search.quick.QuickSearchPanel;
import io.codibase.server.web.page.project.blob.search.result.SearchResultPanel;
...
import io.codibase.server.web.util.EditParamsAware;
...
public class ProjectBlobPage extends ProjectPage implements BlobRenderContext, 
		...
		add(revisionIndexing = new WebMarkupContainer("revisionIndexing") {
			...
			protected void onConfigure() {
				...
				if (resolvedRevision != null) {
					...
					CodeIndexService indexService = CodiBase.getInstance(CodeIndexService.class);
					if (!indexService.isIndexed(getProject().getId(), commit)) {
						CodiBase.getInstance(CodeIndexService.class).indexAsync(getProject().getId(), commit);
						setVisible(true);
					} else {
						setVisible(false);
					}
	...
	public CodeComment getOpenComment() {
		if (state.commentId != null)
			return CodiBase.getInstance(CodeCommentService.class).load(state.commentId);
		else
			return null;
	}
	...
	private void newBlobContent(@Nullable AjaxRequestTarget target) {
		...
		if (getMode() == Mode.VIEW && getBlobIdent().revision == null) {
			...
			if (blobContent == null) {
				for (BlobRenderer contribution: CodiBase.getExtensions(BlobRenderer.class)) {
		...
		Component buildSupportNote = new WebMarkupContainer("buildSupportNote") {
			...
			protected void onConfigure() {
				...
				if (resolvedRevision != null && isOnBranch() && state.blobIdent.path == null && state.mode == Mode.VIEW) {
					BlobIdent oldBlobIdent = new BlobIdent(resolvedRevision.name(), ".codibase-buildspec", FileMode.TYPE_FILE);
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.projectBlob.onDomReady(%s);", callback);
	...
	public static class State implements Serializable {
		...
		/*
		 * Some blob can be rendered in a way for easier understanding, such as .codibase-buildspec.yml, 
	...
	public PullRequest getPullRequest() {
		if (state.requestId != null)
			return CodiBase.getInstance(PullRequestService.class).load(state.requestId);
		else
			return null;
	}
