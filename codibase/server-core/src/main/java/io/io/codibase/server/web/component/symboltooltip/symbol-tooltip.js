codibase.server.symboltooltip = {
	init: function(containerId, queryCallback) {
		...
		var prepareToHide = function() {
			if (container.tooltip) {
				...
				container.tooltip.hideTimer = setTimeout(function(){
					codibase.server.symboltooltip.removeTooltip(container);
				}, 200);
			}
		};
		...
		container.onMouseOverSymbol = function(revision, symbolEl) {
			var $symbol = $(symbolEl);
			if (codibase.server.mouseState.pressed || !codibase.server.mouseState.moved || $symbol.text() == "@")
				return;
...
$(function() {
	function closeAll() {
		$(".symbol-tooltip-container").each(function() {
			codibase.server.symboltooltip.removeTooltip(this);
		});
	}
