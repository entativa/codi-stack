	...
	protected void onInitialize() {
			...
			public void populateItem(Item<ICellPopulator<Group>> cellItem, String componentId, IModel<Group> rowModel) {
				...
				fragment.add(new AjaxLink<Void>("delete") {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						CodiBase.getInstance(GroupService.class).delete(group);
						CodiBase.getInstance(AuditService.class).audit(null, "deleted group \"" + group.getName() + "\"", oldAuditContent, null);
		...
		SortableDataProvider<Group, Void> dataProvider = new SortableDataProvider<>() {
			...
			public Iterator<? extends Group> iterator(long first, long count) {
				return CodiBase.getInstance(GroupService.class).query(query, (int) first, (int) count).iterator();
			}

			@Override
			public long size() {
				return CodiBase.getInstance(GroupService.class).count(query);
			}
			...
			public IModel<Group> model(Group object) {
				...
				return new LoadableDetachableModel<>() {
					...
					protected Group load() {
						return CodiBase.getInstance(GroupService.class).load(id);
					}
