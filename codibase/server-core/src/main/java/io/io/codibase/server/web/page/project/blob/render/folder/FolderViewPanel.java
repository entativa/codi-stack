package io.codibase.server.web.page.project.blob.render.folder;
...
import io.codibase.server.util.FilterIterator;
...
import io.codibase.server.web.behavior.CtrlAwareOnClickAjaxBehavior;
import io.codibase.server.web.component.blob.BlobIcon;
...
import io.codibase.server.web.component.user.card.PersonCardPanel;
...
public class FolderViewPanel extends Panel {
	...
	private final IModel<List<BlobIdent>> childrenModel = new LoadableDetachableModel<>() {
		...
		protected List<BlobIdent> load() {
			...
			BlobIdent oldBuildSpecIdent = new BlobIdent(context.getBlobIdent().revision,
					".codibase-buildspec", FileMode.REGULAR_FILE.getBits());
	...
	protected void onInitialize() {
		...
		add(new ListView<BlobIdent>("children", childrenModel) {
			...
			protected void populateItem(ListItem<BlobIdent> item) {
				...
				else if (blobIdent.path.equals(BuildSpec.BLOB_PATH) || blobIdent.path.equals(".codibase-buildspec"))
		...
		add(userCardBehavior = new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				replace(userCard);
				target.add(userCard);
				target.appendJavaScript("codibase.server.folderView.onUserCardAvailable();");
			}
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.folderView.onDomReady('%s', '%s', %s)", 
