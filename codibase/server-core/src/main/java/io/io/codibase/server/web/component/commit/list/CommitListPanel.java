...
import io.codibase.server.search.commit.CommitCriteria;
import io.codibase.server.search.commit.CommitQuery;
import io.codibase.server.search.commit.FuzzyCriteria;
import io.codibase.server.search.commit.MessageCriteria;
import io.codibase.server.search.commit.PathCriteria;
...
import io.codibase.server.web.behavior.CommitQueryBehavior;
import io.codibase.server.web.behavior.RunTaskBehavior;
import io.codibase.server.web.component.commit.message.CommitMessagePanel;
import io.codibase.server.web.component.commit.status.CommitStatusLink;
import io.codibase.server.web.component.commit.status.CommitStatusSupport;
...
public abstract class CommitListPanel extends Panel {
	...
	private String renderCommitGraph() {
		...
		return String.format("codibase.server.commitList.renderGraph('%s', %s);", body.getMarkupId(), jsonOfCommits);
	}
	...
	private String asJSON(List<RevCommit> commits) {
		...
		try {
			return CodiBase.getInstance(ObjectMapper.class).writeValueAsString(commitIndexes);
	...
	public void renderHead(IHeaderResponse response) {
		...
		String jsonOfCommits = asJSON(commitsModel.getObject().current);
		String script = String.format("codibase.server.commitList.onDomReady('%s', %s);", body.getMarkupId(), jsonOfCommits);		
