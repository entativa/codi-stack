...
import io.codibase.server.data.migration.DataMigrator;
import io.codibase.server.data.migration.MigrationHelper;
...
import static io.codibase.server.persistence.PersistenceUtils.*;
...
public class Upgrade extends AbstractPlugin {
	...
	protected Commandline buildCommandline(File upgradeDir, String command, String...commandArgs) {
		...
		if (version.startsWith("1.0-EAP")) {
			...
			bootstrapClass = "io.codibase.launcher.bootstrap.Bootstrap";
			...
			bootstrapClass = "io.codibase.commons.launcher.bootstrap.Bootstrap";
		} else {
			bootstrapClass = "io.codibase.commons.bootstrap.Bootstrap";
		}
	...
	public void start() {
		...
		if (Bootstrap.command.getArgs().length == 0) {
			logger.error("Missing upgrade target parameter. Usage: {} <CodiBase installation directory>", 
		...
		if (!isEmpty) {
			if (!new File(upgradeDir, "boot").exists()) {
				logger.error("Invalid CodiBase installation directory: {}, make sure you are specifying the top level "
			...
			if (!releaseProps.equals(oldReleaseProps)) {
				...
				if (CodiBase.isServerRunning(upgradeDir)) {
				...
				try {
					var callable = new Callable<Void>() {
						...
						public Void call() {
							...
							if (newAppDataVersion < oldAppDataVersion) 
								throw new ExplicitException("CodiBase program is too old, please use a newer version");
							...
							if (failed) {
								...
								if (dbChanged) {
									if (inDocker)
										errorMessage.append("\nCodiBase is unable to restore old database, please do it manually by first resetting it (delete and create), and then exec into the container to run below command:");
									else
										errorMessage.append("\nCodiBase is unable to restore old database, please do it manually by first resetting it (delete and create), and then running below command:");										
					...
					var maintenanceFile = CodiBase.getMaintenanceFile(upgradeDir);
			...
			if (CodiBase.isServerRunning(Bootstrap.installDir)) {
	...
	protected void updateProgramFiles(File upgradeDir, int oldAppDataVersion) {
		...
		var directoryVersion = ".codibase-directory-version";
		...
		try {
			...
			wrapperConf = StringUtils.replace(wrapperConf, "com.gitplex.commons.bootstrap.Bootstrap", 
					"io.codibase.commons.launcher.bootstrap.Bootstrap");
			...
			wrapperConf = StringUtils.replace(wrapperConf, "com.turbodev.launcher.bootstrap.Bootstrap", 
					"io.codibase.commons.launcher.bootstrap.Bootstrap");
			wrapperConf = StringUtils.replace(wrapperConf, "io.codibase.launcher.bootstrap.Bootstrap", 
					"io.codibase.commons.launcher.bootstrap.Bootstrap");
			wrapperConf = StringUtils.replace(wrapperConf, "io.codibase.commons.launcher.bootstrap.Bootstrap", 
					"io.codibase.commons.bootstrap.Bootstrap");
			wrapperConf = StringUtils.replace(wrapperConf, "wrapper.pidfile=../status/codibase.pid", "");
			...
			hibernateProps = StringUtils.replace(hibernateProps, "GitPlex", "CodiBase");
			hibernateProps = StringUtils.replace(hibernateProps, "TurboDev", "CodiBase");

			if (!hibernateProps.contains("hsqldb.lob_file_scale")) {
				hibernateProps = StringUtils.replace(hibernateProps, "internaldb/codibase;", 
						"internaldb/codibase;hsqldb.lob_file_scale=4;");
			}
			if (!hibernateProps.contains("hsqldb.lob_compressed")) {
				hibernateProps = StringUtils.replace(hibernateProps, "internaldb/codibase;",
						"internaldb/codibase;hsqldb.lob_compressed=true;");
			}
			if (!hibernateProps.contains("hsqldb.tx")) {
				hibernateProps = StringUtils.replace(hibernateProps, "internaldb/codibase;",
						"internaldb/codibase;hsqldb.tx=mvcc;");
			}
			...
			logbackConfig = StringUtils.replace(logbackConfig, "com.turbodev", "io.codibase");
			logbackConfig = StringUtils.replace(logbackConfig, "turbodev", "codibase");
			...
			if (!logbackConfig.contains("MaskingPatternLayout")) {
				...
				logbackConfig = StringUtils.replace(logbackConfig, 
						"<pattern>", 
						"<layout class=\"io.codibase.commons.bootstrap.MaskingPatternLayout\">\n				<pattern>");
				logbackConfig = StringUtils.replace(logbackConfig, 
						"</pattern>", 
						"</pattern>\n			</layout>");
			}
