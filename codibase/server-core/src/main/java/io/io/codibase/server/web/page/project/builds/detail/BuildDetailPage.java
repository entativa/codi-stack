...
import io.codibase.server.terminal.TerminalService;
...
import io.codibase.server.web.component.build.side.BuildSidePanel;
...
import io.codibase.server.web.component.job.joblist.JobListPanel;
import io.codibase.server.web.component.link.BuildSpecLink;
				...
				add(new AjaxLink<Void>("terminal") {
					...
					public void onClick(AjaxRequestTarget target) {
						if (isSubscriptionActive()) {
							target.appendJavaScript(String.format("codibase.server.buildDetail.openTerminal('%s');",
							...
							new MessageModal(target) {
								...
								protected Component newMessageContent(String componentId) {
									return new Label(componentId, _T("Interactive web shell access to running jobs is an enterprise feature. <a href='https://codibase.io/pricing' target='_blank'>Try free</a> for 30 days")).setEscapeModelStrings(false);
								}
							};
						}
					}
