		...
		onDomReady : function(overallContributions, topContributorsDataUrl, userCardCallback, darkMode, translations) {
			...
			function updateDateRange() {
				$contribs.find(".date-range").text(
						codibase.server.codeStats.formatDay(fromDay) 
						+ " ~ " 
						+ codibase.server.codeStats.formatDay(toDay));
			}
			...
			for (var i=0; i<days.length; i++) {
				overallXAxisData.push(codibase.server.codeStats.formatDay(days[i]));
			}
				...
				yAxis: {
			    	...
			    	axisLabel: {
			    		formatter: function(value, index) {
			    			var result = codibase.server.codeStats.formatYAxisLabel(value, index, useKiloForOverallChart);
						...
						success: function(topContributors) {
							...
							for (var i=0; i<days.length; i++) {
								if (days[i] >= fromDay && days[i] <= toDay) 
									xAxisData.push(codibase.server.codeStats.formatDay(days[i]));
							}
							...
							function renderContributor($container, contributor, index) {
								...
								var option = {
									...
									yAxis: {
								    	...
								    	axisLabel: {
								    		formatter: function(value, index) {
								    			var result = codibase.server.codeStats.formatYAxisLabel(value, index, useKilo);
	...
	sourceLines: {
		onDomReady: function(lineIncrements, defaultBranch, darkMode, translations) {
			...
			while (currentDay <= lastDay) {
				...
				xAxisData.push(codibase.server.codeStats.formatDay(currentDay));
				currentDay++;
			}
