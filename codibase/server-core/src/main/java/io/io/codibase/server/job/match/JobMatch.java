...
import static io.codibase.commons.codeassist.AntlrUtils.getLexerRuleName;
import static io.codibase.server.job.match.JobMatchParser.IsNot;
import static io.codibase.server.job.match.JobMatchParser.OnBranch;
...
public class JobMatch extends Criteria<JobMatchContext> {
	...
	public static JobMatch parse(String jobMatchString, boolean withProjectCriteria, boolean withJobCriteria) {
		...
		Criteria<io.codibase.server.job.match.JobMatchContext> criteria = new JobMatchBaseVisitor<Criteria<io.codibase.server.job.match.JobMatchContext>>() {

			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitParensCriteria(JobMatchParser.ParensCriteriaContext ctx) {
				return visit(ctx.criteria()).withParens(true);
			}

			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitFieldOperatorValueCriteria(JobMatchParser.FieldOperatorValueCriteriaContext ctx) {
				int operator = ctx.operator.getType();
				String fieldName = getValue(ctx.criteriaField.getText());
				checkField(fieldName, withProjectCriteria, withJobCriteria);
				
				var criterias = new ArrayList<Criteria<io.codibase.server.job.match.JobMatchContext>>();
			...
			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitOperatorValueCriteria(JobMatchParser.OperatorValueCriteriaContext ctx) {
				var criterias = new ArrayList<Criteria<io.codibase.server.job.match.JobMatchContext>>();
				for (var quoted: ctx.criteriaValue.Quoted()) {
					String fieldValue = getValue(quoted.getText());
			...
			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitOrCriteria(JobMatchParser.OrCriteriaContext ctx) {
				List<Criteria<io.codibase.server.job.match.JobMatchContext>> childCriterias = new ArrayList<>();
			...
			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitAndCriteria(JobMatchParser.AndCriteriaContext ctx) {
				List<Criteria<io.codibase.server.job.match.JobMatchContext>> childCriterias = new ArrayList<>();
				for (JobMatchParser.CriteriaContext childCtx: ctx.criteria())
					childCriterias.add(visit(childCtx));
			...
			@Override
			public Criteria<io.codibase.server.job.match.JobMatchContext> visitNotCriteria(JobMatchParser.NotCriteriaContext ctx) {
