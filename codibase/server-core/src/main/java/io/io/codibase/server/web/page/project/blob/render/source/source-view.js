codibase.server.sourceView = {
	onDomReady: function(filePath, fileContent, openComment, markRange, symbolTooltipId, 
		...
		codibase.server.sourceView.translations = translations;
		var $sourceView = $(".source-view");
		var $code = $sourceView.children(".code");

		var cm = CodeMirror($code[0], {
			value: fileContent,
			readOnly: codibase.server.util.isDevice()?"nocursor": true,
			theme: "eclipse",
			lineNumbers: true,
		...
		codibase.server.codemirror.setModeByFileName(cm, filePath);

		$sourceView.data("callback", callback);
		$sourceView.data("blameMessageCallback", blameMessageCallback);
		
	    if (markRange)
	    	codibase.server.sourceView.mark(markRange, false);
		...
		if (!codibase.server.util.isObjEmpty(annotationInfo.problems))
			gutters.splice(0, 0, "CodeMirror-problems");
		if (!codibase.server.util.isObjEmpty(annotationInfo.coverages))
		...
		if (blameInfos) {
		    codibase.server.sourceView.blame(blameInfos);
    	}

		codibase.server.codemirror.bindShortcuts(cm);
		codibase.server.sourceView.checkShortcutsBinding();
		...
		cm.on("scroll", function() {
			codibase.server.symboltooltip.removeTooltip(document.getElementById(symbolTooltipId));					
			codibase.server.mouseState.moved = false;					
		});
		...
		$code.on("getViewState", function() {
		    return codibase.server.codemirror.getViewState(cm);
		});
		$code.on("setViewState", function(e, viewState) {
		    return codibase.server.codemirror.setViewState(cm, viewState);
		});
		...
		for (var line in annotationInfo.comments) {
		    if (annotationInfo.comments.hasOwnProperty(line) && line<cm.lineCount()) 
		    	codibase.server.sourceView.addCommentGutter(line, annotationInfo.comments[line]);
		}
		
		for (var line in annotationInfo.problems) {
		    if (annotationInfo.problems.hasOwnProperty(line) && line<cm.lineCount()) 
		    	codibase.server.sourceView.addProblemGutter(line, annotationInfo.problems[line]);
		}
		
		for (var line in annotationInfo.coverages) {
			if (annotationInfo.coverages.hasOwnProperty(line) && line<cm.lineCount())
				codibase.server.sourceView.addCoverageGutter(line, annotationInfo.coverages[line]);
		}
		
		codibase.server.sourceView.highlightCommentTrigger();	
	},
	checkShortcutsBinding() {
		if (!($(document).data("SourceViewShortcutsBinded"))) {
			...
			$(document).on("keydown", function(e) {
				if ($(".modal:visible").length == 0 && !codibase.server.util.canInput(e.target) 
						&& e.keyCode == 79) {
					var $sourceView = $(".source-view");
	...
	onWindowLoad: function(markRange) {
		if (codibase.server.viewState.getFromHistory() === undefined
				&& codibase.server.viewState.carryOver === undefined) {
			var cm = $(".source-view>.code>.CodeMirror")[0].CodeMirror;		
			codibase.server.codemirror.scrollTo(cm, markRange);
		}
	},
	...
	addCoverageGutter: function(line, coverageStatus) {
		...
		if (coverageStatus == 'COVERED')
			tooltip = codibase.server.sourceView.translations["covered-by-tests"];
		else if (coverageStatus == 'NOT_COVERED')
			tooltip = codibase.server.sourceView.translations["not-covered-by-any-test"];
		else
			tooltip = codibase.server.sourceView.translations["partially-covered-by-some-tests"];
	...
	addProblemGutter: function(line, problems) {
		...
		var severityInfo = codibase.server.codeProblem.getSeverityInfo(problems);
		
		let svg = `<svg class='icon icon-sm'><use xlink:href='${codibase.server.icons}#exclamation-circle-o'/></svg>`;
		$gutter.append(`<a class='problem-trigger ${severityInfo}'>${svg}</a>`);
		var $trigger = $gutter.children("a");
		$trigger.mouseover(function() {
			codibase.server.codemirror.mark(cm, markRanges);
		}).mouseout(function() {
			codibase.server.sourceView.restoreMark();
		});
		...
		$trigger.mousedown(function() {
			...
			if (!$trigger.data("popoverInited")) {
				$trigger.popover({
					...
					content: codibase.server.codeProblem.renderProblems(problems, codibase.server.sourceView.translations),
					...
					$currentPopover.find(".problem-content").mouseover(function() {
						codibase.server.codemirror.mark(cm, problems[$(this).index()].target.location);
					}).mouseout(function() {
						codibase.server.sourceView.restoreMark();
					}).each(function() {
						var problem = problems[$(this).index()];
						$(this).children(".add-comment").click(function() {
							if (codibase.server.sourceView.confirmUnsavedChanges()) {
	...
	confirmUnsavedChanges: function() {
		return $(".source-view").find("form.dirty").length == 0 || confirm(codibase.server.sourceView.translations["unsaved-changes-prompt"]); 
	},
	addCommentGutter: function(line, comments) {
		...
		if (comments.length != 1) {
			...
			for (var i in comments) { 
				...
				content += `<a class='${cssClasses}' data-tippy-content='${codibase.server.sourceView.translations["show-comment"]}'>#${comments[i].id}</a>`;
			}

			let cssClasses = "comment-indicator";
			if (updated)
				cssClasses += " updated"; 			
			$gutter.append(`<a class='${cssClasses}'><svg class='icon'><use xlink:href='${codibase.server.icons}#comments'/></svg></a>`);
			...
			$indicator.on('shown.bs.popover', function () {
				...
				$currentPopover.find("a").each(function() {
					$(this).mouseover(function() {
						...
						codibase.server.codemirror.mark(cm, comment.range);
					});
					$(this).mouseout(function() {
						codibase.server.sourceView.restoreMark();
					});
					$(this).click(function() {
						if (!$(this).hasClass("active") && codibase.server.sourceView.confirmUnsavedChanges()) { 
							let comment = comments[$(this).index()];
			...
			if (comment.updated)
				cssClasses += " updated";
			var svg = `<svg class='icon'><use xlink:href='${codibase.server.icons}#comment'/></svg>`;
			$gutter.append(`<a class='${cssClasses}' data-tippy-content='${codibase.server.sourceView.translations["show-comment"]}'>${svg}</a>`);
			var $indicator = $gutter.children("a");
			...
			$indicator.click(function() {
				if (!$indicator.hasClass("active") && codibase.server.sourceView.confirmUnsavedChanges()) { 
					let range = comment.range;
					callback("openComment", comment.id, range.fromRow, range.fromColumn, 
							range.toRow, range.toColumn);
				}
	...
	openSelectionPopover: function(selectionRange, selectionUrl, loggedIn) {
		...
		var $content;
		let svg = `<svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#link'/></svg>`;
		$content = $(`<div><a class='permanent'>${svg} ${codibase.server.sourceView.translations["perma-link"]}</a>`);
		$content.children("a.permanent").attr("href", selectionUrl);
		svg = `<svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#copy'/></svg>`;
		$content.append(`<a class='copy-marked'>${svg} ${codibase.server.sourceView.translations["copy-to-clipboard"]}</a>`);
		...
		if (loggedIn) {
			let svg = `<svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#comment'/></svg>`;
			$content.append(`<a class='comment'>${svg} ${codibase.server.sourceView.translations["add-selection-comment"]}</a>`);
			...
			let svg = `<svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#warning'/></svg>`;
			$content.append(`<a href='${loginHref}' class='comment'>${svg} ${codibase.server.sourceView.translations["login-to-comment"]}</a>`);
		}			
	...
	onCommentAdded: function(comment) {
		...
		codibase.server.sourceView.addCommentGutter(line, comments);
		codibase.server.sourceView.highlightCommentTrigger();				
		$(window).resize();
	},
	...
	onAddComment: function(commentRange) {
		...
		codibase.server.sourceView.exitFullScreen();
		var $sourceView = $(".source-view");
		$sourceView.removeData("openComment");
		...
		var cm = $(".source-view>.code>.CodeMirror")[0].CodeMirror;		
		codibase.server.codemirror.clearSelection(cm);
		$(window).resize();

		codibase.server.sourceView.mark(commentRange, false);
		codibase.server.codemirror.scrollIntoView(cm, commentRange);
	...
	onCommentOpened: function(comment) {
		...
		$(window).resize();
		var cm = $(".source-view>.code>.CodeMirror")[0].CodeMirror;		
		codibase.server.codemirror.scrollIntoView(cm, comment.range);
	},
	onCloseComment: function() {
		...
		codibase.server.sourceView.highlightCommentTrigger();
		$(window).resize();
		codibase.server.sourceView.clearMark();
	},
	...
	mark: function(markRange, scroll) {
		...
		codibase.server.codemirror.mark(cm, markRange);
		if (scroll)
			codibase.server.codemirror.scrollTo(cm, markRange);
	...
	restoreMark: function() {
		...
		if (markRange) 
			...
			codibase.server.codemirror.clearMark(cm);
	},
	clearMark: function() {
		var cm = $(".source-view>.code>.CodeMirror")[0].CodeMirror;		
		codibase.server.codemirror.clearMark(cm);
    		...
    		for (var i in blameInfos) {
        		...
        		for (var j in blameInfo.ranges) {
            		...
            		$("<a class='hash'>" + blameInfo.abbreviatedHash + "</a>")
            				...
            				.attr("onclick", "codibase.server.viewState.getFromViewAndSetToHistory();");
    				...
    				$hashLink.hover(function() {
    					...
    					var $tooltip = $(`<div class='blame-message'><div class='loading'>${codibase.server.sourceView.translations["loading"]}...</div></div>`);
