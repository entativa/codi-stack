...
public class IssueNotificationManager {
	...
	public void on(IssueEvent event) {
		...
		for (Map.Entry<String, Group> entry: newGroups.entrySet()) {
			...
			String threadingReferences = String.format("<you-in-field-%s-%s@codibase>", entry.getKey(), issue.getUUID());
				...
				if (mentionedUser != null) {
					...
					if (!isNotified(notifiedEmailAddresses, mentionedUser)) {
						String subject = String.format(
								...
								issue.getReference(), 
								emojis.apply(issue.getTitle()));
						String threadingReferences = String.format("<mentioned-%s@codibase>", issue.getUUID());
