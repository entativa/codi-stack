...
import io.codibase.server.web.component.blob.BlobFolderSelector;
import io.codibase.server.web.component.blob.BlobSelector;
import io.codibase.server.web.component.dropzonefield.DropzoneField;
...
abstract class InsertUrlPanel extends Panel {
	...
	private Component newInputUrlPanel() {
		Fragment fragment = new Fragment(CONTENT_ID, "inputUrlFrag", this) {
			...
			public void renderHead(IHeaderResponse response) {
				...
				String script = String.format("codibase.server.markdown.onInputUrlDomReady('%s');", getMarkupId());
	...
	private Component newUploadPanel() {
		...
		if (attachmentSupport != null) {
			...
			int maxUploadFileSize = CodiBase.getInstance(SettingService.class).getPerformanceSetting().getMaxUploadFileSize();
	...
	private UploadService getUploadService() {
		return CodiBase.getInstance(UploadService.class);
	}
