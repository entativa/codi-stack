...
import io.codibase.server.web.component.markdown.ContentQuoted;
...
public abstract class CodeCommentPanel extends Panel {
	...
	private WebMarkupContainer newCommentOrReplyContainer() {
		...
		if (isContextDifferent(getComment().getCompareContext())) {
			String url = CodiBase.getInstance(UrlService.class).urlFor(getComment(), false);
		...
		actionsContainer.add(new AjaxLink<Void>("delete") {
			...
			public void onClick(AjaxRequestTarget target) {
				CodiBase.getInstance(TransactionService.class).run(() -> {
					...
					CodiBase.getInstance(AuditService.class).audit(getComment().getProject(), "deleted code comment on file \"" + getComment().getMark().getPath() + "\"", oldAuditContent, null);
					CodiBase.getInstance(CodeCommentService.class).delete(getComment());
				});
			}
		...
		RequestCycle.get().getListeners().add(new AbstractRequestCycleListener() {
			...
			public void onEndRequest(RequestCycle cycle) {
				if (SecurityUtils.getAuthUser() != null)
					CodiBase.getInstance(VisitInfoService.class).visitCodeComment(SecurityUtils.getAuthUser(), getComment());
			}
	...
	private class CodeCommentStatusChangeActivity implements CodeCommentActivity {
		...
		public Component render(String componentId) {
			...
			if (isContextDifferent(getChange().getCompareContext())) {
				String url = CodiBase.getInstance(UrlService.class).urlFor(getChange(), false);
	...
	public class CodeCommentReplyActivity implements CodeCommentActivity {
		...
		public Component render(String componentId) {
			...
			if (isContextDifferent(reply.getCompareContext())) {
				String url = CodiBase.getInstance(UrlService.class).urlFor(reply, false);
				viewFragment.add(new ExternalLink("context", url) {
			...
			viewFragment.add(new AjaxLink<Void>("delete") {
				...
				public void onClick(AjaxRequestTarget target) {
					viewFragment.remove();
					CodiBase.getInstance(CodeCommentReplyService.class).delete(getReply());
