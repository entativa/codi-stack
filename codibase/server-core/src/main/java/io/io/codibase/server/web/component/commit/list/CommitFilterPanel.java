...
import io.codibase.server.search.commit.CommitterCriteria;
...
import io.codibase.server.web.component.build.choice.BuildChoiceProvider;
import io.codibase.server.web.component.build.choice.BuildSingleChoice;
...
abstract class CommitFilterPanel extends GenericPanel<CommitQuery> {
	...
	protected void onInitialize() {
        ...
        var fileChoice = new StringSingleChoice("file", new IModel<String>() {
			...
			protected List<String> load() {
				return CodiBase.getInstance(CommitInfoService.class).getFiles(getProject().getId());
			}
		}, true);
	...
	private void addAuthorOrCommitterChoice(String componentId, boolean isAuthor) {
        var choice = new StringMultiChoice(componentId, new IModel<Collection<String>>() {
			...
			@Override
			protected List<String> load() {
				var users = CodiBase.getInstance(CommitInfoService.class).getUsers(getProject().getId());
