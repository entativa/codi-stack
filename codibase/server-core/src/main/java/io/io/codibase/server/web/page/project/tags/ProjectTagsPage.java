...
import io.codibase.server.web.component.user.ident.PersonIdentPanel;
				...
				form.add(new AjaxButton("create") {
					...
					protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
						...
						if (getProject().getObjectId(GitUtils.tag2ref(tagName), false) != null) {
							...
							try {
								CodiBase.getInstance(GitService.class).createTag(getProject(), tagName, 
		...
		columns.add(new AbstractColumn<RefFacade, Void>(Model.of("")) {
			...
			public void populateItem(Item<ICellPopulator<RefFacade>> cellItem, String componentId,
				...
				fragment.add(new AjaxLink<Void>("delete") {
					...
					@Override
					public void onClick(AjaxRequestTarget target) {
						CodiBase.getInstance(ProjectService.class).deleteTag(getProject(), tagName);
