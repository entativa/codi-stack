...
public abstract class PipelinePanel extends Panel {
	...
	protected void onInitialize() {
		...
		if (sortable != null) { 
			String startScript = "codibase.server.pipeline.onSortStart(ui.item);";
			String stopScript = ""
					+ "if (ui.item.fromList == ui.item.toList && ui.item.fromItem == ui.item.toItem)"
					+ "  codibase.server.pipeline.onSortStop(ui.item);";
	...
	public void onEvent(IEvent<?> event) {
		if (event.getPayload() instanceof JobSelectionChange) {
			...
			String script = String.format("codibase.server.pipeline.markJobActive($(\"#%s>.pipeline\"), %s);", 
	...
	private String buildDependencyMap() {
		...
		try {
			return CodiBase.getInstance(ObjectMapper.class).writeValueAsString(dependencyMap);
	...
	public void renderHead(IHeaderResponse response) {
		...
		// dependency line
		String script = String.format("codibase.server.pipeline.onWindowLoad('%s', %s, %s);", 
