...
import io.codibase.server.util.RangeUtils;
...
import io.codibase.server.web.component.floating.AlignTarget;
import io.codibase.server.web.component.floating.Alignment;
import io.codibase.server.web.component.floating.ComponentTarget;
...
public abstract class InputAssistBehavior extends AbstractPostAjaxBehavior {
	...
	protected void respond(AjaxRequestTarget target) {
		...
		if (type.equals("close")) {
			...
			String script = String.format("codibase.server.inputassist.markErrors('%s', %s);", 
				...
				if (!suggestions.isEmpty()) {
					...
					if (hasAppendSpaceSuggestions && !hasOtherSuggestions) {
						target.appendJavaScript(
								String.format("codibase.server.inputassist.appendSpace('%s');", 
						...
						if (dropdown == null) {
							...
							script = String.format("codibase.server.inputassist.assistOpened('%s', '%s', '%s');",
							...
							script = String.format("codibase.server.inputassist.assistUpdated('%s', '%s', '%s');",
									getComponent().getMarkupId(), dropdown.getMarkupId(), JavaScriptEscape.escapeJavaScript(inputContent));
							target.appendJavaScript(script);
						}
					}
	...
	public void renderHead(Component component, IHeaderResponse response) {
		...
		String script;
		try {
			script = String.format("codibase.server.inputassist.onDomReady('%s', %s, %s);", 
