...
import io.codibase.server.util.ColorUtils;
import io.codibase.server.util.DateUtils;
import io.codibase.server.web.asset.emoji.Emojis;
import io.codibase.server.web.asset.lozad.LozadResourceReference;
...
public class MarkdownViewer extends GenericPanel<String> {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				case "issue":
					...
					if (issue != null && SecurityUtils.canAccessIssue(issue)) {
						String color = CodiBase.getInstance(SettingService.class).getIssueSetting().getStateSpec(issue.getState()).getColor();
						String script = String.format("codibase.server.markdown.renderIssueTooltip('%s', '%s', '%s', '%s');", 
						...
						target.appendJavaScript("codibase.server.markdown.renderIssueTooltip();");
					}
					break;
				case "pull request":
					reference = PullRequestReference.of(referenceId, null);
					var request = CodiBase.getInstance(PullRequestService.class).find(reference.getProject(), reference.getNumber());
					...
					if (request != null && SecurityUtils.canReadCode(request.getTargetProject())) {
						...
						String script = String.format("codibase.server.markdown.renderPullRequestTooltip('%s', '%s', '%s');", 
								Emojis.getInstance().apply(JavaScriptEscape.escapeJavaScript(request.getTitle())), 
								status, statusCss);
						target.appendJavaScript(script);
					} else {
						target.appendJavaScript("codibase.server.markdown.renderPullRequestTooltip();");
					}
					break;
				case "build":
					...
					if (build != null && SecurityUtils.canAccessBuild(build)) {
						...
						String script = String.format("codibase.server.markdown.renderBuildTooltip('%s', '%s', '%s');", 
								JavaScriptEscape.escapeJavaScript(title), iconHref, iconCss);
						target.appendJavaScript(script);
					} else {
						target.appendJavaScript("codibase.server.markdown.renderBuildTooltip();");
					}
					break;
				case "user":
					User user = CodiBase.getInstance(UserService.class).findByName(referenceId);
					if (user != null) {
						String avatarUrl = CodiBase.getInstance(AvatarService.class).getUserAvatarUrl(user.getId());
						String script = String.format("codibase.server.markdown.renderUserTooltip('%s', '%s')", 
				...
				case "commit":
					...
					if (referenceId.contains(":")) {
						commitProject = CodiBase.getInstance(ProjectService.class)
					...
					if (commit != null && SecurityUtils.canReadCode(commitProject)) {
						String script = String.format("codibase.server.markdown.renderCommitTooltip('%s', '%s', '%s');", 
						...
						target.appendJavaScript("codibase.server.markdown.renderCommitTooltip();");
					}
					break;
				default:
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.markdown.onViewerDomReady('%s', %s, '%s', %s, %s, %s);", 
				getMarkupId(), 
