package io.codibase.server.web.page.admin.groupmanagement.profile;
...
public class GroupProfilePage extends GroupPage {
	...
	protected void onInitialize() {
		...
		Form<?> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				if (editor.isValid()) {
					...
					CodiBase.getInstance(AuditService.class).audit(null, "changed basic settings of group \"" + group.getName() + "\"", oldAuditContent, newAuditContent);
		...
		form.add(new Link<Void>("delete") {
			...
			public void onClick() {
				var oldAuditContent = VersionedXmlDoc.fromBean(getGroup()).toXML();
				CodiBase.getInstance(GroupService.class).delete(getGroup());
				CodiBase.getInstance(AuditService.class).audit(null, "deleted group \"" + getGroup().getName() + "\"", oldAuditContent, null);
