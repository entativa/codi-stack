package io.codibase.server.web.component.user.ssoaccount;
			...
			public void populateItem(Item<ICellPopulator<SsoAccount>> cellItem, String componentId, 
				...
				fragment.add(new AjaxLink<Void>("delete") {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						CodiBase.getInstance(SsoAccountService.class).delete(ssoAccount);
						if (getPage() instanceof UserPage)
							CodiBase.getInstance(AuditService.class).audit(null, "deleted SSO account \"" + ssoAccount.getProvider() + "/" + ssoAccount.getSubject() + "\" from account \"" + ssoAccount.getUser().getName() + "\"", null, null);
			...
			public IModel<SsoAccount> model(SsoAccount ssoAccount) {
				...
				return new LoadableDetachableModel<SsoAccount>() {
					...
					protected SsoAccount load() {
						return CodiBase.getInstance(SsoAccountService.class).load(id);
					}
