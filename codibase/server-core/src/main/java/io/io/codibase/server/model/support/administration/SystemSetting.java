...
import io.codibase.server.git.location.CurlLocation;
import io.codibase.server.git.location.GitLocation;
import io.codibase.server.git.location.SystemCurl;
import io.codibase.server.git.location.SystemGit;
...
public class SystemSetting implements Serializable, Validatable {
	...
	@Editable(order=200, name="Git Command Line", description="CodiBase requires git command line to manage repositories. The minimum "
	...
	@Editable(order=250, name="curl Command Line", description="CodiBase configures git hooks to communicate with itself via curl")
	@Valid
	@NotNull(message="may not be empty")
	...
	@Editable(order=400, description = "Auto update check is performed by requesting an image in " +
			"your browser from codibase.io indicating new version availability, with color " +
