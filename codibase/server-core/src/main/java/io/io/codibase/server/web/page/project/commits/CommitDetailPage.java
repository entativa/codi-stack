...
import io.codibase.server.web.component.AjaxLazyLoadPanel;
import io.codibase.server.web.component.beaneditmodal.BeanEditModalPanel;
import io.codibase.server.web.component.branch.create.CreateBranchPanel;
import io.codibase.server.web.component.contributorpanel.ContributorPanel;
import io.codibase.server.web.component.createtag.CreateTagPanel;
import io.codibase.server.web.component.diff.revision.RevisionAnnotationSupport;
import io.codibase.server.web.component.diff.revision.RevisionDiffPanel;
import io.codibase.server.web.component.floating.FloatingPanel;
import io.codibase.server.web.component.gitsignature.SignatureStatusPanel;
import io.codibase.server.web.component.job.jobinfo.JobInfoButton;
...
import io.codibase.server.web.component.user.contributoravatars.ContributorAvatars;
...
public class CommitDetailPage extends ProjectPage implements RevisionAnnotationSupport, JobAuthorizationContextAware {
	...
	private final IModel<Collection<CodeComment>> commentsModel =
			new LoadableDetachableModel<>() {
				...
				protected Collection<CodeComment> load() {
					CodeCommentService manager = CodiBase.getInstance(CodeCommentService.class);
			...
			public Component getLazyLoadComponent(String markupId) {
				Fragment fragment = new Fragment(markupId, "refsFrag", CommitDetailPage.this) {
					...
					public void renderHead(IHeaderResponse response) {
						...
						String script = String.format("codibase.server.commitDetail.initRefs('%s');", getMarkupId());
	...
	private Collection<ObjectId> getDescendants() {
		Collection<ObjectId> descendants = CodiBase.getInstance(CommitInfoService.class)
	...
	public Collection<CodeProblem> getOldProblems(String blobPath) {
		...
		for (Build build: getProject().getBuilds(getCompareWith())) {
			for (CodeProblemContribution contribution: CodiBase.getExtensions(CodeProblemContribution.class))
	...
	public Map<Integer, CoverageStatus> getOldCoverages(String blobPath) {
		...
		for (Build build: getProject().getBuilds(getCompareWith())) {
			for (LineCoverageContribution contribution: CodiBase.getExtensions(LineCoverageContribution.class)) {
