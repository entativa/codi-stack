...
import static io.codibase.server.model.Build.NAME_COMMIT;
import static io.codibase.server.model.Build.NAME_JOB;
import static io.codibase.server.model.Build.NAME_PULL_REQUEST;
import static io.codibase.server.model.Build.NAME_TAG;
import static io.codibase.server.search.entity.build.BuildQuery.getRuleName;
import static io.codibase.server.search.entity.build.BuildQueryLexer.And;
import static io.codibase.server.search.entity.build.BuildQueryLexer.Is;
...
import io.codibase.server.annotation.RetryCondition;
import io.codibase.server.annotation.SuggestionProvider;
...
public class Job implements NamedElement, Validatable {
	...
	private static String getJobExecutorPlaceholder() {
		if (CodiBase.getInstance(SettingService.class).getJobExecutors().isEmpty())
	...
	private static List<InputSuggestion> suggestJobExecutors(String matchWith) {
		...
		if (jobName != null) {
			...
			for (JobExecutor executor: CodiBase.getInstance(SettingService.class).getJobExecutors()) {
	...
	@Editable(order=200, description="Steps will be executed serially on same node, sharing the same <a href='https://docs.codibase.io/concepts#job-workspace'>job workspace</a>")
	@Valid
	...
	public boolean isValid(ConstraintValidatorContext context) {
		...
		var jobExecutors = CodiBase.getInstance(SettingService.class).getJobExecutors();
		...
		try {
			io.codibase.server.buildspec.job.retrycondition.RetryCondition.parse(this, getRetryCondition());
		} catch (Exception e) {
