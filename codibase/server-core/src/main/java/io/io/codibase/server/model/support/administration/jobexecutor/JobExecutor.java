...
import io.codibase.server.annotation.DnsName;
...
import io.codibase.server.event.project.build.BuildRunning;
...
public abstract class JobExecutor implements Serializable {
	...
	@Editable(order=30, name="Enable Site Publish", group = "Privilege Settings", description="Enable this to allow to run site publish step. CodiBase will serve project "
	...
	@io.codibase.server.annotation.JobMatch(withProjectCriteria = true, withJobCriteria = true)
	@Nullable
	...
	protected void notifyJobRunning(Long buildId, @Nullable Long agentId) {
		CodiBase.getInstance(TransactionService.class).run(() -> {
			...
			if (agentId != null)
				build.setAgent(CodiBase.getInstance(AgentService.class).load(agentId));
			buildService.update(build);
			CodiBase.getInstance(ListenerRegistry.class).post(new BuildRunning(build));
		});
	}
