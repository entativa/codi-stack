package io.codibase.server.agent;
		...
		try {
			switch (message.getType()) {
				case AGENT_DATA:
					...
					break;
				case REQUEST:
					CodiBase.getInstance(ExecutorService.class).execute(() -> {
						try {
	...
	private Serializable service(Serializable request) {
		try {
			if (request instanceof WantToDisconnectAgent || request instanceof WaitingForAgentResourceToBeReleased) {
				if (agentId != null)
					CodiBase.getInstance(ResourceAllocator.class).agentDisconnecting(agentId);
				return null;
			} else {
