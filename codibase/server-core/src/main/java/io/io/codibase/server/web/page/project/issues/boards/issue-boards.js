codibase.server.issueBoards = {
	...
	onCardDropped: function(fromDroppableId, fromIndex, toDroppableId, toIndex, accepted) {
		...
		if (accepted) {
			...
			codibase.server.issueBoards.updateAddToIteration(fromDroppableId);
			$cardCount = $toDroppable.parent().find(".card-count");
			$cardCount.text(parseInt($cardCount.text()) + 1);
			codibase.server.issueBoards.updateAddToIteration(toDroppableId);
		} else {
	...
	changeCardCount: function(droppableId, change) {
		...
		codibase.server.issueBoards.updateAddToIteration(droppableId);
	},
