package io.codibase.server.web.component.symboltooltip;
	...
	protected void onInitialize() {
			...
			protected void populateItem(ListItem<QueryHit> item) {
				...
				AjaxLink<Void> delegateLink = new ViewStateAwareAjaxLink<Void>("delegateLink") {
					...
					public void onClick(AjaxRequestTarget target) {
						String script = String.format("codibase.server.symboltooltip.removeTooltip(document.getElementById('%s'));", 
				...
				add(runTaskBehavior = new RunTaskBehavior() {
					...
					protected void runTask(AjaxRequestTarget target) {
						...
						if (symbolName.length() >= IndexConstants.NGRAM_SIZE) {
							int maxQueryEntries = CodiBase.getInstance(SettingService.class)
		...
		add(new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				if (symbolName.length() != 0 && symbolName.indexOf('?') == -1 && symbolName.indexOf('*') == -1) {
					...
					if (symbolHits.size() < QUERY_ENTRIES) {
						...
						List<Symbol> symbols = CodiBase.getInstance(CodeSearchService.class).getSymbols(getProject(),
				...
				target.add(content);
				String script = String.format("codibase.server.symboltooltip.doneQuery('%s');", content.getMarkupId());
				target.appendJavaScript(script);
			}
			...
			public void renderHead(Component component, IHeaderResponse response) {
				...
				String script = String.format("codibase.server.symboltooltip.init('%s', %s);", 
