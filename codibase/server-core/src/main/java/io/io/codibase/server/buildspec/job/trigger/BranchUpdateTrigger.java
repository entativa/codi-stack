...
import io.codibase.server.util.usermatch.Anyone;
...
public class BranchUpdateTrigger extends JobTrigger {
	...
	@Editable(name="Branches", order=100, placeholder="Any branch", description="Optionally specify space-separated branches "
			+ "to check. Use '**' or '*' or '?' for <a href='https://docs.codibase.io/appendix/path-wildcard' target='_blank'>path wildcard match</a>. "
	...
	@Editable(name="Touched Files", order=200, placeholder="Any file", 
			description="Optionally specify space-separated files to check. Use '**', '*' or '?' for <a href='https://docs.codibase.io/appendix/path-wildcard' target='_blank'>path wildcard match</a>. "
	...
	private boolean pushedBy(RefUpdated refUpdated) {
		if (refUpdated.getUser() != null) {
			return io.codibase.server.util.usermatch.UserMatch.parse(getUserMatch()).matches(refUpdated.getUser());			
		} else {
			return true;
		}
	}
