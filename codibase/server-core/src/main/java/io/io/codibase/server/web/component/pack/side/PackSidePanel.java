package io.codibase.server.web.component.pack.side;
...
import io.codibase.server.search.entity.issue.FixedBetweenCriteria;
...
import io.codibase.server.web.component.pack.choice.PackChoiceProvider;
import io.codibase.server.web.component.pack.choice.SelectPackToActChoice;
		...
		if (build != null) {
			var choiceProvider = new PackChoiceProvider() {
				...
				public void query(String term, int page, Response<Pack> response) {
					...
					var packService = CodiBase.getInstance(PackService.class);
					if (term.length() == 0)
						term = null;
		...
		if (SecurityUtils.canWritePack(getPack().getProject())) {
			labelsContainer.add(new InplacePropertyEditLink("head") {
				...
				protected void onUpdated(IPartialPageRequestHandler handler, Serializable bean, String propertyName) {
					...
					CodiBase.getInstance(PackLabelService.class).sync(getPack(), labelsBean.getLabels());
