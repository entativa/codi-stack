...
import io.codibase.server.web.component.markdown.SuggestionSupport.Selection;
...
public class MarkdownEditor extends FormComponentPanel<String> {
			...
			protected Component newContent(String id, FloatingPanel dropdown) {
				var fragment = new Fragment(id, "referenceMenuFrag", MarkdownEditor.this) {
					...
					public void renderHead(IHeaderResponse response) {
						...
						String script = String.format("codibase.server.markdown.setupActionMenu($('#%s'), $('#%s'));", 
			...
			preview.add(new Label("rendered", rendered) {
				...
				public void renderHead(IHeaderResponse response) {
					...
					String script = String.format(
							"codibase.server.markdown.initRendered('%s', %s);", 
		...
		container.add(actionBehavior = new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				case "render":
					...
					String script = String.format(
							"codibase.server.markdown.onRendered('%s', '%s');", 
				...
				case "loadEmojis":
					...
					script = String.format("codibase.server.markdown.onEmojisLoaded('%s', %s);", container.getMarkupId(), json);
					target.appendJavaScript(script);
					break;
				case "userQuery":
					...
					try {
						json = CodiBase.getInstance(ObjectMapper.class).writeValueAsString(userList);
					} catch (JsonProcessingException e) {
						throw new RuntimeException(e);
					}
				...
				case "referenceQuery":
					...
					try {
						json = CodiBase.getInstance(ObjectMapper.class).writeValueAsString(referenceList);
					} catch (JsonProcessingException e) {
						throw new RuntimeException(e);
					}
	...
	public void onEvent(IEvent<?> event) {
		...
		if (event.getPayload() instanceof ContentQuoted) {
			...
			String script = String.format("codibase.server.markdown.onQuote('%s', '%s');", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.markdown.onDomReady('%s', %s, %d, %s, %d, %b, %b, '%s', '%s', %s, %s);", 
				container.getMarkupId(), 
				actionCallback, 
				ATWHO_LIMIT, 
		...
		script = String.format("codibase.server.markdown.onLoad('%s');", container.getMarkupId());
	...
	public void insertUrl(AjaxRequestTarget target, boolean isImage, String url, 
		...
		String script = String.format("codibase.server.markdown.insertUrl('%s', %s, '%s', '%s', %s);",
	...
	public void insertText(AjaxRequestTarget target, String text) {
		String script = String.format("codibase.server.markdown.insertText('%s', '%s');",
