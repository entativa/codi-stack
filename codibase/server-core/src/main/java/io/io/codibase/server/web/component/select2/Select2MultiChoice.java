 ...
 */
package io.codibase.server.web.component.select2;
...
import io.codibase.server.web.component.select2.json.JsonBuilder;
...
public class Select2MultiChoice<T> extends AbstractSelect2Choice<T, Collection<T>> {
	...
	protected void renderInitializationScript(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.select2DragSort.onWindowLoad('%s');", getMarkupId());
