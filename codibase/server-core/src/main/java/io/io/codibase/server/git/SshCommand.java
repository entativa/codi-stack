...
import static io.codibase.server.model.Project.decodeFullRepoNameAsPath;
import static io.codibase.server.security.SecurityUtils.asPrincipals;
import static io.codibase.server.security.SecurityUtils.asSubject;
import static io.codibase.server.security.SecurityUtils.asUserPrincipal;
...
import io.codibase.commons.utils.command.ExecutionResult;
import io.codibase.server.CodiBase;
import io.codibase.server.cluster.ClusterService;
import io.codibase.server.service.ProjectService;
import io.codibase.server.git.command.ReceivePackCommand;
import io.codibase.server.git.command.UploadPackCommand;
...
import io.codibase.server.util.OutputStreamWrapper;
...
class SshCommand implements Command, ServerSessionAware {
	...
	public void start(ChannelSession channel, Environment env) throws IOException {
		...
		if (clusterAccess || activeServerAddress.equals(clusterService.getLocalServerAddress())) {
	        File gitDir = CodiBase.getInstance(ProjectService.class).getGitDir(projectFacade.getId());
	        ...
	        WorkExecutionService workExecutionService = CodiBase.getInstance(WorkExecutionService.class);
			...
			ExecutorService executorService = CodiBase.getInstance(ExecutorService.class);
			future = executorService.submit(() -> {
				SshService sshService = CodiBase.getInstance(SshService.class);
