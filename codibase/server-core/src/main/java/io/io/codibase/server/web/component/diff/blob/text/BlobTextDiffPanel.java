...
import static io.codibase.server.codequality.BlobTarget.groupByLine;
import static io.codibase.server.util.diff.DiffRenderer.toHtml;
...
import io.codibase.server.git.BlameCommit;
...
import io.codibase.server.util.diff.DiffBlock;
import io.codibase.server.util.diff.DiffMatchPatch.Operation;
...
import io.codibase.server.web.component.diff.blob.BlobAnnotationSupport;
...
import io.codibase.server.web.util.DiffPlanarRange;
...
public class BlobTextDiffPanel extends Panel {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				switch (params.getParameterValue("action").toString("")) {
					case "expand":
						...
						String script = String.format("codibase.server.blobTextDiff.expand('%s', %d, \"%s\");",
					...
					case "openSelectionPopover":
						...
						script = String.format("codibase.server.blobTextDiff.openSelectionPopover('%s', %s, %s, %s, '%s', %s);", 
					...
					case "addComment":
						...
						script = String.format("codibase.server.blobTextDiff.onAddComment($('#%s'), %s);", 
					...
					case "openComment": 
						...
						script = String.format("codibase.server.blobTextDiff.onCommentOpened($('#%s'), %s);", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.blobTextDiff.onDomReady('%s', '%s', '%s', '%s', '%s', '%s', %s, %s, %s, %s, %s, %s, %s);", 
		...
		script = String.format("codibase.server.blobTextDiff.onLoad('%s', %s);", getMarkupId(), jsonOfMarkRange);
	...
	private void appendBlame(StringBuilder builder, int oldLineNo, int newLineNo) {
		...
		if (commit != null) {
			if (diffMode == DiffViewMode.UNIFIED && !commit.getHash().equals(blameInfo.lastCommitHash)
				...
				if (diffMode == DiffViewMode.UNIFIED) {
					builder.append(String.format("<td class='blame noselect'><a class='hash' href='%s' onclick='codibase.server.viewState.getFromViewAndSetToHistory();' data-hash='%s'>%s</a><span class='date'>%s</span><span class='author'>%s</span></td>",
					...
					builder.append(String.format("<td class='abbr blame noselect'><a class='hash' href='%s' onclick='codibase.server.viewState.getFromViewAndSetToHistory();' data-hash='%s'>%s</a></td>",
							url, commit.getHash(), GitUtils.abbreviateSHA(commit.getHash())));
				}
			} else {
				if (diffMode == DiffViewMode.UNIFIED) {
	...
	public void onCommentDeleted(AjaxRequestTarget target) {
		String script = String.format("codibase.server.blobTextDiff.onCommentDeleted($('#%s'));", getMarkupId());
		target.appendJavaScript(script);
		unmark(target);
	}

	public void onCommentClosed(AjaxRequestTarget target) {
		String script = String.format("codibase.server.blobTextDiff.onCommentClosed($('#%s'));", getMarkupId());
		target.appendJavaScript(script);
		unmark(target);
	}

	public void onCommentAdded(AjaxRequestTarget target, CodeComment comment, DiffPlanarRange range) {
		String script = String.format("codibase.server.blobTextDiff.onCommentAdded($('#%s'), %s);", 
