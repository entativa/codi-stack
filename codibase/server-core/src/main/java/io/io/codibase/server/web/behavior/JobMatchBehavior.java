...
import io.codibase.server.job.match.JobMatchParser;
...
import static io.codibase.server.job.match.JobMatchLexer.*;
		...
		if (terminalExpect.getElementSpec() instanceof LexerRuleRefElementSpec) {
			...
			if ("criteriaValue".equals(spec.getLabel())) {
				...
				if (!fieldElements.isEmpty()) {
					...
					if (fieldName.equals(Build.NAME_PROJECT))
						hints.add("Use '**', '*' or '?' for <a href='https://docs.codibase.io/appendix/path-wildcard' target='_blank'>path wildcard match</a>");
