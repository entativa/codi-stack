codibase.server.commitList = {
	onDomReady: function(containerId, commits) {
		$("#" + containerId).on("resized", function() {
			$(".commit-graph").each(function() {
				codibase.server.commitList.drawGraph($(this));
			});
			return false;
		});
		codibase.server.commitList.renderGraph(containerId, commits);	
	},
	renderGraph: function(containerId, commits) {
		...
		codibase.server.commitList.populateGraphData($graph, commits);
		codibase.server.commitList.drawGraph($graph);
