package io.codibase.server.web.page.project.blob.render.renderers.buildspec;

import static io.codibase.server.web.page.project.blob.render.renderers.buildspec.BuildSpecRenderer.getActiveElementIndex;
import static io.codibase.server.web.page.project.blob.render.renderers.buildspec.BuildSpecRenderer.getUrlSegment;
...
import io.codibase.server.buildspec.Import;
import io.codibase.server.buildspec.NamedElement;
import io.codibase.server.buildspec.ParamSpecAware;
import io.codibase.server.buildspec.Service;
...
import io.codibase.server.buildspec.step.StepTemplate;
import io.codibase.server.data.migration.VersionedYamlDoc;
...
import io.codibase.server.util.PathNode.Indexed;
import io.codibase.server.util.PathNode.Named;
...
import io.codibase.server.web.component.pipeline.JobSelectionChange;
import io.codibase.server.web.component.pipeline.PipelinePanel;
import io.codibase.server.web.component.pipeline.Sortable;
import io.codibase.server.web.editable.BeanDescriptor;
import io.codibase.server.web.editable.BeanEditor;
import io.codibase.server.web.editable.BeanUpdating;
import io.codibase.server.web.editable.PropertyContext;
import io.codibase.server.web.editable.PropertyEditor;
import io.codibase.server.web.editable.PropertyUpdating;
import io.codibase.server.web.page.base.BasePage;
import io.codibase.server.web.page.project.blob.render.BlobRenderContext;
import io.codibase.server.web.page.project.blob.render.edit.EditCompleteAware;
...
public class BuildSpecEditPanel extends FormComponentPanel<byte[]> implements BuildSpecAware, EditCompleteAware {
			...
			add(new ParseableFragment("content") {
					...
					Fragment jobsEditor = new Fragment("content", "jobsFrag", BuildSpecEditPanel.this) {
							...
							add(new PipelinePanel("pipeline") {
												...
												public WebMarkupContainer newLink(String id) {
													return new AjaxLink<Void>(id) {
														...
														public void onClick(AjaxRequestTarget target) {
															...
															target.appendJavaScript("codibase.server.form.markDirty($('.build-spec').closest('form'));");
															target.add(jobsEditor.get("pipeline"));
															resizeWindow(target);
														}
								...
								protected Component renderAction(String componentId) {
									...
									if (context.getBlobIdent().revision != null) {
										for (JobSuggestion suggestion: CodiBase.getExtensions(JobSuggestion.class)) 
						...
						public void renderHead(IHeaderResponse response) {
							...
							response.render(OnDomReadyHeaderItem.forScript(
									String.format("codibase.server.buildSpec.onTabDomReady('.jobs');")));
						}
				...
				private void setupPropertiesEditor(@Nullable AjaxRequestTarget target) {
					...
					propertiesEditor.add(new Behavior() {
						...
						public void renderHead(Component component, IHeaderResponse response) {
							...
							response.render(OnDomReadyHeaderItem.forScript("codibase.server.buildSpec.onTabDomReady('.properties');"));
						}
				...
				private void setupImportsEditor(@Nullable AjaxRequestTarget target) {
					...
					importsEditor.add(new Behavior() {
						...
						public void renderHead(Component component, IHeaderResponse response) {
							super.renderHead(component, response);
							response.render(OnDomReadyHeaderItem.forScript("codibase.server.buildSpec.onTabDomReady('.imports');"));
						}
	...
	private abstract class ElementsEditor<T extends NamedElement> extends Fragment {
		...
		protected void onInitialize() {
			...
			WebMarkupContainer navs = new WebMarkupContainer("navs") {
				...
				public void renderHead(IHeaderResponse response) {
					...
					String script = String.format("codibase.server.buildSpec.markElementActive(%d);", elementIndex);
					...
					item.add(new AjaxSubmitLink("select") {
						...
						protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
							...
							target.appendJavaScript(String.format(
									"codibase.server.buildSpec.markElementActive(%d);", item.getIndex()));
							resizeWindow(target);
						}
		...
		public void renderHead(IHeaderResponse response) {
			super.renderHead(response);
			response.render(OnDomReadyHeaderItem.forScript(
					String.format("codibase.server.buildSpec.onTabDomReady('.%ss');", getUrlSegment(elementClass))));
		}
