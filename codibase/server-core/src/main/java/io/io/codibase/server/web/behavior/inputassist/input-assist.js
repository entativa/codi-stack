codibase.server.inputassist = {
	onDomReady: function(inputId, callback, translations) {
		var $input = $("#" + inputId);
		codibase.server.inputassist.translations = translations;
		
		codibase.server.inputassist.markErrors(inputId, []);
		...
		function keyUpOrDown(keyUp) {
			...
			if ($dropdown) {
				...
				codibase.server.inputassist.updateHelp($dropdown);
	...
	assistOpened: function(inputId, dropdownId, inputContent) {
		...
		codibase.server.inputassist.assistUpdated(inputId, dropdownId, inputContent);
	},
	...
	updateHelp: function($dropdown) {
		if ($dropdown.find("li.active").length != 0) {
			$dropdown.find(".help .complete").empty().append(codibase.server.inputassist.translations["activeHelp"]);
		} else {
			$dropdown.find(".help .complete").empty().append(codibase.server.inputassist.translations["inactiveHelp"]);
		}
	}
};