...
import static io.codibase.server.model.Setting.Key.MAIL;
...
public class DefaultMailService implements MailService, Serializable {
	...
	private boolean isProdTest() {
		if (prodTest == null) {
			prodTest = settingService.getSystemSetting().getServerUrl().equals("https://code.codibase.io") 
					&& !new File("/home/codibase/website").exists();
		}
		return prodTest;
	}
	...
	public void sendMail(SmtpSetting smtpSetting, Collection<String> toList, Collection<String> ccList,
		...
		try {
			...
			if (senderName == null || senderName.equalsIgnoreCase("codibase")) 
	...
	private String getThreadingReferences(String uuid, @Nullable String messageId) {
		var threadingReferences = "<" + uuid + "@codibase>";
		if (messageId != null)
		...
		if (fromInternetAddresses.length != 0 && toInternetAddresses.length != 0) {
			...
			if (!fromAddress.equalsIgnoreCase(systemAddress)) {
				...
				for (InternetAddress receiverInternetAddress : receiverInternetAddresses) {
					...
					if (project != null) {
						if (isOriginatedFromCodiBase(message)) {
							logger.warn("Ignored opening issue from message as it is originated from CodiBase");
						} else {
						...
						try {
							...
							if (fromHeader != null && fromHeader.length != 0 
								...
								sendMailAsync(newArrayList(from.getAddress()), new ArrayList<>(), 
										new ArrayList<>(), "CodiBase is unable to process your message", 
	...
	private boolean isOriginatedFromCodiBase(Message message) {
		try {
			var references = message.getHeader("References");
			return references != null && references.length != 0 && references[0].contains("@codibase>");
		} catch (MessagingException e) {
			throw new RuntimeException(e);
		}
	}
	...
	private void fillAttachments(Project project, String attachmentGroup, Part part, Attachments attachments)
		...
		if (part.getDisposition() != null) {
			...
			var attachmentService = CodiBase.getInstance(AttachmentService.class);
