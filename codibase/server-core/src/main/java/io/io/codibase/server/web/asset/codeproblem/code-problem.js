codibase.server.codeProblem = {
	...
	renderProblems: function(problems, translations) {
		...
		for (var i in problems) {
			...
			var severityInfo = codibase.server.codeProblem.getSeverityInfo(problem);
			...
			$content.append(`<a data-tippy-content='${translations["add-problem-comment"]}' class='add-comment ml-2'><svg class='icon icon-sm mr-2'><use xlink:href='${codibase.server.icons}#comment'/></svg></a>`);
		}
