...
import static io.codibase.k8shelper.SetupCacheFacade.UploadStrategy.UPLOAD_IF_NOT_HIT;
...
@Editable(order=55, name="Set Up Cache", description = "Set up job cache to speed up job execution. " +
		"Check <a href='https://docs.codibase.io/tutorials/cicd/job-cache' target='_blank'>this tutorial</a> " +
...
public class SetupCacheStep extends Step {
	...
	@Editable(order=200, name="Load Keys", description = "In case cache is not hit via above key, " +
			"CodiBase will loop through load keys defined here in order until a matching cache is found " +
	...
	@Editable(order=300, name="Cache Paths", description = "For docker aware executors, this path is inside container, " +
			"and accepts both absolute path and relative path (relative to <a href='https://docs.codibase.io/concepts#job-workspace' target='_blank'>job workspace</a>). " +
	...
	@Editable(order=425, description = "Optionally specify files relative to cache path to ignore when " +
			"detect cache changes. Use '**', '*' or '?' for <a href='https://docs.codibase.io/appendix/path-wildcard' target='_blank'>path wildcard match</a>. " +
	...
	@Editable(order=500, description = "Specify a <a href='https://docs.codibase.io/tutorials/cicd/job-secrets' target='_blank'>job secret</a> whose value is an access token with upload cache permission " +
