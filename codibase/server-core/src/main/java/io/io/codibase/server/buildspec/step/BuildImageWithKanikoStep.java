...
import static io.codibase.agent.DockerExecutorUtils.buildDockerConfig;
...
public class BuildImageWithKanikoStep extends CommandStep {
	...
	@Editable(order=100, description="Optionally specify build context path relative to <a href='https://docs.codibase.io/concepts#job-workspace' target='_blank'>job workspace</a>. "
		...
		return new DefaultInterpreter() {
			...
			public CommandFacade getExecutable(JobExecutor jobExecutor, String jobToken, String image, String runAs,
				...
				if (getBuildContext() != null)
					commandsBuilder.append(" --context=\"/codibase-build/workspace/").append(getBuildContext()).append("\"");
				else
					commandsBuilder.append(" --context=/codibase-build/workspace");
	...
	public static class OCIOutput implements Output {
		...
		@Editable(name="OCI Layout Directory", description = "Specify relative path under <a href='https://docs.codibase.io/concepts#job-workspace' target='_blank'>job workspace</a> to store OCI layout")
		@Interpolative(variableSuggester="suggestVariables")
		@SubPath
		@NoSpace
		@NotEmpty
		...
		@Override
		public String getOptions() {
			return "--no-push --oci-layout-path /codibase-build/workspace/" + getDestPath();
		}
