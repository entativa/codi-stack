...
import io.codibase.server.web.translation.SystemPrompt;
import io.codibase.server.web.translation.Translation;
...
public class Translate extends CommandHandler {
	...
	public void start() {
		...
		if (Bootstrap.command.getArgs().length == 0) {
			logger.error("Missing CodiBase project directory. Usage: {} <path to codibase project directory>",
		...
		try {
			...
			Files.walk(projectDir.toPath())
					...
					.forEach(it -> {
						...
						if (relative != null && relative.startsWith("io/codibase/server/")) {
			...
			Files.walk(projectDir.toPath())
					.filter(it -> it.toString().contains("src/main/java/io/codibase/server"))
