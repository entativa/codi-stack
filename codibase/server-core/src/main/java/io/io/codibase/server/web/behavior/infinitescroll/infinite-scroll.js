codibase.server.infiniteScroll = {
	onLoad: function(containerId, callback, pageSize, itemSelector) {
		...
		var $items = codibase.server.infiniteScroll.getItems($container);
		$container.data("hasMore", $items.length >= pageSize);
		$container.scroll(function() {
			codibase.server.infiniteScroll.check(containerId, true);
		});
		codibase.server.infiniteScroll.check(containerId, false);
	}, 
	...
	check: function(containerId, showIndicator) {
		...
		setTimeout(function() {
			...
			var ajaxIndicator = codibase.server.isDarkMode()?"dark-ajax-indicator.gif":"ajax-indicator.gif";
