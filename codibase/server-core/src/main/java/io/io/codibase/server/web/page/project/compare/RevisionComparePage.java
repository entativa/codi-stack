package io.codibase.server.web.page.project.compare;
...
import io.codibase.server.web.component.revision.AffinalRevisionPicker;
...
public class RevisionComparePage extends ProjectPage implements RevisionAnnotationSupport, EditParamsAware {
		...
		requestModel = new LoadableDetachableModel<PullRequest>() {
			...
			protected PullRequest load() {
				if (state.leftSide.getBranch() != null && state.rightSide.getBranch() != null) {
					...
					return CodiBase.getInstance(PullRequestService.class).findEffective(left, right);
				} else {
					return null;
				}
			}
	...
	public void renderHead(IHeaderResponse response) {
		...
		response.render(CssHeaderItem.forReference(
				new io.codibase.server.web.asset.revisioncompare.RevisionCompareCssResourceReference()));
