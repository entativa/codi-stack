...
codibase.server = {
	form: {
		...
		markDirty: function($forms) {
			$forms.addClass("dirty").each(function() {
				codibase.server.form.dirtyChanged($(this));
			});
		},
		...
		trackDirty: function(container) {
			...
			if ($form.length != 0 && ($form.find(".dirty-aware").length != 0 || $form.hasClass("leave-confirm"))) {
				...
				$(container).find(fieldSelector).on(events, function(e) {
					codibase.server.form.markDirty($form);
				});
				if ($form.find(".is-invalid").length != 0) {
					$form.addClass("dirty");
				}
				codibase.server.form.dirtyChanged($form);
			}
		...
		setupDirtyCheck: function() {
			$("form").each(function() {
				codibase.server.form.trackDirty(this);
			});
			...
			$(document).on("afterElementReplace", function(event, componentId) {
				...
				codibase.server.form.trackDirty($component[0]);
			});
	...
	focus: {
		...
		focusOn: function(componentId) {
			if (componentId)
				codibase.server.focus.doFocus($("#" + componentId));
			else if (document.activeElement != document.body) 
				document.activeElement.blur();
				
			codibase.server.focus.$components = null;
		},
		...
		setupAutoFocus: function() {
			if (typeof(Wicket) != "undefined" && typeof(Wicket.Focus) != "undefined") {
				...
				Wicket.Focus.setFocusOnId = function(componentId) {
					codibase.server.focus.focusOn(componentId);
					wicketSetFocusOnId(componentId);
				}
			}
			
			Wicket.Event.subscribe('/ajax/call/beforeSend', function() {
				codibase.server.focus.$components = $();
			});
			Wicket.Event.subscribe('/ajax/call/complete', function() {
				if (codibase.server.focus.$components != null) {
					codibase.server.focus.doFocus(codibase.server.focus.$components);
				}
			});

			codibase.server.focus.doFocus($(document));

			$(document).on("afterElementReplace", function(event, componentId) {
				if (codibase.server.focus.$components != null)
					codibase.server.focus.$components = codibase.server.focus.$components.add("#" + componentId);
			});			
		},
	...
	choiceFormatter: {
		formatSelection: function(choice) {
            if (choice.id && choice.id.indexOf("<$CodiBaseSpecialChoice$>") == 0)
	...
	viewState: {
		...
		setToHistory: function(viewState) {
			...
			codibase.server.history.current = {
				state: newState,
				url: window.location.href
			};
		},
		getFromViewAndSetToHistory: function() {
			var viewState = codibase.server.viewState.getFromView();
			if (viewState)
				codibase.server.viewState.setToHistory(viewState);
		},
		getFromHistoryAndSetToView: function() {
			var viewState = codibase.server.viewState.getFromHistory();
			if (viewState)
				codibase.server.viewState.setToView(viewState);
		}
	},
	...
	history: {
		...
		init: function(popStateCallback) {
			...
			setTimeout(function() {
				window.onpopstate = function(event) {
					if (codibase.server.history.getHashlessUrl(window.location.href) 
							!= codibase.server.history.getHashlessUrl(codibase.server.history.current.url)) {
						if (codibase.server.form.confirmLeave()) {
							 ...
							 */
							history.pushState(codibase.server.history.current.state, '' , 
									codibase.server.history.current.url);
						}
					} else {
						codibase.server.viewState.getFromHistoryAndSetToView();
					}
				};
			}, 100);
		...
		pushState: function(url, data, title) {
			var state = {data: data};
			codibase.server.history.current = {state: state, url: url};
			history.pushState(state, '', url);
			document.title = title;
			
			// Let others have a chance to do something before marking the page as visited
			setTimeout(function() {
				codibase.server.history.setVisited();
			}, 100);
		},
	...
	mouseState: {
		...
		track: function() {
			$(document).mousedown(function() { 
				codibase.server.mouseState.pressed = true;
				codibase.server.mouseState.moved = false;
			});
			$(document).mouseup(function() {
				codibase.server.mouseState.pressed = false;
				codibase.server.mouseState.moved = false;
			});	
			$(document).mousemove(function(e) {
				...
				if (e.clientX != self.clientX || e.clientY != self.clientY) {
					codibase.server.mouseState.moved = true;
					self.clientX = e.clientX;
					self.clientY = e.clientY;
				}
				codibase.server.mouseState.position = {
					left: e.clientX,
					top: e.clientY
				}
			});
		...
		function doSetup($container) {
			$container.find(".dropdown-toggle:not('.no-dropdown-caret')").addBack(".dropdown-toggle:not('.no-dropdown-caret')").each(function() {
				if ($(this).find(".dropdown-caret").length == 0) {
					$(this).append("<svg class='dropdown-caret icon rotate-90'><use xlink:href='" + codibase.server.icons + "#arrow'/></svg>");
				}
			});
		}	
		$(document).on("afterElementReplace", function(event, componentId) {
		...
		setup: function() {
			function doSetup($container, afterElementReplace) {
				$container.find(".ps.ps-scroll").addBack(".ps.ps-scroll").each(function() {
					if (codibase.server.util.isDevice()) {
		...
		function installClearer($container) {
			...
			$clearableWrappers.each(function() {
				...
				if (!$input.hasClass("clearable")) {
					...
					var $clear = $("<a class='input-clear'><svg class='icon align-middle'><use xlink:href='" + codibase.server.icons + "#times'/></svg></a>");
	...
	formatBriefDuration(seconds) {
		var translations = codibase.server.translations;
		var intervals = [];
	...
	onDomReady: function(bootTimestamp, icons, popStateCallback, removeAutosaveKeys, translations) {
		codibase.server.translations = translations;

		codibase.server.bootTimestamp = bootTimestamp;
		codibase.server.icons = icons;

		for (var i in removeAutosaveKeys) 
			localStorage.removeItem(removeAutosaveKeys[i]);
		
		// fix issue #1640
		if (codibase.server.util.isMac() && codibase.server.util.isSafari()) {
			$(document).on("afterElementReplace", function(event, componentId) {
				var $component = $("#" + componentId);
		...
		codibase.server.setupAjaxLoadingIndicator();
		codibase.server.form.setupDirtyCheck();
		codibase.server.setupWebsocketCallback();
		codibase.server.mouseState.track();
		codibase.server.ajaxRequests.track();
		codibase.server.setupInputClear();
		codibase.server.setupAlertClose();
		codibase.server.setupModalOverlays();
		codibase.server.setupDropdownToggle();
		codibase.server.setupCheckbox();
		codibase.server.setupRadio();
		codibase.server.setupSwitch();
		codibase.server.setupTable();
		codibase.server.setupTippy();
		codibase.server.history.init(popStateCallback);
		codibase.server.perfectScrollbar.setup();
	...
	onWindowLoad: function() {
		codibase.server.setupAutoSize();
		codibase.server.focus.setupAutoFocus();
		...
		*/
		...
		if (location.hash && !codibase.server.viewState.getFromHistory()) {
