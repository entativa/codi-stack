...
import static io.codibase.server.security.SecurityUtils.*;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!fields.get(2).equals(HookUtils.HOOK_TOKEN)) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN,
                    "Git hook callbacks can only be accessed by CodiBase itself");
            return;
        }
			...
			while (true) {
				...
				if (refName.startsWith(PullRequest.REFS_PREFIX) || refName.startsWith(PullRequestUpdate.REFS_PREFIX)) {
					if (!canManageProject(project)) {
						error(output, refName, Lists.newArrayList("Only project managers can update codibase refs."));
						break;
					}
