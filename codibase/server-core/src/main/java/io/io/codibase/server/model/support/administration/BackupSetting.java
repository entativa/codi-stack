...
public class BackupSetting implements Serializable, Validatable {
	...
	@Editable(order = 100, name = "Backup Schedule", description = "Optionally specify a cron expression to schedule database auto-backup. The cron expression format is " +
			...
			"The backup files will be placed into <em>db-backup</em> folder under CodiBase " +
			"installation directory. In case multiple servers connect to form a cluster, auto-backup takes place on the <a href='https://docs.codibase.io/concepts#lead-server' target='_blank'>lead server</a>. " +
