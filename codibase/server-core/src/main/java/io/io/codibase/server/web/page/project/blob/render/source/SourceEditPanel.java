...
import io.codibase.server.web.page.project.blob.render.edit.BlobEditPanel;
...
public class SourceEditPanel extends BlobEditPanel implements Positionable {
	...
	protected WebMarkupContainer newEditOptions(String componentId) {
		sourceFormat = new SourceFormatPanel(componentId, new OptionChangeCallback() {
			...
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("codibase.server.sourceEdit.onIndentTypeChange('%s');", 
			...
			@Override
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("codibase.server.sourceEdit.onTabSizeChange(%s);", 
			...
			@Override
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("codibase.server.sourceEdit.onLineWrapModeChange('%s');", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.sourceEdit.onWindowLoad('%s', %s);", 
	...
	private String getJson(PlanarRange mark) {
		try {
			return CodiBase.getInstance(ObjectMapper.class).writeValueAsString(mark);
	...
	public void position(AjaxRequestTarget target, String position) {
		...
		if (mark != null) 
			script = String.format("codibase.server.sourceEdit.mark(%s);", getJson(mark));
		else 
			script = String.format("codibase.server.sourceEdit.mark();");
		target.appendJavaScript(script);
	}
