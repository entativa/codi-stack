...
import static io.codibase.agent.job.LogRequest.readLog;
...
public class ServerLogResource extends AbstractResource {
	...
	public static List<String> readServerLog(@Nullable String server) {
		...
		if (server != null) 
			return CodiBase.getInstance(ClusterService.class).runOnServer(server, () -> readLog(new File(installDir, logPath)));
		else 
