...
import io.codibase.server.annotation.BuildQuery;
import io.codibase.server.annotation.CodeCommentQuery;
import io.codibase.server.annotation.CommitQuery;
	...
	public boolean matches(ProjectEvent event) {
		if (event instanceof IssueEvent) {
			if (isIssues()) {
				...
				return io.codibase.server.search.entity.issue.IssueQuery.parse(event.getProject(), getIssueQuery(), option, true)
			...
			if (isBuilds()) {
				return io.codibase.server.search.entity.build.BuildQuery.parse(event.getProject(), getBuildQuery(), false, true)
			...
			if (isPullRequests()) {
				return io.codibase.server.search.entity.pullrequest.PullRequestQuery.parse(event.getProject(), getPullRequestQuery(), false)
			...
			if (isCodePush()) {
				return io.codibase.server.search.commit.CommitQuery.parse(event.getProject(), getCommitQuery(), false)
			...
			if (isCodeComments()) {
				return io.codibase.server.search.entity.codecomment.CodeCommentQuery.parse(event.getProject(), getCodeCommentQuery(), false)
