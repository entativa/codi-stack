...
import io.codibase.server.search.entity.issue.*;
...
import io.codibase.server.web.component.issue.create.CreateIssuePanel;
...
import static io.codibase.server.search.entity.issue.IssueQueryLexer.*;
			...
			protected void onBeforeRender() {
				addOrReplace(cardListPanel = new CardListPanel("body") {
					...
					public void renderHead(IHeaderResponse response) {
						...
						String script = String.format("codibase.server.issueBoards.onColumnDomReady('%s', %s);", 
		...
		if (getColumn() != null) {
			...
			if (identifyField.equals(Issue.NAME_STATE)) {
				...
				if (fieldSpec instanceof ChoiceField) {
					...
					user = CodiBase.getInstance(UserService.class).findByName(getColumn());
				}
			}
		} else {
			FieldSpec fieldSpec = getIssueSetting().getFieldSpec(identifyField);
