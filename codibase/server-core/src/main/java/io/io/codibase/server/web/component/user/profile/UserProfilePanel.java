package io.codibase.server.web.component.user.profile;
...
import io.codibase.server.model.support.issue.changedata.IssueBatchUpdateData;
...
import io.codibase.server.model.support.pullrequest.changedata.PullRequestReopenData;
...
import io.codibase.server.util.DateRange;
import io.codibase.server.util.DateUtils;
import io.codibase.server.web.component.datepicker.DateRangePicker;
...
import io.codibase.server.web.component.user.UserDeleteLink;
import io.codibase.server.web.component.user.profile.activity.ApprovePullRequest;
import io.codibase.server.web.component.user.profile.activity.CommentIssue;
import io.codibase.server.web.component.user.profile.activity.CommentPullRequest;
import io.codibase.server.web.component.user.profile.activity.CommitCode;
import io.codibase.server.web.component.user.profile.activity.CreateCodeComment;
import io.codibase.server.web.component.user.profile.activity.DiscardPullRequest;
import io.codibase.server.web.component.user.profile.activity.MergePullRequest;
import io.codibase.server.web.component.user.profile.activity.OpenIssue;
import io.codibase.server.web.component.user.profile.activity.OpenPullRequest;
import io.codibase.server.web.component.user.profile.activity.ReopenPullRequest;
import io.codibase.server.web.component.user.profile.activity.ReplyCodeComment;
import io.codibase.server.web.component.user.profile.activity.RequestChangesToPullRequest;
import io.codibase.server.web.component.user.profile.activity.ResolveCodeComment;
import io.codibase.server.web.component.user.profile.activity.TransitIssue;
import io.codibase.server.web.component.user.profile.activity.UnresolveCodeComment;
import io.codibase.server.web.component.user.profile.activity.UserActivity;
...
public abstract class UserProfilePanel extends GenericPanel<User> {
    ...
    protected void onInitialize() {
		...
		if (user.isDisabled() && WicketUtils.isSubscriptionActive()) {
			...
			var mailSubject = _T("[Reset Password] Please Reset Your CodiBase Password");
			form.add(new TaskButton("tellUserToResetPassword") {

				@Override
				protected TaskResult runTask(TaskLogger logger) {
					var sessionService = CodiBase.getInstance(SessionService.class);
    ...
    private List<UserActivity> getActivities(Date fromDate, Date toDate) {
        ...
        var commitInfoService = CodiBase.getInstance(CommitInfoService.class);
        ...
        var issueService = CodiBase.getInstance(IssueService.class);
        for (var issue: issueService.query(user, fromDate, toDate)) {
            activities.add(new OpenIssue(issue));
        }

        var issueCommentService = CodiBase.getInstance(IssueCommentService.class);
        for (var comment: issueCommentService.query(user, fromDate, toDate)) {
            activities.add(new CommentIssue(comment));
        }

        var issueChangeService = CodiBase.getInstance(IssueChangeService.class);
        ...
        var pullRequestService = CodiBase.getInstance(PullRequestService.class);
        for (var pullRequest: pullRequestService.query(user, fromDate, toDate)) {
            activities.add(new OpenPullRequest(pullRequest));
        }

        var pullRequestCommentService = CodiBase.getInstance(PullRequestCommentService.class);
        for (var comment: pullRequestCommentService.query(user, fromDate, toDate)) {
            activities.add(new CommentPullRequest(comment));
        }

        var pullRequestChangeService = CodiBase.getInstance(PullRequestChangeService.class);
        ...
        var codeCommentService = CodiBase.getInstance(CodeCommentService.class);
        for (var comment: codeCommentService.query(user, fromDate, toDate)) {
            activities.add(new CreateCodeComment(comment));
        }

        var codeCommentReplyService = CodiBase.getInstance(CodeCommentReplyService.class);
        for (var reply: codeCommentReplyService.query(user, fromDate, toDate)) {
            activities.add(new ReplyCodeComment(reply));
        }

        var codeCommentStatusChangeService = CodiBase.getInstance(CodeCommentStatusChangeService.class);
    ...
    public void renderHead(IHeaderResponse response) {
        ...
        var mapper = CodiBase.getInstance(ObjectMapper.class);        
        try {
            response.render(OnDomReadyHeaderItem.forScript(String.format(
                "codibase.server.userProfile.onDomReady(%s, %d, %d, %s, %s, %s);", 
