package io.codibase.server.web.page.project.pullrequests;
...
public class InvalidPullRequestPage extends ProjectPage {
	...
	public InvalidPullRequestPage(PageParameters params) {
		...
		requestModel = new LoadableDetachableModel<PullRequest>() {
			...
			protected PullRequest load() {
				...
				PullRequest request = CodiBase.getInstance(PullRequestService.class).find(getProject(), requestNumber);
				if (request == null)
	...
	protected void onInitialize() {
		...
		add(new Link<Void>("delete") {
			...
			@Override
			public void onClick() {
				CodiBase.getInstance(PullRequestService.class).delete(getPullRequest());
