codibase.server.pipeline = {
	lineWidth: 2,
	lineColor: codibase.server.isDarkMode()?"#535370":"#D1D3E0",
	onWindowLoad: function(containerId, dependencies, activeJobIndex) {
		...
		for (var jobIndexString in dependencies) {
			...
			for (var i in dependencies[jobIndexString]) {
				...
				codibase.server.pipeline.drawDependencyLine($pipeline, dependencyJobIndex, jobIndex);
			}
		}
		
		if (activeJobIndex)
			codibase.server.pipeline.markJobActive($pipeline, activeJobIndex);
	},
	drawDependencyLine: function($pipeline, dependencyJobIndex, jobIndex) {
		...
		var $dependencyJob = codibase.server.pipeline.getJob($pipeline, dependencyJobIndex);
		var $job = codibase.server.pipeline.getJob($pipeline, jobIndex);
		...
		if (dependencyJobIndex.row == jobIndex.row) {
			if (hasIntermediateJobs(jobIndex.row)) {
				...
				codibase.server.pipeline.drawCurve(paper, dependencyStart, leftCurveStop, dependencyJobIndexString, jobIndexString);

				var rightCurveStart = {
					left: dependencyStop.left - columnSpacing,
					top: leftCurveStop.top
				}				
				codibase.server.pipeline.drawCurve(paper, rightCurveStart, dependencyStop, dependencyJobIndexString, jobIndexString);
				
				paper.line(leftCurveStop.left, leftCurveStop.top, rightCurveStart.left, rightCurveStart.top).attr({
					stroke: codibase.server.pipeline.lineColor,
					strokeWidth: codibase.server.pipeline.lineWidth,
			...
			if (hasIntermediateJobs(dependencyJobIndex.row)) {
				...
				codibase.server.pipeline.drawCurve(paper, curveStart, dependencyStop, dependencyJobIndexString, jobIndexString);
			...
			if (hasIntermediateJobs(jobIndex.row)) {
				...
				codibase.server.pipeline.drawCurve(paper, dependencyStart, curveStop, dependencyJobIndexString, jobIndexString);
		...
		paper.path(
			...
			fill: codibase.server.pipeline.lineColor,
			stroke: codibase.server.pipeline.lineColor,
			strokeWidth: codibase.server.pipeline.lineWidth,
			from: dependencyJobIndexString,
			to: jobIndexString
	...
	markJobActive: function($pipeline, jobIndex) {
		...
		codibase.server.pipeline.getJob($pipeline, jobIndex).addClass("active");
		var jobIndexString = jobIndex.column + "-" + jobIndex.row;
