codibase.server.blobTextDiff = {
	...
	onDomReady: function(containerId, symbolTooltipId, oldRev, newRev, oldPath, newPath, 
		...
		codibase.server.blobTextDiff.translations = translations;
		...
		$container.data("onMouseOverContent", function() {
			if (!codibase.server.mouseState.pressed) {
		...
		codibase.server.blobTextDiff.initBlameTooltip(containerId, $container.find("td.blame>a.hash"));
		...
		$container.selectionPopover("init", function(e) {
	    	...
	    	if (!selection.rangeCount || !codibase.server.mouseState.position) {
	    		return "close";
	    	}
			...
			position = codibase.server.mouseState.position;			

			function showInvalidSelection() {
				var $content = $("<div></div>");
				$content.append(`<a class='invalid'><svg class='icon'><use xlink:href='${codibase.server.icons}#warning'/></svg> ${codibase.server.blobTextDiff.translations["invalid-selection"]}</a>`);
				$content.children("a").attr("href", "https://docs.codibase.io/appendix/diff-selection").attr("target", "_blank");
				return {
					position: position, 
					content: $content
				}
		...
		for (var line in oldCoverages) {
			if (oldCoverages.hasOwnProperty(line)) 
				codibase.server.blobTextDiff.addCoverateInfo($container, true, line, oldCoverages[line]);
		}
		
		let newCoverages = annotationInfo.newAnnotations.coverages;
		for (var line in newCoverages) {
			if (newCoverages.hasOwnProperty(line)) 
				codibase.server.blobTextDiff.addCoverateInfo($container, false, line, newCoverages[line]);
		}
		...
		for (var line in oldProblems) {			
			if (oldProblems.hasOwnProperty(line)) { 
				...
				codibase.server.blobTextDiff.addProblemInfo($container, true, line, oldProblems[line]);
			}
		}
		...
		for (var line in newProblems) {
			if (newProblems.hasOwnProperty(line)) {  
				...
				codibase.server.blobTextDiff.addProblemInfo($container, false, line, newProblems[line]);
			}
		}
		...
		for (var line in oldComments) {
		    if (oldComments.hasOwnProperty(line)) {
		    	...
		    	codibase.server.blobTextDiff.addCommentIndicator($container, true, line, oldCommentsOnLine);
		    }
		}
		...
		for (var line in newComments) {
		    if (newComments.hasOwnProperty(line)) {
		    	...
		    	codibase.server.blobTextDiff.addCommentIndicator($container, false, line, newCommentsOnLine);
		    }
		}
		
		codibase.server.blobTextDiff.highlightCommentTrigger($container);				
		
		if (markRange) {
			$container.data("markRange", markRange);
			codibase.server.blobTextDiff.mark($container, markRange);	
		}			
	},
	onLoad: function(containerId, markRange) {
		var $container = $("#" + containerId);
		if (markRange && codibase.server.viewState.getFromHistory() === undefined)
			codibase.server.blobTextDiff.scrollTo($container, markRange);
		...
		if (!$scrollParent.data("onTextDiffScrollInstalled"))	{
			...
			$scrollParent.doneEvents("scroll", function() {
				$(".blob-text-diff").each(function() {
					codibase.server.blobTextDiff.highlightSyntax($(this));
				}, 100);
			});
		}		
		
		codibase.server.blobTextDiff.highlightSyntax($container);
	},
	highlightSyntax($container) {
		...
		$container.find("tbody, tr.expander").each(function() {
			...
			function doneHighlight() {
				if (oldProcessed && newProcessed) {
					...
					while (true) {
						...
						$codeTr.find(codibase.server.blobTextDiff.symbolClasses).mouseover($container.data("symbolHover"));
			...
			var oldModeInfo = codibase.server.codemirror.findModeByFileName(oldBlobPath);
			if (oldModeInfo) {
				codibase.server.codemirror.highlightSyntax(
					oldLines.join("\n"), 
					oldModeInfo, 
			...
			var newModeInfo = codibase.server.codemirror.findModeByFileName(newBlobPath);
			if (newModeInfo) {
				codibase.server.codemirror.highlightSyntax(
					newLines.join("\n"), 
					newModeInfo, 
	...
	initBlameTooltip: function(containerId, $hashLink) {
		...
		$hashLink.hover(function() {
			...
			var $tooltip = $(`<div class='blame-message'><div class='loading'>${codibase.server.blobTextDiff.translations["loading"]}</div></div>`);
	...
	openSelectionPopover: function(containerId, position, markRange, markUrl, markedText, loggedIn) {
		...
		if (!markUrl) {
			$content = $(`<div><span class='invalid'><svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#warning'/></svg> ${codibase.server.blobTextDiff.translations["unable-to-comment"]}</span></div>`);
		} else {
			var $content = $(`<div><a class='permanent'><svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#link'/></svg> ${codibase.server.blobTextDiff.translations["perma-link"]}</a>`);
			$content.children("a.permanent").attr("href", markUrl);
			$content.append(`<a class='copy-marked'><svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#copy'/></svg> ${codibase.server.blobTextDiff.translations["copy-to-clipboard"]}</a>`);
			...
			if (loggedIn) {
				$content.append(`<a class='comment'><svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#comment'/></svg> ${codibase.server.blobTextDiff.translations["add-selection-comment"]}</a>`);
				$content.children("a.comment").click(function() {
					if (codibase.server.blobTextDiff.confirmUnsavedChanges($container)) {
				...
				$content.append(`<a class='comment' href='${loginHref}'><svg class='icon mr-1'><use xlink:href='${codibase.server.icons}#warning'/></svg> ${codibase.server.blobTextDiff.translations["login-to-comment-on-selection"]}</a>`);
			}			
		}		
	...
	confirmUnsavedChanges: function($container) {
		return $("#"+$container.data("commentContainerId")).find("form.dirty").length == 0 
				|| confirm(codibase.server.blobTextDiff.translations["unsaved-changes-prompt"]); 
	},
	expand: function(containerId, blockIndex, expandedHtml) {
		...
		codibase.server.blobTextDiff.initBlameTooltip(containerId, $expandedTrs.find(">td.blame>a.hash"));
		codibase.server.blobTextDiff.highlightSyntax($container);
	...
	scrollTo: function($container, markRange) {
		var markInfo = codibase.server.blobTextDiff.getMarkInfo($container, markRange);
	...
	mark: function($container, markRangeOrMarkRanges) {
		codibase.server.blobTextDiff.clearMark($container);
	...
	addProblemInfo: function($container, leftSide, line, problems) {
		...
		let $lineNumTd = codibase.server.blobTextDiff.getLineNumTd($container, leftSide, line);
		
		let $trigger = $(document.createElement("a"));
		$trigger.addClass("problem-trigger");
		
		$trigger.addClass(codibase.server.codeProblem.getSeverityInfo(problems));

		$trigger.append(`<svg class='icon icon-sm'><use xlink:href='${codibase.server.icons}#exclamation-circle-o'/></svg>`);
		...
		$trigger.mouseover(function() {
			codibase.server.blobTextDiff.mark($container, markRanges);
		}).mouseout(function() {
			codibase.server.blobTextDiff.restoreMark($container);
		});
		...
		$trigger.mousedown(function() {
			...
			if (!$trigger.data("popoverInited")) {
				$trigger.popover({
					...
					placement: "top", 
					container: $container,
					content: codibase.server.codeProblem.renderProblems(problems, codibase.server.blobTextDiff.translations),
					...
					$currentPopover.find(".problem-content").mouseover(function() {
						codibase.server.blobTextDiff.mark($container, problems[$(this).index()].target.location);
					}).mouseout(function() {
						codibase.server.blobTextDiff.restoreMark($container);
					}).each(function() {
	...
	addCoverateInfo: function($container, leftSide, line, coverageStatus) {
		var $lineNumTd = codibase.server.blobTextDiff.getLineNumTd($container, leftSide, line);
		var cssClass = coverageStatus.toLowerCase();
		var title;
		if (coverageStatus == 'COVERED') 
			title = codibase.server.blobTextDiff.translations["covered-by-tests"]; 
		else if (coverageStatus == 'NOT_COVERED')
			title = codibase.server.blobTextDiff.translations["not-covered-by-any-test"];
		else  
			title = codibase.server.blobTextDiff.translations["partially-covered-by-some-tests"];
	...
	addCommentIndicator: function($container, leftSide, line, comments) {
		...
		if (comments.length != 1) {
			...
			for (var i in comments) { 
				...
				content += `<a class='${cssClasses}' data-tippy-content='${codibase.server.blobTextDiff.translations["show-comment"]}'>#${comments[i].id}</a>`;
			}

			if (updated)
				$indicator.addClass("updated");
			
			$indicator.append(`<svg class='icon'><use xlink:href='${codibase.server.icons}#comments'/></svg>`);
			...
			$indicator.on('shown.bs.popover', function () {
				...
				$currentPopover.find("a").each(function() {
					$(this).mouseover(function() {
						...
						codibase.server.blobTextDiff.mark($container, comment.range);
					});
					$(this).mouseout(function() {
						codibase.server.blobTextDiff.restoreMark($container);
					});
					$(this).click(function() {
						if (!$(this).hasClass("active") && codibase.server.blobTextDiff.confirmUnsavedChanges($container)) {
							var comment = comments[$(this).index()];			        						
			...
			if (comment.updated)
				$indicator.addClass("updated");
			$indicator.addClass("comment-trigger").attr("data-tippy-content", codibase.server.blobTextDiff.translations["show-comment"]);
			$indicator.append("<svg class='icon'><use xlink:href='" + codibase.server.icons + "#comment'/></svg>");
			...
			$indicator.click(function() {
				if (!$indicator.hasClass("active") && codibase.server.blobTextDiff.confirmUnsavedChanges($container)) {
					callback("openComment", comment.id, comment.range.leftSide, comment.range.fromRow, 
							comment.range.fromColumn, comment.range.toRow, comment.range.toColumn);
				}
			});
	...
	highlightCommentTrigger: function($container) {
		...
		if (openComment) {
			...
			var $indicator = codibase.server.blobTextDiff.getLineNumTd($container, openComment.range.leftSide, line).children(".comment-indicator");
	...
	onCommentAdded: function($container, comment) {
		...
		var $indicator = codibase.server.blobTextDiff.getLineNumTd($container, leftSide, line).children(".comment-indicator");
		var comments;
		if ($indicator.length != 0) {
		...
		codibase.server.blobTextDiff.addCommentIndicator($container, leftSide, line, comments);
		codibase.server.blobTextDiff.highlightCommentTrigger($container);				
	},
	...
	onCommentOpened: function($container, comment) {
		...
		codibase.server.blobTextDiff.highlightCommentTrigger($container);
		codibase.server.blobTextDiff.mark($container, comment.range);
		codibase.server.blobTextDiff.scrollIntoView($container, comment.range);
	},
	...
	onAddComment: function($container, markRange) {
		...
		setTimeout(function() {
			codibase.server.blobTextDiff.highlightCommentTrigger($container);
			codibase.server.blobTextDiff.mark($container, markRange);
			codibase.server.blobTextDiff.scrollIntoView($container, markRange);
		}, 100);
	}
};
