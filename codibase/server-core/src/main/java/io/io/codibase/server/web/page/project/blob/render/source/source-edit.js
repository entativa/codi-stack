codibase.server.sourceEdit = {
	onDomReady: function(containerId, filePath, mark, indentType, tabSize, lineWrapMode, autoFocus) {
		...
		if (mark)
			codibase.server.codemirror.mark(cm, mark);
		...
		$code.on("setViewState", function(e, viewState) {
			codibase.server.codemirror.setViewState(cm, viewState);
	    });
	...
	onWindowLoad: function(containerId, mark) {
		...
		if (mark && codibase.server.viewState.getFromHistory() === undefined 
				&& codibase.server.viewState.carryOver === undefined) {
			codibase.server.codemirror.scrollTo(cm, mark);					
		}
	},
	mark: function(mark) {
		...
		if (mark) {
			...
			if (newBeginLine!=undefined && newEndLine!=undefined) {
				...
				codibase.server.codemirror.mark(cm, newMark);
				codibase.server.codemirror.scrollTo(cm, newMark);
			}			
		} else {
