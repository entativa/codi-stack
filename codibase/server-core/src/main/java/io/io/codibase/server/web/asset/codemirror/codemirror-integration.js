codibase.server.codemirror = {
	...
	highlightSyntax: function(text, modeInfo, highlighted, startCallback, stopCallback) {
		var modeMime = codibase.server.codemirror.getModeMime(modeInfo);
	...
	findModeByFileName: function(fileName) {
		...
		else if (fileName == ".codibase-buildspec") 
	...
	setModeByName: function(cm, modeName) {
        ...
        if (modeInfo) 
            codibase.server.codemirror.setMode(cm, modeInfo);
	},
	setModeByFileName: function(cm, fileName) {
		var modeInfo = codibase.server.codemirror.findModeByFileName(fileName);
		if (modeInfo)
			codibase.server.codemirror.setMode(cm, modeInfo);
	},
...
CodeMirror.modeURL = '/wicket/resource/io.codibase.server.web.asset.codemirror.codemirrorresourcereference/mode/%N/%N.js';