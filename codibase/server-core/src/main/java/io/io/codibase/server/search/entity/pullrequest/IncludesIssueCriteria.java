...
public class IncludesIssueCriteria extends Criteria<PullRequest> {
	...
	private Collection<Long> getPullRequestIds(Project project) {
		Collection<Long> pullRequestIds = new HashSet<>();
		for (ObjectId commit: CodiBase.getInstance(CommitInfoService.class).getFixCommits(project.getId(), issue.getId(), false))
			pullRequestIds.addAll(CodiBase.getInstance(PullRequestInfoService.class).getPullRequestIds(project, commit));
