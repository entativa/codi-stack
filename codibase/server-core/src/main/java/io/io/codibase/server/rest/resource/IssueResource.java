...
import io.codibase.server.web.page.help.ApiHelpUtils;
import io.codibase.server.web.page.help.ValueInfo;

@Api(description="In most cases, issue resource is operated with issue id, which is different from issue number. "
		+ "To get issue id of a particular issue number, use the <a href='/~help/api/io.codibase.server.rest.IssueResource/queryBasicInfo'>Query Basic Info</a> operation with query for "
...
public class IssueResource {
	...
	private static List<Map<String, Object>> getIssuesExample() {
		...
		issues.add(CodiBase.getInstance(ObjectMapper.class).convertValue(issue, new TypeReference<Map<String, Object>>() {}));
		return issues;
	}
