...
import io.codibase.server.web.page.project.blob.render.edit.plain.PlainEditSupport;
...
public abstract class BlobEditPanel extends Panel {
					...
					protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
						...
						if (currentTab == Tab.EDIT 
							...
							String script = String.format(
									"codibase.server.blobEdit.selectTab($('#%s>.blob-edit>.head>.%s'));", 
									BlobEditPanel.this.getMarkupId(true), tab.name().toLowerCase().replace("_", "-"));
							target.appendJavaScript(script);
							
							currentTab = tab;

							target.prependJavaScript(String.format("codibase.server.blobEdit.recordFormFlags('%s');", form.getMarkupId()));
							target.add(form);
							target.appendJavaScript(String.format("codibase.server.blobEdit.restoreFormFlags('%s');", form.getMarkupId()));
						}
					}
	...
	public void onEvent(IEvent<?> event) {
		...
		if (event.getPayload() instanceof BlobNameChanging) {
			...
			String script = String.format("codibase.server.blobEdit.onNameChanging('%s', %s, %s);", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("codibase.server.blobEdit.onDomReady('%s');", getMarkupId()); 
