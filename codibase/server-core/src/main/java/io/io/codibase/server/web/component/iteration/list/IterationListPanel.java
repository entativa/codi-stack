package io.codibase.server.web.component.iteration.list;
...
import io.codibase.server.util.IterationAndIssueState;
import io.codibase.server.util.IterationSort;
...
import io.codibase.server.web.component.iteration.IterationDateLabel;
import io.codibase.server.web.component.iteration.actions.IterationActionsPanel;
	...
	private final IModel<Collection<IterationAndIssueState>> iterationAndStatesModel =
			new LoadableDetachableModel<>() {
				...
				protected Collection<IterationAndIssueState> load() {
					...
					return CodiBase.getInstance(IssueService.class).queryIterationAndIssueStates(getProject(), iterations);
				}
	...
	protected void onInitialize() {
		...
		SortableDataProvider<Iteration, Void> dataProvider = new LoadableDetachableDataProvider<>() {
			...
			public Iterator<? extends Iteration> iterator(long first, long count) {
				...
				return CodiBase.getInstance(Dao.class).query(criteria, (int) first, (int) count).iterator();
			}

			@Override
			public long calcSize() {
				return CodiBase.getInstance(Dao.class).count(getCriteria(closed));
			}
			...
			public IModel<Iteration> model(Iteration object) {
				...
				return new LoadableDetachableModel<>() {
					...
					protected Iteration load() {
						return CodiBase.getInstance(IterationService.class).load(id);
					}
