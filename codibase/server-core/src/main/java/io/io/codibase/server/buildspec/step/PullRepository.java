...
public class PullRepository extends SyncRepository {
	...
	@Editable(order=650, name="Access Token for Target Project", description = "Specify a <a href='https://docs.codibase.io/tutorials/cicd/job-secrets' target='_blank'>job secret</a> " +
	...
	private static String getLfsDescription() {
		if (!Bootstrap.isInDocker()) {
			return _T("If this option is enabled, git lfs command needs to be installed on CodiBase server "
	...
	private static class PullTask implements ClusterTask<Void> {
		...
		public Void call() throws Exception {
			...
			try {
				...
				CodiBase.getInstance(SessionService.class).runAsync(() -> {
					try {
						...
						ListenerRegistry registry = CodiBase.getInstance(ListenerRegistry.class);
