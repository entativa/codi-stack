package io.codibase.server.web.editable.groupchoice;
...
import io.codibase.server.web.component.groupchoice.GroupSingleChoice;
	...
	protected void onInitialize() {
		...
		try {
			...
			if (groupChoice.value().length() != 0) {
				...
				choices.addAll(CodiBase.getInstance(GroupService.class).query());
		...
		if (getModelObject() != null)
			group = CodiBase.getInstance(GroupService.class).find(getModelObject());
		else
			group = null;
