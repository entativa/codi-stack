...
Project default role is replaced with [base authorization](~help/api/io.codibase.server.rest.resource.BaseAuthorizationResource) to support multiple default role assignment. The defaultRoleId field should be removed when create/update the project resource. 
...
1. Format of contributed project settings inside project settings (refer to endpoint to [get](/~help/api/io.codibase.server.rest.resource.ProjectResource/getSetting) and [set](/~help/api/io.codibase.server.rest.resource.ProjectResource/updateSetting)
project settings) has been changed
1. Format of contributed administration settings (refer to endpoint to [get](/~help/api/io.codibase.server.rest.resource.SettingResource/getContributedSettings) and [set](/~help/api/io.codibase.server.rest.resource.SettingResource/setContributedSettings)
...
If you are using SQL Server, please [switch to use MySQL/PostgreSQL/MariaDB](https://docs.codibase.io/administration-guide/switch-database)
...
private docker registries, please follow [this tutorial](https://docs.codibase.io/tutorials/cicd/insecure-docker-registry) to configure the builder
...
CodiBase now stores job caches on server instead of local disk for better manageability and 
sharing. As result of this, the cache settings defined at job level are removed. One should 
use _Set Up Cache_ step now instead. Check out <a href='https://docs.codibase.io/tutorials/cicd/job-cache'>this tutorial</a> 
to get familiar with the new cache mechanism

### RESTful API

The operation to [query repository commits](/~help/api/io.codibase.server.rest.resource.RepositoryResource/queryCommits) 
and [get repository commit](/~help/api/io.codibase.server.rest.resource.RepositoryResource/getCommit) now return commit 
...
CodiBase now relies on buildx to build docker image. Make sure to update agent image by running 
...
Since 9.0, CodiBase introduces [enterprise edition](https://codibase.io/pricing) which requires a paid subscription to use, 
and now it is distributed under [new license](https://codibase.io/license-agreement). In short, the _server-ee_ folder is 
...
1. CodiBase only supports to run as statefulsets now. Refer to the [upgrade guide](https://docs.codibase.io/upgrade-guide/deploy-to-k8s) to migrate the persistent volume
2. Various database settings are changed to remove the _db_ prefix. For instance, _.Values.database.dbType_ was changed to _.Values.database.type_. Refer to [values.yaml](https://code.codibase.io/codibase/server/~files/main/server-product/helm/values.yaml) for details

### RESTful api 

The operation to [get issue fields](/~help/api/io.codibase.server.rest.IssueResource/getFields) now return a map of field name to value  

# 8.6.0

### Database 

Oracle support is removed as it is hard to test with latest version. If you happen to use this database, please back up data and restore to a supported database following [this guide]((https://docs.codibase.io/administration-guide/backup-restore)) 

# 8.5.0

### Helm Chart

Helm chart has been completely rewritten to be flexible, and it is not compatible with earlier versions. Follow [upgrade guide](https://docs.codibase.io/upgrade-guide/deploy-to-k8s) to upgrade your chart from version <= 8.4.2
...
1. The endpoint to create agent token now do not require any request body. It will generate new token value at CodiBase side
...
To use consistent project url for web and git access, CodiBase now uses project path instead of project id to access projects via web UI, for instance `https://code.codibase.io/projects/160` has been changed to `https://code.codibase.io/codibase/server`. To avoid conflicting with this url scheme, 
...
2. SSO callback url now takes the form `https://<codibase root url>/~sso/callback/<Provider Name>` (use `~sso` instead of `sso`)
3. If you are using OAuth based Office365 or Gmail mail service, make sure to change redirect url as `https://<codibase root url>/~oauth/callback` (use `~oauth` instead of `oauth`)
...
5. If you are setting up reverse proxy using Apache or Nginx, make sure to change proxied path `/server` to `/~server`. Check [the docs](https://docs.codibase.io/administration-guide/reverse-proxy-setup) for details

# 7.7.0

CodiBase server and agent now requires Java 11 or higher. Follow below steps to upgrade agent manually:
...
1. A boolean property "confidential" must be added to create endpoint of [issue resource](/~help/api/io.codibase.server.rest.IssueResource/create) to indicate whether or not the issue is confidential

# 7.3.5
1. [RESTful api] Service desk name property of [project resource](/~help/api/io.codibase.server.rest.ProjectResource) moved from basic info endpoint to setting endpoint
...
1. [RESTful api] Email addresses of a user should now be retrieved via [UserResource.getEmailAddresses](/~help/api/io.codibase.server.rest.UserResource/getEmailAddresses), and should be operated via [EmailAddressResource](/~help/api/io.codibase.server.rest.EmailAddressResource)
2. [RESTful api] Access token of a user should now be retrieved via [UserResource.getAccessToken](/~help/api/io.codibase.server.rest.UserResource/getAccessToken)
3. [RESTful api] User query by login name, full name and email should now be done via [UserResource.queryProfile](/~help/api/io.codibase.server.rest.UserResource/queryProfile)
...
1. In case install CodiBase into a Kubernetes cluster, Kustomization based deployment is replaced by helm based deployment for flexibility reason
...
2. URL of project is changed from *http(s)://\<codibase-server\>/projects/\<project name\>* to *http(s)://\<codibase-server\>/projects/\<project id\>*. This only affects web UI, clone url and REStful api url is not changed.
...
1. Submodule authentication
  ...
  You will need to use custom http/ssh clone credential with permission to access submodule projects to retrieve source. Refer to [tutorial](https://docs.codibase.io/tutorials/cicd/clone-submodules-ssh/) for an example.
