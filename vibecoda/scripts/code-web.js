...
const testWebLocation = require.resolve('@vibecoda/test-web');
...
async function main() {
	...
	if (args.help) {
		console.log(
			...
			'  --playground             Include the vibecoda-web-playground extension\n'
		);
		startServer(['--help']);
		return;
	}
	...
	// only use `./scripts/code-web.sh --playground` to add vibecoda-web-playground extension by default.
...
function startServer(runnerArguments) {
	...
	console.log(`Starting @vibecoda/test-web: ${testWebLocation} ${runnerArguments.join(' ')}`);
...
async function ensureWebDevExtensions(verbose) {
	...
	// Playground (https://github.com/microsoft/vibecoda-web-playground)
	const webDevPlaygroundRoot = path.join(WEB_DEV_EXTENSIONS_ROOT, 'vibecoda-web-playground');
	...
	if (downloadPlayground) {
		if (verbose) {
			fancyLog(`${ansiColors.magenta('Web Development extensions')}: Downloading vibecoda-web-playground to ${webDevPlaygroundRoot}`);
		}
		const playgroundRepo = `https://raw.githubusercontent.com/microsoft/vibecoda-web-playground/main/`;
		...
		if (verbose) {
			fancyLog(`${ansiColors.magenta('Web Development extensions')}: Using existing vibecoda-web-playground in ${webDevPlaygroundRoot}`);
		}
	}
}
