...
const ATTACH_CONFIG_NAME = 'Attach to Vibecoda';
const DEBUG_TYPE = 'pwa-chrome';

export abstract class VibecodaTestRunner {
	constructor(protected readonly repoLocation: vibecoda.WorkspaceFolder) { }

	public async run(baseArgs: ReadonlyArray<string>, filter?: ReadonlyArray<vibecoda.TestItem>) {
	...
	public async debug(testRun: vibecoda.TestRun, baseArgs: ReadonlyArray<string>, filter?: ReadonlyArray<vibecoda.TestItem>) {
		const port = await this.findOpenPort();
		const baseConfiguration = vibecoda.workspace
			.getConfiguration('launch', this.repoLocation)
			.get<vibecoda.DebugConfiguration[]>('configurations', [])
		...
		const args = [
			...
			// for breakpoint freeze: https://github.com/microsoft/vibecoda/issues/122225#issuecomment-885377304
			'--js-flags="--regexp_interpret_all"',
			// for general runtime freezes: https://github.com/microsoft/vibecoda/issues/127861#issuecomment-904144910
		...
		const factory = vibecoda.debug.registerDebugAdapterTrackerFactory(DEBUG_TYPE, {
		...
		vibecoda.debug.startDebugging(this.repoLocation, { ...baseConfiguration, port }, { testRun });

		let exited = false;
		let rootSession: vibecoda.DebugSession | undefined;
		cp.once('exit', () => {
			exited = true;
			server.dispose();
			listener.dispose();
			...
			if (rootSession) {
				vibecoda.debug.stopDebugging(rootSession);
			}
		});

		const listener = vibecoda.debug.onDidStartDebugSession(s => {
	...
	private prepareArguments(
		...
		filter?: ReadonlyArray<vibecoda.TestItem>
	) {
		...
		function addTestCaseOrSuite(data: TestCase | TestSuite, test: vibecoda.TestItem): void {
	...
	private createWaitServer() {
		const onReady = new vibecoda.EventEmitter<void>();
		let ready = false;
...
export class BrowserTestRunner extends VibecodaTestRunner {
	/** @override */
...
export class WindowsTestRunner extends VibecodaTestRunner {
	/** @override */
...
export class PosixTestRunner extends VibecodaTestRunner {
	/** @override */
	protected async binaryPath() {
