...
interface ITrackedRemediation {
	snapshot: vibecoda.TestResultSnapshot;
	failing: IGitState;
	passing: IGitState;
}
...
export class FailureTracker {
	...
	private readonly lastFailed = new Map<
		string,
		{ snapshot: vibecoda.TestResultSnapshot; failing: IGitState }
	>();
	...
	constructor(storageLocation: string, private readonly rootDir: string) {
		this.logFile = join(storageLocation, '.build/vibecoda-test-failures.json');
		mkdirSync(dirname(this.logFile), { recursive: true });

		const oldLogFile = join(rootDir, '.build/vibecoda-test-failures.json');
		...
		this.disposables.push(
			vibecoda.commands.registerCommand('selfhost-test-provider.openFailureLog', async () => {
				const doc = await vibecoda.workspace.openTextDocument(this.logFile);
				await vibecoda.window.showTextDocument(doc);
			})
		);

		this.disposables.push(
			vibecoda.tests.onDidChangeTestResults(() => {
				const last = vibecoda.tests.testResults[0];
				if (!last) {
					return;
				}
				...
				for (let i = 0; i < queue.length; i++) {
					for (const snapshot of queue[i]) {
						...
						if (snapshot.taskStates.some(s => s.state === vibecoda.TestResultState.Failed)) {
