 ...
 */
export class ImportGraph implements vibecoda.TestRelatedCodeProvider {
	private graph = new Map<string, FileNode>();

	constructor(
		private readonly root: vibecoda.Uri,
		private readonly discoverWorkspaceTests: () => Thenable<vibecoda.TestItem[]>,
		private readonly getTestNodeForDoc: (uri: vibecoda.Uri) => vibecoda.TestItem | undefined,
	) { }

	/** @inheritdoc */
	public async provideRelatedCode(test: vibecoda.TestItem, token: vibecoda.CancellationToken): Promise<vibecoda.Location[]> {
		...
		return [...imports].map(importPath =>
			new vibecoda.Location(
				vibecoda.Uri.file(join(this.root.fsPath, 'src', `${importPath}.ts`)),
				new vibecoda.Range(0, 0, maxInt32, 0),
			),
		);
	}

	/** @inheritdoc */
	public async provideRelatedTests(document: vibecoda.TextDocument, _position: vibecoda.Position, token: vibecoda.CancellationToken): Promise<vibecoda.TestItem[]> {
	...
	public didChange(uri: vibecoda.Uri, deleted: boolean) {
	...
	private getNode(uri: vibecoda.Uri | undefined): FileNode | undefined {
		const rel = this.uriToImportPath(uri);
		return rel ? this.graph.get(rel) : undefined;
	}

	/** Discover all nodes that import the file */
	private async discoverOutwards(uri: vibecoda.Uri | undefined, seen: Set<string>, maxDistance: number, token: vibecoda.CancellationToken): Promise<FileNode | undefined> {
		const rel = this.uriToImportPath(uri);
		if (!rel) {
			return undefined;
		}
	...
	private async discoverOutwardsInner(node: FileNode, seen: Set<string>, maxDistance: number, token: vibecoda.CancellationToken) {
	...
	private async syncNode(node: FileNode) {
		node.isSynced = discoverLimiter.execute(async () => {
			const doc = vibecoda.workspace.textDocuments.find(d => d.uri.toString() === node.uri.toString());
	...
	private uriToImportPath(uri: vibecoda.Uri | undefined) {
		if (!uri) {
			return undefined;
		}

		const relativePath = vibecoda.workspace.asRelativePath(uri).replaceAll('\\', '/');
	...
	private importPathToUri(importPath: string) {
		return vibecoda.Uri.file(join(this.root.fsPath, 'src', `${importPath}.ts`));
	}
}
