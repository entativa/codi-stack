...
export class TestOutputScanner implements vibecoda.Disposable {
	protected mochaEventEmitter = new vibecoda.EventEmitter<MochaEventTuple>();
	protected outputEventEmitter = new vibecoda.EventEmitter<string>();
	protected onExitEmitter = new vibecoda.EventEmitter<string | undefined>();
...
type QueuedOutput = string | [string, vibecoda.Location | undefined, vibecoda.TestItem | undefined];

export async function scanTestOutput(
	tests: Map<string, vibecoda.TestItem>,
	task: vibecoda.TestRun,
	scanner: TestOutputScanner,
	coverageDir: string | undefined,
	cancellation: vibecoda.CancellationToken
): Promise<void> {
	...
	let lastTest: vibecoda.TestItem | undefined;
	...
	try {
		...
		await new Promise<void>(resolve => {
			...
			let currentTest: vibecoda.TestItem | undefined;
					...
					case MochaEvent.Fail:
						{
							...
							const testFirstLine =
								tcase.range &&
								new vibecoda.Location(
									tcase.uri!,
									new vibecoda.Range(
										tcase.range.start,
										new vibecoda.Position(tcase.range.start.line, 100)
									)
								);
							...
							enqueueExitBlocker(
								(async () => {
									...
									let message: vibecoda.TestMessage;

									if (hasDiff) {
										message = new vibecoda.TestMessage(tryMakeMarkdown(err));
										...
										message = new vibecoda.TestMessage(
		...
		if (!ranAnyTest) {
			await vibecoda.commands.executeCommand('testing.showMostRecentOutput');
		}
	} catch (e) {
...
const tryMakeMarkdown = (message: string) => {
	...
	lines.push('```');
	return new vibecoda.MarkdownString(lines.join('\n'));
};
...
export type SourceLocationMapper = (line: number, col: number, strategy: SearchStrategy) => vibecoda.Location | undefined;
...
export class SourceMapStore {
	...
	async getSourceLocationMapper(fileUri: string): Promise<SourceLocationMapper> {
		...
		return (line, col, strategy) => {
			...
			if (idealPosition.line !== null && idealPosition.column !== null && idealPosition.source !== null) {
				return new vibecoda.Location(
					this.completeSourceMapUrl(sourceMap, idealPosition.source),
					new vibecoda.Position(idealPosition.line - 1, idealPosition.column)
				);
			}
			...
			do {
				...
				return new vibecoda.Location(
					...
					new vibecoda.Position(segment[MapField.SOURCE_LINE] - 1, segment[MapField.SOURCE_COLUMN])
				);
		...
		for (const bias of sourceMapBiases) {
			...
			if (position.line !== null && position.column !== null && position.source !== null) {
				return new vibecoda.Location(
					...
					new vibecoda.Position(position.line - 1, position.column)
				);
			}
		}
	...
	private completeSourceMapUrl(sm: TraceMap, source: string) {
		if (sm.sourceRoot) {
			try {
				return vibecoda.Uri.parse(new URL(source, sm.sourceRoot).toString());
			} catch {
				// ignored
			}
		}

		return vibecoda.Uri.parse(source);
	}
	...
	private loadSourceMap(fileUri: string) {
		...
		const promise = (async () => {
			try {
				const contents = await getContentFromFilesystem(vibecoda.Uri.parse(fileUri));
...
async function deriveStackLocations(
	...
	const locationsMapped = await Promise.all(locationsRaw.map(async location => {
		...
		const stack = new vibecoda.TestMessageStackFrame(location.label || '<anonymous>', mapped?.uri, mapped?.range.start || new vibecoda.Position(location.lineBase1 - 1, location.columnBase1 - 1));
		return { location: mapped, stack };
	}));

	let best: undefined | { location: vibecoda.Location; score: number };
