...
declare module 'vibecoda' {
	...
	export class RelativePattern {
		 ...
		 * Example:
		 * ```ts
		 * const folder = vibecoda.workspace.workspaceFolders?.[0];
		 * if (folder) {
		 *
		 *   // Match any TypeScript file in the root of this workspace folder
		 *   const pattern1 = new vibecoda.RelativePattern(folder, '*.ts');
		 *
		 *   // Match any TypeScript file in `someFolder` inside this workspace folder
		 *   const pattern2 = new vibecoda.RelativePattern(folder, 'someFolder/*.ts');
		 * }
		 * ```
		 *
	...
	export class MarkdownString {
		 ...
		 * and attributes to be rendered. See https://github.com/microsoft/vibecoda/blob/6d2920473c6f13759c978dd89104c4270a83422d/src/vs/base/browser/markdownRenderer.ts#L296
		 ...
		 *
		 * ```ts
		 * const md = new vibecoda.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = vibecoda.Uri.file('/path/to/dir/');
		 ...
		 * ```ts
		 * const md = new vibecoda.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = vibecoda.Uri.file('/path/to/otherFile.js');
	 ...
	 * const config = workspace.getConfiguration('launch', vibecoda.workspace.workspaceFolders[0].uri);
	 *
	 * // retrieve values
	...
	export interface Terminal {
		 ...
		 *   	vibecoda.window.showInformationMessage(`Exit code: ${t.exitStatus.code}`);
		 *   }
		 * });
		 * ```
		 */
	...
	export interface ExtensionContext {
		 ...
		 * {@linkcode ExtensionContext.extensionUri extensionUri}, e.g. `vibecoda.Uri.joinPath(context.extensionUri, relativePath);`
		 *
	 ...
	 * a file or folder doesn't exist, use them like so: `throw vibecoda.FileSystemError.FileNotFound(someUri);`
	 */
	...
	export interface Webview {
		 ...
		 * ```html
		 * <script>
		 *     const vibecoda = acquireVsCodeApi(); // acquireVsCodeApi can only be invoked once
		 *     vibecoda.postMessage({ message: 'hello!' });
		 * </script>
		 * ```
		 *
		 ...
		 * setting a [content security policy](https://aka.ms/vibecoda-api-webview-csp).
		 */
		html: string;
		 ...
		 *
		 *   For older versions of vibecoda, if an `ArrayBuffer` is included in `message`,
		 ...
		 *
		 *   However if your extension targets vibecoda 1.57+ in the `engines` field of its
		 ...
		 *
		 * ```ts
		 * webview.html = `<img src="${webview.asWebviewUri(vibecoda.Uri.file('/Users/codey/workspace/cat.gif'))}">`
		 * ```
		 */
	...
	/**
	 * Restore webview panels that have been persisted when vibecoda shuts down.
	 *
	 ...
	 * ```js
	 * // Within the webview
	 * const vibecoda = acquireVsCodeApi();
	 *
	 * // Get existing state
	 * const oldState = vibecoda.getState() || { value: 0 };
	 *
	 * // Update state
	...
	export namespace env {
		...
		/**
		 * The application name of the editor, like 'Vibecoda'.
		 */
		 ...
		 * * Vibecoda itself (`vibecoda:` from `vibecoda.env.uriScheme`)
		 *
		 ...
		 * ```typescript
		 * vibecoda.window.registerUriHandler({
		 *   handleUri(uri: vibecoda.Uri): vibecoda.ProviderResult<void> {
		 ...
		 * });
		 *
		 * const callableUri = await vibecoda.env.asExternalUri(vibecoda.Uri.parse(vibecoda.env.uriScheme + '://my.extension/did-authenticate'));
		 * await vibecoda.env.openExternal(callableUri);
		 * ```
		 *
	...
	export namespace window {
		 ...
		 * the following rules:
		 *
		 * - The uri-scheme must be `vibecoda.env.uriScheme`;
	...
	export class TreeItem {
		 ...
		 *
		 * Please use `vibecoda.open` or `vibecoda.diff` as command IDs when the tree item is opening
	...
	export interface Pseudoterminal {
		 ...
		 * ```typescript
		 * const writeEmitter = new vibecoda.EventEmitter<string>();
		 * const pty: vibecoda.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 *   open: () => writeEmitter.fire('\x1b[31mHello world\x1b[0m'),
		 *   close: () => {}
		 * };
		 * vibecoda.window.createTerminal({ name: 'My terminal', pty });
		 * ```
		 *
		 ...
		 * ```typescript
		 * const dimensionsEmitter = new vibecoda.EventEmitter<vibecoda.TerminalDimensions>();
		 * const pty: vibecoda.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * ```typescript
		 * const writeEmitter = new vibecoda.EventEmitter<string>();
		 * const closeEmitter = new vibecoda.EventEmitter<void>();
		 * const pty: vibecoda.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 *   handleInput: data => {
		 *     if (data !== 'y') {
		 *       vibecoda.window.showInformationMessage('Something went wrong');
		 *     }
		 *     closeEmitter.fire();
		 *   }
		 * };
		 * const terminal = vibecoda.window.createTerminal({ name: 'Exit example', pty });
		 * terminal.show(true);
		 * ```
		 */
		 ...
		 * ```typescript
		 * const writeEmitter = new vibecoda.EventEmitter<string>();
		 * const changeNameEmitter = new vibecoda.EventEmitter<string>();
		 * const pty: vibecoda.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * };
		 * vibecoda.window.createTerminal({ name: 'Local echo', pty });
		 * ```
		 */
	...
	export namespace workspace {
		 ...
		 * files, e.g. `vibecoda.workspace.fs.readDirectory(someUri)` allows to retrieve all entries
		 * of a directory or `vibecoda.workspace.fs.stat(anotherUri)` returns the meta data for a
		 * file.
		 */
		 ...
		 *
		 * The location can e.g. be used with the `vibecoda.openFolder` command to
		 * open the workspace again after it has been closed.
		 *
		 * **Example:**
		 * ```typescript
		 * vibecoda.commands.executeCommand('vibecoda.openFolder', uriOfWorkspace);
		 * ```
		 *
		 * Refer to https://code.visualstudio.com/docs/editor/workspaces for more information on
		 * the concept of workspaces.
		 ...
		 * by an optional set of `workspaceFoldersToAdd` on the `vibecoda.workspace.workspaceFolders` array. This "splice"
		 ...
		 *
		 * ```ts
		 * const watcher = vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(<folder>, <pattern>));
		 *
		 ...
		 *
		 * ```ts
		 * vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(vibecoda.workspace.workspaceFolders[0], '**​/*.js'));
		 * ```
		 *
		 * If you want to monitor file events across all opened workspace folders:
		 *
		 * ```ts
		 * vibecoda.workspace.createFileSystemWatcher('**​/*.js');
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(vibecoda.Uri.file(<path to folder outside workspace>), '*.js'));
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(vibecoda.window.activeTextEditor.document.uri, '*'));
		 * ```
		 *
	...
	export namespace languages {
		 ...
		 * doc.uri; // `vibecoda-notebook-cell:///my/notebook.ipynb#gl65s2pmha`;
	...
	export class NotebookCellOutput {
		 ...
		 * 	vibecoda.NotebookCellOutputItem.text('Hello', 'text/plain'),
		 * 	vibecoda.NotebookCellOutputItem.text('<i>Hello</i>', 'text/html'),
		 * 	vibecoda.NotebookCellOutputItem.text('_Hello_', 'text/markdown'),
		 * 	vibecoda.NotebookCellOutputItem.text('Hey', 'text/plain'), // INVALID: repeated type, editor will pick just one
		 * ])
		 * ```
		 */
	...
	export namespace debug {
		 ...
		 * The named configurations are looked up in '.vibecoda/launch.json' found in the given folder.
	 ...
	 * [vibecoda-l10n repo](https://github.com/microsoft/vibecoda-l10n).
	 *
	 ...
	 *
	 * ```js
	 * const sender: vibecoda.TelemetrySender = {...};
	 * const logger = vibecoda.env.createTelemetryLogger(sender);
	 *
	 ...
	 * failure causes, like `if(someError.code === vibecoda.LanguageModelError.NotFound.name) {...}`
	...
	export namespace lm {
		 ...
		 *
		 * ```ts
		 * const models = await vibecoda.lm.selectChatModels({ family: 'gpt-3.5-turbo' });
		 * if (models.length > 0) {
		 * 	const [first] = models;
		 ...
		 * {@link LanguageModelPromptTsxPart prompt-tsx}-parts. If the tool caller is using `@vibecoda/prompt-tsx`, it can
	...
	/**
	 * A language model response part containing a PromptElementJSON from `@vibecoda/prompt-tsx`.
	...
	export class LanguageModelPromptTsxPart {
		 ...
		 * @param value The value of the part, the result of `renderElementJSON` from `@vibecoda/prompt-tsx`.
		 */
		constructor(value: unknown);
	}

	/**
	 * A result returned from a tool invocation. If using `@vibecoda/prompt-tsx`, this result may be rendered using a `ToolResult`.
	 */
