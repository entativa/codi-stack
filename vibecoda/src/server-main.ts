...
(globalThis as { vibecodaServerStartTime?: number }).vibecodaServerStartTime = performance.now();
...
if (shouldSpawnCli) {
	...
	server.listen(nodeListenOptions, async () => {
		...
		// Do not change this line. Vibecoda looks for this in the output.
		output += `Extension host agent listening on ${typeof address === 'string' ? address : address.port}\n`;
		console.log(output);

		perf.mark('code/server/started');
		(globalThis as { vibecodaServerListenTime?: number }).vibecodaServerListenTime = performance.now();
