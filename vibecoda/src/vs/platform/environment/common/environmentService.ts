...
export interface INativeEnvironmentPaths {
	 ...
	 *
	 * Only one instance of Vibecoda can use the same `userDataDir`.
	 */
	userDataDir: string;
...
export abstract class AbstractNativeEnvironmentService implements INativeEnvironmentService {
	...
	@memoize
	get userRoamingDataHome(): URI { return this.appSettingsHome.with({ scheme: Schemas.vibecodaUserData }); }
	...
	get argvResource(): URI {
		const vibecodaPortable = env['VSCODE_PORTABLE'];
		if (vibecodaPortable) {
			return URI.file(join(vibecodaPortable, 'argv.json'));
		}
	...
	get extensionsPath(): string {
		...
		const vibecodaExtensions = env['VSCODE_EXTENSIONS'];
		if (vibecodaExtensions) {
			return vibecodaExtensions;
		}

		const vibecodaPortable = env['VSCODE_PORTABLE'];
		if (vibecodaPortable) {
			return join(vibecodaPortable, 'extensions');
		}
	...
	get policyFile(): URI | undefined {
		if (this.args['__enable-file-policy']) {
			const vibecodaPortable = env['VSCODE_PORTABLE'];
			if (vibecodaPortable) {
				return URI.file(join(vibecodaPortable, 'policy.json'));
			}
