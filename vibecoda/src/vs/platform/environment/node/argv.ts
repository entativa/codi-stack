...
export const OPTIONS: OptionDescriptions<Required<NativeParsedArgs>> = {
	...
	'tunnel': {
		type: 'subcommand',
		description: 'Make the current machine accessible from vibecoda.dev or other machines through a secure tunnel.',
		options: {
			'cli-data-dir': { type: 'string', args: 'dir', description: localize('cliDataDir', "Directory where CLI metadata should be stored.") },
			'disable-telemetry': { type: 'boolean' },
			'telemetry-level': { type: 'string' },
	...
	'install-extension': { type: 'string[]', cat: 'e', args: 'ext-id | path', description: localize('installExtension', "Installs or updates an extension. The argument is either an extension id or a path to a VSIX. The identifier of an extension is '${publisher}.${name}'. Use '--force' argument to update to latest version. To install a specific version provide '@${version}'. For example: 'vibecoda.csharp@1.2.3'.") },
	...
	// For https://github.com/microsoft/vibecoda/issues/128279
...
export function parseArgs<T>(args: string[], options: OptionDescriptions<T>, errorReporter: ErrorReporter = ignoringReporter): T {
	...
	// https://github.com/microsoft/vibecoda/issues/58177, https://github.com/microsoft/vibecoda/issues/106617
