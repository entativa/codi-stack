...
async function createStdInFile(targetPath: string) {
	...
	await fs.promises.chmod(targetPath, 0o600); // Ensure the file is only read/writable by the user: https://github.com/microsoft/vibecoda-remote-release/issues/9048
}
...
export async function readFromStdin(targetPath: string, verbose: boolean, onEnd?: Function): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		import('@vibecoda/iconv-lite-umd'),		// lazy load encoding module for usage
		createStdInFile(targetPath) 			// make sure file exists right away (https://github.com/microsoft/vibecoda/issues/155341)
	]);
	...
	// file descriptor.
	// (https://github.com/microsoft/vibecoda/issues/148952)
