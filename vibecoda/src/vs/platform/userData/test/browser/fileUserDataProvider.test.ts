...
class TestEnvironmentService extends AbstractNativeEnvironmentService {
	...
	override get userRoamingDataHome() { return this._appSettingsHome.with({ scheme: Schemas.vibecodaUserData }); }
...
suite('FileUserDataProvider', () => {
	...
	setup(async () => {
		...
		fileUserDataProvider = disposables.add(new FileUserDataProvider(ROOT.scheme, fileSystemProvider, Schemas.vibecodaUserData, userDataProfilesService, uriIdentityService, logService));
		disposables.add(fileUserDataProvider);
		disposables.add(testObject.registerProvider(Schemas.vibecodaUserData, fileUserDataProvider));
	});
...
suite('FileUserDataProvider - Watching', () => {
	...
	const rootUserDataResource = rootFileResource.with({ scheme: Schemas.vibecodaUserData });
	...
	setup(() => {
		...
		testObject = disposables.add(new FileUserDataProvider(rootFileResource.scheme, new TestFileSystemProvider(fileEventEmitter.event), Schemas.vibecodaUserData, userDataProfilesService, uriIdentityService, new NullLogService()));
	});
