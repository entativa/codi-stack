...
export class AuxiliaryWindowsMainService extends Disposable implements IAuxiliaryWindowsMainService {
	...
	private registerListeners(): void {
		...
		validatedIpcMain.handle('vibecoda:registerAuxiliaryWindow', async (event, mainWindowId: number) => {
			const auxiliaryWindow = this.getWindowByWebContents(event.sender);
			if (auxiliaryWindow) {
				this.logService.trace('[aux window] vibecoda:registerAuxiliaryWindow: Registering auxiliary window to main window');
