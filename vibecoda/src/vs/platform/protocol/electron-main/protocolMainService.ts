...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private readonly validExtensions = new Set(['.svg', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.mp4', '.otf', '.ttf']); // https://github.com/microsoft/vibecoda/issues/119384
	...
	constructor(
		...
		// - storage    : all files in global and workspace storage (https://github.com/microsoft/vibecoda/issues/116735)
	...
	private handleProtocols(): void {
		...
		// Register vibecoda-file:// handler
		defaultSession.protocol.registerFileProtocol(Schemas.vibecodaFileResource, (request, callback) => this.handleResourceRequest(request, callback));
		...
		this._register(toDisposable(() => {
			defaultSession.protocol.unregisterProtocol(Schemas.vibecodaFileResource);
	...
	private handleResourceRequest(request: Electron.ProtocolRequest, callback: ProtocolCallback): void {
		...
		// Refs https://github.com/microsoft/vibecoda/issues/148541#issuecomment-2670891511
		...
		this.logService.error(`${Schemas.vibecodaFileResource}: Refused to load resource ${path} from ${Schemas.vibecodaFileResource}: protocol (original URL: ${request.url})`);
	...
	private requestToNormalizedFilePath(request: Electron.ProtocolRequest): string {
		...
		//     `vibecoda-file:` URI to a `file:` URI.
	...
	createIPCObjectUrl<T>(): IIPCObjectUrl<T> {
		...
		const resource = URI.from({
			scheme: 'vibecoda', // used for all our IPC communication (vibecoda:<channel>)
			path: generateUuid()
		});
