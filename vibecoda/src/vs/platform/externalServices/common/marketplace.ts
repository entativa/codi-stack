...
export async function resolveMarketplaceHeaders(version: string,
	...
	const headers: IHeaders = {
		'X-Market-Client-Id': `Vibecoda ${version}`,
		'User-Agent': `Vibecoda ${version} (${productService.nameShort})`
	};
	...
	if (supportsTelemetry(productService, environmentService) && getTelemetryLevel(configurationService) === TelemetryLevel.USAGE) {
		...
		// Send machineId as Vibecoda-SessionId so we can correlate telemetry events across different services
		// machineId can be undefined sometimes (eg: when launching from CLI), so send serviceMachineId instead otherwise
		// Marketplace will reject the request if there is no Vibecoda-SessionId header
		headers['Vibecoda-SessionId'] = telemetryService.machineId || serviceMachineId;
	}
