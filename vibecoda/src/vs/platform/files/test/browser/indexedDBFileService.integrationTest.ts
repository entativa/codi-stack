...
flakySuite('IndexedDBFileSystemProvider', function () {
	...
	const userdataURIFromPaths = (paths: readonly string[]) => joinPath(URI.from({ scheme: Schemas.vibecodaUserData, path: testDir }), ...paths);
	...
	const reload = async () => {
		...
		const indexedDB = await IndexedDB.create('vibecoda-web-db-test', 1, ['vibecoda-userdata-store', 'vibecoda-logs-store']);

		userdataFileProvider = new IndexedDBFileSystemProvider(Schemas.vibecodaUserData, indexedDB, 'vibecoda-userdata-store', true);
		disposables.add(service.registerProvider(Schemas.vibecodaUserData, userdataFileProvider));
