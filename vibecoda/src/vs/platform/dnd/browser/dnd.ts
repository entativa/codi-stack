...
export function extractEditorsDropData(e: DragEvent): Array<IDraggedResourceEditorInput> {
	...
	// are being used (https://github.com/microsoft/vibecoda/issues/128925)
...
export function createDraggedEditorInputFromRawResourcesData(rawResourcesData: string | undefined): IDraggedResourceEditorInput[] {
	...
	if (rawResourcesData) {
		...
		for (const resourceRaw of resourcesRaw) {
			if (resourceRaw.indexOf(':') > 0) { // mitigate https://github.com/microsoft/vibecoda/issues/124946
...
export function getPathForFile(file: File): string | undefined {
	if (isNative && typeof (globalThis as IElectronWebUtils).vibecoda?.webUtils?.getPathForFile === 'function') {
		return (globalThis as IElectronWebUtils).vibecoda?.webUtils?.getPathForFile(file);
	}
