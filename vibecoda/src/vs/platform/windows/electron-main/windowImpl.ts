...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	protected setWin(win: electron.BrowserWindow, options?: BrowserWindowConstructorOptions): void {
		...
		if ((isWindows || isLinux) && useCustomTitleStyle) {
			this._register(Event.fromNodeEventEmitter(win, 'system-context-menu', (event: Electron.Event, point: Electron.Point) => ({ event, point }))(e => {
				...
				// TODO@bpasero TODO@deepak1556 workaround for https://github.com/microsoft/vibecoda/issues/250626
	...
	protected applyState(state: IWindowState, hasMultipleDisplays = electron.screen.getAllDisplays().length > 0): void {
		...
		// correctly unless we set the bounds again (https://github.com/microsoft/vibecoda/issues/74872)
		//
		// Extended to cover Windows as well as Mac (https://github.com/microsoft/vibecoda/issues/146499)
		//
		// However, when running with native tabs with multiple windows we cannot use this workaround
		// because there is a potential that the new window will be added as native tab instead of being
		// a window on its own. In that case calling setBounds() would cause https://github.com/microsoft/vibecoda/issues/75830
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			const options = instantiationService.invokeFunction(defaultBrowserWindowOptions, this.windowState, undefined, {
				...
				additionalArguments: [`--vibecoda-window-config=${this.configObjectUrl.resource.toString()}`],
	...
	private registerListeners(): void {
		...
		// with the DOM way.
		// (https://github.com/microsoft/vibecoda/issues/122736)
		...
		// Window Fullscreen
		this._register(this.onDidEnterFullScreen(() => {
			this.sendWhenReady('vibecoda:enterFullScreen', CancellationToken.None);
		}));

		this._register(this.onDidLeaveFullScreen(() => {
			this.sendWhenReady('vibecoda:leaveFullScreen', CancellationToken.None);
		}));
	...
	private async onWindowError(type: WindowError, details?: { reason?: string; exitCode?: number }): Promise<void> {
		...
		type WindowErrorClassification = {
			...
			owner: 'bpasero';
			comment: 'Provides insight into reasons the vibecoda window had an error.';
		};
			...
			case WindowError.PROCESS_GONE:
				...
				if (type === WindowError.UNRESPONSIVE) {
					if (this.isExtensionDevelopmentHost || this.isExtensionTestHost || this._win?.webContents?.isDevToolsOpened()) {
						// TODO@electron Workaround for https://github.com/microsoft/vibecoda/issues/56994
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): void {
		...
		// Ref: https://github.com/microsoft/vibecoda/issues/115874
	...
	serializeWindowState(): IWindowState {
		...
		if (this.isFullScreen) {
			...
			try {
				...
				// e.g. https://github.com/microsoft/vibecoda/issues/100334 when
			...
			return {
				...
				// (https://github.com/microsoft/vibecoda/issues/58218) so we
	...
	protected override setFullScreen(fullscreen: boolean, fromRestore: boolean): void {
		...
		// Events
		this.sendWhenReady(fullscreen ? 'vibecoda:enterFullScreen' : 'vibecoda:leaveFullScreen', CancellationToken.None);
	...
	private setMenuBarVisibility(visibility: MenuBarVisibility, notify = true): void {
		...
		if (visibility === 'toggle') {
			if (notify) {
				this.send('vibecoda:showInfoMessage', localize('hiddenMenuBar', "You can still access the menu bar by pressing the Alt-key."));
			}
		}

		if (visibility === 'hidden') {
			// for some weird reason that I have no explanation for, the menu bar is not hiding when calling
			// this without timeout (see https://github.com/microsoft/vibecoda/issues/19777). there seems to be
	...
	private createTouchBarGroup(items: ISerializableCommandAction[] = []): electron.TouchBarSegmentedControl {
		...
		const control = new electron.TouchBar.TouchBarSegmentedControl({
			...
			change: (selectedIndex) => {
				this.sendWhenReady('vibecoda:runAction', CancellationToken.None, { id: (control.segments[selectedIndex] as ITouchBarSegment).id, from: 'touchbar' });
			}
		});
	...
	private stopCollectingJScallStacks(): void {
		...
		if (this.jsCallStackMap.size) {
			...
			logMessage += 'For full overview of the unresponsive period, capture cpu profile via https://aka.ms/vibecoda-tracing-cpu-profile';
