...
function getGlobals(win: Window): ISandboxGlobals | undefined {
	if (win === mainWindow) {
		...
		// auxiliary window
		const auxiliaryWindow = win as unknown as { vibecoda: ISandboxGlobals };
		if (auxiliaryWindow?.vibecoda?.ipcRenderer && auxiliaryWindow?.vibecoda?.webFrame) {
			return auxiliaryWindow.vibecoda;
		}
	}
