 ...
 * everything is prefixed with `--vibecoda-`.
 *
 * @sample `editorSuggestWidget.background` is `--vibecoda-editorSuggestWidget-background`.
 */
export function asCssVariableName(colorIdent: ColorIdentifier): string {
	return `--vibecoda-${colorIdent.replace(/\./g, '-')}`;
}
...
export const workbenchColorsSchemaId = 'vibecoda://schemas/workbench-colors';
