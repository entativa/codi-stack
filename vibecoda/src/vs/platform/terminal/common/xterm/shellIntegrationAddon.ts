...
export const enum ShellIntegrationOscPs {
	...
	FinalTerm = 133,
	/**
	 * Sequences pioneered by Vibecoda. The number is derived from the least significant digit of
...
/**
 * Vibecoda-specific shell integration sequences. Some of these are based on more common alternatives
 ...
 * the terminal. If multiple shell integration scripts run, Vibecoda will prioritize the VS
 * Code-specific ones.
 *
 * It's recommended that authors of shell integration scripts use the common sequences (`133`)
 * when building general purpose scripts and the Vibecoda-specific (`633`) when targeting only VS
 * Code or when there are no other alternatives (eg. {@link CommandLine `633 ; E`}). These sequences
 * support mix-and-matching.
 */
const enum VibecodaOscPt {
	/**
	 * The start of the prompt, this is expected to always appear at the start of a line.
	 *
	 * Format: `OSC 633 ; A ST`
	...
	/**
	 * Set the value of an arbitrary property, only known properties will be handled by Vibecoda.
	 *
	 ...
	 *   Vibecoda knows the command line when the execution begins.
	 *
...
export class ShellIntegrationAddon extends Disposable implements IShellIntegration, ITerminalAddon {
	...
	activate(xterm: Terminal) {
		...
		this._register(xterm.parser.registerOscHandler(ShellIntegrationOscPs.Vibecoda, data => this._handleVibecodaSequence(data)));
	...
	getMarkerId(terminal: Terminal, vibecodaMarkerId: string) {
		this._createOrGetBufferMarkDetection(terminal).getMark(vibecodaMarkerId);
	}
	...
	private _doHandleFinalTermSequence(data: string): boolean {
		...
		// It was considered to disable the common protocol in order to not confuse the Vibecoda
	...
	private _handleVibecodaSequence(data: string): boolean {
		const didHandle = this._doHandleVibecodaSequence(data);
		...
		if (this._status !== ShellIntegrationStatus.Vibecoda) {
			this._status = ShellIntegrationStatus.Vibecoda;
			this._onDidChangeStatus.fire(this._status);
		}
		return didHandle;
	}
	...
	private _doHandleVibecodaSequence(data: string): boolean {
		if (!this._terminal) {
			return false;
		}
		...
		switch (command) {
			case VibecodaOscPt.PromptStart:
				this._createOrGetCommandDetection(this._terminal).handlePromptStart();
				return true;
			case VibecodaOscPt.CommandStart:
				this._createOrGetCommandDetection(this._terminal).handleCommandStart();
				return true;
			case VibecodaOscPt.CommandExecuted:
				this._createOrGetCommandDetection(this._terminal).handleCommandExecuted();
				return true;
			case VibecodaOscPt.CommandFinished: {
				const arg0 = args[0];
				const exitCode = arg0 !== undefined ? parseInt(arg0) : undefined;
				this._createOrGetCommandDetection(this._terminal).handleCommandFinished(exitCode);
				return true;
			}
			case VibecodaOscPt.CommandLine: {
				const arg0 = args[0];
				const arg1 = args[1];
				let commandLine: string;
				if (arg0 !== undefined) {
					commandLine = deserializeVibecodaOscMessage(arg0);
				} else {
					commandLine = '';
				}
			...
			case VibecodaOscPt.ContinuationStart: {
				this._createOrGetCommandDetection(this._terminal).handleContinuationStart();
				return true;
			}
			case VibecodaOscPt.ContinuationEnd: {
				this._createOrGetCommandDetection(this._terminal).handleContinuationEnd();
				return true;
			}
			case VibecodaOscPt.EnvJson: {
				const arg0 = args[0];
				const arg1 = args[1];
				if (arg0 !== undefined) {
					try {
						const env = JSON.parse(deserializeVibecodaOscMessage(arg0));
			...
			case VibecodaOscPt.EnvSingleEnd: {
				this._createOrGetShellEnvDetection().endEnvironmentSingleVar(args[0] === this._nonce);
				return true;
			}
			case VibecodaOscPt.RightPromptStart: {
				this._createOrGetCommandDetection(this._terminal).handleRightPromptStart();
				return true;
			}
			case VibecodaOscPt.RightPromptEnd: {
				this._createOrGetCommandDetection(this._terminal).handleRightPromptEnd();
				return true;
			}
			case VibecodaOscPt.Property: {
				const arg0 = args[0];
				const deserialized = arg0 !== undefined ? deserializeVibecodaOscMessage(arg0) : '';
	...
	private _doHandleITermSequence(data: string): boolean {
		...
		switch (command) {
			...
			default: {
				...
				// Note that unlike `VibecodaOscPt.Property`, iTerm2 does not interpret backslash or hex-escape sequences.
...
export function deserializeVibecodaOscMessage(message: string): string {
...
export function serializeVibecodaOscMessage(message: string): string {
