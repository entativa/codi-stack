...
const productService = { applicationName: 'vibecoda' } as IProductService;
		...
		if (process.platform !== 'win32') {
			suite('zsh', async () => {
				suite('should override args', async () => {
					...
					const expectedDir = new RegExp(`.+\/${username}-vibecoda-zsh`);
					const customZdotdir = '/custom/zsh/dotdir';
					const expectedDests = [
						new RegExp(`.+\\/${username}-vibecoda-zsh\\/\\.zshrc`),
						new RegExp(`.+\\/${username}-vibecoda-zsh\\/\\.zprofile`),
						new RegExp(`.+\\/${username}-vibecoda-zsh\\/\\.zshenv`),
						new RegExp(`.+\\/${username}-vibecoda-zsh\\/\\.zlogin`)
					];
