...
export class ElectronPtyHostStarter extends Disposable implements IPtyHostStarter {
	...
	constructor(
		...
		validatedIpcMain.on('vibecoda:createPtyHostMessageChannel', (e, nonce) => this._onWindowConnection(e, nonce));
		this._register(toDisposable(() => {
			validatedIpcMain.removeHandler('vibecoda:createPtyHostMessageChannel');
		}));
	}
	...
	private _onWindowConnection(e: IpcMainEvent, nonce: string) {
		...
		e.sender.postMessage('vibecoda:createPtyHostMessageChannelResult', nonce, [port]);
	}
}
