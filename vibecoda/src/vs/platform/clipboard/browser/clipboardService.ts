 ...
 */
const vibecodaResourcesMime = 'application/vnd.code.resources';
	...
	async writeResources(resources: URI[]): Promise<void> {
		...
		try {
			await getActiveWindow().navigator.clipboard.write([
				new ClipboardItem({
					[`web ${vibecodaResourcesMime}`]: new Blob([
						JSON.stringify(resources.map(x => x.toJSON()))
					], {
						type: vibecodaResourcesMime
					})
				})
			]);
	...
	async readResources(): Promise<URI[]> {
		...
		try {
			...
			for (const item of items) {
				if (item.types.includes(`web ${vibecodaResourcesMime}`)) {
					const blob = await item.getType(`web ${vibecodaResourcesMime}`);
