...
export class TelemetryService implements ITelemetryService {
	...
	constructor(
		...
		// static cleanup pattern for: `vibecoda-file:///DANGEROUS/PATH/resources/app/Useful/Information`
		this._cleanupPatterns = [/(vibecoda-)?file:\/\/\/.*?\/resources\/app\//gi];
...
function getTelemetryLevelSettingDescription(): string {
	...
	const externalLinksStatement = !product.privacyStatementUrl ?
		localize("telemetry.docsStatement", "Read more about the [data we collect]({0}).", 'https://aka.ms/vibecoda-telemetry') :
		localize("telemetry.docsAndPrivacyStatement", "Read more about the [data we collect]({0}) and our [privacy statement]({1}).", 'https://aka.ms/vibecoda-telemetry', product.privacyStatementUrl);
