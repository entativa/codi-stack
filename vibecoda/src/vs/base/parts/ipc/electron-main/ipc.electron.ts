...
export class Server extends IPCServer {
	...
	private static getOnDidClientConnect(): Event<ClientConnectionEvent> {
		const onHello = Event.fromNodeEventEmitter<WebContents>(validatedIpcMain, 'vibecoda:hello', ({ sender }) => sender);
		...
		return Event.map(onHello, webContents => {
			...
			const onMessage = createScopedOnMessageEvent(id, 'vibecoda:message') as Event<VSBuffer>;
			const onDidClientDisconnect = Event.any(Event.signal(createScopedOnMessageEvent(id, 'vibecoda:disconnect')), onDidClientReconnect.event);
