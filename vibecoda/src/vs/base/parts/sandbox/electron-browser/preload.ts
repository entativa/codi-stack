...
(function () {
	...
	function validateIPC(channel: string): true | never {
		if (!channel?.startsWith('vibecoda:')) {
	...
	const resolveConfiguration: Promise<ISandboxConfiguration> = (async () => {
		const windowConfigIpcChannel = parseArgv('vibecoda-window-config');
		if (!windowConfigIpcChannel) {
			throw new Error('Preload: did not find expected vibecoda-window-config in renderer process arguments list.');
		}
		...
		try {
			...
			// settings per origin (if vibecoda-file:// is used) and
			// we want to ensure that the user configuration wins.
			webFrame.setZoomLevel(resolvedConfiguration.zoomLevel ?? 0);

			return resolvedConfiguration;
		} catch (error) {
			throw new Error(`Preload: unable to fetch vibecoda-window-config: ${error}`);
		}
	})();
	...
	/**
	 * If Vibecoda is not run from a terminal, we should resolve additional
	...
	const resolveShellEnv: Promise<typeof process.env> = (async () => {
		...
		const [userEnv, shellEnv] = await Promise.all([
			...
			ipcRenderer.invoke('vibecoda:fetchShellEnv')
		]);
	...
	try {
		// Use `contextBridge` APIs to expose globals to Vibecoda
		contextBridge.exposeInMainWorld('vibecoda', globals);
	} catch (error) {
		console.error(error);
	}
}());
