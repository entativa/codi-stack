...
export const {
	...
	windows.set(mainWindow.vibecodaWindowId, mainWindowRegistration);
	...
	return {
		...
		registerWindow(window: CodeWindow): IDisposable {
			if (windows.has(window.vibecodaWindowId)) {
			...
			windows.set(window.vibecodaWindowId, registeredWindow);

			disposables.add(toDisposable(() => {
				windows.delete(window.vibecodaWindowId);
		...
		getWindowId(targetWindow: Window): number {
			return (targetWindow as CodeWindow).vibecodaWindowId;
		},
...
export class DragAndDropObserver extends Disposable {
	...
	// calls see https://github.com/microsoft/vibecoda/issues/14470
