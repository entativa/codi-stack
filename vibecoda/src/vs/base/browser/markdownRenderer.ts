		...
		shouldKeep: (element, data) => {
			if (element.tagName === 'SPAN') {
				if (data.attrName === 'style') {
					return /^(color\:(#[0-9a-fA-F]+|var\(--vibecoda(-[a-zA-Z0-9]+)+\));)?(background-color\:(#[0-9a-fA-F]+|var\(--vibecoda(-[a-zA-Z0-9]+)+\));)?(border-radius:[0-9]+px;)?$/.test(data.attrValue);
				}
			}
			return false;
		}
...
function getDomSanitizerConfig(mdStrConfig: MdStrConfig, options: MarkdownSanitizerConfig): domSanitize.DomSanitizerConfig {
	...
	const allowedLinkSchemes = [
		...
		Schemas.mailto,
		Schemas.file,
		Schemas.vibecodaFileResource,
		Schemas.vibecodaRemote,
		Schemas.vibecodaRemoteResource,
		Schemas.vibecodaNotebookCell
	];
	...
	for (let i = token.tokens.length - 1; i >= 0; i--) {
		...
		if (subtoken.type === 'text') {
			...
			if (lastLine.includes('`')) {
				...
				// Where "more text" is a title for the link or an argument to a vibecoda command link
				if (
