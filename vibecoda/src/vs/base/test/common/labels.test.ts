...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Vibecoda', separator: { label: ' - ' } }), 'Vibecoda');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Vibecoda', separator: { label: ' - ' } }), 'Untitled-1 - Vibecoda');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Vibecoda', separator: { label: ' - ' } }), 'monaco - Vibecoda');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Vibecoda', separator: { label: ' - ' } }), 'somefile.txt - monaco - Vibecoda');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Vibecoda', separator: { label: ' - ' } }), '* somefile.txt - monaco - Vibecoda');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Vibecoda', separator: { label: ' - ' } }), 'monaco - Vibecoda');
	});
	...
	test('getPathLabel', () => {
		...
		const nixBadFileUri = URI.revive({ scheme: 'vibecoda', authority: 'file', path: '//some/folder/file.txt' });
		const uncFileUri = URI.file('c:/some/folder/file.txt').with({ authority: 'auth' });
		const remoteFileUri = URI.file('/some/folder/file.txt').with({ scheme: 'vibecoda-test', authority: 'auth' });
		...
		const remoteUserHome = URI.file('/some').with({ scheme: 'vibecoda-test', authority: 'auth' });
