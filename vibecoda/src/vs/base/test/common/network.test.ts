...
suite('network', () => {
	...
	(isWeb ? test.skip : test)('FileAccess: moduleId (native)', () => {
		...
		assert.strictEqual(browserUri.scheme, Schemas.vibecodaFileResource);
	...
	(isWeb ? test.skip : test)('FileAccess: query and fragment is kept if URI is already of same scheme (native)', () => {
		const originalFileUri = URI.file('network.test.ts').with({ query: 'foo=bar', fragment: 'something' });
		const browserUri = FileAccess.uriToBrowserUri(originalFileUri.with({ scheme: Schemas.vibecodaFileResource }));
	...
	test('FileAccess: remote URIs', () => {
		const originalRemoteUri = URI.file('network.test.ts').with({ scheme: Schemas.vibecodaRemote });
