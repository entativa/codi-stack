...
export function listProcesses(rootPid: number): Promise<ProcessItem> {
	...
	return new Promise((resolve, reject) => {
		...
		if (process.platform === 'win32') {
			...
			(import('@vibecoda/windows-process-tree')).then(windowsProcessTree => {
			...
			function calculateLinuxCpuUsage() {
				// Flatten rootItem to get a list of all Vibecoda processes
			...
			exec('which ps', {}, (err, stdout, stderr) => {
				if (err || stderr) {
					...
					exec(`${ps} ${args}`, { maxBuffer: 1000 * 1024, env: { LC_NUMERIC: 'en_US.UTF-8' } }, (err, stdout, stderr) => {
						// Silently ignoring the screen size is bogus error. See https://github.com/microsoft/vibecoda/issues/98590
