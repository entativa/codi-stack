...
class RemoteExtensionHostAgentServer extends Disposable implements IServerAPI {
	...
	public async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (pathname === '/vibecoda-remote-resource') {
...
export async function createServer(address: string | net.AddressInfo | null, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string): Promise<IServerAPI> {
	...
	function initUnexpectedErrorHandler(handler: (err: any) => void) {
		setUnexpectedErrorHandler(err => {
			// See https://github.com/microsoft/vibecoda-remote-release/issues/6481
	...
	process.on('SIGPIPE', () => {
		// See https://github.com/microsoft/vibecoda-remote-release/issues/6543
...
`;
	...
	const vibecodaServerStartTime: number = (<any>global).vibecodaServerStartTime;
	// eslint-disable-next-line local/code-no-any-casts
	const vibecodaServerListenTime: number = (<any>global).vibecodaServerListenTime;
	// eslint-disable-next-line local/code-no-any-casts
	const vibecodaServerCodeLoadedTime: number = (<any>global).vibecodaServerCodeLoadedTime;
	...
	instantiationService.invokeFunction(async (accessor) => {
		...
		telemetryService.publicLog2<ServerStartEvent, ServerStartClassification>('serverStart', {
			startTime: vibecodaServerStartTime,
			startedTime: vibecodaServerListenTime,
			codeLoadedTime: vibecodaServerCodeLoadedTime,
	...
	if (args['print-startup-performance']) {
		let output = '';
		output += `Start-up time: ${vibecodaServerListenTime - vibecodaServerStartTime}\n`;
		output += `Code loading time: ${vibecodaServerCodeLoadedTime - vibecodaServerStartTime}\n`;
		output += `Initialized time: ${currentTime - vibecodaServerStartTime}\n`;
		output += `\n`;
		console.log(output);
	}
