...
/*
 * Implements a standalone CLI app that opens Vibecoda from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Vibecoda on the Windows side.
 *    The Vibecoda desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a Vibecoda terminal.');
		return;
	}
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to Vibecoda server: ' + message);
	console.error(err);
	process.exit(1);
}
...
function mapFileToRemoteUri(uri: string): string {
	return uri.replace(/^file:\/\//, 'vibecoda-remote://' + cliRemoteAuthority);
}
