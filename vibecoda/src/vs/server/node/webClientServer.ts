...
export class WebClientServer {
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
		...
		const resolveWorkspaceURI = (defaultLocation?: string) => defaultLocation && URI.file(resolve(defaultLocation)).with({ scheme: Schemas.vibecodaRemote, authority: remoteAuthority });
		...
		const locale = cookies['vibecoda.nls.locale'] || req.headers['accept-language']?.split(',')[0]?.toLowerCase() || 'en';
		...
		if (useTestResolver) {
			...
			for (const extensionPath of ['vibecoda-test-resolver', 'github-authentication']) {
		...
		const cspDirectives = [
			...
			'child-src \'self\';',
			`frame-src 'self' https://*.vibecoda-cdn.net data:;`,
