...
class SharedProcessMain extends Disposable implements IClientConnectionFilter {
	...
	private async initServices(): Promise<IInstantiationService> {
		...
		const userDataFileSystemProvider = this._register(new FileUserDataProvider(
			...
			Schemas.vibecodaUserData,
			userDataProfilesService,
			uriIdentityService,
			logService
		));
		fileService.registerProvider(Schemas.vibecodaUserData, userDataFileSystemProvider);
		...
		if (isLinux) {
			...
			if (releaseInfo) {
				type ClientPlatformInfoClassification = {
					...
					codeDisplayProtocol: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'A string identifying the vibecoda display protocol type.' };
					owner: 'benibenj';
