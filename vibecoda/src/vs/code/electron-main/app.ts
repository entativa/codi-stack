...
/**
 * The main Vibecoda application. There will only ever be one instance,
...
export class CodeApplication extends Disposable {
	...
	private static readonly SECURITY_PROTOCOL_HANDLING_CONFIRMATION_SETTING_KEY = {
		...
		[Schemas.vibecodaRemote]: 'security.promptForRemoteFileProtocolHandling' as const
	};
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) => requestingUrl?.startsWith(`${Schemas.vibecodaFileResource}://${VSCODE_AUTHORITY}`);
		const isUrlFromWebview = (requestingUrl: string | undefined) => requestingUrl?.startsWith(`${Schemas.vibecodaWebview}://`);
		...
		const allowedPermissionsInWebview = new Set([
			...
			// https://github.com/microsoft/vibecoda/issues/239228
		...
		const supportedSvgSchemes = new Set([Schemas.file, Schemas.vibecodaFileResource, Schemas.vibecodaRemoteResource, Schemas.vibecodaManagedRemoteResource, 'devtools']);
		...
		const isSafeFrame = (requestFrame: WebFrameMain | null | undefined): boolean => {
			for (let frame: WebFrameMain | null | undefined = requestFrame; frame; frame = frame.parent) {
				if (frame.url.startsWith(`${Schemas.vibecodaWebview}://`)) {
					return true;
				}
			}
			return false;
		...
		session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.scheme === Schemas.vibecodaWebview) {
				if (!isAllowedWebviewRequest(uri, details)) {
					this.logService.error('Blocked vibecoda-webview request', details.url);
					return callback({ cancel: true });
				}
			}

			if (uri.scheme === Schemas.vibecodaFileResource) {
				if (!isAllowedVsCodeFileRequest(details)) {
					this.logService.error('Blocked vibecoda-file request', details.url);
					return callback({ cancel: true });
				}
			}
		...
		// https://github.com/microsoft/vibecoda/issues/97564
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			...
			if (contentTypes && Array.isArray(contentTypes)) {
				...
				// http://127.0.0.1:<port>/vibecoda-remote-resource?path=
				if (!uri.path.endsWith(Schemas.vibecodaRemoteResource) && contentTypes.some(contentType => contentType.toLowerCase().includes('image/svg'))) {
					return callback({ cancel: !isSvgRequestFromSafeContext(details) });
				}
			}
		...
		// https://github.com/microsoft/vibecoda-remote-release/issues/9246
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			if (details.url.startsWith('https://vibecoda.download.prss.microsoft.com/')) {
		...
		if (typeof defaultSession.setCodeCachePath === 'function' && this.environmentMainService.codeCachePath) {
			...
			// (https://github.com/microsoft/vibecoda/issues/120655)
	...
	private registerListeners(): void {
		...
		app.on('accessibility-support-changed', (event, accessibilitySupportEnabled) => {
			this.windowsMainService?.sendToAll('vibecoda:accessibilitySupportChanged', accessibilitySupportEnabled);
		});
		...
		app.on('web-contents-created', (event, contents) => {
			...
			if (contents?.opener?.url.startsWith(`${Schemas.vibecodaFileResource}://${VSCODE_AUTHORITY}/`)) {
		...
		validatedIpcMain.handle('vibecoda:fetchShellEnv', event => {
		...
		validatedIpcMain.on('vibecoda:toggleDevTools', event => event.sender.toggleDevTools());
		validatedIpcMain.on('vibecoda:openDevTools', event => event.sender.openDevTools());

		validatedIpcMain.on('vibecoda:reloadWindow', event => event.sender.reload());

		validatedIpcMain.handle('vibecoda:notifyZoomLevel', async (event, zoomLevel: number | undefined) => {
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting Vibecoda');
		...
		// "com.microsoft.", which breaks native tabs for Vibecoda when using this
		// identifier (from the official build).
		// Explicitly opt out of the patch here before creating any windows.
		// See: https://github.com/microsoft/vibecoda/issues/35361#issuecomment-399794085
		try {
		...
		// Setup vibecoda-remote-resource protocol handler
	...
	private setupManagedRemoteResourceUrlHandler(mainProcessElectronServer: ElectronIPCServer) {
		...
		protocol.registerBufferProtocol(Schemas.vibecodaManagedRemoteResource, (request, callback) => {
	...
	private async shouldBlockOpenable(openable: IWindowOpenable, windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService): Promise<boolean> {
		...
		if (openableUri.scheme !== Schemas.file && openableUri.scheme !== Schemas.vibecodaRemote) {

			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//
			// NOTE: we currently only ask for confirmation for `file` and `vibecoda-remote`
		...
		if (checkboxChecked) {
			// Due to https://github.com/microsoft/vibecoda/issues/195436, we can only
			...
			const request = { channel: 'vibecoda:disablePromptForProtocolHandling', args: openableUri.scheme === Schemas.file ? 'local' : 'remote' };
	...
	private getWindowOpenableFromProtocolUrl(uri: URI): IWindowOpenable | undefined {
		...
		// Remote path
		else if (uri.authority === Schemas.vibecodaRemote) {

			// Example conversion:
			// From: vibecoda://vibecoda-remote/wsl+ubuntu/mnt/c/GitDevelopment/monaco
			//   To: vibecoda-remote://wsl+ubuntu/mnt/c/GitDevelopment/monaco
			...
			if (params.get('windowId') === '_blank') {
				...
				// https://github.com/microsoft/vibecoda/issues/191902
				params.delete('windowId');
				query = params.toString();
			}

			const remoteUri = URI.from({ scheme: Schemas.vibecodaRemote, authority, path, query, fragment: uri.fragment });
	...
	private async handleProtocolUrl(windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService, urlService: IURLService, uri: URI, options?: IOpenURLOptions): Promise<boolean> {
		...
		// Support 'workspace' URLs (https://github.com/microsoft/vibecoda/issues/124263)
	...
	private setupSharedProcess(machineId: string, sqmId: string, devDeviceId: string): { sharedProcessReady: Promise<MessagePortClient>; sharedProcessClient: Promise<MessagePortClient> } {
		...
		this._register(sharedProcess.onDidCrash(() => this.windowsMainService?.sendToFocused('vibecoda:reportSharedProcessCrash')));
	...
	private afterWindowOpen(): void {
		...
		// Remote Authorities
		protocol.registerHttpProtocol(Schemas.vibecodaRemoteResource, (request, callback) => {
			callback({
				url: request.url.replace(/^vibecoda-remote-resource:/, 'http:'),
		...
		if (isMacintosh && app.runningUnderARM64Translation) {
			this.windowsMainService?.sendToFocused('vibecoda:showTranslatedBuildWarning');
		}
	}
	...
	private async installMutex(): Promise<void> {
		...
		if (isWindows && win32MutexName) {
			try {
				const WindowsMutex = await import('@vibecoda/windows-mutex');
	...
	private async resolveShellEnvironment(args: NativeParsedArgs, env: IProcessEnvironment, notifyOnError: boolean): Promise<typeof process.env> {
		try {
			...
			if (notifyOnError) {
				this.windowsMainService?.sendToFocused('vibecoda:showResolveShellEnvError', errorMessage);
			} else {
	...
	private async updateCrashReporterEnablement(): Promise<void> {
		...
		try {
			...
			this.windowsMainService?.sendToFocused('vibecoda:showArgvParseWarning');
		}
	}
	...
	private eventuallyAfterWindowOpen(): void {
		...
		// Refs: https://github.com/microsoft/vibecoda/issues/234064
