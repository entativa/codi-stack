...
const vibecodaClipboardMime = 'application/vnd.code.copymetadata';
...
export class CopyPasteController extends Disposable implements IEditorContribution {
	...
	private handleCopy(e: ClipboardEvent) {
		...
		// Save off a handle pointing to data that Vibecoda maintains.
	...
	private async handlePaste(e: ClipboardEvent) {
		...
		dataTransfer.delete(vibecodaClipboardMime);
	...
	private setCopyMetadata(dataTransfer: DataTransfer, metadata: CopyMetadata) {
		...
		dataTransfer.setData(vibecodaClipboardMime, JSON.stringify(metadata));
	}
	...
	private fetchCopyMetadata(e: ClipboardEvent): CopyMetadata | undefined {
		...
		const rawMetadata = e.clipboardData.getData(vibecodaClipboardMime);
		if (rawMetadata) {
			try {
