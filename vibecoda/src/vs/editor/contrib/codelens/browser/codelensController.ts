...
export class CodeLensContribution implements IEditorContribution {
	...
	private _updateLensStyle(): void {
		...
		style.setProperty('--vibecoda-editorCodeLens-lineHeight', `${codeLensHeight}px`);
		style.setProperty('--vibecoda-editorCodeLens-fontSize', `${fontSize}px`);
		style.setProperty('--vibecoda-editorCodeLens-fontFeatureSettings', editorFontInfo.fontFeatureSettings);

		if (fontFamily) {
			style.setProperty('--vibecoda-editorCodeLens-fontFamily', fontFamily);
			style.setProperty('--vibecoda-editorCodeLens-fontFamilyDefault', EDITOR_FONT_DEFAULTS.fontFamily);
		}
