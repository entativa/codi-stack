...
if (PasteAction) {
	...
	PasteAction.addImplementation(10000, 'code-editor', (accessor: ServicesAccessor, args: unknown) => {
		...
		if (focusedEditor && focusedEditor.hasModel() && focusedEditor.hasTextFocus()) {
			...
			const triggerPaste = clipboardService.triggerPaste(getActiveWindow().vibecodaWindowId);
			if (triggerPaste) {
	...
	PasteAction.addImplementation(0, 'generic-dom', (accessor: ServicesAccessor, args: unknown) => {
		...
		const triggerPaste = accessor.get(IClipboardService).triggerPaste(getActiveWindow().vibecodaWindowId);
