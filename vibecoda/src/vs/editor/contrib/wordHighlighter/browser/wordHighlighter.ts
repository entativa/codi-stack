...
class WordHighlighter {
	...
	constructor(
		...
		this.toUnhook.add(editor.onDidBlurEditorWidget(() => {
			...
			if (!activeEditor) { // clicked into nb cell list, outline, terminal, etc
				this._stopAll();
			} else if (activeEditor.getModel()?.uri.scheme === Schemas.vibecodaNotebookCell && this.editor.getModel()?.uri.scheme !== Schemas.vibecodaNotebookCell) { // switched tabs from non-nb to nb
				this._stopAll();
			}
		}));
	...
	private _stopSingular(): void {
		...
		if (this.editor.hasTextFocus()) {
			if (this.editor.getModel()?.uri.scheme !== Schemas.vibecodaNotebookCell && WordHighlighter.query?.modelInfo?.modelURI.scheme !== Schemas.vibecodaNotebookCell) { // clear query if focused non-nb editor
	...
	private getOtherModelsToHighlight(model: ITextModel): ITextModel[] {
		...
		// notebook case
		const isNotebookEditor = model.uri.scheme === Schemas.vibecodaNotebookCell;
		if (isNotebookEditor) {
			const currentModels: ITextModel[] = [];
			const currentEditors = this.codeEditorService.listCodeEditors();
			for (const editor of currentEditors) {
				const tempModel = editor.getModel();
				if (tempModel && tempModel !== model && tempModel.uri.scheme === Schemas.vibecodaNotebookCell) {
	...
	private async _run(multiFileConfigChange?: boolean, delay?: number): Promise<void> {
		...
		} else if (this.model.uri.scheme === Schemas.vibecodaNotebookCell) {
	...
	constructor(
		...
		this._register(editor.onDidChangeModel((e) => {
			if (this._wordHighlighter) {
				if (!e.newModelUrl && e.oldModelUrl?.scheme !== Schemas.vibecodaNotebookCell) { // happens when switching tabs to a notebook that has focus in the cell list, no new model URI (this also doesn't make it to the wordHighlighter, bc no editor.hasModel)
