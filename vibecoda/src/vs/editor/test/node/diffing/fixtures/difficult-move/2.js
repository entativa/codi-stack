...
const { vibecodaWebEntryPoints, vibecodaWebResourceIncludes, createVibecodaWebFileContentMapper } = require('./gulpfile.vibecoda.web');
...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const localWorkspaceExtensions = glob.sync('extensions/*/package.json')
			...
			.filter(name => name !== 'vibecoda-api-tests' && name !== 'vibecoda-test-resolver'); // Do not ship the test extensions
...
['reh', 'reh-web'].forEach(type => {
	const optimizeTask = task.define(`optimize-vibecoda-${type}`, task.series(
		util.rimraf(`out-vibecoda-${type}`),
		optimize.optimizeTask(
			{
				out: `out-vibecoda-${type}`,
				amd: {
					src: 'out-build',
					...
					fileContentMapper: createVibecodaWebFileContentMapper('.build/extensions', type === 'reh-web' ? tweakProductForServerWeb(product) : product)
				},
				commonJS: {
					src: 'out-build',
					entryPoints: [
	...
	const minifyTask = task.define(`minify-vibecoda-${type}`, task.series(
		optimizeTask,
		util.rimraf(`out-vibecoda-${type}-min`),
		optimize.minifyTask(`out-vibecoda-${type}`, `https://ticino.blob.core.windows.net/sourcemaps/${commit}/core`)
	));
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			const sourceFolderName = `out-vibecoda-${type}${dashed(minified)}`;
			const destinationFolderName = `vibecoda-${type}${dashed(platform)}${dashed(arch)}`;

			const serverTaskCI = task.define(`vibecoda-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}-ci`, task.series(
			...
			const serverTask = task.define(`vibecoda-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
				compileBuildTask,
