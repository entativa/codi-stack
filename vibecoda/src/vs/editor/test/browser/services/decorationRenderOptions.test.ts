...
suite('Decoration Render Options', () => {
	...
	const options: IDecorationRenderOptions = {
		gutterIconPath: URI.parse('https://github.com/microsoft/vibecoda/blob/main/resources/linux/code.png'),
	...
	test('css properties', () => {
		...
		assert(sheet.indexOf(`{background:url('${CSS.escape('https://github.com/microsoft/vibecoda/blob/main/resources/linux/code.png')}') center center no-repeat;background-size:contain;}`) >= 0);
	...
	test('css properties, gutterIconPaths', () => {
		...
		if (platform.isWindows) {
			...
			assertBackground(CSS.escape('file:///c:/files/miles/more.png'), CSS.escape('vibecoda-file://vibecoda-app/c:/files/miles/more.png'));
			s.removeDecorationType('example');

			// single quote must always be escaped/encoded
			s.registerDecorationType('test', 'example', { gutterIconPath: URI.file('c:\\files\\foo\\b\'ar.png') });
			assertBackground(CSS.escape('file:///c:/files/foo/b\'ar.png'), CSS.escape('vibecoda-file://vibecoda-app/c:/files/foo/b\'ar.png'));
			s.removeDecorationType('example');
		} else {
			// unix file path (used as string)
			s.registerDecorationType('test', 'example', { gutterIconPath: URI.file('/Users/foo/bar.png') });
			assertBackground(CSS.escape('file:///Users/foo/bar.png'), CSS.escape('vibecoda-file://vibecoda-app/Users/foo/bar.png'));
			s.removeDecorationType('example');

			// single quote must always be escaped/encoded
			s.registerDecorationType('test', 'example', { gutterIconPath: URI.file('/Users/foo/b\'ar.png') });
			assertBackground(CSS.escape('file:///Users/foo/b\'ar.png'), CSS.escape('vibecoda-file://vibecoda-app/Users/foo/b\'ar.png'));
			s.removeDecorationType('example');
		}
