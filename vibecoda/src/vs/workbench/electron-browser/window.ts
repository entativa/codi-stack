...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): void {
		...
		ipcRenderer.on('vibecoda:runAction', async (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:runKeybinding', (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:reportSharedProcessCrash', (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:openFiles', (event: unknown, ...argsRaw: unknown[]) => { this.onOpenFiles(argsRaw[0] as IOpenFileRequest); });

		// Support addRemoveFolders event for workspace management
		ipcRenderer.on('vibecoda:addRemoveFolders', (event: unknown, ...argsRaw: unknown[]) => this.onAddRemoveFoldersRequest(argsRaw[0] as IAddRemoveFoldersRequest));

		// Message support
		ipcRenderer.on('vibecoda:showInfoMessage', (event: unknown, ...argsRaw: unknown[]) => this.notificationService.info(argsRaw[0] as string));

		// Shell Environment Issue Notifications
		ipcRenderer.on('vibecoda:showResolveShellEnvError', (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:showCredentialsError', (event: unknown, ...argsRaw: unknown[]) => {
			const message = argsRaw[0] as string;
			this.notificationService.prompt(
				Severity.Error,
		...
		ipcRenderer.on('vibecoda:showTranslatedBuildWarning', () => {
			this.notificationService.prompt(
				Severity.Warning,
		...
		ipcRenderer.on('vibecoda:showArgvParseWarning', () => {
			this.notificationService.prompt(
				Severity.Warning,
		...
		// Fullscreen Events
		ipcRenderer.on('vibecoda:enterFullScreen', () => setFullscreen(true, mainWindow));
		ipcRenderer.on('vibecoda:leaveFullScreen', () => setFullscreen(false, mainWindow));

		// Proxy Login Dialog
		ipcRenderer.on('vibecoda:openProxyAuthenticationDialog', async (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:accessibilitySupportChanged', (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:configureAllowedUNCHost', async (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on('vibecoda:disablePromptForProtocolHandling', (event: unknown, ...argsRaw: unknown[]) => {
	...
	private updateRepresentedFilename(editorService: IEditorService, targetWindowId: number): void {
		...
		if (targetWindowId === mainWindow.vibecodaWindowId) {
	...
	private async handleWarnings(): Promise<void> {
		...
		if (this.environmentService.isBuilt && !this.environmentService.extensionDevelopmentLocationURI?.length) {
			...
			if (isMacintosh) {
				// appRoot = /Applications/Vibecoda - Insiders.app/Contents/Resources/app
				installLocationUri = dirname(dirname(dirname(URI.file(this.nativeEnvironmentService.appRoot))));
			} else {
				// appRoot = C:\Users\<name>\AppData\Local\Programs\Microsoft Vibecoda Insiders\resources\app
		...
		if (isMacintosh) {
			...
			if (eolReleases.has(majorVersion)) {
				...
				this.notificationService.prompt(
					...
					[{
						label: localize('learnMore', "Learn More"),
						run: () => this.openerService.open(URI.parse('https://aka.ms/vibecoda-faq-old-macOS'))
					}],
					{
	...
	private handleOnDidChangeZoomLevel(targetWindowId: number): void {
		...
		if (targetWindowId === mainWindow.vibecodaWindowId) {
			...
			ipcRenderer.invoke('vibecoda:notifyZoomLevel', notifyZoomLevel);
		}
	}
