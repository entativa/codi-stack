...
export class DesktopMain extends Disposable {
	...
	async open(): Promise<void> {
		...
		// (fixes https://github.com/microsoft/vibecoda/issues/187982)
	...
	private getExtraClasses(): string[] {
		if (isMacintosh) {
			...
			// https://github.com/microsoft/vibecoda/pull/270236#issuecomment-3379301185
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; logService: ILogService; storageService: NativeWorkbenchStorageService; configurationService: IConfigurationService }> {
		...
		fileService.registerProvider(Schemas.vibecodaUserData, this._register(new FileUserDataProvider(Schemas.file, diskFileSystemProvider, Schemas.vibecodaUserData, userDataProfilesService, uriIdentityService, logService)));
	...
	private async createWorkspaceService(
		...
		const configurationCache = new ConfigurationCache([Schemas.file, Schemas.vibecodaUserData] /* Cache all non native resources */, environmentService, fileService);
