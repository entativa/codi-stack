...
export class CloseWindowAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		return nativeHostService.closeWindow({ targetWindowId: getActiveWindow().vibecodaWindowId });
	}
}
...
abstract class BaseZoomAction extends Action2 {
	...
	protected async setZoomLevel(accessor: ServicesAccessor, levelOrReset: number | true): Promise<void> {
		...
		if (level > MAX_ZOOM_LEVEL || level < MIN_ZOOM_LEVEL) {
			return; // https://github.com/microsoft/vibecoda/issues/48357
		}
...
abstract class BaseSwitchWindow extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const currentWindowId = getActiveWindow().vibecodaWindowId;
...
export class ToggleWindowAlwaysOnTopAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		return nativeHostService.toggleWindowAlwaysOnTop({ targetWindowId: getActiveWindow().vibecodaWindowId });
	}
}
...
export class EnableWindowAlwaysOnTopAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		return nativeHostService.setWindowAlwaysOnTop(true, { targetWindowId: targetWindow.vibecodaWindowId });
	}
}
...
export class DisableWindowAlwaysOnTopAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		return nativeHostService.setWindowAlwaysOnTop(false, { targetWindowId: targetWindow.vibecodaWindowId });
	}
}
