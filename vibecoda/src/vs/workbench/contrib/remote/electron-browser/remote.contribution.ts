...
class RemoteAgentDiagnosticListener implements IWorkbenchContribution {
	constructor(
		...
		ipcRenderer.on('vibecoda:getDiagnosticInfo', (event: unknown, ...args: unknown[]): void => {
			const request = args[0] as { replyChannel: string; args: IDiagnosticInfoOptions };
			const connection = remoteAgentService.getConnection();
			if (connection) {
				const hostName = labelService.getHostLabel(Schemas.vibecodaRemote, connection.remoteAuthority);
