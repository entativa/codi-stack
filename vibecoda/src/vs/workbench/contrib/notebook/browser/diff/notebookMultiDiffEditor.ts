...
export class NotebookMultiTextDiffEditor extends EditorPane {
	...
	public getDiffElementViewModel(uri: URI): IDiffElementViewModelBase | undefined {
		if (uri.scheme === Schemas.vibecodaNotebookCellOutput || uri.scheme === Schemas.vibecodaNotebookCellOutputDiff ||
			uri.scheme === Schemas.vibecodaNotebookCellMetadata || uri.scheme === Schemas.vibecodaNotebookCellMetadataDiff
		) {
		...
		if (uri.scheme === Schemas.vibecodaNotebookMetadata) {
		...
		return {
			setUri(uri, options = {}) {
				if (!uri) {
					...
					if (uri.scheme === Schemas.vibecodaNotebookCell) {
						const notebookDocument = uri.scheme === Schemas.vibecodaNotebookCell ? that.notebookDocumentService.getNotebook(uri) : undefined;
						const cellIndex = Schemas.vibecodaNotebookCell ? that.notebookDocumentService.getNotebook(uri)?.getCellIndex(uri) : undefined;
					...
					} else if (uri.scheme === Schemas.vibecodaNotebookCellMetadata || uri.scheme === Schemas.vibecodaNotebookCellMetadataDiff) {
						description = localize('notebookCellMetadataLabel', "Metadata");
					} else if (uri.scheme === Schemas.vibecodaNotebookCellOutput || uri.scheme === Schemas.vibecodaNotebookCellOutputDiff) {
