...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
		...
		if (rendererMessaging) {
			...
			rendererMessaging.receiveMessageHandler = (rendererId, message) => {
				...
				this._sendMessageToWebview({
					__vibecoda_notebook_message: true,
			...
			<head>
				...
				<style nonce="${this.nonce}">
					::highlight(find-highlight) {
						background-color: var(--vibecoda-editor-findMatchBackground, ${findMatchHighlight});
					}

					::highlight(current-find-highlight) {
						background-color: var(--vibecoda-editor-findMatchHighlightBackground, ${currentHighlight});
					}
					...
					#container .cell_container.nb-insertHighlight div.output_container div.output {
						background-color: var(--vibecoda-diffEditor-insertedLineBackground, var(--vibecoda-diffEditor-insertedTextBackground));
					}
					...
					#container .nb-chatGenerationHighlight .output_container .output {
						background-color: var(--vibecoda-notebook-selectedCellBackground);
					}
					...
					#container .no-renderer-error {
						color: var(--vibecoda-editorError-foreground);
					}
					...
					.find-match {
						background-color: var(--vibecoda-editor-findMatchHighlightBackground);
					}

					.current-find-match {
						background-color: var(--vibecoda-editor-findMatchBackground);
					}

					#_defaultColorPalatte {
						color: var(--vibecoda-editor-findMatchHighlightBackground);
						background-color: var(--vibecoda-editor-findMatchBackground);
					}
				</style>
			</head>
	...
	private asWebviewUri(uri: URI, fromExtension: URI | undefined) {
		return asWebviewUri(uri, fromExtension?.scheme === Schemas.vibecodaRemote ? { isRemote: true, authority: fromExtension.authority } : undefined);
	}
	...
	private _initialize(content: string, targetWindow: CodeWindow): Promise<void> {
		...
		this._register(this.webview.onMessage(async (message) => {
			const data: FromWebviewMessage | { readonly __vibecoda_notebook_message: undefined } = message.message;
			if (this._disposed) {
				return;
			}

			if (!data.__vibecoda_notebook_message) {
				return;
			}
			...
			switch (data.type) {
				...
				case 'clicked-link': {
					...
					} else if (matchesScheme(data.href, Schemas.vibecodaNotebookCell)) {
						const uri = URI.parse(data.href);
				...
				case 'notebookPerformanceMessage': {
					...
					if (data.outputSize && data.rendererId === 'vibecoda.builtin-renderer') {
	...
	private createMetadata(output: ICellOutput, mimeType: string) {
		if (mimeType.startsWith('image')) {
			...
			if (buffer?.length && buffer?.length > 0) {
				...
				return { ...output.metadata, vibecoda_altText: altText };
			}
		}
