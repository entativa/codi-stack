...
import type * as rendererApi from 'vibecoda-notebook-renderer';
...
async function webviewPreloads(ctx: PreloadContext) {
	...
	const handleWheel = (event: WheelEvent & { wheelDeltaX?: number; wheelDeltaY?: number; wheelDelta?: number }) => {
		...
		postNotebookMessage<webviewMessages.IWheelMessage>('did-scroll-wheel', {
			payload: {
				...
				deltaY: event.deltaY,
				deltaZ: event.deltaZ,
				// Refs https://github.com/microsoft/vibecoda/issues/146403#issuecomment-1854538928
	...
	const renderFallbackErrorName = 'vibecoda.fallbackToNextRenderer';
	...
	class Renderer {
		...
		private createRendererContext(): RendererContext {
			...
			const context: RendererContext = {
				setState: newState => vibecoda.setState({ ...vibecoda.getState(), [id]: newState }),
				getState: <T>() => {
					const state = vibecoda.getState();
	...
	class MarkdownCodeBlock {
		...
		public static highlightCodeBlock(id: string, html: string) {
			...
			el.innerHTML = trustedHtml as string; // CodeQL [SM03712] The rendered content comes from Vibecoda's tokenizer and is considered safe
		...
		public static requestHighlightCodeBlock(root: HTMLElement | ShadowRoot) {
			...
			let i = 0;
			for (const el of root.querySelectorAll('.vibecoda-code-block')) {
				const lang = el.getAttribute('data-vibecoda-code-block-lang');
	...
	class OutputContainer {
		...
		constructor(
			...
			this.element.setAttribute('data-vibecoda-context', JSON.stringify({ 'preventDefaultContextMenuItems': true }));
			this.element.style.position = 'absolute';
	...
	function postNotebookMessage<T extends webviewMessages.FromWebviewMessage>(
		type: T['type'],
		properties: Omit<T, '__vibecoda_notebook_message' | 'type'>
	) {
		vibecoda.postMessage({
			__vibecoda_notebook_message: true,
			type,
