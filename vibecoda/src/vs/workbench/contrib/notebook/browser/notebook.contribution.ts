...
class CellInfoContentProvider {
	...
	constructor(
		...
		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.vibecodaNotebookCellMetadata, {
			provideTextContent: this.provideMetadataTextContent.bind(this)
		}));

		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.vibecodaNotebookCellOutput, {
			provideTextContent: this.provideOutputTextContent.bind(this)
		}));

		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.vibecodaNotebookCellMetadata,
			formatting: {
				label: '${path} (metadata)',
				separator: '/'
			}
		...
		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.vibecodaNotebookCellOutput,
			formatting: {
				label: '${path} (output)',
				separator: '/'
			}
	...
	async provideMetadataTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const data = CellUri.parseCellPropertyUri(resource, Schemas.vibecodaNotebookCellMetadata);
		if (!data) {
			return null;
		}
	...
	async provideOutputsTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const data = CellUri.parseCellPropertyUri(resource, Schemas.vibecodaNotebookCellOutput);
		if (!data) {
			return null;
		}
...
class NotebookMetadataContentProvider {
	...
	constructor(
		...
		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.vibecodaNotebookMetadata, {
			provideTextContent: this.provideMetadataTextContent.bind(this)
		}));

		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.vibecodaNotebookMetadata,
			formatting: {
				label: '${path} (metadata)',
				separator: '/'
			}
...
class RegisterSchemasContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerMetadataSchemas(): void {
		...
		jsonRegistry.registerSchema('vibecoda://schemas/notebook/cellmetadata', metadataSchema);
	}
}
