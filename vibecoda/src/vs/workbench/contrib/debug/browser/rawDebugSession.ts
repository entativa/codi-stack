 ...
 */
interface ILaunchVibecodaArgument {
	prefix?: string;
	path?: string;
}

interface ILaunchVibecodaArguments {
	args: ILaunchVibecodaArgument[];
...
export class RawDebugSession implements IDisposable {
	...
	private async dispatchRequest(request: DebugProtocol.Request): Promise<void> {
		...
		if (request.command === 'launchVibecoda') {
			try {
				let result = await this.launchVsCode(<ILaunchVibecodaArguments>request.arguments);
				if (!result.success) {
					...
					if (confirmed) {
						result = await this.launchVsCode(<ILaunchVibecodaArguments>request.arguments);
					} else {
	...
	private launchVsCode(vibecodaArgs: ILaunchVibecodaArguments): Promise<IOpenExtensionWindowResult> {

		const args: string[] = [];

		for (const arg of vibecodaArgs.args) {
		...
		if (vibecodaArgs.env) {
			args.push(`--extensionEnvironment=${JSON.stringify(vibecodaArgs.env)}`);
		}

		return this.extensionHostDebugService.openExtensionDevelopmentHostWindow(args, !!vibecodaArgs.debugRenderer);
	}
