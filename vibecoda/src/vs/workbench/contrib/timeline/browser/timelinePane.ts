...
function isTimelineItem(item: TreeElement | undefined): item is TimelineItem {
	return !!item && !item.handle.startsWith('vibecoda-command:');
}
...
class LoadMoreCommand {
	readonly handle = 'vibecoda-command:loadMore';
...
export class TimelinePane extends ViewPane {
	...
	private async loadTimeline(reset: boolean, sources?: string[]) {
		...
		if (sources === undefined) {
			...
			if (this.uri?.scheme === Schemas.vibecodaSettings || this.uri?.scheme === Schemas.webviewPanel || this.uri?.scheme === Schemas.walkThrough) {
				this.uri = undefined;
