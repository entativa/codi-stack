...
export class VibecodaWorkspace extends ObservableWorkspace implements IDisposable {
	...
	constructor(
		...
		const documents = mapObservableArrayCached(this, models, (m, store) => {
			...
			return store.add(new VibecodaDocument(m));
...
export class VibecodaDocument extends Disposable implements IObservableDocument {
