...
suite('terminalUri', () => {
	...
	suite('getTerminalResourcesFromDragEvent', () => {
		...
		test('should return terminal resource when event contains one', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["vibecoda-terminal:/1626874386474/3"]'))?.map(e => e.toString()),
				['vibecoda-terminal:/1626874386474/3']
			);
		});
		test('should return multiple terminal resources when event contains multiple', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["vibecoda-terminal:/foo/1","vibecoda-terminal:/bar/2"]'))?.map(e => e.toString()),
				['vibecoda-terminal:/foo/1', 'vibecoda-terminal:/bar/2']
			);
		});
	});
