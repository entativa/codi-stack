...
export class TerminalNativeContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		ipcRenderer.on('vibecoda:openFiles', (_: unknown, ...args: unknown[]) => { this._onOpenFileRequest(args[0] as INativeOpenFileRequest); });
	...
	private async _onOpenFileRequest(request: INativeOpenFileRequest): Promise<void> {
		...
		if (request.termProgram === 'vibecoda' && request.filesToWait) {
