...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	constructor(
		...
		this.raw = this._register(new xtermCtor({
			...
			logLevel: vibecodaToXtermLogLevel(this._logService.getLevel()),
			logger: this._logService,
			minimumContrastRatio: config.minimumContrastRatio,
			tabStopWidth: config.tabStopWidth,
			cursorBlink: config.cursorBlinking,
			cursorStyle: vibecodaToXtermCursorStyle<'cursorStyle'>(config.cursorStyle),
			cursorInactiveStyle: vibecodaToXtermCursorStyle(config.cursorStyleInactive),
		...
		this._register(this._logService.onDidChangeLogLevel(e => this.raw.options.logLevel = vibecodaToXtermLogLevel(e)));
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	private _updateFindColors(searchOptions: ISearchOptions): void {
		...
		// Theme color names align with monaco/vibecoda whereas xterm.js has some different naming.
	...
	private _setCursorStyle(style: ITerminalConfiguration['cursorStyle']): void {
		const mapped = vibecodaToXtermCursorStyle<'cursorStyle'>(style);
	...
	private _setCursorStyleInactive(style: ITerminalConfiguration['cursorStyleInactive']): void {
		const mapped = vibecodaToXtermCursorStyle(style);
...
function vibecodaToXtermLogLevel(logLevel: LogLevel): XtermLogLevel {
	switch (logLevel) {
...
function vibecodaToXtermCursorStyle<T extends 'cursorStyle' | 'cursorStyleInactive'>(style: ITerminalConfiguration[T]): ICursorStyleVscodeToXtermMap[T] {
	// 'line' is used instead of bar in Vibecoda to be consistent with editor.cursorStyle
	if (style === 'line') {
		return 'bar';
	}
