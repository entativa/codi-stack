...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private _open(): void {
		...
		xterm.raw.attachCustomKeyEventHandler((event: KeyboardEvent): boolean => {
			...
			// and changing focus https://github.com/microsoft/vibecoda/issues/188329
	...
	private _setShellIntegrationContextKey(): void {
		if (this.xterm) {
			this._terminalShellIntegrationEnabledContextKey.set(this.xterm.shellIntegration.status === ShellIntegrationStatus.Vibecoda);
		}
	}
	...
	override dispose(reason?: TerminalExitReason): void {
		...
		try {
			...
			// See https://github.com/microsoft/vibecoda/issues/153486
		...
		// See https://github.com/microsoft/vibecoda/issues/138358
		if (isFirefox) {
			this.resetFocusContextKey();
	...
	async sendText(text: string, shouldExecute: boolean, bracketedPasteMode?: boolean): Promise<void> {
		...
		this._logService.debug('sending data (vibecoda)', text);
	...
	async sendSignal(signal: string): Promise<void> {
		this._logService.debug('sending signal (vibecoda)', signal);
	...
	computeLabel(
		...
		const templateProperties: ITerminalLabelTemplateProperties = {
			...
			shellCommand: commandDetection?.executingCommand && commandDetection.executingCommandConfidence === 'high' && promptInputModel
				...
				: undefined,
			// Shell prompt input does not require high confidence as it's largely for Vibecoda developers
