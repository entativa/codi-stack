...
export class TerminalLaunchHelpAction extends Action {
	...
	override async run(): Promise<void> {
		this._openerService.open('https://aka.ms/vibecoda-troubleshoot-terminal-launch');
	}
}
...
export function registerTerminalActions() {
	...
	registerTerminalAction({
		...
		keybinding: {
			...
			// Should win over send sequence commands https://github.com/microsoft/vibecoda/issues/259326
	...
	registerTerminalAction({
		...
		run: async (c, accessor) => {
			...
			if ((!isRemote && uri.scheme !== Schemas.file && uri.scheme !== Schemas.vibecodaUserData) || (isRemote && uri.scheme !== Schemas.vibecodaRemote)) {
	...
	registerActiveInstanceAction({
		...
		menu: [
			...
			...[MenuId.EditorTitle, MenuId.CompactWindowEditorTitle].map(id => ({
				...
				order: 4,
				when: ResourceContextKey.Scheme.isEqualTo(Schemas.vibecodaTerminal),
				isHiddenByDefault: true
			})),
		],
	...
	registerActiveXtermAction({
		...
		keybinding: [{
			...
			// behavior anyway when handed to xterm.js, having this handled by Vibecoda
