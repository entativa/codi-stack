...
suite('PromptValidator', () => {
	...
	setup(async () => {
		...
		const prExtSource = { type: 'extension', label: 'GitHub Pull Request Extension', extensionId: new ExtensionIdentifier('github.vibecoda-pull-request-github') } satisfies ToolDataSource;
	...
	suite('agents', () => {
		...
		test('unknown attribute in agent file', async () => {
			...
			assert.deepStrictEqual(
				markers.map(m => ({ severity: m.severity, message: m.message })),
				[
					{ severity: MarkerSeverity.Warning, message: `Attribute 'applyTo' is not supported in Vibecoda agent files. Supported: argument-hint, description, handoffs, model, name, target, tools.` },
				]
			);
		});
		...
		test('vibecoda target agent validates normally', async () => {
			const content = [
				'---',
				'description: "Vibecoda agent"',
				'target: vibecoda',
				'model: MAE 4.1',
				`tools: ['tool1', 'tool2']`,
				'---',
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'Vibecoda target should validate normally');
		});

		test('vibecoda target agent warns about unknown tools', async () => {
			const content = [
				'---',
				'description: "Vibecoda agent"',
				'target: vibecoda',
		...
		test('default target (no target specified) validates as vibecoda', async () => {
			const content = [
				'---',
			...
			const markers = await validate(content, PromptsType.agent);
			// Should validate normally as if target was vibecoda
			assert.deepStrictEqual(markers, [], 'Agent without target should validate as vibecoda');
		});
		...
		test('github-copilot target requires name attribute', async () => {
			...
			// Missing name with vibecoda target (should be optional)
			{
				const content = [
					'---',
					'description: "Vibecoda agent"',
				...
				const markers = await validate(content, PromptsType.agent);
				assert.deepStrictEqual(markers, [], 'Name should be optional for vibecoda target');
			}
		});
	});
	...
	suite('body', () => {
		...
		test('body with tool not present in tools list', async () => {
			const content = [
				...
				'---',
				'I need',
				'#tool:ms-azuretools.vibecoda-azure-github-copilot/azure_recommend_custom_modes',
				'#tool:github.vibecoda-pull-request-github/suggest-fix',
			...
			assert.deepEqual(actual, [
				{ message: `Unknown tool or toolset 'ms-azuretools.vibecoda-azure-github-copilot/azure_recommend_custom_modes'.`, startColumn: 7, endColumn: 77 },
				{ message: `Tool or toolset 'github.vibecoda-pull-request-github/suggest-fix' also needs to be enabled in the header.`, startColumn: 7, endColumn: 52 },
