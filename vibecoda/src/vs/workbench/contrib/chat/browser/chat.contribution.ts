...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.AutoApproveEdits]: {
			...
			markdownDescription: nls.localize('chat.tools.autoApprove.edits', "Controls whether edits made by chat are automatically approved. The default is to approve all edits except those made to certain files which have the potential to cause immediate unintended side-effects, such as `**/.vibecoda/*.json`.\n\nSet to `true` to automatically approve edits to matching files, `false` to always require explicit approval. The last pattern matching a given file will determine whether the edit is automatically approved."),
			type: 'object',
			additionalProperties: {
				type: 'boolean',
			}
		...
		[mcpAccessConfig]: {
			...
			enumDescriptions: [
				...
				nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that Vibecoda is connected to."),
			...
			policy: {
				...
				localization: {
					...
					enumDescriptions: [
						...
						{
							key: 'chat.mcp.access.registry', value: nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that Vibecoda is connected to."),
						},
						{
		...
		[PromptsConfig.INSTRUCTIONS_LOCATION_KEY]: {
			...
			examples: [
				...
				{
					[INSTRUCTIONS_DEFAULT_SOURCE_FOLDER]: true,
					'/Users/vibecoda/repos/instructions': true,
				},
			],
		},
		[PromptsConfig.PROMPT_LOCATIONS_KEY]: {
			...
			examples: [
				...
				{
					[PROMPT_DEFAULT_SOURCE_FOLDER]: true,
					'/Users/vibecoda/repos/prompts': true,
				},
			],
		},
		[PromptsConfig.MODE_LOCATION_KEY]: {
			...
			examples: [
				...
				{
					[LEGACY_MODE_DEFAULT_SOURCE_FOLDER]: true,
					'/Users/vibecoda/repos/chatmodes': true,
				},
			],
		},
		...
		[PromptsConfig.PROMPT_FILES_SUGGEST_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Configure which prompt files to recommend in the chat welcome view. Each key is a prompt file name, and the value can be `true` to always recommend, `false` to never recommend, or a [when clause](https://aka.ms/vibecoda-when-clause) expression like `resourceExtname == .js` or `resourceLangId == markdown`.",
			),
			default: {},
			additionalProperties: {
				oneOf: [
...
class ChatResolverContribution extends Disposable {
	...
	constructor(
		...
		this._registerEditor(Schemas.vibecodaChatEditor);
		this._registerEditor(Schemas.vibecodaLocalChatSession);
