...
class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerBuiltInAgents(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): IDisposable {
		return instantiationService.invokeFunction(accessor => {
			...
			// Register Vibecoda agent
			const { disposable: vibecodaDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vibecoda', 'vibecoda', false, localize2('vibecodaAgentDescription', "Ask questions about Vibecoda").value, ChatAgentLocation.Chat, undefined, context, controller);
			disposables.add(vibecodaDisposable);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				modelDescription: localize('setupToolsDescription', "Scaffold a new workspace in Vibecoda"),
				userDescription: localize('setupToolsDescription', "Scaffold a new workspace in Vibecoda"),
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSetupAgents(context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): void {
		...
		const vibecodaAgentDisposables = markAsSingleton(new MutableDisposable());
		...
		const updateRegistration = () => {
			...
			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabled) {
					...
					if ((!context.state.installed || context.state.entitlement === ChatEntitlement.Unknown || context.state.entitlement === ChatEntitlement.Unresolved) && !vibecodaAgentDisposables.value) {
						const disposables = vibecodaAgentDisposables.value = new DisposableStore();
					...
					vibecodaAgentDisposables.clear();
				}

				if (context.state.installed && !context.state.disabled) {
					vibecodaAgentDisposables.clear(); // we need to do this to prevent showing duplicate agent/tool entries in the list
				}
			}
