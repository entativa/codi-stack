	...
	*/
	...
	constructor(
		...
		this._register(codeEditorService.registerCodeEditorOpenHandler(async (input: ITextResourceEditorInput, _source: ICodeEditor | null, _sideBySide?: boolean): Promise<ICodeEditor | null> => {
			...
			if (resource.scheme !== Schemas.vibecodaChatCodeBlock) {
				return null;
			}
	...
	render(parent: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			this.container.style.setProperty('--vibecoda-chat-font-family', fontFamily);
	...
	private _getGenerateInstructionsMessage(): IMarkdownString {
		...
		if (!this._instructionFilesCheckPromise) {
			...
			// Use Vibecoda's idiomatic pattern for disposal-safe promise callbacks
	...
	private getPromptFileSuggestions(): IChatSuggestedPrompts[] {
		...
		if (!this.chatEntitlementService.sentiment.installed) {
			...
			if (isEmpty) {
				return [
					{
						icon: Codicon.vibecoda,
						label: localize('chatWidget.suggestedPrompts.gettingStarted', "Ask @vibecoda"),
						prompt: localize('chatWidget.suggestedPrompts.gettingStartedPrompt', "@vibecoda How do I change the theme to light mode?"),
					},
					{
	...
	private onDidStyleChange(): void {
		this.container.style.setProperty('--vibecoda-interactive-result-editor-background-color', this.editorOptions.configuration.resultEditor.backgroundColor?.toString() ?? '');
		this.container.style.setProperty('--vibecoda-interactive-session-foreground', this.editorOptions.configuration.foreground?.toString() ?? '');
		this.container.style.setProperty('--vibecoda-chat-list-background', this.themeService.getColorTheme().getColor(this.styles.listBackground)?.toString() ?? '');
	}
