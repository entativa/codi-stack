...
abstract class AttachResourceAction extends Action2 {
	...
	protected _getResources(accessor: ServicesAccessor, ...args: unknown[]): URI[] {
		...
		for (const context of contexts) {
			...
			if (uri && [Schemas.file, Schemas.vibecodaRemote, Schemas.untitled].includes(uri.scheme)) {
				files.push(uri);
			}
		}
			...
			menu: [{
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.or(
						...
						ResourceContextKey.Scheme.isEqualTo(Schemas.vibecodaRemote)
					)
				),
			}, {
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.or(
						...
						ResourceContextKey.Scheme.isEqualTo(Schemas.vibecodaUserData)
					)
				)
			}]
		});
...
class AttachSelectionToChatAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
		...
		if (matches && matches.length > 0) {
			...
			if (activeEditor && activeUri && [Schemas.file, Schemas.vibecodaRemote, Schemas.untitled].includes(activeUri.scheme)) {
