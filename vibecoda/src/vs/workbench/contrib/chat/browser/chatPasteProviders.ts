...
export class PasteImageProvider implements DocumentPasteEditProvider {
	...
	constructor(
		...
		this.imagesFolder = joinPath(this.environmentService.workspaceStorageHome, 'vibecoda-chat-images');
...
export class CopyTextProvider implements DocumentPasteEditProvider {
	...
	async prepareDocumentPaste(model: ITextModel, ranges: readonly IRange[], dataTransfer: IReadonlyVSDataTransfer, token: CancellationToken): Promise<undefined | IReadonlyVSDataTransfer> {
		if (model.uri.scheme === Schemas.vibecodaChatInput) {
			return;
		}
...
export class PasteTextProvider implements DocumentPasteEditProvider {
	...
	async provideDocumentPasteEdits(model: ITextModel, ranges: readonly IRange[], dataTransfer: IReadonlyVSDataTransfer, _context: DocumentPasteContext, token: CancellationToken): Promise<DocumentPasteEditsSession | undefined> {
		...
		const text = dataTransfer.get(Mimes.text);
		const editorData = dataTransfer.get('vibecoda-editor-data');
...
export class ChatPasteProvidersFeature extends Disposable {
	constructor(
		...
		super();
		this._register(languageFeaturesService.documentPasteEditProvider.register({ scheme: Schemas.vibecodaChatInput, pattern: '*', hasAccessToAllModels: true }, instaService.createInstance(CopyAttachmentsProvider)));
		this._register(languageFeaturesService.documentPasteEditProvider.register({ scheme: Schemas.vibecodaChatInput, pattern: '*', hasAccessToAllModels: true }, new PasteImageProvider(chatWidgetService, extensionService, fileService, environmentService, logService)));
		this._register(languageFeaturesService.documentPasteEditProvider.register({ scheme: Schemas.vibecodaChatInput, pattern: '*', hasAccessToAllModels: true }, new PasteTextProvider(chatWidgetService, modelService)));
