			...
			(() => {
				...
				if (notebookEditor) {
					...
					activeEditorDisposables.add(notebookEditor.onDidChangeActiveCell(() => {
						...
						if (codeEditor && codeEditor.getModel()?.uri.scheme === Schemas.vibecodaNotebookCell) {
...
export class ChatImplicitContext extends Disposable implements IChatRequestImplicitVariableEntry {
	get id() {
		if (URI.isUri(this.value)) {
			return 'vibecoda.implicit.file';
		} else if (isStringImplicitContextValue(this.value)) {
			return 'vibecoda.implicit.string';
		} else if (this.value) {
			if (this._isSelection) {
				return 'vibecoda.implicit.selection';
			} else {
				return 'vibecoda.implicit.viewport';
			}
		} else {
			return 'vibecoda.implicit';
		}
	}
