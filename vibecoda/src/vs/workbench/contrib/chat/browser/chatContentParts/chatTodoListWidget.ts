...
class TodoListRenderer implements IListRenderer<IChatTodo, ITodoListTemplate> {
	...
	private getStatusIconColor(status: string): string {
		switch (status) {
			case 'completed':
				return 'var(--vibecoda-charts-green)';
			case 'in-progress':
				return 'var(--vibecoda-charts-blue)';
			case 'not-started':
			default:
				return 'var(--vibecoda-foreground)';
		}
	}
}
		...
		if (this._isExpanded) {
			...
			if (todoToShow) {
				...
				if (todoToShow === firstInProgressTodo) {
					...
					icon.style.color = 'var(--vibecoda-charts-blue)';
				} else {
					icon.classList.add('codicon-circle-outline');
					icon.style.color = 'var(--vibecoda-foreground)';
				}
