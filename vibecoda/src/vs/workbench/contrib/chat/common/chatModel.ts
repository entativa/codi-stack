...
export class Response extends AbstractResponse implements IDisposable {
	...
	updateContent(progress: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit | IChatTask, quiet?: boolean): void {
		if (progress.kind === 'clearToPreviousToolInvocation') {
			...
			const useOldApproachForInlineNotebook = progress.uri.scheme === Schemas.vibecodaNotebookCell && !this._responseParts.find(part => part.kind === 'notebookEditGroup');
		...
		try {
			return requests.map((raw: ISerializableChatRequestData) => {
				...
				if (raw.response || raw.result || (raw as any).responseErrorDetails) {
					...
					if (raw.usedContext) { // @ulugbekna: if this's a new vibecoda sessions, doc versions are incorrect anyway?
...
export namespace ChatResponseResource {
	export const scheme = 'vibecoda-chat-response-resource';
