...
export class PromptValidator {
				...
				switch (promptType) {
					...
					case PromptsType.agent:
						if (isGitHubTarget) {
							...
							report(toMarker(localize('promptValidator.unknownAttribute.vibecoda-agent', "Attribute '{0}' is not supported in Vibecoda agent files. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Warning));
						}
						break;
					case PromptsType.instructions:
		...
		switch (attribute.value.type) {
			case 'array':
				if (isGitHubTarget) {
					...
					this.validateVibecodaTools(attribute.value, report);
				}
				break;
			default:
	...
	private validateVibecodaTools(valueItem: IArrayValue, report: (markers: IMarkerData) => void) {
	...
	private validateTarget(attributes: IHeaderAttribute[], report: (markers: IMarkerData) => void): undefined {
		...
		const validTargets = ['github-copilot', 'vibecoda'];
