				...
				if (attribute.range.containsPosition(position)) {
					switch (attribute.key) {
						...
						case PromptHeaderAttributes.target:
							return this.createHover(localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `github-copilot` and `vibecoda`.'), attribute.range);
					}
				}
			}
		} else {
