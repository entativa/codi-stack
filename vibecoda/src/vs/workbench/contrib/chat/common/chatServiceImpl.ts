...
export class ChatService extends Disposable implements IChatService {
	...
	async activateDefaultAgent(location: ChatAgentLocation): Promise<void> {
		...
		// https://github.com/microsoft/vibecoda/issues/250590
	...
	async loadSessionForResource(chatSessionResource: URI, location: ChatAgentLocation, token: CancellationToken): Promise<IChatModel | undefined> {
		...
		if (chatSessionResource.scheme === Schemas.vibecodaLocalChatSession) {
		...
		const sendRequestInternal = async () => {
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					// MCP autostart: only run for native Vibecoda sessions (sidebar, new editors) but not for extension contributed sessions that have inputType set.
