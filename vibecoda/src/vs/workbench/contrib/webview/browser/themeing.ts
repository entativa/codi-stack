	...
	public getWebviewThemeData(): WebviewThemeData {
		if (!this._cachedWebViewThemeData) {
			...
			const exportedColors = colorRegistry.getColorRegistry().getColors().reduce<Record<string, string>>((colors, entry) => {
				...
				if (color) {
					colors['vibecoda-' + entry.id.replace('.', '-')] = color.toString();
				}
				return colors;
			}, {});

			const styles = {
				'vibecoda-font-family': DEFAULT_FONT_FAMILY,
				'vibecoda-font-weight': 'normal',
				'vibecoda-font-size': '13px',
				'vibecoda-editor-font-family': editorFontFamily,
				'vibecoda-editor-font-weight': editorFontWeight,
				'vibecoda-editor-font-size': editorFontSize + 'px',
				'text-link-decoration': linkUnderlines ? 'underline' : 'none',
				...exportedColors,
				'vibecoda-editor-font-feature-settings': editorFontLigatures,
			};
...
enum ApiThemeClassName {
	light = 'vibecoda-light',
	dark = 'vibecoda-dark',
	highContrast = 'vibecoda-high-contrast',
	highContrastLight = 'vibecoda-high-contrast-light',
}
