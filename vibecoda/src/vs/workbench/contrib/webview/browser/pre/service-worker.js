...
const resourceCacheName = `vibecoda-resource-cache-${VERSION}`;
 ...
 */
const resourceBaseAuthority = searchParams.get('vibecoda-resource-base-authority');
...
sw.addEventListener('fetch', (event) => {
	...
	// through Vibecoda itself so that we are authenticated properly.  If the
...
async function processResourceRequest(
	...
	// Refs https://github.com/microsoft/vibecoda/issues/244143
	...
	const resolveResourceEntry = (result, cachedResponse) => {
		...
		const coiRequest = new URL(event.request.url).searchParams.get('vibecoda-coi');
		if (coiRequest === '3') {
