			...
			<head>
				...
				<style nonce="${nonce}">
					...
					code:has(.codesetting) {
						background-color: var(--vibecoda-textPreformat-background);
						color: var(--vibecoda-textPreformat-foreground);
						padding-left: 1px;
						margin-right: 3px;
						padding-right: 0px;
					}
					...
					code:has(.codesetting):focus {
						border: 1px solid var(--vibecoda-button-border, transparent);
					}
					...
					.codesetting svg {
						...
						text-align: center;
						cursor: pointer;
						border: 1px solid var(--vibecoda-button-secondaryBorder, transparent);
					...
					.codesetting:hover {
						color: var(--vibecoda-textPreformat-foreground) !important;
					...
					.codesetting:focus {
						outline: 0 !important;
						text-decoration: none !important;
						color: var(--vibecoda-button-hoverForeground) !important;
					}
					.codesetting .separator {
						...
						margin-bottom: -3px;
						display: inline-block;
						background-color: var(--vibecoda-editor-background);
						font-size: 12px;
						margin-right: 4px;
					}
					...
					/* Release notes enhancements from vibecoda-docs */
					html {
						font-size: 10px;
						height: 100%;
					...
					#scroll-to-top {
						...
						right: 25px;
						bottom: 25px;
						background-color: var(--vibecoda-button-background, #444);
						border-color: var(--vibecoda-button-border);
						border-radius: 50%;
						cursor: pointer;
					...
					#scroll-to-top:hover {
						background-color: var(--vibecoda-button-hoverBackground);
						box-shadow: 2px 2px 2px rgba(0,0,0,.25);
					}

					body.vibecoda-high-contrast #scroll-to-top {
						border-width: 2px;
						border-style: solid;
						box-shadow: none;
					}
					...
					#scroll-to-top span.icon::before {
						content: "";
						display: block;
						background: var(--vibecoda-button-foreground);
						/* Chevron up icon */
					...
					#toc-nav a {
						color: var(--vibecoda-editor-foreground, #ccc);
						text-decoration: none !important;
					...
					#toc-nav a:hover {
						background-color: var(--vibecoda-button-secondaryHoverBackground, #1177bb);
						color: var(--vibecoda-button-secondaryForeground, #ffffff);
						cursor: pointer;
						text-decoration: none !important;
					}
				...
				<script nonce="${nonce}">
					...
					window.addEventListener('click', event => {
						...
						if (href && (href.startsWith('${Schemas.codeSetting}'))) {
							vibecoda.postMessage({ type: 'clickSetting', value: { uri: href, x: event.clientX, y: event.clientY }});
						}
					});
					...
					window.addEventListener('keypress', event => {
						if (event.keyCode === 13) {
							if (event.target.children.length > 0 && event.target.children[0].href) {
								...
								vibecoda.postMessage({ type: 'clickSetting', value: { uri: event.target.children[0].href, x: clientRect.right , y: clientRect.bottom }});
							}
						}
					});

					input.addEventListener('change', event => {
						vibecoda.postMessage({ type: 'showReleaseNotes', value: input.checked }, '*');
					});
				</script>
			</body>
		</html>`;
