...
export class InteractiveDocumentContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		editorResolverService.registerEditor(
			`${Schemas.vibecodaInteractiveInput}:/**`,
			{
				id: 'vibecoda-interactive-input',
				label: 'Interactive Editor',
				priority: RegisteredEditorPriority.exclusive
			},
			{
				canSupportResource: uri => uri.scheme === Schemas.vibecodaInteractiveInput,
		...
		editorResolverService.registerEditor(
			...
			},
			{
				canSupportResource: uri =>
					...
					(uri.scheme === Schemas.vibecodaNotebookCell && extname(uri) === '.interactive'),
				singlePerResource: true
			},
			{
...
class InteractiveInputContentProvider implements ITextModelContentProvider {
	...
	constructor(
		...
		this._registration = textModelService.registerTextModelContentProvider(Schemas.vibecodaInteractiveInput, this);
	}
...
function createEditor(resource: URI, instantiationService: IInstantiationService): EditorInput {
	...
	const inputUri = URI.from({ scheme: Schemas.vibecodaInteractiveInput, path: inputBoxPath });
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor, showOptions?: number | { viewColumn?: number; preserveFocus?: boolean }, resource?: URI, id?: string, title?: string): Promise<{ notebookUri: URI; inputUri: URI; notebookEditorId?: string }> {
		...
		do {
			...
			inputUri = URI.from({ scheme: Schemas.vibecodaInteractiveInput, path: `/InteractiveInput-${counter}` });
