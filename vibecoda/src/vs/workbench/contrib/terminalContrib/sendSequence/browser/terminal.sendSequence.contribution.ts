...
export const terminalSendSequenceCommand = async (accessor: ServicesAccessor, args: unknown) => {
	...
	if (instance) {
		...
		const activeWorkspaceRootUri = historyService.getLastActiveWorkspaceRoot(instance.hasRemoteAuthority ? Schemas.vibecodaRemote : Schemas.file);
...
// https://github.com/microsoft/vibecoda/issues/190629). Note that macOS uses different sequences here
