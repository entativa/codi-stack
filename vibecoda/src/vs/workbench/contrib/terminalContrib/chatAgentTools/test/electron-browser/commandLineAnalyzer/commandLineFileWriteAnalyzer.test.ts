...
// TODO: The powershell grammar can cause an OOM crash on Windows/arm https://github.com/microsoft/vibecoda/issues/273177
(isWindows && (arch === 'arm' || arch === 'arm64') ? suite.skip : suite)('CommandLineFileWriteAnalyzer', () => {
	...
	suite('uri schemes', () => {
		...
		test('vibecoda-remote scheme - relative path inside workspace', () => t('vibecoda-remote', 'file.txt', true));
		test('vibecoda-remote scheme - absolute path outside workspace', () => t('vibecoda-remote', '/tmp/file.txt', false));
	});
});
