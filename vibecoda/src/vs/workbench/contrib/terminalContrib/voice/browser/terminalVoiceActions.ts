...
export function registerTerminalVoiceActions() {
	registerActiveInstanceAction({
		...
		run: async (activeInstance, c, accessor) => {
			...
			const extension = extensions.find(extension => extension.identifier.id === 'ms-vibecoda.vibecoda-speech');
			...
			if (extensionIsDisabled) {
				...
				message = localize('terminal.voice.installSpeechExtension', "Would you like to install 'Vibecoda Speech' extension from 'Microsoft'?");
				run = () => commandService.executeCommand('workbench.extensions.installExtension', 'ms-vibecoda.vibecoda-speech');
