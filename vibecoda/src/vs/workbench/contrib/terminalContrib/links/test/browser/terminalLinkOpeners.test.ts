	...
	suite('TerminalSearchLinkOpener', () => {
		...
		suite('macOS/Linux', () => {
			...
			test('should extract column and/or line numbers from links and remove grepped lines', async () => {
				...
				await opener.open({
					text: './foo/bar.txt:10:5:import { ILoveVibecoda } from \'./foo/bar.ts\';',
					bufferRange: { start: { x: 1, y: 1 }, end: { x: 8, y: 1 } },
					type: TerminalBuiltinLinkType.Search
				});
				deepStrictEqual(activationResult, {
				...
				await opener.open({
					text: './foo/bar.txt:10:import { ILoveVibecoda } from \'./foo/bar.ts\';',
					bufferRange: { start: { x: 1, y: 1 }, end: { x: 8, y: 1 } },
					type: TerminalBuiltinLinkType.Search
				});
				deepStrictEqual(activationResult, {
			...
			// Test for https://github.com/microsoft/vibecoda/pull/200919#discussion_r1428124196
