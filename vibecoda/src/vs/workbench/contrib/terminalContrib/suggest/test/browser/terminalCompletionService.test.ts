	...
	suite('~ -> $HOME', () => {
		...
		setup(() => {
			...
			validResources = [
				...
				URI.parse('file:///home'),
				URI.parse('file:///home/vibecoda'),
				URI.parse('file:///home/vibecoda/foo'),
				URI.parse('file:///home/vibecoda/bar.txt'),
			];
			childResources = [
				{ resource: URI.parse('file:///home/vibecoda'), isDirectory: true },
				{ resource: URI.parse('file:///home/vibecoda/foo'), isDirectory: true },
				{ resource: URI.parse('file:///home/vibecoda/bar.txt'), isFile: true },
			];
		});
		...
		test('~/| should return folder completions relative to $HOME', async () => {
			assertCompletions(await terminalCompletionService.resolveResources(resourceOptions, '~/', 2, provider, capabilities), [
				{ label: '~/', detail: '/home/' },
				{ label: '~/vibecoda/', detail: '/home/vibecoda/' },
			], { replacementRange: [0, 2] });
		});

		test('~/vibecoda/| should return folder completions relative to $HOME/vibecoda', async () => {
			assertCompletions(await terminalCompletionService.resolveResources(resourceOptions, '~/vibecoda/', 9, provider, capabilities), [
				{ label: '~/vibecoda/', detail: '/home/vibecoda/' },
				{ label: '~/vibecoda/foo/', detail: '/home/vibecoda/foo/' },
				{ label: '~/vibecoda/bar.txt', detail: '/home/vibecoda/bar.txt', kind: TerminalCompletionItemKind.File },
