...
suite('LspTerminalModelContentProvider', () => {
	...
	setup(async () => {
		...
		virtualTerminalDocumentUri = URI.from({ scheme: 'vibecodaTerminal', path: '/terminal1.py' });
	...
	suite('setContent', () => {
		...
		test('Mac, Linux - createTerminalLanguageVirtualUri should return the correct URI', () => {
			const expectedUri = URI.from({ scheme: Schemas.vibecodaTerminal, path: '/terminal1.py' });
