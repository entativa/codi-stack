...
export class FileBasedRecommendations extends ExtensionRecommendations {
	...
	private onModelAdded(model: ITextModel): void {
		const uri = model.uri.scheme === Schemas.vibecodaNotebookCell ? CellUri.parse(model.uri)?.notebook : model.uri;
		if (!uri) {
			return;
		}

		const supportedSchemes = distinct([Schemas.untitled, Schemas.file, Schemas.vibecodaRemote, ...this.workspaceContextService.getWorkspace().folders.map(folder => folder.uri.scheme)]);
