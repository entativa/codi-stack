...
`);
	...
	async getChangelog(token: CancellationToken): Promise<string> {
		...
		if (this.type === ExtensionType.System) {
			return Promise.resolve(`Please check the [Vibecoda Release Notes](command:${ShowCurrentReleaseNotesActionId}) for changes to the built-in extensions.`);
		}
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
	...
	private computeExtensionsNotifications(): Array<Omit<IExtensionsNotification, 'dismiss'> & { key: string }> {
		...
		if (invalidExtensions.length) {
			if (invalidExtensions.some(e => e.local && e.local.manifest.engines?.vibecoda &&
				(!isEngineValid(e.local.manifest.engines.vibecoda, this.productService.version, this.productService.date) || areApiProposalsCompatible([...e.local.manifest.enabledApiProposals ?? []]))
			)) {
				computedNotificiations.push({
					...
					if (isSameExtensionRunning) {
						...
						if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
							...
							if (productUpdateVersion
								&& !isEngineValid(extension.local.manifest.engines.vibecoda, productCurrentVersion.version, productCurrentVersion.date)
								&& isEngineValid(extension.local.manifest.engines.vibecoda, productUpdateVersion.version, productUpdateVersion.date)
							) {
	...
	async shouldRequireConsentToUpdate(extension: IExtension): Promise<string | undefined> {
		...
		if (!extension.local.manifest.engines.vibecoda || extension.local.manifest.main || extension.local.manifest.browser) {
			return;
		}
