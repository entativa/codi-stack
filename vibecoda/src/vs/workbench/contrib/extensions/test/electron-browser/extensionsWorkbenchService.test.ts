...
suite('ExtensionsWorkbenchServiceTest', () => {
	...
	test('test for installed extensions', async () => {
		...
		assert.ok(actual.iconUrl === 'file:///localPath1/localIcon1' || actual.iconUrl === 'vibecoda-file://vibecoda-app/localPath1/localIcon1');
	...
	test('Test local UI extension is chosen if it exists in both servers', async () => {
		...
		const extensionKind: ExtensionKind[] = ['ui'];
		const localExtension = aLocalExtension('a', { extensionKind }, { location: URI.file(`pub.a`) });
		const remoteExtension = aLocalExtension('a', { extensionKind }, { location: URI.file(`pub.a`).with({ scheme: Schemas.vibecodaRemote }) });
