...
namespace snippetExt {
	...
	export const snippetsContribution: IJSONSchema = {
		description: localize('vibecoda.extension.contributes.snippets', 'Contributes snippets.'),
		type: 'array',
		...
		items: {
			...
			properties: {
				language: {
					description: localize('vibecoda.extension.contributes.snippets-language', 'Language identifier for which this snippet is contributed to.'),
					type: 'string'
				},
				path: {
					description: localize('vibecoda.extension.contributes.snippets-path', 'Path of the snippets file. The path is relative to the extension folder and typically starts with \'./snippets/\'.'),
					type: 'string'
				}
			}
		}
...
export class SnippetsService implements ISnippetsService {
	...
	private async _initWorkspaceFolderSnippets(workspace: IWorkspace, bucket: DisposableStore): Promise<any> {
		const promises = workspace.folders.map(async folder => {
			const snippetFolder = folder.toResource('.vibecoda');
