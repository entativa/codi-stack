...
export interface ICommandOptionsConfig {
	 ...
	 * If omitted Vibecoda's current workspace root is used.
	 */
	cwd?: string;
...
namespace ConfiguringTask {
	...
	const jake = 'jake.';
	const gulp = 'gulp.';
	const npm = 'vibecoda.npm.';
	const typescript = 'vibecoda.typescript.';
...
namespace CustomTask {
	export function from(this: void, external: ICustomTask, context: IParseContext, index: number, source: TaskConfigSource): Tasks.CustomTask | undefined {
		...
		switch (source) {
			case TaskConfigSource.User: {
				taskSource = { kind: Tasks.TaskSourceKind.User, config: { index, element: external, file: '.vibecoda/tasks.json', workspaceFolder: context.workspaceFolder }, label };
				break;
			}
			case TaskConfigSource.WorkspaceFile: {
				taskSource = { kind: Tasks.TaskSourceKind.WorkspaceFile, config: { index, element: external, file: '.vibecoda/tasks.json', workspaceFolder: context.workspaceFolder, workspace: context.workspace }, label };
				break;
			}
			default: {
				taskSource = { kind: Tasks.TaskSourceKind.Workspace, config: { index, element: external, file: '.vibecoda/tasks.json', workspaceFolder: context.workspaceFolder }, label };
				break;
			}
		}
