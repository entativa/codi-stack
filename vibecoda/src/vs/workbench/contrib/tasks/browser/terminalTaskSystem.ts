...
import { VibecodaOscProperty, VibecodaOscPt, VibecodaSequence } from '../../terminal/browser/terminalEscapeSequences.js';
...
import { serializeVibecodaOscMessage } from '../../../../platform/terminal/common/xterm/shellIntegrationAddon.js';
...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	taskShellIntegrationStartSequence(cwd: string | URI | undefined): string {
		return (
			VibecodaSequence(VibecodaOscPt.Property, `${VibecodaOscProperty.HasRichCommandDetection}=True`) +
			VibecodaSequence(VibecodaOscPt.PromptStart) +
			VibecodaSequence(VibecodaOscPt.Property, `${VibecodaOscProperty.Task}=True`) +
			(cwd
				? VibecodaSequence(VibecodaOscPt.Property, `${VibecodaOscProperty.Cwd}=${typeof cwd === 'string' ? cwd : cwd.fsPath}`)
				: ''
			) +
			VibecodaSequence(VibecodaOscPt.CommandStart)
		);
	}
	getTaskShellIntegrationOutputSequence(commandLineInfo: { commandLine: string; nonce: string } | undefined): string {
		return (
			(commandLineInfo
				? VibecodaSequence(VibecodaOscPt.CommandLine, `${serializeVibecodaOscMessage(commandLineInfo.commandLine)};${commandLineInfo.nonce}`)
				: ''
			) +
			VibecodaSequence(VibecodaOscPt.CommandExecuted)
		);
	}
	...
	private async _executeInTerminal(task: CustomTask | ContributedTask, trigger: string, resolver: VariableResolver, workspaceFolder: IWorkspaceFolder | undefined): Promise<ITaskSummary> {
		...
		if (task.configurationProperties.isBackground) {
			...
			if (problemMatchers.length) {
				...
				// prevent https://github.com/microsoft/vibecoda/issues/174511 from happening
			...
			if (platform === Platform.Platform.Windows) {
				...
				if (!shellSpecified) {
					...
					if (platform === Platform.Platform.Mac) {
						// Background on -l on osx https://github.com/microsoft/vibecoda/issues/107563
...
function taskShellIntegrationWaitOnExitSequence(message: string): (exitCode: number) => string {
	return (exitCode) => {
		return `${VibecodaSequence(VibecodaOscPt.CommandFinished, exitCode.toString())}${message}`;
	};
}
