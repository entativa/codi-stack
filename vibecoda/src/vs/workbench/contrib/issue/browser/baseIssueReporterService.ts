...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		this.issueReporterModel = new IssueReporterModel({
			...
			versionInfo: {
				vibecodaVersion: `${product.nameShort} ${!!product.darwinUniversalAssetId ? `${product.version} (Universal)` : product.version} (${product.commit || 'Commit unknown'}, ${product.date || 'Date unknown'})`,
		...
		const fileOnProduct = data.issueSource === IssueSource.Vibecoda;
	...
	public setEventHandlers(): void {
		...
		this.addEventListener('issue-source', 'change', (e: Event) => {
			...
			if (value === IssueSource.Vibecoda) {
				descriptionTextArea.placeholder = localize('vibecodaPlaceholder', "E.g Workbench is missing problems panel");
			...
			} else if (value === IssueSource.Vibecoda) {
				fileOnProduct = true;
			}
		...
		this.addEventListener('description', 'input', (e: Event) => {
			...
			if (this.issueReporterModel.fileOnExtension() === false) {
				// eslint-disable-next-line no-restricted-syntax
				const title = (<HTMLInputElement>this.getElementById('issue-title')).value;
				this.searchVibecodaIssues(title, issueDescription);
			}
		});
	...
	public searchVibecodaIssues(title: string, issueDescription?: string): void {
		if (title) {
	...
	public searchIssues(title: string, fileOnExtension: boolean | undefined, fileOnMarketplace: boolean | undefined): void {
		...
		this.searchVibecodaIssues(title, description);
	}
	...
	private searchDuplicates(title: string, body?: string): void {
		const url = 'https://vibecoda-probot.westus.cloudapp.azure.com:7890/duplicate_candidates';
		const init = {
			method: 'POST',
			body: JSON.stringify({
				title,
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.Vibecoda, localize('vibecoda', "Vibecoda"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A Vibecoda extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of Vibecoda"));
	...
	public addTemplateToUrl(baseUrl: string, owner?: string, repositoryName?: string): string {
		...
		const needsTemplate = isVscode || (isMicrosoft && (repositoryName === 'vibecoda' || repositoryName === 'vibecoda-python'));
