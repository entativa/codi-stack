...
export class IssueReporterModel {
	...
	constructor(initialData?: Partial<IssueReporterData>) {
		...
		mainWindow.addEventListener('message', async (event) => {
			if (event.data && event.data.sendChannel === 'vibecoda:triggerIssueData') {
				mainWindow.postMessage({
					data: { issueBody: this._data.issueDescription, issueTitle: this._data.issueTitle },
					replyChannel: 'vibecoda:triggerIssueDataResponse'
				}, '*');
			}
		});
	}
...
Vibecoda version: ${this._data.versionInfo && this._data.versionInfo.vibecodaVersion}
