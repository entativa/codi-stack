...
export class ExtHostDebugService extends ExtHostDebugServiceBase {
	...
	protected override createDebugAdapter(adapter: vibecoda.DebugAdapterDescriptor, session: ExtHostDebugSession): AbstractDebugAdapter | undefined {
	...
	public override async $runInTerminal(args: DebugProtocol.RunInTerminalRequestArguments, sessionId: string): Promise<number | undefined> {
		...
		if (args.kind === 'integrated') {
			...
			if (!terminal) {
				const options: vibecoda.TerminalOptions = {
					shellPath: shell,
					shellArgs: shellArgs,
					cwd: args.cwd,
					name: terminalName,
			...
			if (giveShellTimeToInitialize) {
				...
				if (terminal.state.isInteractedWith && !terminal.shellIntegration) {
					...
					await timeout(200); // mirroring https://github.com/microsoft/vibecoda/blob/c67ccc70ece5f472ec25464d3eeb874cfccee9f1/src/vs/workbench/contrib/terminal/browser/terminalInstance.ts#L852-L857
				}
...
class DebugTerminalCollection {
	...
	private _terminalInstances = new Map<vibecoda.Terminal, { lastUsedAt: number; config: string }>();
	...
	public async checkout(config: string, name: string, cleanupOthersByName = false) {
		...
		return await firstParallel(promises, (t): t is vibecoda.Terminal => !!t);
	}

	public insert(terminal: vibecoda.Terminal, termConfig: string) {
		this._terminalInstances.set(terminal, { lastUsedAt: Date.now(), config: termConfig });
	}

	public free(terminal: vibecoda.Terminal) {
	...
	public onTerminalClosed(terminal: vibecoda.Terminal) {
