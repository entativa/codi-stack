...
export class NodeDynamicAuthProvider extends DynamicAuthProvider {
	...
	private async _createWithLoopbackServer(scopes: string[], progress: vibecoda.Progress<IProgressStep>, token: vibecoda.CancellationToken): Promise<IAuthorizationTokenResponse> {
	...
	private async _createWithDeviceCode(scopes: string[], progress: vibecoda.Progress<IProgressStep>, token: vibecoda.CancellationToken): Promise<IAuthorizationTokenResponse> {
		if (!this._serverMetadata.token_endpoint) {
			throw new Error('Token endpoint not available in server metadata');
		}
