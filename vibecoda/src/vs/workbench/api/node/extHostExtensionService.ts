...
class NodeModuleESMInterceptor extends RequireInterceptor {
	...
	export const resolve = async (specifier, context, nextResolve) => {
		if (specifier !== 'vibecoda' || !context.parentURL) {
	...
	private static _vibecodaImportFnName = `_VSCODE_IMPORT_VSCODE_API`;
	...
	protected override _installInterceptor(): void {
		...
		const apiInstances = new BidirectionalMap<typeof vibecoda, string>();
		...
		Object.defineProperty(globalThis, NodeModuleESMInterceptor._vibecodaImportFnName, {
			enumerable: false,
			configurable: false,
			writable: false,
			value: (key: string) => {
		...
		port1LayerCheckerWorkaround.onmessage = (e: { data: Message }) => {
			...
			// Get the vibecoda-module factory - which is the same logic that's also used by
			// the CommonJS require interceptor
			if (!apiModuleFactory) {
				apiModuleFactory = this._factories.get('vibecoda');
			...
			if (!scriptDataUrlSrc) {
				const jsCode = `const _vibecodaInstance = globalThis.${NodeModuleESMInterceptor._vibecodaImportFnName}('${key}');\n\n${Object.keys(apiInstance).map((name => `export const ${name} = _vibecodaInstance['${name}'];`)).join('\n')}`;
