...
export class NativeExtHostSearch extends ExtHostSearch implements IDisposable {
	...
	constructor(
		...
		this._disposables.add(this.registerTextSearchProvider(Schemas.vibecodaUserData, new RipgrepSearchProvider(outputChannel, this.getNumThreadsCached)));
	...
	private handleConfigurationChanged(event: vibecoda.ConfigurationChangeEvent) {
	...
	override $provideFileSearchResults(handle: number, session: number, rawQuery: IRawFileQuery, token: vibecoda.CancellationToken): Promise<ISearchCompleteStats> {
	...
	override async doInternalFileSearchWithCustomCallback(rawQuery: IFileQuery, token: vibecoda.CancellationToken, handleFileMatch: (data: URI[]) => void): Promise<ISearchCompleteStats> {
	...
	private async doInternalFileSearch(handle: number, session: number, rawQuery: IFileQuery, token: vibecoda.CancellationToken): Promise<ISearchCompleteStats> {
