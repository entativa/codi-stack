...
function knownExcludeCmdline(command: string): boolean {
	...
	return !!command.match(/.*\.vibecoda-server-[a-zA-Z]+\/bin.*/)
		|| (command.indexOf('out/server-main.js') !== -1)
		|| (command.indexOf('_productName=Vibecoda') !== -1);
}
...
export class NodeExtHostTunnelService extends ExtHostTunnelService {
	...
	protected override makeManagedTunnelFactory(authority: vibecoda.ManagedResolvedAuthority): vibecoda.RemoteAuthorityResolver['tunnelFactory'] {
...
class ExtHostManagedSocket extends ManagedSocket {
	public static connect(
		passing: vibecoda.ManagedMessagePassing,
	...
	constructor(
		private readonly passing: vibecoda.ManagedMessagePassing,
		debugLabel: string,
