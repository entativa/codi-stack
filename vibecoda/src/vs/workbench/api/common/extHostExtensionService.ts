...
interface ITestRunner {
	/** Old test runner API, as exported from `vibecoda/lib/testrunner` */
...
export abstract class AbstractExtHostExtensionService extends Disposable implements ExtHostExtensionServiceShape {
	...
	private readonly _resolvers: { [authorityPrefix: string]: vibecoda.RemoteAuthorityResolver };
	...
	private _loadExtensionContext(extensionDescription: IExtensionDescription, extensionInternalStore: DisposableStore): Promise<vibecoda.ExtensionContext> {
		...
		return Promise.all([
			...
			const that = this;
			let extension: vibecoda.Extension<any> | undefined;

			let messagePassingProtocol: vibecoda.MessagePassingProtocol | undefined;
			...
			return Object.freeze<vibecoda.ExtensionContext>({
				globalState,
				workspaceState,
				secrets,
				subscriptions: [],
	...
	private static _callActivate(logService: ILogService, extensionId: ExtensionIdentifier, extensionModule: IExtensionModule, context: vibecoda.ExtensionContext, extensionInternalStore: IDisposable, activationTimesBuilder: ExtensionActivationTimesBuilder): Promise<ActivatedExtension> {
	...
	private static _callActivateOptional(logService: ILogService, extensionId: ExtensionIdentifier, extensionModule: IExtensionModule, context: vibecoda.ExtensionContext, activationTimesBuilder: ExtensionActivationTimesBuilder): Promise<IExtensionAPI> {
	...
	private _handleWorkspaceContainsEagerExtensions(folders: ReadonlyArray<vibecoda.WorkspaceFolder>): Promise<void> {
		if (folders.length === 0) {
			return Promise.resolve(undefined);
		}
	...
	private async _handleWorkspaceContainsEagerExtension(folders: ReadonlyArray<vibecoda.WorkspaceFolder>, desc: IExtensionDescription): Promise<void> {
	...
	public registerRemoteAuthorityResolver(authorityPrefix: string, resolver: vibecoda.RemoteAuthorityResolver): vibecoda.Disposable {
	...
	public async getRemoteExecServer(remoteAuthority: string): Promise<vibecoda.ExecServer | undefined> {
	...
	private async _activateAndGetResolver(remoteAuthority: string): Promise<{ authorityPrefix: string; resolver: vibecoda.RemoteAuthorityResolver | undefined }> {
	...
	public async $resolveAuthority(remoteAuthorityChain: string, resolveAttempt: number): Promise<Dto<IResolveAuthorityResult>> {
		...
		let result!: vibecoda.ResolverResult;
		let execServer: vibecoda.ExecServer | undefined;
...
export interface IExtHostExtensionService extends AbstractExtHostExtensionService {
	...
	registerRemoteAuthorityResolver(authorityPrefix: string, resolver: vibecoda.RemoteAuthorityResolver): vibecoda.Disposable;
	getRemoteExecServer(authority: string): Promise<vibecoda.ExecServer | undefined>;

	readonly onDidChangeRemoteConnectionData: Event<void>;
	getRemoteConnectionData(): IRemoteConnectionData | null;
}

export class Extension<T extends object | null | undefined> implements vibecoda.Extension<T> {
	...
	readonly extensionKind: vibecoda.ExtensionKind;
