...
class ExtHostChatSession {
	...
	constructor(
		public readonly session: vibecoda.ChatSession,
...
export class ExtHostChatSessions extends Disposable implements ExtHostChatSessionsShape {
	...
	private readonly _chatSessionItemProviders = new Map<number, {
		...
		readonly provider: vibecoda.ChatSessionItemProvider;
		readonly extension: IExtensionDescription;
		readonly disposable: DisposableStore;
	}>();
	private readonly _chatSessionContentProviders = new Map<number, {
		readonly provider: vibecoda.ChatSessionContentProvider;
		readonly extension: IExtensionDescription;
		readonly capabilities?: vibecoda.ChatSessionCapabilities;
		readonly disposable: DisposableStore;
	}>();
	 ...
	 */
	private readonly _sessionItems = new ResourceMap<vibecoda.ChatSessionItem>();
	...
	registerChatSessionItemProvider(extension: IExtensionDescription, chatSessionType: string, provider: vibecoda.ChatSessionItemProvider): vibecoda.Disposable {
	...
	registerChatSessionContentProvider(extension: IExtensionDescription, chatSessionScheme: string, chatParticipant: vibecoda.ChatParticipant, provider: vibecoda.ChatSessionContentProvider, capabilities?: vibecoda.ChatSessionCapabilities): vibecoda.Disposable {
	...
	private convertChatSessionStatus(status: vibecoda.ChatSessionStatus | undefined): ChatSessionStatus | undefined {
		if (status === undefined) {
			return undefined;
		}

		switch (status) {
			case 0: // vibecoda.ChatSessionStatus.Failed
				return ChatSessionStatus.Failed;
			case 1: // vibecoda.ChatSessionStatus.Completed
				return ChatSessionStatus.Completed;
			case 2: // vibecoda.ChatSessionStatus.InProgress
	...
	private convertChatSessionItem(sessionType: string, sessionContent: vibecoda.ChatSessionItem): IChatSessionItem {
		return {
	...
	async $provideNewChatSessionItem(handle: number, options: { request: IChatAgentRequest; metadata?: any }, token: CancellationToken): Promise<IChatSessionItem> {
		...
		try {
			...
			const vibecodaRequest = typeConvert.ChatAgentRequest.to(
			...
			const vibecodaOptions = {
				request: vibecodaRequest,
				metadata: options.metadata
			};

			const chatSessionItem = await entry.provider.provideNewChatSessionItem(vibecodaOptions, token);
			if (!chatSessionItem) {
	...
	async $provideChatSessionItems(handle: number, token: vibecoda.CancellationToken): Promise<IChatSessionItem[]> {
		const entry = this._chatSessionItemProviders.get(handle);
		if (!entry) {
	...
	private convertReferenceToVariable(ref: vibecoda.ChatPromptReference) {
		const value = ref.value && typeof ref.value === 'object' && 'uri' in ref.value && 'range' in ref.value
			? typeConvert.Location.from(ref.value as vibecoda.Location)
			: ref.value;
