...
class DocumentSymbolAdapter {
	...
	constructor(
		...
		private readonly _provider: vibecoda.DocumentSymbolProvider
	) { }
...
class CodeLensAdapter {
	...
	private readonly _cache = new Cache<vibecoda.CodeLens>('CodeLens');
	private readonly _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: vibecoda.CodeLensProvider,
	...
	async resolveCodeLens(symbol: extHostProtocol.ICodeLensDto, token: CancellationToken): Promise<extHostProtocol.ICodeLensDto | undefined> {
		...
		let resolvedLens: vibecoda.CodeLens | undefined | null;
...
function convertToLocationLinks(value: vibecoda.Location | vibecoda.Location[] | vibecoda.LocationLink[] | undefined | null): languages.LocationLink[] {
...
class DefinitionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DefinitionProvider
	) { }
...
class DeclarationAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DeclarationProvider
	) { }
...
class ImplementationAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.ImplementationProvider
	) { }
...
class TypeDefinitionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.TypeDefinitionProvider
	) { }
...
class HoverAdapter {
	...
	private _hoverMap: Map<number, vibecoda.Hover> = new Map<number, vibecoda.Hover>();

	private static HOVER_MAP_MAX_SIZE = 10;

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.HoverProvider,
	) { }
	...
	async provideHover(resource: URI, position: IPosition, context: languages.HoverContext<{ id: number }> | undefined, token: CancellationToken): Promise<extHostProtocol.HoverWithId | undefined> {
		...
		let value: vibecoda.Hover | null | undefined;
		if (context && context.verbosityRequest) {
			...
			const hoverContext: vibecoda.HoverContext = { verbosityDelta: context.verbosityRequest.verbosityDelta, previousHover };
...
class EvaluatableExpressionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.EvaluatableExpressionProvider,
	) { }
...
class InlineValuesAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.InlineValuesProvider,
	) { }
...
class DocumentHighlightAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentHighlightProvider
	) { }
...
class MultiDocumentHighlightAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.MultiDocumentHighlightProvider,
		private readonly _logService: ILogService,
	) { }
...
class LinkedEditingRangeAdapter {
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.LinkedEditingRangeProvider
	) { }
...
class ReferenceAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.ReferenceProvider
	) { }
...
class CodeActionAdapter {
	...
	private readonly _cache = new Cache<vibecoda.CodeAction | vibecoda.Command>('CodeAction');
	private readonly _disposables = new Map<number, DisposableStore>();
	...
	constructor(
		...
		private readonly _provider: vibecoda.CodeActionProvider,
		private readonly _logService: ILogService,
		private readonly _extension: IExtensionDescription,
	...
	async provideCodeActions(resource: URI, rangeOrSelection: IRange | ISelection, context: languages.CodeActionContext, token: CancellationToken): Promise<extHostProtocol.ICodeActionListDto | undefined> {
		...
		const ran = Selection.isISelection(rangeOrSelection)
			? <vibecoda.Selection>typeConvert.Selection.to(rangeOrSelection)
			: <vibecoda.Range>typeConvert.Range.to(rangeOrSelection);
		const allDiagnostics: vibecoda.Diagnostic[] = [];
		...
		const codeActionContext: vibecoda.CodeActionContext = {
		...
		for (let i = 0; i < commandsOrActions.length; i++) {
			...
			if (CodeActionAdapter._isCommand(candidate) && !(candidate instanceof CodeAction)) {
				...
				const toConvert = candidate as vibecoda.CodeAction;
	...
	private static _isCommand(thing: any): thing is vibecoda.Command {
		return typeof (<vibecoda.Command>thing).command === 'string' && typeof (<vibecoda.Command>thing).title === 'string';
	}
}

class DocumentPasteEditProvider {

	private _cachedPrepare?: Map<string, vibecoda.DataTransferItem>;

	private readonly _editsCache = new Cache<vibecoda.DocumentPasteEdit>('DocumentPasteEdit.edits');

	constructor(
		private readonly _proxy: extHostProtocol.MainThreadLanguageFeaturesShape,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentPasteEditProvider,
	...
	async prepareDocumentPaste(resource: URI, ranges: IRange[], dataTransferDto: extHostProtocol.DataTransferDTO, token: CancellationToken): Promise<extHostProtocol.DataTransferDTO | undefined> {
		...
		const doc = this._documents.getDocument(resource);
		const vibecodaRanges = ranges.map(range => typeConvert.Range.to(range));

		const dataTransfer = typeConvert.DataTransfer.toDataTransfer(dataTransferDto, () => {
			throw new NotImplementedError();
		});
		await this._provider.prepareDocumentPaste(doc, vibecodaRanges, dataTransfer, token);
		if (token.isCancellationRequested) {
			return;
		}
		...
		const newCache = new Map<string, vibecoda.DataTransferItem>();
	...
	async providePasteEdits(requestId: number, resource: URI, ranges: IRange[], dataTransferDto: extHostProtocol.DataTransferDTO, context: extHostProtocol.IDocumentPasteContextDto, token: CancellationToken): Promise<extHostProtocol.IPasteEditDto[]> {
		...
		const items = dataTransferDto.items.map(([mime, value]): [string, vibecoda.DataTransferItem] => {
		...
		const edits = await this._provider.provideDocumentPasteEdits(doc, vibecodaRanges, dataTransfer, {
	...
	async resolvePasteEdit(id: extHostProtocol.ChainedCacheId, token: CancellationToken): Promise<{ insertText?: string | vibecoda.SnippetString; additionalEdit?: extHostProtocol.IWorkspaceEditDto }> {
		const [sessionId, itemId] = id;
...
class DocumentFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentFormattingEditProvider
	) { }
...
class RangeFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentRangeFormattingEditProvider
	) { }
...
class OnTypeFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.OnTypeFormattingEditProvider
	) { }
...
class NavigateTypeAdapter {
	...
	private readonly _cache = new Cache<vibecoda.SymbolInformation>('WorkspaceSymbols');

	constructor(
		private readonly _provider: vibecoda.WorkspaceSymbolProvider,
		private readonly _logService: ILogService
	) { }
...
class RenameAdapter {
	...
	static supportsResolving(provider: vibecoda.RenameProvider): boolean {
		return typeof provider.prepareRename === 'function';
	}

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.RenameProvider,
		private readonly _logService: ILogService
	) { }
...
class NewSymbolNamesAdapter {
	...
	private static languageTriggerKindToVibecodaTriggerKind: Record<languages.NewSymbolNameTriggerKind, vibecoda.NewSymbolNameTriggerKind> = {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.NewSymbolNamesProvider,
		private readonly _logService: ILogService
	) { }
	...
	async provideNewSymbolNames(resource: URI, range: IRange, triggerKind: languages.NewSymbolNameTriggerKind, token: CancellationToken): Promise<languages.NewSymbolName[] | undefined> {
		...
		try {
			const kind = NewSymbolNamesAdapter.languageTriggerKindToVibecodaTriggerKind[triggerKind];
...
type ProvidedSemanticTokens = vibecoda.SemanticTokens | RelaxedSemanticTokens;
type ProvidedSemanticTokensEdits = vibecoda.SemanticTokensEdits | RelaxedSemanticTokensEdits;
...
class DocumentSemanticTokensAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentSemanticTokensProvider,
	) {
	...
	private static _fixProvidedSemanticTokens(v: ProvidedSemanticTokens | ProvidedSemanticTokensEdits): vibecoda.SemanticTokens | vibecoda.SemanticTokensEdits {
	...
	private static _isCorrectSemanticTokens(v: ProvidedSemanticTokens): v is vibecoda.SemanticTokens {
	...
	private static _isCorrectSemanticTokensEdits(v: ProvidedSemanticTokensEdits): v is vibecoda.SemanticTokensEdits {
	...
	private static _convertToEdits(previousResult: SemanticTokensPreviousResult | null | undefined, newResult: vibecoda.SemanticTokens | vibecoda.SemanticTokensEdits): vibecoda.SemanticTokens | vibecoda.SemanticTokensEdits {
	...
	private _send(value: vibecoda.SemanticTokens | vibecoda.SemanticTokensEdits, original: vibecoda.SemanticTokens | vibecoda.SemanticTokensEdits): VSBuffer | null {
...
class DocumentRangeSemanticTokensAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentRangeSemanticTokensProvider,
	) { }
	...
	private _send(value: vibecoda.SemanticTokens): VSBuffer {
		return encodeSemanticTokensDto({
			id: 0,
			type: 'full',
			data: value.data
...
class CompletionsAdapter {
	...
	static supportsResolving(provider: vibecoda.CompletionItemProvider): boolean {
		return typeof provider.resolveCompletionItem === 'function';
	}

	private _cache = new Cache<vibecoda.CompletionItem>('CompletionItem');
	private _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: vibecoda.CompletionItemProvider,
		private readonly _apiDeprecation: IExtHostApiDeprecationService,
		private readonly _extension: IExtensionDescription,
	) { }
	...
	async provideCompletionItems(resource: URI, position: IPosition, context: languages.CompletionContext, token: CancellationToken): Promise<extHostProtocol.ISuggestResultDto | undefined> {
		...
		// https://github.com/microsoft/vibecoda/issues/83400#issuecomment-546851421
	...
	private _convertCompletionItem(item: vibecoda.CompletionItem, id: extHostProtocol.ChainedCacheId, defaultInsertRange?: vibecoda.Range, defaultReplaceRange?: vibecoda.Range): extHostProtocol.ISuggestDataDto {
		...
		let range: vibecoda.Range | { inserting: vibecoda.Range; replacing: vibecoda.Range } | undefined;
		if (item.textEdit) {
...
class InlineCompletionAdapter {
	private readonly _references = new ReferenceMap<{
		dispose(): void;
		items: readonly vibecoda.InlineCompletionItem[];
		list: vibecoda.InlineCompletionList | undefined;
	}>();
	...
	constructor(
		private readonly _extension: IExtensionDescription,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.InlineCompletionItemProvider,
		private readonly _commands: CommandsConverter,
	) {
	...
	private readonly languageTriggerKindToVibecodaTriggerKind: Record<languages.InlineCompletionTriggerKind, InlineCompletionTriggerKind> = {
		...
		const result = await this._provider.provideInlineCompletionItems(doc, pos, {
			selectedCompletionInfo:
				context.selectedSuggestionInfo
					...
					: undefined,
			triggerKind: this.languageTriggerKindToVibecodaTriggerKind[context.triggerKind],
	...
	disposeCompletions(pid: number, reason: languages.InlineCompletionsDisposeReason) {
		...
		if (this._provider.handleListEndOfLifetime && this._isAdditionsProposedApiEnabled && completionList?.list) {
			function translateReason(reason: languages.InlineCompletionsDisposeReason): vibecoda.InlineCompletionsDisposeReason {
				switch (reason.kind) {
					case 'lostRace':
...
class SignatureHelpAdapter {
	...
	private readonly _cache = new Cache<vibecoda.SignatureHelp>('SignatureHelp');

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.SignatureHelpProvider,
	) { }
	...
	async provideSignatureHelp(resource: URI, position: IPosition, context: extHostProtocol.ISignatureHelpContextDto, token: CancellationToken): Promise<extHostProtocol.ISignatureHelpDto | undefined> {
		const doc = this._documents.getDocument(resource);
		const pos = typeConvert.Position.to(position);
		const vibecodaContext = this.reviveContext(context);

		const value = await this._provider.provideSignatureHelp(doc, pos, token, vibecodaContext);
		if (value) {
	...
	private reviveContext(context: extHostProtocol.ISignatureHelpContextDto): vibecoda.SignatureHelpContext {
		let activeSignatureHelp: vibecoda.SignatureHelp | undefined = undefined;
...
class InlayHintsAdapter {
	...
	private _cache = new Cache<vibecoda.InlayHint>('InlayHints');
	private readonly _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: vibecoda.InlayHintsProvider,
		private readonly _logService: ILogService,
		private readonly _extension: IExtensionDescription
	) { }
	...
	private _isValidInlayHint(hint: vibecoda.InlayHint, range?: vibecoda.Range): boolean {
	...
	private _convertInlayHint(hint: vibecoda.InlayHint, id: extHostProtocol.ChainedCacheId): extHostProtocol.IInlayHintDto {
...
class LinkProviderAdapter {
	...
	private _cache = new Cache<vibecoda.DocumentLink>('DocumentLink');

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentLinkProvider
	) { }
	...
	private static _validateLink(link: vibecoda.DocumentLink): boolean {
...
class ColorProviderAdapter {
	...
	constructor(
		...
		private _provider: vibecoda.DocumentColorProvider
	) { }
...
class FoldingProviderAdapter {
	...
	constructor(
		private _documents: ExtHostDocuments,
		private _provider: vibecoda.FoldingRangeProvider
	) { }
...
class SelectionRangeAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.SelectionRangeProvider,
		private readonly _logService: ILogService
	) { }
	...
	async provideSelectionRanges(resource: URI, pos: IPosition[], token: CancellationToken): Promise<languages.SelectionRange[][]> {
		...
		for (let i = 0; i < positions.length; i++) {
			...
			let last: vibecoda.Position | vibecoda.Range = positions[i];
...
class CallHierarchyAdapter {
	...
	private readonly _cache = new Map<string, Map<string, vibecoda.CallHierarchyItem>>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.CallHierarchyProvider
	) { }
	...
	private _cacheAndConvertItem(sessionId: string, item: vibecoda.CallHierarchyItem): extHostProtocol.ICallHierarchyItemDto {
	...
	private _itemFromCache(sessionId: string, itemId: string): vibecoda.CallHierarchyItem | undefined {
		const map = this._cache.get(sessionId);
...
class TypeHierarchyAdapter {
	...
	private readonly _idPool = new IdGenerator('');
	private readonly _cache = new Map<string, Map<string, vibecoda.TypeHierarchyItem>>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.TypeHierarchyProvider
	) { }
	...
	private _cacheAndConvertItem(sessionId: string, item: vibecoda.TypeHierarchyItem): extHostProtocol.ITypeHierarchyItemDto {
		const map = this._cache.get(sessionId)!;
	...
	private _itemFromCache(sessionId: string, itemId: string): vibecoda.TypeHierarchyItem | undefined {
		const map = this._cache.get(sessionId);
		return map?.get(itemId);
	}
}
...
class DocumentDropEditAdapter {
	...
	private readonly _cache = new Cache<vibecoda.DocumentDropEdit>('DocumentDropEdit');

	constructor(
		private readonly _proxy: extHostProtocol.MainThreadLanguageFeaturesShape,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: vibecoda.DocumentDropEditProvider,
		private readonly _handle: number,
		private readonly _extension: IExtensionDescription,
	) { }
...
export class ExtHostLanguageFeatures extends CoreDisposable implements extHostProtocol.ExtHostLanguageFeaturesShape {
	...
	private _inlineCompletionsUnificationState: vibecoda.InlineCompletionsUnificationState;
	public get inlineCompletionsUnificationState(): vibecoda.InlineCompletionsUnificationState {
	...
	private _transformDocumentSelector(selector: vibecoda.DocumentSelector, extension: IExtensionDescription): Array<extHostProtocol.IDocumentFilterDto> {
	...
	registerDocumentSymbolProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentSymbolProvider, metadata?: vibecoda.DocumentSymbolProviderMetadata): vibecoda.Disposable {
	...
	registerCodeLensProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.CodeLensProvider): vibecoda.Disposable {
		const handle = this._nextHandle();
	...
	registerDefinitionProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DefinitionProvider): vibecoda.Disposable {
	...
	registerDeclarationProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DeclarationProvider): vibecoda.Disposable {
	...
	registerImplementationProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.ImplementationProvider): vibecoda.Disposable {
	...
	registerTypeDefinitionProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.TypeDefinitionProvider): vibecoda.Disposable {
	...
	registerHoverProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.HoverProvider, extensionId?: ExtensionIdentifier): vibecoda.Disposable {
	...
	registerEvaluatableExpressionProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.EvaluatableExpressionProvider, extensionId?: ExtensionIdentifier): vibecoda.Disposable {
	...
	registerInlineValuesProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.InlineValuesProvider, extensionId?: ExtensionIdentifier): vibecoda.Disposable {
	...
	registerDocumentHighlightProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentHighlightProvider): vibecoda.Disposable {
	...
	registerMultiDocumentHighlightProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.MultiDocumentHighlightProvider): vibecoda.Disposable {
	...
	registerLinkedEditingRangeProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.LinkedEditingRangeProvider): vibecoda.Disposable {
	...
	registerReferenceProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.ReferenceProvider): vibecoda.Disposable {
	...
	registerCodeActionProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.CodeActionProvider, metadata?: vibecoda.CodeActionProviderMetadata): vibecoda.Disposable {
	...
	registerDocumentFormattingEditProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentFormattingEditProvider): vibecoda.Disposable {
	...
	registerDocumentRangeFormattingEditProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentRangeFormattingEditProvider): vibecoda.Disposable {
	...
	registerOnTypeFormattingEditProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.OnTypeFormattingEditProvider, triggerCharacters: string[]): vibecoda.Disposable {
	...
	registerWorkspaceSymbolProvider(extension: IExtensionDescription, provider: vibecoda.WorkspaceSymbolProvider): vibecoda.Disposable {
	...
	registerRenameProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.RenameProvider): vibecoda.Disposable {
	...
	registerNewSymbolNamesProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.NewSymbolNamesProvider): vibecoda.Disposable {
	...
	registerDocumentSemanticTokensProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentSemanticTokensProvider, legend: vibecoda.SemanticTokensLegend): vibecoda.Disposable {
	...
	registerDocumentRangeSemanticTokensProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentRangeSemanticTokensProvider, legend: vibecoda.SemanticTokensLegend): vibecoda.Disposable {
	...
	registerCompletionItemProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.CompletionItemProvider, triggerCharacters: string[]): vibecoda.Disposable {
	...
	registerInlineCompletionsProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.InlineCompletionItemProvider, metadata: vibecoda.InlineCompletionItemProviderMetadata | undefined): vibecoda.Disposable {
	...
	registerSignatureHelpProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.SignatureHelpProvider, metadataOrTriggerChars: string[] | vibecoda.SignatureHelpProviderMetadata): vibecoda.Disposable {
	...
	registerInlayHintsProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.InlayHintsProvider): vibecoda.Disposable {
	...
	registerDocumentLinkProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentLinkProvider): vibecoda.Disposable {
	...
	registerColorProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentColorProvider): vibecoda.Disposable {
	...
	registerFoldingRangeProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.FoldingRangeProvider): vibecoda.Disposable {
		const handle = this._nextHandle();
	...
	$provideFoldingRanges(handle: number, resource: UriComponents, context: vibecoda.FoldingContext, token: CancellationToken): Promise<languages.FoldingRange[] | undefined> {
		return this._withAdapter(
			handle,
	...
	registerSelectionRangeProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.SelectionRangeProvider): vibecoda.Disposable {
	...
	registerCallHierarchyProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.CallHierarchyProvider): vibecoda.Disposable {
	...
	// --- type hierarchy
	registerTypeHierarchyProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.TypeHierarchyProvider): vibecoda.Disposable {
	...
	registerDocumentOnDropEditProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentDropEditProvider, metadata?: vibecoda.DocumentDropEditProviderMetadata) {
		const handle = this._nextHandle();
	...
	registerDocumentPasteEditProvider(extension: IExtensionDescription, selector: vibecoda.DocumentSelector, provider: vibecoda.DocumentPasteEditProvider, metadata: vibecoda.DocumentPasteProviderMetadata): vibecoda.Disposable {
		const handle = this._nextHandle();
	...
	private static _serializeIndentationRule(indentationRule: vibecoda.IndentationRule): extHostProtocol.IIndentationRuleDto {
		return {
	...
	private static _serializeOnEnterRule(onEnterRule: vibecoda.OnEnterRule): extHostProtocol.IOnEnterRuleDto {
		return {
	...
	private static _serializeOnEnterRules(onEnterRules: vibecoda.OnEnterRule[]): extHostProtocol.IOnEnterRuleDto[] {
		return onEnterRules.map(ExtHostLanguageFeatures._serializeOnEnterRule);
	}

	private static _serializeAutoClosingPair(autoClosingPair: vibecoda.AutoClosingPair): IAutoClosingPairConditional {
		return {
	...
	private static _serializeAutoClosingPairs(autoClosingPairs: vibecoda.AutoClosingPair[]): IAutoClosingPairConditional[] {
		return autoClosingPairs.map(ExtHostLanguageFeatures._serializeAutoClosingPair);
	}

	setLanguageConfiguration(extension: IExtensionDescription, languageId: string, configuration: vibecoda.LanguageConfiguration): vibecoda.Disposable {
