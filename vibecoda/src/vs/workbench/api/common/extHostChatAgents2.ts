...
export class ChatAgentResponseStream {
	...
	private _apiObject: vibecoda.ChatResponseStream | undefined;
	...
	get apiObject() {
		...
		if (!this._apiObject) {
			...
			const _report = (progress: IChatProgressDto, task?: (progress: vibecoda.Progress<vibecoda.ChatResponseWarningPart | vibecoda.ChatResponseReferencePart>) => Thenable<string | void>) => {
				...
				if (task) {
					...
					const progressReporter = {
						report: (p: vibecoda.ChatResponseWarningPart | vibecoda.ChatResponseReferencePart) => {
							progressReporterPromise.then(() => {
								if (extHostTypes.MarkdownString.isMarkdownString(p.value)) {
									send(typeConvert.ChatResponseWarningPart.from(<vibecoda.ChatResponseWarningPart>p), myHandle);
								} else {
									send(typeConvert.ChatResponseReferencePart.from(<vibecoda.ChatResponseReferencePart>p), myHandle);
								}
							});
						}
					};
			...
			this._apiObject = Object.freeze<vibecoda.ChatResponseStream>({
				...
				progress(value, task?: ((progress: vibecoda.Progress<vibecoda.ChatResponseWarningPart>) => Thenable<string | void>)) {
				...
				thinkingProgress(thinkingDelta: vibecoda.ThinkingDelta) {
				...
				codeCitation(value: vibecoda.Uri, license: string, snippet: string): void {
...
interface InFlightChatRequest {
	requestId: string;
	extRequest: vibecoda.ChatRequest;
...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	private readonly _onDidChangeChatRequestTools = this._register(new Emitter<vibecoda.ChatRequest>());
	...
	transferActiveChat(newWorkspace: vibecoda.Uri): void {
		this._proxy.$transferActiveChatSession(newWorkspace);
	}

	createChatAgent(extension: IExtensionDescription, id: string, handler: vibecoda.ChatExtendedRequestHandler): vibecoda.ChatParticipant {
	...
	createDynamicChatAgent(extension: IExtensionDescription, id: string, dynamicProps: vibecoda.DynamicChatParticipantProps, handler: vibecoda.ChatExtendedRequestHandler): vibecoda.ChatParticipant {
		const handle = ExtHostChatAgents2._idPool++;
		const agent = new ExtHostChatAgent(extension, id, this._proxy, handle, handler);
		this._agents.set(handle, agent);
	...
	registerChatParticipantDetectionProvider(extension: IExtensionDescription, provider: vibecoda.ChatParticipantDetectionProvider): vibecoda.Disposable {
	...
	registerRelatedFilesProvider(extension: IExtensionDescription, provider: vibecoda.ChatRelatedFilesProvider, metadata: vibecoda.ChatRelatedFilesProviderMetadata): vibecoda.Disposable {
	...
	async $detectChatParticipant(handle: number, requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[] }, options: { location: ChatAgentLocation; participants?: vibecoda.ChatParticipantMetadata[] }, token: CancellationToken): Promise<vibecoda.ChatParticipantDetectionResult | null | undefined> {
	...
	private async _createRequest(requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[] }, extension: IExtensionDescription) {
		...
		let location: vibecoda.ChatRequestEditorData | vibecoda.ChatRequestNotebookData | undefined;
	...
	private async getModelForRequest(request: IChatAgentRequest, extension: IExtensionDescription): Promise<vibecoda.LanguageModelChat> {
		let model: vibecoda.LanguageModelChat | undefined;
	...
	async $invokeAgent(handle: number, requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[]; chatSessionContext?: IChatSessionContextDto; chatSummary?: { prompt?: string; history?: string } }, token: CancellationToken): Promise<IChatAgentResult | undefined> {
		...
		try {
			...
			let chatSessionContext: vibecoda.ChatSessionContext | undefined;
			...
			const chatContext: vibecoda.ChatContext = {
				history,
				chatSessionContext,
	...
	private async prepareHistoryTurns(extension: Readonly<IRelaxedExtensionDescription>, agentId: string, context: { history: IChatAgentHistoryEntryDto[] }): Promise<(vibecoda.ChatRequestTurn | vibecoda.ChatResponseTurn)[]> {
		const res: (vibecoda.ChatRequestTurn | vibecoda.ChatResponseTurn)[] = [];

		for (const h of context.history) {
			const ehResult = typeConvert.ChatAgentResult.to(h.result);
			const result: vibecoda.ChatResult = agentId === h.request.agentId ?
				ehResult :
				{ ...ehResult, metadata: undefined };

			// REQUEST turn
			const varsWithoutTools: vibecoda.ChatPromptReference[] = [];
			const toolReferences: vibecoda.ChatLanguageModelToolReference[] = [];
	...
	$acceptFeedback(handle: number, result: IChatAgentResult, voteAction: IChatVoteAction): void {
		...
		const feedback: vibecoda.ChatResultFeedback = {
			result: ehResult,
			kind,
...
class ExtHostParticipantDetector {
	constructor(
		...
		public readonly provider: vibecoda.ChatParticipantDetectionProvider,
	) { }
}
...
class ExtHostRelatedFilesProvider {
	constructor(
		public readonly extension: IExtensionDescription,
		public readonly provider: vibecoda.ChatRelatedFilesProvider,
	) { }
}

class ExtHostChatAgent {

	private _followupProvider: vibecoda.ChatFollowupProvider | undefined;
	private _iconPath: vibecoda.Uri | { light: vibecoda.Uri; dark: vibecoda.Uri } | vibecoda.ThemeIcon | undefined;
	private _helpTextPrefix: string | vibecoda.MarkdownString | undefined;
	private _helpTextPostfix: string | vibecoda.MarkdownString | undefined;
	private _onDidReceiveFeedback = new Emitter<vibecoda.ChatResultFeedback>();
	private _onDidPerformAction = new Emitter<vibecoda.ChatUserActionEvent>();
	private _supportIssueReporting: boolean | undefined;
	private _agentVariableProvider?: { provider: vibecoda.ChatParticipantCompletionItemProvider; triggerCharacters: string[] };
	private _additionalWelcomeMessage?: string | vibecoda.MarkdownString | undefined;
	private _titleProvider?: vibecoda.ChatTitleProvider | undefined;
	private _summarizer?: vibecoda.ChatSummarizer | undefined;
	private _requester: vibecoda.ChatRequesterInformation | undefined;
	private _pauseStateEmitter = new Emitter<vibecoda.ChatParticipantPauseStateEvent>();
	...
	constructor(
		...
		private _requestHandler: vibecoda.ChatExtendedRequestHandler,
	) { }

	acceptFeedback(feedback: vibecoda.ChatResultFeedback) {
		this._onDidReceiveFeedback.fire(feedback);
	}

	acceptAction(event: vibecoda.ChatUserActionEvent) {
		this._onDidPerformAction.fire(event);
	}

	setChatRequestPauseState(pauseState: vibecoda.ChatParticipantPauseStateEvent) {
		this._pauseStateEmitter.fire(pauseState);
	}

	async invokeCompletionProvider(query: string, token: CancellationToken): Promise<vibecoda.ChatCompletionItem[]> {
	...
	async provideFollowups(result: vibecoda.ChatResult, context: vibecoda.ChatContext, token: CancellationToken): Promise<vibecoda.ChatFollowup[]> {
	...
	async provideTitle(context: vibecoda.ChatContext, token: CancellationToken): Promise<string | undefined> {
	...
	async provideSummary(context: vibecoda.ChatContext, token: CancellationToken): Promise<string | undefined> {
		if (!this._summarizer) {
			return;
		}
	...
	get apiAgent(): vibecoda.ChatParticipant {
		let disposed = false;
		...
		} satisfies vibecoda.ChatParticipant;
	}

	invoke(request: vibecoda.ChatRequest, context: vibecoda.ChatContext, response: vibecoda.ChatResponseStream, token: CancellationToken): vibecoda.ProviderResult<vibecoda.ChatResult | void> {
