...
export interface IFileOperationOptions {
	...
	readonly contents?: Uint8Array | vibecoda.DataTransferFile;
}
...
export interface IFileOperation {
	...
	readonly metadata?: vibecoda.WorkspaceEditEntryMetadata;
}
...
export interface IFileSnippetTextEdit {
	...
	readonly edit: vibecoda.SnippetString;
	readonly metadata?: vibecoda.WorkspaceEditEntryMetadata;
...
export class WorkspaceEdit implements vibecoda.WorkspaceEdit {
	...
	// --- file
	renameFile(from: vibecoda.Uri, to: vibecoda.Uri, options?: { readonly overwrite?: boolean; readonly ignoreIfExists?: boolean }, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from, to, options, metadata });
	}

	createFile(uri: vibecoda.Uri, options?: { readonly overwrite?: boolean; readonly ignoreIfExists?: boolean; readonly contents?: Uint8Array | vibecoda.DataTransferFile }, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from: undefined, to: uri, options, metadata });
	}

	deleteFile(uri: vibecoda.Uri, options?: { readonly recursive?: boolean; readonly ignoreIfNotExists?: boolean }, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from: uri, to: undefined, options, metadata });
	}

	// --- notebook
	private replaceNotebookMetadata(uri: URI, value: Record<string, unknown>, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Cell, metadata, uri, edit: { editType: CellEditType.DocumentMetadata, metadata: value } });
	}

	private replaceNotebookCells(uri: URI, startOrRange: vibecoda.NotebookRange, cellData: vibecoda.NotebookCellData[], metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
	...
	private replaceNotebookCellMetadata(uri: URI, index: number, cellMetadata: Record<string, unknown>, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Cell, metadata, uri, edit: { editType: CellEditType.Metadata, index, metadata: cellMetadata } });
	}

	// --- text
	replace(uri: URI, range: Range, newText: string, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Text, uri, edit: new TextEdit(range, newText), metadata });
	}

	insert(resource: URI, position: Position, newText: string, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
		this.replace(resource, new Range(position, position), newText, metadata);
	}

	delete(resource: URI, range: Range, metadata?: vibecoda.WorkspaceEditEntryMetadata): void {
	...
	set(uri: URI, edits: ReadonlyArray<[TextEdit | SnippetTextEdit, vibecoda.WorkspaceEditEntryMetadata | undefined]>): void;
	set(uri: URI, edits: readonly NotebookEdit[]): void;
	set(uri: URI, edits: ReadonlyArray<[NotebookEdit, vibecoda.WorkspaceEditEntryMetadata | undefined]>): void;

	set(uri: URI, edits: null | undefined | ReadonlyArray<TextEdit | SnippetTextEdit | NotebookEdit | [NotebookEdit, vibecoda.WorkspaceEditEntryMetadata | undefined] | [TextEdit | SnippetTextEdit, vibecoda.WorkspaceEditEntryMetadata | undefined]>): void {
		if (!edits) {
			...
			for (const editOrTuple of edits) {
				...
				let metadata: vibecoda.WorkspaceEditEntryMetadata | undefined;
