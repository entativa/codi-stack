...
class FsLinkProvider {
	...
	provideDocumentLinks(document: vibecoda.TextDocument): vibecoda.ProviderResult<vibecoda.DocumentLink[]> {
		this._initStateMachine();

		const result: vibecoda.DocumentLink[] = [];
...
export class ExtHostFileSystem implements ExtHostFileSystemShape {
	...
	private readonly _fsProvider = new Map<number, vibecoda.FileSystemProvider>();
	...
	registerFileSystemProvider(extension: IExtensionDescription, scheme: string, provider: vibecoda.FileSystemProvider, options: { isCaseSensitive?: boolean; isReadonly?: boolean | vibecoda.MarkdownString } = {}) {
	...
	private static _validateFileSystemProvider(provider: vibecoda.FileSystemProvider) {
		if (!provider) {
	...
	private static _asIStat(stat: vibecoda.FileStat): files.IStat {
	...
	private _getFsProvider(handle: number): vibecoda.FileSystemProvider {
