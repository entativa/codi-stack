...
export class ExtHostLanguages implements ExtHostLanguagesShape {
	...
	async changeLanguage(uri: vibecoda.Uri, languageId: string): Promise<vibecoda.TextDocument> {
	...
	async tokenAtPosition(document: vibecoda.TextDocument, position: vibecoda.Position): Promise<vibecoda.TokenInformation> {
	...
	createLanguageStatusItem(extension: IExtensionDescription, id: string, selector: vibecoda.DocumentSelector): vibecoda.LanguageStatusItem {
		...
		const data: Omit<vibecoda.LanguageStatusItem, 'dispose' | 'text2'> = {
			selector,
			id,
		...
		const result: vibecoda.LanguageStatusItem = {
			dispose() {
