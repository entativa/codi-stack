...
class ExtHostEditorTab {
	private _apiObject: vibecoda.Tab | undefined;
	...
	get apiObject(): vibecoda.Tab {
		if (!this._apiObject) {
			...
			this._apiObject = Object.freeze<vibecoda.Tab>(obj);
		}
...
class ExtHostEditorTabGroup {
	...
	private _apiObject: vibecoda.TabGroup | undefined;
	...
	get apiObject(): vibecoda.TabGroup {
		if (!this._apiObject) {
			// Don't want to lose reference to parent `this` in the getters
			const that = this;
			const obj: vibecoda.TabGroup = {
				get isActive() {
			...
			this._apiObject = Object.freeze<vibecoda.TabGroup>(obj);
		}
		return this._apiObject;
	}
...
export class ExtHostEditorTabs implements IExtHostEditorTabs {
	...
	private readonly _onDidChangeTabs = new Emitter<vibecoda.TabChangeEvent>();
	private readonly _onDidChangeTabGroups = new Emitter<vibecoda.TabGroupChangeEvent>();
	...
	private _apiObject: vibecoda.TabGroups | undefined;
	...
	get tabGroups(): vibecoda.TabGroups {
		if (!this._apiObject) {
			...
			const obj: vibecoda.TabGroups = {
				...
				close: async (tabOrTabGroup: vibecoda.Tab | readonly vibecoda.Tab[] | vibecoda.TabGroup | readonly vibecoda.TabGroup[], preserveFocus?: boolean) => {
					...
					if (isTabGroup(tabsOrTabGroups[0])) {
						return this._closeGroups(tabsOrTabGroups as vibecoda.TabGroup[], preserveFocus);
					} else {
						return this._closeTabs(tabsOrTabGroups as vibecoda.Tab[], preserveFocus);
					}
				},
				// move: async (tab: vibecoda.Tab, viewColumn: ViewColumn, index: number, preserveFocus?: boolean) => {
	...
	$acceptEditorTabModel(tabGroups: IEditorTabGroupDto[]): void {
		...
		const closed: vibecoda.TabGroup[] = this._extHostTabGroups.filter(group => diff.removed.includes(group.groupId)).map(group => group.apiObject);
		const opened: vibecoda.TabGroup[] = [];
		const changed: vibecoda.TabGroup[] = [];
	...
	private _findExtHostTabFromApi(apiTab: vibecoda.Tab): ExtHostEditorTab | undefined {
	...
	private _findExtHostTabGroupFromApi(apiTabGroup: vibecoda.TabGroup): ExtHostEditorTabGroup | undefined {
		return this._extHostTabGroups.find(candidate => candidate.apiObject === apiTabGroup);
	}

	private async _closeTabs(tabs: vibecoda.Tab[], preserveFocus?: boolean): Promise<boolean> {
	...
	private async _closeGroups(groups: vibecoda.TabGroup[], preserverFoucs?: boolean): Promise<boolean> {
...
//#region Utils
function isTabGroup(obj: unknown): obj is vibecoda.TabGroup {
	const tabGroup = obj as vibecoda.TabGroup;
