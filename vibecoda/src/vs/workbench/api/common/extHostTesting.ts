...
interface ControllerInfo {
	...
	profiles: Map<number, vibecoda.TestRunProfile>;
	collection: ExtHostTestItemCollection;
	extension: IExtensionDescription;
	relatedCodeProvider?: vibecoda.TestRelatedCodeProvider;
...
const testResultInternalIDs = new WeakMap<vibecoda.TestRunResult, string>();
...
export class ExtHostTesting extends Disposable implements ExtHostTestingShape {
	...
	private readonly followupProviders = new Set<vibecoda.TestFollowupProvider>();
	private readonly testFollowups = new Map<number, vibecoda.Command>();

	public onResultsChanged = this.resultsChangedEmitter.event;
	public results: ReadonlyArray<vibecoda.TestRunResult> = [];
	...
	/**
	 * Implements vibecoda.test.registerTestProvider
	 */
	public createTestController(extension: IExtensionDescription, controllerId: string, label: string, refreshHandler?: (token: CancellationToken) => Thenable<void> | void): vibecoda.TestController {
		...
		const profiles = new Map<number, vibecoda.TestRunProfile>();
		...
		const controller: vibecoda.TestController = {
			...
			set relatedCodeProvider(value: vibecoda.TestRelatedCodeProvider | undefined) {
				checkProposedApiEnabled(extension, 'testRelatedCode');
				info.relatedCodeProvider = value;
				proxy.$updateController(controllerId, { capabilities: getCapability() });
			},
			createRunProfile: (label, group, runHandler, isDefault, tag?: vibecoda.TestTag | undefined, supportsContinuousRun?: boolean) => {
			...
			get resolveHandler() {
				return collection.resolveHandler as undefined | ((item?: vibecoda.TestItem) => void);
			},
			dispose: () => {
				disposable.dispose();
			},
	...
	/**
	 * Implements vibecoda.test.createTestObserver
	 */
	...
	/**
	 * Implements vibecoda.test.runTests
	 */
	public async runTests(req: vibecoda.TestRunRequest, token = CancellationToken.None) {
		const profile = tryGetProfileFromTestRunReq(req);
		if (!profile) {
			throw new Error('The request passed to `vibecoda.test.runTests` must include a profile');
		}
	...
	/**
	 * Implements vibecoda.test.registerTestFollowupProvider
	 */
	public registerTestFollowupProvider(provider: vibecoda.TestFollowupProvider): vibecoda.Disposable {
	...
	async $getTestsRelatedToCode(uri: UriComponents, _position: IPosition, token: CancellationToken): Promise<string[]> {
		...
		await Promise.all([...this.controllers.values()].map(async (c) => {
			let tests: vibecoda.TestItem[] | undefined | null;
			try {
	...
	public async $provideTestFollowups(req: TestMessageFollowupRequest, token: CancellationToken): Promise<TestMessageFollowupResponse[]> {
		...
		let followups: vibecoda.Command[] = [];
	...
	public getMetadataForRun(run: vibecoda.TestRun) {
...
class TestRunTracker extends Disposable {
	...
	private running = 0;
	private readonly tasks = new Map</* task ID */string, { cts: CancellationTokenSource; run: vibecoda.TestRun }>();
	...
	private readonly publishedCoverage = new Map<string, { report: vibecoda.FileCoverage; extIds: string[] }>();
	...
	constructor(
		...
		private readonly profile: vibecoda.TestRunProfile | undefined,
	...
	public getTaskIdForRun(run: vibecoda.TestRun) {
	...
	public async getCoverageDetails(id: string, testId: string | undefined, token: CancellationToken): Promise<vibecoda.FileCoverageDetail[]> {
		...
		let testItem: vibecoda.TestItem | undefined;
	...
	public createRun(name: string | undefined): vibecoda.TestRun {
		const runId = this.dto.id;
		const ctrlId = this.dto.controllerId;
		const taskId = generateUuid();

		const guardTestMutation = <Args extends unknown[]>(fn: (test: vibecoda.TestItem, ...args: Args) => void) =>
			(test: vibecoda.TestItem, ...args: Args) => {
				if (ended) {
		...
		const appendMessages = (test: vibecoda.TestItem, messages: vibecoda.TestMessage | readonly vibecoda.TestMessage[]) => {
		...
		const run: vibecoda.TestRun = {
			...
			//#endregion
			appendOutput: (output, location?: vibecoda.Location, test?: vibecoda.TestItem) => {
				if (ended) {
					return;
				}
	...
	private ensureTestIsKnown(test: vibecoda.TestItem) {
...
export class TestRunCoordinator {
	private readonly tracked = new Map<vibecoda.TestRunRequest, TestRunTracker>();
	 ...
	 */
	public getCoverageDetails(id: string, testId: string | undefined, token: vibecoda.CancellationToken) {
	 ...
	 */
	public prepareForMainThreadTestRun(extension: IExtensionDescription, req: vibecoda.TestRunRequest, dto: TestRunDto, profile: vibecoda.TestRunProfile, token: CancellationToken) {
	 ...
	 */
	public createTestRun(extension: IExtensionDescription, controllerId: string, collection: ExtHostTestItemCollection, request: vibecoda.TestRunRequest, name: string | undefined, persist: boolean): vibecoda.TestRun {
	...
	private getTracker(req: vibecoda.TestRunRequest, dto: TestRunDto, profile: vibecoda.TestRunProfile | undefined, extension: IExtensionDescription, token?: CancellationToken) {
...
const tryGetProfileFromTestRunReq = (request: vibecoda.TestRunRequest) => {
	if (!request.profile) {
		return undefined;
	}
...
export class TestRunDto {
	public static fromPublic(controllerId: string, collection: ExtHostTestItemCollection, request: vibecoda.TestRunRequest, persist: boolean) {
...
class MirroredChangeCollector implements IncrementalChangeCollector<MirroredCollectionTestItem> {
	...
	constructor(private readonly emitter: Emitter<vibecoda.TestsChangeEvent>) {
	}
	 ...
	 * @inheritdoc
	 */
	public getChangeEvent(): vibecoda.TestsChangeEvent {
...
class MirroredTestCollection extends AbstractIncrementalTestCollection<MirroredCollectionTestItem> {
	private changeEmitter = new Emitter<vibecoda.TestsChangeEvent>();
	 ...
	 */
	public getMirroredTestDataByReference(item: vibecoda.TestItem) {
	...
	protected createItem(item: InternalTestItem, parent?: MirroredCollectionTestItem): MirroredCollectionTestItem {
		return {
			...
			revived: Convert.TestItem.toPlain(item.item) as vibecoda.TestItem,
...
class TestObservers {
	...
	public checkout(): vibecoda.TestObserver {
		if (!this.current) {
	 ...
	 */
	public getMirroredTestDataByReference(ref: vibecoda.TestItem) {
...
export class TestRunProfileImpl extends TestRunProfileBase implements vibecoda.TestRunProfile {
	...
	public set tag(tag: vibecoda.TestTag | undefined) {
	...
	constructor(
		...
		public runHandler: (request: vibecoda.TestRunRequest, token: vibecoda.CancellationToken) => Thenable<void> | void,
		_isDefault = false,
		public _tag: vibecoda.TestTag | undefined = undefined,
...
function findTestInResultSnapshot(extId: TestId, snapshot: readonly Readonly<vibecoda.TestResultSnapshot>[]) {
