...
interface ProviderWithMetadata {
	label: string;
	provider: vibecoda.AuthenticationProvider;
	disposable?: vibecoda.Disposable;
	options: vibecoda.AuthenticationProviderOptions;
}
...
export class ExtHostAuthentication implements ExtHostAuthenticationShape {
	...
	private _onDidChangeSessions = new Emitter<vibecoda.AuthenticationSessionsChangeEvent & { extensionIdFilter?: string[] }>();
	private _getSessionTaskSingler = new TaskSingler<vibecoda.AuthenticationSession | undefined>();
	 ...
	 */
	getExtensionScopedSessionsEvent(extensionId: string): Event<vibecoda.AuthenticationSessionsChangeEvent> {
	...
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopesOrRequest: readonly string[] | vibecoda.AuthenticationWwwAuthenticateRequest, options: vibecoda.AuthenticationGetSessionOptions & ({ createIfNone: true } | { forceNewSession: true } | { forceNewSession: vibecoda.AuthenticationForceNewSessionOptions })): Promise<vibecoda.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopesOrRequest: readonly string[] | vibecoda.AuthenticationWwwAuthenticateRequest, options: vibecoda.AuthenticationGetSessionOptions & { forceNewSession: true }): Promise<vibecoda.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopesOrRequest: readonly string[] | vibecoda.AuthenticationWwwAuthenticateRequest, options: vibecoda.AuthenticationGetSessionOptions & { forceNewSession: vibecoda.AuthenticationForceNewSessionOptions }): Promise<vibecoda.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopesOrRequest: readonly string[] | vibecoda.AuthenticationWwwAuthenticateRequest, options: vibecoda.AuthenticationGetSessionOptions): Promise<vibecoda.AuthenticationSession | undefined>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopesOrRequest: readonly string[] | vibecoda.AuthenticationWwwAuthenticateRequest, options: vibecoda.AuthenticationGetSessionOptions = {}): Promise<vibecoda.AuthenticationSession | undefined> {
		const extensionId = ExtensionIdentifier.toKey(requestingExtension.identifier);
		const keys: (keyof vibecoda.AuthenticationGetSessionOptions)[] = Object.keys(options) as (keyof vibecoda.AuthenticationGetSessionOptions)[];
		...
		if (isAuthenticationWwwAuthenticateRequest(scopesOrRequest)) {
			const challenge = scopesOrRequest as vibecoda.AuthenticationWwwAuthenticateRequest;
	...
	$createSession(providerId: string, scopes: string[], options: vibecoda.AuthenticationProviderSessionOptions): Promise<vibecoda.AuthenticationSession> {
	...
	$getSessions(providerId: string, scopes: ReadonlyArray<string> | undefined, options: vibecoda.AuthenticationProviderSessionOptions): Promise<ReadonlyArray<vibecoda.AuthenticationSession>> {
		return this._providerOperations.queue(providerId, async () => {
			const providerData = this._authenticationProviders.get(providerId);
			if (providerData) {
				options.authorizationServer = URI.revive(options.authorizationServer);
	...
	$getSessionsFromChallenges(providerId: string, constraint: vibecoda.AuthenticationConstraint, options: vibecoda.AuthenticationProviderSessionOptions): Promise<ReadonlyArray<vibecoda.AuthenticationSession>> {
		return this._providerOperations.queue(providerId, async () => {
			const providerData = this._authenticationProviders.get(providerId);
			if (providerData) {
	...
	$createSessionFromChallenges(providerId: string, constraint: vibecoda.AuthenticationConstraint, options: vibecoda.AuthenticationProviderSessionOptions): Promise<vibecoda.AuthenticationSession> {
		return this._providerOperations.queue(providerId, async () => {
			const providerData = this._authenticationProviders.get(providerId);
			if (providerData) {
				const provider = providerData.provider;
...
export class DynamicAuthProvider implements vibecoda.AuthenticationProvider {
	readonly id: string;
	readonly label: string;

	private _onDidChangeSessions = new Emitter<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent>();
	...
	protected readonly _createFlows: Array<{
		label: string;
		handler: (scopes: string[], progress: vibecoda.Progress<{ message: string }>, token: vibecoda.CancellationToken) => Promise<IAuthorizationTokenResponse>;
	}>;
	...
	async getSessions(scopes: readonly string[] | undefined, _options: vibecoda.AuthenticationProviderSessionOptions): Promise<vibecoda.AuthenticationSession[]> {
	...
	async createSession(scopes: string[], _options: vibecoda.AuthenticationProviderSessionOptions): Promise<vibecoda.AuthenticationSession> {
	...
	private async _createWithUrlHandler(scopes: string[], progress: vibecoda.Progress<IProgressStep>, token: vibecoda.CancellationToken): Promise<IAuthorizationTokenResponse> {
		...
		const redirectUri = 'https://vibecoda.dev/redirect';
...
class TokenStore implements Disposable {
	...
	private readonly _sessionsObservable: IObservable<vibecoda.AuthenticationSession[]>;

	private readonly _onDidChangeSessions = new Emitter<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent>();
	readonly onDidChangeSessions = this._onDidChangeSessions.event;
	...
	get sessions(): vibecoda.AuthenticationSession[] {
	...
	private _registerChangeEventAutorun(): IDisposable {
		let previousSessions: vibecoda.AuthenticationSession[] = [];
		return autorun((reader) => {
			...
			const added: vibecoda.AuthenticationSession[] = [];
			const removed: vibecoda.AuthenticationSession[] = [];
	...
	private _getSessionFromToken(token: IAuthorizationTokenResponse): vibecoda.AuthenticationSession {
