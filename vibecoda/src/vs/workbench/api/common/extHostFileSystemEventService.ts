...
class FileSystemWatcher implements vibecoda.FileSystemWatcher {

	private readonly session = Math.random();

	private readonly _onDidCreate = new Emitter<vibecoda.Uri>();
	private readonly _onDidChange = new Emitter<vibecoda.Uri>();
	private readonly _onDidDelete = new Emitter<vibecoda.Uri>();
	...
	constructor(mainContext: IMainContext, configuration: ExtHostConfigProvider, workspace: IExtHostWorkspace, extension: IExtensionDescription, dispatcher: Event<FileSystemEvents>, globPattern: string | IRelativePatternDto, options: FileSystemWatcherCreateOptions) {
		...
		// unexpected.
		// https://github.com/microsoft/vibecoda/issues/3025
	...
	private ensureWatching(mainContext: IMainContext, workspace: IExtHostWorkspace, configuration: ExtHostConfigProvider, extension: IExtensionDescription, globPattern: string | IRelativePatternDto, options: FileSystemWatcherCreateOptions, correlate: boolean | undefined): Disposable {
		...
		// Uncorrelated: adjust includes and excludes based on settings
			...
			// (https://github.com/microsoft/vibecoda/issues/148245)
			else if (!recursive) {
				const workspaceFolder = workspace.getWorkspaceFolder(URI.revive(globPattern.baseUri));
				if (workspaceFolder) {
					const watcherExcludes = configuration.getConfiguration('files', workspaceFolder).get<IGlobPatterns>('watcherExclude');
	...
	get onDidCreate(): Event<vibecoda.Uri> {
		return this._onDidCreate.event;
	}

	get onDidChange(): Event<vibecoda.Uri> {
		return this._onDidChange.event;
	}

	get onDidDelete(): Event<vibecoda.Uri> {
...
export class ExtHostFileSystemEventService implements ExtHostFileSystemEventServiceShape {
	...
	private readonly _onDidRenameFile = new Emitter<vibecoda.FileRenameEvent>();
	private readonly _onDidCreateFile = new Emitter<vibecoda.FileCreateEvent>();
	private readonly _onDidDeleteFile = new Emitter<vibecoda.FileDeleteEvent>();
	private readonly _onWillRenameFile = new AsyncEmitter<vibecoda.FileWillRenameEvent>();
	private readonly _onWillCreateFile = new AsyncEmitter<vibecoda.FileWillCreateEvent>();
	private readonly _onWillDeleteFile = new AsyncEmitter<vibecoda.FileWillDeleteEvent>();

	readonly onDidRenameFile: Event<vibecoda.FileRenameEvent> = this._onDidRenameFile.event;
	readonly onDidCreateFile: Event<vibecoda.FileCreateEvent> = this._onDidCreateFile.event;
	readonly onDidDeleteFile: Event<vibecoda.FileDeleteEvent> = this._onDidDeleteFile.event;
	...
	createFileSystemWatcher(workspace: IExtHostWorkspace, configProvider: ExtHostConfigProvider, extension: IExtensionDescription, globPattern: vibecoda.GlobPattern, options: FileSystemWatcherCreateOptions): vibecoda.FileSystemWatcher {
	...
	getOnWillRenameFileEvent(extension: IExtensionDescription): Event<vibecoda.FileWillRenameEvent> {
		return this._createWillExecuteEvent(extension, this._onWillRenameFile);
	}

	getOnWillCreateFileEvent(extension: IExtensionDescription): Event<vibecoda.FileWillCreateEvent> {
		return this._createWillExecuteEvent(extension, this._onWillCreateFile);
	}

	getOnWillDeleteFileEvent(extension: IExtensionDescription): Event<vibecoda.FileWillDeleteEvent> {
