...
class CustomDocumentStoreEntry {
	...
	constructor(
		public readonly document: vibecoda.CustomDocument,
		private readonly _storagePath: URI | undefined,
	) { }

	private readonly _edits = new Cache<vibecoda.CustomDocumentEditEvent>('custom documents');

	private _backup?: vibecoda.CustomDocumentBackup;

	addEdit(item: vibecoda.CustomDocumentEditEvent): number {
	...
	updateBackup(backup: vibecoda.CustomDocumentBackup): void {
		this._backup?.delete();
		this._backup = backup;
	}
	...
	private getEdit(editId: number): vibecoda.CustomDocumentEditEvent {
...
class CustomDocumentStore {
	...
	public get(viewType: string, resource: vibecoda.Uri): CustomDocumentStoreEntry | undefined {
		return this._documents.get(this.key(viewType, resource));
	}

	public add(viewType: string, document: vibecoda.CustomDocument, storagePath: URI | undefined): CustomDocumentStoreEntry {
	...
	public delete(viewType: string, document: vibecoda.CustomDocument) {
		const key = this.key(viewType, document.uri);
		this._documents.delete(key);
	}

	private key(viewType: string, resource: vibecoda.Uri): string {
...
type ProviderEntry = {
	...
	readonly provider: vibecoda.CustomTextEditorProvider;
} | {
	readonly extension: IExtensionDescription;
	readonly type: CustomEditorType.Custom;
	readonly provider: vibecoda.CustomReadonlyEditorProvider;
};

class EditorProviderStore {
	private readonly _providers = new Map<string, ProviderEntry>();

	public addTextProvider(viewType: string, extension: IExtensionDescription, provider: vibecoda.CustomTextEditorProvider): vibecoda.Disposable {
		return this.add(viewType, { type: CustomEditorType.Text, extension, provider });
	}

	public addCustomProvider(viewType: string, extension: IExtensionDescription, provider: vibecoda.CustomReadonlyEditorProvider): vibecoda.Disposable {
	...
	private add(viewType: string, entry: ProviderEntry): vibecoda.Disposable {
...
export class ExtHostCustomEditors implements extHostProtocol.ExtHostCustomEditorsShape {
	...
	public registerCustomEditorProvider(
		...
		viewType: string,
		provider: vibecoda.CustomReadonlyEditorProvider | vibecoda.CustomTextEditorProvider,
		options: { webviewOptions?: vibecoda.WebviewPanelOptions; supportsMultipleEditorsPerDocument?: boolean },
	): vibecoda.Disposable {
	...
	async $onMoveCustomEditor(handle: string, newResourceComponents: UriComponents, viewType: string): Promise<void> {
		...
		if (!(entry.provider as vibecoda.CustomTextEditorProvider).moveCustomTextEditor) {
		...
		await (entry.provider as vibecoda.CustomTextEditorProvider).moveCustomTextEditor!(document, webview, CancellationToken.None);
	}
	...
	private getCustomEditorProvider(viewType: string): vibecoda.CustomEditorProvider {
		const entry = this._editorProviders.get(viewType);
...
function isCustomEditorProviderWithEditingCapability(provider: vibecoda.CustomTextEditorProvider | vibecoda.CustomEditorProvider | vibecoda.CustomReadonlyEditorProvider): provider is vibecoda.CustomEditorProvider {
	return !!(provider as vibecoda.CustomEditorProvider).onDidChangeCustomDocument;
}

function isCustomTextEditorProvider(provider: vibecoda.CustomReadonlyEditorProvider<vibecoda.CustomDocument> | vibecoda.CustomTextEditorProvider): provider is vibecoda.CustomTextEditorProvider {
	return typeof (provider as vibecoda.CustomTextEditorProvider).resolveCustomTextEditor === 'function';
}

function isEditEvent(e: vibecoda.CustomDocumentContentChangeEvent | vibecoda.CustomDocumentEditEvent): e is vibecoda.CustomDocumentEditEvent {
	return typeof (e as vibecoda.CustomDocumentEditEvent).undo === 'function'
		&& typeof (e as vibecoda.CustomDocumentEditEvent).redo === 'function';
}
