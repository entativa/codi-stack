...
type LanguageModelProviderData = {
	...
	readonly provider: vibecoda.LanguageModelChatProvider;
};

type LMResponsePart = vibecoda.LanguageModelTextPart | vibecoda.LanguageModelToolCallPart | vibecoda.LanguageModelDataPart | vibecoda.LanguageModelThinkingPart;


class LanguageModelResponse {

	readonly apiObject: vibecoda.LanguageModelChatResponse;
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	private readonly _localModels = new Map<string, { metadata: ILanguageModelChatMetadata; info: vibecoda.LanguageModelChatInformation }>();
	...
	private readonly _ignoredFileProviders = new Map<number, vibecoda.LanguageModelIgnoredFileProvider>();
	private _languageModelProxyProvider: vibecoda.LanguageModelProxyProvider | undefined;
	...
	registerLanguageModelChatProvider(extension: IExtensionDescription, vendor: string, provider: vibecoda.LanguageModelChatProvider): IDisposable {
	...
	async $provideLanguageModelChatInfo(vendor: string, options: { silent: boolean }, token: CancellationToken): Promise<ILanguageModelChatMetadataAndIdentifier[]> {
		...
		const modelInformation: vibecoda.LanguageModelChatInformation[] = await data.provider.provideLanguageModelChatInformation(options, token) ?? [];
	...
	async $startChatRequest(modelId: string, requestId: number, from: ExtensionIdentifier, messages: SerializableObjectWithBuffers<IChatMessage[]>, options: vibecoda.LanguageModelChatRequestOptions, token: CancellationToken): Promise<void> {
		...
		const progress = new Progress<vibecoda.LanguageModelTextPart | vibecoda.LanguageModelToolCallPart | vibecoda.LanguageModelDataPart | vibecoda.LanguageModelThinkingPart>(async fragment => {
	...
	async getDefaultLanguageModel(extension: IExtensionDescription, forceResolveModels?: boolean): Promise<vibecoda.LanguageModelChat | undefined> {
	...
	async getLanguageModelByIdentifier(extension: IExtensionDescription, modelId: string): Promise<vibecoda.LanguageModelChat | undefined> {
		...
		let apiObject: vibecoda.LanguageModelChat | undefined;
		if (!apiObject) {
			const that = this;
			apiObject = {
				id: model.info.id,
	...
	async selectLanguageModels(extension: IExtensionDescription, selector: vibecoda.LanguageModelChatSelector) {
		...
		const result: vibecoda.LanguageModelChat[] = [];
	...
	private async _sendChatRequest(extension: IExtensionDescription, languageModelId: string, messages: vibecoda.LanguageModelChatMessage2[], options: vibecoda.LanguageModelChatRequestOptions, token: CancellationToken) {
	...
	private _convertMessages(extension: IExtensionDescription, messages: vibecoda.LanguageModelChatMessage2[]) {
	...
	private async _computeTokenLength(modelId: string, value: string | vibecoda.LanguageModelChatMessage2, token: vibecoda.CancellationToken): Promise<number> {
	...
	createLanguageModelAccessInformation(from: Readonly<IExtensionDescription>): vibecoda.LanguageModelAccessInformation {
		...
		return {
			...
			canSendRequest(chat: vibecoda.LanguageModelChat): boolean | undefined {
				return true;
				// TODO @lramos15 - Fix
	...
	fileIsIgnored(extension: IExtensionDescription, uri: vibecoda.Uri, token: vibecoda.CancellationToken = CancellationToken.None): Promise<boolean> {
	...
	async getModelProxy(extension: IExtensionDescription): Promise<vibecoda.LanguageModelProxy> {
	...
	registerIgnoredFileProvider(extension: IExtensionDescription, provider: vibecoda.LanguageModelIgnoredFileProvider): vibecoda.Disposable {
	...
	registerLanguageModelProxyProvider(extension: IExtensionDescription, provider: vibecoda.LanguageModelProxyProvider): vibecoda.Disposable {
		checkProposedApiEnabled(extension, 'chatParticipantPrivate');
