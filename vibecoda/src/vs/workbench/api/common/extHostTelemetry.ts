...
export class ExtHostTelemetry extends Disposable implements ExtHostTelemetryShape {
	...
	private readonly _onDidChangeTelemetryConfiguration = this._register(new Emitter<vibecoda.TelemetryConfiguration>());
	readonly onDidChangeTelemetryConfiguration: Event<vibecoda.TelemetryConfiguration> = this._onDidChangeTelemetryConfiguration.event;
	...
	getTelemetryDetails(): vibecoda.TelemetryConfiguration {
		return {
	...
	instantiateLogger(extension: IExtensionDescription, sender: vibecoda.TelemetrySender, options?: vibecoda.TelemetryLoggerOptions) {
	...
	getBuiltInCommonProperties(extension: IExtensionDescription): ICommonProperties {
		...
		commonProperties['common.vibecodamachineid'] = this.initData.telemetryInfo.machineId;
		commonProperties['common.vibecodasessionid'] = this.initData.telemetryInfo.sessionId;
		commonProperties['common.vibecodacommithash'] = this.initData.commit;
		...
		commonProperties['common.vibecodaversion'] = this.initData.version;
		commonProperties['common.vibecodareleasedate'] = this.initData.date;
...
export class ExtHostTelemetryLogger {
	...
	static validateSender(sender: vibecoda.TelemetrySender): void {
	...
	private readonly _onDidChangeEnableStates = new Emitter<vibecoda.TelemetryLogger>();
	...
	private _apiObject: vibecoda.TelemetryLogger | undefined;
	private _sender: vibecoda.TelemetrySender | undefined;

	constructor(
		sender: vibecoda.TelemetrySender,
		options: vibecoda.TelemetryLoggerOptions | undefined,
	...
	private logEvent(eventName: string, data?: Record<string, any>): void {
		...
		// If it's a built-in extension (vibecoda publisher) we don't prefix the publisher and only the ext name
		if (this._extension.publisher === 'vibecoda') {
	...
	get apiTelemetryLogger(): vibecoda.TelemetryLogger {
		if (!this._apiObject) {
			const that = this;
			const obj: vibecoda.TelemetryLogger = {
	...
	dispose(): void {
		if (this._sender?.flush) {
			let tempSender: vibecoda.TelemetrySender | undefined = this._sender;
