...
export interface IExtHostDataChannels extends ExtHostDataChannelsShape {
	...
	createDataChannel<T>(extension: IExtensionDescription, channelId: string): vibecoda.DataChannel<T>;
}
...
class DataChannelImpl<T> extends Disposable implements vibecoda.DataChannel<T> {
	private readonly _onDidReceiveData = new Emitter<vibecoda.DataChannelEvent<T>>();
	public readonly onDidReceiveData: Event<vibecoda.DataChannelEvent<T>> = this._onDidReceiveData.event;
