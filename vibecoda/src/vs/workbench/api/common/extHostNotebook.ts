...
export class ExtHostNotebookController implements ExtHostNotebookShape {
	...
	private readonly _notebookStatusBarItemProviders = new Map<number, vibecoda.NotebookCellStatusBarItemProvider>();
	private readonly _documents = new ResourceMap<ExtHostNotebookDocument>();
	private readonly _editors = new Map<string, ExtHostNotebookEditor>();
	private readonly _commandsConverter: CommandsConverter;

	private readonly _onDidChangeActiveNotebookEditor = new Emitter<vibecoda.NotebookEditor | undefined>();
	...
	get visibleNotebookEditors(): vibecoda.NotebookEditor[] {
		return this._visibleNotebookEditors.map(editor => editor.apiEditor);
	}

	private _onDidOpenNotebookDocument = new Emitter<vibecoda.NotebookDocument>();
	readonly onDidOpenNotebookDocument: Event<vibecoda.NotebookDocument> = this._onDidOpenNotebookDocument.event;
	private _onDidCloseNotebookDocument = new Emitter<vibecoda.NotebookDocument>();
	readonly onDidCloseNotebookDocument: Event<vibecoda.NotebookDocument> = this._onDidCloseNotebookDocument.event;

	private _onDidChangeVisibleNotebookEditors = new Emitter<vibecoda.NotebookEditor[]>();
	...
	getIdByEditor(editor: vibecoda.NotebookEditor): string | undefined {
	...
	private static _convertNotebookRegistrationData(extension: IExtensionDescription, registration: vibecoda.NotebookRegistrationData | undefined): INotebookContributionData | undefined {
		if (!registration) {
			return;
		}
	...
	registerNotebookCellStatusBarItemProvider(extension: IExtensionDescription, notebookType: string, provider: vibecoda.NotebookCellStatusBarItemProvider) {
	...
	async createNotebookDocument(options: { viewType: string; content?: vibecoda.NotebookData }): Promise<URI> {
	...
	async openNotebookDocument(uri: URI): Promise<vibecoda.NotebookDocument> {
	...
	async showNotebookDocument(notebook: vibecoda.NotebookDocument, options?: vibecoda.NotebookDocumentShowOptions): Promise<vibecoda.NotebookEditor> {
	...
	private readonly _notebookSerializer = new Map<number, { viewType: string; serializer: vibecoda.NotebookSerializer; options: vibecoda.NotebookDocumentContentOptions | undefined }>();

	registerNotebookSerializer(extension: IExtensionDescription, viewType: string, serializer: vibecoda.NotebookSerializer, options?: vibecoda.NotebookDocumentContentOptions, registration?: vibecoda.NotebookRegistrationData): vibecoda.Disposable {
	...
	async $saveNotebook(handle: number, uriComponents: UriComponents, versionId: number, options: files.IWriteFileOptions, token: CancellationToken): Promise<INotebookPartialFileStatsWithMetadata | files.FileOperationError> {
		...
		try {
			...
			const data: vibecoda.NotebookData = {
					...
					return this._extHostSearch.doInternalFileSearchWithCustomCallback(query, token, (data) => {
						data.forEach(uri => {
							...
							const hasOtherMatches = otherViewTypeFileTargets.some(target => {
								...
								// https://github.com/microsoft/vibecoda/blob/ac1631528e67637da65ec994c6dc35d73f6e33cc/src/vs/workbench/services/editor/browser/editorResolverService.ts#L359-L366
						...
						// temporary fix for https://github.com/microsoft/vibecoda/issues/205044: don't show notebook results for remotehub repos.
						if (err.code === 'ENOENT') {
	...
	$acceptDocumentAndEditorsDelta(delta: SerializableObjectWithBuffers<INotebookDocumentsAndEditorsDelta>): void {
		...
		if (delta.value.addedDocuments) {
			...
			for (const modelData of delta.value.addedDocuments) {
				...
				// add cell document as vibecoda.TextDocument
	...
	private static _registerApiCommands(extHostCommands: ExtHostCommands) {
		...
		const commandDataToNotebook = new ApiCommand(
			'vibecoda.executeDataToNotebook', '_executeDataToNotebook', 'Invoke notebook serializer',
			...
			new ApiCommandResult<SerializableObjectWithBuffers<NotebookDataDto>, vibecoda.NotebookData>('Notebook Data', data => typeConverters.NotebookData.to(data.value))
		);

		const commandNotebookToData = new ApiCommand(
			'vibecoda.executeNotebookToData', '_executeNotebookToData', 'Invoke notebook serializer',
			[notebookTypeArg, new ApiCommandArgument<vibecoda.NotebookData, SerializableObjectWithBuffers<NotebookDataDto>>('NotebookData', 'Notebook data to convert to bytes', v => true, v => new SerializableObjectWithBuffers(typeConverters.NotebookData.from(v)))],
