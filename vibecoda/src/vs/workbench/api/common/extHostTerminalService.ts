...
import { Disposable as VibecodaDisposable, EnvironmentVariableMutatorType, TerminalExitReason, TerminalCompletionItem } from './extHostTypes.js';
...
export interface IExtHostTerminalService extends ExtHostTerminalServiceShape, IDisposable {
	...
	activeTerminal: vibecoda.Terminal | undefined;
	terminals: vibecoda.Terminal[];

	readonly onDidCloseTerminal: Event<vibecoda.Terminal>;
	readonly onDidOpenTerminal: Event<vibecoda.Terminal>;
	readonly onDidChangeActiveTerminal: Event<vibecoda.Terminal | undefined>;
	readonly onDidChangeTerminalDimensions: Event<vibecoda.TerminalDimensionsChangeEvent>;
	readonly onDidChangeTerminalState: Event<vibecoda.Terminal>;
	readonly onDidWriteTerminalData: Event<vibecoda.TerminalDataWriteEvent>;
	readonly onDidExecuteTerminalCommand: Event<vibecoda.TerminalExecutedCommand>;
	readonly onDidChangeShell: Event<string>;

	createTerminal(name?: string, shellPath?: string, shellArgs?: readonly string[] | string): vibecoda.Terminal;
	createTerminalFromOptions(options: vibecoda.TerminalOptions, internalOptions?: ITerminalInternalOptions): vibecoda.Terminal;
	createExtensionTerminal(options: vibecoda.ExtensionTerminalOptions): vibecoda.Terminal;
	attachPtyToTerminal(id: number, pty: vibecoda.Pseudoterminal): void;
	getDefaultShell(useAutomationShell: boolean): string;
	getDefaultShellArgs(useAutomationShell: boolean): string[] | string;
	registerLinkProvider(provider: vibecoda.TerminalLinkProvider): vibecoda.Disposable;
	registerProfileProvider(extension: IExtensionDescription, id: string, provider: vibecoda.TerminalProfileProvider): vibecoda.Disposable;
	registerTerminalQuickFixProvider(id: string, extensionId: string, provider: vibecoda.TerminalQuickFixProvider): vibecoda.Disposable;
	...
	getTerminalIdByApiObject(apiTerminal: vibecoda.Terminal): number | null;
	registerTerminalCompletionProvider(extension: IExtensionDescription, provider: vibecoda.TerminalCompletionProvider<vibecoda.TerminalCompletionItem>, ...triggerCharacters: string[]): vibecoda.Disposable;
}

interface IEnvironmentVariableCollection extends vibecoda.EnvironmentVariableCollection {
	getScoped(scope: vibecoda.EnvironmentVariableScope): vibecoda.EnvironmentVariableCollection;
}
...
export class ExtHostTerminal extends Disposable {
	...
	private _exitStatus: vibecoda.TerminalExitStatus | undefined;
	private _state: vibecoda.TerminalState = { isInteractedWith: false, shell: undefined };
	private _selection: string | undefined;

	shellIntegration: vibecoda.TerminalShellIntegration | undefined;

	public isOpen: boolean = false;

	readonly value: vibecoda.Terminal;
	...
	constructor(
		...
		private readonly _creationOptions: vibecoda.TerminalOptions | vibecoda.ExtensionTerminalOptions,
		...
		this.value = {
			...
			get creationOptions(): Readonly<vibecoda.TerminalOptions | vibecoda.ExtensionTerminalOptions> {
				return that._creationOptions;
			},
			get exitStatus(): vibecoda.TerminalExitStatus | undefined {
				return that._exitStatus;
			},
			get state(): vibecoda.TerminalState {
				return that._state;
			},
			get selection(): string | undefined {
				return that._selection;
			},
			get shellIntegration(): vibecoda.TerminalShellIntegration | undefined {
			...
			get dimensions(): vibecoda.TerminalDimensions | undefined {
	...
	public async create(
		options: vibecoda.TerminalOptions,
	...
	public async createExtensionTerminal(location?: TerminalLocation | vibecoda.TerminalEditorLocationOptions | vibecoda.TerminalSplitLocationOptions, internalOptions?: ITerminalInternalOptions, parentTerminal?: ExtHostTerminalIdentifier, iconPath?: TerminalIcon, color?: ThemeColor, shellIntegrationNonce?: string): Promise<number> {
		if (typeof this._id !== 'string') {
			throw new Error('Terminal has already been created');
		}
		await this._proxy.$createTerminal(this._id, {
	...
	private _serializeParentTerminal(location?: TerminalLocation | vibecoda.TerminalEditorLocationOptions | vibecoda.TerminalSplitLocationOptions, parentTerminal?: ExtHostTerminalIdentifier): TerminalLocation | { viewColumn: EditorGroupColumn; preserveFocus?: boolean } | { parentTerminal: ExtHostTerminalIdentifier } | undefined {
...
class ExtHostPseudoterminal implements ITerminalChildProcess {
	...
	constructor(private readonly _pty: vibecoda.Pseudoterminal) { }
	...
	acknowledgeDataEvent(charCount: number): void {
		...
		// implemented it will need new pause and resume Vibecoda APIs.
	}
...
interface ICachedLinkEntry {
	provider: vibecoda.TerminalLinkProvider;
	link: vibecoda.TerminalLink;
}
...
export abstract class BaseExtHostTerminalService extends Disposable implements IExtHostTerminalService, ExtHostTerminalServiceShape {
	...
	private readonly _linkProviders: Set<vibecoda.TerminalLinkProvider> = new Set();
	private readonly _completionProviders: Map<string, vibecoda.TerminalCompletionProvider<vibecoda.TerminalCompletionItem>> = new Map();
	private readonly _profileProviders: Map<string, vibecoda.TerminalProfileProvider> = new Map();
	private readonly _quickFixProviders: Map<string, vibecoda.TerminalQuickFixProvider> = new Map();
	private readonly _terminalLinkCache: Map<number, Map<number, ICachedLinkEntry>> = new Map();
	private readonly _terminalLinkCancellationSource: Map<number, CancellationTokenSource> = new Map();

	public get activeTerminal(): vibecoda.Terminal | undefined { return this._activeTerminal?.value; }
	public get terminals(): vibecoda.Terminal[] { return this._terminals.map(term => term.value); }

	protected readonly _onDidCloseTerminal = new Emitter<vibecoda.Terminal>();
	readonly onDidCloseTerminal = this._onDidCloseTerminal.event;
	protected readonly _onDidOpenTerminal = new Emitter<vibecoda.Terminal>();
	readonly onDidOpenTerminal = this._onDidOpenTerminal.event;
	protected readonly _onDidChangeActiveTerminal = new Emitter<vibecoda.Terminal | undefined>();
	readonly onDidChangeActiveTerminal = this._onDidChangeActiveTerminal.event;
	protected readonly _onDidChangeTerminalDimensions = new Emitter<vibecoda.TerminalDimensionsChangeEvent>();
	readonly onDidChangeTerminalDimensions = this._onDidChangeTerminalDimensions.event;
	protected readonly _onDidChangeTerminalState = new Emitter<vibecoda.Terminal>();
	...
	protected readonly _onDidWriteTerminalData = new Emitter<vibecoda.TerminalDataWriteEvent>({
	...
	protected readonly _onDidExecuteCommand = new Emitter<vibecoda.TerminalExecutedCommand>({
	...
	public abstract createTerminal(name?: string, shellPath?: string, shellArgs?: string[] | string): vibecoda.Terminal;
	public abstract createTerminalFromOptions(options: vibecoda.TerminalOptions, internalOptions?: ITerminalInternalOptions): vibecoda.Terminal;
	...
	public createExtensionTerminal(options: vibecoda.ExtensionTerminalOptions, internalOptions?: ITerminalInternalOptions): vibecoda.Terminal {
	...
	protected _serializeParentTerminal(options: vibecoda.TerminalOptions, internalOptions?: ITerminalInternalOptions): ITerminalInternalOptions {
	...
	public attachPtyToTerminal(id: number, pty: vibecoda.Pseudoterminal): void {
		...
		if (terminal) {
			if (terminal.setDimensions(cols, rows)) {
				this._onDidChangeTerminalDimensions.fire({
					...
					dimensions: terminal.value.dimensions as vibecoda.TerminalDimensions
				});
			}
		}
	}
	...
	public $acceptTerminalOpened(id: number, extHostTerminalId: string | undefined, name: string, shellLaunchConfigDto: IShellLaunchConfigDto): void {
		...
		const creationOptions: vibecoda.TerminalOptions = {
	...
	public registerProfileProvider(extension: IExtensionDescription, id: string, provider: vibecoda.TerminalProfileProvider): vibecoda.Disposable {
		...
		return new VibecodaDisposable(() => {
			this._profileProviders.delete(id);
			this._proxy.$unregisterProfileProvider(id);
		});
	}

	public registerTerminalCompletionProvider(extension: IExtensionDescription, provider: vibecoda.TerminalCompletionProvider<TerminalCompletionItem>, ...triggerCharacters: string[]): vibecoda.Disposable {
	...
	public registerTerminalQuickFixProvider(id: string, extensionId: string, provider: vibecoda.TerminalQuickFixProvider): vibecoda.Disposable {
	...
	public registerLinkProvider(provider: vibecoda.TerminalLinkProvider): vibecoda.Disposable {
	...
	public async $provideLinks(terminalId: number, line: string): Promise<ITerminalLinkDto[]> {
		...
		const context: vibecoda.TerminalLinkContext = { terminal: terminal.value, line };
		const promises: vibecoda.ProviderResult<{ provider: vibecoda.TerminalLinkProvider; links: vibecoda.TerminalLink[] }>[] = [];
	...
	public getTerminalIdByApiObject(terminal: vibecoda.Terminal): number | null {
...
class UnifiedEnvironmentVariableCollection extends Disposable {
	...
	getScopedEnvironmentVariableCollection(scope: vibecoda.EnvironmentVariableScope | undefined): IEnvironmentVariableCollection {
	...
	replace(variable: string, value: string, options: vibecoda.EnvironmentVariableMutatorOptions | undefined, scope: vibecoda.EnvironmentVariableScope | undefined): void {
	...
	append(variable: string, value: string, options: vibecoda.EnvironmentVariableMutatorOptions | undefined, scope: vibecoda.EnvironmentVariableScope | undefined): void {
	...
	prepend(variable: string, value: string, options: vibecoda.EnvironmentVariableMutatorOptions | undefined, scope: vibecoda.EnvironmentVariableScope | undefined): void {
	...
	private _setIfDiffers(variable: string, mutator: vibecoda.EnvironmentVariableMutator & { scope: vibecoda.EnvironmentVariableScope | undefined }): void {
	...
	get(variable: string, scope: vibecoda.EnvironmentVariableScope | undefined): vibecoda.EnvironmentVariableMutator | undefined {
	...
	private getKey(variable: string, scope: vibecoda.EnvironmentVariableScope | undefined) {
		const scopeKey = this.getScopeKey(scope);
		return scopeKey.length ? `${variable}:::${scopeKey}` : variable;
	}

	private getScopeKey(scope: vibecoda.EnvironmentVariableScope | undefined): string {
		return this.getWorkspaceKey(scope?.workspaceFolder) ?? '';
	}

	private getWorkspaceKey(workspaceFolder: vibecoda.WorkspaceFolder | undefined): string | undefined {
		return workspaceFolder ? workspaceFolder.uri.toString() : undefined;
	}

	public getVariableMap(scope: vibecoda.EnvironmentVariableScope | undefined): Map<string, vibecoda.EnvironmentVariableMutator> {
		const map = new Map<string, vibecoda.EnvironmentVariableMutator>();
	...
	delete(variable: string, scope: vibecoda.EnvironmentVariableScope | undefined): void {
		const key = this.getKey(variable, scope);
		this.map.delete(key);
		this._onDidChangeCollection.fire();
	}

	clear(scope: vibecoda.EnvironmentVariableScope | undefined): void {
	...
	setDescription(description: string | vibecoda.MarkdownString | undefined, scope: vibecoda.EnvironmentVariableScope | undefined): void {
	...
	public getDescription(scope: vibecoda.EnvironmentVariableScope | undefined): string | vibecoda.MarkdownString | undefined {
		const key = this.getScopeKey(scope);
		return this.descriptionMap.get(key)?.description;
	}

	private clearDescription(scope: vibecoda.EnvironmentVariableScope | undefined): void {
		const key = this.getScopeKey(scope);
...
class ScopedEnvironmentVariableCollection implements IEnvironmentVariableCollection {
	...
	constructor(
		...
		private readonly scope: vibecoda.EnvironmentVariableScope | undefined
	) {
	}

	getScoped(scope: vibecoda.EnvironmentVariableScope | undefined) {
		return this.collection.getScopedEnvironmentVariableCollection(scope);
	}

	replace(variable: string, value: string, options?: vibecoda.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.replace(variable, value, options, this.scope);
	}

	append(variable: string, value: string, options?: vibecoda.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.append(variable, value, options, this.scope);
	}

	prepend(variable: string, value: string, options?: vibecoda.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.prepend(variable, value, options, this.scope);
	}

	get(variable: string): vibecoda.EnvironmentVariableMutator | undefined {
		return this.collection.get(variable, this.scope);
	}

	forEach(callback: (variable: string, mutator: vibecoda.EnvironmentVariableMutator, collection: vibecoda.EnvironmentVariableCollection) => any, thisArg?: any): void {
		this.collection.getVariableMap(this.scope).forEach((value, variable) => callback.call(thisArg, variable, value, this), this.scope);
	}

	[Symbol.iterator](): IterableIterator<[variable: string, mutator: vibecoda.EnvironmentVariableMutator]> {
	...
	set description(description: string | vibecoda.MarkdownString | undefined) {
		this.collection.setDescription(description, this.scope);
	}

	get description(): string | vibecoda.MarkdownString | undefined {
...
function asTerminalIcon(iconPath?: vibecoda.Uri | { light: vibecoda.Uri; dark: vibecoda.Uri } | vibecoda.ThemeIcon): TerminalIcon | undefined {
...
function asTerminalColor(color?: vibecoda.ThemeColor): ThemeColor | undefined {
	return ThemeColor.isThemeColor(color) ? color as ThemeColor : undefined;
}

function convertMutator(mutator: IEnvironmentVariableMutator): vibecoda.EnvironmentVariableMutator {
	const newMutator = { ...mutator };
	delete newMutator.scope;
	newMutator.options = newMutator.options ?? undefined;
	// eslint-disable-next-line local/code-no-any-casts
	delete (newMutator as any).variable;
	return newMutator as vibecoda.EnvironmentVariableMutator;
}
