...
const testItemPropAccessor = <K extends keyof vibecoda.TestItem>(
	api: IExtHostTestItemApi,
	defaultValue: vibecoda.TestItem[K],
	equals: (a: vibecoda.TestItem[K], b: vibecoda.TestItem[K]) => boolean,
	toUpdate: (newValue: vibecoda.TestItem[K], oldValue: vibecoda.TestItem[K]) => ExtHostTestItemEvent,
) => {
	...
	return {
		...
		set(newValue: vibecoda.TestItem[K]) {
...
type WritableProps = Pick<vibecoda.TestItem, 'range' | 'label' | 'description' | 'sortText' | 'canResolveChildren' | 'busy' | 'error' | 'tags'>;

const strictEqualComparator = <T>(a: T, b: T) => a === b;

const propComparators: { [K in keyof Required<WritableProps>]: (a: vibecoda.TestItem[K], b: vibecoda.TestItem[K]) => boolean } = {
	range: (a, b) => {
		if (a === b) { return true; }
		if (!a || !b) { return false; }
		return a.isEqual(b);
...
const makePropDescriptors = (api: IExtHostTestItemApi, label: string): { [K in keyof Required<WritableProps>]: PropertyDescriptor } => ({
	range: (() => {
		let value: vibecoda.Range | undefined;
		const updateProps = evSetProps<vibecoda.Range | undefined>(r => ({ range: editorRange.Range.lift(Convert.Range.from(r)) }));
		return {
			enumerable: true,
			configurable: false,
			get() {
			...
			set(newValue: vibecoda.Range | undefined) {
...
export class TestItemImpl implements vibecoda.TestItem {
	public readonly id!: string;
	public readonly uri!: vibecoda.Uri | undefined;
	public readonly children!: ITestItemChildren<vibecoda.TestItem>;
	public readonly parent!: TestItemImpl | undefined;

	public range!: vibecoda.Range | undefined;
	public description!: string | undefined;
	public sortText!: string | undefined;
	public label!: string;
	public error!: string | vibecoda.MarkdownString;
	public busy!: boolean;
	public canResolveChildren!: boolean;
	public tags!: readonly vibecoda.TestTag[];

	/**
	 * Note that data is deprecated and here for back-compat only
	 */
	constructor(controllerId: string, id: string, label: string, uri: vibecoda.Uri | undefined) {
