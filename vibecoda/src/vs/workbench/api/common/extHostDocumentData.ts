...
export class ExtHostDocumentData extends MirrorTextModel {
	...
	private _document?: vibecoda.TextDocument;
	...
	get document(): vibecoda.TextDocument {
		if (!this._document) {
			...
			this._document = {
				...
				lineAt(lineOrPos: number | vibecoda.Position) { return that._lineAt(lineOrPos); },
	...
	private _getTextInRange(_range: vibecoda.Range): string {
	...
	private _lineAt(lineOrPosition: number | vibecoda.Position): vibecoda.TextLine {
	...
	private _offsetAt(position: vibecoda.Position): number {
		position = this._validatePosition(position);
		this._ensureLineStarts();
		return this._lineStarts!.getPrefixSum(position.line - 1) + position.character;
	}

	private _positionAt(offset: number): vibecoda.Position {
	...
	private _validateRange(range: vibecoda.Range): vibecoda.Range {
	...
	private _validatePosition(position: vibecoda.Position): vibecoda.Position {
		if (this._strictInstanceofChecks) {
	...
	private _getWordRangeAtPosition(_position: vibecoda.Position, regexp?: RegExp): vibecoda.Range | undefined {
...
export class ExtHostDocumentLine implements vibecoda.TextLine {
