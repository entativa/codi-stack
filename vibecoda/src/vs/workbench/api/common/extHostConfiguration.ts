...
function isWorkspaceFolder(thing: any): thing is vibecoda.WorkspaceFolder {
	return thing
		&& thing.uri instanceof URI
...
function scopeToOverrides(scope: vibecoda.ConfigurationScope | undefined | null): IConfigurationOverrides | undefined {
	if (isUri(scope)) {
...
export class ExtHostConfigProvider {
	...
	private readonly _onDidChangeConfiguration = new Emitter<vibecoda.ConfigurationChangeEvent>();
	private readonly _proxy: MainThreadConfigurationShape;
	private readonly _extHostWorkspace: ExtHostWorkspace;
	...
	get onDidChangeConfiguration(): Event<vibecoda.ConfigurationChangeEvent> {
	...
	getConfiguration(section?: string, scope?: vibecoda.ConfigurationScope | null, extensionDescription?: IExtensionDescription): vibecoda.WorkspaceConfiguration {
		...
		const result: vibecoda.WorkspaceConfiguration = {
	...
	private _toConfigurationChangeEvent(change: IConfigurationChange, previous: { data: IConfigurationData; workspace: Workspace | undefined }): vibecoda.ConfigurationChangeEvent {
		...
		return Object.freeze({
			affectsConfiguration: (section: string, scope?: vibecoda.ConfigurationScope) => event.affectsConfiguration(section, scopeToOverrides(scope))
		});
	}
