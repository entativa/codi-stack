...
class RawContentChangeEvent {
	...
	constructor(
		...
		readonly deletedItems: vibecoda.NotebookCell[],
		readonly items: ExtHostCell[]
	) { }

	asApiEvent(): vibecoda.NotebookDocumentContentChange {
		return {
...
export class ExtHostCell {
	...
	private _outputs: vibecoda.NotebookCellOutput[];
	private _metadata: Readonly<notebookCommon.NotebookCellMetadata>;
	private _previousResult: Readonly<vibecoda.NotebookCellExecutionSummary | undefined>;
	...
	private _apiCell: vibecoda.NotebookCell | undefined;
	...
	get apiCell(): vibecoda.NotebookCell {
		if (!this._apiCell) {
			const that = this;
			...
			const apiCell: vibecoda.NotebookCell = {
...
export class ExtHostNotebookDocument {
	...
	private _notebook: vibecoda.NotebookDocument | undefined;
	...
	get apiNotebook(): vibecoda.NotebookDocument {
		if (!this._notebook) {
			const that = this;
			const apiObject: vibecoda.NotebookDocument = {
	...
	acceptModelChanged(event: extHostProtocol.NotebookCellsChangedEventDto, isDirty: boolean, newMetadata: notebookCommon.NotebookDocumentMetadata | undefined): vibecoda.NotebookDocumentChangeEvent {
		...
		const result = {
			...
			cellChanges: <vibecoda.NotebookDocumentCellChange[]>[],
			contentChanges: <vibecoda.NotebookDocumentContentChange[]>[],
		};

		type RelaxedCellChange = Partial<vibecoda.NotebookDocumentCellChange> & { cell: vibecoda.NotebookCell };
		...
		const map = new Map<vibecoda.NotebookCell, number>();
	...
	private _validateRange(range: vibecoda.NotebookRange): vibecoda.NotebookRange {
	...
	private _getCells(range: vibecoda.NotebookRange): ExtHostCell[] {
	...
	private _spliceNotebookCells(splices: notebookCommon.NotebookCellTextModelSplice<extHostProtocol.NotebookCellDto>[], initialization: boolean, bucket: vibecoda.NotebookDocumentContentChange[] | undefined): void {
		if (this._disposed) {
			return;
		}
	...
	private _moveCells(index: number, length: number, newIdx: number, bucket: vibecoda.NotebookDocumentContentChange[]): void {
	...
	getCellFromApiCell(apiCell: vibecoda.NotebookCell): ExtHostCell | undefined {
