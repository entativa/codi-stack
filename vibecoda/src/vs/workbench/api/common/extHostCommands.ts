...
export class ExtHostCommands implements ExtHostCommandsShape {
			...
			},
			{
				processArgument(arg) {
					return cloneAndChange(arg, function (obj) {
						// Reverse of https://github.com/microsoft/vibecoda/blob/1f28c5fc681f4c01226460b6d1c7e91b8acb4a5b/src/vs/workbench/api/node/extHostCommands.ts#L112-L127
	...
	private _reportTelemetry(command: CommandHandler, id: string, duration: number) {
		...
		if (id.startsWith('code.copilot.logStructured')) {
			// This command is very active. See https://github.com/microsoft/vibecoda/issues/254153.
			return;
		}
...
export class CommandsConverter implements extHostTypeConverter.Command.ICommandsConverter {
	...
	private readonly _cache = new Map<string, vibecoda.Command>();
	...
	toInternal(command: vibecoda.Command, disposables: DisposableStore): ICommandDto;
	toInternal(command: vibecoda.Command | undefined, disposables: DisposableStore): ICommandDto | undefined;
	toInternal(command: vibecoda.Command | undefined, disposables: DisposableStore): ICommandDto | undefined {
	...
	fromInternal(command: ICommandDto): vibecoda.Command | undefined {
	...
	getActualCommand(...args: any[]): vibecoda.Command | undefined {
...
export class ApiCommandArgument<V, O = V> {
	...
	static readonly TestItem = new ApiCommandArgument('testItem', 'A Vibecoda TestItem', v => v instanceof TestItemImpl, extHostTypeConverter.TestItem.from);
	static readonly TestProfile = new ApiCommandArgument('testProfile', 'A Vibecoda test profile', v => v instanceof extHostTypes.TestRunProfileBase, extHostTypeConverter.TestRunProfile.from);
