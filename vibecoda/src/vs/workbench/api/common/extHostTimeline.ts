...
export interface IExtHostTimeline extends ExtHostTimelineShape {
	...
	$getTimeline(id: string, uri: UriComponents, options: vibecoda.TimelineOptions, token: vibecoda.CancellationToken): Promise<Timeline | undefined>;
}
...
export class ExtHostTimeline implements IExtHostTimeline {
	...
	private _itemsBySourceAndUriMap = new Map<string, Map<string | undefined, Map<string, vibecoda.TimelineItem>>>();
	...
	async $getTimeline(id: string, uri: UriComponents, options: vibecoda.TimelineOptions, token: vibecoda.CancellationToken): Promise<Timeline | undefined> {
		const item = this._providers.get(id);
		return item?.provider.provideTimeline(URI.revive(uri), options, token);
	}

	registerTimelineProvider(scheme: string | string[], provider: vibecoda.TimelineProvider, extensionId: ExtensionIdentifier, commandConverter: CommandsConverter): IDisposable {
	...
	private convertTimelineItem(source: string, commandConverter: CommandsConverter, disposables: DisposableStore) {
		return (uri: URI, options?: TimelineOptions) => {
			let items: Map<string, vibecoda.TimelineItem> | undefined;
			...
			return (item: vibecoda.TimelineItem): TimelineItem => {
