...
export class TextEditorDecorationType {
	...
	readonly value: vibecoda.TextEditorDecorationType;

	constructor(proxy: MainThreadTextEditorsShape, extension: IExtensionDescription, options: vibecoda.DecorationRenderOptions) {
...
class TextEditorEdit {
	...
	private readonly _document: vibecoda.TextDocument;
	...
	constructor(document: vibecoda.TextDocument, options: { undoStopBefore: boolean; undoStopAfter: boolean }) {
...
export class ExtHostTextEditorOptions {
	...
	readonly value: vibecoda.TextEditorOptions;
	...
	public assign(newOptions: vibecoda.TextEditorOptions) {
...
export class ExtHostTextEditor {
	...
	private _diffInformation: vibecoda.TextEditorDiffInformation[] | undefined;

	readonly value: vibecoda.TextEditor;

	constructor(
		readonly id: string,
		private readonly _proxy: MainThreadTextEditorsShape,
		private readonly _logService: ILogService,
		document: Lazy<vibecoda.TextDocument>,
		selections: Selection[], options: IResolvedTextEditorConfiguration,
		visibleRanges: Range[], viewColumn: vibecoda.ViewColumn | undefined
	) {
		...
		this.value = Object.freeze({
			...
			// --- options
			get options(): vibecoda.TextEditorOptions {
				return that._options.value;
			},
			set options(value: vibecoda.TextEditorOptions) {
				if (!that._disposed) {
					that._options.assign(value);
				}
			},
			// --- view column
			get viewColumn(): vibecoda.ViewColumn | undefined {
			...
			setDecorations(decorationType: vibecoda.TextEditorDecorationType, ranges: Range[] | vibecoda.DecorationOptions[]): void {
			...
			revealRange(range: Range, revealType: vibecoda.TextEditorRevealType): void {
			...
			show(column: vibecoda.ViewColumn) {
	...
	_acceptViewColumn(value: vibecoda.ViewColumn) {
		ok(!this._disposed);
	...
	_acceptDiffInformation(diffInformation: vibecoda.TextEditorDiffInformation[] | undefined): void {
		ok(!this._disposed);
		this._diffInformation = diffInformation;
	}

	private async _trySetSelection(): Promise<vibecoda.TextEditor | null | undefined> {
