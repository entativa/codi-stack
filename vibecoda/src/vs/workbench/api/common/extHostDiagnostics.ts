...
export class DiagnosticCollection implements vibecoda.DiagnosticCollection {

	readonly #proxy: MainThreadDiagnosticsShape | undefined;
	readonly #onDidChangeDiagnostics: Emitter<readonly vibecoda.Uri[]>;
	readonly #data: ResourceMap<vibecoda.Diagnostic[]>;
	...
	constructor(
		...
		onDidChangeDiagnostics: Emitter<readonly vibecoda.Uri[]>
	) {
	...
	set(uri: vibecoda.Uri, diagnostics: ReadonlyArray<vibecoda.Diagnostic>): void;
	set(entries: ReadonlyArray<[vibecoda.Uri, ReadonlyArray<vibecoda.Diagnostic>]>): void;
	set(first: vibecoda.Uri | ReadonlyArray<[vibecoda.Uri, ReadonlyArray<vibecoda.Diagnostic>]>, diagnostics?: ReadonlyArray<vibecoda.Diagnostic>) {
		...
		if (URI.isUri(first)) {
			...
			// update many rows
			toSync = [];
			let lastUri: vibecoda.Uri | undefined;
	...
	forEach(callback: (uri: URI, diagnostics: ReadonlyArray<vibecoda.Diagnostic>, collection: DiagnosticCollection) => any, thisArg?: any): void {
		this._checkDisposed();
	...
	*[Symbol.iterator](): IterableIterator<[uri: vibecoda.Uri, diagnostics: readonly vibecoda.Diagnostic[]]> {
		this._checkDisposed();
	...
	get(uri: URI): ReadonlyArray<vibecoda.Diagnostic> {
		this._checkDisposed();
	...
	private static _compareIndexedTuplesByUri(a: [vibecoda.Uri, readonly vibecoda.Diagnostic[]], b: [vibecoda.Uri, readonly vibecoda.Diagnostic[]]): number {
...
export class ExtHostDiagnostics implements ExtHostDiagnosticsShape {
	...
	private readonly _onDidChangeDiagnostics = new DebounceEmitter<readonly vibecoda.Uri[]>({ merge: all => all.flat(), delay: 50 });

	static _mapper(last: readonly vibecoda.Uri[]): { uris: readonly vibecoda.Uri[] } {
		const map = new ResourceMap<vibecoda.Uri>();
	...
	readonly onDidChangeDiagnostics: Event<vibecoda.DiagnosticChangeEvent> = Event.map(this._onDidChangeDiagnostics.event, ExtHostDiagnostics._mapper);
	...
	createDiagnosticCollection(extensionId: ExtensionIdentifier, name?: string): vibecoda.DiagnosticCollection {
	...
	getDiagnostics(resource: vibecoda.Uri): ReadonlyArray<vibecoda.Diagnostic>;
	getDiagnostics(): ReadonlyArray<[vibecoda.Uri, ReadonlyArray<vibecoda.Diagnostic>]>;
	getDiagnostics(resource?: vibecoda.Uri): ReadonlyArray<vibecoda.Diagnostic> | ReadonlyArray<[vibecoda.Uri, ReadonlyArray<vibecoda.Diagnostic>]>;
	getDiagnostics(resource?: vibecoda.Uri): ReadonlyArray<vibecoda.Diagnostic> | ReadonlyArray<[vibecoda.Uri, ReadonlyArray<vibecoda.Diagnostic>]> {
		if (resource) {
			return this._getDiagnostics(resource);
		} else {
			const index = new Map<string, number>();
			const res: [vibecoda.Uri, vibecoda.Diagnostic[]][] = [];
	...
	private _getDiagnostics(resource: vibecoda.Uri): ReadonlyArray<vibecoda.Diagnostic> {
		let res: vibecoda.Diagnostic[] = [];
		for (const collection of this._collections.values()) {
	...
	private _mirrorCollection: vibecoda.DiagnosticCollection | undefined;
