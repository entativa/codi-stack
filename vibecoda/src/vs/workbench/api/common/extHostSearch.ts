...
export interface IExtHostSearch extends ExtHostSearchShape {
	registerTextSearchProviderOld(scheme: string, provider: vibecoda.TextSearchProvider): IDisposable;
	registerFileSearchProviderOld(scheme: string, provider: vibecoda.FileSearchProvider): IDisposable;
	registerTextSearchProvider(scheme: string, provider: vibecoda.TextSearchProvider2): IDisposable;
	registerAITextSearchProvider(scheme: string, provider: vibecoda.AITextSearchProvider): IDisposable;
	registerFileSearchProvider(scheme: string, provider: vibecoda.FileSearchProvider2): IDisposable;
...
export class ExtHostSearch implements IExtHostSearch {
	...
	private readonly _textSearchProvider: Map<number, vibecoda.TextSearchProvider2>;
	private readonly _textSearchUsedSchemes: Set<string>;

	private readonly _aiTextSearchProvider: Map<number, vibecoda.AITextSearchProvider>;
	private readonly _aiTextSearchUsedSchemes: Set<string>;

	private readonly _fileSearchProvider: Map<number, vibecoda.FileSearchProvider2>;
	...
	constructor(
		...
		this._handlePool = 0;
		this._textSearchProvider = new Map<number, vibecoda.TextSearchProvider2>();
		this._textSearchUsedSchemes = new Set<string>();
		this._aiTextSearchProvider = new Map<number, vibecoda.AITextSearchProvider>();
		this._aiTextSearchUsedSchemes = new Set<string>();
		this._fileSearchProvider = new Map<number, vibecoda.FileSearchProvider2>();
	...
	$provideFileSearchResults(handle: number, session: number, rawQuery: IRawFileQuery, token: vibecoda.CancellationToken): Promise<ISearchCompleteStats> {
	...
	$provideTextSearchResults(handle: number, session: number, rawQuery: IRawTextQuery, token: vibecoda.CancellationToken): Promise<ISearchCompleteStats> {
	...
	$provideAITextSearchResults(handle: number, session: number, rawQuery: IRawAITextQuery, token: vibecoda.CancellationToken): Promise<ISearchCompleteStats> {
	...
	protected createTextSearchManager(query: ITextQuery, provider: vibecoda.TextSearchProvider2): TextSearchManager {
	...
	protected createAITextSearchManager(query: IAITextQuery, provider: vibecoda.AITextSearchProvider): TextSearchManager {
		return new TextSearchManager({ query, provider }, {
			readdir: resource => Promise.resolve([]),
			toCanonicalName: encoding => encoding
