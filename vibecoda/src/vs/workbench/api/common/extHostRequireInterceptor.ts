...
export abstract class RequireInterceptor {
	...
	async install(): Promise<void> {
		...
		this.register(new VibecodaNodeModuleFactory(this._apiFactory, extensionPaths, this._extensionRegistry, configProvider, this._logService));
...
class NodeModuleAliasingModuleFactory implements IAlternativeModuleProvider {
	...
	private static readonly aliased: ReadonlyMap<string, string> = new Map([
		['vibecoda-ripgrep', '@vibecoda/ripgrep'],
		['vibecoda-windows-registry', '@vibecoda/windows-registry'],
	]);
	...
	constructor(@IExtHostInitDataService initData: IExtHostInitDataService) {
		if (initData.environment.appRoot && NodeModuleAliasingModuleFactory.aliased.size) {
			...
			// to ['${appRoot}/node_modules.asar.unpacked/',' @vibecoda/foo', '/bin'].
...
//#region --- vibecoda-module

class VibecodaNodeModuleFactory implements INodeModuleFactory {
	public readonly nodeModuleName = 'vibecoda';

	private readonly _extApiImpl = new ExtensionIdentifierMap<typeof vibecoda>();
	private _defaultApiImpl?: typeof vibecoda;
	...
	public load(_request: string, parent: URI): any {
		...
		if (!this._defaultApiImpl) {
			...
			this._logService.warn(`Could not identify extension for 'vibecoda' require call from ${parent}. These are the extension path mappings: \n${extensionPathsPretty}`);
