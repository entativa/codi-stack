...
export interface IExtHostWorkspaceProvider {
	getWorkspaceFolder2(uri: vibecoda.Uri, resolveParent?: boolean): Promise<vibecoda.WorkspaceFolder | undefined>;
	resolveWorkspaceFolder(uri: vibecoda.Uri): Promise<vibecoda.WorkspaceFolder | undefined>;
	getWorkspaceFolders2(): Promise<vibecoda.WorkspaceFolder[] | undefined>;
...
function compareWorkspaceFolderByUri(a: vibecoda.WorkspaceFolder, b: vibecoda.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo): number {
	return isFolderEqual(a.uri, b.uri, extHostFileSystemInfo) ? 0 : compare(a.uri.toString(), b.uri.toString());
}

function compareWorkspaceFolderByUriAndNameAndIndex(a: vibecoda.WorkspaceFolder, b: vibecoda.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo): number {
	if (a.index !== b.index) {
...
function delta(oldFolders: vibecoda.WorkspaceFolder[], newFolders: vibecoda.WorkspaceFolder[], compare: (a: vibecoda.WorkspaceFolder, b: vibecoda.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo) => number, extHostFileSystemInfo: IExtHostFileSystemInfo): { removed: vibecoda.WorkspaceFolder[]; added: vibecoda.WorkspaceFolder[] } {
...
interface MutableWorkspaceFolder extends vibecoda.WorkspaceFolder {
	name: string;
	index: number;
}
...
class ExtHostWorkspaceImpl extends Workspace {
	...
	private readonly _workspaceFolders: vibecoda.WorkspaceFolder[] = [];
	private readonly _structure: TernarySearchTree<URI, vibecoda.WorkspaceFolder>;

	constructor(id: string, private _name: string, folders: vibecoda.WorkspaceFolder[], transient: boolean, configuration: URI | null, private _isUntitled: boolean, ignorePathCasing: (key: URI) => boolean) {
		super(id, folders.map(f => new WorkspaceFolder(f)), transient, configuration, ignorePathCasing);
		this._structure = TernarySearchTree.forUris<vibecoda.WorkspaceFolder>(ignorePathCasing, () => true);
	...
	get workspaceFolders(): vibecoda.WorkspaceFolder[] {
		return this._workspaceFolders.slice(0);
	}

	getWorkspaceFolder(uri: URI, resolveParent?: boolean): vibecoda.WorkspaceFolder | undefined {
	...
	resolveWorkspaceFolder(uri: URI): vibecoda.WorkspaceFolder | undefined {
...
export class ExtHostWorkspace implements ExtHostWorkspaceShape, IExtHostWorkspaceProvider {
	...
	private readonly _onDidChangeWorkspace = new Emitter<vibecoda.WorkspaceFoldersChangeEvent>();
	readonly onDidChangeWorkspace: Event<vibecoda.WorkspaceFoldersChangeEvent> = this._onDidChangeWorkspace.event;
	...
	private readonly _editSessionIdentityProviders = new Map<string, vibecoda.EditSessionIdentityProvider>();
	...
	get workspaceFile(): vibecoda.Uri | undefined {
	...
	getWorkspaceFolders(): vibecoda.WorkspaceFolder[] | undefined {
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async getWorkspaceFolders2(): Promise<vibecoda.WorkspaceFolder[] | undefined> {
	...
	updateWorkspaceFolders(extension: IExtensionDescription, index: number, deleteCount: number, ...workspaceFoldersToAdd: { uri: vibecoda.Uri; name?: string }[]): boolean {
		const validatedDistinctWorkspaceFoldersToAdd: { uri: vibecoda.Uri; name?: string }[] = [];
	...
	getWorkspaceFolder(uri: vibecoda.Uri, resolveParent?: boolean): vibecoda.WorkspaceFolder | undefined {
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async getWorkspaceFolder2(uri: vibecoda.Uri, resolveParent?: boolean): Promise<vibecoda.WorkspaceFolder | undefined> {
		await this._barrier.wait();
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async resolveWorkspaceFolder(uri: vibecoda.Uri): Promise<vibecoda.WorkspaceFolder | undefined> {
		await this._barrier.wait();
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	getRelativePath(pathOrUri: string | vibecoda.Uri, includeWorkspace?: boolean): string {
	...
	private trySetWorkspaceFolders(folders: vibecoda.WorkspaceFolder[]): void {
	 ...
	 */
	findFiles(include: vibecoda.GlobPattern | undefined, exclude: vibecoda.GlobPattern | null | undefined, maxResults: number | undefined, extensionId: ExtensionIdentifier, token: vibecoda.CancellationToken = CancellationToken.None): Promise<vibecoda.Uri[]> {
	...
	findFiles2(filePatterns: readonly vibecoda.GlobPattern[],
		options: vibecoda.FindFiles2Options = {},
		extensionId: ExtensionIdentifier,
		token: vibecoda.CancellationToken = CancellationToken.None): Promise<vibecoda.Uri[]> {
	...
	private async _findFilesImpl(
		...
		query: { readonly type: 'include'; readonly value: vibecoda.GlobPattern | undefined } | { readonly type: 'filePatterns'; readonly value: readonly vibecoda.GlobPattern[] },
		options: vibecoda.FindFiles2Options,
		token: vibecoda.CancellationToken
	): Promise<vibecoda.Uri[]> {
	...
	private async _findFilesBase(
		...
		const uriMap = new Map<string, vibecoda.Uri>();
	...
	findTextInFiles2(query: vibecoda.TextSearchQuery2, options: vibecoda.FindTextInFilesOptions2 | undefined, extensionId: ExtensionIdentifier, token: vibecoda.CancellationToken = CancellationToken.None): vibecoda.FindTextInFilesResponse {
		...
		const getOptions = (include: vibecoda.GlobPattern | undefined): QueryOptions<ITextQueryBuilderOptions> => {
			if (!options) {
				return {
					folder: undefined,
					options: {}
		...
		const asyncIterable = new AsyncIterableObject<vibecoda.TextSearchResult2>(async emitter => {
	...
	async findTextInFilesBase(query: vibecoda.TextSearchQuery, queryOptions: QueryOptions<ITextQueryBuilderOptions>[] | undefined, callback: (result: ITextSearchResult<URI>, uri: URI) => void, token: vibecoda.CancellationToken = CancellationToken.None): Promise<vibecoda.TextSearchComplete> {
	...
	async findTextInFiles(query: vibecoda.TextSearchQuery, options: vibecoda.FindTextInFilesOptions & { useSearchExclude?: boolean }, callback: (result: vibecoda.TextSearchResult) => void, extensionId: ExtensionIdentifier, token: vibecoda.CancellationToken = CancellationToken.None): Promise<vibecoda.TextSearchComplete> {
		this._logService.trace(`extHostWorkspace#findTextInFiles: textSearch, extension: ${extensionId.value}, entryPoint: findTextInFiles`);

		const previewOptions: vibecoda.TextSearchPreviewOptions = typeof options.previewOptions === 'undefined' ?
			{
				matchLines: 100,
				charsPerLine: 10000
			} :
		...
		const progress = (result: ITextSearchResult<URI>, uri: URI) => {
			if (resultIsMatch(result)) {
				...
				} satisfies vibecoda.TextSearchMatch);
			} else {
				callback({
					uri,
					text: result.text,
					lineNumber: result.lineNumber
				} satisfies vibecoda.TextSearchContext);
			}
		};
	...
	requestWorkspaceTrust(options?: vibecoda.WorkspaceTrustRequestOptions): Promise<boolean | undefined> {
	...
	private readonly _onWillCreateEditSessionIdentityEvent = new AsyncEmitter<vibecoda.EditSessionIdentityWillCreateEvent>();

	getOnWillCreateEditSessionIdentityEvent(extension: IExtensionDescription): Event<vibecoda.EditSessionIdentityWillCreateEvent> {
		return (listener, thisArg, disposables) => {
			const wrappedListener: IExtensionListener<vibecoda.EditSessionIdentityWillCreateEvent> = function wrapped(e) { listener.call(thisArg, e); };
	...
	async $onWillCreateEditSessionIdentity(workspaceFolder: UriComponents, token: CancellationToken, timeout: number): Promise<void> {
		...
		await this._onWillCreateEditSessionIdentityEvent.fireAsync({ workspaceFolder: folder }, token, async (thenable: Promise<unknown>, listener) => {
			...
			if (Date.now() - now > timeout) {
				this._logService.warn('SLOW edit session create-participant', (<IExtensionListener<vibecoda.EditSessionIdentityWillCreateEvent>>listener).extension.identifier);
			}
		});
	...
	private readonly _canonicalUriProviders = new Map<string, vibecoda.CanonicalUriProvider>();
	...
	async provideCanonicalUri(uri: URI, options: vibecoda.CanonicalUriRequestOptions, cancellationToken: CancellationToken): Promise<URI | undefined> {
	...
	async decode(content: Uint8Array, args?: { uri?: vibecoda.Uri; encoding?: string }): Promise<string> {
	...
	async encode(content: string, args?: { uri?: vibecoda.Uri; encoding?: string }): Promise<Uint8Array> {
	...
	private toEncodeDecodeParameters(opts?: { uri?: vibecoda.Uri; encoding?: string }): [UriComponents | undefined, { encoding: string } | undefined] {
...
function globsToISearchPatternBuilder(excludes: vibecoda.GlobPattern[] | undefined): ISearchPatternBuilder<URI>[] {
	return (
