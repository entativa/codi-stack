...
class ExtHostOutputChannel extends AbstractMessageLogger implements vibecoda.LogOutputChannel {
	...
	show(columnOrPreserveFocus?: vibecoda.ViewColumn | boolean, preserveFocus?: boolean): void {
		this.logger.flush();
...
export class ExtHostOutputService implements ExtHostOutputServiceShape {
	...
	createOutputChannel(name: string, options: string | { log: true } | undefined, extension: IExtensionDescription): vibecoda.OutputChannel | vibecoda.LogOutputChannel {
		name = name.trim();
		if (!name) {
	...
	private createExtHostOutputChannel(name: string, channelPromise: Promise<ExtHostOutputChannel>, channelDisposables: DisposableStore): vibecoda.OutputChannel {
		const validate = () => {
	...
	private createExtHostLogOutputChannel(name: string, logLevel: LogLevel, channelPromise: Promise<ExtHostOutputChannel>, channelDisposables: DisposableStore): vibecoda.LogOutputChannel {
		const validate = () => {
			if (channelDisposables.isDisposed) {
				throw new Error('Channel has been closed');
			}
