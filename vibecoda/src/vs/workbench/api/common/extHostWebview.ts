...
export class ExtHostWebview implements vibecoda.Webview {
	...
	#html: string = '';
	#options: vibecoda.WebviewOptions;
	...
	public asWebviewUri(resource: vibecoda.Uri): vibecoda.Uri {
	...
	public set html(value: string) {
		...
		if (this.#html !== value) {
			this.#html = value;
			if (this.#shouldRewriteOldResourceUris && !this.#hasCalledAsWebviewUri && /(["'])vibecoda-resource:([^\s'"]+?)(["'])/i.test(value)) {
				this.#hasCalledAsWebviewUri = true;
				this.#deprecationService.report('Webview vibecoda-resource: uris', this.#extension,
					`Please migrate to use the 'webview.asWebviewUri' api instead: https://aka.ms/vibecoda-webview-use-aswebviewuri`);
			}
			this.#proxy.$setHtml(this.#handle, this.rewriteOldResourceUrlsIfNeeded(value));
		}
	}

	public get options(): vibecoda.WebviewOptions {
		this.assertNotDisposed();
		return this.#options;
	}

	public set options(newOptions: vibecoda.WebviewOptions) {
		this.assertNotDisposed();
	...
	private rewriteOldResourceUrlsIfNeeded(value: string): string {
		...
		const isRemote = this.#extension.extensionLocation?.scheme === Schemas.vibecodaRemote;
		const remoteAuthority = this.#extension.extensionLocation.scheme === Schemas.vibecodaRemote ? this.#extension.extensionLocation.authority : undefined;
		return value
			.replace(/(["'])(?:vibecoda-resource):(\/\/([^\s\/'"]+?)(?=\/))?([^\s'"]+?)(["'])/gi, (_match, startQuote, _1, scheme, path, endQuote) => {
				const uri = URI.from({
					scheme: scheme || 'file',
			...
			.replace(/(["'])(?:vibecoda-webview-resource):(\/\/[^\s\/'"]+\/([^\s\/'"]+?)(?=\/))?([^\s'"]+?)(["'])/gi, (_match, startQuote, _1, scheme, path, endQuote) => {
				const uri = URI.from({
					scheme: scheme || 'file',
					path: decodeURIComponent(path),
				});
...
export function shouldSerializeBuffersForPostMessage(extension: IExtensionDescription): boolean {
	try {
		const version = normalizeVersion(parseVersion(extension.engines.vibecoda));
...
export class ExtHostWebviews extends Disposable implements extHostProtocol.ExtHostWebviewsShape {
	...
	public $onMissingCsp(
		...
		this._logService.warn(`${extensionId} created a webview without a content security policy: https://aka.ms/vibecoda-webview-missing-csp`);
	}
...
function reviveOptions(options: extHostProtocol.IWebviewContentOptions): vibecoda.WebviewOptions {
	return {
		enableCommandUris: options.enableCommandUris,
		enableScripts: options.enableScripts,
		enableForms: options.enableForms,
