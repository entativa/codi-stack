...
class Tool {
	...
	private _apiObject = new Lazy<vibecoda.LanguageModelToolInformation>(() => {
		const that = this;
	...
	private _apiObjectWithChatParticipantAdditions = new Lazy<vibecoda.LanguageModelToolInformation>(() => {
		const that = this;
	...
	get apiObject(): vibecoda.LanguageModelToolInformation {
...
export class ExtHostLanguageModelTools implements ExtHostLanguageModelToolsShape {
	...
	private readonly _registeredTools = new Map<string, { extension: IExtensionDescription; tool: vibecoda.LanguageModelTool<Object> }>();
	private readonly _proxy: MainThreadLanguageModelToolsShape;
	private readonly _tokenCountFuncs = new Map</* call ID */string, (text: string, token?: vibecoda.CancellationToken) => Thenable<number>>();

	/** A map of all known tools, from other EHs or registered in vibecoda core */
	...
	async invokeTool(extension: IExtensionDescription, toolId: string, options: vibecoda.LanguageModelToolInvocationOptions<any>, token?: CancellationToken): Promise<vibecoda.LanguageModelToolResult> {
	...
	getTools(extension: IExtensionDescription): vibecoda.LanguageModelToolInformation[] {
	...
	async $invokeTool(dto: IToolInvocation, token: CancellationToken): Promise<Dto<IToolResult> | SerializableObjectWithBuffers<Dto<IToolResult>>> {
		...
		const options: vibecoda.LanguageModelToolInvocationOptions<Object> = {
			input: dto.parameters,
			toolInvocationToken: dto.context as vibecoda.ChatParticipantToolToken | undefined,
		};
		...
		let progress: vibecoda.Progress<{ message?: string | vibecoda.MarkdownString; increment?: number }> | undefined;
	...
	private async getModel(modelId: string, extension: IExtensionDescription): Promise<vibecoda.LanguageModelChat> {
	...
	async $prepareToolInvocation(toolId: string, context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		const options: vibecoda.LanguageModelToolInvocationPrepareOptions<any> = {
	...
	registerTool(extension: IExtensionDescription, id: string, tool: vibecoda.LanguageModelTool<any>): IDisposable {
