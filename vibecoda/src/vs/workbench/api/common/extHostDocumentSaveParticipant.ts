...
export class ExtHostDocumentSaveParticipant implements ExtHostDocumentSaveParticipantShape {
	...
	getOnWillSaveTextDocumentEvent(extension: IExtensionDescription): Event<vibecoda.TextDocumentWillSaveEvent> {
	...
	private _deliverEventAsyncAndBlameBadListeners([listener, thisArg, extension]: Listener, stubEvent: vibecoda.TextDocumentWillSaveEvent): Promise<any> {
	...
	private _deliverEventAsync(extension: IExtensionDescription, listener: Function, thisArg: any, stubEvent: vibecoda.TextDocumentWillSaveEvent): Promise<any> {

		const promises: Promise<vibecoda.TextEdit[]>[] = [];

		const t1 = Date.now();
		const { document, reason } = stubEvent;
		const { version } = document;

		const event = Object.freeze<vibecoda.TextDocumentWillSaveEvent>({
			document,
			reason,
			waitUntil(p: Promise<any | vibecoda.TextEdit[]>) {
		...
		return new Promise<vibecoda.TextEdit[][]>((resolve, reject) => {
			...
			for (const value of values) {
				if (Array.isArray(value) && (<vibecoda.TextEdit[]>value).every(e => e instanceof TextEdit)) {
