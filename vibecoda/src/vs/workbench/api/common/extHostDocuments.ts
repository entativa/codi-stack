...
export class ExtHostDocuments implements ExtHostDocumentsShape {
	...
	private readonly _onDidAddDocument = new Emitter<vibecoda.TextDocument>();
	private readonly _onDidRemoveDocument = new Emitter<vibecoda.TextDocument>();
	private readonly _onDidChangeDocument = new Emitter<Omit<vibecoda.TextDocumentChangeEvent, 'detailedReason'>>();
	private readonly _onDidChangeDocumentWithReason = new Emitter<vibecoda.TextDocumentChangeEvent>();
	private readonly _onDidSaveDocument = new Emitter<vibecoda.TextDocument>();

	readonly onDidAddDocument: Event<vibecoda.TextDocument> = this._onDidAddDocument.event;
	readonly onDidRemoveDocument: Event<vibecoda.TextDocument> = this._onDidRemoveDocument.event;
	readonly onDidChangeDocument: Event<vibecoda.TextDocumentChangeEvent> = this._onDidChangeDocument.event as Event<vibecoda.TextDocumentChangeEvent>;
	readonly onDidChangeDocumentWithReason: Event<vibecoda.TextDocumentChangeEvent> = this._onDidChangeDocumentWithReason.event;
	readonly onDidSaveDocument: Event<vibecoda.TextDocument> = this._onDidSaveDocument.event;
	...
	public getDocumentData(resource: vibecoda.Uri): ExtHostDocumentData | undefined {
		if (!resource) {
			return undefined;
		}
	...
	public getDocument(resource: vibecoda.Uri): vibecoda.TextDocument {
	...
	public $acceptModelChanged(uriComponents: UriComponents, events: ISerializedModelContentChangedEvent, isDirty: boolean): void {
		...
		let reason: vibecoda.TextDocumentChangeReason | undefined = undefined;
		if (events.isUndoing) {
		...
		this._onDidChangeDocument.fire(deepFreeze<Omit<vibecoda.TextDocumentChangeEvent, 'detailedReason'>>({
			document: data.document,
		...
		this._onDidChangeDocumentWithReason.fire(deepFreeze<vibecoda.TextDocumentChangeEvent>({
			document: data.document,
			contentChanges: events.changes.map((change) => {
				return {
					range: TypeConverters.Range.to(change.range),
