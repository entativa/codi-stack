...
export class ExtHostCodeMapper implements extHostProtocol.ExtHostCodeMapperShape {
	...
	private readonly providers = new Map<number, vibecoda.MappedEditsProvider2>();
	...
	async $mapCode(handle: number, internalRequest: extHostProtocol.ICodeMapperRequestDto, token: CancellationToken): Promise<ICodeMapperResult | null> {
		...
		const stream: vibecoda.MappedEditsResponseStream = {
			textEdit: (target: vibecoda.Uri, edits: vibecoda.TextEdit | vibecoda.TextEdit[]) => {
				edits = asArray(edits);
			...
			notebookEdit: (target: vibecoda.Uri, edits: vibecoda.NotebookEdit | vibecoda.NotebookEdit[]) => {
				edits = asArray(edits);
				this._proxy.$handleProgress(internalRequest.requestId, {
					uri: target,
		...
		const request: vibecoda.MappedEditsRequest = {
	...
	registerMappedEditsProvider(extension: IExtensionDescription, provider: vibecoda.MappedEditsProvider2): vibecoda.Disposable {
