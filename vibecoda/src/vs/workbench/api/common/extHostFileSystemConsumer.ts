...
export class ExtHostConsumerFileSystem {
	...
	readonly value: vibecoda.FileSystem;

	private readonly _proxy: MainThreadFileSystemShape;
	private readonly _fileSystemProvider = new Map<string, { impl: vibecoda.FileSystemProvider; extUri: IExtUri; isReadonly: boolean }>();
	...
	constructor(
		...
		this.value = Object.freeze({
			...
			async readDirectory(uri: vibecoda.Uri): Promise<[string, vibecoda.FileType][]> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
					if (provider) {
						// use shortcut
			...
			async createDirectory(uri: vibecoda.Uri): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async readFile(uri: vibecoda.Uri): Promise<Uint8Array> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
					if (provider) {
						// use shortcut
			...
			async writeFile(uri: vibecoda.Uri, content: Uint8Array): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
					if (provider && !provider.isReadonly) {
						// use shortcut
			...
			async delete(uri: vibecoda.Uri, options?: { recursive?: boolean; useTrash?: boolean }): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async rename(oldUri: vibecoda.Uri, newUri: vibecoda.Uri, options?: { overwrite?: boolean }): Promise<void> {
				try {
			...
			async copy(source: vibecoda.Uri, destination: vibecoda.Uri, options?: { overwrite?: boolean }): Promise<void> {
				try {
					// no shortcut: potentially involves different schemes, does mkdirp
	...
	private async mkdirp(provider: vibecoda.FileSystemProvider, providerExtUri: IExtUri, directory: vibecoda.Uri): Promise<void> {
		...
		for (let i = directoriesToCreate.length - 1; i >= 0; i--) {
			...
			try {
				...
				if (files.toFileSystemProviderErrorCode(error) !== files.FileSystemProviderErrorCode.FileExists) {
					...
					// (see also https://github.com/microsoft/vibecoda/issues/89834)
					throw error;
				}
			}
		}
	...
	addFileSystemProvider(scheme: string, provider: vibecoda.FileSystemProvider, options?: { isCaseSensitive?: boolean; isReadonly?: boolean | IMarkdownString }): IDisposable {
