...
interface IKernelData {
	...
	controller: vibecoda.NotebookController;
	onDidChangeSelection: Emitter<{ selected: boolean; notebook: vibecoda.NotebookDocument }>;
	onDidReceiveMessage: Emitter<{ editor: vibecoda.NotebookEditor; message: unknown }>;
	associatedNotebooks: ResourceMap<boolean>;
}

type ExtHostSelectKernelArgs = ControllerInfo | { notebookEditor: vibecoda.NotebookEditor } | ControllerInfo & { notebookEditor: vibecoda.NotebookEditor } | undefined;
...
export class ExtHostNotebookKernels implements ExtHostNotebookKernelsShape {
	...
	private _kernelDetectionTask = new Map<number, vibecoda.NotebookControllerDetectionTask>();
	private _kernelDetectionTaskHandlePool: number = 0;

	private _kernelSourceActionProviders = new Map<number, vibecoda.NotebookKernelSourceActionProvider>();
	...
	constructor(
		...
		const requestKernelVariablesApiCommand = new ApiCommand(
			'vibecoda.executeNotebookVariableProvider',
			'_executeNotebookVariableProvider',
			'Execute notebook variable provider',
			[ApiCommandArgument.Uri],
			new ApiCommandResult<VariablesResult[], vibecoda.VariablesResult[]>('A promise that resolves to an array of variables', (value, apiArgs) => {
	...
	createNotebookController(extension: IExtensionDescription, id: string, viewType: string, label: string, handler?: (cells: vibecoda.NotebookCell[], notebook: vibecoda.NotebookDocument, controller: vibecoda.NotebookController) => void | Thenable<void>, preloads?: vibecoda.NotebookRendererScript[]): vibecoda.NotebookController {
		...
		const onDidChangeSelection = new Emitter<{ selected: boolean; notebook: vibecoda.NotebookDocument }>();
		const onDidReceiveMessage = new Emitter<{ editor: vibecoda.NotebookEditor; message: unknown }>();
		...
		let _interruptHandler: ((this: vibecoda.NotebookController, notebook: vibecoda.NotebookDocument) => void | Thenable<void>) | undefined;
		let _variableProvider: vibecoda.NotebookVariableProvider | undefined;
		...
		const controller: vibecoda.NotebookController = {
			get id() { return id; },
	...
	getIdByController(controller: vibecoda.NotebookController) {
	...
	createNotebookControllerDetectionTask(extension: IExtensionDescription, viewType: string): vibecoda.NotebookControllerDetectionTask {
		...
		const detectionTask: vibecoda.NotebookControllerDetectionTask = {
			dispose: () => {
	...
	registerKernelSourceActionProvider(extension: IExtensionDescription, viewType: string, provider: vibecoda.NotebookKernelSourceActionProvider) {
		...
		let subscription: vibecoda.Disposable | undefined;
	...
	async $executeCells(handle: number, uri: UriComponents, handles: number[]): Promise<void> {
		...
		const cells: vibecoda.NotebookCell[] = [];
	...
	private id = 0;
	private variableStore: Record<string, vibecoda.Variable> = {};
	...
	async $provideVariables(handle: number, requestId: string, notebookUri: UriComponents, parentId: number | undefined, kind: 'named' | 'indexed', start: number, token: CancellationToken): Promise<void> {
		...
		let parent: vibecoda.Variable | undefined = undefined;
	...
	_createNotebookCellExecution(cell: vibecoda.NotebookCell, controllerId: string): vibecoda.NotebookCellExecution {
		if (cell.index < 0) {
	...
	_createNotebookExecution(nb: vibecoda.NotebookDocument, controllerId: string): vibecoda.NotebookExecution {
...
class NotebookCellExecutionTask extends Disposable {
	...
	private cellIndexToHandle(cellOrCellIndex: vibecoda.NotebookCell | undefined): number {
	...
	private validateAndConvertOutputs(items: vibecoda.NotebookCellOutput[]): NotebookOutputDto[] {
	...
	private async updateOutputs(outputs: vibecoda.NotebookCellOutput | vibecoda.NotebookCellOutput[], cell: vibecoda.NotebookCell | undefined, append: boolean): Promise<void> {
	...
	private async updateOutputItems(items: vibecoda.NotebookCellOutputItem | vibecoda.NotebookCellOutputItem[], output: vibecoda.NotebookCellOutput, append: boolean): Promise<void> {
	...
	asApiObject(): vibecoda.NotebookCellExecution {
		const that = this;
		const result: vibecoda.NotebookCellExecution = {
			...
			end(success: boolean | undefined, endTime?: number, executionError?: vibecoda.CellExecutionError): void {
			...
			clearOutput(cell?: vibecoda.NotebookCell): Thenable<void> {
				that.verifyStateForOutput();
				return that.updateOutputs([], cell, false);
			},

			appendOutput(outputs: vibecoda.NotebookCellOutput | vibecoda.NotebookCellOutput[], cell?: vibecoda.NotebookCell): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputs(outputs, cell, true);
			},

			replaceOutput(outputs: vibecoda.NotebookCellOutput | vibecoda.NotebookCellOutput[], cell?: vibecoda.NotebookCell): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputs(outputs, cell, false);
			},

			appendOutputItems(items: vibecoda.NotebookCellOutputItem | vibecoda.NotebookCellOutputItem[], output: vibecoda.NotebookCellOutput): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputItems(items, output, true);
			},

			replaceOutputItems(items: vibecoda.NotebookCellOutputItem | vibecoda.NotebookCellOutputItem[], output: vibecoda.NotebookCellOutput): Promise<void> {
				that.verifyStateForOutput();
...
function createSerializeableError(executionError: vibecoda.CellExecutionError | undefined) {
	const convertRange = (range: vibecoda.Range | undefined) => (range ? {
	...
	const convertStackFrame = (frame: vibecoda.CellErrorStackFrame) => ({
		uri: frame.uri,
...
class NotebookExecutionTask extends Disposable {
	...
	asApiObject(): vibecoda.NotebookExecution {
		const result: vibecoda.NotebookExecution = {
			start: () => {
