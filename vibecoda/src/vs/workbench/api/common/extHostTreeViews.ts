...
export class ExtHostTreeViews extends Disposable implements ExtHostTreeViewsShape {
	...
	private _treeDragAndDropService: ITreeViewsDnDService<vibecoda.DataTransfer> = new TreeViewsDnDService<vibecoda.DataTransfer>();
	...
	registerTreeDataProvider<T>(id: string, treeDataProvider: vibecoda.TreeDataProvider<T>, extension: IExtensionDescription): vibecoda.Disposable {
		const treeView = this.createTreeView(id, { treeDataProvider }, extension);
		return { dispose: () => treeView.dispose() };
	}

	createTreeView<T>(viewId: string, options: vibecoda.TreeViewOptions<T>, extension: IExtensionDescription): vibecoda.TreeView<T> {
		...
		const view = {
			...
			set message(message: string | vibecoda.MarkdownString) {
			...
			set badge(badge: vibecoda.ViewBadge | undefined) {
		...
		this._register(view);
		return view as vibecoda.TreeView<T>;
	}
	...
	private async _addAdditionalTransferItems(treeDataTransfer: vibecoda.DataTransfer, treeView: ExtHostTreeView<any>,
		sourceTreeItemHandles: string[], token: CancellationToken, operationUuid?: string): Promise<vibecoda.DataTransfer | undefined> {
	...
	$resolve(treeViewId: string, treeItemHandle: string, token: vibecoda.CancellationToken): Promise<ITreeItem | undefined> {
		const treeView = this._treeViews.get(treeViewId);
		if (!treeView) {
			throw new NoTreeViewError(treeViewId);
		}
	...
	private _createExtHostTreeView<T>(id: string, options: vibecoda.TreeViewOptions<T>, extension: IExtensionDescription): ExtHostTreeView<T> {
...
interface TreeNode extends IDisposable {
	item: ITreeItem;
	extensionItem: vibecoda.TreeItem;
...
class ExtHostTreeView<T> extends Disposable {
	...
	private readonly _dataProvider: vibecoda.TreeDataProvider<T>;
	private readonly _dndController: vibecoda.TreeDragAndDropController<T> | undefined;
	...
	private _onDidExpandElement: Emitter<vibecoda.TreeViewExpansionEvent<T>> = this._register(new Emitter<vibecoda.TreeViewExpansionEvent<T>>());
	readonly onDidExpandElement: Event<vibecoda.TreeViewExpansionEvent<T>> = this._onDidExpandElement.event;

	private _onDidCollapseElement: Emitter<vibecoda.TreeViewExpansionEvent<T>> = this._register(new Emitter<vibecoda.TreeViewExpansionEvent<T>>());
	readonly onDidCollapseElement: Event<vibecoda.TreeViewExpansionEvent<T>> = this._onDidCollapseElement.event;

	private _onDidChangeSelection: Emitter<vibecoda.TreeViewSelectionChangeEvent<T>> = this._register(new Emitter<vibecoda.TreeViewSelectionChangeEvent<T>>());
	readonly onDidChangeSelection: Event<vibecoda.TreeViewSelectionChangeEvent<T>> = this._onDidChangeSelection.event;

	private _onDidChangeActiveItem: Emitter<vibecoda.TreeViewActiveItemChangeEvent<T>> = this._register(new Emitter<vibecoda.TreeViewActiveItemChangeEvent<T>>());
	readonly onDidChangeActiveItem: Event<vibecoda.TreeViewActiveItemChangeEvent<T>> = this._onDidChangeActiveItem.event;

	private _onDidChangeVisibility: Emitter<vibecoda.TreeViewVisibilityChangeEvent> = this._register(new Emitter<vibecoda.TreeViewVisibilityChangeEvent>());
	readonly onDidChangeVisibility: Event<vibecoda.TreeViewVisibilityChangeEvent> = this._onDidChangeVisibility.event;

	private _onDidChangeCheckboxState = this._register(new Emitter<vibecoda.TreeCheckboxChangeEvent<T>>());
	readonly onDidChangeCheckboxState: Event<vibecoda.TreeCheckboxChangeEvent<T>> = this._onDidChangeCheckboxState.event;
	...
	constructor(
		private _viewId: string, options: vibecoda.TreeViewOptions<T>,
		private _proxy: MainThreadTreeViewsShape,
	...
	private _message: string | vibecoda.MarkdownString = '';
	get message(): string | vibecoda.MarkdownString {
	...
	private _badge: vibecoda.ViewBadge | undefined;
	get badge(): vibecoda.ViewBadge | undefined {
		return this._badge;
	}
	...
	async setCheckboxState(checkboxUpdates: CheckboxUpdate[]) {
		type CheckboxUpdateWithItem = { extensionItem: NonNullable<T>; treeItem: vibecoda.TreeItem; newState: extHostTypes.TreeItemCheckboxState };
	...
	async handleDrag(sourceTreeItemHandles: TreeItemHandle[], treeDataTransfer: vibecoda.DataTransfer, token: CancellationToken): Promise<vibecoda.DataTransfer | undefined> {
	...
	async onDrop(treeDataTransfer: vibecoda.DataTransfer, targetHandleOrNode: TreeItemHandle | undefined, token: CancellationToken): Promise<void> {
	...
	async resolveTreeItem(treeItemHandle: string, token: vibecoda.CancellationToken): Promise<ITreeItem | undefined> {
	...
	private _createAndRegisterTreeNode(element: T, extTreeItem: vibecoda.TreeItem, parentNode: TreeNode | Root): TreeNode {
	...
	private _getTooltip(tooltip?: string | vibecoda.MarkdownString): string | IMarkdownString | undefined {
	...
	private _getCommand(disposable: DisposableStore, command?: vibecoda.Command): TreeCommand | undefined {
		return command ? { ...this._commands.toInternal(command, disposable), originalId: command.command } : undefined;
	}

	private _getCheckbox(extensionTreeItem: vibecoda.TreeItem): ITreeItemCheckboxState | undefined {
	...
	private _validateTreeItem(extensionTreeItem: vibecoda.TreeItem) {
	...
	private _createTreeNode(element: T, extensionTreeItem: vibecoda.TreeItem, parent: TreeNode | Root): TreeNode {
	...
	private _getThemeIcon(extensionTreeItem: vibecoda.TreeItem): extHostTypes.ThemeIcon | undefined {
		return extensionTreeItem.iconPath instanceof extHostTypes.ThemeIcon ? extensionTreeItem.iconPath : undefined;
	}

	private _createHandle(element: T, { id, label, resourceUri }: vibecoda.TreeItem, parent: TreeNode | Root, returnFirst?: boolean): TreeItemHandle {
		if (id) {
	...
	private _getLightIconPath(extensionTreeItem: vibecoda.TreeItem): URI | undefined {
	...
	private _getDarkIconPath(extensionTreeItem: vibecoda.TreeItem): URI | undefined {
