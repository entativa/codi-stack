...
function shouldTransformUri(uri: string): boolean {
	...
	return /^(file|vibecoda-remote):/i.test(uri);
}
...
if ((<any>self).Worker) {
	...
	Worker = <any>function (stringUrl: string | URL, options?: WorkerOptions) {
		...
		} else if (/^vibecoda-remote:/i.test(stringUrl.toString())) {
			// Supporting transformation of vibecoda-remote URIs requires an async call to the main thread,
		...
		const bootstrapFnSource = (function bootstrapFn(workerUrl: string) {
			function asWorkerBrowserUrl(url: string | URL | TrustedScriptURL): any {
				if (typeof url === 'string' || url instanceof URL) {
					return String(url).replace(/^file:\/\//i, 'vibecoda-file://vibecoda-app');
				}
				return url;
			}
...
interface IInitMessage {
	readonly type: 'vibecoda.init';
	readonly data: ReadonlyMap<string, MessagePort>;
}

function isInitMessage(a: any): a is IInitMessage {
	return !!a && typeof a === 'object' && a.type === 'vibecoda.init' && a.data instanceof Map;
}
