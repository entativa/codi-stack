...
function isLocation(value: vibecoda.Location | vibecoda.LocationLink): value is vibecoda.Location {
	const candidate = value as vibecoda.Location;
...
suite('ExtHostLanguageFeatureCommands', function () {
	...
	let disposables: vibecoda.Disposable[] = [];
	...
	test('WorkspaceSymbols, invalid arguments', function () {
		const promises = [
			assertRejects(() => commands.executeCommand('vibecoda.executeWorkspaceSymbolProvider')),
			assertRejects(() => commands.executeCommand('vibecoda.executeWorkspaceSymbolProvider', null)),
			assertRejects(() => commands.executeCommand('vibecoda.executeWorkspaceSymbolProvider', undefined)),
			assertRejects(() => commands.executeCommand('vibecoda.executeWorkspaceSymbolProvider', true))
		];
	...
	test('WorkspaceSymbols, back and forth', function () {
		...
		disposables.push(extHost.registerWorkspaceSymbolProvider(nullExtensionDescription, <vibecoda.WorkspaceSymbolProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.SymbolInformation[]>('vibecoda.executeWorkspaceSymbolProvider', 'testing').then(value => {
	...
	test('executeWorkspaceSymbolProvider should accept empty string, #39522', async function () {
		...
		disposables.push(extHost.registerWorkspaceSymbolProvider(nullExtensionDescription, {
			provideWorkspaceSymbols(): vibecoda.SymbolInformation[] {
				return [new types.SymbolInformation('hello', types.SymbolKind.Array, new types.Range(0, 0, 0, 0), URI.parse('foo:bar')) as vibecoda.SymbolInformation];
			}
		}));

		await rpcProtocol.sync();
		let symbols = await commands.executeCommand<vibecoda.SymbolInformation[]>('vibecoda.executeWorkspaceSymbolProvider', '');
		assert.strictEqual(symbols.length, 1);

		await rpcProtocol.sync();
		symbols = await commands.executeCommand<vibecoda.SymbolInformation[]>('vibecoda.executeWorkspaceSymbolProvider', '*');
		assert.strictEqual(symbols.length, 1);
	});
	...
	test('executeFormatDocumentProvider, back and forth', async function () {
		...
		disposables.push(extHost.registerDocumentFormattingEditProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.DocumentFormattingEditProvider {
		...
		await rpcProtocol.sync();
		const edits = await commands.executeCommand<vibecoda.SymbolInformation[]>('vibecoda.executeFormatDocumentProvider', model.uri, {
			insertSpaces: false,
			tabSize: 4,
	...
	// --- rename
	test('vibecoda.prepareRename', async function () {
		disposables.push(extHost.registerRenameProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.RenameProvider {

			prepareRename(document: vibecoda.TextDocument, position: vibecoda.Position) {
				return {
			...
			provideRenameEdits(document: vibecoda.TextDocument, position: vibecoda.Position, newName: string) {
		...
		const data = await commands.executeCommand<{ range: vibecoda.Range; placeholder: string }>('vibecoda.prepareRename', model.uri, new types.Position(0, 12));
	...
	test('vibecoda.executeDocumentRenameProvider', async function () {
		disposables.push(extHost.registerRenameProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.RenameProvider {
			provideRenameEdits(document: vibecoda.TextDocument, position: vibecoda.Position, newName: string) {
				const edit = new types.WorkspaceEdit();
				edit.insert(document.uri, <types.Position>position, newName);
		...
		const edit = await commands.executeCommand<vibecoda.WorkspaceEdit>('vibecoda.executeDocumentRenameProvider', model.uri, new types.Position(0, 12), 'newNameOfThis');
	...
	test('Definition, invalid arguments', function () {
		const promises = [
			assertRejects(() => commands.executeCommand('vibecoda.executeDefinitionProvider')),
			assertRejects(() => commands.executeCommand('vibecoda.executeDefinitionProvider', null)),
			assertRejects(() => commands.executeCommand('vibecoda.executeDefinitionProvider', undefined)),
			assertRejects(() => commands.executeCommand('vibecoda.executeDefinitionProvider', true, false))
		];
	...
	test('Definition, back and forth', function () {
		...
		disposables.push(extHost.registerDefinitionProvider(nullExtensionDescription, defaultSelector, <vibecoda.DefinitionProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Location[]>('vibecoda.executeDefinitionProvider', model.uri, new types.Position(0, 0)).then(values => {
	...
	test('Definition Link', () => {
		disposables.push(extHost.registerDefinitionProvider(nullExtensionDescription, defaultSelector, <vibecoda.DefinitionProvider>{
			provideDefinition(doc: any): (vibecoda.Location | vibecoda.LocationLink)[] {
				return [
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<(vibecoda.Location | vibecoda.LocationLink)[]>('vibecoda.executeDefinitionProvider', model.uri, new types.Position(0, 0)).then(values => {
	...
	test('Declaration, back and forth', function () {
		...
		disposables.push(extHost.registerDeclarationProvider(nullExtensionDescription, defaultSelector, <vibecoda.DeclarationProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Location[]>('vibecoda.executeDeclarationProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 4);
				for (const v of values) {
					assert.ok(v.uri instanceof URI);
					assert.ok(v.range instanceof types.Range);
	...
	test('Declaration Link', () => {
		disposables.push(extHost.registerDeclarationProvider(nullExtensionDescription, defaultSelector, <vibecoda.DeclarationProvider>{
			provideDeclaration(doc: any): (vibecoda.Location | vibecoda.LocationLink)[] {
				return [
					new types.Location(doc.uri, new types.Range(0, 0, 0, 0)),
					{ targetUri: doc.uri, targetRange: new types.Range(1, 0, 0, 0), targetSelectionRange: new types.Range(1, 1, 1, 1), originSelectionRange: new types.Range(2, 2, 2, 2) }
				];
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<(vibecoda.Location | vibecoda.LocationLink)[]>('vibecoda.executeDeclarationProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 2);
				for (const v of values) {
					if (isLocation(v)) {
						assert.ok(v.uri instanceof URI);
	...
	test('Type Definition, invalid arguments', function () {
		const promises = [
			assertRejects(() => commands.executeCommand('vibecoda.executeTypeDefinitionProvider')),
			assertRejects(() => commands.executeCommand('vibecoda.executeTypeDefinitionProvider', null)),
			assertRejects(() => commands.executeCommand('vibecoda.executeTypeDefinitionProvider', undefined)),
			assertRejects(() => commands.executeCommand('vibecoda.executeTypeDefinitionProvider', true, false))
		];
	...
	test('Type Definition, back and forth', function () {
		...
		disposables.push(extHost.registerTypeDefinitionProvider(nullExtensionDescription, defaultSelector, <vibecoda.TypeDefinitionProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Location[]>('vibecoda.executeTypeDefinitionProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 4);
				for (const v of values) {
					assert.ok(v.uri instanceof URI);
					assert.ok(v.range instanceof types.Range);
	...
	test('Type Definition Link', () => {
		disposables.push(extHost.registerTypeDefinitionProvider(nullExtensionDescription, defaultSelector, <vibecoda.TypeDefinitionProvider>{
			provideTypeDefinition(doc: any): (vibecoda.Location | vibecoda.LocationLink)[] {
				return [
					new types.Location(doc.uri, new types.Range(0, 0, 0, 0)),
					{ targetUri: doc.uri, targetRange: new types.Range(1, 0, 0, 0), targetSelectionRange: new types.Range(1, 1, 1, 1), originSelectionRange: new types.Range(2, 2, 2, 2) }
				];
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<(vibecoda.Location | vibecoda.LocationLink)[]>('vibecoda.executeTypeDefinitionProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 2);
				for (const v of values) {
					if (isLocation(v)) {
						assert.ok(v.uri instanceof URI);
	...
	test('Implementation, invalid arguments', function () {
		const promises = [
			assertRejects(() => commands.executeCommand('vibecoda.executeImplementationProvider')),
			assertRejects(() => commands.executeCommand('vibecoda.executeImplementationProvider', null)),
			assertRejects(() => commands.executeCommand('vibecoda.executeImplementationProvider', undefined)),
			assertRejects(() => commands.executeCommand('vibecoda.executeImplementationProvider', true, false))
		];
	...
	test('Implementation, back and forth', function () {
		...
		disposables.push(extHost.registerImplementationProvider(nullExtensionDescription, defaultSelector, <vibecoda.ImplementationProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Location[]>('vibecoda.executeImplementationProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 4);
				for (const v of values) {
					assert.ok(v.uri instanceof URI);
					assert.ok(v.range instanceof types.Range);
	...
	test('Implementation Definition Link', () => {
		disposables.push(extHost.registerImplementationProvider(nullExtensionDescription, defaultSelector, <vibecoda.ImplementationProvider>{
			provideImplementation(doc: any): (vibecoda.Location | vibecoda.LocationLink)[] {
				return [
					new types.Location(doc.uri, new types.Range(0, 0, 0, 0)),
					{ targetUri: doc.uri, targetRange: new types.Range(1, 0, 0, 0), targetSelectionRange: new types.Range(1, 1, 1, 1), originSelectionRange: new types.Range(2, 2, 2, 2) }
				];
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<(vibecoda.Location | vibecoda.LocationLink)[]>('vibecoda.executeImplementationProvider', model.uri, new types.Position(0, 0)).then(values => {
				assert.strictEqual(values.length, 2);
				for (const v of values) {
					if (isLocation(v)) {
						assert.ok(v.uri instanceof URI);
	...
	test('reference search, back and forth', function () {
		...
		disposables.push(extHost.registerReferenceProvider(nullExtensionDescription, defaultSelector, <vibecoda.ReferenceProvider>{
			provideReferences() {
				return [
		...
		return commands.executeCommand<vibecoda.Location[]>('vibecoda.executeReferenceProvider', model.uri, new types.Position(0, 0)).then(values => {
	...
	test('"vibecoda.executeDocumentHighlights" API has stopped returning DocumentHighlight[]#200056', async function () {


		disposables.push(extHost.registerDocumentHighlightProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentHighlightProvider>{
		...
		return commands.executeCommand<vibecoda.DocumentHighlight[]>('vibecoda.executeDocumentHighlights', model.uri, new types.Position(0, 0)).then(values => {
	...
	test('Outline, back and forth', function () {
		disposables.push(extHost.registerDocumentSymbolProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentSymbolProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.SymbolInformation[]>('vibecoda.executeDocumentSymbolProvider', model.uri).then(values => {
				assert.strictEqual(values.length, 2);
	...
	test('vibecoda.executeDocumentSymbolProvider command only returns SymbolInformation[] rather than DocumentSymbol[] #57984', function () {
		disposables.push(extHost.registerDocumentSymbolProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentSymbolProvider>{
			provideDocumentSymbols(): any {
				return [
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<(vibecoda.SymbolInformation & vibecoda.DocumentSymbol)[]>('vibecoda.executeDocumentSymbolProvider', model.uri).then(values => {
				assert.strictEqual(values.length, 2);
				const [first, second] = values;
				assert.strictEqual(first instanceof types.SymbolInformation, true);
	...
	testApiCmd('triggerCharacter is null when completion provider is called programmatically #159914', async function () {
		...
		let actualContext: vibecoda.CompletionContext | undefined;

		disposables.push(extHost.registerCompletionItemProvider(nullExtensionDescription, defaultSelector, <vibecoda.CompletionItemProvider>{
		...
		await commands.executeCommand<vibecoda.CompletionList>('vibecoda.executeCompletionItemProvider', model.uri, new types.Position(0, 4));
	...
	testApiCmd('Suggest, back and forth', async function () {
		...
		const list = await commands.executeCommand<vibecoda.CompletionList>('vibecoda.executeCompletionItemProvider', model.uri, new types.Position(0, 4));
	...
	testApiCmd('Suggest, resolve completion items', async function () {
		...
		const list = await commands.executeCommand<vibecoda.CompletionList>(
			'vibecoda.executeCompletionItemProvider',
			model.uri,
	...
	testApiCmd('"vibecoda.executeCompletionItemProvider" doesnot return a preselect field #53749', async function () {
	...
	testApiCmd('vibecoda.executeCompletionItemProvider returns the wrong CompletionItemKinds in insiders #95715', async function () {
		disposables.push(extHost.registerCompletionItemProvider(nullExtensionDescription, defaultSelector, <vibecoda.CompletionItemProvider>{
			provideCompletionItems(): any {
				return [
	...
	test('Parameter Hints, back and forth', async () => {
		disposables.push(extHost.registerSignatureHelpProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.SignatureHelpProvider {
			provideSignatureHelp(_document: vibecoda.TextDocument, _position: vibecoda.Position, _token: vibecoda.CancellationToken, context: vibecoda.SignatureHelpContext): vibecoda.SignatureHelp {
				return {
					activeSignature: 0,
					activeParameter: 1,
					signatures: [
						{
							label: 'abc',
							documentation: `${context.triggerKind === 1 /* vibecoda.SignatureHelpTriggerKind.Invoke */ ? 'invoked' : 'unknown'} ${context.triggerCharacter}`,
							parameters: []
						}
					]
				};
		...
		const firstValue = await commands.executeCommand<vibecoda.SignatureHelp>('vibecoda.executeSignatureHelpProvider', model.uri, new types.Position(0, 1), ',');
	...
	testApiCmd('QuickFix, back and forth', function () {
		disposables.push(extHost.registerCodeActionProvider(nullExtensionDescription, defaultSelector, {
			provideCodeActions(): vibecoda.Command[] {
				return [{ command: 'testing', title: 'Title', arguments: [1, 2, true] }];
			}
		}));

		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Command[]>('vibecoda.executeCodeActionProvider', model.uri, new types.Range(0, 0, 1, 1)).then(value => {
	...
	testApiCmd('vibecoda.executeCodeActionProvider results seem to be missing their `command` property #45124', function () {
		disposables.push(extHost.registerCodeActionProvider(nullExtensionDescription, defaultSelector, {
			provideCodeActions(document, range): vibecoda.CodeAction[] {
				return [{
					command: {
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.CodeAction[]>('vibecoda.executeCodeActionProvider', model.uri, new types.Range(0, 0, 1, 1)).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
	...
	testApiCmd('vibecoda.executeCodeActionProvider passes Range to provider although Selection is passed in #77997', function () {
		disposables.push(extHost.registerCodeActionProvider(nullExtensionDescription, defaultSelector, {
			provideCodeActions(document, rangeOrSelection): vibecoda.CodeAction[] {
				return [{
					command: {
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.CodeAction[]>('vibecoda.executeCodeActionProvider', model.uri, selection).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
				assert.ok(first.command);
	...
	testApiCmd('vibecoda.executeCodeActionProvider results seem to be missing their `isPreferred` property #78098', function () {
		disposables.push(extHost.registerCodeActionProvider(nullExtensionDescription, defaultSelector, {
			provideCodeActions(document, rangeOrSelection): vibecoda.CodeAction[] {
				return [{
					command: {
	...
	testApiCmd('resolving code action', async function () {
		...
		disposables.push(extHost.registerCodeActionProvider(nullExtensionDescription, defaultSelector, {
			...
			resolveCodeAction(action): vibecoda.CodeAction {
		...
		const value = await commands.executeCommand<vibecoda.CodeAction[]>('vibecoda.executeCodeActionProvider', model.uri, selection, undefined, 1000);
	...
	testApiCmd('CodeLens, back and forth', function () {
		...
		disposables.push(extHost.registerCodeLensProvider(nullExtensionDescription, defaultSelector, <vibecoda.CodeLensProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.CodeLens[]>('vibecoda.executeCodeLensProvider', model.uri).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
	...
	testApiCmd('CodeLens, resolve', async function () {
		...
		let value = await commands.executeCommand<vibecoda.CodeLens[]>('vibecoda.executeCodeLensProvider', model.uri, 2);

		assert.strictEqual(value.length, 3); // the resolve argument defines the number of results being returned
		assert.strictEqual(resolveCount, 2);

		resolveCount = 0;
		value = await commands.executeCommand<vibecoda.CodeLens[]>('vibecoda.executeCodeLensProvider', model.uri);
	...
	testApiCmd('Links, back and forth', function () {
		...
		disposables.push(extHost.registerDocumentLinkProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentLinkProvider>{
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.DocumentLink[]>('vibecoda.executeLinkProvider', model.uri).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
	...
	testApiCmd('What\'s the condition for DocumentLink target to be undefined? #106308', async function () {
		...
		const links1 = await commands.executeCommand<vibecoda.DocumentLink[]>('vibecoda.executeLinkProvider', model.uri);
		assert.strictEqual(links1.length, 1);
		assert.strictEqual(links1[0].target, undefined);

		const links2 = await commands.executeCommand<vibecoda.DocumentLink[]>('vibecoda.executeLinkProvider', model.uri, 1000);
		assert.strictEqual(links2.length, 1);
		assert.strictEqual(links2[0].target!.toString(), URI.parse('foo:bar').toString());

	});

	testApiCmd('DocumentLink[] vibecoda.executeLinkProvider returns lack tooltip #213970', async function () {
		disposables.push(extHost.registerDocumentLinkProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentLinkProvider>{
			provideDocumentLinks(): any {
	...
	test('Color provider', function () {
		...
		disposables.push(extHost.registerColorProvider(nullExtensionDescription, defaultSelector, <vibecoda.DocumentColorProvider>{
			provideDocumentColors(): vibecoda.ColorInformation[] {
				return [new types.ColorInformation(new types.Range(0, 0, 0, 20), new types.Color(0.1, 0.2, 0.3, 0.4))];
			},
			provideColorPresentations(): vibecoda.ColorPresentation[] {
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.ColorInformation[]>('vibecoda.executeDocumentColorProvider', model.uri).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
			...
			return commands.executeCommand<vibecoda.ColorPresentation[]>('vibecoda.executeColorPresentationProvider', color, { uri: model.uri, range }).then(value => {
				assert.strictEqual(value.length, 1);
				const [first] = value;
	...
	test('"TypeError: e.onCancellationRequested is not a function" calling hover provider in Insiders #54174', function () {
		...
		disposables.push(extHost.registerHoverProvider(nullExtensionDescription, defaultSelector, <vibecoda.HoverProvider>{
			provideHover(): any {
		...
		return rpcProtocol.sync().then(() => {
			return commands.executeCommand<vibecoda.Hover[]>('vibecoda.executeHoverProvider', model.uri, new types.Position(1, 1)).then(value => {
				assert.strictEqual(value.length, 1);
	...
	testApiCmd('Inlay Hints, back and forth', async function () {
		disposables.push(extHost.registerInlayHintsProvider(nullExtensionDescription, defaultSelector, <vibecoda.InlayHintsProvider>{
			provideInlayHints() {
		...
		const value = await commands.executeCommand<vibecoda.InlayHint[]>('vibecoda.executeInlayHintProvider', model.uri, new types.Range(0, 0, 20, 20));
		assert.strictEqual(value.length, 1);
	...
	test('Selection Range, back and forth', async function () {
		...
		disposables.push(extHost.registerSelectionRangeProvider(nullExtensionDescription, defaultSelector, <vibecoda.SelectionRangeProvider>{
		...
		await rpcProtocol.sync();
		const value = await commands.executeCommand<vibecoda.SelectionRange[]>('vibecoda.executeSelectionRangeProvider', model.uri, [new types.Position(0, 10)]);
		assert.strictEqual(value.length, 1);
	...
	test('CallHierarchy, back and forth', async function () {
		...
		disposables.push(extHost.registerCallHierarchyProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.CallHierarchyProvider {

			prepareCallHierarchy(document: vibecoda.TextDocument, position: vibecoda.Position,): vibecoda.ProviderResult<vibecoda.CallHierarchyItem> {
			...
			provideCallHierarchyIncomingCalls(item: vibecoda.CallHierarchyItem, token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.CallHierarchyIncomingCall[]> {
			...
			provideCallHierarchyOutgoingCalls(item: vibecoda.CallHierarchyItem, token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.CallHierarchyOutgoingCall[]> {
		...
		const root = await commands.executeCommand<vibecoda.CallHierarchyItem[]>('vibecoda.prepareCallHierarchy', model.uri, new types.Position(0, 0));

		assert.ok(Array.isArray(root));
		assert.strictEqual(root.length, 1);
		assert.strictEqual(root[0].name, 'ROOT');

		const incoming = await commands.executeCommand<vibecoda.CallHierarchyIncomingCall[]>('vibecoda.provideIncomingCalls', root[0]);
		assert.strictEqual(incoming.length, 1);
		assert.strictEqual(incoming[0].from.name, 'INCOMING');

		const outgoing = await commands.executeCommand<vibecoda.CallHierarchyOutgoingCall[]>('vibecoda.provideOutgoingCalls', root[0]);
	...
	test('TypeHierarchy, back and forth', async function () {
		...
		disposables.push(extHost.registerTypeHierarchyProvider(nullExtensionDescription, defaultSelector, new class implements vibecoda.TypeHierarchyProvider {
			prepareTypeHierarchy(document: vibecoda.TextDocument, position: vibecoda.Position, token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.TypeHierarchyItem[]> {
			...
			provideTypeHierarchySupertypes(item: vibecoda.TypeHierarchyItem, token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.TypeHierarchyItem[]> {
			...
			provideTypeHierarchySubtypes(item: vibecoda.TypeHierarchyItem, token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.TypeHierarchyItem[]> {
		...
		const root = await commands.executeCommand<vibecoda.TypeHierarchyItem[]>('vibecoda.prepareTypeHierarchy', model.uri, new types.Position(0, 0));

		assert.ok(Array.isArray(root));
		assert.strictEqual(root.length, 1);
		assert.strictEqual(root[0].name, 'ROOT');

		const incoming = await commands.executeCommand<vibecoda.TypeHierarchyItem[]>('vibecoda.provideSupertypes', root[0]);
		assert.strictEqual(incoming.length, 1);
		assert.strictEqual(incoming[0].name, 'SUPER');

		const outgoing = await commands.executeCommand<vibecoda.TypeHierarchyItem[]>('vibecoda.provideSubtypes', root[0]);
		assert.strictEqual(outgoing.length, 1);
	...
	test('more element test of selectionRangeProvider on inner array always returns outer array #91852', async function () {
		...
		await rpcProtocol.sync();
		const value = await commands.executeCommand<vibecoda.SelectionRange[]>(
			'vibecoda.executeSelectionRangeProvider',
			model.uri,
