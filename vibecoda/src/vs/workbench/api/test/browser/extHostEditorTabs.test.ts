...
suite('ExtHostEditorTabs', function () {
	...
	test('Ensure tabGroup change events fires', function () {
		...
		const activeTabGroup: vibecoda.TabGroup = extHostEditorTabs.tabGroups.activeTabGroup;
		assert.strictEqual(extHostEditorTabs.tabGroups.all.length, 1);
	...
	test('Check TabGroupChangeEvent properties', function () {
		...
		const events: vibecoda.TabGroupChangeEvent[] = [];
	...
	test('vibecoda.window.tagGroups is immutable', function () {
	...
	test('Update tab only sends tab change event', async function () {
		...
		const p = new Promise<vibecoda.TabChangeEvent>(resolve => store.add(extHostEditorTabs.tabGroups.onDidChangeTabs(resolve)));
