...
suite('ExtHostWebview', () => {
	...
	function createWebview(rpcProtocol: (IExtHostRpcService & IExtHostContext) | undefined, remoteAuthority: string | undefined) {
		...
		return disposables.add(extHostWebviewPanels.createWebviewPanel({
			extensionLocation: URI.from({
				scheme: remoteAuthority ? Schemas.vibecodaRemote : Schemas.file,
				authority: remoteAuthority,
				path: '/ext/path',
			})
	...
	test('Cannot register multiple serializers for the same view type', async () => {
		...
		let lastInvokedDeserializer: vibecoda.WebviewPanelSerializer | undefined = undefined;

		class NoopSerializer implements vibecoda.WebviewPanelSerializer {
			async deserializeWebviewPanel(webview: vibecoda.WebviewPanel, _state: any): Promise<void> {
	...
	test('asWebviewUri for local file paths', () => {
		...
		assert.strictEqual(
			...
			`https://file%2B.vibecoda-resource.${webviewResourceBaseHost}/Users/codey/file.html`,
			'Unix basic'
		);

		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file:///Users/codey/file.html#frag')).toString()),
			`https://file%2B.vibecoda-resource.${webviewResourceBaseHost}/Users/codey/file.html#frag`,
			'Unix should preserve fragment'
		);

		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file:///Users/codey/f%20ile.html')).toString()),
			`https://file%2B.vibecoda-resource.${webviewResourceBaseHost}/Users/codey/f%20ile.html`,
			'Unix with encoding'
		);

		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file://localhost/Users/codey/file.html')).toString()),
			`https://file%2Blocalhost.vibecoda-resource.${webviewResourceBaseHost}/Users/codey/file.html`,
			'Unix should preserve authority'
		);

		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file:///c:/codey/file.txt')).toString()),
			`https://file%2B.vibecoda-resource.${webviewResourceBaseHost}/c%3A/codey/file.txt`,
			'Windows C drive'
		);
	});
	...
	test('asWebviewUri for remote file paths', () => {
		...
		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file:///Users/codey/file.html')).toString()),
			`https://vibecoda-remote%2Bremote.vibecoda-resource.${webviewResourceBaseHost}/Users/codey/file.html`,
			'Unix basic'
		);
	});
	...
	test('asWebviewUri for remote with / and + in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vibecoda-remote%2Bssh-002dremote-002blocalhost-003dfoo-002fbar.vibecoda-resource.vibecoda-cdn.net/Users/cody/x.png`,
			'Check transform');

		assert.strictEqual(
			decodeAuthority(webviewUri.authority),
			`vibecoda-remote+${authority}.vibecoda-resource.vibecoda-cdn.net`,
	...
	test('asWebviewUri for remote with port in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vibecoda-remote%2Blocalhost-003a8080.vibecoda-resource.vibecoda-cdn.net/Users/cody/x.png`,
			'Check transform');
