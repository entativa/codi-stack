...
suite('NotebookCell#Document', function () {
	...
	setup(async function () {
		...
		const reg = extHostNotebooks.registerNotebookSerializer(nullExtensionDescription, 'test', new class extends mock<vibecoda.NotebookSerializer>() { });
	...
	test('cell document is vibecoda.TextDocument', async function () {
	...
	test('cell document is vibecoda.TextDocument after changing it', async function () {
	...
	test('cell document stays open when notebook is still open', async function () {
		...
		const docs: vibecoda.TextDocument[] = [];
	...
	test('Opening a notebook results in Vibecoda firing the event onDidChangeActiveNotebookEditor twice #118470', function () {
		let count = 0;
