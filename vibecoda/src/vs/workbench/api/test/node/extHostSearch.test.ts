...
function extensionResultIsMatch(data: vibecoda.TextSearchResult): data is vibecoda.TextSearchMatch {
	return !!(<vibecoda.TextSearchMatch>data).preview;
}

suite('ExtHostSearch', () => {
	const disposables = ensureNoDisposablesAreLeakedInTestSuite();

	async function registerTestTextSearchProvider(provider: vibecoda.TextSearchProvider, scheme = 'file'): Promise<void> {
		disposables.add(extHostSearch.registerTextSearchProviderOld(scheme, provider));
		await rpcProtocol.sync();
	}

	async function registerTestFileSearchProvider(provider: vibecoda.FileSearchProvider, scheme = 'file'): Promise<void> {
	...
	setup(() => {
		...
		extHostSearch = disposables.add(new class extends NativeExtHostSearch {
				...
				super(
					...
					new class extends mock<IExtHostConfiguration>() {
						override async getConfigProvider(): Promise<ExtHostConfigProvider> {
							return {
								onDidChangeConfiguration(_listener: (event: vibecoda.ConfigurationChangeEvent) => void) { },
								getConfiguration(): vibecoda.WorkspaceConfiguration {
									return {
										get() { },
										has() {
											return false;
			...
			protected override createTextSearchManager(query: ITextQuery, provider: vibecoda.TextSearchProvider2): TextSearchManager {
	...
	suite('File:', () => {
		...
		test('no results', async () => {
			await registerTestFileSearchProvider({
				provideFileSearchResults(query: vibecoda.FileSearchQuery, options: vibecoda.FileSearchOptions, token: vibecoda.CancellationToken): Promise<URI[]> {
		...
		test('session cancellation should work', async () => {
			...
			const disposables: (vibecoda.Disposable | undefined)[] = [];
			await registerTestFileSearchProvider({
				provideFileSearchResults(query: vibecoda.FileSearchQuery, options: vibecoda.FileSearchOptions, token: vibecoda.CancellationToken): Promise<URI[]> {
		...
		// https://github.com/microsoft/vibecoda-remotehub/issues/255
		...
		test('multiroot max results', async () => {
			let cancels = 0;
			await registerTestFileSearchProvider({
				async provideFileSearchResults(query: vibecoda.FileSearchQuery, options: vibecoda.FileSearchOptions, token: vibecoda.CancellationToken): Promise<URI[]> {
	...
	suite('Text:', () => {
		...
		function makePreview(text: string): vibecoda.TextSearchMatch['preview'] {
			return {
		...
		function makeTextResult(baseFolder: URI, relativePath: string): vibecoda.TextSearchMatch {
			return {
		...
		function assertResults(actual: IFileMatch[], expected: vibecoda.TextSearchResult[]) {
			const actualTextSearchResults: vibecoda.TextSearchResult[] = [];
			for (const fileMatch of actual) {
				...
				for (const lineResult of fileMatch.results!) {
					if (resultIsMatch(lineResult)) {
						...
						actualTextSearchResults.push(<vibecoda.TextSearchContext>{
			...
			const rangeToString = (r: vibecoda.Range) => `(${r.start.line}, ${r.start.character}), (${r.end.line}, ${r.end.character})`;

			const makeComparable = (results: vibecoda.TextSearchResult[]) => results
				.sort((a, b) => {
		...
		test('no results', async () => {
			await registerTestTextSearchProvider({
				provideTextSearchResults(query: vibecoda.TextSearchQuery, options: vibecoda.TextSearchOptions, progress: vibecoda.Progress<vibecoda.TextSearchResult>, token: vibecoda.CancellationToken): Promise<vibecoda.TextSearchComplete> {
					return Promise.resolve(null!);
				}
			});
		...
		test('basic results', async () => {
			const providedResults: vibecoda.TextSearchResult[] = [
		...
		test('multiroot max results', async () => {
			let cancels = 0;
			await registerTestTextSearchProvider({
				async provideTextSearchResults(query: vibecoda.TextSearchQuery, options: vibecoda.TextSearchOptions, progress: vibecoda.Progress<vibecoda.TextSearchResult>, token: vibecoda.CancellationToken): Promise<vibecoda.TextSearchComplete> {
					disposables.add(token.onCancellationRequested(() => cancels++));
					await new Promise(r => process.nextTick(r));
					[
						'file1.ts',
