...
suite('URI Label', () => {
	...
	test('custom scheme', function () {
		...
		const uri1 = URI.parse('vibecoda://microsoft.com/1/2/3/4/5');
	...
	test('custom query', function () {
		...
		const uri1 = URI.parse(`vibecoda://microsoft.com/1/2/3/4/5?${encodeURIComponent(JSON.stringify({ prefix: 'prefix', path: 'path' }))}`);
	...
	test('custom query without value', function () {
		...
		const uri1 = URI.parse(`vibecoda://microsoft.com/1/2/3/4/5?${encodeURIComponent(JSON.stringify({ path: 'path' }))}`);
	...
	test('custom query without query json', function () {
		...
		const uri1 = URI.parse('vibecoda://microsoft.com/1/2/3/4/5?path=foo');
