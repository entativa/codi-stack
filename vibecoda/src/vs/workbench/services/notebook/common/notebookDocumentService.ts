...
export function parse(cell: URI): { notebook: URI; handle: number } | undefined {
	if (cell.scheme !== Schemas.vibecodaNotebookCell) {
		return undefined;
	}
...
export function generate(notebook: URI, handle: number): URI {
	...
	return notebook.with({ scheme: Schemas.vibecodaNotebookCell, fragment });
}

export function parseMetadataUri(metadata: URI): URI | undefined {
	if (metadata.scheme !== Schemas.vibecodaNotebookMetadata) {
		return undefined;
	}
...
export function generateMetadataUri(notebook: URI): URI {
	...
	return notebook.with({ scheme: Schemas.vibecodaNotebookMetadata, fragment });
}
...
export function extractCellOutputDetails(uri: URI): { notebook: URI; openIn: string; outputId?: string; cellFragment?: string; outputIndex?: number; cellHandle?: number; cellIndex?: number } | undefined {
	if (uri.scheme !== Schemas.vibecodaNotebookCellOutput) {
		return;
	}
	...
	const parsedCell = parse(uri.with({ scheme: Schemas.vibecodaNotebookCell, query: null }));
