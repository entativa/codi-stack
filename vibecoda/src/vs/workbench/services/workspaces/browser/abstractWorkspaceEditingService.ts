...
export abstract class AbstractWorkspaceEditingService extends Disposable implements IWorkspaceEditingService {
	...
	async pickNewWorkspacePath(): Promise<URI | undefined> {
		...
		if (this.environmentService.remoteAuthority) {
			availableFileSystems.unshift(Schemas.vibecodaRemote);
		}
		...
		if (!hasWorkspaceFileExtension(workspacePath)) {
			...
			// (see https://github.com/microsoft/vibecoda/issues/84818)
	...
	private async doAddFolders(foldersToAdd: IWorkspaceFolderCreationData[], index?: number, donotNotifyError = false): Promise<void> {
		...
		if (remoteAuthority) {
			// https://github.com/microsoft/vibecoda/issues/94191
			foldersToAdd = foldersToAdd.filter(folder => folder.uri.scheme !== Schemas.file && (folder.uri.scheme !== Schemas.vibecodaRemote || isEqualAuthority(folder.uri.authority, remoteAuthority)));
		}
	...
	async createAndEnterWorkspace(folders: IWorkspaceFolderCreationData[], path?: URI): Promise<void> {
		...
		if (path) {
			try {
				...
				await this.workspacesService.deleteUntitledWorkspace(untitledWorkspace); // https://github.com/microsoft/vibecoda/issues/100276
			}
		} else {
