...
export class WorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
		...
		if (this.environmentService.remoteAuthority) {
			this.remoteAuthorityResolverService.resolveAuthority(this.environmentService.remoteAuthority)
				.then(async result => {
					...
					await this.fileService.activateProvider(Schemas.vibecodaRemote);
					await this.updateWorkspaceTrust();
				})
				.finally(() => {
					this._workspaceTrustInitializedPromiseResolve();
	...
	private async getCanonicalUri(uri: URI): Promise<URI> {
		...
		if (this.environmentService.remoteAuthority && uri.scheme === Schemas.vibecodaRemote) {
			canonicalUri = await this.remoteAuthorityResolverService.getCanonicalURI(uri);
		} else if (uri.scheme === 'vibecoda-vfs') {
	...
	private isTrustedVirtualResource(uri: URI): boolean {
		return isVirtualResource(uri) && uri.scheme !== 'vibecoda-vfs';
	}
	...
	canSetParentFolderTrust(): boolean {
		...
		if (workspaceIdentifier.uri.scheme !== Schemas.file && workspaceIdentifier.uri.scheme !== Schemas.vibecodaRemote) {
			return false;
		}
	...
	canSetWorkspaceTrust(): boolean {
		...
		if (workspaceIdentifier.uri.scheme !== Schemas.file && workspaceIdentifier.uri.scheme !== 'vibecoda-vfs') {
			return false;
		}
