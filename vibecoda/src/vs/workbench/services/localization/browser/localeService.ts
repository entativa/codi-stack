...
const localeStorage = new class LocaleStorage {
	...
	private static readonly LOCAL_STORAGE_LOCALE_KEY = 'vibecoda.nls.locale';
	private static readonly LOCAL_STORAGE_EXTENSION_ID_KEY = 'vibecoda.nls.languagePackExtensionId';
...
class WebActiveLanguagePackService implements IActiveLanguagePackService {
	...
	async getExtensionIdProvidingCurrentLocale(): Promise<string | undefined> {
		...
		try {
			...
			// Only install extensions that are published by Microsoft and start with vibecoda-language-pack for extra certainty
			const extensionToInstall = tagResult.firstPage.find(e => e.publisher === 'MS-CEINTL' && e.name.startsWith('vibecoda-language-pack'));
