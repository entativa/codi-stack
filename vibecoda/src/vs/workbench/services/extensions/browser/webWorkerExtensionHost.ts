...
export class WebWorkerExtensionHost extends Disposable implements IExtensionHost {
	...
	private async _startInsideIframe(): Promise<IMessagePassingProtocol> {
		...
		const vibecodaWebWorkerExtHostId = generateUuid();
		iframe.setAttribute('src', `${webWorkerExtensionHostIframeSrc}&vibecodaWebWorkerExtHostId=${vibecodaWebWorkerExtHostId}`);
		...
		this._register(dom.addDisposableListener(mainWindow, 'message', (event) => {
			...
			if (event.data.vibecodaWebWorkerExtHostId !== vibecodaWebWorkerExtHostId) {
				return;
			}
			if (event.data.error) {
			...
			if (event.data.type === 'vibecoda.bootstrap.nls') {
		...
		iframe.contentWindow!.postMessage({ type: 'vibecoda.init', data: messagePorts }, '*', [...messagePorts.values()]);
