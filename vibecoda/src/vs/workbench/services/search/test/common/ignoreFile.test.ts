...
suite('Parsing .gitignore files', () => {
	...
	test('real world example: vibecoda-js-debug', () => {
		const i = `.cache/
			.profile/
			.cdp-profile/
			.headless-profile/
			...
			/coverage
			/.nyc_output
			demos/web-worker/vibecoda-pwa-dap.log
			demos/web-worker/vibecoda-pwa-cdp.log
		...
		const included = [
			...
			'/inner/demos/web-worker/vibecoda-pwa-dap.log',
			'/inner/demos/web-worker/vibecoda-pwa-cdp.log',
	...
	test('real world example: vibecoda', () => {
		const i = `.DS_Store
			.cache
			npm-debug.log
			Thumbs.db
