...
export class HistoryService extends Disposable implements IHistoryService {
	...
	constructor(
		...
		// properly (fixes https://github.com/microsoft/vibecoda/issues/59908)
	...
	private onDidActiveEditorChange(): void {
		...
		// (https://github.com/microsoft/vibecoda/issues/211769)
	...
	private async doReopenLastClosedEditor(lastClosedEditor: IRecentlyClosedEditor): Promise<void> {
		...
		if (!this.editorGroupService.activeGroup.contains(lastClosedEditor.editor)) {
			...
			// Fix for https://github.com/microsoft/vibecoda/issues/107850
		...
		if (!editorPane) {
			...
			// Fix for https://github.com/microsoft/vibecoda/issues/67882
	...
	private loadHistoryFromStorage(): Array<IResourceEditorInput> {
		...
		if (entriesRaw) {
			try {
				...
				onUnexpectedError(error); // https://github.com/microsoft/vibecoda/issues/99075
			}
		}
...
class EditorHelper {
	...
	preferResourceEditorInput(editor: EditorInput | IResourceEditorInput): EditorInput | IResourceEditorInput | undefined {
		...
		// issues such as https://github.com/microsoft/vibecoda/issues/85204
		...
		const hasValidResourceEditorInputScheme =
			...
			resource?.scheme === Schemas.vibecodaRemote ||
			resource?.scheme === Schemas.vibecodaUserData ||
	...
	matchesFile(resource: URI, arg2: EditorInput | IResourceEditorInput | FileChangesEvent | FileOperationEvent): boolean {
		...
		if (isEditorInput(arg2)) {
			...
			if (this.lifecycleService.phase >= LifecyclePhase.Restored && !this.fileService.hasProvider(inputResource)) {
				return false; // make sure to only check this when workbench has restored (for https://github.com/microsoft/vibecoda/issues/48275)
			}
