...
import type { ModelOperations, ModelResult } from '@vibecoda/vibecoda-languagedetection';
...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private async getModelOperations(): Promise<ModelOperations> {
		...
		const { ModelOperations } = await importAMDNodeModule(uri, '') as typeof import('@vibecoda/vibecoda-languagedetection');
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in Vibecoda and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in Vibecoda
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in Vibecoda
			case 'csv':
			case 'toml':
