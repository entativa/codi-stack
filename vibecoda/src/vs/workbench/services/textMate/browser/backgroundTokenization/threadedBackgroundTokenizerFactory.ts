...
import type { IRawTheme } from 'vibecoda-textmate';
...
export class ThreadedBackgroundTokenizerFactory implements IDisposable {
	...
	private async _createWorkerProxy(): Promise<Proxied<TextMateTokenizationWorker> | null> {
		const onigurumaModuleLocation: AppResourcePath = `${nodeModulesPath}/vibecoda-oniguruma`;
		const onigurumaModuleLocationAsar: AppResourcePath = `${nodeModulesAsarPath}/vibecoda-oniguruma`;
