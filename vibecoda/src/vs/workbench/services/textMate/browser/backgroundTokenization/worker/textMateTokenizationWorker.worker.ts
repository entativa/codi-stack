...
import type { IOnigLib, IRawTheme, StackDiff } from 'vibecoda-textmate';
...
export class TextMateTokenizationWorker implements IWebWorkerServerRequestHandler {
	...
	private async _loadTMGrammarFactory(grammarDefinitions: IValidGrammarDefinition[], onigurumaWASMUri: string): Promise<TMGrammarFactory> {
		const vibecodaTextmate = await importAMDNodeModule<typeof import('vibecoda-textmate')>('vibecoda-textmate', 'release/main.js');
		const vibecodaOniguruma = await importAMDNodeModule<typeof import('vibecoda-oniguruma')>('vibecoda-oniguruma', 'release/main.js');
		...
		await vibecodaOniguruma.loadWASM(bytes);

		const onigLib: Promise<IOnigLib> = Promise.resolve({
			createOnigScanner: (sources) => vibecodaOniguruma.createOnigScanner(sources),
			createOnigString: (str) => vibecodaOniguruma.createOnigString(str)
		});
		...
		}, grammarDefinitions, vibecodaTextmate, onigLib);
	}
