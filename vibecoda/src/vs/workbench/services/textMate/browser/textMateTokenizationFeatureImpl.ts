...
import type { IGrammar, IOnigLib, IRawTheme } from 'vibecoda-textmate';
...
export class TextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
	...
	constructor(
		...
		this._vibecodaOniguruma = null;

		this._styleElement = domStylesheets.createStyleSheet();
		this._styleElement.className = 'vibecoda-tokens-styles';
		...
		if (this._debugMode) {
			this._progressService.withProgress(
				...
				(progress) => {
					...
					return this._getVibecodaOniguruma().then((vibecodaOniguruma) => {
						vibecodaOniguruma.setDefaultDebugCall(true);
						progress.report({
				...
				(choice) => {
					this._getVibecodaOniguruma().then((vibecodaOniguruma) => {
						this._debugModePrintFunc = () => { };
						this._debugMode = false;
						vibecodaOniguruma.setDefaultDebugCall(false);
						onStop();
					});
				}
			);
	...
	private async _getOrCreateGrammarFactory(): Promise<TMGrammarFactory> {
		...
		const [vibecodaTextmate, vibecodaOniguruma] = await Promise.all([importAMDNodeModule<typeof import('vibecoda-textmate')>('vibecoda-textmate', 'release/main.js'), this._getVibecodaOniguruma()]);
		const onigLib: Promise<IOnigLib> = Promise.resolve({
			createOnigScanner: (sources: string[]) => vibecodaOniguruma.createOnigScanner(sources),
			createOnigString: (str: string) => vibecodaOniguruma.createOnigString(str)
		});
		...
		}, this._grammarDefinitions || [], vibecodaTextmate, onigLib);
	...
	private _vibecodaOniguruma: Promise<typeof import('vibecoda-oniguruma')> | null;
	private _getVibecodaOniguruma(): Promise<typeof import('vibecoda-oniguruma')> {
		if (!this._vibecodaOniguruma) {
			this._vibecodaOniguruma = (async () => {
				const [vibecodaOniguruma, wasm] = await Promise.all([importAMDNodeModule<typeof import('vibecoda-oniguruma')>('vibecoda-oniguruma', 'release/main.js'), this._loadVibecodaOnigurumaWASM()]);
				await vibecodaOniguruma.loadWASM({
					data: wasm,
					print: (str: string) => {
						this._debugModePrintFunc(str);
					}
				});
				return vibecodaOniguruma;
			})();
		}
		return this._vibecodaOniguruma;
	}

	private async _loadVibecodaOnigurumaWASM(): Promise<Response | ArrayBuffer> {
		if (isWeb) {
			const response = await fetch(resolveAmdNodeModulePath('vibecoda-oniguruma', 'release/onig.wasm'));
			...
			const response = await fetch(canASAR && this._environmentService.isBuilt
				? FileAccess.asBrowserUri(`${nodeModulesAsarUnpackedPath}/vibecoda-oniguruma/release/onig.wasm`).toString(true)
				: FileAccess.asBrowserUri(`${nodeModulesPath}/vibecoda-oniguruma/release/onig.wasm`).toString(true));
			return response;
		}
	}
