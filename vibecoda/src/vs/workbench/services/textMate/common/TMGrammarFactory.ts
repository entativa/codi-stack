...
import type { IGrammar, IOnigLib, IRawTheme, Registry, StateStack } from 'vibecoda-textmate';
...
export class TMGrammarFactory extends Disposable {
	...
	constructor(host: ITMGrammarFactoryHost, grammarDefinitions: IValidGrammarDefinition[], vibecodaTextmate: typeof import('vibecoda-textmate'), onigLib: Promise<IOnigLib>) {
		super();
		this._host = host;
		this._initialState = vibecodaTextmate.INITIAL;
		this._scopeRegistry = new TMScopeRegistry();
		this._injections = {};
		this._injectedEmbeddedLanguages = {};
		this._languageToScope = new Map<string, string>();
		this._grammarRegistry = this._register(new vibecodaTextmate.Registry({
			onigLib: onigLib,
			loadGrammar: async (scopeName: string) => {
				...
				try {
					...
					return vibecodaTextmate.parseRawGrammar(content, location.path);
				} catch (e) {
