...
type NativeCodeWindow = CodeWindow & {
	readonly vibecoda: ISandboxGlobals;
};
...
export class NativeAuxiliaryWindow extends AuxiliaryWindow {
	...
	private handleMaximizedState(): void {
		(async () => {
			this.maximized = await this.nativeHostService.isMaximized({ targetWindowId: this.window.vibecodaWindowId });
		})();

		this._register(this.nativeHostService.onDidMaximizeWindow(windowId => {
			if (windowId === this.window.vibecodaWindowId) {
				this.maximized = true;
			}
		}));
	...
	private handleAlwaysOnTopState(): void {
		(async () => {
			this.alwaysOnTop = await this.nativeHostService.isWindowAlwaysOnTop({ targetWindowId: this.window.vibecodaWindowId });
		})();
	...
	private async handleFullScreenState(): Promise<void> {
		const fullscreen = await this.nativeHostService.isFullScreen({ targetWindowId: this.window.vibecodaWindowId });
		if (fullscreen) {
	...
	protected override async confirmBeforeClose(e: BeforeUnloadEvent): Promise<void> {
		...
		if (confirmed) {
			...
			this.nativeHostService.closeWindow({ targetWindowId: this.window.vibecodaWindowId });
		}
	}
...
export class NativeAuxiliaryWindowService extends BrowserAuxiliaryWindowService {
	...
	protected override async resolveWindowId(auxiliaryWindow: NativeCodeWindow): Promise<number> {
		...
		const windowId = await auxiliaryWindow.vibecoda.ipcRenderer.invoke('vibecoda:registerAuxiliaryWindow', this.nativeHostService.windowId);
