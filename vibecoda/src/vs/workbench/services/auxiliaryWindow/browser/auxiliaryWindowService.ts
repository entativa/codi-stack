...
export class BrowserAuxiliaryWindowService extends Disposable implements IAuxiliaryWindowService {
	...
	async open(options?: IAuxiliaryWindowOpenOptions): Promise<IAuxiliaryWindow> {
		...
		// Add a `vibecodaWindowId` property to identify auxiliary windows
		...
		this.windows.set(targetWindow.vibecodaWindowId, auxiliaryWindow);
		registryDisposables.add(toDisposable(() => this.windows.delete(targetWindow.vibecodaWindowId)));
