...
export const TestEnvironmentService = new TestEnvironmentServiceWithArgs('', URI.file('tests').with({ scheme: 'vibecoda-tests' }), Object.create(null), TestProductService);
...
export class TestEditorGroupsService implements IEditorGroupsService {
	...
	windowId = mainWindow.vibecodaWindowId;
...
export class InMemoryTestWorkingCopyBackupService extends BrowserWorkingCopyBackupService {
	...
	constructor() {
		...
		disposables.add(fileService.registerProvider(Schemas.vibecodaUserData, disposables.add(new InMemoryFileSystemProvider())));
...
export class RemoteFileSystemProvider implements IFileSystemProvider {
	...
	constructor(private readonly wrappedFsp: IFileSystemProvider, private readonly remoteAuthority: string) {
		...
		this.onDidChangeFile = Event.map(this.wrappedFsp.onDidChangeFile, changes => changes.map(c => {
			return {
				type: c.type,
				resource: c.resource.with({ scheme: Schemas.vibecodaRemote, authority: this.remoteAuthority }),
			};
		}));
	}
