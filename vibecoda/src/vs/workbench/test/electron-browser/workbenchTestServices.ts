...
export class TestNativeWorkingCopyBackupService extends NativeWorkingCopyBackupService implements IDisposable {
	...
	constructor() {
		...
		this._register(fileService.registerProvider(Schemas.vibecodaUserData, this._register(new FileUserDataProvider(Schemas.file, inMemoryFileSystemProvider, Schemas.vibecodaUserData, userDataProfilesService, uriIdentityService, logService))));
...
export class TestIPCFileSystemProvider implements IFileSystemProvider {
	...
	async stat(resource: URI): Promise<IStat> {
		...
		const stats = await ipcRenderer.invoke('vibecoda:statFile', resource.fsPath);
		return {
	...
	async readFile(resource: URI): Promise<Uint8Array> {
		const { ipcRenderer } = require('electron');
		const result = await ipcRenderer.invoke('vibecoda:readFile', resource.fsPath);
