...
export class TestLoggerService extends AbstractLoggerService {
	constructor(logsHome?: URI) {
		super(LogLevel.Info, logsHome ?? URI.file('tests').with({ scheme: 'vibecoda-tests' }));
	}
...
export class TestUserDataProfileService implements IUserDataProfileService {
	...
	readonly currentProfile = toUserDataProfile('test', 'test', URI.file('tests').with({ scheme: 'vibecoda-tests' }), URI.file('tests').with({ scheme: 'vibecoda-tests' }));
