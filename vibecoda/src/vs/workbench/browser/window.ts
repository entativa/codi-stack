...
export abstract class BaseWindow extends Disposable {
	...
	constructor(
		...
		this.registerFullScreenListeners(targetWindow.vibecodaWindowId);
	...
	protected enableMultiWindowAwareTimeout(targetWindow: Window, dom = { getWindowsCount, getWindows }): void {
		...
		Object.defineProperty(targetWindow, 'vibecodaOriginalSetTimeout', { get: () => originalSetTimeout });

		const originalClearTimeout = targetWindow.clearTimeout;
		Object.defineProperty(targetWindow, 'vibecodaOriginalClearTimeout', { get: () => originalClearTimeout });
		...
		targetWindow.setTimeout = function (this: unknown, handler: TimerHandler, timeout = 0, ...args: unknown[]): number {
			...
			for (const { window, disposables } of dom.getWindows()) {
				...
				const handle = (window as { vibecodaOriginalSetTimeout?: typeof window.setTimeout }).vibecodaOriginalSetTimeout?.apply(this, [(...args: unknown[]) => {
					if (didClear) {
						return;
					}
					handlerFn(...args);
				...
				const timeoutDisposable = toDisposable(() => {
					didClear = true;
					(window as { vibecodaOriginalClearTimeout?: typeof window.clearTimeout }).vibecodaOriginalClearTimeout?.apply(this, [handle]);
...
export class BrowserWindow extends BaseWindow {
	...
	private setupOpenHandlers(): void {
		...
		// vibecoda itself. We do not want to open these links
					...
					if (isSafari) {
						...
						if (!opened) {
							await this.dialogService.prompt({
								...
								custom: {
									markdownDetails: [{ markdown: new MarkdownString(localize('unableToOpenWindowDetail', "Please allow pop-ups for this website in your [browser settings]({0}).", 'https://aka.ms/allow-vibecoda-popup'), true) }]
								},
								buttons: [
									{
	...
	private registerLabelFormatters(): void {
		this._register(this.labelService.registerFormatter({
			scheme: Schemas.vibecodaUserData,
			priority: true,
			formatting: {
				label: '(Settings) ${path}',
				separator: '/',
