...
export class WindowTitle extends Disposable {
	...
	constructor(
		...
		this.windowId = targetWindow.vibecodaWindowId;
	...
	private doUpdateTitle(): void {
		...
		if (title !== this.title) {
			...
			if (!window.document.title && isMacintosh && nativeTitle === this.productService.nameLong) {
				...
				// See: https://github.com/microsoft/vibecoda/issues/191288
	...
	getWindowTitle(): string {
		...
		if (this.environmentService.remoteAuthority && !isWeb) {
			remoteName = this.labelService.getHostLabel(Schemas.vibecodaRemote, this.environmentService.remoteAuthority);
		} else {
