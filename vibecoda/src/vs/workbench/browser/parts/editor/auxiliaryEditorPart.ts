...
registerAction2(class extends Action2 {
	...
	override async run(): Promise<void> {
		compactWindowEmitter.fire({ windowId: getActiveWindow().vibecodaWindowId, compact: 'toggle' });
	}
});
...
registerAction2(class extends Action2 {
	...
	override async run(): Promise<void> {
		compactWindowEmitter.fire({ windowId: getActiveWindow().vibecodaWindowId, compact: true });
	}
});
...
registerAction2(class extends Action2 {
	...
	override async run(): Promise<void> {
		compactWindowEmitter.fire({ windowId: getActiveWindow().vibecodaWindowId, compact: false });
	}
});
...
export class AuxiliaryEditorPart {
	...
	async create(label: string, options?: IAuxiliaryEditorPartOpenOptions): Promise<ICreateAuxiliaryEditorPartResult> {
		...
		const editorPart = disposables.add(this.instantiationService.createInstance(AuxiliaryEditorPartImpl, auxiliaryWindow.window.vibecodaWindowId, this.editorPartsView, options?.state, label));
		editorPart.updateOptions({ compact });
		...
		if (useCustomTitle) {
			...
			disposables.add(onDidChangeFullscreen(windowId => {
				if (windowId !== auxiliaryWindow.window.vibecodaWindowId) {
		...
		disposables.add(compactWindowEmitter.event(e => {
			if (e.windowId === auxiliaryWindow.window.vibecodaWindowId) {
