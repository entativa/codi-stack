...
export class EditorPart extends Part<IEditorPartMemento> implements IEditorPart, IEditorGroupsView {
		...
		const gridDescriptor = createSerializedGrid({
			orientation: this.toGridViewOrientation(
				...
				this.isTwoDimensionalGrid() ?
					...
					orthogonal(this.gridWidget.orientation) // otherwise flip (fix https://github.com/microsoft/vibecoda/issues/52975)
			),
		...
		for (const editor of sourceView.editors) {
			...
			if (targetView.contains(editor) &&
				(
					...
					// See https://github.com/microsoft/vibecoda/issues/239549
	...
	protected override createContentArea(parent: HTMLElement, options?: IEditorPartCreationOptions): HTMLElement {
		...
		// Container
		this.element = parent;
		if (this.windowId !== mainWindow.vibecodaWindowId) {
	...
	private handleContextKeys(): void {
		...
		isAuxiliaryWindowContext.set(this.windowId !== mainWindow.vibecodaWindowId);
...
export class MainEditorPart extends EditorPart {
	...
	constructor(
		...
		super(editorPartsView, Parts.EDITOR_PART, '', mainWindow.vibecodaWindowId, instantiationService, themeService, configurationService, storageService, layoutService, hostService, contextKeyService);
	}
}
