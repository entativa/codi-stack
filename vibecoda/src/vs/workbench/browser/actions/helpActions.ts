...
class OpenNewsletterSignupUrlAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('newsletterSignup', 'Signup for the Vibecoda Newsletter'),
			category: Categories.Help,
			f1: true
		});
	}
...
class AskVibecodaCopilot extends Action2 {
	static readonly ID = 'workbench.action.askVScode';

	constructor() {
		super({
			id: AskVibecodaCopilot.ID,
			title: localize2('askVScode', 'Ask @vibecoda'),
			category: Categories.Help,
			f1: true,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('workbench.action.chat.open', { mode: 'ask', query: '@vibecoda ', isPartialQuery: true });
	}
}
...
registerAction2(AskVibecodaCopilot);
