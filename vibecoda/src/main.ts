...
const userDataPath = getUserDataPath(args, product.nameShort ?? 'vibecoda-dev');
...
// (https://github.com/microsoft/vibecoda/issues/56651)
...
protocol.registerSchemesAsPrivileged([
	{
		scheme: 'vibecoda-webview',
		privileges: { standard: true, secure: true, supportFetchAPI: true, corsEnabled: true, allowServiceWorkers: true, codeCache: true }
	},
	{
		scheme: 'vibecoda-file',
...
// https://github.com/microsoft/vibecoda/issues/167543.
...
function configureCommandlineSwitchesSync(cliArgs: NativeParsedArgs) {
	...
	const SUPPORTED_MAIN_PROCESS_SWITCHES = [
		...
		// Persistently enable proposed api via argv.json: https://github.com/microsoft/vibecoda/issues/99775
		'enable-proposed-api',
	...
	// `NetAdapterMaxBufSizeFeature` - Specify the max buffer size for NetToMojoPendingBuffer, refs https://github.com/microsoft/vibecoda/issues/268800
	...
	// `FontationsLinuxSystemFonts` - Revert to FreeType for system fonts on Linux Refs https://github.com/microsoft/vibecoda/issues/260391
	...
	// `FontMatchingCTMigration` - Siwtch font matching on macOS to Appkit (Refs https://github.com/microsoft/vibecoda/issues/224496#issuecomment-2270418470).
	// `StandardizedBrowserZoom` - Disable zoom adjustment for bounding box (https://github.com/microsoft/vibecoda/issues/232750#issuecomment-2459495394)
	...
	// to address https://github.com/microsoft/vibecoda/issues/213780
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to Vibecoda.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of Vibecoda.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in Vibecoda.',
...
function getArgvConfigPath(): string {
	const vibecodaPortable = process.env['VSCODE_PORTABLE'];
	if (vibecodaPortable) {
		return path.join(vibecodaPortable, 'argv.json');
	}
	...
	// Otherwise we configure the crash reporter from product.json
		...
		if (appCenter) {
			...
			if (crashReporterId && uuidPattern.test(crashReporterId)) {
				...
				// For vibecoda this is ExtensionHost process currently.
...
function registerListeners(): void {
	...
	/**
	 * macOS: when someone drops a file to the not-yet running Vibecoda, the open-file event fires even before
...
async function resolveNlsConfiguration(): Promise<INLSConfiguration> {
	...
	if (!userLocale) {
		return {
			...
			// NLS: below 2 are a relic from old times only used by vibecoda-nls and deprecated
			locale: 'en',
			availableLanguages: {}
		};
	}
