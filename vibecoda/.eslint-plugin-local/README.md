...
- Enforcing conventions in `vibecoda.d.ts`
...
- `vibecoda-dts-RULE-NAME.ts` — Rules that apply just to `vibecoda.d.ts`.
...
For example, here's a configuration that enables the no `test.only` rule in all `*.test.ts` files in the Vibecoda repo:
