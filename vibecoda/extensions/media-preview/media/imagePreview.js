...
(function () {
	...
	const initialState = vibecoda.getState() || { scale: 'fit', offsetX: 0, offsetY: 0 };
	...
	function updateScale(newScale) {
		...
		if (newScale === 'fit') {
			...
			vibecoda.setState({ scale: scale, offsetX: newScrollX, offsetY: newScrollY });
		}
	...
	window.addEventListener('scroll', e => {
		...
		const entry = vibecoda.getState();
		if (entry) {
			vibecoda.setState({ scale: entry.scale, offsetX: window.scrollX, offsetY: window.scrollY });
		}
	}, { passive: true });
