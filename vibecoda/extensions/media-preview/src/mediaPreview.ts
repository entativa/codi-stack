...
export async function reopenAsText(resource: vibecoda.Uri, viewColumn: vibecoda.ViewColumn | undefined): Promise<void> {
	await vibecoda.commands.executeCommand('vibecoda.openWith', resource, 'default', viewColumn);
}
...
export abstract class MediaPreview extends Disposable {
	...
	constructor(
		extensionRoot: vibecoda.Uri,
		protected readonly _resource: vibecoda.Uri,
		protected readonly _webviewEditor: vibecoda.WebviewPanel,
		...
		const watcher = this._register(vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(_resource, '*')));
	...
	protected updateBinarySize() {
		vibecoda.workspace.fs.stat(this._resource).then(({ size }) => {
