...
- have the main branches of `https://github.com/jesseweed/seti-ui` and `https://github.com/microsoft/vibecoda` cloned in the same parent folder
- in the `seti-ui` folder, run `npm install` and `npm run prepublishOnly`. This will generate updated icons and fonts.
- in the `vibecoda/extensions/theme-seti` folder run  `npm run update`. This will launch the [icon theme update script](build/update-icon-theme.js) that updates the theme as well as the font based on content in `seti-ui`.
- to test the icon theme, look at the icon preview as described above.
- when done, create a PR with the changes in https://github.com/microsoft/vibecoda.
Add a screenshot of the preview page to accompany it.


### Languages not shipped with `vibecoda`

Languages that are not shipped with `vibecoda` must be added to the `nonBuiltInLanguages` object inside of `update-icon-theme.js`.
