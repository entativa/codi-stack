 ...
 */
const viewType = 'vibecoda.chatMermaidDiagram';
...
export function activate(context: vibecoda.ExtensionContext) {
	...
	context.subscriptions.push(
		vibecoda.lm.registerTool<{ markup: string }>('renderMermaidDiagram', {
	...
	context.subscriptions.push(
		vibecoda.chat.registerChatOutputRenderer(viewType, {
			async renderChatOutput({ value }, webview, _ctx, _token) {
				...
				const mediaRoot = vibecoda.Uri.joinPath(context.extensionUri, 'chat-webview-out');
				webview.options = {
					enableScripts: true,
				...
				const mermaidScript = vibecoda.Uri.joinPath(mediaRoot, 'index.js');
...
function writeMermaidToolOutput(sourceCode: string): vibecoda.LanguageModelToolResult {
	// Expose the source code as a tool result for the LM
	const result = new vibecoda.LanguageModelToolResult([
		new vibecoda.LanguageModelTextPart(sourceCode)
	]);
	...
	(result as vibecoda.ExtendedLanguageModelToolResult2).toolResultDetails2 = {
		mime,
