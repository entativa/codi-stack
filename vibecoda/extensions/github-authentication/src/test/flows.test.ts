...
suite('getFlows', () => {
	...
	const testCases: Array<{ label: string; query: IFlowQuery; expectedFlows: Flows[] }> = [
		{
			label: 'Vibecoda Desktop. Local filesystem. GitHub.com',
			query: {
		...
		{
			label: 'Vibecoda Desktop. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'Vibecoda Desktop. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'vibecoda.dev. serverful. GitHub.com',
			query: {
		...
		{
			label: 'vibecoda.dev. serverful. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Remote,
				isSupportedClient: true,
				target: GitHubTarget.HostedEnterprise
		...
		{
			label: 'vibecoda.dev. serverful. GitHub Enterprise',
			query: {
				extensionHost: ExtensionHost.Remote,
				isSupportedClient: true,
				target: GitHubTarget.Enterprise
		...
		{
			label: 'vibecoda.dev. serverless. GitHub.com',
			query: {
		...
		{
			label: 'vibecoda.dev. serverless. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.WebWorker,
				isSupportedClient: true,
				target: GitHubTarget.HostedEnterprise
		...
		{
			label: 'vibecoda.dev. serverless. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.WebWorker,
				isSupportedClient: true,
				target: GitHubTarget.Enterprise
		...
		{
			label: 'Vibecoda. Local filesystem. GitHub.com',
			query: {
				extensionHost: ExtensionHost.Local,
		...
		{
			label: 'Vibecoda. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: false,
				target: GitHubTarget.HostedEnterprise
		...
		{
			label: 'Vibecoda. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: false,
				target: GitHubTarget.Enterprise
	...
	suite('preferDeviceCodeFlow configuration', () => {
		...
		suiteSetup(async () => {
			const config = vibecoda.workspace.getConfiguration('github-authentication');
			originalConfig = config.get<boolean>('preferDeviceCodeFlow');
		});

		suiteTeardown(async () => {
			const config = vibecoda.workspace.getConfiguration('github-authentication');
			await config.update('preferDeviceCodeFlow', originalConfig, vibecoda.ConfigurationTarget.Global);
		});

		test('returns device code flow first when preferDeviceCodeFlow is true - Vibecoda Desktop', async () => {
			const config = vibecoda.workspace.getConfiguration('github-authentication');
			await config.update('preferDeviceCodeFlow', true, vibecoda.ConfigurationTarget.Global);
