...
interface GitHubAuthenticationProviderOptions extends vibecoda.AuthenticationProviderSessionOptions {
	/**
...
export class UriEventHandler extends vibecoda.EventEmitter<vibecoda.Uri> implements vibecoda.UriHandler {
	...
	private readonly _codeExchangePromises = new Map<string, { promise: Promise<string>; cancel: vibecoda.EventEmitter<void> }>();

	public handleUri(uri: vibecoda.Uri) {
		this.fire(uri);
	}

	public async waitForCode(logger: Log, scopes: string, nonce: string, token: vibecoda.CancellationToken) {
	...
	private handleEvent: (logger: Log, scopes: string) => PromiseAdapter<vibecoda.Uri, string> =
...
export class GitHubAuthenticationProvider implements vibecoda.AuthenticationProvider, vibecoda.Disposable {
	private readonly _sessionChangeEmitter = new vibecoda.EventEmitter<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent>();
	...
	private readonly _disposable: vibecoda.Disposable | undefined;

	private _sessionsPromise: Promise<vibecoda.AuthenticationSession[]>;

	constructor(
		private readonly context: vibecoda.ExtensionContext,
		uriHandler: UriEventHandler,
		ghesUri?: vibecoda.Uri
	) {
		...
		const supportedAuthorizationServers = ghesUri
			? [vibecoda.Uri.joinPath(ghesUri, '/login/oauth')]
			: [vibecoda.Uri.parse('https://github.com/login/oauth')];
		this._disposable = vibecoda.Disposable.from(
			this._telemetryReporter,
			vibecoda.authentication.registerAuthenticationProvider(
				type,
	...
	async getSessions(scopes: string[] | undefined, options?: vibecoda.AuthenticationProviderSessionOptions): Promise<vibecoda.AuthenticationSession[]> {
	...
	private async afterSessionLoad(session: vibecoda.AuthenticationSession): Promise<void> {
	...
	private async readSessions(): Promise<vibecoda.AuthenticationSession[]> {
		...
		const sessionPromises = sessionData.map(async (session: SessionData): Promise<vibecoda.AuthenticationSession | undefined> => {
			// For GitHub scope list, order doesn't matter so we immediately sort the scopes
		...
		const verifiedSessions = (await Promise.allSettled(sessionPromises))
			...
			.map(p => (p as PromiseFulfilledResult<vibecoda.AuthenticationSession | undefined>).value)
	...
	private async storeSessions(sessions: vibecoda.AuthenticationSession[]): Promise<void> {
	...
	public async createSession(scopes: string[], options?: GitHubAuthenticationProviderOptions): Promise<vibecoda.AuthenticationSession> {
		try {
			...
			*/
			...
			const removed = new Array<vibecoda.AuthenticationSession>();
			if (sessionIndex > -1) {
			...
			*/
			...
			vibecoda.window.showErrorMessage(vibecoda.l10n.t('Sign in failed: {0}', `${e}`));
			this._logger.error(e);
			throw e;
		}
	}

	private async tokenToSession(token: string, scopes: string[]): Promise<vibecoda.AuthenticationSession> {
	...
	public async removeSession(id: string) {
		try {
			...
			*/
			...
			vibecoda.window.showErrorMessage(vibecoda.l10n.t('Sign out failed: {0}', `${e}`));
			this._logger.error(e);
			throw e;
		}
	}
