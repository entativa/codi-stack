...
const REDIRECT_URL_STABLE = 'https://vibecoda.dev/redirect';
const REDIRECT_URL_INSIDERS = 'https://insiders.vibecoda.dev/redirect';
...
export interface IGitHubServer {
	...
	logout(session: vibecoda.AuthenticationSession): Promise<void>;
	getUserInfo(token: string): Promise<{ id: string; accountName: string }>;
	sendAdditionalTelemetryInfo(session: vibecoda.AuthenticationSession): Promise<void>;
	friendlyName: string;
}
...
export class GitHubServer implements IGitHubServer {
	...
	constructor(
		...
		private readonly _extensionKind: vibecoda.ExtensionKind,
		private readonly _ghesUri?: vibecoda.Uri
	) {
	...
	get baseUri() {
		if (this._type === AuthProviderType.github) {
			return vibecoda.Uri.parse('https://github.com/');
		}
	...
	private async getRedirectEndpoint(): Promise<string> {
		...
		if (this._type === AuthProviderType.github) {
			const proxyEndpoints = await vibecoda.commands.executeCommand<{ [providerId: string]: string } | undefined>('workbench.getCodeExchangeProxyEndpoints');
			// If we are running in insiders vibecoda.dev, then ensure we use the redirect route on that.
			this._redirectEndpoint = REDIRECT_URL_STABLE;
			if (proxyEndpoints?.github && new URL(proxyEndpoints.github).hostname === 'insiders.vibecoda.dev') {
				this._redirectEndpoint = REDIRECT_URL_INSIDERS;
			}
		} else {
			// GHE only supports a single redirect endpoint, so we can't use
			// insiders.vibecoda.dev/redirect when we're running in Insiders, unfortunately.
			...
			this._redirectEndpoint = 'https://vibecoda.dev/redirect';
		}
		return this._redirectEndpoint;
	}
	...
	private async isNoCorsEnvironment(): Promise<boolean> {
		...
		const uri = await vibecoda.env.asExternalUri(vibecoda.Uri.parse(`${vibecoda.env.uriScheme}://vibecoda.github-authentication/dummy`));
		this._isNoCorsEnvironment = (uri.scheme === 'https' && /^((insiders\.)?vibecoda|github)\./.test(uri.authority)) || (uri.scheme === 'http' && /^localhost/.test(uri.authority));
		return this._isNoCorsEnvironment;
	}
	...
	public async login(scopes: string, signInProvider?: GitHubSocialSignInProvider, extraAuthorizeParameters?: Record<string, string>, existingLogin?: string): Promise<string> {
		...
		const promptToContinue = async (mode: string) => {
			...
			const message = userCancelled
				? vibecoda.l10n.t('Having trouble logging in? Would you like to try a different way? ({0})', mode)
				: vibecoda.l10n.t('You have not yet finished authorizing this extension to use GitHub. Would you like to try a different way? ({0})', mode);
			const result = await vibecoda.window.showWarningMessage(message, yes, no);
			if (result !== yes) {
		...
		const callbackUri = await vibecoda.env.asExternalUri(vibecoda.Uri.parse(`${vibecoda.env.uriScheme}://vibecoda.github-authentication/did-authenticate?nonce=${encodeURIComponent(nonce)}`));
		...
		const flows = getFlows({
			...
			extensionHost: isNodeEnvironment
				? this._extensionKind === vibecoda.ExtensionKind.UI ? ExtensionHost.Local : ExtensionHost.Remote
		...
		for (const flow of flows) {
			try {
				...
				return await flow.trigger({
					...
					redirectUri: vibecoda.Uri.parse(await this.getRedirectEndpoint()),
					existingLogin
				});
			} catch (e) {
	...
	public async logout(session: vibecoda.AuthenticationSession): Promise<void> {
		...
		try {
			...
			const result = await fetching(uri.toString(true), {
				...
				headers: {
					...
					'User-Agent': `${vibecoda.env.appName} (${vibecoda.env.appHost})`
				},
	...
	private getServerUri(path: string = '') {
		...
		if (isSupportedTarget(this._type, this._ghesUri)) {
			return vibecoda.Uri.parse(`${apiUri.scheme}://api.${apiUri.authority}`).with({ path });
		}
		// GitHub Enterprise Server (aka on-prem)
		return vibecoda.Uri.parse(`${apiUri.scheme}://${apiUri.authority}/api/v3${path}`);
	}
	...
	public async sendAdditionalTelemetryInfo(session: vibecoda.AuthenticationSession): Promise<void> {
		if (!vibecoda.env.isTelemetryEnabled) {
			return;
		}
	...
	private async checkUserDetails(session: vibecoda.AuthenticationSession): Promise<void> {
