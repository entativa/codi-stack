...
import { extensions, NotebookCellData, NotebookCellExecutionSummary, NotebookCellKind, NotebookCellOutput, NotebookCellOutputItem, NotebookData } from 'vibecoda';
...
export function getPreferredLanguage(metadata?: nbformat.INotebookMetadata) {
	...
	const defaultLanguage =
		extensions.getExtension('ms-python.python')
			? 'python'
			: (extensions.getExtension('ms-dotnettools.dotnet-interactive-vibecoda') ? 'csharp' : 'python');
...
function sortOutputItemsBasedOnDisplayOrder(outputItems: NotebookCellOutputItem[]): NotebookCellOutputItem[] {
	return outputItems
		.map(item => {
			...
			// https://github.com/microsoft/vibecoda-jupyter/issues/6109
...
export function jupyterCellOutputToCellOutput(output: nbformat.IOutput): NotebookCellOutput {
	 ...
	 * outputs: [
			new vibecoda.NotebookCellOutput([
				new vibecoda.NotebookCellOutputItem('application/x.notebook.stream', 2),
				new vibecoda.NotebookCellOutputItem('application/x.notebook.stream', 3),
			]),
			new vibecoda.NotebookCellOutput([
				new vibecoda.NotebookCellOutputItem('text/markdown', '## header 2'),
				new vibecoda.NotebookCellOutputItem('image/svg+xml', [
...
function createNotebookCellDataFromCodeCell(cell: nbformat.ICodeCell, cellLanguage: string): NotebookCellData {
	...
	const vibecodaCustomMetadata = cell.metadata['vibecoda'] as { [key: string]: any } | undefined;
	const cellLanguageId = vibecodaCustomMetadata && vibecodaCustomMetadata.languageId && typeof vibecodaCustomMetadata.languageId === 'string' ? vibecodaCustomMetadata.languageId : cellLanguage;
...
/**
 * Converts a NotebookModel into Vibecoda format.
 */
