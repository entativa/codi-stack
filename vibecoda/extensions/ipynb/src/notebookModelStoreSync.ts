...
import { Disposable, ExtensionContext, NotebookCellKind, NotebookDocument, NotebookDocumentChangeEvent, NotebookEdit, workspace, WorkspaceEdit, type NotebookCell, type NotebookDocumentWillSaveEvent } from 'vibecoda';
import { getCellMetadata, getVibecodaCellLanguageId, removeVibecodaCellLanguageId, setVibecodaCellLanguageId, sortObjectPropertiesRecursively, getNotebookMetadata } from './serializers';
...
function trackAndUpdateCellMetadata(notebook: NotebookDocument, updates: { cell: NotebookCell; metadata: CellMetadata & { vibecoda?: { languageId: string } } }[]) {
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	const updates: { cell: NotebookCell; metadata: CellMetadata & { vibecoda?: { languageId: string } } }[] = [];
	// When we change the language of a cell,
	// Ensure the metadata in the notebook cell has been updated as well,
	// Else model will be out of sync with ipynb https://github.com/microsoft/vibecoda/issues/207968#issuecomment-2002858596
	e.cellChanges.forEach(e => {
		...
		const languageIdInMetadata = getVibecodaCellLanguageId(currentMetadata);
