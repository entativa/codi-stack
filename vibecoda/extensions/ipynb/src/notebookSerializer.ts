...
export abstract class NotebookSerializerBase extends vibecoda.Disposable implements vibecoda.NotebookSerializer {
	protected disposed: boolean = false;
	constructor(protected readonly context: vibecoda.ExtensionContext) {
		super(() => { });
	}
	...
	public async deserializeNotebook(content: Uint8Array, _token: vibecoda.CancellationToken): Promise<vibecoda.NotebookData> {
		let contents = '';
		try {
		...
		if (json.__webview_backup) {
			...
			const folder = uri.with({ path: this.context.globalStorageUri.path.replace('vibecoda.ipynb', 'ms-toolsai.jupyter') });
			const fileHash = fnv.fast1a32hex(backupId) as string;
			const fileName = `${fileHash}.ipynb`;
			const file = vibecoda.Uri.joinPath(folder, fileName);
			const data = await vibecoda.workspace.fs.readFile(file);
	...
	public async serializeNotebook(data: vibecoda.NotebookData, _token: vibecoda.CancellationToken): Promise<Uint8Array> {
		if (this.disposed) {
