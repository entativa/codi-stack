...
export class NotebookSerializer extends NotebookSerializerBase {
	private experimentalSave = vibecoda.workspace.getConfiguration('ipynb').get('experimental.serialization', true);
	private worker?: import('node:worker_threads').Worker;
	private tasks = new Map<string, DeferredPromise<Uint8Array>>();

	constructor(context: vibecoda.ExtensionContext) {
		super(context);
		context.subscriptions.push(vibecoda.workspace.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('ipynb.experimental.serialization')) {
				this.experimentalSave = vibecoda.workspace.getConfiguration('ipynb').get('experimental.serialization', true);
			}
		}));
	}
	...
	public override async serializeNotebook(data: vibecoda.NotebookData, token: vibecoda.CancellationToken): Promise<Uint8Array> {
		if (this.disposed) {
	...
	private async startWorker() {
		...
		this.worker = new Worker(vibecoda.Uri.joinPath(this.context.extensionUri, outputDir, 'notebookSerializerWorker.js').fsPath, {});
	...
	private async serializeViaWorker(data: vibecoda.NotebookData): Promise<Uint8Array> {
...
function getOutputDir(context: vibecoda.ExtensionContext): string {
