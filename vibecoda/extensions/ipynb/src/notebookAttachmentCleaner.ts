...
interface AttachmentCleanRequest {
	notebook: vibecoda.NotebookDocument;
	document: vibecoda.TextDocument;
	cell: vibecoda.NotebookCell;
}
...
export class AttachmentCleaner implements vibecoda.CodeActionProvider {
	...
	private _imageDiagnosticCollection: vibecoda.DiagnosticCollection;
	private readonly _delayer = new Delayer(750);

	constructor() {
		this._disposables = [];
		this._imageDiagnosticCollection = vibecoda.languages.createDiagnosticCollection('Notebook Image Attachment');
		this._disposables.push(this._imageDiagnosticCollection);

		this._disposables.push(vibecoda.commands.registerCommand(ATTACHMENT_CLEANUP_COMMANDID, async (document: vibecoda.Uri, range: vibecoda.Range) => {
		...
		this._disposables.push(vibecoda.languages.registerCodeActionsProvider(JUPYTER_NOTEBOOK_MARKDOWN_SELECTOR, this, {
			providedCodeActionKinds: [
				vibecoda.CodeActionKind.QuickFix
			],
		}));

		this._disposables.push(vibecoda.workspace.onDidChangeNotebookDocument(e => {
			this._delayer.trigger(() => {
				...
				e.cellChanges.forEach(change => {
					...
					if (change.cell.kind !== vibecoda.NotebookCellKind.Markup) {
						return;
					}
					...
					if (metadataEdit) {
						...
						vibecoda.workspace.applyEdit(workspaceEdit);
					}
				});
			});
		}));
		...
		this._disposables.push(vibecoda.workspace.onWillSaveNotebookDocument(e => {
			if (e.reason === vibecoda.TextDocumentSaveReason.Manual) {
				this._delayer.dispose();
				if (e.notebook.getCells().length === 0) {
					return;
				}
				const notebookEdits: vibecoda.NotebookEdit[] = [];
				for (const cell of e.notebook.getCells()) {
					if (cell.kind !== vibecoda.NotebookCellKind.Markup) {
						continue;
					}
		...
		this._disposables.push(vibecoda.workspace.onDidCloseNotebookDocument(e => {
			this._attachmentCache.delete(e.uri.toString());
		}));

		this._disposables.push(vibecoda.workspace.onWillRenameFiles(e => {
			const re = /\.ipynb$/;
		...
		this._disposables.push(vibecoda.workspace.onDidOpenTextDocument(e => {
			this.analyzeMissingAttachments(e);
		}));

		this._disposables.push(vibecoda.workspace.onDidCloseTextDocument(e => {
			this.analyzeMissingAttachments(e);
		}));

		vibecoda.workspace.textDocuments.forEach(document => {
	 ...
	 * @returns vibecoda.NotebookEdit, the metadata alteration performed on the json behind the ipynb
	 */
	private cleanNotebookAttachments(e: AttachmentCleanRequest): vibecoda.NotebookEdit | undefined {
		...
		if (cell.index > -1 && !objectEquals(markdownAttachmentsInUse || {}, cell.metadata.attachments || {})) {
			...
			const metadataEdit = vibecoda.NotebookEdit.updateCellMetadata(cell.index, updateMetadata);
			return metadataEdit;
		}
		return;
	}

	private analyzeMissingAttachments(document: vibecoda.TextDocument): void {
		if (document.uri.scheme !== 'vibecoda-notebook-cell') {
			// not notebook
			return;
		}
		...
		let activeCell: vibecoda.NotebookCell | undefined;
		for (const notebookDocument of vibecoda.workspace.notebookDocuments) {
	...
	private updateDiagnostics(cellUri: vibecoda.Uri, diagnostics: IAttachmentDiagnostic[]) {
		const vibecodaDiagnostics: vibecoda.Diagnostic[] = [];
		for (const currDiagnostic of diagnostics) {
			currDiagnostic.ranges.forEach(range => {
				const diagnostic = new vibecoda.Diagnostic(range, `The image named: '${currDiagnostic.name}' is not present in cell metadata.`, vibecoda.DiagnosticSeverity.Warning);
				diagnostic.code = DiagnosticCode.missing_attachment;
				vibecodaDiagnostics.push(diagnostic);
			});
		}

		this._imageDiagnosticCollection.set(cellUri, vibecodaDiagnostics);
	}
	 ...
	 */
	private getAttachmentNames(document: vibecoda.TextDocument) {
		const source = document.getText();
		const filenames: Map<string, { valid: boolean; ranges: vibecoda.Range[] }> = new Map();
		...
		while ((match = re.exec(source))) {
			if (match.groups?.filename) {
				...
				const range = new vibecoda.Range(startPosition, endPosition);
