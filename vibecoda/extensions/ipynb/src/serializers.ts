...
import type { NotebookCell, NotebookCellData, NotebookCellOutput, NotebookData, NotebookDocument } from 'vibecoda';
 ...
 * https://github.com/microsoft/vibecoda-python/issues/13155
 */
...
export function getVibecodaCellLanguageId(metadata: CellMetadata): string | undefined {
	return metadata.metadata?.vibecoda?.languageId;
}
export function setVibecodaCellLanguageId(metadata: CellMetadata, languageId: string) {
	metadata.metadata = metadata.metadata || {};
	metadata.metadata.vibecoda = { languageId };
}
export function removeVibecodaCellLanguageId(metadata: CellMetadata) {
	if (metadata.metadata?.vibecoda) {
		delete metadata.metadata.vibecoda;
	}
}
...
function createCodeCellFromNotebookCell(cell: NotebookCellData, preferredLanguage: string | undefined): nbformat.ICodeCell {
	...
	if (cell.languageId !== preferredLanguage) {
		setVibecodaCellLanguageId(cellMetadata, cell.languageId);
	} else {
		// cell current language is the same as the preferred cell language in the document, flush the vibecoda custom language id metadata
		removeVibecodaCellLanguageId(cellMetadata);
	}
...
function translateCellErrorOutput(output: NotebookCellOutput): nbformat.IError {
	...
	return {
		...
		// Vibecoda needs an `Error` object which requires a `stack` property as a string.
...
function convertOutputMimeToJupyterOutput(mime: string, value: Uint8Array) {
	...
	try {
		if (mime === CellOutputMimeTypes.error) {
			...
			// Vibecoda expects bytes when rendering images.
