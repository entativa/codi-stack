...
function getImageMimeType(uri: vibecoda.Uri): string | undefined {
	return imageExtToMime.get(extname(uri.fsPath).toLowerCase());
}

class DropOrPasteEditProvider implements vibecoda.DocumentPasteEditProvider, vibecoda.DocumentDropEditProvider {

	public static readonly kind = vibecoda.DocumentDropOrPasteEditKind.Empty.append('markdown', 'link', 'image', 'attachment');

	async provideDocumentPasteEdits(
		document: vibecoda.TextDocument,
		_ranges: readonly vibecoda.Range[],
		...
		const enabled = vibecoda.workspace.getConfiguration('ipynb', document).get('pasteImagesAsAttachments.enabled', true);
		if (!enabled) {
			return;
		}
		...
		const pasteEdit = new vibecoda.DocumentPasteEdit(insert.insertText, vibecoda.l10n.t('Insert Image as Attachment'), DropOrPasteEditProvider.kind);
	...
	async provideDocumentDropEdits(
		...
		const dropEdit = new vibecoda.DocumentDropEdit(insert.insertText);
		dropEdit.yieldTo = [vibecoda.DocumentDropOrPasteEditKind.Text];
		dropEdit.additionalEdit = insert.additionalEdit;
		dropEdit.title = vibecoda.l10n.t('Insert Image as Attachment');
		return dropEdit;
	}
	...
	private async createInsertImageAttachmentEdit(
		...
		dataTransfer: vibecoda.DataTransfer,
		token: vibecoda.CancellationToken,
	): Promise<{ insertText: vibecoda.SnippetString; additionalEdit: vibecoda.WorkspaceEdit } | undefined> {
		...
		// build edits
		const additionalEdit = new vibecoda.WorkspaceEdit();
		const nbEdit = vibecoda.NotebookEdit.updateCellMetadata(currentCell.index, newAttachment.metadata);
		const notebookUri = currentCell.notebook.uri;
		additionalEdit.set(notebookUri, [nbEdit]);

		// create a snippet for paste
		const insertText = new vibecoda.SnippetString();
...
async function getDroppedImageData(
	...
	if (urlList) {
		...
		for (const resource of urlList.split(/\r?\n/g)) {
			try {
				uris.push(vibecoda.Uri.parse(resource));
			} catch {
				// noop
			}
		}
		...
		const entries = await Promise.all(uris.map(async (uri) => {
			...
			const data = await vibecoda.workspace.fs.readFile(uri);
...
function getCellFromCellDocument(cellDocument: vibecoda.TextDocument): vibecoda.NotebookCell | undefined {
...
/**
 *  Taken from https://github.com/microsoft/vibecoda/blob/743b016722db90df977feecde0a4b3b4f58c2a4c/src/vs/base/common/buffer.ts#L350-L387
 */
...
export function notebookImagePasteSetup(): vibecoda.Disposable {
	const provider = new DropOrPasteEditProvider();
	return vibecoda.Disposable.from(
		vibecoda.languages.registerDocumentPasteEditProvider(JUPYTER_NOTEBOOK_MARKDOWN_SELECTOR, provider, {
		...
		vibecoda.languages.registerDocumentDropEditProvider(JUPYTER_NOTEBOOK_MARKDOWN_SELECTOR, provider, {
