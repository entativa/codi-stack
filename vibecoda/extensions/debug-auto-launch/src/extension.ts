...
const TEXT_STATUSBAR_LABEL = {
	[State.Disabled]: vibecoda.l10n.t('Auto Attach: Disabled'),
	[State.Always]: vibecoda.l10n.t('Auto Attach: Always'),
	[State.Smart]: vibecoda.l10n.t('Auto Attach: Smart'),
	[State.OnlyWithFlag]: vibecoda.l10n.t('Auto Attach: With Flag'),
};

const TEXT_STATE_LABEL = {
	[State.Disabled]: vibecoda.l10n.t('Disabled'),
	[State.Always]: vibecoda.l10n.t('Always'),
	[State.Smart]: vibecoda.l10n.t('Smart'),
	[State.OnlyWithFlag]: vibecoda.l10n.t('Only With Flag'),
};
const TEXT_STATE_DESCRIPTION = {
	[State.Disabled]: vibecoda.l10n.t('Auto attach is disabled and not shown in status bar'),
	[State.Always]: vibecoda.l10n.t('Auto attach to every Node.js process launched in the terminal'),
	[State.Smart]: vibecoda.l10n.t("Auto attach when running scripts that aren't in a node_modules folder"),
	[State.OnlyWithFlag]: vibecoda.l10n.t('Only auto attach when the `--inspect` flag is given')
};

const TEXT_TOGGLE_TITLE = vibecoda.l10n.t('Toggle Auto Attach');
const TEXT_TOGGLE_WORKSPACE = vibecoda.l10n.t('Toggle auto attach in this workspace');
const TEXT_TOGGLE_GLOBAL = vibecoda.l10n.t('Toggle auto attach on this machine');
const TEXT_TEMP_DISABLE = vibecoda.l10n.t('Temporarily disable auto attach in this session');
const TEXT_TEMP_ENABLE = vibecoda.l10n.t('Re-enable auto attach');
const TEXT_TEMP_DISABLE_LABEL = vibecoda.l10n.t('Auto Attach: Disabled');
...
let currentState: Promise<{ context: vibecoda.ExtensionContext; state: State | null }>;
let statusItem: vibecoda.StatusBarItem | undefined; // and there is no status bar item
...
export function activate(context: vibecoda.ExtensionContext): void {
	...
	context.subscriptions.push(
		vibecoda.commands.registerCommand(TOGGLE_COMMAND, toggleAutoAttachSetting.bind(null, context)),
	);
...
function getDefaultScope(info: ReturnType<vibecoda.WorkspaceConfiguration['inspect']>) {
	if (!info) {
...
type PickResult = { state: State } | { setTempDisabled: boolean } | { scope: vibecoda.ConfigurationTarget } | undefined;
type PickItem = vibecoda.QuickPickItem & ({ state: State } | { setTempDisabled: boolean });

async function toggleAutoAttachSetting(context: vibecoda.ExtensionContext, scope?: vibecoda.ConfigurationTarget): Promise<void> {
	const section = vibecoda.workspace.getConfiguration(SETTING_SECTION);
	scope = scope || getDefaultScope(section.inspect(SETTING_STATE));

	const isGlobalScope = scope === vibecoda.ConfigurationTarget.Global;
	const quickPick = vibecoda.window.createQuickPick<PickItem>();
	...
	quickPick.buttons = [
		{
			iconPath: new vibecoda.ThemeIcon(isGlobalScope ? 'folder' : 'globe'),
	...
	let result = await new Promise<PickResult>(resolve => {
		...
		quickPick.onDidTriggerButton(() => {
			resolve({
				scope: isGlobalScope
					? vibecoda.ConfigurationTarget.Workspace
					: vibecoda.ConfigurationTarget.Global,
			});
		});
	});
...
async function clearJsDebugAttachState(context: vibecoda.ExtensionContext) {
	if (server || await context.workspaceState.get(STORAGE_IPC)) {
		await context.workspaceState.update(STORAGE_IPC, undefined);
		await vibecoda.commands.executeCommand('extension.js-debug.clearAutoAttachVariables');
		await destroyAttachServer();
	}
}
 ...
 * if it's successful.
 */
async function createAttachServer(context: vibecoda.ExtensionContext) {
	...
	new Promise<Server>((resolve, reject) => {
		const s = createServer(socket => {
			...
			socket.on('data', async chunk => {
				...
				try {
					await vibecoda.commands.executeCommand(
 ...
 */
const transitions: { [S in State]: (context: vibecoda.ExtensionContext) => Promise<void> } = {
 ...
 */
function updateStatusBar(context: vibecoda.ExtensionContext, state: State, busy = false) {
	...
	if (!statusItem) {
		statusItem = vibecoda.window.createStatusBarItem('status.debug.autoAttach', vibecoda.StatusBarAlignment.Left);
		statusItem.name = vibecoda.l10n.t("Debug Auto Attach");
		statusItem.command = TOGGLE_COMMAND;
		statusItem.tooltip = vibecoda.l10n.t("Automatically attach to node.js processes in debug mode");
 ...
 */
async function getIpcAddress(context: vibecoda.ExtensionContext) {
	...
	const jsDebugPath =
		vibecoda.extensions.getExtension('ms-vibecoda.js-debug-nightly')?.extensionPath ||
		vibecoda.extensions.getExtension('ms-vibecoda.js-debug')?.extensionPath;
	...
	const result = await vibecoda.commands.executeCommand<{ ipcAddress: string }>(
...
function getJsDebugSettingKey() {
	...
	const config = vibecoda.workspace.getConfiguration(SETTING_SECTION);
