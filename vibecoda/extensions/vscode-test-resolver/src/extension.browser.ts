...
export function activate(_context: vibecoda.ExtensionContext) {
	vibecoda.workspace.registerRemoteAuthorityResolver('test', {
		async resolve(_authority: string): Promise<vibecoda.ResolverResult> {
			console.log(`Resolving ${_authority}`);
			console.log(`Activating vibecoda.github-authentication to simulate auth`);
			await vibecoda.extensions.getExtension('vibecoda.github-authentication')?.activate();
			return new vibecoda.ManagedResolvedAuthority(async () => {
 ...
 * actual WebSocket.
 */
class InitialManagedMessagePassing implements vibecoda.ManagedMessagePassing {
	private readonly dataEmitter = new vibecoda.EventEmitter<Uint8Array>();
	private readonly closeEmitter = new vibecoda.EventEmitter<Error | undefined>();
	private readonly endEmitter = new vibecoda.EventEmitter<void>();
...
class OpeningManagedMessagePassing {
	...
	constructor(
		url: URL,
		dataEmitter: vibecoda.EventEmitter<Uint8Array>,
		closeEmitter: vibecoda.EventEmitter<Error | undefined>,
		_endEmitter: vibecoda.EventEmitter<void>
	) {
