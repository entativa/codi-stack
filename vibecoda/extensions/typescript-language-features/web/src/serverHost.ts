...
import { ApiClient, FileStat, FileType, Requests } from '@vibecoda/sync-api-client';
import { ClientConnection } from '@vibecoda/sync-api-common/browser';
...
function createServerHost(
	...
	const fs = apiClient?.vibecoda.workspace.fileSystem;
	...
	return {
		...
		write: s => {
			apiClient?.vibecoda.terminal.write(s);
		},
		writeOutputIsTTY() {
			return true;
		},
		readFile(path) {
			...
			try {
				...
				try {
					contents = fs.readFile(mapUri(uri, 'vibecoda-node-modules'));
				} catch (e) {
					return undefined;
				}
			}
		...
		getFileSize(path) {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						ret = fs.stat(mapUri(uri, 'vibecoda-node-modules')).size;
					} catch (_error) {
					}
				}
			}
		...
		writeFile(path, data, writeByteOrderMark) {
			...
			try {
				...
				if (uri.scheme !== 'vibecoda-global-typings' && (name === 'package.json' || name === 'package-lock.json' || name === 'package-lock.kdl')) {
					fs.writeFile(mapUri(uri, 'vibecoda-node-modules'), encoded);
				}
			} catch (error) {
		...
		fileExists(path: string): boolean {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						ret = fs.stat(mapUri(uri, 'vibecoda-node-modules')).type === FileType.File;
					} catch (_error) {
					}
				}
			}
		...
		directoryExists(path: string): boolean {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						stat = fs.stat(mapUri(uri, 'vibecoda-node-modules'));
					} catch (_error) {
					}
				}
			}
		...
		getModifiedTime(path: string): Date | undefined {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						s = fs.stat(mapUri(uri, 'vibecoda-node-modules'));
					} catch (_e) {
					}
				}
			}
	...
	function realpath(path: string): string {
		...
		const isNm = looksLikeNodeModules(path)
			&& !path.startsWith('/vibecoda-global-typings/')
			// Handle the case where a local folder has been opened in Vibecoda
		...
		if (isNm) {
			uri = mapUri(uri, 'vibecoda-node-modules');
		}
	...
	function getAccessibleFileSystemEntries(path: string): { files: readonly string[]; directories: readonly string[] } {
		...
		try {
			...
			try {
				entries = fs.readDirectory(mapUri(uri, 'vibecoda-node-modules'));
			} catch (_e) {
			}
		}
