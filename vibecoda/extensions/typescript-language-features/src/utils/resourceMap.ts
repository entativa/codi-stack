...
export class ResourceMap<T> {
	...
	private static readonly defaultPathNormalizer = (resource: vibecoda.Uri): string => {
	...
	private readonly _map = new Map<string, { readonly resource: vibecoda.Uri; value: T }>();

	constructor(
		protected readonly _normalizePath: (resource: vibecoda.Uri) => string | undefined = ResourceMap.defaultPathNormalizer,
	...
	public set(resource: vibecoda.Uri, value: T) {
		const file = this.toKey(resource);
		if (!file) {
			return;
		}
	...
	public delete(resource: vibecoda.Uri): void {
		const file = this.toKey(resource);
		if (file) {
	...
	public entries(): Iterable<{ resource: vibecoda.Uri; value: T }> {
		return this._map.values();
	}

	private toKey(resource: vibecoda.Uri): string | undefined {
