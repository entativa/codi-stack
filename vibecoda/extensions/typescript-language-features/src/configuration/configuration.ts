...
export class ImplicitProjectConfiguration {
	...
	constructor(configuration: vibecoda.WorkspaceConfiguration) {
	...
	private static readTarget(configuration: vibecoda.WorkspaceConfiguration): string | undefined {
		return configuration.get<string>('js/ts.implicitProjectConfig.target');
	}

	private static readModule(configuration: vibecoda.WorkspaceConfiguration): string | undefined {
		return configuration.get<string>('js/ts.implicitProjectConfig.module');
	}

	private static readCheckJs(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.checkJs', false);
	}

	private static readExperimentalDecorators(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.experimentalDecorators', false);
	}

	private static readImplicitStrictNullChecks(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.strictNullChecks', true);
	}

	private static readImplicitStrictFunctionTypes(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.strictFunctionTypes', true);
	}

	private static readImplicitStrict(configuration: vibecoda.WorkspaceConfiguration): boolean {
...
const vibecodaWatcherName = 'vibecoda';
type vibecodaWatcherName = typeof vibecodaWatcherName;
...
export abstract class BaseServiceConfigurationProvider implements ServiceConfigurationProvider {
	...
	public loadFromWorkspace(): TypeScriptServiceConfiguration {
		const configuration = vibecoda.workspace.getConfiguration();
		return {
	...
	protected abstract readGlobalTsdk(configuration: vibecoda.WorkspaceConfiguration): string | null;
	protected abstract readLocalTsdk(configuration: vibecoda.WorkspaceConfiguration): string | null;
	protected abstract readLocalNodePath(configuration: vibecoda.WorkspaceConfiguration): string | null;
	protected abstract readGlobalNodePath(configuration: vibecoda.WorkspaceConfiguration): string | null;

	protected readTsServerLogLevel(configuration: vibecoda.WorkspaceConfiguration): TsServerLogLevel {
		const setting = configuration.get<string>('typescript.tsserver.log', 'off');
		return TsServerLogLevel.fromString(setting);
	}

	protected readTsServerPluginPaths(configuration: vibecoda.WorkspaceConfiguration): string[] {
		return configuration.get<string[]>('typescript.tsserver.pluginPaths', []);
	}

	protected readNpmLocation(configuration: vibecoda.WorkspaceConfiguration): string | null {
		return configuration.get<string | null>('typescript.npm', null);
	}

	protected readDisableAutomaticTypeAcquisition(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.disableAutomaticTypeAcquisition', false);
	}

	protected readLocale(configuration: vibecoda.WorkspaceConfiguration): string | null {
		const value = configuration.get<string>('typescript.locale', 'auto');
		return !value || value === 'auto' ? null : value;
	}

	protected readUseSyntaxServer(configuration: vibecoda.WorkspaceConfiguration): SyntaxServerConfiguration {
	...
	protected readEnableDiagnosticsTelemetry(configuration: vibecoda.WorkspaceConfiguration): boolean {
	...
	protected readEnableProjectDiagnostics(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.experimental.enableProjectDiagnostics', false);
	}

	private readUseVsCodeWatcher(configuration: vibecoda.WorkspaceConfiguration): boolean {
		const watcherExcludes = configuration.get<Record<string, boolean>>('files.watcherExclude') ?? {};
		if (
			watcherExcludes['**/node_modules/*/**'] === true || // Vibecoda default prior to 1.94.x
			watcherExcludes['**/node_modules/**'] === true ||
			watcherExcludes['**/node_modules'] === true ||
			watcherExcludes['**'] === true	 					// Vibecoda Watching is entirely disabled
		) {
			return false;
		}
		...
		return configuration.get<Proto.WatchOptions | vibecodaWatcherName>('typescript.tsserver.watchOptions', vibecodaWatcherName) === vibecodaWatcherName;
	}

	private readWatchOptions(configuration: vibecoda.WorkspaceConfiguration): Proto.WatchOptions | undefined {
		const watchOptions = configuration.get<Proto.WatchOptions | vibecodaWatcherName>('typescript.tsserver.watchOptions');
		if (watchOptions === vibecodaWatcherName) {
			return undefined;
		}
	...
	protected readIncludePackageJsonAutoImports(configuration: vibecoda.WorkspaceConfiguration): 'auto' | 'on' | 'off' | undefined {
		return configuration.get<'auto' | 'on' | 'off'>('typescript.preferences.includePackageJsonAutoImports');
	}

	protected readMaxTsServerMemory(configuration: vibecoda.WorkspaceConfiguration): number {
	...
	protected readEnablePromptUseWorkspaceTsdk(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.enablePromptUseWorkspaceTsdk', false);
	}

	protected readEnableTsServerTracing(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.enableTracing', false);
	}

	private readWorkspaceSymbolsExcludeLibrarySymbols(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.workspaceSymbols.excludeLibrarySymbols', true);
	}

	private readWebProjectWideIntellisenseEnable(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.web.projectWideIntellisense.enabled', true);
	}

	private readWebProjectWideIntellisenseSuppressSemanticErrors(configuration: vibecoda.WorkspaceConfiguration): boolean {
	...
	private readWebTypeAcquisition(configuration: vibecoda.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.web.typeAcquisition.enabled', true);
	}

	private readEnableRegionDiagnostics(configuration: vibecoda.WorkspaceConfiguration): boolean {
