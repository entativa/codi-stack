...
export default class LanguageProvider extends Disposable {
	...
	private get documentSelector(): DocumentSelector {
		const semantic: vibecoda.DocumentFilter[] = [];
		const syntax: vibecoda.DocumentFilter[] = [];
	...
	private configurationChanged(): void {
		const config = vibecoda.workspace.getConfiguration(this.id, null);
		this.updateValidate(config.get(validateSetting, true));
		this.updateSuggestionDiagnostics(config.get(suggestionSetting, true));
	}

	public handlesUri(resource: vibecoda.Uri): boolean {
	...
	public handlesDocument(doc: vibecoda.TextDocument): boolean {
		return this.description.languageIds.includes(doc.languageId) || this.handlesConfigFile(doc.uri);
	}

	private handlesConfigFile(resource: vibecoda.Uri) {
	...
	public diagnosticsReceived(
		...
		file: vibecoda.Uri,
		diagnostics: (vibecoda.Diagnostic & { reportUnnecessary: any; reportDeprecated: any })[],
		ranges: vibecoda.Range[] | undefined): void {
		...
		const config = vibecoda.workspace.getConfiguration(this.id, file);
		...
		this.client.diagnosticsManager.updateDiagnostics(file, this._diagnosticLanguage, diagnosticsKind, diagnostics.filter(diag => {
			...
			if (!reportUnnecessary) {
				if (diag.reportUnnecessary && diag.severity === vibecoda.DiagnosticSeverity.Hint) {
					return false;
				}
			}
			if (!reportDeprecated) {
				if (diag.reportDeprecated && diag.severity === vibecoda.DiagnosticSeverity.Hint) {
					return false;
				}
			}
			return true;
	...
	public configFileDiagnosticsReceived(file: vibecoda.Uri, diagnostics: vibecoda.Diagnostic[]): void {
