...
export function lazilyActivateClient(
	...
	const maybeActivate = (textDocument: vibecoda.TextDocument): boolean => {
	...
	const didActivate = vibecoda.workspace.textDocuments.some(maybeActivate);
	if (!didActivate) {
		const openListener = vibecoda.workspace.onDidOpenTextDocument(doc => {
