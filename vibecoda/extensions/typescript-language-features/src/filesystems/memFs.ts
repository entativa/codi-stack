...
export class MemFs implements vibecoda.FileSystemProvider {
	...
	readDirectory(uri: vibecoda.Uri): [string, vibecoda.FileType][] {
		...
		if (!(entry instanceof FsDirectoryEntry)) {
			throw vibecoda.FileSystemError.FileNotADirectory();
		}
	...
	writeFile(uri: vibecoda.Uri, content: Uint8Array, { create, overwrite }: { create: boolean; overwrite: boolean }): void {
		...
		if (!entry) {
			if (create) {
				...
				this._emitter.fire([{ type: vibecoda.FileChangeType.Created, uri }]);
			} else {
				throw vibecoda.FileSystemError.FileNotFound();
			}
		} else {
			...
			if (overwrite) {
				entry.mtime = time;
				entry.data = content;
				this._emitter.fire([{ type: vibecoda.FileChangeType.Changed, uri }]);
			} else {
				throw vibecoda.FileSystemError.NoPermissions('overwrite option was not passed in');
			}
		}
	}

	rename(_oldUri: vibecoda.Uri, _newUri: vibecoda.Uri, _options: { overwrite: boolean }): void {
	...
	delete(uri: vibecoda.Uri): void {
		try {
			...
			this._emitter.fire([{ type: vibecoda.FileChangeType.Deleted, uri }]);
		} catch (e) { }
	}
	...
	private getEntry(uri: vibecoda.Uri): FsEntry | undefined {
	...
	private getParent(uri: vibecoda.Uri): FsDirectoryEntry {
	...
	private readonly _emitter = new vibecoda.EventEmitter<vibecoda.FileChangeEvent[]>();
	...
	watch(resource: vibecoda.Uri): vibecoda.Disposable {
...
class FsFileEntry {
	readonly type = vibecoda.FileType.File;
...
class FsDirectoryEntry {
	readonly type = vibecoda.FileType.Directory;
