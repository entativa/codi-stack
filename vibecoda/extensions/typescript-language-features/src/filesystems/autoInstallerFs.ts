...
import { PackageManager, ResolvedProject } from '@vibecoda/ts-package-manager';
import { basename, join } from 'path';
import * as vibecoda from 'vibecoda';
import { URI } from 'vibecoda-uri';
...
export class AutoInstallerFs extends Disposable implements vibecoda.FileSystemProvider {

	private readonly memfs: MemFs;
	private readonly packageManager: PackageManager;
	private readonly _projectCache = new Map</* root */ string, Promise<void> | undefined>();

	private readonly _emitter = this._register(new vibecoda.EventEmitter<vibecoda.FileChangeEvent[]>());
		...
		this.packageManager = new PackageManager({
			...
			directoryExists(path: string): boolean {
				try {
					...
					return stat.type === vibecoda.FileType.Directory;
				} catch (e) {
					return false;
				}
			},
	...
	async stat(uri: vibecoda.Uri): Promise<vibecoda.FileStat> {
		...
		if (basename(mapped.path) === 'node_modules' || basename(mapped.path) === '@types') {
			return {
				mtime: 0,
				ctime: 0,
				type: vibecoda.FileType.Directory,
				size: 0
			};
		}
	...
	writeFile(_uri: vibecoda.Uri, _content: Uint8Array, _options: { create: boolean; overwrite: boolean }): void {
	...
	delete(_uri: vibecoda.Uri): void {
		throw new Error('not implemented');
	}

	createDirectory(_uri: vibecoda.Uri): void {
		throw new Error('not implemented');
	}
	...
	private async getInstallOpts(originalUri: URI, root: string) {
		const vsfs = vibecoda.workspace.fs;
...
class MappedUri {
	readonly raw: vibecoda.Uri;
	readonly original: vibecoda.Uri;
	readonly mapped: vibecoda.Uri;
