...
export class Condition extends Disposable {
	...
	private readonly _onDidChange = this._register(new vibecoda.EventEmitter<void>());
...
class ConditionalRegistration {
	private state?: {
		...
		readonly registration: vibecoda.Disposable | undefined;
	};

	public constructor(
		private readonly conditions: readonly Condition[],
		private readonly doRegister: () => vibecoda.Disposable,
		private readonly elseDoRegister?: () => vibecoda.Disposable
	) {
...
export function conditionalRegistration(
	...
	doRegister: () => vibecoda.Disposable,
	elseDoRegister?: () => vibecoda.Disposable
): vibecoda.Disposable {
...
export function requireGlobalConfiguration(
	...
	return new Condition(
		() => {
			const config = vibecoda.workspace.getConfiguration(section, null);
...
export function requireHasVsCodeExtension(
	...
	return new Condition(
		() => {
			return !!vibecoda.extensions.getExtension(extensionId);
		},
		vibecoda.extensions.onDidChange
	);
}
