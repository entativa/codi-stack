			...
			expand.kind === 'navtree-function'
				...
				: expand.kind === 'refactor-info'
					...
					: expand.kind === 'code-action'
						...
						: expand.range;
		const initialSelection = initialRange ? new vibecoda.Selection(initialRange.start, initialRange.end) : undefined;
		await vibecoda.commands.executeCommand('vibecoda.editorChat.start', {
			initialRange,
			initialSelection,
			message,
			autoSend: true,
...
export class CompositeCommand implements Command {
	...
	public async execute(...commands: vibecoda.Command[]): Promise<void> {
...
export type Expand =
	| { kind: 'none'; readonly range: vibecoda.Range }
	| { kind: 'navtree-function'; readonly pos: vibecoda.Position }
...
async function findEditScope(
	...
	return new vibecoda.Range(start, expandEnd?.end ?? end);
}
