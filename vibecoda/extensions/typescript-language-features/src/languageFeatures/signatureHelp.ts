...
class TypeScriptSignatureHelpProvider implements vibecoda.SignatureHelpProvider {
	...
	public async provideSignatureHelp(
		...
		context: vibecoda.SignatureHelpContext,
	): Promise<vibecoda.SignatureHelp | undefined> {
		...
		const result = new vibecoda.SignatureHelp();
	...
	private getActiveSignature(context: vibecoda.SignatureHelpContext, info: Proto.SignatureHelpItems, signatures: readonly vibecoda.SignatureInformation[]): number {
	...
	private convertSignature(item: Proto.SignatureHelpItem, baseUri: vibecoda.Uri) {
		const signature = new vibecoda.SignatureInformation(
		...
		for (let i = 0; i < item.parameters.length; ++i) {
			...
			signature.parameters.push(
				new vibecoda.ParameterInformation(
...
function toTsTriggerReason(context: vibecoda.SignatureHelpContext): Proto.SignatureHelpTriggerReason {
	switch (context.triggerKind) {
		case vibecoda.SignatureHelpTriggerKind.TriggerCharacter:
		...
		case vibecoda.SignatureHelpTriggerKind.ContentChange:
			return context.isRetrigger ? { kind: 'retrigger' } : { kind: 'invoked' };

		case vibecoda.SignatureHelpTriggerKind.Invoke:
		default:
			return { kind: 'invoked' };
	}
}
export function register(
	...
	return conditionalRegistration([
		...
		return vibecoda.languages.registerSignatureHelpProvider(selector.syntax,
