...
class TsconfigLinkProvider implements vibecoda.DocumentLinkProvider {
	...
	private getExtendsLink(document: vibecoda.TextDocument, root: jsonc.Node): vibecoda.DocumentLink | undefined {
	...
	private getReferencesLinks(document: vibecoda.TextDocument, root: jsonc.Node) {
		return mapChildren(
	...
	private tryCreateTsConfigLink(document: vibecoda.TextDocument, node: jsonc.Node, linkType: TsConfigLinkType): vibecoda.DocumentLink | undefined {
		...
		const link = new vibecoda.DocumentLink(
			this.getRange(document, node),
			vibecoda.Uri.parse(`command:${openExtendsLinkCommandId}?${JSON.stringify(args)}`));
		link.tooltip = vibecoda.l10n.t("Follow link");
		return link;
	}

	private getFilesLinks(document: vibecoda.TextDocument, root: jsonc.Node) {
		return mapChildren(
	...
	): vibecoda.DocumentLink | undefined {
		return this.isPathValue(node)
			? new vibecoda.DocumentLink(this.getRange(document, node), this.getFileTarget(document, node))
			: undefined;
	}
	...
	private getFileTarget(document: vibecoda.TextDocument, node: jsonc.Node): vibecoda.Uri {
		if (isAbsolute(node.value)) {
			return vibecoda.Uri.file(node.value);
		}

		return vibecoda.Uri.joinPath(Utils.dirname(document.uri), node.value);
	}

	private getRange(document: vibecoda.TextDocument, node: jsonc.Node) {
		const offset = node.offset;
		const start = document.positionAt(offset + 1);
		const end = document.positionAt(offset + (node.length - 1));
		return new vibecoda.Range(start, end);
	}
}

async function resolveNodeModulesPath(baseDirUri: vibecoda.Uri, pathCandidates: string[]): Promise<vibecoda.Uri | undefined> {
	...
	while (true) {
		const moduleAbsoluteUrl = vibecoda.Uri.joinPath(currentUri, 'node_modules', moduleBasePath);
		let moduleStat: vibecoda.FileStat | undefined;
		try {
			moduleStat = await vibecoda.workspace.fs.stat(moduleAbsoluteUrl);
		} catch (err) {
			// noop
		}

		if (moduleStat && (moduleStat.type & vibecoda.FileType.Directory)) {
			for (const uriCandidate of pathCandidates
				...
				.filter(Boolean)
				.map((relativeModulePath) => vibecoda.Uri.joinPath(moduleAbsoluteUrl, relativeModulePath))
			) {
		...
		currentUri = vibecoda.Uri.joinPath(currentUri, '..');
...
*/
async function getTsconfigPath(baseDirUri: vibecoda.Uri, pathValue: string, linkType: TsConfigLinkType): Promise<vibecoda.Uri | undefined> {
	async function resolve(absolutePath: vibecoda.Uri): Promise<vibecoda.Uri> {
	...
	if (isRelativePath) {
		return resolve(vibecoda.Uri.joinPath(baseDirUri, pathValue));
	}

	if (pathValue.startsWith('/') || looksLikeAbsoluteWindowsPath(pathValue)) {
		return resolve(vibecoda.Uri.file(pathValue));
	}
...
export function register() {
	const patterns: vibecoda.GlobPattern[] = [
		'**/[jt]sconfig.json',
		'**/[jt]sconfig.*.json',
	];
	...
	const selector: vibecoda.DocumentSelector =
		languages.map(language => patterns.map((pattern): vibecoda.DocumentFilter => ({ language, pattern })))
			.flat();

	return vibecoda.Disposable.from(
		vibecoda.commands.registerCommand(openExtendsLinkCommandId, async ({ resourceUri, extendsValue, linkType }: OpenExtendsLinkCommandArgs) => {
			const tsconfigPath = await getTsconfigPath(Utils.dirname(vibecoda.Uri.from(resourceUri)), extendsValue, linkType);
			if (tsconfigPath === undefined) {
				vibecoda.window.showErrorMessage(vibecoda.l10n.t("Failed to resolve {0} as module", extendsValue));
				return;
			}
			// Will suggest to create a .json variant if it doesn't exist yet (but only for relative paths)
			await vibecoda.commands.executeCommand('vibecoda.open', tsconfigPath);
		}),
		vibecoda.languages.registerDocumentLinkProvider(selector, new TsconfigLinkProvider()),
	);
}
