...
class CopyMetadata {
	...
	static parse(data: string): CopyMetadata | undefined {
		try {
			...
			const resource = vibecoda.Uri.parse(parsedData.resource);
			const ranges = parsedData.ranges.map((range: any) => new vibecoda.Range(range.start, range.end));
...
class TsPasteEdit extends vibecoda.DocumentPasteEdit {
	...
	constructor() {
		super('', vibecoda.l10n.t("Paste with imports"), DocumentPasteProvider.kind);
		this.yieldTo = [
			vibecoda.DocumentDropOrPasteEditKind.Text.append('plain')
		];
	}
}
...
class DocumentPasteProvider implements vibecoda.DocumentPasteEditProvider<TsPasteEdit> {

	static readonly kind = vibecoda.DocumentDropOrPasteEditKind.TextUpdateImports.append('jsts');
	...
	async prepareDocumentPaste(document: vibecoda.TextDocument, ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, token: vibecoda.CancellationToken) {
		...
		if (response) {
			...
			dataTransfer.set(DocumentPasteProvider.metadataMimeType,
				new vibecoda.DataTransferItem(new CopyMetadata(document.uri, ranges, undefined)));
		} else {
			...
			dataTransfer.set(DocumentPasteProvider.metadataMimeType,
				new vibecoda.DataTransferItem(new CopyMetadata(document.uri, ranges, copyRequest)));
		}
	}
	...
	async provideDocumentPasteEdits(
		...
		const pasteCts = new vibecoda.CancellationTokenSource();
	...
	async resolveDocumentPasteEdit(inEdit: TsPasteEdit, _token: vibecoda.CancellationToken): Promise<TsPasteEdit | undefined> {
	...
	private async extractMetadata(dataTransfer: vibecoda.DataTransfer, token: vibecoda.CancellationToken): Promise<CopyMetadata | undefined> {
	...
	private isEnabled(document: vibecoda.TextDocument) {
		const config = vibecoda.workspace.getConfiguration(this._modeId, document.uri);
...
export function register(selector: DocumentSelector, language: LanguageDescription, client: ITypeScriptServiceClient, fileConfigurationManager: FileConfigurationManager) {
	return conditionalRegistration([
		...
		return vibecoda.languages.registerDocumentPasteEditProvider(selector.semantic, new DocumentPasteProvider(language.id, client, fileConfigurationManager), {
