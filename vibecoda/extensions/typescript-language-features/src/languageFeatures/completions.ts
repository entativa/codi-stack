...
interface CompletionContext {
	...
	readonly wordRange: vibecoda.Range | undefined;
	readonly line: string;
	readonly optionalReplacementRange: vibecoda.Range | undefined;
}

type ResolvedCompletionItem = {
	readonly edits?: readonly vibecoda.TextEdit[];
	readonly commands: readonly vibecoda.Command[];
};

class MyCompletionItem extends vibecoda.CompletionItem {

	public readonly useCodeSnippet: boolean;

	constructor(
		public readonly position: vibecoda.Position,
		...
		if (tsEntry.source && tsEntry.hasAction && client.apiVersion.lt(API.v490)) {
			...
			// https://github.com/microsoft/vibecoda/issues/40311
		...
		if (tsEntry.source && tsEntry.hasAction) {
			...
			const qualifierCandidate = vibecoda.workspace.asRelativePath(tsEntry.source);
		...
		this.useCodeSnippet = completionContext.completeFunctionCalls && (this.kind === vibecoda.CompletionItemKind.Function || this.kind === vibecoda.CompletionItemKind.Method);
		...
		this.insertText = isSnippet && tsEntry.insertText ? new vibecoda.SnippetString(tsEntry.insertText) : tsEntry.insertText;
		this.filterText = tsEntry.filterText || this.getFilterText(completionContext.line, tsEntry.insertText);

		if (completionContext.isMemberCompletion && completionContext.dotAccessorContext && !(this.insertText instanceof vibecoda.SnippetString)) {
		...
		if (tsEntry.kindModifiers) {
			...
			if (kindModifiers.has(PConst.KindModifiers.deprecated)) {
				this.tags = [vibecoda.CompletionItemTag.Deprecated];
			}

			if (kindModifiers.has(PConst.KindModifiers.color)) {
				this.kind = vibecoda.CompletionItemKind.Color;
			}
	...
	private _resolvedPromise?: {
		readonly requestToken: vibecoda.CancellationTokenSource;
	...
	public async resolveCompletionItem(
		...
		const requestToken = new vibecoda.CancellationTokenSource();
		...
		const promise = (async (): Promise<ResolvedCompletionItem | undefined> => {
			...
			const commands: vibecoda.Command[] = [{
			...
			if (this.useCodeSnippet) {
				...
				if (shouldCompleteFunction) {
					...
					if (parameterCount > 0) {
						//Fix for https://github.com/microsoft/vibecoda/issues/104059
						//Don't show parameter hints if "editor.parameterHints.enabled": false
						if (vibecoda.workspace.getConfiguration('editor.parameterHints').get('enabled')) {
	...
	): vibecoda.MarkdownString | undefined {
		const documentation = new vibecoda.MarkdownString();
	...
	private async isValidFunctionCompletionContext(
		...
		// https://github.com/microsoft/vibecoda/issues/18131
	...
	private getCodeActions(
		detail: Proto.CompletionEntryDetails,
		filepath: string
	): { command?: vibecoda.Command; additionalTextEdits?: vibecoda.TextEdit[] } {
		...
		// using a vibecoda command
		const additionalTextEdits: vibecoda.TextEdit[] = [];
		...
		let command: vibecoda.Command | undefined = undefined;
	...
	private getRangeFromReplacementSpan(tsEntry: Proto.CompletionEntry, completionContext: CompletionContext) {
		if (!tsEntry.replacementSpan) {
			if (completionContext.optionalReplacementRange) {
				return {
					inserting: new vibecoda.Range(completionContext.optionalReplacementRange.start, this.position),
		...
		if (!replaceRange.isSingleLine) {
			replaceRange = new vibecoda.Range(replaceRange.start.line, replaceRange.start.character, replaceRange.start.line, completionContext.line.length);
		}
		return {
	...
	private resolveRange(): void {
		...
		if (replaceRange) {
			this.range = {
				inserting: new vibecoda.Range(replaceRange.start, this.position),
				replacing: replaceRange
			};
		}
	}

	private static convertKind(kind: string): vibecoda.CompletionItemKind {
		switch (kind) {
			case PConst.Kind.primitiveType:
			case PConst.Kind.keyword:
				return vibecoda.CompletionItemKind.Keyword;
			...
			case PConst.Kind.parameter:
				return vibecoda.CompletionItemKind.Variable;

			case PConst.Kind.memberVariable:
			case PConst.Kind.memberGetAccessor:
			case PConst.Kind.memberSetAccessor:
				return vibecoda.CompletionItemKind.Field;

			case PConst.Kind.function:
			case PConst.Kind.localFunction:
				return vibecoda.CompletionItemKind.Function;

			case PConst.Kind.method:
			case PConst.Kind.constructSignature:
			case PConst.Kind.callSignature:
			case PConst.Kind.indexSignature:
				return vibecoda.CompletionItemKind.Method;

			case PConst.Kind.enum:
				return vibecoda.CompletionItemKind.Enum;

			case PConst.Kind.enumMember:
				return vibecoda.CompletionItemKind.EnumMember;

			case PConst.Kind.module:
			case PConst.Kind.externalModuleName:
				return vibecoda.CompletionItemKind.Module;

			case PConst.Kind.class:
			case PConst.Kind.type:
				return vibecoda.CompletionItemKind.Class;

			case PConst.Kind.interface:
				return vibecoda.CompletionItemKind.Interface;

			case PConst.Kind.warning:
				return vibecoda.CompletionItemKind.Text;

			case PConst.Kind.script:
				return vibecoda.CompletionItemKind.File;

			case PConst.Kind.directory:
				return vibecoda.CompletionItemKind.Folder;

			case PConst.Kind.string:
				return vibecoda.CompletionItemKind.Constant;

			default:
				return vibecoda.CompletionItemKind.Property;
		}
	}
...
class CompletionAcceptedCommand implements Command {
	...
	public constructor(
		private readonly onCompletionAccepted: (item: vibecoda.CompletionItem) => void,
		private readonly telemetryReporter: TelemetryReporter,
	) { }

	public execute(item: vibecoda.CompletionItem) {
...
class ApplyCompletionCommand implements Command {
	...
	public async execute(item: MyCompletionItem) {
		...
		for (const command of commands) {
			await vibecoda.commands.executeCommand(command.command, ...(command.arguments ?? []));
		}
	}
}
...
class ApplyCompletionCodeActionCommand implements Command {
	...
	public async execute(_file: string, codeActions: Proto.CodeAction[]): Promise<boolean> {
		...
		const selection = await vibecoda.window.showQuickPick(
			codeActions.map(action => ({
				label: action.description,
				description: '',
				action,
			})), {
			placeHolder: vibecoda.l10n.t("Select code action to apply")
		});
...
namespace CompletionConfiguration {
	...
	export function getConfigurationForResource(
		...
		const config = vibecoda.workspace.getConfiguration(modeId, resource);
		return {
...
class TypeScriptCompletionItemProvider implements vibecoda.CompletionItemProvider<MyCompletionItem> {
	...
	constructor(
		...
		private readonly telemetryReporter: TelemetryReporter,
		onCompletionAccepted: (item: vibecoda.CompletionItem) => void
	) {
	...
	public async provideCompletionItems(
		...
		position: vibecoda.Position,
		token: vibecoda.CancellationToken,
		context: vibecoda.CompletionContext
	): Promise<vibecoda.CompletionList<MyCompletionItem> | undefined> {
		if (!vibecoda.workspace.getConfiguration(this.language.id, document).get('suggest.enabled')) {
			return undefined;
		}

		if (this.typingsStatus.isAcquiringTypings) {
			return Promise.reject<vibecoda.CompletionList<MyCompletionItem>>({
				label: vibecoda.l10n.t({
		...
		if (wordRange && !wordRange.isEmpty) {
			...
			const firstChar = document.getText(new vibecoda.Range(wordRange.start, secondCharPosition));
			if (firstChar === '@') {
		...
		let optionalReplacementRange: vibecoda.Range | undefined;
		...
		if (isMemberCompletion) {
			...
			if (dotMatch) {
				const range = new vibecoda.Range(position.translate({ characterDelta: -dotMatch[0].length }), position);
		...
		return new vibecoda.CompletionList(items, isIncomplete);
	}
	...
	private getTsTriggerCharacter(context: vibecoda.CompletionContext): Proto.CompletionsTriggerCharacter | undefined {
