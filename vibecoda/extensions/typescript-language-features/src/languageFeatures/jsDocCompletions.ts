...
const defaultJsDoc = new vibecoda.SnippetString(`/**\n * $0\n */`);

class JsDocCompletionItem extends vibecoda.CompletionItem {
	constructor(
		public readonly document: vibecoda.TextDocument,
		public readonly position: vibecoda.Position
	) {
		super('/** */', vibecoda.CompletionItemKind.Text);
		this.detail = vibecoda.l10n.t("JSDoc comment");
		this.sortText = '\0';
		...
		const range = new vibecoda.Range(start, position.translate(0, suffix ? suffix[0].length : 0));
		this.range = { inserting: range, replacing: range };
	}
}

class JsDocCompletionProvider implements vibecoda.CompletionItemProvider {
	...
	): Promise<vibecoda.CompletionItem[] | undefined> {
		if (!vibecoda.workspace.getConfiguration(this.language.id, document).get('suggest.completeJSDocs')) {
			return undefined;
		}
...
export function templateToSnippet(template: string): vibecoda.SnippetString {
	...
	return new vibecoda.SnippetString(template);
}
