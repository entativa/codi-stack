...
async function isDirectory(resource: vibecoda.Uri): Promise<boolean> {
	try {
		return (await vibecoda.workspace.fs.stat(resource)).type === vibecoda.FileType.Directory;
	} catch {
		return false;
	}
}
...
interface RenameAction {
	...
	readonly jsTsFileThatIsBeingMoved: vibecoda.Uri;
}
...
class UpdateImportsOnFileRenameHandler extends Disposable {
	...
	public constructor(
		...
		private readonly _handles: (uri: vibecoda.Uri) => Promise<boolean>,
	) {
		super();
			...
			for (const { newUri, oldUri } of e.files) {
				...
				this._delayer.trigger(() => {
					vibecoda.window.withProgress({
						...
						title: vibecoda.l10n.t("Checking for update of JS/TS imports")
					}, () => this.flushRenames());
				});
			}
		}));
	...
	private async flushRenames(): Promise<void> {
		...
		for (const group of this.groupRenames(renames)) {
			const edits = new vibecoda.WorkspaceEdit();
			const resourcesBeingRenamed: vibecoda.Uri[] = [];

			for (const { oldUri, newUri, newFilePath, oldFilePath, jsTsFileThatIsBeingMoved } of group) {
				const document = await vibecoda.workspace.openTextDocument(jsTsFileThatIsBeingMoved);
			...
			if (edits.size) {
				if (await this.confirmActionWithUser(resourcesBeingRenamed)) {
					await vibecoda.workspace.applyEdit(edits, { isRefactoring: true });
				}
			}
		}
	}

	private async confirmActionWithUser(newResources: readonly vibecoda.Uri[]): Promise<boolean> {
	...
	private getConfiguration(resource: vibecoda.Uri) {
		return vibecoda.workspace.getConfiguration(doesResourceLookLikeATypeScriptFile(resource) ? 'typescript' : 'javascript', resource);
	}

	private async promptUser(newResources: readonly vibecoda.Uri[]): Promise<boolean> {
		if (!newResources.length) {
			return false;
		}
		...
		const response = await vibecoda.window.showInformationMessage(
			newResources.length === 1
				? vibecoda.l10n.t("Update imports for '{0}'?", path.basename(newResources[0].fsPath))
				: this.getConfirmMessage(vibecoda.l10n.t("Update imports for the following {0} files?", newResources.length), newResources), {
			modal: true,
	...
	private async getJsTsFileBeingMoved(resource: vibecoda.Uri): Promise<vibecoda.Uri | undefined> {
		if (resource.scheme !== fileSchemes.file) {
			return undefined;
		}

		if (await isDirectory(resource)) {
			const files = await vibecoda.workspace.findFiles(new vibecoda.RelativePattern(resource, '**/*.{ts,tsx,js,jsx}'), '**/node_modules/**', 1);
			return files[0];
		}
	...
	private getConfirmMessage(start: string, resourcesToConfirm: readonly vibecoda.Uri[]): string {
	...
	private getConfigTargetScope(config: vibecoda.WorkspaceConfiguration, settingsName: string): vibecoda.ConfigurationTarget {
...
export function register(
	...
	handles: (uri: vibecoda.Uri) => Promise<boolean>,
) {
