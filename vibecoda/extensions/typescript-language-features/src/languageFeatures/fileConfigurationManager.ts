...
export default class FileConfigurationManager extends Disposable {
	...
	private getFormattingOptions(document: vibecoda.TextDocument): FormattingOptions | undefined {
		const editor = vibecoda.window.visibleTextEditors.find(editor => editor.document.uri.toString() === document.uri.toString());
		if (!editor) {
			return undefined;
		}
	...
	private getFormatOptions(
		...
		const config = vibecoda.workspace.getConfiguration(
	...
	private getPreferences(document: vibecoda.TextDocument): Proto.UserPreferences {
		const config = vibecoda.workspace.getConfiguration(
			isTypeScriptDocument(document) ? 'typescript' : 'javascript',
			document);

		const preferencesConfig = vibecoda.workspace.getConfiguration(
		...
		const preferences: Proto.UserPreferences = {
			...
			autoImportFileExcludePatterns: this.getAutoImportFileExcludePatternsPreference(preferencesConfig, vibecoda.workspace.getWorkspaceFolder(document.uri)?.uri),
	...
	private getQuoteStylePreference(config: vibecoda.WorkspaceConfiguration) {
	...
	private getAutoImportFileExcludePatternsPreference(config: vibecoda.WorkspaceConfiguration, workspaceFolder: vibecoda.Uri | undefined): string[] | undefined {
		return workspaceFolder && config.get<string[]>('autoImportFileExcludePatterns')?.map(p => {
			...
			return path.isAbsolute(p) ? p :
				p.startsWith('*') ? wildcardPrefix + p :
					isRelative ? this.client.toTsFilePath(vibecoda.Uri.joinPath(workspaceFolder, p))! :
						wildcardPrefix + '**' + path.sep + p;
		});
	}

	private getOrganizeImportsPreferences(config: vibecoda.WorkspaceConfiguration): Proto.UserPreferences {
	...
	private getMaximumHoverLength(document: vibecoda.TextDocument): number {
		const defaultMaxLength = 500;
		const maximumHoverLength = vibecoda.workspace.getConfiguration('js/ts', document).get<number>('hover.maximumLength', defaultMaxLength);
...
export function getInlayHintsPreferences(config: vibecoda.WorkspaceConfiguration) {
	return {
...
function getInlayParameterNameHintsPreference(config: vibecoda.WorkspaceConfiguration) {
...
function getImportModuleSpecifierPreference(config: vibecoda.WorkspaceConfiguration) {
...
function getImportModuleSpecifierEndingPreference(config: vibecoda.WorkspaceConfiguration) {
...
function getJsxAttributeCompletionStyle(config: vibecoda.WorkspaceConfiguration) {
