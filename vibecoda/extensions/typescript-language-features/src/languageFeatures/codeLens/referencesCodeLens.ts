...
export class TypeScriptReferencesCodeLensProvider extends TypeScriptBaseCodeLensProvider {
	...
	public async resolveCodeLens(codeLens: ReferencesCodeLens, token: vibecoda.CancellationToken): Promise<vibecoda.CodeLens> {
	...
	private getCodeLensLabel(locations: ReadonlyArray<vibecoda.Location>): string {
		return locations.length === 1
			? vibecoda.l10n.t("1 reference")
			: vibecoda.l10n.t("{0} references", locations.length);
	}
	...
	protected extractSymbol(
		...
		switch (item.kind) {
			case PConst.Kind.function: {
				const showOnAllFunctions = vibecoda.workspace.getConfiguration(this.language.id).get<boolean>('referencesCodeLens.showOnAllFunctions');
			...
			case PConst.Kind.memberVariable:
				...
				// For https://github.com/microsoft/vibecoda/issues/90396
				if (parent &&
