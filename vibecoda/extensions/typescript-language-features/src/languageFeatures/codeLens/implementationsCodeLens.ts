...
export default class TypeScriptImplementationsCodeLensProvider extends TypeScriptBaseCodeLensProvider {
	public constructor(
		...
		this._register(
			vibecoda.workspace.onDidChangeConfiguration(evt => {
	...
	public async resolveCodeLens(
		...
		const locations = response.body
			.map(reference =>
				// Only take first line on implementation: https://github.com/microsoft/vibecoda/issues/23924
				new vibecoda.Location(this.client.toResource(reference.file),
					reference.start.line === reference.end.line
						? typeConverters.Range.fromTextSpan(reference)
						: new vibecoda.Range(
							typeConverters.Position.fromLocation(reference.start),
							new vibecoda.Position(reference.start.line, 0))))
	...
	private getCommand(locations: vibecoda.Location[], codeLens: ReferencesCodeLens): vibecoda.Command | undefined {
		return {
	...
	private getTitle(locations: vibecoda.Location[]): string {
		return locations.length === 1
			? vibecoda.l10n.t("1 implementation")
			: vibecoda.l10n.t("{0} implementations", locations.length);
	}
	...
	protected extractSymbol(
		...
		const cfg = vibecoda.workspace.getConfiguration(this.language.id);
...
export function register(
	...
	return conditionalRegistration([
		...
		return vibecoda.languages.registerCodeLensProvider(selector.semantic,
