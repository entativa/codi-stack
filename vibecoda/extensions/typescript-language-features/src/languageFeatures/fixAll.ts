...
async function buildIndividualFixes(
	...
	edit: vibecoda.WorkspaceEdit,
	client: ITypeScriptServiceClient,
	file: string,
	diagnostics: readonly vibecoda.Diagnostic[],
...
abstract class SourceAction extends vibecoda.CodeAction {
	abstract build(
		client: ITypeScriptServiceClient,
		file: string,
		diagnostics: readonly vibecoda.Diagnostic[],
...
class SourceFixAll extends SourceAction {
	...
	static readonly kind = vibecoda.CodeActionKind.SourceFixAll.append('ts');

	constructor() {
		super(vibecoda.l10n.t("Fix all fixable JS/TS issues"), SourceFixAll.kind);
	}

	async build(client: ITypeScriptServiceClient, file: string, diagnostics: readonly vibecoda.Diagnostic[], token: vibecoda.CancellationToken): Promise<void> {
		this.edit = new vibecoda.WorkspaceEdit();
...
class SourceRemoveUnused extends SourceAction {
	...
	static readonly kind = vibecoda.CodeActionKind.Source.append('removeUnused').append('ts');

	constructor() {
		super(vibecoda.l10n.t("Remove all unused code"), SourceRemoveUnused.kind);
	}
...
class SourceAddMissingImports extends SourceAction {
	...
	static readonly kind = vibecoda.CodeActionKind.Source.append('addMissingImports').append('ts');

	constructor() {
		super(vibecoda.l10n.t("Add all missing imports"), SourceAddMissingImports.kind);
	}
...
class TypeScriptAutoFixProvider implements vibecoda.CodeActionProvider {
	...
	public get metadata(): vibecoda.CodeActionProviderMetadata {
		return {
	...
	public async provideCodeActions(
		...
		token: vibecoda.CancellationToken
	): Promise<vibecoda.CodeAction[] | undefined> {
		if (!context.only || !vibecoda.CodeActionKind.Source.intersects(context.only)) {
			return undefined;
		}
	...
	private getFixAllActions(only: vibecoda.CodeActionKind): SourceAction[] {
...
export function register(
	...
	return conditionalRegistration([
		...
		return vibecoda.languages.registerCodeActionsProvider(selector.semantic, provider, provider.metadata);
	});
}
