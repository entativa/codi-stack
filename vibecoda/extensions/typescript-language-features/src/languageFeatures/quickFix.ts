...
type ApplyCodeActionCommand_args = {
	...
	readonly diagnostic: vibecoda.Diagnostic;
...
class DiagnosticsSet {
	public static from(diagnostics: vibecoda.Diagnostic[]) {
		const values = new Map<string, vibecoda.Diagnostic>();
	...
	private static key(diagnostic: vibecoda.Diagnostic) {
		const { start, end } = diagnostic.range;
		return `${diagnostic.code}-${start.line},${start.character}-${end.line},${end.character}`;
	}

	private constructor(
		private readonly _values: Map<string, vibecoda.Diagnostic>
	) { }

	public get values(): Iterable<vibecoda.Diagnostic> {
...
class VsCodeCodeAction extends vibecoda.CodeAction {
	constructor(
...
class SupportedCodeActionProvider {
	...
	public async getFixableDiagnosticsForContext(diagnostics: readonly vibecoda.Diagnostic[]): Promise<DiagnosticsSet> {
...
class TypeScriptQuickFixProvider implements vibecoda.CodeActionProvider<VsCodeCodeAction> {
	...
	public static readonly metadata: vibecoda.CodeActionProviderMetadata = {
		providedCodeActionKinds: [vibecoda.CodeActionKind.QuickFix]
	};
	...
	public async resolveCodeAction(codeAction: VsCodeCodeAction, token: vibecoda.CancellationToken): Promise<VsCodeCodeAction> {
	...
	private async getFixesForDiagnostic(
		document: vibecoda.TextDocument,
		file: string,
		diagnostic: vibecoda.Diagnostic,
	...
	private getFixesForTsCodeAction(
		...
		const codeAction = new VsCodeCodeAction(action, action.description, vibecoda.CodeActionKind.QuickFix);
		...
		const copilot = vibecoda.extensions.getExtension('github.copilot-chat');
		if (copilot?.isActive) {
			...
			if (action.fixName === fixNames.classIncorrectlyImplementsInterface) {
				title = vibecoda.l10n.t('{0} with AI', action.description);
				message = vibecoda.l10n.t(`Implement the stubbed-out class members for ${document.getText(diagnostic.range)} with a useful implementation.`);
				expand = { kind: 'code-action', action };
			} else if (action.fixName === fixNames.fixClassDoesntImplementInheritedAbstractMember) {
				title = vibecoda.l10n.t('{0} with AI', action.description);
				message = vibecoda.l10n.t(`Implement the stubbed-out class members for {0} with a useful implementation.`, document.getText(diagnostic.range));
				expand = { kind: 'code-action', action };
			} else if (action.fixName === fixNames.fixMissingFunctionDeclaration) {
				title = vibecoda.l10n.t(`Implement missing function declaration '{0}' using AI`, document.getText(diagnostic.range));
				message = vibecoda.l10n.t(`Provide a reasonable implementation of the function {0} given its type and the context it's called in.`, document.getText(diagnostic.range));
				expand = { kind: 'code-action', action };
			} else if (action.fixName === fixNames.inferFromUsage) {
				const inferFromBody = new VsCodeCodeAction(action, vibecoda.l10n.t('Infer types using AI'), vibecoda.CodeActionKind.QuickFix);
				inferFromBody.edit = new vibecoda.WorkspaceEdit();
				...
				inferFromBody.command = {
					...
					arguments: [{
						message: vibecoda.l10n.t('Add types to this code. Add separate interfaces when possible. Do not change the code except for adding types.'),
				...
				title = vibecoda.l10n.t('Add meaningful parameter name with AI');
				message = vibecoda.l10n.t(`Rename the parameter {0} with a more meaningful name.`, newText);
				expand = {
			...
			if (expand && message !== undefined) {
				const aiCodeAction = new VsCodeCodeAction(action, title, vibecoda.CodeActionKind.QuickFix);
	...
	private addFixAllForTsCodeAction(
		...
		const action = new VsCodeFixAllCodeAction(
			tsAction,
			file,
			tsAction.fixAllDescription || vibecoda.l10n.t("{0} (Fix all in file)", tsAction.description),
