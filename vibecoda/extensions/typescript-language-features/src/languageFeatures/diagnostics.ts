...
function diagnosticsEquals(a: vibecoda.Diagnostic, b: vibecoda.Diagnostic): boolean {
	if (a === b) {
		return true;
	}
...
class FileDiagnostics {
	...
	private readonly _diagnostics = new Map<DiagnosticKind, ReadonlyArray<vibecoda.Diagnostic>>();

	constructor(
		public readonly file: vibecoda.Uri,
	...
	public updateDiagnostics(
		...
		kind: DiagnosticKind,
		diagnostics: ReadonlyArray<vibecoda.Diagnostic>,
		ranges: ReadonlyArray<vibecoda.Range> | undefined
	): boolean {
	...
	public getAllDiagnostics(settings: DiagnosticSettings): vibecoda.Diagnostic[] {
	...
	public delete(toDelete: vibecoda.Diagnostic): void {
	...
	private updateRegionDiagnostics(
		diagnostics: ReadonlyArray<vibecoda.Diagnostic>,
		ranges: ReadonlyArray<vibecoda.Range>): boolean {
	...
	private getSuggestionDiagnostics(settings: DiagnosticSettings) {
		...
		return this.get(DiagnosticKind.Suggestion).filter(x => {
			if (!enableSuggestions) {
				// Still show unused
				return x.tags && (x.tags.includes(vibecoda.DiagnosticTag.Unnecessary) || x.tags.includes(vibecoda.DiagnosticTag.Deprecated));
			}
			return true;
		});
	}

	private get(kind: DiagnosticKind): ReadonlyArray<vibecoda.Diagnostic> {
...
class DiagnosticsTelemetryManager extends Disposable {
	...
	private readonly _diagnosticSnapshotsMap = new ResourceMap<readonly vibecoda.Diagnostic[]>(uri => uri.toString(), { onCaseInsensitiveFileSystem: false });
	private _timeout: NodeJS.Timeout | undefined;
	private _telemetryEmitter: NodeJS.Timeout | undefined;

	constructor(
		private readonly _telemetryReporter: TelemetryReporter,
		private readonly _diagnosticsCollection: vibecoda.DiagnosticCollection,
	) {
		super();
...
export class DiagnosticsManager extends Disposable {
	...
	private readonly _currentDiagnostics: vibecoda.DiagnosticCollection;
	...
	constructor(
		...
		this._currentDiagnostics = this._register(vibecoda.languages.createDiagnosticCollection(owner));
	...
	public deleteAllDiagnosticsInFile(resource: vibecoda.Uri): void {
		this._currentDiagnostics.delete(resource);
		this._diagnostics.delete(resource);
	}

	public deleteDiagnostic(resource: vibecoda.Uri, diagnostic: vibecoda.Diagnostic): void {
	...
	public getDiagnostics(file: vibecoda.Uri): ReadonlyArray<vibecoda.Diagnostic> {
	...
	private scheduleDiagnosticsUpdate(file: vibecoda.Uri) {
	...
	private updateCurrentDiagnostics(file: vibecoda.Uri): void {
		if (this._pendingUpdates.has(file)) {
