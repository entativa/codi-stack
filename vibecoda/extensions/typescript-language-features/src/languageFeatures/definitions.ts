...
export default class TypeScriptDefinitionProvider extends DefinitionProviderBase implements vibecoda.DefinitionProvider {

	public async provideDefinition(
		document: vibecoda.TextDocument,
		position: vibecoda.Position,
		token: vibecoda.CancellationToken
	): Promise<vibecoda.DefinitionLink[] | vibecoda.Definition | undefined> {
		...
		if (vibecoda.workspace.getConfiguration(document.languageId).get('preferGoToSourceDefinition', false) && this.client.apiVersion.gte(API.v470)) {
		...
		return definitions
			.map((location): vibecoda.DefinitionLink => {
...
export function register(
	...
	return conditionalRegistration([
		...
		return vibecoda.languages.registerDefinitionProvider(selector.syntax,
