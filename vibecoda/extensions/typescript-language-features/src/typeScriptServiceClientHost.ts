...
export default class TypeScriptServiceClientHost extends Disposable {
	...
	constructor(
		...
		vibecoda.workspace.onDidChangeConfiguration(this.configurationChanged, this, this._disposables);
		this.configurationChanged();
		this._register(new LogLevelMonitor(context));
	}

	private registerExtensionLanguageProvider(description: LanguageDescription, onCompletionAccepted: (item: vibecoda.CompletionItem) => void) {
		const manager = new LanguageProvider(this.client, description, this.commandManager, this.client.telemetryReporter, this.typingsStatus, this.fileConfigurationManager, onCompletionAccepted);
		this.languages.push(manager);
		this._register(manager);
		this.languagePerId.set(description.id, manager);
	...
	public async handles(resource: vibecoda.Uri): Promise<boolean> {
	...
	private configurationChanged(): void {
		const typescriptConfig = vibecoda.workspace.getConfiguration('typescript');

		this.reportStyleCheckAsWarnings = typescriptConfig.get('reportStyleChecksAsWarnings', true);
	}

	private async findLanguage(resource: vibecoda.Uri): Promise<LanguageProvider | undefined> {
		try {
			...
			// be correct
			const doc = await vibecoda.workspace.openTextDocument(resource);
	...
	private configFileDiagnosticsReceived(event: Proto.ConfigFileDiagnosticEvent): void {
		...
		this.findLanguage(this.client.toResource(body.configFile)).then(language => {
			language?.configFileDiagnosticsReceived(this.client.toResource(body.configFile), body.diagnostics.map(tsDiag => {
				const range = tsDiag.start && tsDiag.end ? typeConverters.Range.fromTextSpan(tsDiag) : new vibecoda.Range(0, 0, 0, 1);
				const diagnostic = new vibecoda.Diagnostic(range, tsDiag.text, this.getDiagnosticSeverity(tsDiag));
	...
	private createMarkerDatas(
		diagnostics: Proto.Diagnostic[],
		source: string
	): (vibecoda.Diagnostic & { reportUnnecessary: any; reportDeprecated: any })[] {
		return diagnostics.map(tsDiag => this.tsDiagnosticToVsDiagnostic(tsDiag, source));
	}

	private tsDiagnosticToVsDiagnostic(diagnostic: Proto.Diagnostic, source: string): vibecoda.Diagnostic & { reportUnnecessary: any; reportDeprecated: any } {
		const { start, end, text } = diagnostic;
		const range = new vibecoda.Range(typeConverters.Position.fromLocation(start), typeConverters.Position.fromLocation(end));
		const converted = new vibecoda.Diagnostic(range, text, this.getDiagnosticSeverity(diagnostic));
		...
		if (relatedInformation) {
			converted.relatedInformation = coalesce(relatedInformation.map((info: any) => {
				...
				return new vibecoda.DiagnosticRelatedInformation(typeConverters.Location.fromTextSpan(this.client.toResource(span.file), span), info.message);
			}));
		}
		const tags: vibecoda.DiagnosticTag[] = [];
		if (diagnostic.reportsUnnecessary) {
			tags.push(vibecoda.DiagnosticTag.Unnecessary);
		}
		if (diagnostic.reportsDeprecated) {
			tags.push(vibecoda.DiagnosticTag.Deprecated);
		}
		converted.tags = tags.length ? tags : undefined;

		const resultConverted = converted as vibecoda.Diagnostic & { reportUnnecessary: any; reportDeprecated: any };
	...
	private getDiagnosticSeverity(diagnostic: Proto.Diagnostic): vibecoda.DiagnosticSeverity {
		if (this.reportStyleCheckAsWarnings
			&& this.isStyleCheckDiagnostic(diagnostic.code)
			&& diagnostic.category === PConst.DiagnosticCategory.error
		) {
			return vibecoda.DiagnosticSeverity.Warning;
		}

		switch (diagnostic.category) {
			case PConst.DiagnosticCategory.error:
				return vibecoda.DiagnosticSeverity.Error;

			case PConst.DiagnosticCategory.warning:
				return vibecoda.DiagnosticSeverity.Warning;

			case PConst.DiagnosticCategory.suggestion:
				return vibecoda.DiagnosticSeverity.Hint;
