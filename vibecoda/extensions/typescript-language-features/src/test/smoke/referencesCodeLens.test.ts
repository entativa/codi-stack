...
export async function updateConfig(newConfig: VsCodeConfiguration): Promise<VsCodeConfiguration> {
	...
	const config = vibecoda.workspace.getConfiguration(undefined);
...
suite('TypeScript References', () => {
	...
	test('Should show on basic class', async () => {
		const testDocumentUri = vibecoda.Uri.parse('untitled:test1.ts');
	...
	test('Should show on basic class properties', async () => {
		const testDocumentUri = vibecoda.Uri.parse('untitled:test2.ts');
		await createTestEditor(testDocumentUri,
			`class Foo {`,
			`	prop: number;`,
			`	meth(): void {}`,
	...
	test('Should not show on const property', async () => {
		const testDocumentUri = vibecoda.Uri.parse('untitled:test3.ts');
		await createTestEditor(testDocumentUri,
			`const foo = {`,
			`	prop: 1;`,
			`	meth(): void {}`,
	...
	test.skip('Should not show duplicate references on ES5 class (https://github.com/microsoft/vibecoda/issues/90396)', async () => {
		const testDocumentUri = vibecoda.Uri.parse('untitled:test3.js');
		await createTestEditor(testDocumentUri,
			`function A() {`,
			`    console.log("hi");`,
			`}`,
...
function getCodeLenses(document: vibecoda.Uri): Thenable<readonly vibecoda.CodeLens[] | undefined> {
	return vibecoda.commands.executeCommand<readonly vibecoda.CodeLens[]>('vibecoda.executeCodeLensProvider', document, 100);
}
