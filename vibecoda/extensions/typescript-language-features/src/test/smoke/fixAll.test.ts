...
const emptyRange = new vibecoda.Range(new vibecoda.Position(0, 0), new vibecoda.Position(0, 0));
...
suite.skip('TypeScript Fix All', () => {
	...
	test('Fix all should remove unreachable code', async () => {
		...
		const fixes = await vibecoda.commands.executeCommand<vibecoda.CodeAction[]>('vibecoda.executeCodeActionProvider',
			testDocumentUri,
			emptyRange,
			vibecoda.CodeActionKind.SourceFixAll
		);

		await vibecoda.workspace.applyEdit(fixes![0].edit!);
	...
	test('Remove unused should handle nested ununused', async () => {
		...
		const fixes = await vibecoda.commands.executeCommand<vibecoda.CodeAction[]>('vibecoda.executeCodeActionProvider',
			testDocumentUri,
			emptyRange,
			vibecoda.CodeActionKind.Source.append('removeUnused')
		);
