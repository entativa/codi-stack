...
export function createRandomFile(contents = '', fileExtension = 'txt'): Thenable<vibecoda.Uri> {
	return new Promise((resolve, reject) => {
		...
		fs.writeFile(tmpFile, contents, (error) => {
			...
			resolve(vibecoda.Uri.file(tmpFile));
		});
	});
}


export function deleteFile(file: vibecoda.Uri): Thenable<boolean> {
	return new Promise((resolve, reject) => {
...
export function withRandomFileEditor(
	...
	run: (editor: vibecoda.TextEditor, doc: vibecoda.TextDocument) => Thenable<void>
): Thenable<boolean> {
		...
		return vibecoda.workspace.openTextDocument(file).then(doc => {
			return vibecoda.window.showTextDocument(doc).then((editor) => {
				if (cursorIndex >= 0) {
					...
					editor.selection = new vibecoda.Selection(pos, pos);
				}
...
export async function createTestEditor(uri: vibecoda.Uri, ...lines: string[]) {
	const document = await vibecoda.workspace.openTextDocument(uri);
	const editor = await vibecoda.window.showTextDocument(document);
	await editor.insertSnippet(new vibecoda.SnippetString(joinLines(...lines)), new vibecoda.Range(0, 0, 1000, 0));
	return editor;
}

export function assertEditorContents(editor: vibecoda.TextEditor, expectedDocContent: string, message?: string): void {
...
export async function updateConfig(documentUri: vibecoda.Uri, newConfig: VsCodeConfiguration): Promise<VsCodeConfiguration> {
	const oldConfig: VsCodeConfiguration = {};
	const config = vibecoda.workspace.getConfiguration(undefined, documentUri);

	for (const configKey of Object.keys(newConfig)) {
		oldConfig[configKey] = config.get(configKey);
		await new Promise<void>((resolve, reject) =>
			config.update(configKey, newConfig[configKey], vibecoda.ConfigurationTarget.Global)
				.then(() => resolve(), reject));
	}
	return oldConfig;
}
...
export function onChangedDocument(documentUri: vibecoda.Uri, disposables: vibecoda.Disposable[]) {
	return new Promise<vibecoda.TextDocument>(resolve => vibecoda.workspace.onDidChangeTextDocument(e => {
...
export async function retryUntilDocumentChanges(
	...
	disposables: vibecoda.Disposable[],
