...
export type ExecConfig = {
	...
	readonly cancelOnResourceChange?: vibecoda.Uri;
...
export interface ITypeScriptServiceClient {
	...
	/**
	 * Convert a (Vibecoda) resource to a path that TypeScript server understands.
	 */
	toTsFilePath(resource: vibecoda.Uri): string | undefined;

	/**
	 * Convert a path to a resource.
	 */
	toResource(filepath: string): vibecoda.Uri;

	/**
	 * Tries to ensure that a vibecoda document is open on the TS server.
	 *
	 * @return The normalized path or `undefined` if the document is not open on the server.
	 */
	toOpenTsFilePath(document: vibecoda.TextDocument | vibecoda.Uri, options?: {
	 ...
	 */
	hasCapabilityForResource(resource: vibecoda.Uri, capability: ClientCapability): boolean;

	getWorkspaceRootForResource(resource: vibecoda.Uri): vibecoda.Uri | undefined;

	readonly onTsServerStarted: vibecoda.Event<{ version: TypeScriptVersion; usedApiVersion: API }>;
	readonly onProjectLanguageServiceStateChanged: vibecoda.Event<Proto.ProjectLanguageServiceStateEventBody>;
	readonly onDidBeginInstallTypings: vibecoda.Event<Proto.BeginInstallTypesEventBody>;
	readonly onDidEndInstallTypings: vibecoda.Event<Proto.EndInstallTypesEventBody>;
	readonly onTypesInstallerInitializationFailed: vibecoda.Event<Proto.TypesInstallerInitializationFailedEventBody>;

	readonly capabilities: ClientCapabilities;
	readonly onDidChangeCapabilities: vibecoda.Event<void>;
