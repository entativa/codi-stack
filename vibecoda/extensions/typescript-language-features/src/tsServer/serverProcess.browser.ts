...
import { ServiceConnection } from '@vibecoda/sync-api-common/browser';
import { ApiService, Requests } from '@vibecoda/sync-api-service';
...
export class WorkerServerProcessFactory implements TsServerProcessFactory {
	constructor(
		private readonly _extensionUri: vibecoda.Uri,
	...
	public constructor(
		...
		this._watcher.onmessage = (event: MessageEvent<BrowserWatchEvent>) => {
			switch (event.data.type) {
				...
				case 'watchDirectory':
				case 'watchFile': {
					this._watches.create(event.data.id, vibecoda.Uri.from(event.data.uri), /*watchParentDirs*/ true, !!event.data.recursive, {
		...
		new ApiService('vibecoda-wasm-typescript', connection);
