...
import { Cancellation } from '@vibecoda/sync-api-common/lib/common/messageCancellation';
...
export type TsServerLog =
	{ readonly type: 'file'; readonly uri: vibecoda.Uri } |
	{ readonly type: 'output'; readonly output: vibecoda.OutputChannel };

export interface ITypeScriptServer {
	readonly onEvent: vibecoda.Event<Proto.Event>;
	readonly onExit: vibecoda.Event<TypeScriptServerExitEvent>;
	readonly onError: vibecoda.Event<unknown>;
	 ...
	 */
	executeImpl(command: keyof TypeScriptRequests, args: unknown, executeInfo: { isAsync: boolean; token?: vibecoda.CancellationToken; expectsResult: boolean; lowPriority?: boolean; executionTarget?: ExecutionTarget }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined>;
...
export class SingleTsServer extends Disposable implements ITypeScriptServer {
	...
	private readonly _onEvent = this._register(new vibecoda.EventEmitter<Proto.Event>());
	public readonly onEvent = this._onEvent.event;

	private readonly _onExit = this._register(new vibecoda.EventEmitter<TypeScriptServerExitEvent>());
	public readonly onExit = this._onExit.event;

	private readonly _onError = this._register(new vibecoda.EventEmitter<unknown>());
	...
	public executeImpl(command: keyof TypeScriptRequests, args: unknown, executeInfo: { isAsync: boolean; token?: vibecoda.CancellationToken; expectsResult: boolean; lowPriority?: boolean; executionTarget?: ExecutionTarget }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined> {
...
interface ExecuteInfo {
	...
	readonly token?: vibecoda.CancellationToken;
...
class RequestRouter {
	...
	public execute(
		...
		if (RequestRouter.sharedCommands.has(command) && typeof executeInfo.executionTarget === 'undefined') {
			...
			let token: vibecoda.CancellationToken | undefined = undefined;
			if (executeInfo.token) {
