...
export class DiskTypeScriptVersionProvider implements ITypeScriptVersionProvider {
	...
	public get bundledVersion(): TypeScriptVersion {
		const version = this.getContributedVersion(TypeScriptVersionSource.Bundled, 'vibecoda.typescript-language-features', ['..', 'node_modules']);
		if (version) {
			return version;
		}

		vibecoda.window.showErrorMessage(vibecoda.l10n.t("Vibecoda\'s tsserver was deleted by another application such as a misbehaving virus detection tool. Please reinstall Vibecoda."));
		throw new Error('Could not find bundled tsserver.js');
	}

	private get contributedTsNextVersion(): TypeScriptVersion | undefined {
		return this.getContributedVersion(TypeScriptVersionSource.TsNightlyExtension, 'ms-vibecoda.vibecoda-typescript-next', ['node_modules']);
	}
	...
	private getContributedVersion(source: TypeScriptVersionSource, extensionId: string, pathToTs: readonly string[]): TypeScriptVersion | undefined {
		try {
			const extension = vibecoda.extensions.getExtension(extensionId);
			if (extension) {
	...
	private loadTypeScriptVersionsFromPath(source: TypeScriptVersionSource, relativePath: string): TypeScriptVersion[] {
		...
		for (const root of vibecoda.workspace.workspaceFolders) {
			let label: string = relativePath;
			if (vibecoda.workspace.workspaceFolders.length > 1) {
	...
	private static getApiVersion(serverPath: string): API | undefined {
		...
		const tsdkVersion = vibecoda.workspace.getConfiguration().get<string | undefined>('typescript.tsdk_version', undefined);
		if (tsdkVersion) {
