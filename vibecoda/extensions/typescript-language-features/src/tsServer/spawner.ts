...
export class TypeScriptServerSpawner {
	...
	public static readonly tsServerLogOutputChannel = new Lazy<vibecoda.OutputChannel>(() => {
		return vibecoda.window.createOutputChannel(vibecoda.l10n.t("TypeScript Server Log"));
	});
	...
	private getTsServerArgs(
	...
	): { args: string[]; tsServerLog: TsServerLog | undefined; tsServerTraceDirectory: vibecoda.Uri | undefined } {
		const args: string[] = [];
		let tsServerLog: TsServerLog | undefined;
		let tsServerTraceDirectory: vibecoda.Uri | undefined;
		...
		if (TypeScriptServerSpawner.isLoggingEnabled(configuration)) {
			if (isWeb()) {
				...
				if (logDir) {
					const logFilePath = vibecoda.Uri.joinPath(logDir, `tsserver.log`);
		...
		if (
			...
			&& !isYarnPnp // Disable for yarn pnp as it currently breaks with the Vibecoda watcher
		) {
