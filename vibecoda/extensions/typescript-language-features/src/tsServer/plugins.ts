...
export interface TypeScriptServerPlugin {
	readonly extension: vibecoda.Extension<unknown>;
...
export class PluginManager extends Disposable {
	...
	private readonly _onDidUpdatePlugins = this._register(new vibecoda.EventEmitter<this>());
	public readonly onDidChangePlugins = this._onDidUpdatePlugins.event;

	private readonly _onDidUpdateConfig = this._register(new vibecoda.EventEmitter<{ pluginId: string; config: unknown }>());
	...
	private readPlugins() {
		...
		for (const extension of vibecoda.extensions.all) {
