...
class BufferSynchronizer {
	...
	constructor(
		...
		pathNormalizer: (path: vibecoda.Uri) => string | undefined,
	...
	public open(resource: vibecoda.Uri, args: Proto.OpenRequestArgs) {
	 ...
	 */
	public close(resource: vibecoda.Uri, filepath: string, scriptKind: ScriptKind | undefined): boolean {
		return this.updatePending(resource, new CloseOperation(filepath, scriptKind));
	}

	public change(resource: vibecoda.Uri, filepath: string, events: readonly vibecoda.TextDocumentContentChangeEvent[]) {
		if (!events.length) {
			return;
		}
	...
	private updatePending(resource: vibecoda.Uri, op: BufferOperation): boolean {
		switch (op.type) {
...
class SyncedBuffer {
	...
	constructor(
		private readonly document: vibecoda.TextDocument,
	...
	private getProjectRootPath(resource: vibecoda.Uri): string | undefined {
		...
		if (!workspaceRoot && vibecoda.workspace.workspaceFolders) {
			for (const root of Array.from(vibecoda.workspace.workspaceFolders).sort((a, b) => a.uri.path.length - b.uri.path.length)) {
	...
	public onContentChanged(events: readonly vibecoda.TextDocumentContentChangeEvent[]): void {
		if (this.state !== BufferState.Open) {
...
class SyncedBufferMap extends ResourceMap<SyncedBuffer> {
	...
	public getForPath(filePath: string): SyncedBuffer | undefined {
		return this.get(vibecoda.Uri.file(filePath));
	}
...
class PendingDiagnostics extends ResourceMap<number> {
	public getOrderedFileSet(): ResourceMap<void | vibecoda.Range[]> {
		const orderedResources = Array.from(this.entries())
			.sort((a, b) => a.value - b.value)
			.map(entry => entry.resource);

		const map = new ResourceMap<void | vibecoda.Range[]>(this._normalizePath, this.config);
...
class GetErrRequest {
	...
	public static executeGetErrRequest(
		...
		files: ResourceMap<void | vibecoda.Range[]>,
		onDone: () => void
	) {
	...
	private readonly _token: vibecoda.CancellationTokenSource = new vibecoda.CancellationTokenSource();

	private constructor(
		private readonly client: ITypeScriptServiceClient,
		public readonly files: ResourceMap<void | vibecoda.Range[]>,
		onDone: () => void
	) {
...
class TabResourceTracker extends Disposable {
	...
	private readonly _onDidChange = this._register(new vibecoda.EventEmitter<{
		readonly closed: Iterable<vibecoda.Uri>;
		readonly opened: Iterable<vibecoda.Uri>;
	}>());
	public readonly onDidChange = this._onDidChange.event;

	private readonly _tabResources: ResourceMap<{ readonly tabs: Set<vibecoda.Tab> }>;

	constructor(
		normalizePath: (resource: vibecoda.Uri) => string | undefined,
		config: {
		...
		this._tabResources = new ResourceMap<{ readonly tabs: Set<vibecoda.Tab> }>(normalizePath, config);

		for (const tabGroup of vibecoda.window.tabGroups.all) {
			for (const tab of tabGroup.tabs) {
				this.add(tab);
			}
		}

		this._register(vibecoda.window.tabGroups.onDidChangeTabs(e => {
	...
	public has(resource: vibecoda.Uri): boolean {
		if (resource.scheme === fileSchemes.vibecodaNotebookCell) {
			const notebook = vibecoda.workspace.notebookDocuments.find(doc =>
	...
	private add(tab: vibecoda.Tab): vibecoda.Uri[] {
		const addedResources: vibecoda.Uri[] = [];
	...
	private delete(tab: vibecoda.Tab): vibecoda.Uri[] {
		const closedResources: vibecoda.Uri[] = [];
		for (const uri of this.getResourcesForTab(tab)) {
			const entry = this._tabResources.get(uri);
			if (!entry) {
				continue;
	...
	private getResourcesForTab(tab: vibecoda.Tab): vibecoda.Uri[] {
		if (tab.input instanceof vibecoda.TabInputText) {
			return [tab.input.uri];
		} else if (tab.input instanceof vibecoda.TabInputTextDiff) {
			return [tab.input.original, tab.input.modified];
		} else if (tab.input instanceof vibecoda.TabInputNotebook) {
			return [tab.input.uri];
		} else {
			return [];
		}
...
export default class BufferSyncSupport extends Disposable {
	...
	constructor(
		...
		const pathNormalizer = (path: vibecoda.Uri) => this.client.toTsFilePath(path);
		...
		vibecoda.workspace.onDidChangeConfiguration(this.updateConfiguration, this, this._disposables);
	}

	private readonly _onDelete = this._register(new vibecoda.EventEmitter<vibecoda.Uri>());
	public readonly onDelete = this._onDelete.event;

	private readonly _onWillChange = this._register(new vibecoda.EventEmitter<vibecoda.Uri>());
	...
	public listen(): void {
		...
		this.listening = true;
		vibecoda.workspace.onDidOpenTextDocument(this.openTextDocument, this, this._disposables);
		vibecoda.workspace.onDidCloseTextDocument(this.onDidCloseTextDocument, this, this._disposables);
		vibecoda.workspace.onDidChangeTextDocument(this.onDidChangeTextDocument, this, this._disposables);
		...
		vibecoda.workspace.textDocuments.forEach(this.openTextDocument, this);
	}

	public handles(resource: vibecoda.Uri): boolean {
		return this.syncedBuffers.has(resource);
	}

	public ensureHasBuffer(resource: vibecoda.Uri): boolean {
		if (this.syncedBuffers.has(resource)) {
			return true;
		}

		const existingDocument = vibecoda.workspace.textDocuments.find(doc => doc.uri.toString() === resource.toString());
		if (existingDocument) {
	...
	public toVsCodeResource(resource: vibecoda.Uri): vibecoda.Uri {
	...
	public toResource(filePath: string): vibecoda.Uri {
		...
		return vibecoda.Uri.file(filePath);
	}
	...
	public openTextDocument(document: vibecoda.TextDocument): boolean {
	...
	public closeResource(resource: vibecoda.Uri): void {
	...
	public lineCount(resource: vibecoda.Uri): number | undefined {
		return this.syncedBuffers.get(resource)?.lineCount;
	}

	private onDidCloseTextDocument(document: vibecoda.TextDocument): void {
		this.closeResource(document.uri);
	}

	private onDidChangeTextDocument(e: vibecoda.TextDocumentChangeEvent): void {
	...
	public getErr(resources: readonly vibecoda.Uri[]): any {
	...
	public hasPendingDiagnostics(resource: vibecoda.Uri): boolean {
	...
	private sendPendingDiagnostics(): void {
		...
		for (const buffer of this.syncedBuffers.values()) {
			const editors = vibecoda.window.visibleTextEditors.filter(editor => editor.document.uri.toString() === buffer.resource.toString());
	...
	private updateConfiguration() {
		const jsConfig = vibecoda.workspace.getConfiguration('javascript', null);
		const tsConfig = vibecoda.workspace.getConfiguration('typescript', null);
