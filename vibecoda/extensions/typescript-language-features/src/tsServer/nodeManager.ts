...
export class NodeVersionManager extends Disposable {
	...
	private async computeNewVersion() {
		...
		const workspaceVersion = this.configuration.localNodePath;
		if (vibecoda.workspace.isTrusted && workspaceVersion) {
			const useWorkspaceNode = this.canUseWorkspaceNode(workspaceVersion);
			if (useWorkspaceNode === undefined) {
	...
	private async promptUseWorkspaceNode(): Promise<string | undefined> {
		...
		const allow = vibecoda.l10n.t("Yes");
		const disallow = vibecoda.l10n.t("No");
		const dismiss = vibecoda.l10n.t("Not now");

		const result = await vibecoda.window.showInformationMessage(vibecoda.l10n.t("This workspace wants to use the Node installation at '{0}' to run TS Server. Would you like to use it?", workspaceVersion),
			allow,
			disallow,
			dismiss,
		);
