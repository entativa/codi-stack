...
import VsCodeTelemetryReporter from '@vibecoda/extension-telemetry';
...
export async function activate(context: vibecoda.ExtensionContext): Promise<Api> {
	...
	const onCompletionAccepted = new vibecoda.EventEmitter<vibecoda.CompletionItem>();
	...
	const versionProvider = new StaticVersionProvider(
		new TypeScriptVersion(
			...
			vibecoda.Uri.joinPath(context.extensionUri, 'dist/browser/typescript/tsserver.web.js').toString(),
...
async function startPreloadWorkspaceContentsIfNeeded(context: vibecoda.ExtensionContext, logger: Logger): Promise<void> {
	...
	await Promise.all(vibecoda.workspace.workspaceFolders.map(async folder => {
		const workspaceUri = folder.uri;
		if (workspaceUri.scheme !== 'vibecoda-vfs' || !workspaceUri.authority.startsWith('github')) {
...
class RemoteWorkspaceContentsPreloader extends Disposable {
	...
	constructor(
		private readonly workspaceUri: vibecoda.Uri,
		private readonly logger: Logger,
	) {
		super();

		const fsWatcher = this._register(vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(workspaceUri, '*')));
