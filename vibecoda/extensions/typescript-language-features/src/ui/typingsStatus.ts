...
export class AtaProgressReporter extends Disposable {
	...
	private _onBegin(eventId: number): void {
		...
		vibecoda.window.withProgress({
			...
			title: vibecoda.l10n.t("Fetching data for better TypeScript IntelliSense")
		}, () => promise);
	}
	...
	private async onTypesInstallerInitializationFailed() {
		const config = vibecoda.workspace.getConfiguration('typescript');

		if (config.get<boolean>('check.npmIsInstalled', true)) {
			const dontShowAgain: vibecoda.MessageItem = {
				title: vibecoda.l10n.t("Don't Show Again"),
			};
			const selected = await vibecoda.window.showWarningMessage(
				vibecoda.l10n.t(
