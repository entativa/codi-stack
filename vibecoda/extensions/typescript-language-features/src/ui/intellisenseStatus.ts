...
namespace IntellisenseState {
	...
	export class Pending {
		...
		public readonly cancellation = new vibecoda.CancellationTokenSource();
...
type CreateOrOpenConfigCommandArgs = [root: vibecoda.Uri, projectType: ProjectType];
...
export class IntellisenseStatus extends Disposable {
	...
	private _statusItem?: vibecoda.LanguageStatusItem;
	...
	private updateState(newState: IntellisenseState.State): void {
		...
		switch (this._state.type) {
			...
			case IntellisenseState.Type.Pending: {
				...
				statusItem.severity = vibecoda.LanguageStatusSeverity.Information;
				statusItem.text = vibecoda.l10n.t("Loading IntelliSense status");
			...
			case IntellisenseState.Type.Resolved: {
				const noConfigFileText = this._state.projectType === ProjectType.TypeScript
					? vibecoda.l10n.t("No tsconfig")
					: vibecoda.l10n.t("No jsconfig");

				const rootPath = this._client.getWorkspaceRootForResource(this._state.resource);
				if (!rootPath) {
					if (this._statusItem) {
						this._statusItem.text = noConfigFileText;
						this._statusItem.detail = !vibecoda.workspace.workspaceFolders
							? vibecoda.l10n.t("No opened folders")
							: vibecoda.l10n.t("File is not part opened folders");
				...
				if (isImplicitProjectConfigFile(this._state.configFile)) {
					...
					statusItem.command = {
						...
						title: this._state.projectType === ProjectType.TypeScript
							? vibecoda.l10n.t("Configure TSConfig")
							: vibecoda.l10n.t("Configure JSConfig"),
						arguments: [rootPath, this._state.projectType] satisfies CreateOrOpenConfigCommandArgs,
					};
				} else {
					statusItem.text = vibecoda.workspace.asRelativePath(this._state.configFile);
					statusItem.detail = undefined;
					statusItem.command = {
						command: this.openOpenConfigCommandId,
						title: vibecoda.l10n.t("Open Config File"),
						arguments: [rootPath, this._state.projectType] satisfies CreateOrOpenConfigCommandArgs,
					};
				}
				break;
			...
			case IntellisenseState.Type.SyntaxOnly: {
				const statusItem = this.ensureStatusItem();
				statusItem.severity = vibecoda.LanguageStatusSeverity.Warning;
				statusItem.text = vibecoda.l10n.t("Partial mode");
				statusItem.detail = vibecoda.l10n.t("Project wide IntelliSense not available");
				statusItem.busy = false;
				statusItem.command = {
					title: vibecoda.l10n.t("Learn More"),
					command: 'vibecoda.open',
					arguments: [
						vibecoda.Uri.parse('https://aka.ms/vibecoda/jsts/partial-mode'),
					]
				};
				break;
			}
	...
	private ensureStatusItem(): vibecoda.LanguageStatusItem {
		if (!this._statusItem) {
			this._statusItem = vibecoda.languages.createLanguageStatusItem('typescript.projectStatus', jsTsLanguageModes);
			this._statusItem.name = vibecoda.l10n.t("JS/TS IntelliSense Status");
		}
