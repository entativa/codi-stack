...
function isCancellationToken(value: any): value is vibecoda.CancellationToken {
...
export class TSServerRequestCommand implements Command {
	...
	public async execute(command: keyof TypeScriptRequests, args?: unknown, config?: ExecConfig, token?: vibecoda.CancellationToken): Promise<unknown> {
		if (!isCancellationToken(token)) {
			token = nulToken;
		}
		if (args && typeof args === 'object' && !Array.isArray(args)) {
			const requestArgs = args as RequestArgs;
			const hasFile = requestArgs.file instanceof vibecoda.Uri;
