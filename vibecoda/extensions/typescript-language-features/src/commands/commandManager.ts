...
export class CommandManager {
	private readonly commands = new Map<string, { refCount: number; readonly registration: vibecoda.Disposable }>();
	...
	public register<T extends Command>(command: T): vibecoda.Disposable {
		...
		if (!entry) {
			entry = { refCount: 1, registration: vibecoda.commands.registerCommand(command.id, command.execute, command) };
