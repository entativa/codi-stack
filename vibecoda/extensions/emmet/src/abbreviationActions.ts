...
interface PreviewRangesWithContent {
	previewRange: vibecoda.Range;
	originalRange: vibecoda.Range;
...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
	...
	const operationRanges = Array.from(editor.selections).sort((a, b) => a.start.compareTo(b.start)).map(selection => {
		let rangeToReplace: vibecoda.Range = selection;
		// wrap around the node if the selection falls inside its open or close tag
		{
			...
			rangeToReplace = new vibecoda.Range(start, end);
		}
		...
		if (!rangeToReplace.isSingleLine && rangeToReplace.end.character === 0) {
			...
			rangeToReplace = new vibecoda.Range(rangeToReplace.start, document.lineAt(previousLine).range.end);
		}
		...
		if (!firstLineOfRange.isEmptyOrWhitespace && firstLineOfRange.firstNonWhitespaceCharacterIndex > rangeToReplace.start.character) {
			rangeToReplace = rangeToReplace.with(new vibecoda.Position(rangeToReplace.start.line, firstLineOfRange.firstNonWhitespaceCharacterIndex));
		}
	...
	// Also helps with https://github.com/microsoft/vibecoda/issues/113930 by avoiding `editor.linkedEditing`
	// execution if selection is inside an open or close tag
	const oldSelections = editor.selections;
	editor.selections = operationRanges.map(range => new vibecoda.Selection(range.start, range.end));
	...
	function applyPreview(expandAbbrList: ExpandAbbreviationInput[]): Thenable<boolean> {
		let lastOldPreviewRange = new vibecoda.Range(0, 0, 0, 0);
		let lastNewPreviewRange = new vibecoda.Range(0, 0, 0, 0);
		...
		return editor.edit(builder => {
			...
			for (let i = 0; i < rangesToReplace.length; i++) {
				...
				lastNewPreviewRange = new vibecoda.Range(newPreviewLineStart, newPreviewStart, newPreviewLineEnd, newPreviewEnd);
	...
	const prompt = vibecoda.l10n.t("Enter Abbreviation");
	const inputAbbreviation = (args && args['abbreviation'])
		? (args['abbreviation'] as string)
		: await vibecoda.window.showInputBox({ prompt, validateInput: inputChanged });
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
	if (!validate() || !vibecoda.window.activeTextEditor) {
		return fallbackTab();
	}
	 ...
	 */
	if (vibecoda.window.activeTextEditor.selections.length === 1 &&
		vibecoda.window.activeTextEditor.selection.isEmpty
	) {
		const anchor = vibecoda.window.activeTextEditor.selection.anchor;
		if (anchor.character === 0) {
			return fallbackTab();
		}

		const prevPositionAnchor = anchor.translate(0, -1);
		const prevText = vibecoda.window.activeTextEditor.document.getText(new vibecoda.Range(prevPositionAnchor, anchor));
	...
	args = args || {};
	if (!args['language']) {
		args['language'] = vibecoda.window.activeTextEditor.document.languageId;
	} else {
		const excludedLanguages = vibecoda.workspace.getConfiguration('emmet')['excludeLanguages'] ? vibecoda.workspace.getConfiguration('emmet')['excludeLanguages'] : [];
		if (excludedLanguages.includes(vibecoda.window.activeTextEditor.document.languageId)) {
			return fallbackTab();
		}
	}
	...
	if (vibecoda.workspace.getConfiguration('emmet')['triggerExpansionOnTab'] === true && editor.selections.find(x => !x.isEmpty)) {
		return fallbackTab();
	}
	...
	const getAbbreviation = (document: vibecoda.TextDocument, selection: vibecoda.Selection, position: vibecoda.Position, syntax: string): [vibecoda.Range | null, string, string | undefined] => {
		...
		if (syntax === 'html') {
			...
			if (matches) {
				abbr = matches[1];
				rangeToReplace = new vibecoda.Range(position.translate(0, -(abbr.length + 1)), position);
		...
		return [new vibecoda.Range(abbreviationRange.start.line, abbreviationRange.start.character, abbreviationRange.end.line, abbreviationRange.end.character), abbreviation, filter];
	};
	...
	function getRootNode() {
		...
		const usePartialParsing = vibecoda.workspace.getConfiguration('emmet')['optimizeStylesheetParsing'] === true;
	...
	selectionsInReverseOrder.forEach(selection => {
		...
		if (isStyleSheet(syntax) && abbreviation.endsWith(':')) {
			// Fix for https://github.com/Microsoft/vibecoda/issues/1623
			return;
		}
...
function fallbackTab(): Thenable<boolean | undefined> {
	if (vibecoda.workspace.getConfiguration('emmet')['triggerExpansionOnTab'] === true) {
		return vibecoda.commands.executeCommand('tab');
	}
 ...
 */
export function isValidLocationForEmmetAbbreviation(document: vibecoda.TextDocument, rootNode: Node | undefined, currentNode: Node | undefined, syntax: string, offset: number, abbreviationRange: vibecoda.Range): boolean {
	if (isStyleSheet(syntax)) {
		...
		// Fixes https://github.com/microsoft/vibecoda/issues/74505
		// Stylesheet abbreviations starting with @ should bring up suggestions
		// even at outer-most level
		const abbreviation = document.getText(new vibecoda.Range(abbreviationRange.start.line, abbreviationRange.start.character, abbreviationRange.end.line, abbreviationRange.end.character));
		if (abbreviation.startsWith('@')) {
			return true;
		}

		// Fix for https://github.com/microsoft/vibecoda/issues/34162
		...
		// Workaround for https://github.com/microsoft/vibecoda/30188
	...
	if (currentHtmlNode) {
		...
		// Fix for https://github.com/microsoft/vibecoda/issues/28829
		if (!currentHtmlNode.open || !currentHtmlNode.close ||
			!(currentHtmlNode.open.end <= offset && offset <= currentHtmlNode.close.start)) {
			return false;
		}

		// Fix for https://github.com/microsoft/vibecoda/issues/35128
	...
	let textToBackTrack = document.getText(new vibecoda.Range(startPos.line, startPos.character, abbreviationRange.start.line, abbreviationRange.start.character));
	...
	while (i >= 0) {
		...
		// Fix for https://github.com/microsoft/vibecoda/issues/55411
 ...
 */
async function expandAbbreviationInRange(editor: vibecoda.TextEditor, expandAbbrList: ExpandAbbreviationInput[], insertSameSnippet: boolean): Promise<boolean> {
	...
	if (!insertSameSnippet) {
		...
		for (const expandAbbrInput of expandAbbrList) {
			...
			if (expandedText) {
				await editor.insertSnippet(new vibecoda.SnippetString(expandedText), expandAbbrInput.rangeToReplace, { undoStopBefore: false, undoStopAfter: false });
	...
	if (expandedText) {
		return editor.insertSnippet(new vibecoda.SnippetString(expandedText), allRanges);
	}
	return false;
}
...
function expandAbbr(input: ExpandAbbreviationInput): string | undefined {
	...
	if (input.textToWrap) {
		...
		if (expandOptions.options) {
			// Below fixes https://github.com/microsoft/vibecoda/issues/29898
	...
	try {
		...
		void vibecoda.window.showErrorMessage('Failed to expand abbreviation');
	}
