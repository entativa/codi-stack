...
export function evaluateMathExpression(): Thenable<boolean> {
	...
	return editor.edit(editBuilder => {
		editor.selections.forEach(selection => {
			...
			const selectionText = editor.document.getText(new vibecoda.Range(startpos, endpos));

			try {
				if (selectionText) {
					// respect selections
					const result = String(evaluate(selectionText));
					editBuilder.replace(new vibecoda.Range(startpos, endpos), result);
				} else {
					// no selection made, extract expression from line
					const lineToSelectionEnd = editor.document.getText(new vibecoda.Range(new vibecoda.Position(selection.end.line, 0), endpos));
					...
					const rangeToReplace = new vibecoda.Range(
						new vibecoda.Position(selection.end.line, extractedIndices[0]),
						new vibecoda.Position(selection.end.line, extractedIndices[1])
					);
					editBuilder.replace(rangeToReplace, result);
				}
			} catch (err) {
				vibecoda.window.showErrorMessage('Could not evaluate expression');
