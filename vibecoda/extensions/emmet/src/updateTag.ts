...
export async function updateTag(tagName: string | undefined): Promise<boolean | undefined> {
	...
	if (tagName === undefined) {
		tagName = await vibecoda.window.showInputBox({
			prompt: 'Enter Tag',
...
function getRangesFromNode(node: HtmlFlatNode, document: vibecoda.TextDocument): TagRange[] {
	const ranges: TagRange[] = [];
	if (node.open) {
		const start = document.positionAt(node.open.start);
		ranges.push({
			name: node.name,
			range: new vibecoda.Range(start.translate(0, 1), start.translate(0, 1).translate(0, node.name.length))
		});
	}
	if (node.close) {
		...
		ranges.push({
			name: node.name,
			range: new vibecoda.Range(endTagStart.translate(0, 2), end.translate(0, -1))
		});
	}
	return ranges;
}

function getRangesToUpdate(document: vibecoda.TextDocument, selection: vibecoda.Selection, rootNode: HtmlFlatNode): TagRange[] {
