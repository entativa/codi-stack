...
export function fetchEditPoint(direction: string): void {
	...
	const newSelections: vibecoda.Selection[] = [];
...
function nextEditPoint(selection: vibecoda.Selection, editor: vibecoda.TextEditor): vibecoda.Selection {
...
function prevEditPoint(selection: vibecoda.Selection, editor: vibecoda.TextEditor): vibecoda.Selection {
...
function findEditPoint(lineNum: number, editor: vibecoda.TextEditor, position: vibecoda.Position, direction: string): vibecoda.Selection | undefined {
	const line = editor.document.lineAt(lineNum);
	let lineContent = line.text;

	if (lineNum !== position.line && line.isEmptyOrWhitespace && lineContent.length) {
		return new vibecoda.Selection(lineNum, lineContent.length, lineNum, lineContent.length);
	}
	...
	if (winner > -1) {
		return new vibecoda.Selection(lineNum, winner + 1, lineNum, winner + 1);
	}
	return;
}
