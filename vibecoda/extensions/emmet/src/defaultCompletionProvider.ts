...
import { Range as LSRange } from 'vibecoda-languageserver-textdocument';
import { getRootNode } from './parseDocument';

export class DefaultCompletionItemProvider implements vibecoda.CompletionItemProvider {

	private lastCompletionType: string | undefined;

	public provideCompletionItems(document: vibecoda.TextDocument, position: vibecoda.Position, _: vibecoda.CancellationToken, context: vibecoda.CompletionContext): Thenable<vibecoda.CompletionList | undefined> | undefined {
	...
	private provideCompletionItemsInternal(document: vibecoda.TextDocument, position: vibecoda.Position, context: vibecoda.CompletionContext): Thenable<vibecoda.CompletionList | undefined> | undefined {
		...
		const lineRange = new vibecoda.Range(position.line, 0, position.line, position.character);
		...
		if (syntax === 'html') {
			if (context.triggerKind === vibecoda.CompletionTriggerKind.TriggerForIncompleteCompletions) {
		...
		if (isStyleSheet(document.languageId) && context.triggerKind !== vibecoda.CompletionTriggerKind.TriggerForIncompleteCompletions) {
		...
		// Fix for https://github.com/microsoft/vibecoda/issues/107578
		...
		if (!isStyleSheet(document.languageId) && isStyleSheet(syntax) && context.triggerKind !== vibecoda.CompletionTriggerKind.TriggerForIncompleteCompletions) {
			validateLocation = true;
		...
		// Fix for https://github.com/microsoft/vibecoda/issues/32647
		...
		if (!isStyleSheet(syntax) && (document.languageId === 'javascript' || document.languageId === 'javascriptreact' || document.languageId === 'typescript' || document.languageId === 'typescriptreact')) {
			...
			if (abbreviation.startsWith('this.') || /\[[^\]=]*\]/.test(abbreviation)) {
				...
				isNoisePromise = vibecoda.commands.executeCommand<vibecoda.SymbolInformation[] | undefined>('vibecoda.executeDocumentSymbolProvider', document.uri).then(symbols => {
					return !!symbols && symbols.some(x => abbreviation === x.name || (abbreviation.startsWith(x.name + '.') && !/>|\*|\+/.test(abbreviation)));
				});
			}
		}

		return isNoisePromise.then((isNoise): vibecoda.CompletionList | undefined => {
			if (isNoise) {
				return undefined;
			}
			...
			const newItems: vibecoda.CompletionItem[] = [];
			if (result && result.items) {
				result.items.forEach((item: any) => {
					const newItem = new vibecoda.CompletionItem(item.label);
					newItem.documentation = item.documentation;
					newItem.detail = item.detail;
					newItem.insertText = new vibecoda.SnippetString(item.textEdit.newText);
					const oldrange = item.textEdit.range;
					newItem.range = new vibecoda.Range(oldrange.start.line, oldrange.start.character, oldrange.end.line, oldrange.end.character);

					newItem.filterText = item.filterText;
					newItem.sortText = item.sortText;

					if (emmetConfig['showSuggestionsAsSnippets'] === true) {
						newItem.kind = vibecoda.CompletionItemKind.Snippet;
					}
					newItems.push(newItem);
				});
			}

			return new vibecoda.CompletionList(newItems, true);
		});
	}
}

function toRange(lsRange: LSRange) {
	return new vibecoda.Range(lsRange.start.line, lsRange.start.character, lsRange.end.line, lsRange.end.character);
}
