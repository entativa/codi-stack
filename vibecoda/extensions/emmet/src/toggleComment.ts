...
export function toggleComment(): Thenable<boolean> | undefined {
	...
	return editor.edit(editBuilder => {
		const allEdits: vibecoda.TextEdit[][] = [];
		...
		let lastEditPosition = new vibecoda.Position(0, 0);
...
function toggleCommentHTML(document: vibecoda.TextDocument, selection: vibecoda.Selection, rootNode: Node): vibecoda.TextEdit[] {
	...
	edits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, allNodes[0].start, allNodes[0].start), startCommentHTML));
	edits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, allNodes[allNodes.length - 1].end, allNodes[allNodes.length - 1].end), endCommentHTML));

	return edits;
}

function getRangesToUnCommentHTML(node: Node, document: vibecoda.TextDocument): vibecoda.TextEdit[] {
	let unCommentTextEdits: vibecoda.TextEdit[] = [];

	// If current node is commented, then uncomment and return
	if (node.type === 'comment') {
		unCommentTextEdits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, node.start, node.start + startCommentHTML.length), ''));
		unCommentTextEdits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, node.end - endCommentHTML.length, node.end), ''));
...
function toggleCommentStylesheet(document: vibecoda.TextDocument, selection: vibecoda.Selection, rootNode: Stylesheet): vibecoda.TextEdit[] {
	const selectionStart = selection.isReversed ? selection.active : selection.anchor;
	const selectionEnd = selection.isReversed ? selection.anchor : selection.active;
	...
	const rangesToUnComment: vibecoda.Range[] = [];
	const edits: vibecoda.TextEdit[] = [];
	rootNode.comments.forEach(comment => {
		...
		if (selection.intersection(commentRange)) {
			...
			edits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, comment.start, comment.start + startCommentStylesheet.length), ''));
			edits.push(new vibecoda.TextEdit(offsetRangeToVsRange(document, comment.end - endCommentStylesheet.length, comment.end), ''));
		}
	});
	...
	return [
		new vibecoda.TextEdit(new vibecoda.Range(selection.start, selection.start), startCommentStylesheet),
		new vibecoda.TextEdit(new vibecoda.Range(selection.end, selection.end), endCommentStylesheet)
	];
}

function setupCommentSpacing() {
	const config: boolean | undefined = vibecoda.workspace.getConfiguration('editor.comments').get('insertSpace');
	if (config) {
