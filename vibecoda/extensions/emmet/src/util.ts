...
import * as EmmetHelper from '@vibecoda/emmet-helper';
import { TextDocument as LSTextDocument } from 'vibecoda-languageserver-textdocument';
import { getRootNode } from './parseDocument';

let _emmetHelper: typeof EmmetHelper;
let _currentExtensionsPath: string[] | undefined;

let _homeDir: vibecoda.Uri | undefined;


export function setHomeDir(homeDir: vibecoda.Uri) {
	_homeDir = homeDir;
}

export function getEmmetHelper() {
	// Lazy load vibecoda-emmet-helper instead of importing it
	// directly to reduce the start-up time of the extension
	if (!_emmetHelper) {
		_emmetHelper = require('@vibecoda/emmet-helper');
	}
	return _emmetHelper;
}
...
export function updateEmmetExtensionsPath(forceRefresh: boolean = false) {
	...
	let extensionsPath = vibecoda.workspace.getConfiguration('emmet').get<string[]>('extensionsPath');
	if (!extensionsPath) {
		extensionsPath = [];
	}
	if (forceRefresh || _currentExtensionsPath !== extensionsPath) {
		_currentExtensionsPath = extensionsPath;
		const rootPaths = vibecoda.workspace.workspaceFolders?.length ? vibecoda.workspace.workspaceFolders.map(f => f.uri) : undefined;
		const fileSystem = vibecoda.workspace.fs;
 ...
 * https://github.com/microsoft/vibecoda/issues/117517
 */
export function migrateEmmetExtensionsPath() {
	// Get the detail info of emmet.extensionsPath setting
	const config = vibecoda.workspace.getConfiguration().inspect('emmet.extensionsPath');

	// Update Global setting if the value type is string or the value is null
	if (typeof config?.globalValue === 'string') {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', [config.globalValue], true);
	} else if (config?.globalValue === null) {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', [], true);
	}
	// Update Workspace setting if the value type is string or the value is null
	if (typeof config?.workspaceValue === 'string') {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', [config.workspaceValue], false);
	} else if (config?.workspaceValue === null) {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', [], false);
	}
	...
	if (typeof config?.workspaceFolderValue === 'string') {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', [config.workspaceFolderValue]);
	} else if (config?.workspaceFolderValue === null) {
		vibecoda.workspace.getConfiguration().update('emmet.extensionsPath', []);
	}
}
...
export function validate(allowStylesheet: boolean = true): boolean {
	...
	if (!editor) {
		vibecoda.window.showInformationMessage('No editor is active');
		return false;
	}
...
export function getMappingForIncludedLanguages(): Record<string, string> {
	// Explicitly map languages that have built-in grammar in Vibecoda to their parent language
	...
	const includeLanguagesConfig = vibecoda.workspace.getConfiguration('emmet').get<Record<string, string>>('includeLanguages');
...
/**
* Get the corresponding emmet mode for given vibecoda language mode
 ...
 * @param document vibecoda.TextDocument
 * @param position vibecoda.Position
 */
export function parsePartialStylesheet(document: vibecoda.TextDocument, position: vibecoda.Position): FlatStylesheet | undefined {
	...
	function consumeLineCommentBackwards() {
		...
		if (!isCSS && currentLine !== posLineNumber) {
			...
			if (startLineComment > -1) {
				stream.pos = document.offsetAt(new vibecoda.Position(currentLine, startLineComment));
			}
		}
	}
	...
	function consumeCommentForwards() {
		if (stream.eat(slash)) {
			if (stream.eat(slash) && !isCSS) {
				const posLineNumber = document.positionAt(stream.pos).line;
				stream.pos = document.offsetAt(new vibecoda.Position(posLineNumber + 1, 0));
...
export function offsetRangeToSelection(document: vibecoda.TextDocument, start: number, end: number): vibecoda.Selection {
	const startPos = document.positionAt(start);
	const endPos = document.positionAt(end);
	return new vibecoda.Selection(startPos, endPos);
}

export function offsetRangeToVsRange(document: vibecoda.TextDocument, start: number, end: number): vibecoda.Range {
	const startPos = document.positionAt(start);
	const endPos = document.positionAt(end);
	return new vibecoda.Range(startPos, endPos);
}
...
export function getEmmetConfiguration(syntax: string) {
	const emmetConfig = vibecoda.workspace.getConfiguration('emmet');
 ...
 * be found
 */
export function getCssPropertyFromDocument(editor: vibecoda.TextEditor, position: vibecoda.Position): FlatProperty | null {
...
export function getEmbeddedCssNodeIfAny(document: vibecoda.TextDocument, currentNode: FlatNode | undefined, position: vibecoda.Position): FlatNode | undefined {
	if (!currentNode) {
		return;
	}
...
export function toLSTextDocument(doc: vibecoda.TextDocument): LSTextDocument {
