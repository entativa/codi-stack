...
vibecoda.workspace.registerFileSystemProvider(testFs.scheme, testFs, { isCaseSensitive: testFs.isCaseSensitive });

export async function createRandomFile(contents: string | Uint8Array = '', dir: vibecoda.Uri | undefined = undefined, ext = ''): Promise<vibecoda.Uri> {
	let fakeFile: vibecoda.Uri;
	if (dir) {
		assert.strictEqual(dir.scheme, testFs.scheme);
		fakeFile = dir.with({ path: dir.path + '/' + rndName() + ext });
	} else {
		fakeFile = vibecoda.Uri.parse(`${testFs.scheme}:/${rndName() + ext}`);
	}
	testFs.writeFile(fakeFile, typeof contents === 'string' ? Buffer.from(contents) : Buffer.from(contents), { create: true, overwrite: true });
	return fakeFile;
}

export async function deleteFile(file: vibecoda.Uri): Promise<boolean> {
	try {
		testFs.delete(file);
		return true;
	} catch {
...
export function saveAllEditors(): Thenable<any> {
	return vibecoda.commands.executeCommand('workbench.action.files.saveAll');
}

export async function revertAllDirty(): Promise<void> {
	return vibecoda.commands.executeCommand('_workbench.revertAllDirty');
}

export function disposeAll(disposables: vibecoda.Disposable[]) {
	vibecoda.Disposable.from(...disposables).dispose();
}
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<void> {
	return async (): Promise<void> => {
		const logLevel = await vibecoda.commands.executeCommand('_extensionTests.getLogLevel');
		await vibecoda.commands.executeCommand('_extensionTests.setLogLevel', level);

		try {
			await runnable();
		} finally {
			await vibecoda.commands.executeCommand('_extensionTests.setLogLevel', logLevel);
		}
	};
}
...
export function assertNoRpc() {
	assertNoRpcFromEntry([vibecoda, 'vibecoda']);
}
...
export async function asPromise<T>(event: vibecoda.Event<T>, timeout = vibecoda.env.uiKind === vibecoda.UIKind.Desktop ? 5000 : 15000): Promise<T> {
