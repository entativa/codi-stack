...
suite('vibecoda API - configuration', () => {

	teardown(assertNoRpc);

	test('configurations, language defaults', function () {
		const defaultLanguageSettings = vibecoda.workspace.getConfiguration().get('[abcLang]');
	...
	test('configuration, defaults', () => {
		const config = vibecoda.workspace.getConfiguration('farboo');
