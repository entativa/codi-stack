...
import { env, ExtensionKind, extensions, UIKind, Uri } from 'vibecoda';
import { assertNoRpc, Mutable } from '../utils';

suite('vibecoda API - env', () => {
	...
	test('env.remoteName', function () {
		...
		const knownWorkspaceExtension = extensions.getExtension('vibecoda.git');
		const knownUiAndWorkspaceExtension = extensions.getExtension('vibecoda.media-preview');
	...
	test('env.uiKind', async function () {
		const uri = Uri.parse(`${env.uriScheme}:://vibecoda.vibecoda-api-tests/path?key=value&other=false`);
