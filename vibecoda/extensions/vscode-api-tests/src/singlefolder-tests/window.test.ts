...
import { CancellationTokenSource, commands, MarkdownString, TabInputNotebook, Position, QuickPickItem, Selection, StatusBarAlignment, TextEditor, TextEditorSelectionChangeKind, TextEditorViewColumnChangeEvent, TabInputText, Uri, ViewColumn, window, workspace, TabInputTextDiff, UIKind, env } from 'vibecoda';
import { assertNoRpc, closeAllEditors, createRandomFile, pathEquals } from '../utils';


suite('vibecoda API - window', () => {
	...
	test('issue #27408 - showTextDocument & vibecoda.diff always default to ViewColumn.One', async () => {
		const [docA, docB, docC] = await Promise.all([
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile())
	...
	test('Tabs - vibecoda.open & vibecoda.diff', async function () {
		...
		await commands.executeCommand('vibecoda.open', commandFile, ViewColumn.Three);
		// Ensure active tab is correct after calling vibecoda.open
		...
		await commands.executeCommand('vibecoda.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Four, preview: false });
	...
	(env.uiKind === UIKind.Web ? test.skip : test)('Tabs - Ensure tabs getter is correct', async function () {
		...
		const leftDiff = await createRandomFile();
		const rightDiff = await createRandomFile();
		await commands.executeCommand('vibecoda.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Three, preview: false });
	...
	// TODO@lramos15 https://github.com/microsoft/vibecoda/issues/145846
	...
	*/
	...
	test('showQuickPick, keep selection (microsoft/vibecoda-azure-account#67)', async function () {
	...
	test('createStatusBar', async function () {
		...
		assert.strictEqual(statusBarEntryWithoutId.id, 'vibecoda.vibecoda-api-tests');
