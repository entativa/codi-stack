...
suite('lm', function () {
	...
	const testProviderOptions: vibecoda.LanguageModelChatInformation = {
		id: 'test-lm',
		name: 'test-lm',
		version: '1.0.0',
		family: 'test',
	...
	test('lm request and stream', async function () {
		...
		let p: vibecoda.Progress<vibecoda.LanguageModelResponsePart> | undefined;
		const defer = new DeferredPromise<void>();

		try {
			disposables.push(vibecoda.lm.registerLanguageModelChatProvider('test-lm-vendor', {
		...
		const models = await vibecoda.lm.selectChatModels({ id: 'test-lm' });
		assert.strictEqual(models.length, 1);

		const request = await models[0].sendRequest([vibecoda.LanguageModelChatMessage.User('Hello')]);
		...
		p.report(new vibecoda.LanguageModelTextPart('Hello'));
		defer.complete();
	...
	test('lm request fail', async function () {
		...
		try {
			await models[0].sendRequest([vibecoda.LanguageModelChatMessage.User('Hello')]);
	...
	test('lm stream fail', async function () {
		...
		const res = await models[0].sendRequest([vibecoda.LanguageModelChatMessage.User('Hello')]);
		assert.ok(res);
	...
	test('LanguageModelError instance is not thrown to extensions#235322 (SYNC)', async function () {
		...
		disposables.push(vibecoda.lm.registerLanguageModelChatProvider('test-lm-vendor', {
			...
			provideLanguageModelChatResponse(_model, _messages, _options, _progress, _token) {
				throw vibecoda.LanguageModelError.Blocked('You have been blocked SYNC');
			},
			async provideTokenCount(_model, _text, _token) {
				return 1;
			}
		...
		try {
			...
			assert.ok(error instanceof vibecoda.LanguageModelError);
	...
	test('LanguageModelError instance is not thrown to extensions#235322 (ASYNC)', async function () {
		...
		disposables.push(vibecoda.lm.registerLanguageModelChatProvider('test-lm-vendor', {
			...
			async provideLanguageModelChatResponse(_model, _messages, _options, _progress, _token) {
				throw vibecoda.LanguageModelError.Blocked('You have been blocked ASYNC');
			},
			async provideTokenCount(_model, _text, _token) {
				return 1;
			}
		...
		const response = await models[0].sendRequest([vibecoda.LanguageModelChatMessage.User('Hello')]);
		assert.ok(response);
