...
import { resetCaches } from '@vibecoda/proxy-agent';
...
(vibecoda.env.uiKind === vibecoda.UIKind.Web ? suite.skip : suite)('vibecoda API - network proxy support', () => {
	...
	test('basic auth', async () => {
		...
		try {
			...
			await vibecoda.workspace.getConfiguration().update('http.proxy', `http://127.0.0.1:${proxyPort}`, vibecoda.ConfigurationTarget.Global);
			await change;
			await new Promise<void>((resolve, reject) => {
				https.get(url, res => {
			...
			sf.close();
			const change = waitForConfigChange('http.proxy');
			await vibecoda.workspace.getConfiguration().update('http.proxy', undefined, vibecoda.ConfigurationTarget.Global);
			await change;
			await vibecoda.workspace.getConfiguration().update('integration-test.http.proxyAuth', undefined, vibecoda.ConfigurationTarget.Global);
		}
	});

	(vibecoda.env.remoteName ? test : test.skip)('separate local / remote proxy settings', async () => {
		// Assumes test resolver runs with `--use-host-proxy`.
		const localProxy = 'http://localhost:1234';
		const remoteProxy = 'http://localhost:4321';

		const actualLocalProxy1 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p1 = waitForConfigChange('http.proxy');
		await vibecoda.workspace.getConfiguration().update('http.proxy', localProxy, vibecoda.ConfigurationTarget.Global);
		await p1;
		const actualLocalProxy2 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p2 = waitForConfigChange('http.useLocalProxyConfiguration');
		await vibecoda.workspace.getConfiguration().update('http.useLocalProxyConfiguration', false, vibecoda.ConfigurationTarget.Global);
		await p2;
		const actualRemoteProxy1 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p3 = waitForConfigChange('http.proxy');
		await vibecoda.workspace.getConfiguration().update('http.proxy', remoteProxy, vibecoda.ConfigurationTarget.Global);
		await p3;
		const actualRemoteProxy2 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p4 = waitForConfigChange('http.proxy');
		await vibecoda.workspace.getConfiguration().update('http.proxy', undefined, vibecoda.ConfigurationTarget.Global);
		await p4;
		const actualRemoteProxy3 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p5 = waitForConfigChange('http.proxy');
		await vibecoda.workspace.getConfiguration().update('http.useLocalProxyConfiguration', true, vibecoda.ConfigurationTarget.Global);
		await p5;
		const actualLocalProxy3 = vibecoda.workspace.getConfiguration().get('http.proxy');

		const p6 = waitForConfigChange('http.proxy');
		await vibecoda.workspace.getConfiguration().update('http.proxy', undefined, vibecoda.ConfigurationTarget.Global);
		await p6;
		const actualLocalProxy4 = vibecoda.workspace.getConfiguration().get('http.proxy');
	...
	function waitForConfigChange(key: string) {
		return new Promise<void>(resolve => {
			const s = vibecoda.workspace.onDidChangeConfiguration(e => {
