...
suite.skip('vibecoda API - Copy Paste', function () {
	...
	test('Copy should be able to overwrite text/plain', async () => {
		...
		editor.selections = [new vibecoda.Selection(0, 1, 0, 6)];

		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				const existing = dataTransfer.get(textPlain);
				if (existing) {
					const str = await existing.asString();
					const reversed = reverseString(str);
					dataTransfer.set(textPlain, new vibecoda.DataTransferItem(reversed));
				}
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));

		await vibecoda.commands.executeCommand('editor.action.clipboardCopyAction');
		const newDocContent = getNextDocumentText(testDisposables, doc);
		await vibecoda.commands.executeCommand('editor.action.clipboardPasteAction');
	...
	test('Copy with multiple selections should get all selections', async () => {
		...
		editor.selections = [
			new vibecoda.Selection(0, 0, 0, 3),
			new vibecoda.Selection(2, 0, 2, 3),
		];

		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(document: vibecoda.TextDocument, ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				const existing = dataTransfer.get(textPlain);
				if (existing) {
					const selections = ranges.map(range => document.getText(range));
					dataTransfer.set(textPlain, new vibecoda.DataTransferItem(`(${ranges.length})${selections.join(' ')}`));
				}
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));

		await vibecoda.commands.executeCommand('editor.action.clipboardCopyAction');
		editor.selections = [new vibecoda.Selection(0, 0, 0, 0)];
		const newDocContent = getNextDocumentText(testDisposables, doc);
		await vibecoda.commands.executeCommand('editor.action.clipboardPasteAction');
	...
	test('Earlier invoked copy providers should win when writing values', async () => {
		...
		const editor = await vibecoda.window.showTextDocument(doc);
		editor.selections = [new vibecoda.Selection(0, 0, 0, 3)];
		...
		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				callOrder.push(a_id);
				dataTransfer.set(textPlain, new vibecoda.DataTransferItem('a'));
				providerAResolve();
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));
		...
		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				...
				dataTransfer.set(textPlain, new vibecoda.DataTransferItem('b'));
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));
	...
	test('Copy providers should not be able to effect the data transfer of another', async () => {
		...
		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				dataTransfer.set(textPlain, new vibecoda.DataTransferItem('xyz'));
				providerAResolve();
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));
		...
		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
				...
				dataTransfer.set(textPlain, new vibecoda.DataTransferItem(reverseString(str)));
			}
		}, { providedPasteEditKinds: [vibecoda.DocumentDropOrPasteEditKind.Empty.append('test')], copyMimeTypes: [textPlain] }));
	...
	test('One failing provider should not effect other', async () => {
		...
		testDisposables.push(vibecoda.languages.registerDocumentPasteEditProvider({ language: 'plaintext' }, new class implements vibecoda.DocumentPasteEditProvider {
			async prepareDocumentPaste(_document: vibecoda.TextDocument, _ranges: readonly vibecoda.Range[], _dataTransfer: vibecoda.DataTransfer, _token: vibecoda.CancellationToken): Promise<void> {
...
function getNextDocumentText(disposables: vibecoda.Disposable[], doc: vibecoda.TextDocument): Promise<string> {
