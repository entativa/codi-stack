...
function getOutputChannel(): vibecoda.OutputChannel {
	if (!_channel) {
		_channel = vibecoda.window.createOutputChannel('Jake Auto Detection');
	}
	return _channel;
}

function showError() {
	vibecoda.window.showWarningMessage(vibecoda.l10n.t("Problem finding jake tasks. See the output for more information."),
...
interface JakeTaskDefinition extends vibecoda.TaskDefinition {
	task: string;
	file?: string;
}
...
class FolderDetector {
	...
	public isEnabled(): boolean {
		return vibecoda.workspace.getConfiguration('jake', this._workspaceFolder.uri).get<AutoDetect>('autoDetect') === 'on';
	}
	...
	public async getTask(_task: vibecoda.Task): Promise<vibecoda.Task | undefined> {
		...
		if (jakeTask) {
			...
			const task = new vibecoda.Task(kind, this.workspaceFolder, jakeTask, 'jake', new vibecoda.ShellExecution(await this._jakeCommand, [jakeTask], options));
			return task;
		}
		return undefined;
	}
	...
	private async computeTasks(): Promise<vibecoda.Task[]> {
		...
		try {
			...
			if (stdout) {
				...
				for (const line of lines) {
					...
					if (matches && matches.length === 2) {
						...
						const options: vibecoda.ShellExecutionOptions = { cwd: this.workspaceFolder.uri.fsPath };
						const task = new vibecoda.Task(kind, taskName, 'jake', new vibecoda.ShellExecution(`${await this._jakeCommand} ${taskName}`, options));
						result.push(task);
			...
			channel.appendLine(vibecoda.l10n.t("Auto detecting Jake for folder {0} failed with error: {1}', this.workspaceFolder.name, err.error ? err.error.toString() : 'unknown"));
			showError();
			return emptyTasks;
		}
	}
...
class TaskDetector {
	...
	private updateProvider(): void {
		if (!this.taskProvider && this.detectors.size > 0) {
			...
			this.taskProvider = vibecoda.tasks.registerTaskProvider('jake', {
