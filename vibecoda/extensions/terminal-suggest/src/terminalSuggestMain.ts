...
let globalStorageUri: vibecoda.Uri;
	...
	if (cached) {
		...
		if (now - cached.timestamp > CACHE_MAX_AGE_MS) {
			...
			if (!shouldRefresh && cached.commands) {
				...
				// (https://github.com/microsoft/vibecoda/issues/259343).
...
async function writeGlobalsCache(): Promise<void> {
	...
	try {
		...
		const terminalSuggestDir = vibecoda.Uri.joinPath(globalStorageUri, 'terminal-suggest');
		await vibecoda.workspace.fs.createDirectory(terminalSuggestDir);
		const cacheFile = vibecoda.Uri.joinPath(terminalSuggestDir, `${CACHE_KEY}.json`);
		const data = Buffer.from(JSON.stringify(obj), 'utf8');
		await vibecoda.workspace.fs.writeFile(cacheFile, data);
	} catch (err) {
...
async function readGlobalsCache(): Promise<void> {
	...
	try {
		const terminalSuggestDir = vibecoda.Uri.joinPath(globalStorageUri, 'terminal-suggest');
		const cacheFile = vibecoda.Uri.joinPath(terminalSuggestDir, `${CACHE_KEY}.json`);
		const data = await vibecoda.workspace.fs.readFile(cacheFile);
		...
		if (err instanceof vibecoda.FileSystemError && err.code === 'FileNotFound') {
...
export async function activate(context: vibecoda.ExtensionContext) {
	...
	const machineId = await vibecoda.env.machineId;
	const remoteAuthority = vibecoda.env.remoteName;

	context.subscriptions.push(vibecoda.window.registerTerminalCompletionProvider({
		async provideTerminalCompletions(terminal: vibecoda.Terminal, terminalContext: vibecoda.TerminalCompletionContext, token: vibecoda.CancellationToken): Promise<vibecoda.TerminalCompletionItem[] | vibecoda.TerminalCompletionList | undefined> {
			...
			if (terminal.shellIntegration?.env) {
				...
				if (homeDirCompletion && terminal.shellIntegration.env?.value?.HOME) {
					homeDirCompletion.documentation = getFriendlyResourcePath(vibecoda.Uri.file(terminal.shellIntegration.env.value.HOME), pathSeparator, vibecoda.TerminalCompletionItemKind.Folder);
					homeDirCompletion.kind = vibecoda.TerminalCompletionItemKind.Folder;
				}
			}
			...
			if (cwd && (result.showFiles || result.showFolders)) {
				...
				return new vibecoda.TerminalCompletionList(result.items, {
	...
	context.subscriptions.push(vibecoda.commands.registerCommand('terminal.integrated.suggest.clearCachedGlobals', () => {
		cachedGlobals.clear();
	}));
}
 ...
 */
export async function resolveCwdFromCurrentCommandString(currentCommandString: string, currentCwd?: vibecoda.Uri): Promise<vibecoda.Uri | undefined> {
	...
	try {
		...
		// Use vibecoda.Uri.joinPath for path resolution
		const resolvedUri = vibecoda.Uri.joinPath(currentCwd, relativeFolder);

		const stat = await vibecoda.workspace.fs.stat(resolvedUri);
...
export async function getCompletionItemsFromSpecs(
	specs: Fig.Spec[],
	terminalContext: vibecoda.TerminalCompletionContext,
	availableCommands: ICompletionResource[],
	currentCommandString: string,
	tokenType: TokenType,
	shellIntegrationCwd: vibecoda.Uri | undefined,
	env: Record<string, string>,
	name: string,
	token?: vibecoda.CancellationToken,
	executeExternals?: IFigExecuteExternals,
): Promise<{ items: vibecoda.TerminalCompletionItem[]; showFiles: boolean; showFolders: boolean; fileExtensions?: string[]; cwd?: vibecoda.Uri }> {
	let items: vibecoda.TerminalCompletionItem[] = [];
	let showFiles = false;
		...
		for (const command of availableCommands) {
			...
			if (!labels.has(labelWithoutExtension)) {
				items.push(createCompletionItem(
					...
					vibecoda.TerminalCompletionItemKind.Method
				));
	...
	let cwd: vibecoda.Uri | undefined;
...
function getTerminalShellType(shellType: string | undefined): TerminalShellType | undefined {
	switch (shellType) {
		...
		case 'pwsh':
			return basename(vibecoda.env.shell, '.exe') === 'powershell' ? TerminalShellType.WindowsPowerShell : TerminalShellType.PowerShell;
		case 'fish':
