...
export interface IFigSpecSuggestionsResult {
	...
	items: vibecoda.TerminalCompletionItem[];
}
	...
	for (const spec of specs) {
		...
		for (const specLabel of specLabels) {
			...
			if (tokenType === TokenType.Command) {
				if (availableCommand.kind !== vibecoda.TerminalCompletionItemKind.Alias) {
					...
					result.items.push(createCompletionItem(
						...
						currentCommandAndArgString,
						{
							...
							kind: vibecoda.TerminalCompletionItemKind.Method
						},
						description,
...
async function getFigSpecSuggestions(
	...
	const items: vibecoda.TerminalCompletionItem[] = [];
...
export async function collectCompletionItemResult(
	...
	const addSuggestions = async (specArgs: SpecArg[] | Record<string, SpecArg> | undefined, kind: vibecoda.TerminalCompletionItemKind, parsedArguments?: ArgumentParserResult) => {
		if (kind === vibecoda.TerminalCompletionItemKind.Argument && parsedArguments?.currentArg?.generators) {
		...
		const flagsToExclude = kind === vibecoda.TerminalCompletionItemKind.Flag ? parsedArguments?.passedOptions.map(option => option.name).flat() : undefined;
		...
		function addItem(label: string, item: SpecArg) {
			...
			if (lastArgType === 'subcommand_arg') {
				if (typeof item === 'object' && 'args' in item && (asArray(item.args ?? [])).length > 0) {
					itemKind = vibecoda.TerminalCompletionItemKind.Option;
				}
			}
			else if (lastArgType === 'option_arg') {
				itemKind = vibecoda.TerminalCompletionItemKind.OptionValue;
			}
	...
	if (parsedArguments.suggestionFlags & SuggestionFlag.Args) {
		await addSuggestions(parsedArguments.currentArg?.suggestions, vibecoda.TerminalCompletionItemKind.Argument, parsedArguments);
	}
	if (parsedArguments.suggestionFlags & SuggestionFlag.Subcommands) {
		await addSuggestions(parsedArguments.completionObj.subcommands, vibecoda.TerminalCompletionItemKind.Method);
	}
	if (parsedArguments.suggestionFlags & SuggestionFlag.Options) {
		await addSuggestions(parsedArguments.completionObj.options, vibecoda.TerminalCompletionItemKind.Flag, parsedArguments);
	}
...
function convertIconToKind(icon: string | undefined): vibecoda.TerminalCompletionItemKind | undefined {
	switch (icon) {
		case 'vibecoda://icon?type=10': return vibecoda.TerminalCompletionItemKind.ScmCommit;
		case 'vibecoda://icon?type=11': return vibecoda.TerminalCompletionItemKind.ScmBranch;
		case 'vibecoda://icon?type=12': return vibecoda.TerminalCompletionItemKind.ScmTag;
		case 'vibecoda://icon?type=13': return vibecoda.TerminalCompletionItemKind.ScmStash;
		case 'vibecoda://icon?type=14': return vibecoda.TerminalCompletionItemKind.ScmRemote;
		case 'vibecoda://icon?type=15': return vibecoda.TerminalCompletionItemKind.PullRequest;
		case 'vibecoda://icon?type=16': return vibecoda.TerminalCompletionItemKind.PullRequestDone;
