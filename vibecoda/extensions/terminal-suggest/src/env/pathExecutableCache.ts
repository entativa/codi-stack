...
export class PathExecutableCache implements vibecoda.Disposable {
	...
	constructor() {
		if (isWindows) {
			this._cachedWindowsExeExtensions = vibecoda.workspace.getConfiguration(SettingsIds.SuggestPrefix).get(SettingsIds.CachedWindowsExecutableExtensionsSuffixOnly);
			this._disposables.push(vibecoda.workspace.onDidChangeConfiguration(e => {
	...
	private async _getExecutablesInSinglePath(path: string, pathSeparator: string, labels: Set<string>): Promise<Set<ICompletionResource> | undefined> {
		try {
			...
			const fileResource = vibecoda.Uri.file(path);
			const files = await vibecoda.workspace.fs.readDirectory(fileResource);
			await Promise.all(
				files.map(([file, fileType]) => (async () => {
					let kind: vibecoda.TerminalCompletionItemKind | undefined;
					let formattedPath: string | undefined;
					const resource = vibecoda.Uri.joinPath(fileResource, file);

					// Skip unknown or directory file types early
					if (fileType === vibecoda.FileType.Unknown || fileType === vibecoda.FileType.Directory) {
						return;
					}
					...
					const isExec = kind === vibecoda.TerminalCompletionItemKind.Method || await isExecutable(formattedPath, this._cachedWindowsExeExtensions);
					if (!isExec) {
						return;
					}
					...
					result.add({
						...
						kind: kind ?? vibecoda.TerminalCompletionItemKind.Method
					});
					labels.add(file);
				})())
			);
...
export async function watchPathDirectories(context: vibecoda.ExtensionContext, env: ITerminalEnvironment, pathExecutableCache: PathExecutableCache | undefined): Promise<void> {
	...
	for (const dir of pathDirectories) {
		try {
			...
			context.subscriptions.push(new vibecoda.Disposable(() => {
				try {
					watcher.close();
