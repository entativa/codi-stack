...
export const extensionManagementOptions = (cliName: string): Fig.Option[] => [
	...
	},
	{
		...
		description:
			`Installs or updates an extension. The argument is either an extension id or a path to a VSIX. The identifier of an extension is '\${ publisher }.\${ name }'. Use '--force' argument to update to latest version. To install a specific version provide '@\${version}'. For example: 'vibecoda.csharp@1.2.3'`,
		args: {
...
export const troubleshootingOptions = (cliName: string): Fig.Option[] => [
	...
	{
		name: '--log',
		description: `Log level to use. Default is 'info'. Allowed values are 'critical', 'error', 'warn', 'info', 'debug', 'trace', 'off'. You can also configure the log level of an extension by passing extension id and log level in the following format: '{publisher}.{name}:{logLevel}'. For example: 'vibecoda.csharp:trace'. Can receive one or more such entries.`,
		isRepeatable: true,
		args: {
			name: 'level',
...
export const codeTunnelSubcommands: Fig.Subcommand[] = [
	{
		name: 'tunnel',
		description: 'Create a tunnel that\'s accessible on vibecoda.dev from anywhere. Run`code tunnel --help` for more usage info',
		subcommands: [
			{
				name: 'prune',
	...
	{
		name: 'serve-web',
		description: 'Runs a local web version of Vibecoda',
		options: [
			{
				name: '--host',
