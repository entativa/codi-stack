						...
						if (oldParts && oldParts.length > 1) {
							if (oldParts[0] === "*") {
								...
								return {
									...
									priority: 100,
									icon: `vibecoda://icon?type=${vibecoda.TerminalCompletionItemKind.ScmBranch}`
								};
							} else if (oldParts[0] === "+") {
								name = branch.replaceAll("+", "").trim();
							}
...
export const gitGenerators = {
		...
		postProcess: function (out) {
			...
			return lines.map((line) => {
				return {
					...
					icon: `vibecoda://icon?type=${vibecoda.TerminalCompletionItemKind.ScmCommit}`,
		...
		postProcess: function (out) {
			...
			return output.split("\n").map((file) => {
				return {
					...
					icon: `vibecoda://icon?type=${vibecoda.TerminalCompletionItemKind.ScmStash}`,
				};
			});
		},
	} satisfies Fig.Generator,
	...
	remotes: {
		...
		postProcess: function (out) {
			...
			return Object.keys(remoteURLs).map((remote) => {
				return {
					name: remote,
					icon: `vibecoda://icon?type=${vibecoda.TerminalCompletionItemKind.ScmRemote}`,
					description: "Remote",
				};
			});
		},
	...
	tags: {
		...
		postProcess: function (output) {
			return output.split("\n").map((tag) => ({
				name: tag,
				icon: `vibecoda://icon?type=${vibecoda.TerminalCompletionItemKind.ScmTag}`
			}));
		},
	} satisfies Fig.Generator,
