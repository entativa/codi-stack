...
import { TextDocument, JSONDocument, JSONSchema, getLanguageService, DocumentLanguageSettings, SchemaConfiguration, ClientCapabilities, Range, Position, SortOptions } from 'vibecoda-json-languageservice';
import { getLanguageModelCache } from './languageModelCache';
import { Utils, URI } from 'vibecoda-uri';
import * as l10n from '@vibecoda/l10n';
...
namespace VibecodaContentRequest {
	export const type: RequestType<string, string, any> = new RequestType('vibecoda/content');
}
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
	...
	function getSchemaRequestService(handledSchemas: string[] = ['https', 'http', 'file']) {
		...
		return (uri: string): Thenable<string> => {
			...
			return connection.sendRequest(VibecodaContentRequest.type, uri).then(responseText => {
				return responseText;
			}, error => {
	...
	connection.onRequest(ValidateContentRequest.type, async ({ schemaUri, content }) => {
		const docURI = 'vibecoda://schemas/temp/' + new Date().getTime();
		...
		if (jsonConfigurationSettings) {
			jsonConfigurationSettings.forEach((schema, index) => {
				...
				if (!uri && schema.schema) {
					uri = schema.schema.id || `vibecoda://schemas/custom/${index}`;
				}
				if (uri) {
	...
	connection.onDidChangeWatchedFiles((change) => {
		// Monitored files have changed in Vibecoda
