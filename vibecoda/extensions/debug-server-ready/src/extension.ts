...
interface ServerReadyAction {
	...
	webRoot?: string;
	name?: string;
	config?: vibecoda.DebugConfiguration;
 ...
 * @see https://github.com/microsoft/vibecoda/blob/22a2a0e833175c32a2005b977d7fbd355582e416/src/vs/base/common/strings.ts#L736
 */
...
class ServerReadyDetector extends vibecoda.Disposable {

	private static detectors = new Map<vibecoda.DebugSession, ServerReadyDetector>();
	private static terminalDataListener: vibecoda.Disposable | undefined;

	private readonly stoppedEmitter = new vibecoda.EventEmitter<void>();
	private readonly onDidSessionStop = this.stoppedEmitter.event;
	private readonly disposables = new Set<vibecoda.Disposable>([]);
	private trigger: Trigger;
	private shellPid?: number;
	private regexp: RegExp;

	static start(session: vibecoda.DebugSession): ServerReadyDetector | undefined {
	...
	static stop(session: vibecoda.DebugSession): void {
	...
	static rememberShellPid(session: vibecoda.DebugSession, pid: number) {
		const detector = ServerReadyDetector.detectors.get(session);
		if (detector) {
	...
	static async startListeningTerminalData() {
		if (!this.terminalDataListener) {
			this.terminalDataListener = vibecoda.window.onDidWriteTerminalData(async e => {
	...
	private constructor(private session: vibecoda.DebugSession) {
	...
	private openExternalWithString(session: vibecoda.DebugSession, captureString: string) {
		...
		if (captureString === '') {
			...
			if (format.indexOf('%s') >= 0) {
				const errMsg = vibecoda.l10n.t("Format uri ('{0}') uses a substitution placeholder but pattern did not capture anything.", format);
				vibecoda.window.showErrorMessage(errMsg, { modal: true }).then(_ => undefined);
				return;
			}
			uri = format;
		} else {
			...
			if (s.length !== 2) {
				const errMsg = vibecoda.l10n.t("Format uri ('{0}') must contain exactly one substitution placeholder.", format);
				vibecoda.window.showErrorMessage(errMsg, { modal: true }).then(_ => undefined);
				return;
			}
	...
	private async openExternalWithUri(session: vibecoda.DebugSession, uri: string) {

		const args: ServerReadyAction = session.configuration.serverReadyAction;
		switch (args.action || 'openExternally') {

			case 'openExternally':
				await vibecoda.env.openExternal(vibecoda.Uri.parse(uri));
				break;
	...
	private async debugWithBrowser(type: string, session: vibecoda.DebugSession, uri: string) {
		...
		const cts = new vibecoda.CancellationTokenSource();
		...
		const stopListener = this.onDidSessionStop(async () => {
			...
			await vibecoda.debug.stopDebugging(createdSession);
		});
		this.disposables.add(stopListener);
	}

	private startBrowserDebugSession(type: string, session: vibecoda.DebugSession, uri: string, trackerId?: string) {
		return vibecoda.debug.startDebugging(session.workspaceFolder, {
			type,
			name: 'Browser Debug',
			request: 'launch',
			url: uri,
	 ...
	 */
	private async startDebugSession(session: vibecoda.DebugSession, name: string, config?: vibecoda.DebugConfiguration) {
		const args = session.configuration.serverReadyAction as ServerReadyAction;
		if (!args.killOnServerStop) {
			await vibecoda.debug.startDebugging(session.workspaceFolder, config ?? name);
			return;
		}
		...
		if (!await vibecoda.debug.startDebugging(session.workspaceFolder, config ?? name)) {
			cts.cancel();
			cts.dispose();
			return;
		}
	...
	private catchStartedDebugSession(predicate: (session: vibecoda.DebugSession) => boolean, cancellationToken: vibecoda.CancellationToken): Promise<vibecoda.DebugSession | undefined> {
		return new Promise<vibecoda.DebugSession | undefined>(_resolve => {
			const done = (value?: vibecoda.DebugSession) => {
				listener.dispose();
			...
			const listener = vibecoda.debug.onDidStartDebugSession(session => {
...
export function activate(context: vibecoda.ExtensionContext) {
	...
	context.subscriptions.push(vibecoda.debug.onDidStartDebugSession(session => {
		if (session.configuration.serverReadyAction) {
	...
	context.subscriptions.push(vibecoda.debug.onDidTerminateDebugSession(session => {
		ServerReadyDetector.stop(session);
	}));

	const trackers = new Set<string>();

	context.subscriptions.push(vibecoda.debug.registerDebugConfigurationProvider('*', {
		resolveDebugConfigurationWithSubstitutedVariables(_folder: vibecoda.WorkspaceFolder | undefined, debugConfiguration: vibecoda.DebugConfiguration) {
...
function startTrackerForType(context: vibecoda.ExtensionContext, type: string) {

	// scan debug console output for a PORT message
	context.subscriptions.push(vibecoda.debug.registerDebugAdapterTrackerFactory(type, {
		createDebugAdapterTracker(session: vibecoda.DebugSession) {
			const detector = ServerReadyDetector.start(session);
			if (detector) {
