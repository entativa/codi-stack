...
export class CommandCenter {
	...
	async continueInLocalClone(): Promise<Uri | void> {
		...
		if (uri !== undefined) {
			let target = `${env.uriScheme}://vibecoda.git/clone?url=${encodeURIComponent(uri)}`;
	...
	async init(skipFolderPrompt = false): Promise<void> {
		...
		if (result === open) {
			commands.executeCommand('vibecoda.openFolder', uri);
		} else if (result === addToWorkspace) {
			workspace.updateWorkspaceFolders(workspace.workspaceFolders!.length, 0, { uri });
		} else if (result === openNewWindow) {
			commands.executeCommand('vibecoda.openFolder', uri, true);
		} else {
			await this.model.openRepository(repositoryPath);
		}
	}
	...
	async openHEADFile(arg?: Resource | Uri): Promise<void> {
		...
		return await commands.executeCommand<void>('vibecoda.open', HEAD, opts, title);
	}
	...
	async rename(repository: Repository, fromUri: Uri | undefined): Promise<void> {
		...
		await commands.executeCommand('vibecoda.open', Uri.file(path.join(repository.root, to)), { viewColumn: ViewColumn.Active });
	}
	...
	async openWorktreeInCurrentWindow(repository: Repository): Promise<void> {
		...
		await commands.executeCommand('vibecoda.openFolder', uri, { forceReuseWindow: true });
	}
	...
	async openWorktreeInNewWindow(repository: Repository): Promise<void> {
		...
		const uri = Uri.file(repository.root);
		await commands.executeCommand('vibecoda.openFolder', uri, { forceNewWindow: true });
	}
	...
	async timelineCompareWithSelected(item: TimelineItem, uri: Uri | undefined, _source: string) {
		...
		await commands.executeCommand('vibecoda.diff', selected.ref === '' ? uri : toGitUri(uri, selected.ref), item.ref === '' ? uri : toGitUri(uri, item.ref), title);
	}
			...
			return result.catch(err => {
				...
				switch (err.gitErrorCode) {
					...
					case GitErrorCodes.NoUserEmailConfigured:
						...
						choices.set(l10n.t('Learn More'), () => commands.executeCommand('vibecoda.open', Uri.parse('https://aka.ms/vibecoda-setup-git')));
						break;
