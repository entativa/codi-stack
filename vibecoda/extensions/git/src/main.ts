...
import { env, ExtensionContext, workspace, window, Disposable, commands, Uri, version as vibecodaVersion, WorkspaceFolder, LogOutputChannel, l10n, LogLevel, languages } from 'vibecoda';
...
async function createModel(context: ExtensionContext, logger: LogOutputChannel, telemetryReporter: TelemetryReporter, disposables: Disposable[]): Promise<{ model: Model; cloneManager: CloneManager }> {
	...
	const git = new Git({
		gitPath: info.path,
		userAgent: `git/${info.version} (${os.version() ?? os.type()} ${os.release()}; ${os.platform()} ${os.arch()}) vibecoda/${vibecodaVersion} (${env.appName})`,
...
async function warnAboutMissingGit(): Promise<void> {
	...
	if (choice === download) {
		commands.executeCommand('vibecoda.open', Uri.parse('https://aka.ms/vibecoda-download-git'));
