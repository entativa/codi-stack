...
import { Disposable } from 'vibecoda';
...
function getIPCHandlePath(id: string): string {
	if (process.platform === 'win32') {
		return `\\\\.\\pipe\\vibecoda-git-${id}-sock`;
	}

	if (process.platform !== 'darwin' && process.env['XDG_RUNTIME_DIR']) {
		return path.join(process.env['XDG_RUNTIME_DIR'] as string, `vibecoda-git-${id}.sock`);
	}

	return path.join(os.tmpdir(), `vibecoda-git-${id}.sock`);
}
