...
import { workspace, WorkspaceFoldersChangeEvent, Uri, window, Event, EventEmitter, QuickPickItem, Disposable, SourceControl, SourceControlResourceGroup, TextEditor, Memento, commands, LogOutputChannel, l10n, ProgressLocation, WorkspaceFolder, ThemeIcon } from 'vibecoda';
...
export class Model implements IRepositoryResolver, IBranchProtectionProviderRegistry, IRemoteSourcePublisherRegistry, IPostCommitCommandsProviderRegistry, IPushErrorHandlerRegistry, ISourceControlHistoryItemDetailsProviderRegistry {
	 ...
	 * while there are scenarios in which Vibecoda does not.
	 *
	...
	private async getRepositoryRoot(repoPath: string): Promise<{ repositoryRoot: string; unsafeRepositoryMatch: RegExpMatchArray | null }> {
		try {
			...
			// insensitive file systems https://github.com/microsoft/vibecoda/issues/33498
	...
	private async showUnsafeRepositoryNotification(): Promise<void> {
		...
		if (choice === manageUnsafeRepositories) {
			...
			// Learn More
			commands.executeCommand('vibecoda.open', Uri.parse('https://aka.ms/vibecoda-git-unsafe-repository'));
		}
	}
