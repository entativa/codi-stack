...
export function getHistoryItemHover(authorAvatar: string | undefined, authorName: string | undefined, authorEmail: string | undefined, authorDate: Date | number | undefined, message: string, shortStats: CommitShortStat | undefined, commands: Command[][] | undefined): MarkdownString {
	...
	if (shortStats) {
		...
		if (shortStats.insertions) {
			markdownString.appendMarkdown(`,&nbsp;<span style="color:var(--vibecoda-scmGraph-historyItemHoverAdditionsForeground);">${shortStats.insertions === 1 ?
				l10n.t('{0} insertion{1}', shortStats.insertions, '(+)') :
				l10n.t('{0} insertions{1}', shortStats.insertions, '(+)')}</span>`);
		}

		if (shortStats.deletions) {
			markdownString.appendMarkdown(`,&nbsp;<span style="color:var(--vibecoda-scmGraph-historyItemHoverDeletionsForeground);">${shortStats.deletions === 1 ?
	...
	// 		return `<span style="color:var(--vibecoda-scmGraph-historyItemHoverDefaultLabelForeground);background-color:var(--vibecoda-scmGraph-historyItemHoverDefaultLabelBackground);border-radius:10px;">&nbsp;$(${labelIconId})&nbsp;${ref.name}&nbsp;&nbsp;</span>`;
	// 	}).join('&nbsp;&nbsp;'));
	// 	markdownString.appendMarkdown(`\n\n---\n\n`);
	// }
