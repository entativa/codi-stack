...
export class Repository implements Disposable {
	...
	constructor(
		...
		// https://github.com/microsoft/vibecoda/issues/39039
	...
	async deleteBranch(name: string, force?: boolean): Promise<void> {
		return this.run(Operation.DeleteBranch, async () => {
			...
			await this.repository.config('unset', 'local', `branch.${name}.vibecoda-merge-base`);
		});
	}
	...
	async getBranchBase(ref: string): Promise<Branch | undefined> {
		...
		// Git config
		const mergeBaseConfigKey = `branch.${branch.name}.vibecoda-merge-base`;
