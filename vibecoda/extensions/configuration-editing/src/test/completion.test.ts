	...
	test('remote.extensionKind', async () => {
		{
			...
			const expected = { label: 'vibecoda.npm' };
			await testCompletion(testFile, 'jsonc', content, expected);
		}
	});
	...
	test('change recommendation', async () => {
		{
			...
			const resultText = [
				'{',
				'  "recommendations": [',
				'    "ms-vibecoda.js-debug"',
				'  ]',
				'}',
			].join('\n');
			const expected = { label: 'ms-vibecoda.js-debug', resultText };
			await testCompletion(testFile, 'jsonc', content, expected);
		}
	});
...
async function testCompletion(testFileName: string, languageId: string, content: string, expected: ItemDescription) {
	...
	const docUri = vibecoda.Uri.file(path.join(await testFolder, testFileName));
	await fs.writeFile(docUri.fsPath, content);

	const editor = await setTestContent(docUri, languageId, content);
	const position = editor.document.positionAt(offset);

	// Executing the command `vibecoda.executeCompletionItemProvider` to simulate triggering completion
	const actualCompletions = (await vibecoda.commands.executeCommand('vibecoda.executeCompletionItemProvider', docUri, position)) as vibecoda.CompletionList;
	...
	if (expected.notAvailable) {
		...
		if (expected.resultText) {
			...
			if (match.range && match.insertText) {
				const range = match.range instanceof vibecoda.Range ? match.range : match.range.replacing;
...
async function setTestContent(docUri: vibecoda.Uri, languageId: string, content: string): Promise<vibecoda.TextEditor> {
	const ext = vibecoda.extensions.getExtension('vibecoda.configuration-editing')!;
	await ext.activate();

	const doc = await vibecoda.workspace.openTextDocument(docUri);
	await vibecoda.languages.setTextDocumentLanguage(doc, languageId);
	const editor = await vibecoda.window.showTextDocument(doc);

	const fullRange = new vibecoda.Range(new vibecoda.Position(0, 0), doc.positionAt(doc.getText().length));
