...
function getPick<T extends vibecoda.QuickPickItem>(quickpick: vibecoda.QuickPick<T>): Promise<T | undefined> {
...
export async function publishRepository(gitAPI: GitAPI, repository?: Repository): Promise<void> {
	...
	if (repository) {
		...
		folder = vibecoda.workspace.workspaceFolders[0].uri;
	} else {
		const picks = vibecoda.workspace.workspaceFolders.map(folder => ({ label: folder.name, folder }));
		const placeHolder = vibecoda.l10n.t('Pick a folder to publish to GitHub');
		const pick = await vibecoda.window.showQuickPick(picks, { placeHolder });
	...
	let quickpick = vibecoda.window.createQuickPick<vibecoda.QuickPickItem & { repo?: string; auth?: 'https' | 'ssh'; isPrivate?: boolean }>();
	...
	if (!repository) {
		const gitignore = vibecoda.Uri.joinPath(folder, '.gitignore');
		let shouldGenerateGitignore = false;

		try {
			await vibecoda.workspace.fs.stat(gitignore);
		} catch (err) {
			shouldGenerateGitignore = true;
		}

		if (shouldGenerateGitignore) {
			quickpick = vibecoda.window.createQuickPick();
			quickpick.placeholder = vibecoda.l10n.t('Select which files should be included in the repository.');
			...
			try {
				...
				const children = (await vibecoda.workspace.fs.readDirectory(folder))
					.map(([name]) => name)
				...
				const result = await Promise.race([
					new Promise<readonly vibecoda.QuickPickItem[]>(c => quickpick.onDidAccept(() => c(quickpick.selectedItems))),
					new Promise<undefined>(c => quickpick.onDidHide(() => c(undefined)))
				]);
				...
				if (ignored.size > 0) {
					...
					await vibecoda.workspace.fs.writeFile(gitignore, encoder.encode(raw));
				}
			} finally {
				quickpick.dispose();
			}
	...
	const githubRepository = await vibecoda.window.withProgress({ location: vibecoda.ProgressLocation.Notification, cancellable: false, title: 'Publish to GitHub' }, async progress => {
		progress.report({
			message: isPrivate
				? vibecoda.l10n.t('Publishing to a private GitHub repository')
				: vibecoda.l10n.t('Publishing to a public GitHub repository'),
			increment: 25
		});
		...
		if (isInCodespaces()) {
			createdGithubRepository = await vibecoda.commands.executeCommand<CreateRepositoryResponseData>('github.codespaces.publish', { name: repo!, isPrivate });
		} else {
		...
		if (createdGithubRepository) {
			progress.report({ message: vibecoda.l10n.t('Creating first commit'), increment: 25 });
			...
			progress.report({ message: vibecoda.l10n.t('Uploading files'), increment: 25 });

			const branch = await repository.getBranch('HEAD');
			const protocol = vibecoda.workspace.getConfiguration('github').get<'https' | 'ssh'>('gitProtocol');
	...
	const openOnGitHub = vibecoda.l10n.t('Open on GitHub');
	vibecoda.window.showInformationMessage(vibecoda.l10n.t('Successfully published the "{0}" repository to GitHub.', `${owner}/${repo}`), openOnGitHub).then(action => {
		if (action === openOnGitHub) {
			vibecoda.commands.executeCommand('vibecoda.open', vibecoda.Uri.parse(githubRepository.html_url));
		}
	});
}
