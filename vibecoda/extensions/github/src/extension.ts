...
import { commands, Disposable, ExtensionContext, extensions, l10n, LogLevel, LogOutputChannel, window } from 'vibecoda';
...
function initializeGitBaseExtension(): Disposable {
	...
	const gitBaseExtension = extensions.getExtension<GitBaseExtension>('vibecoda.git-base')!.exports;
...
function initializeGitExtension(context: ExtensionContext, octokitService: OctokitService, telemetryReporter: TelemetryReporter, logger: LogOutputChannel): Disposable {
	...
	let gitExtension = extensions.getExtension<GitExtension>('vibecoda.git');
	...
	if (gitExtension) {
		...
		const listener = extensions.onDidChange(() => {
			if (!gitExtension && extensions.getExtension<GitExtension>('vibecoda.git')) {
				gitExtension = extensions.getExtension<GitExtension>('vibecoda.git');
				initialize();
				listener.dispose();
			}
		});
