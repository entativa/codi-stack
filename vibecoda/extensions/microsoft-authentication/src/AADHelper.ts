...
const redirectUrl = 'https://vibecoda.dev/redirect';
...
export class AzureActiveDirectoryService {
	// For details on why this is set to 2/3... see https://github.com/microsoft/vibecoda/issues/133201#issuecomment-966668197
	private static REFRESH_TIMEOUT_MODIFIER = 1000 * 2 / 3;
	private static POLLING_CONSTANT = 1000 * 60 * 30;

	private _tokens: IToken[] = [];
	private _refreshTimeouts: Map<string, NodeJS.Timeout> = new Map<string, NodeJS.Timeout>();
	private _sessionChangeEmitter: vibecoda.EventEmitter<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent> = new vibecoda.EventEmitter<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent>();

	// Used to keep track of current requests when not using the local server approach.
	private _pendingNonces = new Map<string, string[]>();
	private _codeExchangePromises = new Map<string, Promise<vibecoda.AuthenticationSession>>();
	...
	constructor(
		private readonly _logger: vibecoda.LogOutputChannel,
		_context: vibecoda.ExtensionContext,
		...
		_context.subscriptions.push(vibecoda.window.onDidChangeWindowState(async (e) => e.focused && await this.storePendingTokens()));

		// In the event that a window isn't focused for a long time, we should still try to store the tokens at some point.
		const timer = new IntervalTimer();
		timer.cancelAndSet(
			() => !vibecoda.window.state.focused && this.storePendingTokens(),
		...
		const refreshes = sessions.map(async session => {
			...
			try {
				...
				if (e.message === REFRESH_NETWORK_FAILURE) {
					...
					vibecoda.window.showErrorMessage(vibecoda.l10n.t('You have been signed out because reading stored authentication information failed.'));
	...
	public get onDidChangeSessions(): vibecoda.Event<vibecoda.AuthenticationProviderAuthenticationSessionsChangeEvent> {
		return this._sessionChangeEmitter.event;
	}

	public getSessions(scopes: string[] | undefined, { account, authorizationServer }: vibecoda.AuthenticationProviderSessionOptions = {}): Promise<vibecoda.AuthenticationSession[]> {
		if (!scopes) {
	...
	private async doGetSessions(scopeData: IScopeData, account?: vibecoda.AuthenticationSessionAccountInformation): Promise<vibecoda.AuthenticationSession[]> {
		...
		return results
			...
			.map(result => (result as PromiseFulfilledResult<vibecoda.AuthenticationSession>).value);
	}

	public createSession(scopes: string[], { account, authorizationServer }: vibecoda.AuthenticationProviderSessionOptions = {}): Promise<vibecoda.AuthenticationSession> {
		let modifiedScopes = [...scopes];
		if (!modifiedScopes.includes('openid')) {
			modifiedScopes.push('openid');
		}
	...
	private async doCreateSession(scopeData: IScopeData, account?: vibecoda.AuthenticationSessionAccountInformation): Promise<vibecoda.AuthenticationSession> {
		this._logger.info(`[${scopeData.scopeStr}] Creating session` + account ? ` for ${account?.label}` : '');

		const runsRemote = vibecoda.env.remoteName !== undefined;
		const runsServerless = vibecoda.env.remoteName === undefined && vibecoda.env.uiKind === vibecoda.UIKind.Web;
		...
		return await vibecoda.window.withProgress({ location: vibecoda.ProgressLocation.Notification, title: vibecoda.l10n.t('Signing in to your account...'), cancellable: true }, async (_progress, token) => {
	...
	private async createSessionWithLocalServer(scopeData: IScopeData, loginHint: string | undefined, token: vibecoda.CancellationToken): Promise<vibecoda.AuthenticationSession> {
		...
		let codeToExchange;
		try {
			vibecoda.env.openExternal(vibecoda.Uri.parse(`http://127.0.0.1:${server.port}/signin?nonce=${encodeURIComponent(server.nonce)}`));
	...
	private async createSessionWithoutLocalServer(scopeData: IScopeData, loginHint: string | undefined, token: vibecoda.CancellationToken): Promise<vibecoda.AuthenticationSession> {
		this._logger.trace(`[${scopeData.scopeStr}] Starting login flow without local server`);
		let callbackUri = await vibecoda.env.asExternalUri(vibecoda.Uri.parse(`${vibecoda.env.uriScheme}://vibecoda.microsoft-authentication`));
		...
		const uri = vibecoda.Uri.parse(signInUrl.toString());
		vibecoda.env.openExternal(uri);
		...
		let inputBox: vibecoda.InputBox | undefined;
		if (!existingPromise) {
	...
	public async removeSessionById(sessionId: string, writeToDisk: boolean = true): Promise<vibecoda.AuthenticationSession | undefined> {
	...
	private async removeSessionByIToken(token: IToken, writeToDisk: boolean = true): Promise<vibecoda.AuthenticationSession | undefined> {
	 ...
	 */
	private convertToSessionSync(token: IToken): vibecoda.AuthenticationSession {
		return {
			id: token.sessionId,
	...
	private async convertToSession(token: IToken, scopeData: IScopeData): Promise<vibecoda.AuthenticationSession> {
	...
	private async handleCodeResponse(scopeData: IScopeData): Promise<vibecoda.AuthenticationSession> {
		let uriEventListener: vibecoda.Disposable;
		return new Promise((resolve: (value: vibecoda.AuthenticationSession) => void, reject) => {
			uriEventListener = this._uriHandler.event(async (uri: vibecoda.Uri) => {
				try {
	...
	private async exchangeCodeForSession(code: string, codeVerifier: string, scopeData: IScopeData): Promise<vibecoda.AuthenticationSession> {
	...
	private async fetchTokenResponse(postData: string, scopeData: IScopeData): Promise<ITokenResponse> {
		...
		if (this._env.activeDirectoryEndpointUrl !== defaultActiveDirectoryEndpointUrl) {
			...
			const proxyEndpoints: { [providerId: string]: string } | undefined = await vibecoda.commands.executeCommand('workbench.getCodeExchangeProxyEndpoints');
	...
	private async storeToken(token: IToken, scopeData: IScopeData): Promise<void> {
		if (!vibecoda.window.state.focused) {
