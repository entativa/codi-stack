...
export function activate(context: vibecoda.ExtensionContext): any {
	...
	const legend = new vibecoda.SemanticTokensLegend(tokenTypes, tokenModifiers);

	const outputChannel = vibecoda.window.createOutputChannel('Semantic Tokens Test');

	const documentSemanticHighlightProvider: vibecoda.DocumentSemanticTokensProvider = {
		provideDocumentSemanticTokens(document: vibecoda.TextDocument): vibecoda.ProviderResult<vibecoda.SemanticTokens> {
	...
	context.subscriptions.push(vibecoda.languages.registerDocumentSemanticTokensProvider({ pattern: '**/*semantic-test.json' }, documentSemanticHighlightProvider, legend));
