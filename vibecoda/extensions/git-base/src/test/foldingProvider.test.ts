...
suite('GitCommitFoldingProvider', () => {
	...
	function createMockDocument(content: string): vibecoda.TextDocument {
		...
		} as vibecoda.TextDocument;
	}

	const mockContext: vibecoda.FoldingContext = {} as vibecoda.FoldingContext;
	const mockToken: vibecoda.CancellationToken = { isCancellationRequested: false } as vibecoda.CancellationToken;
	...
	test('two comment lines create one folding range', () => {
		...
		const ranges = provider.provideFoldingRanges(doc, mockContext, mockToken) as vibecoda.FoldingRange[];

		assert.strictEqual(ranges.length, 1);
		assert.strictEqual(ranges[0].start, 0);
		assert.strictEqual(ranges[0].end, 1);
		assert.strictEqual(ranges[0].kind, vibecoda.FoldingRangeKind.Comment);
	});
	...
	test('multiple separated comment blocks', () => {
		...
		assert.strictEqual(ranges[1].kind, vibecoda.FoldingRangeKind.Comment);
	});
	...
	test('realistic git commit message with comments and verbose diff', () => {
		...
		const content = [
			...
			'-import { ExtensionContext } from \'vibecoda\';',
			'+import { ExtensionContext, languages } from \'vibecoda\';',
	...
	test('mixed comment and diff content', () => {
		...
		assert.strictEqual(ranges[2].kind, vibecoda.FoldingRangeKind.Comment);
	});
});
