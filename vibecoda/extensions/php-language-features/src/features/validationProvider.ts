...
export default class PHPValidationProvider {
	...
	private documentListener: vibecoda.Disposable | null = null;
	private diagnosticCollection?: vibecoda.DiagnosticCollection;
	...
	public activate(subscriptions: vibecoda.Disposable[]) {
		this.diagnosticCollection = vibecoda.languages.createDiagnosticCollection();
		subscriptions.push(this);
		subscriptions.push(vibecoda.workspace.onDidChangeConfiguration(() => this.loadConfigP = this.loadConfiguration()));

		vibecoda.workspace.onDidOpenTextDocument(this.triggerValidate, this, subscriptions);
		vibecoda.workspace.onDidCloseTextDocument((textDocument) => {
	...
	private async loadConfiguration(): Promise<void> {
		const section = vibecoda.workspace.getConfiguration();
		...
		if (this.validationEnabled) {
			if (this.config.trigger === RunTrigger.onType) {
				this.documentListener = vibecoda.workspace.onDidChangeTextDocument((e) => {
					this.triggerValidate(e.document);
				});
			} else {
				this.documentListener = vibecoda.workspace.onDidSaveTextDocument(this.triggerValidate, this);
			}
			// Configuration has changed. Reevaluate all documents.
			vibecoda.workspace.textDocuments.forEach(this.triggerValidate, this);
		}
	}

	private async triggerValidate(textDocument: vibecoda.TextDocument): Promise<void> {
	...
	private doValidate(textDocument: vibecoda.TextDocument): Promise<void> {
		return new Promise<void>(resolve => {
			const executable = this.config!.executable;
			if (!executable) {
				this.showErrorMessage(vibecoda.l10n.t("Cannot validate since a PHP installation could not be found. Use the setting 'php.validate.executablePath' to configure the PHP executable."));
			...
			const processLine = (line: string) => {
				...
				if (matches) {
					...
					const diagnostic: vibecoda.Diagnostic = new vibecoda.Diagnostic(
						new vibecoda.Range(line, 0, line, 2 ** 31 - 1), // See https://github.com/microsoft/vibecoda/issues/80288#issuecomment-650636442 for discussion
						message
					);
			...
			const options = (vibecoda.workspace.workspaceFolders && vibecoda.workspace.workspaceFolders[0]) ? { cwd: vibecoda.workspace.workspaceFolders[0].uri.fsPath } : undefined;
			let args: string[];
	...
	private async showError(error: any, executable: string): Promise<void> {
		...
		if (error.code === 'ENOENT') {
			if (this.config!.executable) {
				message = vibecoda.l10n.t("Cannot validate since {0} is not a valid php executable. Use the setting 'php.validate.executablePath' to configure the PHP executable.", executable);
			} else {
				message = vibecoda.l10n.t("Cannot validate since no PHP executable is set. Use the setting 'php.validate.executablePath' to configure the PHP executable.");
			}
		} else {
			message = error.message ? error.message : vibecoda.l10n.t("Failed to run php using path: {0}. Reason is unknown.", executable);
		}
		if (!message) {
			return;
		}
	...
	private async showErrorMessage(message: string): Promise<void> {
		const openSettings = vibecoda.l10n.t("Open Settings");
		if (await vibecoda.window.showInformationMessage(message, openSettings) === openSettings) {
			vibecoda.commands.executeCommand('workbench.action.openSettings', Setting.ExecutablePath);
		}
	}
}
...
async function getConfig(): Promise<IPhpConfig> {
	...
	if (executable && !path.isAbsolute(executable)) {
		const first = vibecoda.workspace.workspaceFolders && vibecoda.workspace.workspaceFolders[0];
		if (first) {
			executable = vibecoda.Uri.joinPath(first.uri, executable).fsPath;
		} else {
			executable = undefined;
		}
	} else if (!executable) {
