...
export function asResourceUrl(uri: vibecoda.Uri, range: vibecoda.Range): vibecoda.Uri {
	return uri.with({ fragment: `L${1 + range.start.line},${1 + range.start.character}-${1 + range.end.line},${1 + range.end.character}` });
}

export async function isValidRequestPosition(uri: vibecoda.Uri, position: vibecoda.Position) {
...
export function getPreviewChunks(doc: vibecoda.TextDocument, range: vibecoda.Range, beforeLen: number = 8, trim: boolean = true) {
	...
	let before = doc.getText(new vibecoda.Range(wordRange ? wordRange.start : previewStart, range.start));
	const inside = doc.getText(range);
	const previewEnd = range.end.translate(0, 331);
	let after = doc.getText(new vibecoda.Range(range.end, previewEnd));
	if (trim) {
...
export class ContextKey<V> {
	...
	async set(value: V) {
		await vibecoda.commands.executeCommand('setContext', this.name, value);
	}

	async reset() {
		await vibecoda.commands.executeCommand('setContext', this.name, undefined);
	}
}
...
export class WordAnchor {
	...
	constructor(private readonly _doc: vibecoda.TextDocument, private readonly _position: vibecoda.Position) {
		this._version = _doc.version;
		this._word = this._getAnchorWord(_doc, _position);
	}

	private _getAnchorWord(doc: vibecoda.TextDocument, pos: vibecoda.Position): string | undefined {
		const range = doc.getWordRangeAtPosition(pos) || doc.getWordRangeAtPosition(pos, /[^\s]+/);
		return range && doc.getText(range);
	}

	guessedTrackedPosition(): vibecoda.Position | undefined {
		// funky entry
		if (!this._word) {
		...
		do {
			...
			if (line < this._doc.lineCount) {
				...
				if (ch >= 0) {
					return new vibecoda.Position(line, ch);
				}
			}
			i += 1;
			// nth line up
...
// vibecoda.SymbolKind.File === 0, Module === 1, etc...
const _themeIconIds = [
...
export function getThemeIcon(kind: vibecoda.SymbolKind): vibecoda.ThemeIcon | undefined {
	const id = _themeIconIds[kind];
	return id ? new vibecoda.ThemeIcon(id) : undefined;
}
