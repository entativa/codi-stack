...
export function register(tree: SymbolsTree, context: vibecoda.ExtensionContext): void {
	...
	function showCallHierarchy() {
		if (vibecoda.window.activeTextEditor) {
			const input = new CallsTreeInput(new vibecoda.Location(vibecoda.window.activeTextEditor.document.uri, vibecoda.window.activeTextEditor.selection.active), direction.value);
			tree.setInput(input);
		}
	}
	...
	function setCallsDirection(value: CallsDirection, anchor: CallItem | unknown) {
		...
		if (anchor instanceof CallItem) {
			newInput = new CallsTreeInput(new vibecoda.Location(anchor.item.uri, anchor.item.selectionRange.start), direction.value);
	...
	context.subscriptions.push(
		vibecoda.commands.registerCommand('references-view.showCallHierarchy', showCallHierarchy),
		vibecoda.commands.registerCommand('references-view.showOutgoingCalls', (item: CallItem | unknown) => setCallsDirection(CallsDirection.Outgoing, item)),
		vibecoda.commands.registerCommand('references-view.showIncomingCalls', (item: CallItem | unknown) => setCallsDirection(CallsDirection.Incoming, item)),
		vibecoda.commands.registerCommand('references-view.removeCallItem', removeCallItem)
	);
}
