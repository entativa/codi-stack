...
export class ReferencesTreeInput implements SymbolTreeInput<FileItem | ReferenceItem> {
	...
	constructor(
		...
		private readonly _result?: vibecoda.Location[] | vibecoda.LocationLink[]
	) {
	...
	async resolve(): Promise<SymbolTreeModel<FileItem | ReferenceItem> | undefined> {
		...
		if (this._result) {
			...
			const resut = await Promise.resolve(vibecoda.commands.executeCommand<vibecoda.Location[] | vibecoda.LocationLink[]>(this._command, this.location.uri, this.location.range.start));
	...
	with(location: vibecoda.Location): ReferencesTreeInput {
...
export class ReferencesModel implements SymbolItemNavigation<FileItem | ReferenceItem>, SymbolItemEditorHighlights<FileItem | ReferenceItem>, SymbolItemDragAndDrop<FileItem | ReferenceItem> {
	...
	private _onDidChange = new vibecoda.EventEmitter<FileItem | ReferenceItem | undefined>();
	readonly onDidChangeTreeData = this._onDidChange.event;

	readonly items: FileItem[] = [];

	constructor(locations: vibecoda.Location[] | vibecoda.LocationLink[]) {
		let last: FileItem | undefined;
		for (const item of locations.sort(ReferencesModel._compareLocations)) {
			const loc = item instanceof vibecoda.Location
				? item
				: new vibecoda.Location(item.targetUri, item.targetRange);
	...
	private static _compareUriIgnoreFragment(a: vibecoda.Uri, b: vibecoda.Uri): number {
	...
	private static _compareLocations(a: vibecoda.Location | vibecoda.LocationLink, b: vibecoda.Location | vibecoda.LocationLink): number {
		const aUri = a instanceof vibecoda.Location ? a.uri : a.targetUri;
		const bUri = b instanceof vibecoda.Location ? b.uri : b.targetUri;
		...
		const aRange = a instanceof vibecoda.Location ? a.range : a.targetRange;
		const bRange = b instanceof vibecoda.Location ? b.range : b.targetRange;
	...
	get message() {
		if (this.items.length === 0) {
			return vibecoda.l10n.t('No results.');
		}
		const total = this.items.reduce((prev, cur) => prev + cur.references.length, 0);
		const files = this.items.length;
		if (total === 1 && files === 1) {
			return vibecoda.l10n.t('{0} result in {1} file', total, files);
		} else if (total === 1) {
			return vibecoda.l10n.t('{0} result in {1} files', total, files);
		} else if (files === 1) {
			return vibecoda.l10n.t('{0} results in {1} file', total, files);
		} else {
			return vibecoda.l10n.t('{0} results in {1} files', total, files);
		}
	}
	...
	location(item: FileItem | ReferenceItem) {
		return item instanceof ReferenceItem
			? item.location
			: new vibecoda.Location(item.uri, item.references[0]?.location.range ?? new vibecoda.Position(0, 0));
	}

	nearest(uri: vibecoda.Uri, position: vibecoda.Position): FileItem | ReferenceItem | undefined {
	...
	getEditorHighlights(_item: FileItem | ReferenceItem, uri: vibecoda.Uri): vibecoda.Range[] | undefined {
	...
	getDragUri(item: FileItem | ReferenceItem): vibecoda.Uri | undefined {
		if (item instanceof FileItem) {
			return item.uri;
		} else {
...
class ReferencesTreeDataProvider implements vibecoda.TreeDataProvider<FileItem | ReferenceItem> {

	private readonly _listener: vibecoda.Disposable;
	private readonly _onDidChange = new vibecoda.EventEmitter<FileItem | ReferenceItem | undefined>();
	...
	async getTreeItem(element: FileItem | ReferenceItem) {
		if (element instanceof FileItem) {
			// files
			const result = new vibecoda.TreeItem(element.uri);
			result.contextValue = 'file-item';
			result.description = true;
			result.iconPath = vibecoda.ThemeIcon.File;
			result.collapsibleState = vibecoda.TreeItemCollapsibleState.Collapsed;
			return result;
			...
			const label: vibecoda.TreeItemLabel = {
				label: before + inside + after,
				highlights: [[before.length, before.length + inside.length]]
			};

			const result = new vibecoda.TreeItem(label);
			result.collapsibleState = vibecoda.TreeItemCollapsibleState.None;
			result.contextValue = 'reference-item';
			result.command = {
				command: 'vibecoda.open',
				title: vibecoda.l10n.t('Open Reference'),
				arguments: [
					element.location.uri,
					{ selection: range.with({ end: range.start }) } satisfies vibecoda.TextDocumentShowOptions
				]
			};
			return result;
		}
...
export class FileItem {
	...
	async asCopyText() {
		let result = `${vibecoda.workspace.asRelativePath(this.uri)}\n`;
...
export class ReferenceItem {
	...
	private _document: Thenable<vibecoda.TextDocument> | undefined;
	...
	async getDocument(warmUpNext?: boolean) {
		if (!this._document) {
			this._document = vibecoda.workspace.openTextDocument(this.location.uri);
		}
		if (warmUpNext) {
			...
			if (next instanceof FileItem && next !== this.file) {
				vibecoda.workspace.openTextDocument(next.uri);
			} else if (next instanceof ReferenceItem) {
				vibecoda.workspace.openTextDocument(next.location.uri);
			}
		}
