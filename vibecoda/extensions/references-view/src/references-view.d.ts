 ...
 * const api = await vibecoda.extensions.getExtension<SymbolTree>('vibecoda.references-view').activate();
 *
...
export interface SymbolTreeInput<T> {
	 ...
	 */
	resolve(): vibecoda.ProviderResult<SymbolTreeModel<T>>;
	 ...
	 */
	with(location: vibecoda.Location): SymbolTreeInput<T>;
}
...
export interface SymbolTreeModel<T> {
	 ...
	 */
	provider: vibecoda.TreeDataProvider<T>;
...
export interface SymbolItemNavigation<T> {
	 ...
	 */
	nearest(uri: vibecoda.Uri, position: vibecoda.Position): T | undefined;
	/**
	 ...
	 */
	location(item: T): vibecoda.Location | undefined;
}
...
export interface SymbolItemEditorHighlights<T> {
	 ...
	 */
	getEditorHighlights(item: T, uri: vibecoda.Uri): vibecoda.Range[] | undefined;
}

export interface SymbolItemDragAndDrop<T> {

	getDragUri(item: T): vibecoda.Uri | undefined;
}
