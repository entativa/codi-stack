...
suite('StackTraceHelper', () => {
	...
	test('IPython stack line numbers are linkified for IPython 8.3.6', () => {
		...
		assert.ok(cleanStack.indexOf('Cell In[3], <a href=\'vibecoda-notebook-cell:?execution_count=3&line=2\'>line 2</a>') > 0, 'Missing line link in ' + cleanStack);
		assert.ok(cleanStack.indexOf('<a href=\'vibecoda-notebook-cell:?execution_count=3&line=2\'>2</a>') > 0, 'Missing frame link in ' + cleanStack);
		assert.ok(cleanStack.indexOf('<a href=\'C:\\venvs\\myLib.py:2\'>2</a>') > 0, 'Missing frame link in ' + cleanStack);
		assert.equal(errorLocation, '<a href=\'vibecoda-notebook-cell:?execution_count=3&line=2\'>line 2</a>');
	});
	...
	test('Does not have catastrophic backtracking https://github.com/microsoft/vibecoda/issues/251731', () => {
		const stack =
			'\u001b[31m---------------------------------------------------------------------------\u001b[39m\n' +
		...
		const { formattedStack, errorLocation } = formatStackTrace(stack, true);
		const cleanStack = stripAsciiFormatting(formattedStack);
		assert.ok(cleanStack.indexOf('Cell In[1], <a href=\'vibecoda-notebook-cell:?execution_count=1&line=2\'>line 2</a>') > 0, 'Missing line link in ' + cleanStack);
		assert.ok(cleanStack.indexOf('<a href=\'vibecoda-notebook-cell:?execution_count=1&line=2\'>2</a>') > 0, 'Missing frame link in ' + cleanStack);
		assert.equal(errorLocation, '<a href=\'vibecoda-notebook-cell:?execution_count=1&line=2\'>line 2</a>');
	});
	...
	test('IPython stack line numbers are linkified for IPython 8.3', () => {
		...
		assert.ok(formatted.indexOf('Input <a href=\'vibecoda-notebook-cell:?execution_count=2\'>In [2]</a>, in <cell line: 5>') > 0, 'Missing cell link in ' + formatted);
		assert.ok(formatted.indexOf('Input <a href=\'vibecoda-notebook-cell:?execution_count=1\'>In [1]</a>, in myfunc()') > 0, 'Missing cell link in ' + formatted);
		assert.ok(formatted.indexOf('<a href=\'vibecoda-notebook-cell:?execution_count=2&line=5\'>5</a>') > 0, 'Missing frame link in ' + formatted);
	});
	...
	test('IPython stack without line numbers are not linkified', () => {
		...
		assert.ok(/<a href='vibecoda-notebook-cell.*>/.test(formattedLines[0]), 'line should contain a link: ' + formattedLines[0]);
