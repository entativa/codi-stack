...
import type TelemetryReporter from '@vibecoda/extension-telemetry';

export class DocumentMergeConflict implements interfaces.IDocumentMergeConflict {

	public range: vibecoda.Range;
	...
	public splitter: vibecoda.Range;
	...
	public commitEdit(type: interfaces.CommitType, editor: vibecoda.TextEditor, edit?: vibecoda.TextEditorEdit): Thenable<boolean> {
	...
	public applyEdit(type: interfaces.CommitType, document: vibecoda.TextDocument, edit: { replace(range: vibecoda.Range, newText: string): void }): void {
		if (this.applied) {
			return;
		}
		this.applied = true;
	...
	private replaceRangeWithContent(content: string, edit: { replace(range: vibecoda.Range, newText: string): void }) {
