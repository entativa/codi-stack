...
class OriginDocumentMergeConflictTracker implements interfaces.IDocumentMergeConflictTracker {
	...
	getConflicts(document: vibecoda.TextDocument): PromiseLike<interfaces.IDocumentMergeConflict[]> {
	...
	forget(document: vibecoda.TextDocument) {
		this.parent.forget(document);
	}
}

export default class DocumentMergeConflictTracker implements vibecoda.Disposable, interfaces.IDocumentMergeConflictTrackerService {
	private cache: Map<string, ScanTask> = new Map();
	private delayExpireTime: number = 0;

	constructor(private readonly telemetryReporter: TelemetryReporter) { }

	getConflicts(document: vibecoda.TextDocument, origin: string): PromiseLike<interfaces.IDocumentMergeConflict[]> {
		// Attempt from cache
	...
	isPending(document: vibecoda.TextDocument, origin: string): boolean {
		if (!document) {
			return false;
		}
	...
	private getConflictsOrEmpty(document: vibecoda.TextDocument, _origins: string[]): interfaces.IDocumentMergeConflict[] {
	...
	private getCacheKey(document: vibecoda.TextDocument): string | null {
		if (document.uri) {
