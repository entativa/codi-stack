...
export default class CommandHandler implements vibecoda.Disposable {
	...
	private registerTextEditorCommand(command: string, cb: (editor: vibecoda.TextEditor, ...args: any[]) => Promise<void>, resourceCB?: (uris: vibecoda.Uri[]) => Promise<void>) {
		return vibecoda.commands.registerCommand(command, (...args) => {
	...
	acceptCurrent(editor: vibecoda.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Current, editor, ...args);
	}

	acceptIncoming(editor: vibecoda.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Incoming, editor, ...args);
	}

	acceptBoth(editor: vibecoda.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Both, editor, ...args);
	}

	acceptAllCurrent(editor: vibecoda.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Current, editor);
	}

	acceptAllIncoming(editor: vibecoda.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Incoming, editor);
	}

	acceptAllCurrentResources(resources: vibecoda.Uri[]): Promise<void> {
		return this.acceptAllResources(interfaces.CommitType.Current, resources);
	}

	acceptAllIncomingResources(resources: vibecoda.Uri[]): Promise<void> {
		return this.acceptAllResources(interfaces.CommitType.Incoming, resources);
	}

	acceptAllBoth(editor: vibecoda.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Both, editor);
	}

	async compare(editor: vibecoda.TextEditor, conflict: interfaces.IDocumentMergeConflict | null) {
		...
		if (!conflict) {
			...
			if (!conflict) {
				vibecoda.window.showWarningMessage(vibecoda.l10n.t("Editor cursor is not within a merge conflict"));
				return;
			}
		}
		...
		const selection = new vibecoda.Range(
		...
		const title = vibecoda.l10n.t("{0}: Current Changes ↔ Incoming Changes", fileName);
		const mergeConflictConfig = vibecoda.workspace.getConfiguration('merge-conflict');
		const openToTheSide = mergeConflictConfig.get<string>('diffViewPosition');
		const opts: vibecoda.TextDocumentShowOptions = {
			viewColumn: openToTheSide === 'Beside' ? vibecoda.ViewColumn.Beside : vibecoda.ViewColumn.Active,
			selection
		};

		if (openToTheSide === 'Below') {
			await vibecoda.commands.executeCommand('workbench.action.newGroupBelow');
		}

		await vibecoda.commands.executeCommand('vibecoda.diff', leftUri, rightUri, title, opts);
	}

	navigateNext(editor: vibecoda.TextEditor): Promise<void> {
		return this.navigate(editor, NavigationDirection.Forwards);
	}

	navigatePrevious(editor: vibecoda.TextEditor): Promise<void> {
		return this.navigate(editor, NavigationDirection.Backwards);
	}

	async acceptSelection(editor: vibecoda.TextEditor): Promise<void> {
		...
		let tokenAfterCurrentBlock: vibecoda.Range = conflict.splitter;
		...
		if (editor.selection.active.isBefore(tokenAfterCurrentBlock.start)) {
			...
			vibecoda.window.showWarningMessage(vibecoda.l10n.t('Editor cursor is within the common ancestors block, please move it to either the "current" or "incoming" block'));
			return;
		}
		else {
			vibecoda.window.showWarningMessage(vibecoda.l10n.t('Editor cursor is within the merge conflict splitter, please move it to either the "current" or "incoming" block'));
			return;
		}
	...
	private async navigate(editor: vibecoda.TextEditor, direction: NavigationDirection): Promise<void> {
		...
		if (!navigationResult) {
			...
			vibecoda.window.showWarningMessage(vibecoda.l10n.t("No merge conflicts found in this file"));
			return;
		}
		else if (!navigationResult.canNavigate) {
			vibecoda.window.showWarningMessage(vibecoda.l10n.t("No other merge conflicts within this file"));
			return;
		}
		...
		editor.selection = new vibecoda.Selection(navigationResult.conflict.range.start, navigationResult.conflict.range.start);
		editor.revealRange(navigationResult.conflict.range, vibecoda.TextEditorRevealType.Default);
	}

	private async accept(type: interfaces.CommitType, editor: vibecoda.TextEditor, ...args: any[]): Promise<void> {
	...
	private async acceptAll(type: interfaces.CommitType, editor: vibecoda.TextEditor): Promise<void> {
		const conflicts = await this.tracker.getConflicts(editor.document);
	...
	private async acceptAllResources(type: interfaces.CommitType, resources: vibecoda.Uri[]): Promise<void> {
		const documents = await Promise.all(resources.map(resource => vibecoda.workspace.openTextDocument(resource)));
		...
		vibecoda.workspace.applyEdit(edit);
	}

	private async findConflictContainingSelection(editor: vibecoda.TextEditor, conflicts?: interfaces.IDocumentMergeConflict[]): Promise<interfaces.IDocumentMergeConflict | null> {
	...
	private async findConflictForNavigation(editor: vibecoda.TextEditor, direction: NavigationDirection, conflicts?: interfaces.IDocumentMergeConflict[]): Promise<IDocumentMergeConflictNavigationResults | null> {
		if (!conflicts) {
			conflicts = await this.tracker.getConflicts(editor.document);
		}
