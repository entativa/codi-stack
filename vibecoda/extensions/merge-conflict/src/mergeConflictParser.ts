...
interface IScanMergedConflict {
	startHeader: vibecoda.TextLine;
	commonAncestors: vibecoda.TextLine[];
	splitter?: vibecoda.TextLine;
	endFooter?: vibecoda.TextLine;
}

export class MergeConflictParser {

	static scanDocument(document: vibecoda.TextDocument, telemetryReporter: TelemetryReporter): interfaces.IDocumentMergeConflict[] {
	...
	private static scanItemTolMergeConflictDescriptor(document: vibecoda.TextDocument, scanned: IScanMergedConflict): interfaces.IDocumentMergeConflictDescriptor | null {
		// Validate we have all the required lines within the scan item.
		if (!scanned.startHeader || !scanned.splitter || !scanned.endFooter) {
			return null;
		}

		const tokenAfterCurrentBlock: vibecoda.TextLine = scanned.commonAncestors[0] || scanned.splitter;
		...
		return {
			current: {
				...
				decoratorContent: new vibecoda.Range(
			...
			range: new vibecoda.Range(scanned.startHeader.range.start, scanned.endFooter.rangeIncludingLineBreak.end)
		};
	}

	static containsConflict(document: vibecoda.TextDocument): boolean {
		if (!document) {
			return false;
		}
	...
	private static shiftBackOneCharacter(document: vibecoda.TextDocument, range: vibecoda.Position, unlessEqual: vibecoda.Position): vibecoda.Position {
		...
		return new vibecoda.Position(line, character);
	}
}
