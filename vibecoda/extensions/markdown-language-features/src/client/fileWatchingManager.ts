...
export class FileWatcherManager {
	...
	create(id: number, uri: vibecoda.Uri, watchParentDirs: boolean, listeners: { create?: () => void; change?: () => void; delete?: () => void }): void {
		// Non-writable file systems do not support file watching
		if (!vibecoda.workspace.fs.isWritableFileSystem(uri.scheme)) {
			return;
		}

		const watcher = vibecoda.workspace.createFileSystemWatcher(new vibecoda.RelativePattern(uri, '*'), !listeners.create, !listeners.change, !listeners.delete);
