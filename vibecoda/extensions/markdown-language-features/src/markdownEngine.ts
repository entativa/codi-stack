...
interface RenderEnv {
	containingImages: Set<string>;
	currentDocument: vibecoda.Uri | undefined;
...
export class MarkdownItEngine implements IMdParser {
	...
	public async getEngine(resource: vibecoda.Uri | undefined): Promise<MarkdownIt> {
	...
	private _getConfig(resource?: vibecoda.Uri): MarkdownItConfig {
	...
	private _addLinkNormalizer(md: MarkdownIt): void {
		...
		md.normalizeLink = (link: string) => {
			try {
				// Normalize Vibecoda schemes to target the current version
				if (isOfScheme(Schemes.vibecoda, link) || isOfScheme(Schemes['vibecoda-insiders'], link)) {
					return normalizeLink(vibecoda.Uri.parse(link).with({ scheme: vibecoda.env.uriScheme }).toString());
				}
	...
	private _addLinkValidator(md: MarkdownIt): void {
		...
		md.validateLink = (link: string) => {
			return validateLink(link)
				|| isOfScheme(Schemes.vibecoda, link)
				|| isOfScheme(Schemes['vibecoda-insiders'], link)
	...
	private _toResourceUri(href: string, currentDocument: vibecoda.Uri | undefined, resourceProvider: WebviewResourceProvider | undefined): string {
		try {
			// Support file:// links
			if (isOfScheme(Schemes.file, href)) {
				const uri = vibecoda.Uri.parse(href);
				if (resourceProvider) {
			...
			if (!/^[a-z\-]+:/i.test(href)) {
				...
				let uri = vibecoda.Uri.parse('markdown-link:' + href);
				...
				if (uri.path[0] === '/' && currentDocument) {
					const root = vibecoda.workspace.getWorkspaceFolder(currentDocument);
					if (root) {
						uri = vibecoda.Uri.joinPath(root.uri, uri.fsPath).with({
