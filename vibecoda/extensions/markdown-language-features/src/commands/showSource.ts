...
export class ShowSourceCommand implements Command {
	...
	public execute() {
		...
		if (activePreviewResource && activePreviewResourceColumn) {
			return vibecoda.workspace.openTextDocument(activePreviewResource).then(document => {
				return vibecoda.window.showTextDocument(document, activePreviewResourceColumn);
			});
		}
		return undefined;
	}
