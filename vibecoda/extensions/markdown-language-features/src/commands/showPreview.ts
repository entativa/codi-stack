...
async function showPreview(
	...
	uri: vibecoda.Uri | undefined,
	previewSettings: ShowPreviewSettings,
): Promise<any> {
	let resource = uri;
	if (!(resource instanceof vibecoda.Uri)) {
	...
	if (!(resource instanceof vibecoda.Uri)) {
		if (!vibecoda.window.activeTextEditor) {
			...
			return vibecoda.commands.executeCommand('markdown.showSource');
		}
		// nothing found that could be shown or toggled
		return;
	}

	const resourceColumn = vibecoda.window.activeTextEditor?.viewColumn || vibecoda.ViewColumn.One;
	webviewManager.openDynamicPreview(resource, {
		resourceColumn: resourceColumn,
		previewColumn: previewSettings.sideBySide ? vibecoda.ViewColumn.Beside : resourceColumn,
		locked: !!previewSettings.locked
	});

	telemetryReporter.sendTelemetryEvent('openPreview', {
		where: previewSettings.sideBySide ? 'sideBySide' : 'inPlace',
		how: (uri instanceof vibecoda.Uri) ? 'action' : 'pallete'
	});
}
...
export class ShowPreviewCommand implements Command {
	...
	public execute(mainUri?: vibecoda.Uri, allUris?: vibecoda.Uri[], previewSettings?: DynamicPreviewSettings) {
...
export class ShowPreviewToSideCommand implements Command {
	...
	public execute(uri?: vibecoda.Uri, previewSettings?: DynamicPreviewSettings) {
		showPreview(this._webviewManager, this._telemetryReporter, uri, {
			sideBySide: true,
			locked: previewSettings?.locked
		});
...
export class ShowLockedPreviewToSideCommand implements Command {
	...
	public execute(uri?: vibecoda.Uri) {
		showPreview(this._webviewManager, this._telemetryReporter, uri, {
			sideBySide: true,
			locked: true
		});
