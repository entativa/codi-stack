 ...
 *--------------------------------------------------------------------------------------------*/
import { default as VibecodaTelemetryReporter } from '@vibecoda/extension-telemetry';
...
class ExtensionReporter implements TelemetryReporter {
	private readonly _reporter: VibecodaTelemetryReporter;

	constructor(
		packageInfo: IPackageInfo
	) {
		this._reporter = new VibecodaTelemetryReporter(packageInfo.aiKey);
	}
	sendTelemetryEvent(eventName: string, properties?: {
		[key: string]: string;
	}) {
...
function getPackageInfo(): IPackageInfo | null {
	const extension = vibecoda.extensions.getExtension('Microsoft.vibecoda-markdown');
