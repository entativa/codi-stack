...
async function shouldSmartPasteForSelection(
	...
	const line = document.getText(new vibecoda.Range(selectedRange.start.line, 0, selectedRange.start.line, Number.MAX_SAFE_INTEGER));
...
export function findValidUriInText(text: string): string | undefined {
	...
	try {
		uri = vibecoda.Uri.parse(trimmedUrlList);
	} catch {
		// Could not parse
		return;
	}
