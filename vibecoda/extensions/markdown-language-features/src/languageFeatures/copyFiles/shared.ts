...
export const baseLinkEditKind = vibecoda.DocumentDropOrPasteEditKind.Empty.append('markdown', 'link');
...
export function getSnippetLabelAndKind(counter: { readonly insertedAudioCount: number; readonly insertedVideoCount: number; readonly insertedImageCount: number; readonly insertedLinkCount: number }): {
	...
	if (counter.insertedVideoCount > 0 || counter.insertedAudioCount > 0) {
		...
		if (counter.insertedImageCount > 0) {
			return {
				label: vibecoda.l10n.t('Insert Markdown Media and Images'),
				kind: baseLinkEditKind,
			};
		}
		...
		if (counter.insertedAudioCount > 0 && !counter.insertedVideoCount) {
			return {
				label: vibecoda.l10n.t('Insert Markdown Audio'),
				kind: audioEditKind,
			};
		}
		...
		if (counter.insertedVideoCount > 0 && !counter.insertedAudioCount) {
			return {
				label: vibecoda.l10n.t('Insert Markdown Video'),
				kind: videoEditKind,
			};
		}
		...
		return {
			label: vibecoda.l10n.t('Insert Markdown Media'),
			kind: baseLinkEditKind,
		};
		...
		if (counter.insertedLinkCount > 0) {
			return {
				label: vibecoda.l10n.t('Insert Markdown Images and Links'),
				kind: baseLinkEditKind,
			};
		}
		...
		return {
			label: counter.insertedImageCount > 1
				? vibecoda.l10n.t('Insert Markdown Images')
				: vibecoda.l10n.t('Insert Markdown Image'),
			kind: imageEditKind,
		};
	} else {
		return {
			label: counter.insertedLinkCount > 1
				? vibecoda.l10n.t('Insert Markdown Links')
				: vibecoda.l10n.t('Insert Markdown Link'),
			kind: linkEditKind,
		};
	}
}
...
export function createInsertUriListEdit(
...
): { edits: vibecoda.SnippetTextEdit[]; label: string; kind: vibecoda.DocumentDropOrPasteEditKind } | undefined {
	if (!ranges.length || !urlList.entries.length) {
		return;
	}

	const edits: vibecoda.SnippetTextEdit[] = [];
	...
	for (const range of orderedRanges) {
		...
		edits.push(new vibecoda.SnippetTextEdit(range, snippet.snippet));
	}
...
interface UriListSnippetOptions {
	 ...
	 */
	readonly linkKindHint?: vibecoda.DocumentDropOrPasteEditKind | 'media';
...
export interface UriSnippet {
	readonly snippet: vibecoda.SnippetString;
...
export function createUriListSnippet(
	...
	const config = vibecoda.workspace.getConfiguration('markdown', document);
...
function getDesiredLinkKind(uri: vibecoda.Uri, uriFileKind: MediaKind | undefined, options: UriListSnippetOptions | undefined): DesiredLinkKind {
	if (options?.linkKindHint instanceof vibecoda.DocumentDropOrPasteEditKind) {
...
function getRelativeMdPath(dir: vibecoda.Uri | undefined, file: vibecoda.Uri): string | undefined {
...
export interface DropOrPasteEdit {
	readonly snippet: vibecoda.SnippetString;
	readonly kind: vibecoda.DocumentDropOrPasteEditKind;
	readonly label: string;
	readonly additionalEdits: vibecoda.WorkspaceEdit;
	readonly yieldTo: vibecoda.DocumentDropOrPasteEditKind[];
}
