...
export function getCopyFileConfiguration(document: vibecoda.TextDocument): CopyFileConfiguration {
...
function readOverwriteBehavior(config: vibecoda.WorkspaceConfiguration): OverwriteBehavior {
...
export function parseGlob(rawGlob: string): Iterable<string> {
	if (rawGlob.startsWith('/')) {
		...
		return (vibecoda.workspace.workspaceFolders ?? []).map(folder => vibecoda.Uri.joinPath(folder.uri, rawGlob).path);
	}
...
type GetWorkspaceFolder = (documentUri: vibecoda.Uri) => vibecoda.Uri | undefined;

export function resolveCopyDestination(documentUri: vibecoda.Uri, fileName: string, dest: string, getWorkspaceFolder: GetWorkspaceFolder): vibecoda.Uri {
...
function resolveCopyDestinationSetting(documentUri: vibecoda.Uri, fileName: string, dest: string, getWorkspaceFolder: GetWorkspaceFolder): string {
