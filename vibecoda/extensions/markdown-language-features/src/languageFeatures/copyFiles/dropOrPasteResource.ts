 ...
 */
class ResourcePasteOrDropProvider implements vibecoda.DocumentPasteEditProvider, vibecoda.DocumentDropEditProvider {
	...
	private readonly _yieldTo = [
		...
		vibecoda.DocumentDropOrPasteEditKind.Empty.append('markdown', 'link', 'image', 'attachment'), // Prefer notebook attachments
	];
	...
	public async provideDocumentDropEdits(
		...
		const edit = await this._createEdit(document, [new vibecoda.Range(position, position)], dataTransfer, {
			insert: this._getEnabled(document, 'editor.drop.enabled'),
			copyIntoWorkspace: vibecoda.workspace.getConfiguration('markdown', document).get<CopyFilesSettings>('editor.drop.copyIntoWorkspace', CopyFilesSettings.MediaFiles)
		}, undefined, token);
		...
		const dropEdit = new vibecoda.DocumentDropEdit(edit.snippet);
	...
	public async provideDocumentPasteEdits(
		...
		const edit = await this._createEdit(document, ranges, dataTransfer, {
			...
			copyIntoWorkspace: vibecoda.workspace.getConfiguration('markdown', document).get<CopyFilesSettings>('editor.paste.copyIntoWorkspace', CopyFilesSettings.MediaFiles)
		}, context, token);
		...
		const pasteEdit = new vibecoda.DocumentPasteEdit(edit.snippet, edit.label, edit.kind);
		pasteEdit.additionalEdit = edit.additionalEdits;
		pasteEdit.yieldTo = [...this._yieldTo, ...edit.yieldTo];
		return [pasteEdit];
	}

	private _getEnabled(document: vibecoda.TextDocument, settingName: string): InsertMarkdownLink {
		const setting = vibecoda.workspace.getConfiguration('markdown', document).get<boolean | InsertMarkdownLink>(settingName, true);
	...
	private async _createEdit(
		...
		context: vibecoda.DocumentPasteEditContext | undefined,
		token: vibecoda.CancellationToken,
		...
		if (!(await shouldInsertMarkdownLinkByDefault(this._parser, document, settings.insert, ranges, token))) {
			edit.yieldTo.push(vibecoda.DocumentDropOrPasteEditKind.Empty.append('uri'));
		}
	...
	private async _createEditFromUriListData(
		...
		const additionalEdits = new vibecoda.WorkspaceEdit();
		additionalEdits.set(document.uri, edit.edits);

		return {
			label: edit.label,
			kind: edit.kind,
			snippet: new vibecoda.SnippetString(''),
			additionalEdits,
			yieldTo: []
		};
	}
	...
	private async _createEditForMediaFiles(
		...
		interface FileEntry {
			...
			readonly newFile?: { readonly contents: vibecoda.DataTransferFile; readonly overwrite: boolean };
		}
		...
		const fileEntries = coalesce(await Promise.all(Array.from(dataTransfer, async ([mime, item]): Promise<FileEntry | undefined> => {
			...
			if (file.uri) {
				...
				const workspaceFolder = vibecoda.workspace.getWorkspaceFolder(file.uri);
				if (workspaceFolder) {
...
function textMatchesUriList(text: string, uriList: UriList): boolean {
	...
	try {
		const uri = vibecoda.Uri.parse(text);
...
export function registerResourceDropOrPasteSupport(selector: vibecoda.DocumentSelector, parser: IMdParser): vibecoda.Disposable {
	...
	return vibecoda.Disposable.from(
		vibecoda.languages.registerDocumentPasteEditProvider(selector, new ResourcePasteOrDropProvider(parser), {
			providedPasteEditKinds: providedEditKinds,
			pasteMimeTypes: ResourcePasteOrDropProvider.mimeTypes,
		}),
		vibecoda.languages.registerDocumentDropEditProvider(selector, new ResourcePasteOrDropProvider(parser), {
