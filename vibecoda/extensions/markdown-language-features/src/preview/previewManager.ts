...
export interface DynamicPreviewSettings {
	...
	readonly previewColumn: vibecoda.ViewColumn;
...
class PreviewStore<T extends IManagedMarkdownPreview> extends Disposable {
	...
	public get(resource: vibecoda.Uri, previewSettings: DynamicPreviewSettings): T | undefined {
	...
	private _resolvePreviewColumn(previewSettings: DynamicPreviewSettings): vibecoda.ViewColumn | undefined {
		if (previewSettings.previewColumn === vibecoda.ViewColumn.Active) {
			return vibecoda.window.tabGroups.activeTabGroup.viewColumn;
		}

		if (previewSettings.previewColumn === vibecoda.ViewColumn.Beside) {
			return vibecoda.window.tabGroups.activeTabGroup.viewColumn + 1;
		}
...
export class MarkdownPreviewManager extends Disposable implements vibecoda.WebviewPanelSerializer, vibecoda.CustomTextEditorProvider {
	...
	public constructor(
		...
		this._register(vibecoda.window.registerWebviewPanelSerializer(DynamicMarkdownPreview.viewType, this));

		this._register(vibecoda.window.registerCustomEditorProvider(StaticMarkdownPreview.customEditorViewType, this, {
			webviewOptions: { enableFindWidget: true }
		}));

		this._register(vibecoda.window.onDidChangeActiveTextEditor(textEditor => {
	...
	public findPreview(resource: vibecoda.Uri): IManagedMarkdownPreview | undefined {
	...
	public openDocumentLink(linkText: string, fromResource: vibecoda.Uri) {
	...
	public async deserializeWebviewPanel(
		...
		try {
			const resource = vibecoda.Uri.parse(state.resource);
			...
			<body class="error-container">
				<p>${vibecoda.l10n.t("An unexpected error occurred while restoring the Markdown preview.")}</p>
			</body>
			</html>`;
		}
	}
	...
	private _createNewDynamicPreview(
		...
		const activeTextEditorURI = vibecoda.window.activeTextEditor?.document.uri;
		const scrollLine = (activeTextEditorURI?.toString() === resource.toString()) ? vibecoda.window.activeTextEditor?.visibleRanges[0].start.line : undefined;
