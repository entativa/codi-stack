...
import type { ActivationFunction } from 'vibecoda-notebook-renderer';
...
export const activate: ActivationFunction<void> = (ctx) => {
	const markdownIt: MarkdownIt = new MarkdownIt({
		...
		highlight: (str: string, lang?: string) => {
			if (lang) {
				return `<div class="vibecoda-code-block" data-vibecoda-code-block-lang="${markdownIt.utils.escapeHtml(lang)}">${markdownIt.utils.escapeHtml(str)}</div>`;
			}
