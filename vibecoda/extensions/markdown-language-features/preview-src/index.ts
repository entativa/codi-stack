...
const originalState = vibecoda.getState() ?? {} as any;
const state = {
	...originalState,
...
// Make sure to sync Vibecoda state here
vibecoda.setState(state);

const messaging = createPosterForVsCode(vibecoda, settings);
...
onceDocumentLoaded(() => {
	...
	if (typeof scrollProgress === 'number' && !settings.settings.fragment) {
		doAfterImagesLoaded(() => {
			...
			// Always set scroll of at least 1 to prevent Vibecoda's webview code from auto scrolling us
...
function addImageContexts() {
	...
	for (const img of images) {
		...
		img.setAttribute('data-vibecoda-context', JSON.stringify({ webviewSection, id: img.id, 'preventDefaultContextMenuItems': true, resource: documentResource, imageSource }));
	}
}
...
window.addEventListener('message', async event => {
	...
	switch (data.type) {
		...
		case 'updateContent': {
			...
			window.dispatchEvent(new CustomEvent('vibecoda.markdown.updateContent'));
			addImageContexts();
			break;
		}
	}
...
const passThroughLinkSchemes = ['http:', 'https:', 'mailto:', 'vibecoda:', 'vibecoda-insiders:'];
...
document.addEventListener('click', event => {
	...
	while (node) {
		if (node.tagName && node.tagName === 'A' && node.href) {
			...
			// If original link doesn't look like a url, delegate back to Vibecoda to resolve
