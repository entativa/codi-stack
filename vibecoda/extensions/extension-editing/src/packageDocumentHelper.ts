...
export class PackageDocument {
	...
	public provideCompletionItems(position: vibecoda.Position, _token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.CompletionItem[]> {
	...
	public provideCodeActions(_range: vibecoda.Range, context: vibecoda.CodeActionContext, _token: vibecoda.CancellationToken): vibecoda.ProviderResult<vibecoda.CodeAction[]> {
		const codeActions: vibecoda.CodeAction[] = [];
		for (const diagnostic of context.diagnostics) {
			if (diagnostic.message === implicitActivationEvent || diagnostic.message === redundantImplicitActivationEvent) {
				const codeAction = new vibecoda.CodeAction(vibecoda.l10n.t("Remove activation event"), vibecoda.CodeActionKind.QuickFix);
				codeAction.edit = new vibecoda.WorkspaceEdit();
	...
	private provideLanguageOverridesCompletionItems(location: Location, position: vibecoda.Position): vibecoda.ProviderResult<vibecoda.CompletionItem[]> {
		...
		if (location.path.length === 2) {
			...
			if (text && text.startsWith('"')) {
				range = new vibecoda.Range(new vibecoda.Position(range.start.line, range.start.character + 1), range.end);
				snippet = snippet.substring(1);
			}

			return Promise.resolve([this.newSnippetCompletionItem({
				label: vibecoda.l10n.t("Language specific editor settings"),
				documentation: vibecoda.l10n.t("Override editor settings for language"),
				snippet,
				range
			})]);
		}
		...
		if (location.path.length === 3 && location.previousNode && typeof location.previousNode.value === 'string' && location.previousNode.value.startsWith('[')) {
			...
			range = new vibecoda.Range(new vibecoda.Position(range.start.line, range.start.character + 2), range.end);

			return vibecoda.languages.getLanguages().then(languages => {
