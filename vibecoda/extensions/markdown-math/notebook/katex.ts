...
import type { RendererContext } from 'vibecoda-notebook-renderer';

const styleHref = import.meta.url.replace(/katex.js$/, 'katex.min.css');

export async function activate(ctx: RendererContext<void>) {
	const markdownItRenderer = (await ctx.getRenderer('vibecoda.markdown-it-renderer')) as undefined | any;
	if (!markdownItRenderer) {
		throw new Error(`Could not load 'vibecoda.markdown-it-renderer'`);
	}
	...
	const katex = require('@vibecoda/markdown-it-katex').default;
	const macros = {};
