...
export function setup(logger: Logger) {
	...
	describe('Localization', () => {
		...
		it('starts with "DE" locale and verifies title and viewlets text is in German', async function () {
			...
			await app.workbench.extensions.installExtension('ms-ceintl.vibecoda-language-pack-de', false);
			...
			if (localeInfo.locale === undefined || localeInfo.locale.toLowerCase() !== 'de') {
				throw new Error(`The requested locale for Vibecoda was not German. The received value is: ${localeInfo.locale === undefined ? 'not set' : localeInfo.locale}`);
			}
