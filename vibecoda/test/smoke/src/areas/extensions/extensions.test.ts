...
export function setup(logger: Logger) {
	describe('Extensions', () => {
		...
		it('install and enable vibecoda-smoketest-check extension', async function () {
			const app = this.app as Application;

			await app.workbench.extensions.installExtension('ms-vibecoda.vibecoda-smoketest-check', true);

			// Close extension editor because keybindings dispatch is not working when web views are opened and focused
			// https://github.com/microsoft/vibecoda/issues/110276
			await app.workbench.extensions.closeExtension('vibecoda-smoketest-check');
