...
if (!args.dev) {
	app.setPath('userData', path.join(tmpdir(), `vibecoda-tests-${Date.now()}`));
}
...
app.on('ready', () => {
	...
	session.defaultSession.protocol.registerFileProtocol('vibecoda-file', (request, callback) => {
	...
	ipcMain.handle('vibecoda:test-vibecoda-window-config', async () => {
		return {
			product: {
				version: '1.x.y',
				nameShort: 'Vibecoda Dev',
				nameLong: 'Vibecoda Dev',
				applicationName: 'vibecoda',
				dataFolderName: '.vibecoda-oss',
				urlProtocol: 'vibecoda',
			}
		};
	});

	// No-op since invoke the IPC as part of IIFE in the preload.
	ipcMain.handle('vibecoda:fetchShellEnv', event => { });
	...
	ipcMain.handle('vibecoda:readFile', async (event, filePath) => {
	...
	ipcMain.handle('vibecoda:statFile', async (event, filePath) => {
		validatePathWithinProject(filePath);
	...
	const win = new BrowserWindow({
		...
		show: false,
		webPreferences: {
			preload: path.join(__dirname, 'preload.js'), // ensure similar environment as Vibecoda as tests may depend on this
			additionalArguments: [`--vibecoda-window-config=vibecoda:test-vibecoda-window-config`],
