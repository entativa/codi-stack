...
async function loadTests(opts) {
	...
	const _allowedTestsWithOutput = new Set([
		...
		'issue #149412: Vibecoda hangs when bad semantic token data is received', // https://github.com/microsoft/vibecoda/issues/192440
		'issue #134973: invalid semantic tokens should be handled better', // https://github.com/microsoft/vibecoda/issues/192440
		'issue #148651: Vibecoda UI process can hang if a semantic token with negative values is returned by language service', // https://github.com/microsoft/vibecoda/issues/192440
		'issue #149130: vibecoda freezes because of Bracket Pair Colorization', // https://github.com/microsoft/vibecoda/issues/192440
		'property limits', // https://github.com/microsoft/vibecoda/issues/192443
		'Error events', // https://github.com/microsoft/vibecoda/issues/192443
...
const setTimeout0 = (() => {
	if (setTimeout0IsFaster) {
		...
		$globalThis.addEventListener('message', (e) => {
			if (e.data && e.data.vibecodaScheduleAsyncWork) {
				for (let i = 0, len = pending.length; i < len; i++) {
					const candidate = pending[i];
					if (candidate.id === e.data.vibecodaScheduleAsyncWork) {
						pending.splice(i, 1);
						candidate.callback();
						return;
					}
		...
		return (callback) => {
			...
			$globalThis.postMessage({ vibecodaScheduleAsyncWork: myId }, '*');
		};
	}
