# Vibecoda Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides Playwright browser automation capabilities for Vibecoda development and testing. The MCP server exposes Vibecoda's Playwright testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with browsers programmatically.
...
```
MCP: List Servers → vibecoda-playwright-mcp → Start Server
```
or open [mcp.json](../../.vibecoda/mcp.json) and start it from there.
...
Open the [mcp.json](../../.vibecoda/mcp.json) and modify the `args`:
...
│   ├── playwright.ts        # Vibecoda Playwright integration
...
- **Vibecoda Integration**: Uses Vibecoda's existing Playwright test infrastructure
...
### Server Won't Start
- Ensure Vibecoda has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the Vibecoda development infrastructure. When making changes:
