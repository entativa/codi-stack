...
export async function getServer(appService: ApplicationService): Promise<Server> {
	const server = new McpServer({
		name: 'Vibecoda Automation Server',
		version: '1.0.0',
		title: 'An MCP Server that can interact with a local build of Vibecoda. Used for verifying UI behavior.'
	}, { capabilities: { logging: {} } });

	server.tool(
		'vibecoda_automation_start',
		'Start Vibecoda Build',
		{
		...
		async ({ recordVideo }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: app ? `Vibecoda started successfully` : `Failed to start Vibecoda`
				}]
			};
		}
	);

	// Apply all Vibecoda automation tools using the modular structure
