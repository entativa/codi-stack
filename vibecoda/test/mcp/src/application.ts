...
import * as vibecodatest from '@vibecoda/test-electron';
import { createApp, retry } from './utils';
import { opts } from './options';

const rootPath = path.join(__dirname, '..', '..', '..');
const logsRootPath = path.join(rootPath, '.build', 'vibecoda-playwright-mcp', 'logs');
const crashesRootPath = path.join(rootPath, '.build', 'vibecoda-playwright-mcp', 'crashes');
const videoRootPath = path.join(rootPath, '.build', 'vibecoda-playwright-mcp', 'videos');
...
if (!opts.web) {
	...
	if (!fs.existsSync(electronPath || '')) {
		fail(`Cannot find Vibecoda at ${electronPath}. Please run Vibecoda once first (scripts/code.sh, scripts\\code.bat) and try again.`);
	}
...
logger.log(`Vibecoda product quality: ${quality}.`);
...
async function ensureStableCode(): Promise<void> {
	...
	if (!stableCodePath) {
		...
		logger.log(`Found Vibecoda v${version}, downloading previous Vibecoda version ${stableVersion}...`);

		let lastProgressMessage: string | undefined = undefined;
		let lastProgressReportedAt = 0;
		const stableCodeDestination = path.join(testDataPath, 's');
		const stableCodeExecutable = await retry(() => measureAndLog(() => vibecodatest.download({
		...
		if (process.platform === 'darwin') {
			// Vibecoda.app/Contents/MacOS/Electron
			stableCodePath = path.dirname(path.dirname(path.dirname(stableCodeExecutable)));
		} else {
			// Vibecoda/Code.exe (Windows) | Vibecoda/code (Linux)
	...
	if (!fs.existsSync(stableCodePath)) {
		throw new Error(`Cannot find Stable Vibecoda at ${stableCodePath}.`);
	}
...
export async function getApplication({ recordVideo }: { recordVideo?: boolean } = {}) {
	...
	if (!fs.existsSync(electronPath || '')) {
		throw new Error(`Cannot find Vibecoda at ${electronPath}. Please run Vibecoda once first (scripts/code.sh, scripts\\code.bat) and try again.`);
	}
	process.env.VSCODE_REPOSITORY = rootPath;
	process.env.VSCODE_DEV = '1';
	process.env.VSCODE_CLI = '1';
