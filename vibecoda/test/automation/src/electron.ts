...
export async function resolveElectronConfiguration(options: LaunchOptions): Promise<IElectronConfiguration> {
	...
	const args = [
		...
		'--no-cached-data',
		'--disable-updates',
		'--disable-extension=vibecoda.vibecoda-api-tests',
	...
	if (remote) {
		...
		args[0] = `--${workspacePath.endsWith('.code-workspace') ? 'file' : 'folder'}-uri=vibecoda-remote://test+test/${URI.file(workspacePath).path}`;
		...
		if (codePath) {
			...
			await measureAndLog(() => copyExtension(root, extensionsPath, 'vibecoda-test-resolver'), 'copyExtension(vibecoda-test-resolver)', logger);
		}
		args.push('--enable-proposed-api=vibecoda.vibecoda-test-resolver');
		if (userDataDir) {
